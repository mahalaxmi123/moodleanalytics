...
function show_rpl($type, $user, $rpl, $describe, $fulldescribe, $cmid = null) {
    ...
    if ($edituser == $user->id) {
        ...
        print '<a href="index.php?course='.$course->id.'&sort='.$sort.'&start='.$start.'&edituser='.$user->id.'#user-'.$user->id.'" class="rpledit"><img src="'.$OUTPUT->pix_url('i/'.$rplicon, 'corplms_core').
...
if ($csv) {
    ...
    $args = array(
        'args' => json_encode(array(
            'course'      => $course->id,
            'pix_rply'    => $OUTPUT->pix_url('i/completion-rpl-y', 'corplms_core')->out(),
            'pix_rpln'    => $OUTPUT->pix_url('i/completion-rpl-n', 'corplms_core')->out(),
            'pix_cross'   => $OUTPUT->pix_url('i/invalid')->out(),
            'pix_loading' => $OUTPUT->pix_url('loading_small', 'corplms_core')->out(),
        ))
    );

    $jsmodule = array(
        'name'      => 'corplms_completionrpl',
        'fullpath'  => '/report/completion/rpl.js',
        'required'  => array('json'));

    $PAGE->requires->js_init_call('M.corplms_completionrpl.init',
             $args, false, $jsmodule);
