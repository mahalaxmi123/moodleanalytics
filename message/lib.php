...
function message_search($searchterms, $fromme=true, $tome=true, $courseid='none', $userid=0) {
    ...
    // exclude all corplms messages
    $corplmsmessagewhere = " AND (m.id IS NULL OR NOT EXISTS (SELECT md.messageid FROM {message_metadata} md WHERE md.messageid = m.id))";
    $corplmsmessagereadwhere = " AND (m.id IS NULL OR NOT EXISTS (SELECT messagereadid FROM {message_metadata} md WHERE md.messagereadid = m.id))";
    ...
    if ($courseid == SITEID) { // Admin is searching all messages.
        ...
        $m_read   = $DB->get_records_sql("SELECT m.id, m.useridto, m.useridfrom, m.smallmessage, m.fullmessage, m.timecreated
                                            FROM {message_read} m
                                           WHERE $searchcond $corplmsmessagereadwhere", $params, 0, MESSAGE_SEARCH_MAX_RESULTS);
        $m_unread = $DB->get_records_sql("SELECT m.id, m.useridto, m.useridfrom, m.smallmessage, m.fullmessage, m.timecreated
                                            FROM {message} m
                                           WHERE $searchcond $corplmsmessagewhere", $params, 0, MESSAGE_SEARCH_MAX_RESULTS);
...
function message_get_history($user1, $user2, $limitnum=0, $viewingnewmessages=false) {
    ...
    // never show corplms tasks or alerts
    $corplmsmessagewhere = " AND (m.id IS NULL OR NOT EXISTS (SELECT md.messageid FROM {message_metadata} md WHERE md.messageid = m.id))";
    $corplmsmessagereadwhere = " AND (m.id IS NULL OR NOT EXISTS (SELECT messagereadid FROM {message_metadata} md WHERE md.messagereadid = m.id))";
    if ($messages_read = $DB->get_records_sql("SELECT * FROM {message_read} m
                                               ...
                                               $notificationswhere $ownnotificationwhere $corplmsmessagereadwhere
    ...
    if ($messages_new =  $DB->get_records_sql("SELECT * FROM {message} m
                                               ...
                                               $ownnotificationwhere $corplmsmessagewhere
                                              ORDER BY m.timecreated $sort",
                                               array($user1->id, $user2->id, $user2->id, $user1->id, $user1->id),
                                               0, $limitnum)) {
