...
class message_output_email extends message_output {
    ...
    function send_message($eventdata) {
        ...
        // corplms specific extension to allow system to send alert/task but prevent email
        // even when users have it enabled
        require_once($CFG->dirroot.'/corplms/message/messagelib.php');
        if (isset($eventdata->sendemail) && $eventdata->sendemail !== CORPLMS_MSG_EMAIL_YES) {
            return true;
        }
        ...
        $footerplain = $strmgr->get_string('alertfooter2', 'corplms_message', $CFG->wwwroot . '/message/edit.php', $eventdata->userto->lang);
        $footerhtml = str_repeat(html_writer::empty_tag('br'), 2) . html_writer::empty_tag('hr') . $strmgr->get_string('alertfooter2html', 'corplms_message', $CFG->wwwroot . '/message/edit.php', $eventdata->userto->lang);
