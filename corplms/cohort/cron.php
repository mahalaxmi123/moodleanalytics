...
function tcohort_cron() {
    ...
    $lasthourlycron = get_config('corplms_cohort', 'lasthourlycron');
    ...
    if ($runhourlycron) {
        ...
        corplms_cohort_check_and_update_dynamic_cohort_members(null, $trace);
    }

    $trace->output(date("H:i:s", time()).' Sending queued cohort notifications...');
    corplms_cohort_send_queued_notifications();
