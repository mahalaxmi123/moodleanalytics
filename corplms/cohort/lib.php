...
require_once($CFG->dirroot . '/corplms/program/program_assignments.class.php');
...
global $COHORT_ALERT;
$COHORT_ALERT = array(
    COHORT_ALERT_NONE => get_string('alertmembersnone', 'corplms_cohort'),
    COHORT_ALERT_AFFECTED => get_string('alertmembersaffected', 'corplms_cohort'),
    COHORT_ALERT_ALL => get_string('alertmembersall', 'corplms_cohort'),
);
...
$COHORT_VISIBILITY = array(
    COHORT_VISIBLE_NOUSERS => get_string('visiblenousers', 'corplms_cohort'),
    COHORT_VISIBLE_ENROLLED => get_string('visibleenrolled', 'corplms_cohort'),
    COHORT_VISIBLE_AUDIENCE => get_string('visibleaudience', 'corplms_cohort'),
    COHORT_VISIBLE_ALL => get_string('visibleall', 'corplms_cohort'),
);

/**
 * This function is called automatically when the corplms cohort module is installed.
 *
 * @return bool Success
 */
function corplms_cohort_install() {
    global $CFG, $DB;
    return true;
}

/**
 * Run the corplms_cohort cron
 */
function corplms_cohort_cron() {
    global $CFG;
    require_once($CFG->dirroot . '/corplms/cohort/cron.php');
    tcohort_cron();
}
 ...
 */
function corplms_cohort_update_audience_visibility($type, $instanceid, $visiblevalue) {
    global $DB, $CFG, $COHORT_VISIBILITY;

    $result = false;

    // Capabilities to update programs or certifications.
    $progcap = 'corplms/program:configuredetails';
    $certcap = 'corplms/certification:configuredetails';
 ...
 * @return boolean
 */
function corplms_cohort_add_association($cohortid, $instanceid, $instancetype, $value=COHORT_ASSN_VALUE_ENROLLED) {
        ...
        switch ($instancetype) {
            case COHORT_ASSN_ITEMTYPE_COURSE:
                $courseassociations = array_map(
                    ...
                    corplms_cohort_get_associations($cohortid, COHORT_ASSN_ITEMTYPE_COURSE)
                );
            ...
            case COHORT_ASSN_ITEMTYPE_PROGRAM:
                ...
                $progassociations = array_map(
                    ...
                    corplms_cohort_get_associations($cohortid, $itemtype)
                );
 ...
 * @param $value COHORT_ASSN_VALUE_ENROLLED, COHORT_ASSN_VALUE_VISIBLE
 */
function corplms_cohort_delete_association($cohortid, $assid, $instancetype, $value=COHORT_ASSN_VALUE_ENROLLED) {
 ...
 */
function corplms_cohort_get_associations($cohortid, $instancetype=null, $value=null) {
    global $DB;
...
class cohort {
    ...
    public static function getCohortTypes() {
        return array(
            self::TYPE_STATIC => get_string('set', 'corplms_cohort'),
            self::TYPE_DYNAMIC => get_string('dynamic', 'corplms_cohort')
        );
    }
    ...
    public static function display_goal_table($cohort, $can_edit) {
        ...
        foreach ($assignments as $assignment) {
            $nameurl = new moodle_url('/corplms/hierarchy/item/view.php', array('prefix' => 'goal', 'id' => $assignment->goalid));
            ...
            if ($can_edit) {
                ...
                $url = new moodle_url('/corplms/hierarchy/prefix/goal/assign/remove.php', $params);
...
/******************************************************************************
 * Cohort event handlers, called from /corplms/cohort/db/events.php
 ******************************************************************************/
class corplmscohort_event_handler {
    /**
     * Event handler for when a user custom profiler field is deleted.
     *
     * @param \corplms_customfield\event\profilefield_deleted $event
     * @return boolean
     */
    public static function profilefield_deleted(\corplms_customfield\event\profilefield_deleted $event) {
 ...
 */
function corplms_cohort_update_operator($cohortid, $id, $type, $operatorvalue) {
    global $DB, $USER;
 ...
 * @return string
 */
function corplms_cohort_get_dynamic_cohort_whereclause($cohortid) {
 ...
 */
function corplms_cohort_update_dynamic_cohort_members($cohortid, $userid=0, $delaymessages=false, $updatenested=true) {
    global $CFG, $DB;
    require_once($CFG->dirroot . '/corplms/cohort/rules/lib.php');

    /// update necessary nested cohorts first (if any)
    if ($updatenested) {
        $nestedcohorts = corplms_cohort_get_nested_dynamic_cohorts($cohortid, array(), true);
        foreach ($nestedcohorts as $ncohortid) {
            corplms_cohort_update_dynamic_cohort_members($ncohortid, $userid, $delaymessages, false);
        }
    }
    ...
    $whereclause = corplms_cohort_get_dynamic_cohort_whereclause($cohortid);
    ...
    $numadd = 0;
    $numdel = 0;
    $currentcohortroles = corplms_get_cohort_roles($cohortid); // Current roles assigned to this cohort.
    foreach ($changedmembers as $mem) {
        ...
        if (!empty($newmembers)) {
            ...
            corplms_set_role_assignments_cohort($currentcohortroles, $cohortid, array_keys($newmembers));
            corplms_cohort_notify_add_users($cohortid, array_keys($newmembers), $delaymessages);
        ...
        if (!empty($delmembers)) {
            ...
            corplms_unset_role_assignments_cohort($currentcohortroles, $cohortid, $delids);
            corplms_cohort_notify_del_users($cohortid, $delids, $delaymessages);
            $numdel += count($delids);
            unset($delids);
        }
 ...
 */
function corplms_cohort_get_nested_dynamic_cohorts($cohortid, $current = array(), $activeonly = false) {
    global $DB;
    ...
    foreach ($cohorts as $ncohortid) {
        $current = array_merge($current, corplms_cohort_get_nested_dynamic_cohorts($ncohortid, $current, $activeonly));
    }
 ...
 */
function corplms_cohort_send_queued_notifications(){
    global $CFG, $DB;
    ...
    foreach ($batchlist as $batch) {
        ...
        corplms_cohort_notify_users($batch->cohortid, array_keys($msglist), $batch->action);
 ...
 * @return void
 */
function corplms_cohort_check_and_update_dynamic_cohort_members($courseid, progress_trace $trace) {
    global $DB;

    $trace->output('removing user memberships of deleted users...');
    corplms_cohort_clean_deleted_users();
    ...
    if (empty($courseid)) {
        ...
        $dcohorts = corplms_cohort_get_course_cohorts($courseid, cohort::TYPE_DYNAMIC);
    }

    // Looking for cohorts with broken rules.
    $cohortwithbrokenrules = corplms_cohort_broken_rules($courseid, null, $trace);
    ...
    if (!empty($cohortwithbrokenrules)) {
        ...
        foreach ($cohortwithbrokenrules as $brokencohort) {
            if ($brokencohort->broken == COHORT_BROKEN_RULE_NONE) { // Cohort has not been marked as broken.
                ...
                if (corplms_send_notification_broken_rule($siteadmins, $brokencohort)) {
        ...
        corplms_update_broken_field(array_keys($notifiedcohorts), COHORT_BROKEN_RULE_NOTIFIED);
        // Update broken field for cohorts that could have not been notified.
        $notnotified = corplms_search_for_value($cohortwithbrokenrules, 'broken', CORPLMS_SEARCH_OP_NOT_EQUAL,
                        COHORT_BROKEN_RULE_NOTIFIED);
        $notnotified = array_diff(array_keys($notnotified), array_keys($notifiedcohorts));
        corplms_update_broken_field($notnotified, COHORT_BROKEN_RULE_NOT_NOTIFIED);
    }

    // Update broken field for active cohorts.
    $brokencohorts = corplms_search_for_value($dcohorts, 'broken', CORPLMS_SEARCH_OP_NOT_EQUAL, COHORT_BROKEN_RULE_NONE);
    corplms_update_broken_field(array_keys($brokencohorts), COHORT_BROKEN_RULE_NONE);

    foreach ($dcohorts as $cohort) {
        $active = corplms_cohort_is_active($cohort);
        if (!$active) {
        ...
        try {
            ...
            $result = corplms_cohort_update_dynamic_cohort_members($cohort->id);
    ...
    // Set lastrun time to be checked by corplms_cohort cron - if $courseid is null we have arrived here from cron.
    if (empty($courseid)) {
        $timenow = time();
        if (!set_config('lasthourlycron', $timenow, 'corplms_cohort')) {
            $trace->output("Error: could not update lasthourlycron timestamp for corplms_cohort plugin.");
        }
    }
}


function corplms_cohort_clean_deleted_users() {
    global $DB;
 ...
 * @return string
 */
function corplms_cohort_date_where_clause($cohorttablealias = null, $now = null) {
    if ($now === null) {
        $now = time();
    }
 ...
 */
function corplms_cohort_delete_stale_memberships() {
    global $DB;
 ...
 * @param $cohortid
 */
function corplms_cohort_clone_cohort($oldcohortid) {
    global $CFG, $DB, $USER;
    ...
    $newcohort->name =              get_string('clonename', 'corplms_cohort', $oldcohort->name);
    $newcohort->idnumber =          corplms_cohort_next_automatic_id();
    ...
    $oldlearningitems = corplms_cohort_get_associations($oldcohortid);
    foreach ($oldlearningitems as $olditem) {
        corplms_cohort_add_association($newcohort->id, $olditem->instanceid, $olditem->type);
    }
...
function corplms_cohort_notify_add_users($cohortid, $adduserids, $delaymessages=false) {
    return corplms_cohort_notify_users($cohortid, $adduserids, 'membersadded', $delaymessages);
}

function corplms_cohort_notify_del_users($cohortid, $deluserids, $delaymessages=false) {
    return corplms_cohort_notify_users($cohortid, $deluserids, 'membersremoved', $delaymessages);
}

/**
 * Processor function to be passed in to {@link insert_records_via_batch()}. Used by
 * {@link corplms_cohort_notify_users()}.
 *
 ...
 */
function corplms_process_user_notifications($userid, $templateobject) {
 ...
 */
function corplms_cohort_notify_users($cohortid, $userids, $action, $delaymessages=false) {
    global $CFG, $DB, $USER;
    ...
    if ($delaymessages) {
        ...
        return $DB->insert_records_via_batch('cohort_msg_queue', $userids, 'corplms_process_user_notifications', array($msg));
    }
    ...
    foreach ($tousers as $touser) {
        ...
        $emailsubject = $strmgr->get_string("msg:{$action}_{$towho}_emailsubject", 'corplms_cohort', $a, $touser->lang);
        $notice = $strmgr->get_string("msg:{$action}_{$towho}_notice", 'corplms_cohort', $a, $touser->lang);
 ...
 * @return bool
 */
function corplms_cohort_is_active($cohort, $now = null){
    if ($now === null) {
        $now = time();
    }
 ...
 * NOTE: After using this function, make sure to call corplms_cohort_increment_automatic_id
 * to "mark off" the id number you used.
 * @return str
 */
function corplms_cohort_next_automatic_id() {
    global $CFG, $DB;
...
/**
 * Increment the automatic id number counter from corplms_cohort_next_automatic_id()
 * @param $idnumberused
 */
function corplms_cohort_increment_automatic_id($idnumberused) {
    global $CFG;
 ...
 */
function corplms_cohort_navlinks($cohortid=false, $cohortname=false, $subpagetitle=false) {
    global $CFG, $PAGE;
 ...
 * @param $programid
 */
function corplms_cohort_program_completion_link($cohortid, $programid){
    global $DB;
 ...
 */
function corplms_cohort_get_course_cohorts($courseid, $type=null, $fields='c.*') {
    global $DB;
 ...
 */
function corplms_cohort_get_visible_learning($instanceid, $instancetype = COHORT_ASSN_ITEMTYPE_COURSE) {
    global $DB;
 ...
 */
function corplms_cohort_broken_rules($courseid, $cohortid, progress_trace $trace) {
    global $DB, $CFG;
    ...
    if (empty($courseid)) {
        ...
        $dynamiccohorts = corplms_cohort_get_course_cohorts($courseid, cohort::TYPE_DYNAMIC);
        if (!empty($cohortid)) {
            // Get cohort.
            $dynamiccohorts = corplms_search_for_value($dynamiccohorts, 'id', CORPLMS_SEARCH_OP_EQUAL, $cohortid);
        }
    }
 ...
 * @return void
 */
function corplms_display_broken_rules_box($class = 'notifyproblem clearfix', $style='display:block') {
    $contents =  html_writer::tag('span', get_string('cohortbrokenrulesnotice', 'corplms_cohort'));
 ...
 * @return boolean $sent
 */
function corplms_send_notification_broken_rule($users, $cohort) {
    $sent = false;

    $subject = get_string('cohortbrokenrulesubject', 'corplms_cohort');
    $fullmessage = get_string('cohortbrokenrulesmessage', 'corplms_cohort');
    $fullmessage .= html_writer::start_tag('ul');
    $url = html_writer::link(new moodle_url('/corplms/cohort/rules.php', array('id' => $cohort->id)), $cohort->name);
    ...
    foreach ($users as $user) {
        ...
        $newevent->urgency     = CORPLMS_MSG_URGENCY_URGENT;
 ...
 * @return boolean.
 */
function corplms_update_broken_field($cohortids, $status) {
    global $DB;
 ...
 */
function corplms_get_members_cohort($cohortid) {
    global $DB;
 ...
 */
function corplms_get_cohort_roles($cohortid) {
    global $DB;
 ...
 */
function corplms_delete_roles_cohort($roleids, $cohortid) {
    global $DB;
    $success = true;
...
/**
 * Processor function to be passed in to {@link insert_records_via_batch()}. Used by
 * {@link corplms_create_roles_cohort()}.
 *
 ...
 */
function corplms_process_role_cohort($role, $templateobject) {
 ...
 * @return bool True success, false otherwise
 */
function corplms_create_roles_cohort($roleids, $cohortid) {
    global $DB, $USER;
    ...
    return $DB->insert_records_via_batch('cohort_role', $roleids, 'corplms_process_role_cohort', array($cohortrole));
}
 ...
 * @return void
 */
function corplms_unset_role_assignments_cohort($roles, $cohortid, $members) {
    foreach ($roles as $role) {
        $params = array(
            'roleid' => $role->roleid,
            'userids' => $members,
            'contextid' => $role->contextid,
            'component' => 'corplms_cohort',
            'itemid' => $cohortid
        );
 ...
 * @return void
 */
function corplms_set_role_assignments_cohort($roles, $cohortid, $members) {
    foreach ($roles as $role) {
        role_assign_bulk($role->roleid, $members, $role->contextid, 'corplms_cohort', $cohortid);
    }
}
 ...
 * @return bool
 */
function corplms_assign_roles_cohort($roles, $cohortid, $members) {
    global $DB;
    ...
    try {
        ...
        if (!empty($members)) {
            corplms_set_role_assignments_cohort($roles, $cohortid, $members);
        }
        corplms_create_roles_cohort($roles, $cohortid);
 ...
 * @return bool
 */
function corplms_unassign_roles_cohort($roles, $cohortid, $members) {
    global $DB;
    ...
    try {
        ...
        if (!empty($members)) {
            corplms_unset_role_assignments_cohort($roles, $cohortid, $members);
        }
        corplms_delete_roles_cohort($roles, $cohortid);
 ...
 */
function corplms_cohort_process_assigned_roles($cohortid, $roles) {
    $success = true;
    ...
    $memberids = array();
    if ($members = corplms_get_members_cohort($cohortid)) {
        $memberids = array_keys($members);
    }

    // Current roles assigned to this cohort.
    $currentroles = corplms_get_cohort_roles($cohortid);

    // Unassign roles.
    if (!empty($currentroles)) {
        $rolestounassign = array_diff_key($currentroles, $selectedroles);
        $success = corplms_unassign_roles_cohort($rolestounassign, $cohortid, $memberids);
    }

    // Assign roles.
    $rolestoassign = array_diff_key($selectedroles, $currentroles);
    if (!empty($rolestoassign)) {
        $success = $success && corplms_assign_roles_cohort($rolestoassign, $cohortid, $memberids);
    }
...
function check_access_audience_visibility($type, $instance, $userid = null) {
    ...
    list($visibilitysql, $visibilityparams) = corplms_visibility_where($userid,
                                                                      $fieldid,
                                                                      $fieldvis,
                                                                      $fieldaudvis,
                                                                      $alias,
...
class corplms_cohort_visible_learning_cohorts extends corplms_cohort_course_cohorts {
    function build_visible_learning_table($instanceid, $instancetype, $readonly = false) {
        $this->headers = array(
            get_string('cohortname', 'corplms_cohort'),
            get_string('type', 'corplms_cohort'),
            get_string('numlearners', 'corplms_cohort')
        );

        $items = corplms_cohort_get_visible_learning($instanceid, $instancetype);
...
class corplms_cohort_course_cohorts
{
    function build_table($courseid) {
        ...
        $items = corplms_cohort_get_course_cohorts($courseid, null,
