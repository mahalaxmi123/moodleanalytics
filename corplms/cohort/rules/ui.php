...
abstract class cohort_rule_ui {
    ...
    public function param_delete_action_icon($paramid) {
        ...
        return $OUTPUT->action_icon('#', new pix_icon('i/bullet_delete', get_string('deleteruleparam', 'corplms_cohort'), 'corplms_core', array('class' => 'ruleparam-delete', 'ruleparam-id' => $paramid)));
    }
}
...
abstract class cohort_rule_ui_form extends cohort_rule_ui {
    ...
    public function constructForm(){
        global $CFG;
        if ($this->form == null) {
            $this->form = new $this->formclass($CFG->wwwroot.'/corplms/cohort/rules/ruledetail.php');
    ...
    public function printDialogContent($hidden=array(), $ruleinstanceid=false) {
        ...
        echo $OUTPUT->heading(get_string('ruledialogdesc', 'corplms_cohort', $this->description), '2', 'cohort-rule-dialog-heading');
    ...
    protected function addFormFields(&$mform) {
        $mform->addElement('static', 'noconfig', '', get_string('ruleneedsnoconfiguration', 'corplms_cohort'));
    }
}
    ...
    protected function addFormFields(&$mform) {
        ...
        $mform->addGroupRule(
            'row1',
                array(
                    1 => array(
                        array(0 => get_string('error:mustpickonevalue', 'corplms_cohort'), 1 => 'callback', 2 => 'validate_emptyruleuiform', 3 => 'client')
                    )
                )
        );

        $error = get_string('error:mustpickonevalue', 'corplms_cohort');
...
JS;
    ...
    public function getRuleDescription($ruleid, $static=true) {
        ...
        if (!isset($this->equal) || !isset($this->listofvalues)) {
            return get_string('error:rulemissingparams', 'corplms_cohort');
        }
        ...
        return get_string('ruleformat-descjoinvars', 'corplms_cohort', $strvar);
    }
...
class cohort_rule_ui_menu extends cohort_rule_ui_form {
    ...
    protected function addFormFields(&$mform) {
        ...
        $row[0] = $mform->createElement(
            ...
            '',
            array(
                COHORT_RULES_OP_IN_EQUAL=>get_string('equalto','corplms_cohort'),
                COHORT_RULES_OP_IN_NOTEQUAL=>get_string('notequalto', 'corplms_cohort')
            )
        );
        ...
        $mform->addGroupRule(
            'row1',
                array(
                    0=>array(
                        array(0=>get_string('error:mustpickonevalue', 'corplms_cohort'), 1=>'callback',2=>'validate_menu', 3=>'client')
                    )
                )
        );
        $js = <<<JS
...
JS;
    ...
    public function getRuleDescription($ruleid, $static=true) {
        ...
        $strvar = new stdClass();
        $strvar->desc = $this->description;
        $strvar->join = get_string("is{$COHORT_RULES_OP_IN[$this->equal]}to", 'corplms_cohort');
...
class ruleuidateform extends emptyruleuiform {
    public function validation($data, $files){
        ...
        if (empty($data['fixedordynamic']) || !in_array($data['fixedordynamic'], array(1,2))) {
            $errors['beforeafterrow'] = get_string('error:baddateoption', 'corplms_cohort');
            $errors['durationrow'] = ' ';
        }
        ...
        if (
            ...
            $errors['beforeafterrow'] = get_string('error:baddate', 'corplms_cohort');
        }
        ...
        if (
            ...
            $errors['durationrow'] = get_string('error:badduration', 'corplms_cohort');
        }
...
class cohort_rule_ui_date extends cohort_rule_ui_form {
    ...
    protected function addFormData() {
        ...
        $formdata['beforeafterdate'] = get_string('datepickerlongyearplaceholder', 'corplms_core');
        if (isset($this->operator)) {
            if ($this->operator == COHORT_RULE_DATE_OP_AFTER_FIXED_DATE || $this->operator == COHORT_RULE_DATE_OP_BEFORE_FIXED_DATE) {
                ...
                if (!empty($this->date)) {
                    // todo: make this configurable!
                    $formdata['beforeafterdate'] = userdate($this->date, get_string('datepickerlongyearphpuserdate', 'corplms_core'), $CFG->timezone, false);
                }
            } else if (
                    in_array(
                        $this->operator,
    ...
    public function addFormFields(&$mform) {
        ...
        $row[1] = $mform->createElement(
            ...
            '',
            array(
                COHORT_RULE_DATE_OP_BEFORE_FIXED_DATE=>get_string('datemenufixeddatebefore', 'corplms_cohort'),
                COHORT_RULE_DATE_OP_AFTER_FIXED_DATE=>get_string('datemenufixeddateafter', 'corplms_cohort')
            )
        );
        ...
        $('#id_beforeafterdate').datepicker(
            {
                dateFormat: '
JS;
        $datepickerjs .= get_string('datepickerlongyeardisplayformat', 'corplms_core');
        $datepickerjs .= <<<JS
',
                showOn: 'both',
...
JS;
        ...
        $row[1] = $mform->createElement(
            ...
            '',
            array(
                COHORT_RULE_DATE_OP_BEFORE_PAST_DURATION =>   get_string('datemenudurationbeforepast', 'corplms_cohort'),
                COHORT_RULE_DATE_OP_WITHIN_PAST_DURATION =>   get_string('datemenudurationwithinpast', 'corplms_cohort'),
                COHORT_RULE_DATE_OP_WITHIN_FUTURE_DURATION => get_string('datemenudurationwithinfuture', 'corplms_cohort'),
                COHORT_RULE_DATE_OP_AFTER_FUTURE_DURATION =>  get_string('datemenudurationafterfuture', 'corplms_cohort'),
            )
        );
        $row[2] = $mform->createElement('text', 'durationdate', '');
        $row[3] = $mform->createElement('static', '', '', get_string('durationdays', 'corplms_cohort'));
    ...
    public function getRuleDescription($ruleid, $static=true) {
        ...
        switch ($this->operator) {
            ...
            case COHORT_RULE_DATE_OP_AFTER_FIXED_DATE:
                $a = userdate($this->date, get_string('datepickerlongyearphpuserdate', 'corplms_core'), $CFG->timezone, false);
                break;
        ...
        $strvar->vars = get_string("dateis{$COHORT_RULE_DATE_OP[$this->operator]}", 'corplms_cohort', $a);

        return get_string('ruleformat-descvars', 'corplms_cohort', $strvar);
    }
    ...
    public function handleDialogUpdate($sqlhandler){
        ...
        switch($fixedordynamic) {
            case 1:
                ...
                $date = corplms_date_parse_from_format(get_string('datepickerlongyearparseformat', 'corplms_core'), required_param('beforeafterdate', PARAM_TEXT));
                break;
            case 2:
...
class corplms_dialog_content_hierarchy_multi_cohortrule extends corplms_dialog_content_hierarchy_multi {
    ...
    */
    public function populate_selected_items_pane($elements) {
        ...
        $operatormenu = array();
        $operatormenu[1] = get_string('equalto', 'corplms_cohort');
        $operatormenu[0] = get_string('notequalto', 'corplms_cohort');
        ...
        $childmenu = array();
        $childmenu[0] = get_string('includechildrenno', 'corplms_cohort');
        $childmenu[1] = get_string('includechildrenyes', 'corplms_cohort');
...
//        $compid = required_param('id', PARAM_INT);
        ...
        $dialog = new corplms_dialog_content_hierarchy_multi_cohortrule($this->prefix, $frameworkid, $showhidden);
    ...
    public function getRuleDescription($ruleid, $static=true) {
        ...
        if ($this->includechildren) {
            $strvar->ext = get_string('orachildof', 'corplms_cohort');
        }
        ...
        if (!empty($strvar->ext)) {
            return get_string('ruleformat-descjoinextvars', 'corplms_cohort', $strvar);
        } else {
            return get_string('ruleformat-descjoinvars', 'corplms_cohort', $strvar);
        }
    }
...
require_once($CFG->dirroot.'/corplms/core/dialogs/dialog_content_courses.class.php');
class corplms_dialog_content_cohort_rules_courses extends corplms_dialog_content_courses {
...
abstract class cohort_rule_ui_picker_course_program extends cohort_rule_ui {
    ...
    public function printDialogContent($hidden=array(), $ruleinstanceid=false) {
        ...
        if ($this->pickertype == COHORT_PICKER_COURSE_COMPLETION) {
            ...
            require_once($CFG->dirroot.'/corplms/core/dialogs/dialog_content_programs.class.php');
        } else {
            echo get_string('error:typecompletion', 'corplms_cohort');
            return;
        }
        ...
        // Load dialog content generator.
        if ($this->pickertype == COHORT_PICKER_COURSE_COMPLETION) {
            $dialog = new corplms_dialog_content_cohort_rules_courses($categoryid);
        } else {
            $dialog = new corplms_dialog_content_cohort_rules_programs($categoryid);
        }
...
class cohort_rule_ui_picker_course_allanynotallnone extends cohort_rule_ui_picker_course_program {
    ...
    public function getExtraSelectedItemsPaneWidgets(){
        $operatormenu = array();
        $operatormenu[COHORT_RULE_COMPLETION_OP_ALL] = get_string('completionmenuall', 'corplms_cohort');
        $operatormenu[COHORT_RULE_COMPLETION_OP_ANY] = get_string('completionmenuany', 'corplms_cohort');
        $operatormenu[COHORT_RULE_COMPLETION_OP_NOTALL] = get_string('completionmenunotall', 'corplms_cohort');
        $operatormenu[COHORT_RULE_COMPLETION_OP_NONE] = get_string('completionmenunotany', 'corplms_cohort');
    ...
    public function getRuleDescription($ruleid, $static=true) {
        ...
        switch ($this->operator) {
            case COHORT_RULE_COMPLETION_OP_ALL:
                $strvar->desc = get_string('ccdescall', 'corplms_cohort');
                break;
            case COHORT_RULE_COMPLETION_OP_ANY:
                $strvar->desc = get_string('ccdescany', 'corplms_cohort');
                break;
            case COHORT_RULE_COMPLETION_OP_NOTALL:
                $strvar->desc = get_string('ccdescnotall', 'corplms_cohort');
                break;
            default:
                $strvar->desc = get_string('ccdescnotany', 'corplms_cohort');
        }
...
class cohort_rule_ui_picker_course_duration extends cohort_rule_ui_picker_course_program {
    ...
    public function getExtraSelectedItemsPaneWidgets(){
        ...
        $operatormenu = array();
        $operatormenu[COHORT_RULE_COMPLETION_OP_DATE_LESSTHAN] = get_string('completiondurationmenulessthan', 'corplms_cohort');
        $operatormenu[COHORT_RULE_COMPLETION_OP_DATE_GREATERTHAN] = get_string('completiondurationmenumorethan', 'corplms_cohort');
        $selected = isset($this->operator) ? $this->operator : '';
        ...
        $html .= '" /> ' . get_string('completiondurationdays', 'corplms_cohort');
        $html .= '</fieldset>';
        $html .= '</div>';
        $validnumberstr = get_string('error:badduration', 'corplms_cohort');
        $html .= <<<JS
...
JS;
    ...
    public function getRuleDescription($ruleid, $static=true) {
        ...
        $strvar = new stdClass();
        $strvar->desc = get_string($descstr, 'corplms_cohort', $this->date);
...
class cohort_rule_ui_picker_course_program_date extends cohort_rule_ui_picker_course_program {
    ...
    public function getExtraSelectedItemsPaneWidgets(){
        ...
        $opmenufix[COHORT_RULE_COMPLETION_OP_DATE_LESSTHAN] = get_string('datemenufixeddatebefore', 'corplms_cohort');
        $opmenufix[COHORT_RULE_COMPLETION_OP_DATE_GREATERTHAN] = get_string('datemenufixeddateafter', 'corplms_cohort');

        $opmenurel[COHORT_RULE_COMPLETION_OP_BEFORE_PAST_DURATION] = get_string('datemenudurationbeforepast', 'corplms_cohort');
        $opmenurel[COHORT_RULE_COMPLETION_OP_WITHIN_PAST_DURATION] = get_string('datemenudurationwithinpast', 'corplms_cohort');
        $opmenurel[COHORT_RULE_COMPLETION_OP_WITHIN_FUTURE_DURATION] = get_string('datemenudurationwithinfuture', 'corplms_cohort');
        $opmenurel[COHORT_RULE_COMPLETION_OP_AFTER_FUTURE_DURATION] = get_string('datemenudurationafterfuture', 'corplms_cohort');

        // Set default values.
        $selected = isset($this->operator) ? $this->operator : '';
        $htmldate = get_string('datepickerlongyearplaceholder', 'corplms_core');
        ...
        if (isset($this->operator) && array_key_exists($this->operator, $opmenufix)) {
            ...
            $htmldate = userdate($this->date, get_string('datepickerlongyearphpuserdate', 'corplms_core'), $CFG->timezone, false);
        ...
        // Fixed date.
        $html .= get_string('completionusercompletedbeforeafter', 'corplms_cohort');
        ...
        // Relative date.
        $html .= get_string('or', 'corplms_cohort');
        $html .= html_writer::start_tag('fieldset');
    ...
    public function handleDialogUpdate($sqlhandler){
        ...
        switch($fixedordynamic) {
            case 1:
                $operator = required_param('beforeaftermenu', PARAM_INT);
                $date = corplms_date_parse_from_format(get_string('datepickerlongyearparseformat', 'corplms_core'),
    ...
    public function getRuleDescription($ruleid, $static=true) {
        ...
        $strvar->join = get_string("dateis{$COHORT_RULE_COMPLETION_OP[$this->operator]}", 'corplms_cohort', $a);
...
require_once($CFG->dirroot.'/corplms/core/dialogs/dialog_content_programs.class.php');
class corplms_dialog_content_cohort_rules_programs extends corplms_dialog_content_programs {
...
class cohort_rule_ui_picker_program_allanynotallnone extends cohort_rule_ui_picker_course_program {
    ...
    public function getRuleDescription($ruleid, $static=true) {
        ...
        $strvar = new stdClass();
        $strvar->desc = get_string($getstr, 'corplms_cohort');
...
class cohort_rule_ui_picker_program_duration extends cohort_rule_ui_picker_course_program {
    ...
    public function getExtraSelectedItemsPaneWidgets(){
        ...
        $html .= '</fieldset>';
        $html .= '</div>';
        $badduration = get_string('error:badduration', 'corplms_cohort');
        $html .= <<<JS
...
JS;
    ...
    public function getRuleDescription($ruleid, $static=true) {
        ...
        $strvar = new stdClass();
        $strvar->desc = get_string($getstr, 'corplms_cohort', $this->date);
...
require_once($CFG->dirroot . '/corplms/core/dialogs/dialog_content_manager.class.php');
class corplms_dialog_content_manager_cohortreportsto extends corplms_dialog_content_manager {
    /**
    * Returns markup to be used in the selected pane of a multi-select dialog
    *
    * @param   $elements    array elements to be created in the pane
    ...
    */
    public function populate_selected_items_pane($elements) {
        $operatormenu = array();
        $operatormenu[0] = get_string('reportsto', 'corplms_cohort');
        $operatormenu[1] = get_string('reportsdirectlyto', 'corplms_cohort');
...
class cohort_rule_ui_reportsto extends cohort_rule_ui {
    ...
    public function printDialogContent($hidden=array(), $ruleinstanceid=false) {
        ...
        $dialog = new corplms_dialog_content_manager_cohortreportsto();
    ...
    public function getRuleDescription($ruleid, $static=true) {
        ...
        if ($this->isdirectreport) {
            $strvar->desc = get_string('userreportsdirectlyto', 'corplms_cohort');
        } else {
            $strvar->desc = get_string('userreportsto', 'corplms_cohort');
        }
...
require_once($CFG->dirroot . '/corplms/core/dialogs/dialog_content_manager.class.php');
class corplms_dialog_content_manager_cohortmember extends corplms_dialog_content_manager {
    /**
    * Returns markup to be used in the selected pane of a multi-select dialog
    *
    * @param   $elements    array elements to be created in the pane
    ...
    */
    public function populate_selected_items_pane($elements) {
        ...
        $operatormenu = array();
        $operatormenu[1] = get_string('incohort', 'corplms_cohort');
        $operatormenu[0] = get_string('notincohort', 'corplms_cohort');
...
class cohort_rule_ui_cohortmember extends cohort_rule_ui {
    ...
    public function printDialogContent($hidden=array(), $ruleinstanceid=false) {
        ...
        // Set up dialog
        $dialog = new corplms_dialog_content_manager_cohortmember();
        $dialog->type = corplms_dialog_content::TYPE_CHOICE_MULTI;
        $dialog->items = $items;
    ...
    public function getRuleDescription($ruleid, $static=true) {
        ...
        $strvar = new stdClass();
        if ($this->incohort) {
            $strvar->desc = get_string('useriscohortmember', 'corplms_cohort');
        } else {
            $strvar->desc = get_string('userisnotcohortmember', 'corplms_cohort');
        }
