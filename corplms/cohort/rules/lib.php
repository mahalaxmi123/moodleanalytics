...
require_once($CFG->dirroot.'/corplms/cohort/rules/settings.php');
...
$COHORT_RULES_OP_IN_LIST = array(
    COHORT_RULES_OP_IN_CONTAINS   => get_string('contains', 'corplms_cohort'),
    COHORT_RULES_OP_IN_NOTCONTAIN => get_string('doesnotcontain', 'corplms_cohort'),
    COHORT_RULES_OP_IN_ISEQUALTO  => get_string('isequalto', 'corplms_cohort'),
    COHORT_RULES_OP_IN_STARTSWITH => get_string('startswith', 'corplms_cohort'),
    COHORT_RULES_OP_IN_ENDSWITH   => get_string('endswith', 'corplms_cohort'),
    COHORT_RULES_OP_IN_ISEMPTY    => get_string('isempty', 'corplms_cohort'),
    COHORT_RULES_OP_IN_NOTEQUALTO => get_string('isnotequalto', 'corplms_cohort'),
);
...
function cohort_rules_get_menu_options() {
    ...
    if (!$rulesmenu) {
        ...
        foreach ($rules as $groupname => $group) {
            $curlabel = get_string("rulegroup-{$groupname}", 'corplms_cohort');
        ...
        $rulesmenu = array_merge(
            array(
                get_string('new') => array(
                    '' => get_string('addrule', 'corplms_cohort')
                )
            ),
            $rulesmenu
        );
...
function cohort_rule_form_html($ruleid, $description, $rulegroup = '', $rulename = '', $first = false, $operator = '',
    ...
    $strdelete = get_string('deleterule', 'corplms_cohort');
    $stredit = get_string('editrule', 'corplms_cohort');
    ...
    if ($first) {
        ...
        switch ($operator) {
            case COHORT_RULES_OP_OR:
                $opstr = get_string('or', 'corplms_cohort');
                break;
            case COHORT_RULES_OP_AND:
                $opstr = get_string('and', 'corplms_cohort');
                break;
            default:
                $opstr = $operator;
        }
    ...
    if (!$brokenrule) {
        ...
        $url = new moodle_url($CFG->wwwroot . '/corplms/cohort/rules/ruledetail.php', array('type' => 'rule', 'id' => $ruleid));
...
function cohort_rules_approve_changes($cohort) {
    ...
    corplms_cohort_update_dynamic_cohort_members($cohort->id, 0, true);
