...
require_capability('corplms/cohort:managerules', $syscontext);
...
if (!preg_match('/^([a-z]+)-([a-z0-9_]+)$/', $rulegroupname, $matches)) {
    print_error('error:rulemissing', 'corplms_cohort');
} else {
    $rulegroup = $matches[1];
    $rulename = $matches[2];
    unset($matches);
...
if (!$rule) {
    print_error('error:badrule', 'corplms_cohort', '', "{$rulegroup}-{$rulename}");
}
...
if ($update) {
    ...
    if ($ui->validateResponse()) {
        ...
        switch($type) {
            case 'cohort':
                ...
                if (!$cohort = $DB->get_record('cohort', array('id' => $id))) {
                    print_error(get_string('error:badcohortid', 'corplms_cohort'));
                }
                if (!$cohort->cohorttype == cohort::TYPE_DYNAMIC) {
                    print_error(get_string('error:notdynamiccohort', 'corplms_cohort'));
                }

                $rulesetid = cohort_rule_create_ruleset($cohort->draftcollectionid);
                add_to_log(SITEID, 'cohort', 'create ruleset', 'corplms/cohort/rules.php?id='.$id, "rulesetid={$rulesetid}");
        ...
        add_to_log(SITEID, 'cohort', $logaction, 'corplms/cohort/rules.php?id='.$cohortid, $loginfo);
