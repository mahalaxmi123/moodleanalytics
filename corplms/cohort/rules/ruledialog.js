...
/**
 * This file defines the Corplms dialog for creating/editing a single rule for a dynamic cohort.
 */

M.corplms_cohortrules = M.corplms_cohortrules || {
    ...
    init: function(Y, args) {
        ...
        if (typeof $ === 'undefined') {
            throw new Error('M.corplms_cohortrules.init()-> jQuery dependency required for this module.');
        }

        this.corplms_cohortrules_init_dialogs();
    },

    corplms_cohortrules_init_dialogs: function() {
        // Dialog & handler for form-based UIs
        var url = M.cfg.wwwroot + '/corplms/cohort/rules/ruledetail.php';
        var saveurl = url;

        var fhandler = new corplmsDialog_handler_cohortruleform();
        fhandler.baseurl = url;

        var fbuttons = {};
        fbuttons[M.util.get_string('cancel','moodle')] = function() { fhandler._cancel() }
        fbuttons[M.util.get_string('save','corplms_core')] = function() { fhandler.submit() }
        var fdialog = new corplmsDialog(
            'cohortruleformdialog',
            'nobutton',
            {
                buttons: fbuttons,
                title: '<h2>' + M.util.get_string('addrule', 'corplms_cohort') + '</h2>'
            },
            url,
            fhandler
        );
        fdialog.cohort_base_url = url;
        corplmsDialogs['cohortruleform'] = fdialog;
        ...
        $(document).on('click', 'input:radio', function(event) {
             ...
             var cohortid = M.corplms_cohortrules.config.cohortid;
             var type = radioname;
             var id = '';

             if (radioname === 'cohortoperator') {
                 id = cohortid;
                 type = M.corplms_cohortrules.config.operator_type_cohort;
             } else if (radioname.substr(0, 15) === 'rulesetoperator') {
                 type = M.corplms_cohortrules.config.operator_type_ruleset;
             ...
             $.ajax({
                 type: "POST",
                 url: M.cfg.wwwroot + '/corplms/cohort/rules/updateoperator.php',
                 data: ({
                     id: id,
                     type: type,
                     value: opvalue,
                 ...
                 success: function(o) {
                     ...
                     if (o.length > 0) {
                         ...
                         if (o.action === 'updcohortop') {
                             operator = M.util.get_string('andcohort', 'corplms_cohort');
                             if (o.value !== 0) {
                                 operator = M.util.get_string('orcohort', 'corplms_cohort');
                             }
                             ...
                             operator = M.util.get_string('and', 'corplms_cohort');
                             if (o.value !== 0) {
                                 operator = M.util.get_string('or', 'corplms_cohort');
                             }
        ...
        function show_notifications(type, id, success) {
            ...
            var notification_message = M.util.get_string('rulesupdatesuccess', 'corplms_cohort');
            if (success) {
                // Show approve - cancel options
                $('div#cohort_rules_action_box').removeAttr("style");
            } else {
                notification_class = 'notifyproblem';
                notification_message = M.util.get_string('rulesupdatefailure', 'corplms_cohort');
            }
        ...
        var url = M.cfg.wwwroot + '/corplms/cohort/rules/ruledetail.php';
        var thandler = new corplmsDialog_handler_cohortruletreeview();
        var tbuttons = {};
        tbuttons[M.util.get_string('cancel','moodle')] = function() { thandler._cancel() }
        tbuttons[M.util.get_string('save','corplms_core')] = function() { thandler._save() }
        var tdialog = new corplmsDialog(
        ...
        corplmsDialogs['cohortruletreeview'] = tdialog;
        ...
        $(document).on('change', 'select.rule_selector', function(event) {
            ...
            var dialog = corplmsDialogs['cohortrule' + ruleHandlerMap[ruletype]];
...
var funccompletiondate =  function(element) {
    ...
    if (!element.val().match(M.util.get_string('datepickerlongyearregexjs', 'corplms_core'))){
        parent.addClass('error');
        if ($('#id_error_completiondate').length == 0) {
            parent.prepend('<span id="id_error_completiondate" class="error">' +
                M.util.get_string('error:baddate','corplms_cohort') +
                '</span>');
        }
        return false;
    } else {
    ...
    if (!element.val().match(/[1-9]+[0-9]*/)){
        ...
        if ( $('#id_error_completiondurationdate').length == 0 ) {
            parent.prepend('<span id="id_error_completiondurationdate" class="error">' +
                M.util.get_string('error:badduration','corplms_cohort') +
                '</span>');
        }
        return false;
    } else {
...
// As a corplmsDialog_handler_form, it means that the content of this dialog should contain an HTML
...
corplmsDialog_handler_cohortruleform = function() {
    // Base url
    var baseurl = '';
}

corplmsDialog_handler_cohortruleform.prototype = new corplmsDialog_handler_form();
 ...
 * @return  void
 */
corplmsDialog_handler_cohortruleform.prototype._updatePage = cohort_handler_responsefunc;

/**
 * Add custom submit handler to forms in dialog
 */
corplmsDialog_handler_cohortruleform.prototype.every_load = function() {
    var handler = this;
...
corplmsDialog_handler_cohortruletreeview = function() {};
corplmsDialog_handler_cohortruletreeview.prototype = new corplmsDialog_handler_treeview_multiselect();
 ...
 * @return void
 */
corplmsDialog_handler_cohortruletreeview.prototype._save = function() {
    // Serialize data
...
corplmsDialog_handler_cohortruletreeview.prototype._update = cohort_handler_responsefunc;
