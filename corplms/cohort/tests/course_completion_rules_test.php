 ...
 * vendor/bin/phpunit corplms_cohort_course_completion_rules_testcase
 *
 */
class corplms_cohort_course_completion_rules_testcase extends reportcache_advanced_testcase {
    ...
    */
    ...
    public function test_coursecompletion_date_rule($params, $courses, $usercount) {
        ...
        // Use corplms_date_parse_from_format to transform a date to int as we do when creating this type of rules.
        if ($params['operator'] === COHORT_RULE_COMPLETION_OP_DATE_LESSTHAN ||
            $params['operator'] === COHORT_RULE_COMPLETION_OP_DATE_GREATERTHAN ) {
            $format = get_string('datepickerlongyearparseformat', 'corplms_core');
            $time = time() + ($params['date'] * DAYSECS);
            $params['date'] = corplms_date_parse_from_format($format, date($format, $time));
        }
