...
require_once($CFG->dirroot . '/corplms/reportbuilder/tests/reportcache_advanced_testcase.php');
 ...
 * vendor/bin/phpunit corplms_cohort_certification_audiencevisibility_testcase
 *
 */
class corplms_cohort_certification_audiencevisibility_testcase extends reportcache_advanced_testcase {
    ...
    public function setUp() {
        ...
        // Set corplms_cohort generator.
        $this->cohort_generator = $this->getDataGenerator()->get_plugin_generator('corplms_cohort');

        // Set corplms_program generator.
        $this->program_generator = $this->getDataGenerator()->get_plugin_generator('corplms_program');
        ...
        assign_capability('corplms/coursecatalog:manageaudiencevisibility', CAP_ALLOW, $rolestaffmanager->id, $syscontext);
        unassign_capability('corplms/certification:viewhiddencertifications', $rolestaffmanager->id, $syscontext->id);

        $roletrainer = $DB->get_record('role', array('shortname'=>'teacher'));
        role_assign($roletrainer->id, $this->user9->id, $syscontext->id);
        assign_capability('corplms/certification:viewhiddencertifications', CAP_ALLOW, $roletrainer->id, $syscontext);
        ...
        corplms_cohort_add_association($this->audience1->id, $this->certif2->id, COHORT_ASSN_ITEMTYPE_CERTIF,
                                        COHORT_ASSN_VALUE_VISIBLE);
        corplms_cohort_add_association($this->audience2->id, $this->certif2->id, COHORT_ASSN_ITEMTYPE_CERTIF,
                                        COHORT_ASSN_VALUE_VISIBLE);

        // Assign audience2 to certif3 and certif4.
        corplms_cohort_add_association($this->audience2->id, $this->certif3->id, COHORT_ASSN_ITEMTYPE_CERTIF,
                                        COHORT_ASSN_VALUE_VISIBLE);
        corplms_cohort_add_association($this->audience2->id, $this->certif4->id, COHORT_ASSN_ITEMTYPE_CERTIF,
                                        COHORT_ASSN_VALUE_VISIBLE);
    ...
    public function test_audiencevisibility($user, $certificationsvisible, $certificationsnotvisible, $audvisibilityon) {
        ...
        for ($i = 1; $i <= 2; $i++) {
            ...
            $this->assertEquals(corplms_get_category_item_count($this->category->id, 'certification'), $countprogramsvisbile);
    ...
    protected function get_visible_info($user, $content, $certification) {
        ...
        if ($CFG->enhancedcatalog) { // New catalog.
            $search = array();
            if (is_array($content)) {
                $search = corplms_search_for_value($content, 'prog_progexpandlink', CORPLMS_SEARCH_OP_EQUAL,
    ...
    protected function create_certifications_old_visibility() {
        ...
        corplms_cohort_add_association($this->audience2->id, $this->certif6->id, COHORT_ASSN_ITEMTYPE_CERTIF,
                                        COHORT_ASSN_VALUE_VISIBLE);
        corplms_cohort_add_association($this->audience1->id, $this->certif5->id, COHORT_ASSN_ITEMTYPE_CERTIF,
                                        COHORT_ASSN_VALUE_VISIBLE);
    }
    ...
    protected function create_certifications_with_availability() {
        ...
        corplms_cohort_add_association($this->audience1->id, $this->certif7->id, COHORT_ASSN_ITEMTYPE_CERTIF,
                                        COHORT_ASSN_VALUE_VISIBLE);
        corplms_cohort_add_association($this->audience1->id, $this->certif8->id, COHORT_ASSN_ITEMTYPE_CERTIF,
                                        COHORT_ASSN_VALUE_VISIBLE);
        corplms_cohort_add_association($this->audience1->id, $this->certif9->id, COHORT_ASSN_ITEMTYPE_CERTIF,
                                        COHORT_ASSN_VALUE_VISIBLE);
    }
