 ...
 * vendor/bin/phpunit corplms_cohort_position_rules_testcase
 *
 */
class corplms_cohort_position_rules_testcase extends advanced_testcase {
    ...
    public function setUp() {
        ...
        $this->users = array();
        $this->dateformat = get_string('datepickerlongyearparseformat', 'corplms_core');

        // Set corplms_cohort generator.
        $this->cohort_generator = $this->getDataGenerator()->get_plugin_generator('corplms_cohort');

        // Set corplms_hierarchy generator.
        $this->hierarchy_generator = $this->getDataGenerator()->get_plugin_generator('corplms_hierarchy');
        ...
        for ($i = 1; $i <= self::TEST_POSITION_COUNT_MEMBERS; $i++) {
            ...
            $postimestarted = corplms_date_parse_from_format($this->dateformat, $postimestarted);
            $postimefinish = corplms_date_parse_from_format($this->dateformat, $postimefinish);
    ...
    public function test_position_date_rules($rulename, $params, $usercount, $sourcemembersmatched) {
        ...
        if ($params['operator'] === COHORT_RULE_DATE_OP_AFTER_FIXED_DATE ||
            ...
            $params['date'] = corplms_date_parse_from_format($this->dateformat, date($this->dateformat, $time));
        }
