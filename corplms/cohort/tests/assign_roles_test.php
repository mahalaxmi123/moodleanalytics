 ...
 * vendor/bin/phpunit corplms_cohort_assign_roles_testcase
 *
 */
class corplms_cohort_assign_roles_testcase extends advanced_testcase {
    ...
    public function test_assign_roles_to_cohort() {
        ...
        $this->assertTrue(corplms_cohort_process_assigned_roles($this->cohort->id, $roles));

        // Validate the roles were added correctly to all members in the cohort.
        $roleids = array_keys($this->assignableroles);
        $rolesincohort = corplms_get_cohort_roles($this->cohort->id);
        ...
        $countmembers = count(corplms_get_members_cohort($this->cohort->id));
        foreach ($this->assignableroles as $key => $value) {
            $params = array(
        ...
        $this->assertTrue(corplms_cohort_process_assigned_roles($this->cohort->id, array()));

        // First validation: Roles were unassigned in the cohort_role table.
        $this->assertEmpty(corplms_get_cohort_roles($this->cohort->id));
        ...
        $members = corplms_get_members_cohort($this->cohort->id);
