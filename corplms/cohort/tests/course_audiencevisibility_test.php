 ...
 * vendor/bin/phpunit corplms_cohort_course_audiencevisibility_testcase
 *
 */
class corplms_cohort_course_audiencevisibility_testcase extends reportcache_advanced_testcase {
    ...
    public function setUp() {
        ...
        corplms_cohort_add_association($this->audience1->id, $this->course2->id,
                                        COHORT_ASSN_ITEMTYPE_COURSE, COHORT_ASSN_VALUE_VISIBLE);
        corplms_cohort_add_association($this->audience2->id, $this->course2->id,
                                        COHORT_ASSN_ITEMTYPE_COURSE, COHORT_ASSN_VALUE_VISIBLE);

        // Assign audience2 to course3 and course4.
        corplms_cohort_add_association($this->audience2->id, $this->course3->id,
                                        COHORT_ASSN_ITEMTYPE_COURSE, COHORT_ASSN_VALUE_VISIBLE);
        corplms_cohort_add_association($this->audience2->id, $this->course4->id,
                                        COHORT_ASSN_ITEMTYPE_COURSE, COHORT_ASSN_VALUE_VISIBLE);
    ...
    protected function get_visible_info($audiencevisibility, $content, $course) {
        ...
        if ($CFG->enhancedcatalog) { // New catalog.
            $search = array();
            if (is_array($content)) {
                $search = corplms_search_for_value($content, 'course_courseexpandlink', CORPLMS_SEARCH_OP_EQUAL, $course->fullname);
    ...
    protected function create_courses_old_visibility() {
        ...
        corplms_cohort_add_association($this->audience2->id, $this->course6->id, COHORT_ASSN_ITEMTYPE_COURSE, COHORT_ASSN_VALUE_VISIBLE);
        corplms_cohort_add_association($this->audience1->id, $this->course5->id, COHORT_ASSN_ITEMTYPE_COURSE, COHORT_ASSN_VALUE_VISIBLE);
    }
}
