...
$returnurl = $CFG->wwwroot . "/corplms/cohort/globalsettings.php";
...
if ($fromform = $mform->get_data()) {
    ...
    if (empty($fromform->submitbutton)) {
        corplms_set_notification(get_string('error:unknownbuttonclicked', 'corplms_cohort'), $returnurl);
    }

    update_global_settings($fromform);

    corplms_set_notification(get_string('globalsettingsupdated', 'corplms_cohort'), $returnurl, array('class' => 'notifysuccess'));
}

echo $OUTPUT->header();

echo $OUTPUT->heading(get_string('cohortglobalsettings', 'corplms_cohort'));
