...
require_capability('corplms/plan:cancreateplancohort', $context);
...
$url = new moodle_url('/corplms/cohort/learningplan.php', array('id' => $id));
...
$PAGE->requires->strings_for_js(array('confirmcreateplans'), 'corplms_plan');
$PAGE->requires->strings_for_js(array('continue', 'cancel'), 'moodle');
$args = array('args' => '{"id":"'.$cohort->id.'"}');
$jsmodule = array(
        'name' => 'corplms_cohortplans',
        'fullpath' => '/corplms/cohort/dialog/learningplan.js',
        'requires' => array('json', 'corplms_core'));
$PAGE->requires->js_init_call('M.corplms_cohortplans.init', $args, false, $jsmodule);
...
if ($data = $form->get_data()) {
    if (isset($data->submitbutton)) {
        ...
        corplms_set_notification(get_string('successfullycreatedplans', 'corplms_cohort', $createdplancount), $currenturl, array('class' => 'notifysuccess'));
    }
}
...
echo $OUTPUT->heading(get_string('createlpforaudience', 'corplms_cohort'));

echo get_string('createlpforaudienceblurb', 'corplms_cohort');

echo $form->display();

$tableheaders = array(
    get_string('template', 'corplms_core'),
    get_string('user'),
    get_string('date'),
    get_string('planstatus', 'corplms_plan'),
    get_string('numaffectedusers', 'corplms_plan'),
    get_string('manuallycreated', 'corplms_plan'),
    get_string('autocreated', 'corplms_plan'),
    get_string('complete')
);
$tablecolumns = array(
    'template',
...
echo $OUTPUT->heading(get_string('history', 'corplms_cohort'));
