...
class rb_source_cohort_associations_visible extends rb_base_source {
    ...
    protected function define_columnoptions() {
        ...
        $columnoptions[] = new rb_column_option(
            'associations',
            'name',
            get_string('associationname', 'corplms_cohort'),
            'associations.name',
        ...
        $columnoptions[] = new rb_column_option(
            'associations',
            'status',
            get_string('visibility', 'corplms_cohort'),
        ...
        $columnoptions[] = new rb_column_option(
            'associations',
            'type',
            get_string('associationtype', 'corplms_cohort'),
            'base.instancetype',
        ...
        $columnoptions[] = new rb_column_option(
            'associations',
            'nameiconlink',
            get_string('associationnameiconlink', 'corplms_cohort'),
            'associations.name',
            array(
                'joins' => 'associations',
        ...
        $columnoptions[] = new rb_column_option(
            'associations',
            'actionsvisible',
            get_string('associationactionsvisible', 'corplms_cohort'),
            'base.id',
            array(
        ...
        $columnoptions[] = new rb_column_option(
            'cohort',
            'name',
            get_string('name', 'corplms_cohort'),
            'cohort.name',
            array('joins' => 'cohort',
                  'dbdatatype' => 'char',
                  'outputformat' => 'text')
        ...
        $columnoptions[] = new rb_column_option(
            'cohort',
            'idnumber',
            get_string('idnumber', 'corplms_cohort'),
            'cohort.idnumber',
            array('joins' => 'cohort',
                  'dbdatatype' => 'char',
                  'outputformat' => 'text')
        ...
        $columnoptions[] = new rb_column_option(
            ...
            get_string('associationprogramcompletionlink', 'corplms_cohort'),
            'base.instanceid',
            array(
    ...
    protected function define_filteroptions() {
        ...
        $filteroptions[] = new rb_filter_option(
            ...
            array(
                'selectchoices' => array(
                    COHORT_ASSN_ITEMTYPE_COURSE => get_string('associationcoursesonly', 'corplms_cohort'),
                    COHORT_ASSN_ITEMTYPE_PROGRAM  => get_string('associationprogramsonly', 'corplms_cohort'),
                    COHORT_ASSN_ITEMTYPE_CERTIF  => get_string('associationcertificationsonly', 'corplms_cohort'),
                ),
                'simplemode' => true,
            )
        );
    ...
    public function rb_display_associationtype($instancetype, $row) {
        switch ($instancetype) {
            ...
            case COHORT_ASSN_ITEMTYPE_PROGRAM:
                $ret = get_string('program', 'corplms_program');
                break;
            case COHORT_ASSN_ITEMTYPE_CERTIF:
                $ret = get_string('certification', 'corplms_certification');
                break;
            default:
                $ret = '';
        }
    ...
    public function rb_display_visibility_status($status, $row) {
        ...
        if ($type == COHORT_ASSN_ITEMTYPE_COURSE) {
            ...
            $hascapability = has_capability('corplms/program:configuredetails', $programcontext);
        }
    ...
    public function rb_display_associationnameiconlink($instancename, $row) {
        ...
        if ($row->type == COHORT_ASSN_ITEMTYPE_COURSE) {
            ...
            $url = new moodle_url('/corplms/program/view.php', array('id' => $row->insid));
        }
    ...
    private function cohort_association_delete_link($associationid, $row) {
        ...
        if ($strdelete === false) {
            $strdelete = get_string('deletelearningitem', 'corplms_cohort');
        }
        $delurl = new moodle_url('/corplms/cohort/dialog/updatelearning.php',
    ...
    public function rb_display_programcompletionlink($instanceid, $row) {
        ...
        if ($canedit && $row->type == COHORT_ASSN_ITEMTYPE_PROGRAM) {
            return corplms_cohort_program_completion_link($row->cohortid, $instanceid);
        }
        return get_string('na', 'corplms_cohort');
    }
    ...
    public function get_required_jss() {
        ...
        $jsdetails->initcall = 'M.corplms_cohortvisiblelearning.init';
        $jsdetails->jsmodule = array(
            'name' => 'corplms_cohortvisiblelearning',
            'fullpath' => '/corplms/cohort/visiblelearning.js',
            'requires' => array('json'));
