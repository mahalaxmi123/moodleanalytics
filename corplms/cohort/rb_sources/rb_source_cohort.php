...
class rb_source_cohort extends rb_base_source {
    ...
    protected function define_columnoptions() {
        ...
        $columnoptions[] = new rb_column_option(
            ...
            get_string('name', 'corplms_cohort'), // Name for the column.
        ...
        $columnoptions[] = new rb_column_option(
            'cohort',
            'namelink',
            get_string('namelink', 'corplms_cohort'),
            'base.name',
            array(
        ...
        $columnoptions[] = new rb_column_option(
            'cohort',
            'numofmembers',
            get_string('numofmembers', 'corplms_cohort'),
        ...
        $columnoptions[] = new rb_column_option(
            'cohort',
            'actions',
            get_string('actions', 'corplms_cohort'),
            'base.id',
            array(
        ...
        $columnoptions[] = new rb_column_option(
            'cohort',
            'startdate',
            get_string('startdate', 'corplms_cohort'),
            'base.startdate',
            array(
        ...
        $columnoptions[] = new rb_column_option(
            'cohort',
            'enddate',
            get_string('enddate', 'corplms_cohort'),
            'base.enddate',
            array(
                'displayfunc' => 'nice_date',
                'dbdatatype' => 'timestamp',
        ...
        $columnoptions[] = new rb_column_option(
            'cohort',
            'status',
            get_string('status', 'corplms_cohort'),
            'base.id',
            array(
    ...
    protected function define_filteroptions() {
        ...
        $filteroptions[] = new rb_filter_option(
            ...
            array(
                'selectchoices' => array(
                    cohort::TYPE_DYNAMIC => get_string('dynamic', 'corplms_cohort'),
                    cohort::TYPE_STATIC  => get_string('set', 'corplms_cohort'),
                ),
                'simplemode' => true,
            )
        );
    ...
    public function rb_display_cohort_type($cohorttype, $row) {
        ...
        switch( $cohorttype ) {
            case cohort::TYPE_DYNAMIC:
                $ret = get_string('dynamic', 'corplms_cohort');
                break;
            case cohort::TYPE_STATIC:
                $ret = get_string('set', 'corplms_cohort');
                break;
            default:
                $ret = get_string('typeunknown', 'corplms_cohort', $cohorttype);
        }
        return $ret;
    }
    ...
    public function rb_display_cohort_actions( $cohortid, $row ) {
        ...
        if ($canedit) {
            ...
            $str .= html_writer::link(
                $cloneurl,
                $OUTPUT->pix_icon('t/copy', get_string('copy', 'corplms_cohort'), null, array('class' => 'iconsmall')),
                null,
                array('class' => 'action-icon')
            );
    ...
    public function rb_display_cohort_status($cohortid, $row) {
        $now = time();
        if (corplms_cohort_is_active($row, $now)) {
            return get_string('cohortdateactive', 'corplms_cohort');
        }

        if ($row->startdate && $row->startdate > $now) {
            return get_string('cohortdatenotyetstarted', 'corplms_cohort');
        }

        if ($row->enddate && $row->enddate < $now) {
            return get_string('cohortdatealreadyended', 'corplms_cohort');
        }
