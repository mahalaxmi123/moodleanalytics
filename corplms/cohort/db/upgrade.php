...
/**
 * DB upgrades for Corplms dynamic cohorts
 */

function xmldb_corplms_cohort_upgrade($oldversion) {
    ...
    if ($oldversion < 2012060500) {
        ...
        try {
            ...
            foreach ($cohorts as $cohort) {
                ...
                foreach ($criteria as $criterion) {
                    ...
                    $todb->name = get_string('rulesetname', 'corplms_cohort', $todb->sortorder);
                    $todb->timecreated = $todb->timemodified = time();
                    $todb->modifierid = $USER->id;
                    ...
                    if (!empty($criterion->profilefield) && !empty($criterion->profilefieldvalues)) {
                        if (substr($criterion->profilefield, 0, 11) == 'customfield') {
                            ...
                            if ($fieldtype == 'textarea') {
                                $warning = get_string('error:badruleonupgrade', 'corplms_cohort', $cohort->name);
        ...
        upgrade_plugin_savepoint(true, 2012060500, 'corplms', 'cohort');
    }
    ...
    if ($oldversion < 2012061800) {
        ...
        upgrade_plugin_savepoint(true, 2012061800, 'corplms', 'cohort');
    }
    ...
    if ($oldversion < 2012072600) {
        ...
        upgrade_plugin_savepoint(true, 2012072600, 'corplms', 'cohort');
    }
    ...
    if ($oldversion < 2012080202) {
        ...
        upgrade_plugin_savepoint(true, 2012080202, 'corplms', 'cohort');
    }
    ...
    if ($oldversion < 2012082100) {
        ...
        upgrade_plugin_savepoint(true, 2012082100, 'corplms', 'cohort');
    }
    ...
    if ($oldversion < 2012102300) {
        ...
        upgrade_plugin_savepoint(true, 2012102300, 'corplms', 'cohort');
    }
    ...
    if ($oldversion < 2013100100) {
        ...
        upgrade_plugin_savepoint(true, 2013100100, 'corplms', 'cohort');
    }
    ...
    if ($oldversion < 2013100300) {
        ...
        upgrade_plugin_savepoint(true, 2013100300, 'corplms', 'cohort');
    }

    if ($oldversion < 2013103000) {
        // Don't run again if this upgrade has occurred before.
        $hasrun = get_config('corplms_cohort', 'cohort_rule_fix_has_run');
        if (empty($hasrun)) {
            ...
            if (!empty($brokenrules)) {
                echo $OUTPUT->notification(
                    get_string('cohortbugheading', 'corplms_cohort'),
                    'notifysuccess');
                ...
                foreach ($brokenrules as $rule) {
                    if (corplms_cohort_is_rule_fixable($rule)) {
                        $rulestofix[] = $rule;
                    } else {
                        $unfixablerules[] = $rule;
                    }
                ...
                if (!empty($rulestofix)) {
                    echo $OUTPUT->notification(
                        get_string('cohortbugfixingxrules', 'corplms_cohort', count($rulestofix)),
                        'notifysuccess');
                    corplms_cohort_remap_rules($rulestofix);
                }

                // Record that we've run this upgrade now.
                set_config('cohort_rule_fix_has_run', 1, 'corplms_cohort');

                // Advise users about remaining bad rules.
                if (!empty($unfixablerules)) {
                    $output = $PAGE->get_renderer('corplms_core', null);
                    echo $output->show_text_broken_rules($unfixablerules);
                } else {
                    echo $OUTPUT->notification(
                        get_string('cohortbugnounfixable', 'corplms_cohort'),
                        'notifysuccess');
                }
        ...
        // Cohort savepoint reached.
        upgrade_plugin_savepoint(true, 2013103000, 'corplms', 'cohort');
    }
    ...
    if ($oldversion < 2014031900) {
        ...
        upgrade_plugin_savepoint(true, 2014031900, 'corplms', 'cohort');
    }
    ...
    if ($oldversion < 2014040700) {
        ...
        // Cohort savepoint reached.
        upgrade_plugin_savepoint(true, 2014040700, 'corplms', 'cohort');
    }
    ...
    if ($oldversion < 2014090300) {
        ...
        upgrade_plugin_savepoint(true, 2014090300, 'corplms', 'cohort');
    }
