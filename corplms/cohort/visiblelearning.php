...
$url = new moodle_url('/corplms/cohort/visiblelearning.php', array('id' => $id, 'format' => $format));
admin_externalpage_setup('cohorts', '', null, $url, array('pagelayout' => 'report'));

if (empty($CFG->audiencevisibility)) {
    echo $OUTPUT->header();
    echo $OUTPUT->notification(get_string('error:visiblelearningdisabled', 'corplms_cohort'));
    echo $OUTPUT->footer();
    exit;
}
...
if (!$id) {
    ...
    echo $OUTPUT->container(get_string('cohortvisiblelearningselect', 'corplms_cohort', $url->out()));
    echo $OUTPUT->footer();
    exit;
}
...
// Setup lightbox.
local_js(array(CORPLMS_JS_DIALOG, CORPLMS_JS_TREEVIEW));

$PAGE->requires->strings_for_js(array('none'), 'moodle');
$PAGE->requires->strings_for_js(array('assignvisiblelearning', 'assignenrolledlearning', 'deletelearningconfirm', 'savinglearning'), 'corplms_cohort');
$jsmodule = array(
...
$args = array('args' => '{"cohortid":' . $cohort->id . ',' .
            ...
            '"saveurl":"/corplms/cohort/visiblelearning.php" }');
$PAGE->requires->js_init_call('M.corplms_cohortlearning.init', $args, false, $jsmodule);

$strheading = get_string('visiblelearning', 'corplms_cohort');
corplms_cohort_navlinks($cohort->id, format_string($cohort->name), $strheading);
echo $OUTPUT->header();
...
if ($canedit) {
    ...
    if (has_capability('corplms/program:configuredetails', context_system::instance())) {
        // Add programs.
        echo html_writer::start_tag('div', array('class' => 'singlebutton'));
        ...
        echo html_writer::empty_tag('input', array('type' => 'submit', 'id' => 'add-certification-learningitem-dialog',
            'value' => get_string('addcertifications', 'corplms_cohort')));
        echo html_writer::end_tag('div');
    }
