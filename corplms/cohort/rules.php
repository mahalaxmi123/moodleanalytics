...
$url = new moodle_url('/corplms/cohort/rules.php', array('id' => $id));
if ($debug) {
...
require_capability('corplms/cohort:managerules', $context);
...
$PAGE->requires->strings_for_js(
    ...
    'corplms_cohort');
$PAGE->requires->strings_for_js(array('datepickerlongyearregexjs', 'datepickerlongyeardisplayformat'), 'corplms_core');
$jsmodule = array(
        'name' => 'corplms_cohortrules',
        'fullpath' => '/corplms/cohort/rules/ruledialog.js',
        'requires' => array('json'));
$PAGE->requires->js_init_call('M.corplms_cohortrules.init', $args, false, $jsmodule);
...
$PAGE->requires->strings_for_js(array('deleteruleconfirm', 'deleteruleparamconfirm', 'savingrule', 'error:noresponsefromajax',
    'error:badresponsefromajax'), 'corplms_cohort');
$jsmodule = array(
        'name' => 'corplms_cohortruledelete',
        'fullpath' => '/corplms/cohort/rules/ruledelete.js',
        'requires' => array('json'));
$PAGE->requires->js_init_call('M.corplms_cohortruledelete.init', null, false, $jsmodule);
...
if (!$cohort->cohorttype == cohort::TYPE_DYNAMIC) {
    print_error('error:notdynamiccohort', 'corplms_cohort');
}
...
if (($data = data_submitted()) && confirm_sesskey()) {
    if ($canapproverules && !empty($data->approverulechanges)) {
        if (!cohort_rules_approve_changes($cohort)) {
            print_error('error:couldnotapprovechanges', 'corplms_cohort');
        }
        corplms_set_notification(get_string('rulesapprovesuccess', 'corplms_cohort'), $url->out(), array('class' => 'notifysuccess'));
    }
    if ($canapproverules && isset($data->cancelrulechanges)) {
        if (!cohort_rules_cancel_changes($cohort)) {
            print_error('error:couldnotcancelchanges', 'corplms_cohort');
        }
        corplms_set_notification(get_string('rulescancelsuccess', 'corplms_cohort'), $url->out());
    }
}
...
if ($formdata = $mform->get_data()) {
    ...
    corplms_set_notification(get_string('rulesupdatesuccess', 'corplms_cohort'), $url->out(), array('class' => 'notifysuccess'));
...
/// Output
///
$strheading = get_string('editrules', 'corplms_cohort');
...
if ($debug) {
    $whereclause = corplms_cohort_get_dynamic_cohort_whereclause($id);
    echo $OUTPUT->heading(get_string('querydebugheader', 'corplms_cohort'), 3);
    echo html_writer::tag('pre', "select count(*) from {user} u where " . $whereclause->sql, array('class' => 'notifymessage'));
    echo $OUTPUT->heading(get_string('querydebugparams', 'corplms_cohort'), 3);
...
function display_approval_action_box($cohortid, $debug=false, $style='display:block') {
    ...
    $attrs = array('action' => new moodle_url("/corplms/cohort/rules.php"), 'method' => 'POST', 'class' => 'approvalform');
    ...
    echo get_string('cohortruleschanged', 'corplms_cohort');
    $attrs = array('type' => 'submit', 'name' => 'approverulechanges', 'value' => get_string('approvechanges', 'corplms_cohort'));
    echo html_writer::empty_tag('input', $attrs);
    $attrs = array('type' => 'submit', 'name' => 'cancelrulechanges', 'value' => get_string('cancelchanges', 'corplms_cohort'));
    echo html_writer::empty_tag('input', $attrs);
