...
if ($action == 'stages') {
    if ($subjectid == $USER->id) {
        require_capability('corplms/appraisal:printownappraisals', $systemcontext);
    } else {
        $usercontext = context_user::instance($subjectid);
        require_capability('corplms/appraisal:printstaffappraisals', $usercontext);
    }
}
...
$PAGE->set_url(new moodle_url('/corplms/appraisal/snapshot.php', array('role' => $role,
    'subjectid' => $subjectid, 'appraisalid' => $appraisalid, 'action' => $action)));

$PAGE->set_pagelayout('popup');
// Force standardcorplms theme on this page to avoid responsive theme PDF rendering errors.
$originalthemeorder = (!empty($CFG->themeorder)) ? $CFG->themeorder : null;
$CFG->themeorder = array('corplmspdf');
...
if ($action == 'snapshot') {
    ...
    $strsource->link = $renderer->action_link(new moodle_url('/corplms/appraisal/index.php'),
            get_string('allappraisals', 'corplms_appraisal'));
    echo html_writer::empty_tag('input', array('type' => 'hidden', 'name' => 'downloadurl', 'id' => 'downloadurl',
            'value' => $downloadurl));
    echo html_writer::tag('div', get_string('snapshotdone', 'corplms_appraisal', $strsource),
