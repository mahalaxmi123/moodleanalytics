        ...
        if (!$preview) {
            ...
            if (!$formissubmitted) {
                ...
                if (($answers->submitaction == 'saveprogress') && ($roleassignment->activepageid == $pageid)) {
                    ...
                    $returnurl = new moodle_url('/corplms/appraisal/myappraisal.php', array('role' => $role,
                        'subjectid' => $subjectid, 'appraisalid' => $appraisalid, 'action' => $action, 'pageid' => $pageid));
                    corplms_set_notification(get_string('progresssaved', 'corplms_appraisal'), $returnurl,
                    ...
                    if (!$formisvalid) {
                        corplms_set_notification(get_string('error:submitteddatainvalid', 'corplms_appraisal'));
                        ...
                        if ($appraisal->save_answers($answers, $roleassignment)) {
                            ...
                            if ($answers->submitaction == 'next') {
                                ...
                                redirect(new moodle_url('/corplms/appraisal/myappraisal.php', array('role' => $role,
                                ...
                                corplms_set_notification(get_string('stagecompleted', 'corplms_appraisal'), $returnurl,
                                    array('class' => 'notifysuccess'));
                                ...
                                $returnurl = new moodle_url('/corplms/appraisal/myappraisal.php', array('role' => $role,
                                    ...
                                    'pageid' => $pageid));
                                corplms_set_notification(get_string('changessaved', 'corplms_appraisal'), $returnurl,
                                    array('class' => 'notifysuccess'));
                            }
                        }
                    }
...
local_js(array(
    CORPLMS_JS_DIALOG));
$jsmodule = array(
    'name' => 'corplms_appraisal_myappraisal',
    'fullpath' => '/corplms/appraisal/js/myappraisal.js',
    'requires' => array('json'));
$PAGE->requires->strings_for_js(array('printnow', 'printyourappraisal', 'snapshotgeneration', 'downloadnow',
    'snapshotdialogtitle'), 'corplms_appraisal');
$PAGE->requires->js_init_call('M.corplms_appraisal_myappraisal.init',  array('args' => json_encode(array(
...
$pageurl = new moodle_url('/corplms/appraisal/myappraisal.php', $urlparams);
$PAGE->set_corplms_menu_selected('appraisals');
if ($role == appraisal::ROLE_LEARNER) {
    $PAGE->navbar->add(get_string('myappraisals', 'corplms_appraisal'), new moodle_url('/corplms/appraisal/index.php'));
} else {
    $PAGE->navbar->add(get_string('myteamappraisals', 'corplms_appraisal'),
            new moodle_url('/corplms/appraisal/index.php', array('role' => $role)));
}
...
if ($action == 'stages') {
    ...
    if ($subjectid == $USER->id) {
        $showprint = has_capability('corplms/appraisal:printownappraisals', $systemcontext);
    } else {
        $showprint = has_capability('corplms/appraisal:printstaffappraisals', $usercontext);
    }
