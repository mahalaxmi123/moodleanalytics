...
/**
 * Output renderer for corplms_appraisals module
 */
class corplms_appraisal_renderer extends plugin_renderer_base {
    /**
    ...
    public function appraisal_manage_table($appraisals = array(), $userid = null) {
        ...
        if (empty($appraisals)) {
            return get_string('noappraisals', 'corplms_appraisal');
        }

        $tableheader = array(get_string('name', 'corplms_appraisal'),
                             get_string('start', 'corplms_appraisal'),
                             get_string('learners', 'corplms_appraisal'),
                             get_string('status', 'corplms_appraisal'),
                             get_string('options', 'corplms_appraisal'));
        ...
        $stractivate = get_string('activate', 'corplms_appraisal');
        $strclose = get_string('close', 'corplms_appraisal');
        $strsettings = get_string('settings', 'corplms_appraisal');
        $strdelete = get_string('delete', 'corplms_appraisal');
        $strcannotdelete = get_string('error:appraisalisactive', 'corplms_appraisal');
        $strnoteditable = get_string('error:appraisalnoteditable', 'corplms_appraisal');
        ...
        foreach ($appraisals as $appraisal) {
            ...
            $activateurl = new moodle_url('/corplms/appraisal/activation.php',
                    array('id' => $appraisal->id, 'action' => 'activate'));
            $closeurl = new moodle_url('/corplms/appraisal/activation.php',
                    array('id' => $appraisal->id, 'action' => 'close'));
            $editurl = new moodle_url('/corplms/appraisal/general.php',
                    array('id' => $appraisal->id));
            $assignurl = new moodle_url('/corplms/appraisal/learners.php',
                    array('appraisalid' => $appraisal->id));
            $deleteurl = new moodle_url('/corplms/appraisal/manage.php',
                    array('id' => $appraisal->id, 'action' => 'delete'));
            $cloneurl = new moodle_url('/corplms/appraisal/manage.php',
                    array('id' => $appraisal->id, 'action' => 'copy', 'sesskey' => sesskey()));

            $row = array();
            if (has_capability('corplms/appraisal:manageappraisals', $systemcontext, $userid)) {
            ...
            if ($appraisal->timestarted) {
                ...
                $row[] = get_string('none', 'corplms_appraisal');
            }
            $row[] = $appraisal->lnum;
            ...
            $activate = '';
            if (has_capability('corplms/appraisal:manageactivation', $systemcontext, $userid)) {
                if ($appraisal->status == appraisal::STATUS_ACTIVE) {
    ...
    public function appraisal_management_tabs($appraisalid, $currenttab = 'general') {
        ...
        $systemcontext = context_system::instance();
        if (has_capability('corplms/appraisal:manageappraisals', $systemcontext)) {
            $row[] = new tabobject('general', $CFG->wwwroot . '/corplms/appraisal/general.php?id=' .
                    $appraisalid, get_string('general'));
        }
        if (has_capability('corplms/appraisal:managepageelements', $systemcontext)) {
            $row[] = new tabobject('content', $CFG->wwwroot . '/corplms/appraisal/stage.php?appraisalid=' .
                    $appraisalid, get_string('content', 'corplms_appraisal'));
        }
        $capabilities = array('corplms/appraisal:viewassignedusers', 'corplms/appraisal:assignappraisaltogroup');
        if (has_any_capability($capabilities, $systemcontext)) {
            $row[] = new tabobject('learners', $CFG->wwwroot . '/corplms/appraisal/learners.php?appraisalid=' .
                    $appraisalid, get_string('assignments', 'corplms_appraisal'));
        }
        if (has_capability('corplms/appraisal:managenotifications', $systemcontext)) {
            $row[] = new tabobject('messages', $CFG->wwwroot . '/corplms/appraisal/message.php?id=' .
                    $appraisalid, get_string('messages', 'corplms_appraisal'));
        }
    ...
    public function appraisal_message_table(array $events) {
        if (empty($events)) {
            return get_string('nomessages', 'corplms_appraisal');
        }

        $tableheader = array(get_string('messagetitle', 'corplms_appraisal'),
                             get_string('messageevent', 'corplms_appraisal'),
                             get_string('messagetiming', 'corplms_appraisal'),
                             get_string('messagerecipients', 'corplms_appraisal'),
                             get_string('options', 'corplms_appraisal'));
        ...
        foreach ($events as $event) {
            ...
            $editurl = new moodle_url('/corplms/appraisal/message.php',
                    array('id' => $event->appraisalid, 'messageid' => $event->id, 'action' => 'edit'));
            $deleteurl = new moodle_url('/corplms/appraisal/message.php',
    ...
    public function display_event($type) {
        ...
        $strevent = get_string($type, 'corplms_appraisal');
        return $strevent;
    }
    ...
    public function display_timing($delta, $period) {
        if ($delta == 0) {
            return get_string('immediate', 'corplms_appraisal');
        }
        $str = new stdClass();
        $str->delta = abs($delta);
        switch ($period) {
            case appraisal_message::PERIOD_DAY:
                $str->period = get_string('perioddays', 'corplms_appraisal');
                break;
            case appraisal_message::PERIOD_WEEK;
                $str->period = get_string('periodweeks', 'corplms_appraisal');
                break;
            case appraisal_message::PERIOD_MONTH:
                $str->period = get_string('periodmonths', 'corplms_appraisal');
                break;
        }

        if ($delta < 0) {
            return get_string('eventbefore', 'corplms_appraisal', $str);
        } else {
            return get_string('eventafter', 'corplms_appraisal', $str);
        }
    }
    ...
    public function report_active_table($appraisals = array()) {
        if (empty($appraisals)) {
            return get_string('noappraisalsactive', 'corplms_appraisal');
        }

        $tableheader = array(get_string('name', 'corplms_appraisal'),
                             get_string('overdue', 'corplms_appraisal'),
                             get_string('ontarget', 'corplms_appraisal'),
                             get_string('cancelled', 'corplms_appraisal'),
                             get_string('complete', 'corplms_appraisal'),
                             get_string('reports', 'corplms_appraisal'));
        ...
        foreach ($appraisals as $appraisal) {
            ...
            $appraisalurl = new moodle_url('/corplms/appraisal/general.php',
                    array('id' => $appraisal->id));
            $statusreporturl = new moodle_url('/corplms/appraisal/statusreport.php',
                    array('appraisalid' => $appraisal->id, 'clearfilters' => 1));
            $detailreporturl = new moodle_url('/corplms/appraisal/detailreport.php',
                    array('appraisalid' => $appraisal->id, 'clearfilters' => 1));
            ...
            $row[] = html_writer::link($statusreporturl, get_string('statusreport', 'corplms_appraisal')) . '&nbsp' .
                     html_writer::link($detailreporturl, get_string('detailreport', 'corplms_appraisal'));
    ...
    public function report_inactive_table($appraisals = array()) {
        if (empty($appraisals)) {
            return get_string('noappraisalsinactive', 'corplms_appraisal');
        }

        $tableheader = array(get_string('name', 'corplms_appraisal'),
                             get_string('status', 'corplms_appraisal'),
                             get_string('finished', 'corplms_appraisal'),
                             get_string('complete', 'corplms_appraisal'),
                             get_string('incomplete', 'corplms_appraisal'),
                             get_string('reports', 'corplms_appraisal'));
    ...
    public function detail_report_table($detailreportid, $appraisals = array()) {
        ...
        foreach ($appraisals as $appraisal) {
            ...
            $detailreporturl = new moodle_url('/corplms/reportbuilder/report.php',
    ...
    public function display_user_appraisals($appraisals, $role) {
        ...
        if ($role != appraisal::ROLE_LEARNER) {
            $tableheader[] = get_string('name', 'corplms_appraisal');
        }
        $tableheader[] = get_string('appraisal', 'corplms_appraisal');
        $tableheader[] = get_string('datestart', 'corplms_appraisal');
        $tableheader[] = get_string('dateend', 'corplms_appraisal');
        $tableheader[] = get_string('status', 'corplms_appraisal');
        ...
        foreach ($appraisals as $appraisal) {
            ...
            $params = array('role' => $role, 'subjectid' => $appraisal->userid, 'appraisalid' => $appraisal->id,
                'action' => 'stages');
            $editurl = new moodle_url('/corplms/appraisal/myappraisal.php', $params);
            ...
            if (has_capability('corplms/appraisal:viewownappraisals', $systemcontext)) {
                ...
                foreach ($snapshots as $snapshot) {
                    ...
                    $link = $this->output->action_link($snapshot->url, get_string('snapshotname', 'corplms_appraisal', $snapshot));
    ...
    public function appraisal_stages_table($stages = array(), $userid = 0, $stageid = 0) {
        ...
        if (empty($stages)) {
            return get_string('nostages', 'corplms_appraisal');
        }

        $tableheader = array(get_string('stageheader', 'corplms_appraisal'),
                             get_string('completeby', 'corplms_appraisal'),
                             get_string('rolescananswer', 'corplms_appraisal') .
                                    $this->output->help_icon('rolescananswer', 'corplms_appraisal', null),
                             get_string('rolescanview', 'corplms_appraisal') .
                                    $this->output->help_icon('rolescanview', 'corplms_appraisal', null),
                             get_string('options', 'corplms_appraisal'));
        ...
        foreach ($stages as $stage) {
            $contenturl = new moodle_url('/corplms/appraisal/stage.php',
                    array('appraisalid' => $stage->appraisalid, 'id' => $stage->id));
            $editurl = new moodle_url('/corplms/appraisal/stage.php',
                    array('appraisalid' => $stage->appraisalid, 'action' => 'edit', 'id' => $stage->id));
            $deleteurl = new moodle_url('/corplms/appraisal/stage.php',
            ...
            if ($stage->timedue) {
                ...
                $row->cells[] = get_string('none', 'corplms_appraisal');
            }

            if ($stage->roles) {
                $row->cells[] = '<span class="appraisal-rolescananswer">' . $this->display_roles($stage->roles) .'</span>';
            } else {
                $row->cells[] = '<span class="appraisal-rolescananswer">' . get_string('none', 'corplms_appraisal') .'</span>';
            }
            ...
            if (!empty($viewers)) {
                ...
                $row->cells[] = '<span class="appraisal-rolescanview">' . get_string('none', 'corplms_appraisal') .'</span>';
            }
    ...
    public function display_roles($roles) {
        ...
        foreach ($roles as $role => $rolename) {
            if (isset($appraisalroles[$role])) {
                $strroles[] = get_string($appraisalroles[$role], 'corplms_appraisal');
            } else {
                switch($role) {
                    case appraisal::ROLE_ADMINISTRATOR:
                        $strroles[] = get_string('roleadministrator', 'corplms_appraisal');
                        break;
                }
            }
        }
    ...
    public function appraisal_additional_actions($status, $id) {
        $activateurl = new moodle_url('/corplms/appraisal/activation.php', array('id' => $id, 'action' => 'activate'));
        $closeurl = new moodle_url('/corplms/appraisal/activation.php', array('id' => $id, 'action' => 'close'));
        $previewurl = new moodle_url('/corplms/appraisal/myappraisal.php',
                array('appraisalid' => $id, 'preview' => 1));

        $strstatusnow = appraisal::display_status($status);
        $strstatusat = get_string('statusat', 'corplms_appraisal');
        $strpreviewappraisal = get_string('previewappraisal', 'corplms_appraisal');
        ...
        if ($appraisal->status == appraisal::STATUS_ACTIVE) {
            $activate = $this->output->action_link($closeurl, get_string('close', 'corplms_appraisal'));
        } else if ($appraisal->status == appraisal::STATUS_DRAFT) {
            $activate = $this->output->action_link($activateurl,  get_string('activatenow', 'corplms_appraisal'));
        } else {
            $activate = '';
        }
    ...
    public function create_appraisal_button() {
        return $this->output->single_button(new moodle_url('/corplms/appraisal/general.php'),
                get_string('createappraisal', 'corplms_appraisal'), 'get');
    }
    ...
    public function create_message_button($appraisalid) {
        return $this->output->single_button(new moodle_url('/corplms/appraisal/message.php',
                array('id' => $appraisalid, 'action' => 'edit')), get_string('messagecreate', 'corplms_appraisal'), 'get');
    }
    ...
    public function create_stage_button($appraisal) {
        if (appraisal::is_draft($appraisal)) {
            return $this->output->single_button(new moodle_url('/corplms/appraisal/stage.php',
                    array('appraisalid' => $appraisal->id, 'action' => 'edit')),
                    get_string('addstage', 'corplms_appraisal'), 'get');
        }
        return '';
    }
    ...
    public function create_new_page_link(appraisal_stage $stage) {
        ...
        if (appraisal::is_draft($stage->appraisalid)) {
            $stageid = $stage->id;
            $newpageurl = new moodle_url('/corplms/appraisal/ajax/page.php',
                    array('appraisalstageid' => $stageid));
            $strnewpage = get_string('addpage', 'corplms_appraisal');
    ...
    public function list_pages($pages, $pageid, appraisal_stage $stage) {
        $list = array();
        if ($pages) {
            $stredit = get_string('settings', 'corplms_question');
            $strdelete = get_string('delete', 'corplms_question');
            $strup =  get_string('moveup', 'corplms_question');
            $strdown =  get_string('movedown', 'corplms_question');
            $last = end($pages);
            $first = reset($pages);
            foreach ($pages as $page) {
                ...
                if ($page->id != $first->id) {
                    $posupurl = new moodle_url('/corplms/appraisal/ajax/page.php', array('action' => 'posup',
                ...
                if ($page->id != $last->id) {
                    $posdownurl = new moodle_url('/corplms/appraisal/ajax/page.php', array('action' => 'posdown',
                            'id' => $page->id, 'sesskey' => sesskey()));
                ...
                $pageurl = new moodle_url('/corplms/appraisal/ajax/question.php', array('appraisalstagepageid' => $page->id));
                $editurl = new moodle_url('/corplms/appraisal/ajax/page.php', array('action' => 'edit', 'id' => $page->id));
                $deleteurl = new moodle_url('/corplms/appraisal/ajax/page.php', array('action' => 'delete', 'id' => $page->id));
        ...
        if ($quests) {
            ...
            foreach ($quests as $quest) {
                ...
                if ($quest->id != $first->id) {
                    $posupurl = new moodle_url('/corplms/appraisal/ajax/question.php', array('action' => 'posup',
                        'id' => $quest->id));
                    $posuplink = $this->output->action_icon($posupurl, new pix_icon('/t/up', $strup, 'moodle'), null,
                            array('class' => 'action-icon js-hide'));
                } else {
                ...
                if ($quest->id != $last->id) {
                    $posdownurl = new moodle_url('/corplms/appraisal/ajax/question.php', array('action' => 'posdown',
                            'id' => $quest->id));
                    $posdownlink = $this->output->action_icon($posdownurl, new pix_icon('/t/down', $strdown, 'moodle'), null,
                            array('class' => 'action-icon js-hide'));
                } else {
                ...
                $editurl = new moodle_url('/corplms/appraisal/ajax/question.php', array('action' => 'edit',
                    'id' => $quest->id, 'sesskey' => sesskey()));
                $duplicateurl = new moodle_url('/corplms/appraisal/ajax/question.php', array('action' => 'duplicate',
                    'id' => $quest->id, 'sesskey' => sesskey()));
                $deleteurl = new moodle_url('/corplms/appraisal/ajax/question.php', array('action' => 'delete',
                    'id' => $quest->id, 'sesskey' => sesskey()));

                if ($quest->hasredisplay) {
                    $deleteurl->param('hasredisplay', 'true');
                    $redisplayed = $this->output->pix_icon('link', get_string('hasredisplayitems', 'corplms_appraisal'),
                            'corplms_appraisal', array('class' => 'action-icon iconsmall redirect'));
                } else {
                    $redisplayed = '';
                }
                ...
                if ($question->is_invalid_redisplay()) {
                    $questtext .= html_writer::tag('div',
                        $this->output->notification(get_string('error:redisplayoutoforder', 'corplms_appraisal'), 'notifyproblem'),
    ...
    public function stage_page_container($stage = null, $pages = null, $pageid = 0) {
        ...
        $header = $this->heading(get_string('managestage', 'corplms_appraisal', format_string($stage->name)), 3);
    ...
    public function confirm_appraisal_activation($appraisal, $errors, $warnings) {
        $out = '';
        if (!empty($errors)) {
            $out .= $this->heading(get_string('appraisalinvalid', 'corplms_appraisal'));
            // Output the errors to the screen.
            $out .= html_writer::tag('p', get_string('appraisalfixerrors', 'corplms_appraisal'));
            $errordesc = array();
            ...
            if (!empty($warnings)) {
                $out .= html_writer::tag('p', get_string('appraisalfixwarnings', 'corplms_appraisal'));
                $warndesc = array();
            ...
            $buttons = array();
            $buttons[] = $this->output->single_button(new moodle_url('/corplms/appraisal/stage.php',
                    array('appraisalid' => $appraisal->id)), get_string('backtoappraisal', 'corplms_appraisal',
                    format_string($appraisal->name)), 'get');
            $out .= html_writer::tag('div', implode(' ', $buttons), array('class' => 'buttons'));
        } else {
            $out .= html_writer::tag('p', get_string('appraisallastwarning', 'corplms_appraisal'));
            // Output the warnings to the screen.
            if (!empty($warnings)) {
                $out .= html_writer::tag('p', get_string('confirmactivatewarning', 'corplms_appraisal'));
                $warndesc = array();
                foreach ($warnings as $warn) {
                    $warndesc[] = html_writer::tag('li', $warn);
                }
                ...
                $out .= html_writer::tag('p', get_string('confirmactivateappraisal', 'corplms_appraisal'));
            }
            $buttons = array();
            $buttons[] = $this->output->single_button(new moodle_url('/corplms/appraisal/activation.php',
                    array('action' => 'activate', 'confirm' => 1, 'id' => $appraisal->id)),
                    get_string('activate', 'corplms_appraisal'), 'post');
            $buttons[] = $this->output->single_button(new moodle_url('/corplms/appraisal/stage.php',
                    array('appraisalid' => $appraisal->id)), get_string('backtoappraisal', 'corplms_appraisal',
                    format_string($appraisal->name)), 'get');
            $out .= html_writer::tag('div', implode(' ', $buttons), array('class' => 'buttons'));
    ...
    public function confirm_appraisal_close($incompleteusers) {
        return html_writer::tag('p', get_string('closeusersincomplete', 'corplms_appraisal', $incompleteusers));
    }
    ...
    public function confirm_delete_appraisal($appraisalid, array $content) {
        ...
        if (count($content['stages']) > 0) {
            $out .= get_string('appraisalhasstages', 'corplms_appraisal');
            $stages = '';
        ...
        $out .= html_writer::tag('p', get_string('confirmdeleteappraisal', 'corplms_appraisal'));

        $buttons = array();
        $buttons[] = $this->output->single_button(new moodle_url('/corplms/appraisal/manage.php',
                array('action' => 'delete', 'confirm' => 1, 'id' => $appraisalid)),
                get_string('delete', 'corplms_appraisal'), 'post');
        $buttons[] = $this->output->single_button(new moodle_url('/corplms/appraisal/manage.php'),
    ...
    public function confirm_delete_stage($stageid) {
        if (appraisal_stage::has_redisplayed_items($stageid)) {
            $out = html_writer::tag('p', get_string('confirmdeletestagewithredisplay', 'corplms_appraisal'));
        } else {
            $out = html_writer::tag('p', get_string('confirmdeletestage', 'corplms_appraisal'));
        }
    ...
    public function confirm_delete_page($pageid, $stageid) {
        $out = '';
        $out .= html_writer::tag('p', get_string('confirmdeletepage', 'corplms_appraisal'));

        $buttons = array();
        $buttons[] = $this->output->single_button(new moodle_url('/corplms/appraisal/ajax/page.php',
    ...
    public function confirm_delete_question($questionid, $pageid) {
        $out = '';
        $out .= html_writer::tag('p', get_string('confirmdeletequestion', 'corplms_appraisal'));

        $buttons = array();
        $buttons[] = $this->output->single_button(new moodle_url('/corplms/appraisal/ajax/question.php',
    ...
    public function confirm_delete_message($messageid, $appraisalid) {
        $out = '';
        $out .= html_writer::tag('p', get_string('confirmdeletemessage', 'corplms_appraisal'));

        $buttons = array();
        $buttons[] = $this->output->single_button(new moodle_url('/corplms/appraisal/message.php',
    ...
    public function display_assigned_groups($assignments, $itemid) {
        $tableheader = array(get_string('assigngrouptypename', 'corplms_appraisal'),
                             get_string('assignsourcename', 'corplms_appraisal'),
                             get_string('assignincludechildren', 'corplms_appraisal'),
                             get_string('assignnumusers', 'corplms_appraisal'),
                             get_string('actions'));
        ...
        if (empty($assignments)) {
            $emptycell = new html_table_cell(get_string('noassignments', 'corplms_appraisal'));
            ...
            foreach ($assignments as $assign) {
                ...
                if (!appraisal::is_closed($itemid)) {
                    $delete = $this->output->action_icon(
                            new moodle_url('/corplms/appraisal/learners.php',
    ...
    public function display_user_datatable() {
        ...
        $table->head = array(get_string('learner'),
                             get_string('assignedvia', 'corplms_core'),
                             );
    ...
    public function display_notlive_notice($appraisalid, $canassign = false) {
        ...
        $out = get_string('changesnotlive', 'corplms_appraisal');

        if ($canassign) {
            // Update learner assignments button.
            $updatestr = get_string('updatenow', 'corplms_appraisal');
            $updateparams = array('appraisalid' => $appraisalid, 'update' => true, 'sesskey' => sesskey());
            $updateurl = new moodle_url('/corplms/appraisal/learners.php', $updateparams);
    ...
    public function display_learner_warnings($appraisalid, $warnings, $canview = false) {
        ...
        if (!empty($warnings['learners'])) {
            ...
            $out = get_string('appraisalinvalid:learners', 'corplms_appraisal');
        } else if (!empty($warnings)) {
            // The only other warnings are role warnings.
            if ($canview) {
                // Add the link to list all the role warings.
                $infostr = get_string('missingroleslink', 'corplms_appraisal');
                $infourl = new moodle_url('/corplms/appraisal/missing.php', array('appraisalid' => $appraisalid));
                $infolink = html_writer::link($infourl->out(), $infostr, array('class' => 'missingroleslink'));

                $out = get_string('missingrolesinfo', 'corplms_appraisal', $infolink);
            } else {
                // Just the plain warning without a link.
                $out = get_string('missingroles', 'corplms_appraisal');
            }
        }
    ...
    public function display_appraisal_header($appraisal, $userassignment, $roleassignments, $preview = false) {
        ...
        foreach ($appraisal->get_roles_involved() as $role) {
            $rolestring = get_string($rolestringkeys[$role], 'corplms_appraisal');
            if (empty($roleassignments[$role]) || empty($roleassignments[$role]->userid)) {
                $participant = $rolestring . ": " . get_string('rolecurrentlyempty', 'corplms_appraisal');
            } else {
            ...
            if ($appraisal->status == appraisal::STATUS_ACTIVE && $roleassignments[$role]->userid != $actual[$role]) {
                $participant .= $this->container(get_string('rolehaschanged', 'corplms_appraisal'), 'rolechangedwarning');
            }

            $participants[] = $participant;
        }
        $out .= html_writer::tag('div', html_writer::tag('label', get_string('participants', 'corplms_appraisal')) .
                    html_writer::alist($participants), array('class' => 'appraisal-participants'));

        if (!$preview) {
            if (!empty($userassignment->timecompleted)) {
                $icon = $this->output->pix_icon('i/valid', get_string('completed', 'corplms_appraisal'));
                $out .= html_writer::tag('p', $icon . get_string('completedon', 'corplms_appraisal',
                    userdate($userassignment->timecompleted, get_string('strftimedate', 'langconfig'))));
            } else if ($appraisal->status == appraisal::STATUS_CLOSED) {
                $icon = $this->output->pix_icon('i/invalid', get_string('closed', 'corplms_appraisal'));
                $out .= html_writer::tag('p', $icon . get_string('closedon', 'corplms_appraisal',
                ...
                $out .= html_writer::tag('p', $icon . get_string('appraisalclosedforuser', 'corplms_appraisal', $userfullname));
            }
    ...
    public function display_missing_roles($userassignmentid, $appraisalid) {
        ...
        $out = html_writer::tag('p', get_string('rolesmissing', 'corplms_appraisal'));

        $allroles = appraisal::get_roles();
        $out .= html_writer::start_tag('ul');
        foreach ($missingroles as $role) {
            $out .= html_writer::tag('li', get_string($allroles[$role->appraisalrole], 'corplms_appraisal'));
        }
    ...
    public function display_appraisal_actions($appraisal, $userassignment, $showprint, $preview = false) {
        ...
        if ($preview || !$appraisal->is_locked($userassignment)) {
            $savepdfstr = get_string('savepdfsnapshot', 'corplms_appraisal');
        ...
        if ($showprint) {
            $printstr = get_string('print', 'corplms_appraisal');
    ...
    public function display_stage($appraisal, $stage, $userassignment, $roleassignment, $actions, $preview = false) {
        ...
        if (!empty($stage->timedue)) {
            $title .= html_writer::tag('div', get_string('completebydate', 'corplms_appraisal',
        ...
        $info = '';
        if ($preview) {
            $info .= html_writer::tag('h4', get_string('inprogress', 'corplms_appraisal'));
            ...
            $info .= html_writer::tag('h4', get_string('incomplete', 'corplms_appraisal'));
            if ($stageinprogress) {
            ...
            $info .= html_writer::tag('h4', get_string('overdue', 'corplms_appraisal'));
            $stagestatus = 'appraisal-stage-overdue';
        } else if ($stageinprogress) {
            $info .= html_writer::tag('h4', get_string('inprogress', 'corplms_appraisal'));
            $stagestatus = 'appraisal-stage-inprogress';
        } else if ($stage->is_completed($userassignment)) {
            $info .= html_writer::tag('h4', get_string('completed', 'corplms_appraisal'));
            $stagestatus = 'appraisal-stage-completed';
        } else {
            $info .= html_writer::tag('h4', get_string('unavailable', 'corplms_appraisal'));
            $stagestatus = 'appraisal-stage-locked';
        }

        $description = file_rewrite_pluginfile_urls($stage->description, 'pluginfile.php', $TEXTAREA_OPTIONS['context']->id,
                'corplms_appraisal', 'appraisal_stage', $stage->id);
        ...
        foreach ($rolescompletion as $rolecompletion) {
            ...
            if (isset($rolecompletion->timecompleted)) {
                $icon = $this->output->pix_icon('i/completion-manual-y', get_string('completed', 'corplms_appraisal'));
            } else {
                $icon = $this->output->pix_icon('i/completion-manual-n', get_string('incomplete', 'corplms_appraisal'));
            }
            // Text.
            if ($rolecompletion->appraisalrole == $roleassignment->appraisalrole) {
                $rolecomplete = get_string('rolecompleteyou', 'corplms_appraisal');
            } else if ($rolecompletion->appraisalrole == appraisal::ROLE_LEARNER) {
                $rolecomplete = get_string('rolecompleteuser', 'corplms_appraisal', fullname($userassignment->user));
            } else if ($userassignment->userid == $USER->id &&
                    (!$preview || $roleassignment->appraisalrole == appraisal::ROLE_LEARNER)) {
                $rolecomplete = get_string('rolecompleteyour', 'corplms_appraisal',
                        get_string($allroles[$rolecompletion->appraisalrole], 'corplms_appraisal'));
            } else {
                $a = new stdClass();
                $a->username = fullname($userassignment->user);
                $a->rolename = get_string($allroles[$rolecompletion->appraisalrole], 'corplms_appraisal');
                $rolecomplete = get_string('rolecompleteusers', 'corplms_appraisal', $a);
            }
    ...
    public function display_stage_actions_for_stages($appraisal, $stage, $userassignment, $roleassignment,
        ...
        $pagesurl = new moodle_url('/corplms/appraisal/myappraisal.php', $urlparams);

        $action = '';
        if ($preview) {
            $pagesurl->param('stageid', $stage->id);
            $button = new single_button($pagesurl, get_string('preview', 'corplms_appraisal'), 'get');
            $action .= $this->output->render($button);
        } else if ($userassignment->is_closed()) {
            $button = new single_button($pagesurl, get_string('view', 'corplms_appraisal'), 'get');
            $action .= $this->output->render($button);
        } else if ($stage->is_completed($userassignment)) {
            ...
            $action .= $this->output->pix_icon('tick2', get_string('completed', 'corplms_appraisal'), 'corplms_appraisal',
            ...
            if ($appraisal->status == appraisal::STATUS_CLOSED ||
                ...
                $button = new single_button($pagesurl, get_string('continue', 'corplms_appraisal'), 'get');
                $action .= $this->output->render($button);
            } else {
                $button = new single_button($pagesurl, get_string('start', 'corplms_appraisal'), 'get');
                $action .= $this->output->render($button);
            }
        }
    ...
    public function display_stage_actions_for_pages($showsaveprogress, $showcompletestage, $urlparams) {
        ...
        // Link back.
        $stagesstr = get_string('backtoappraisal', 'corplms_appraisal');
        $stagesurl = new moodle_url('/corplms/appraisal/myappraisal.php', array_merge($urlparams, array('action' => 'stages')));
        $actions .= $this->action_link($stagesurl, $stagesstr, null, array('class' => 'link-as-button'));
        $actions .= html_writer::empty_tag('br');

        // Save progress button.
        $saveprogressurl = new moodle_url('/corplms/appraisal/myappraisal.php', $urlparams);
        $saveprogressstr = get_string('saveprogress', 'corplms_appraisal');
        ...
        $completestageurl = new moodle_url('/corplms/appraisal/myappraisal.php', $urlparams);
        $completestagestr = get_string('completestage', 'corplms_appraisal');
    ...
    public function display_stages($appraisal, $stages, $roleassignment, $showprint, $preview = false) {
        ...
        if (empty($stages)) {
            return $out . get_string('nostages', 'corplms_appraisal');
        }
    ...
    public function display_pages($pages, $page, appraisal_role_assignment $roleassignment, $preview = false,
        ...
        if (empty($pages)) {
            return $out . get_string('nopagestoview', 'corplms_appraisal');
        }
        ...
        foreach ($pages as $tabpage) {
            ...
            if (!$pageiscomplete && !$preview) {
                ...
                $icon = $this->output->pix_icon('i/completion-auto-enabled', get_string('completed', 'corplms_appraisal'));
            }
            $rows[] = new tabobject($tabpage->id, new moodle_url('/corplms/appraisal/myappraisal.php',
    ...
    public function display_viewing_appraisal_header($subject = null, $customlangtext = '', $role = 0) {
        ...
        if ($role) {
            ...
            $a->rolename = get_string($roles[$role], 'corplms_appraisal');
        }

        $r = new html_table_row(array($this->output->user_picture($subject),
            get_string($langtext, 'corplms_appraisal', $a)));
    ...
    public function display_preview_header($appraisal, $role, $urlparams) {
        ...
        foreach ($roles as $roleid => $rolecode) {
            $roles[$roleid] = get_string($rolecode, 'corplms_appraisal');
        }
        ...
        $heading = html_writer::tag('h3', get_string('previewingappraisal', 'corplms_appraisal', $a));

        // The roles control.
        $previewurl = new moodle_url('/corplms/appraisal/myappraisal.php', $urlparams);
        $select = new single_select($previewurl, 'role', $roles);
        $select->set_label(get_string('previewappraisalas', 'corplms_appraisal'));
        ...
        // The message.
        $description = html_writer::tag('div', get_string('previewinfo', 'corplms_appraisal', $a->rolename));
