...
$assign = new corplms_assign_appraisal($module, $appraisal);
...
$returnurl = new moodle_url('/corplms/appraisal/learners.php', $returnparams);
...
if ($deleteid && $canassign && (!appraisal::is_closed($appraisalid))) {
    ...
    if (appraisal::is_active($appraisalid) && !$confirm) {
        ...
        $deleteurl = new moodle_url('/corplms/appraisal/learners.php', $deleteparams);
        ...
        $confirmstr = get_string('confirmdeletegroup', 'corplms_appraisal', $confirmparams);
...
// Setup the JS.
corplms_setup_assigndialogs($module, $appraisalid, $canviewusers, $notlivenotice);
echo $output->header();
if ($appraisal->id) {
...
echo $output->heading(get_string('assigncurrentgroups', 'corplms_appraisal'), 3);

if ($canassign) {
    if ($appraisal->status == appraisal::STATUS_CLOSED) {
        echo get_string('appraisalclosednochangesallowed', 'corplms_appraisal');
    } else {
        $options = array_merge(array("" => get_string('assigngroup', 'corplms_core')),
                $grouptypes);
...
echo $output->heading(get_string('assigncurrentusers', 'corplms_appraisal'), 3);
