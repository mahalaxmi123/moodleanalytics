...
$returnurl = new moodle_url('/corplms/appraisal/stage.php', array('appraisalid' => $appraisalid));
...
switch ($action) {
    case 'edit':
        ...
        $defaults = file_prepare_standard_editor($defaults, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
            'corplms_appraisal', 'appraisal_stage', $id);
        ...
        if ($isdraft && $fromform = $mform->get_data()) {
            ...
            if (!empty($fromform->timedue)) {
                ...
                if ($fromform->timedue < time()) {
                    corplms_set_notification(get_string('error:completebyinvalid', 'corplms_appraisal'));
                    break;
                }
            }
            $stage->set($fromform);
            ...
            corplms_set_notification(get_string('stageupdated', 'corplms_appraisal'), $returnurl,
                    array('class' => 'notifysuccess'));
        }
        break;
    case 'delete':
        if ($stage->id < 1) {
            corplms_set_notification(get_string('error:stagenotfound', 'corplms_appraisal'), $returnurl,
        ...
        if ($isdraft) {
            ...
            if ($confirm == 1) {
                ...
                $stage->delete();
                corplms_set_notification(get_string('deletedstage', 'corplms_appraisal'), $returnurl,
                        array('class' => 'notifysuccess'));
            }
        } else {
...
$PAGE->requires->string_for_js('confirmdeleteitem', 'corplms_appraisal');
$PAGE->requires->string_for_js('confirmdeleteitemwithredisplay', 'corplms_appraisal');
$PAGE->requires->string_for_js('error:cannotdelete', 'corplms_appraisal');
$PAGE->requires->string_for_js('addpage', 'corplms_appraisal');
$jsmodule = array(
    'name' => 'corplms_appraisal_stage',
    'fullpath' => '/corplms/appraisal/js/stage.js',
    'requires' => array('json'));

$args = array('args' => '{"sesskey":"'.sesskey().'"}');
$PAGE->requires->js_init_call('M.corplms_appraisal_stage.init', $args, false, $jsmodule);
...
switch ($action) {
    case 'delete':
        echo $output->heading(get_string('deletestage', 'corplms_appraisal', $stage->name),3);
    ...
    case 'edit':
        ...
        break;
    default:
        echo $output->heading(get_string('stages', 'corplms_appraisal'), 3);
