 ...
 */
function xmldb_corplms_appraisal_upgrade($oldversion) {
    ...
    if ($oldversion < 2013080501) {
        ...
        upgrade_plugin_savepoint(true, 2013080501, 'corplms', 'appraisal');
    }
    ...
    if ($oldversion < 2014061600) {
        ...
        if ($usercount > 0) {
            ...
            foreach ($deletedusers as $user) {
                ...
                corplms_appraisal_observer::user_deleted($event);
                $i++;
                $pbar->update($i, $usercount, "Fixing Appraisals for deleted users - {$i}/{$usercount}.");
            }
            $deletedusers->close();
        }
        upgrade_plugin_savepoint(true, 2014061600, 'corplms', 'appraisal');
    }
    ...
    if ($oldversion < 2014062000) {
        ...
        upgrade_plugin_savepoint(true, 2014062000, 'corplms', 'appraisal');
    }
    ...
    if ($oldversion < 2014090100) {
        ...
        foreach ($records as $record) {
            ...
            if ($timestring !== '23:59:59') {
                ...
                $datestring .= " 23:59:59";
                if ($newtimestamp = corplms_date_parse_from_format('Y-m-d H:i:s', $datestring)) {
                    $DB->set_field('appraisal_stage', 'timedue', $newtimestamp, array('id' => $record->id));
                }
            }
        }
        $transaction->allow_commit();
        upgrade_plugin_savepoint(true, 2014090100, 'corplms', 'appraisal');
    }
    ...
    if ($oldversion < 2014090801) {
        ...
        upgrade_plugin_savepoint(true, 2014090801, 'corplms', 'appraisal');
    }
    ...
    if ($oldversion < 2014090802) {
        ...
        upgrade_plugin_savepoint(true, 2014090802, 'corplms', 'appraisal');
    }
    ...
    if ($oldversion < 2014090803) {
        ...
        upgrade_plugin_savepoint(true, 2014090803, 'corplms', 'appraisal');
    }
