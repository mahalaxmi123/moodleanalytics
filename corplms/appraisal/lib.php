...
class appraisal {
    ...
    public function set_status($newstatus) {
        ...
        if (!in_array($newstatus, $allowedstatus[$this->status])) {
            ...
            throw new appraisal_exception(get_string('error:cannotchangestatus', 'corplms_appraisal', $a));
        } else {
    ...
    public function activate($time = 0) {
        ...
        $assign = new corplms_assign_appraisal('appraisal', $this);
        ...
        $event = \corplms_appraisal\event\appraisal_activation::create(
            array(
                'objectid' => $this->id,
    ...
    public function validate($time = null) {
        ...
        if ($this->status != self::STATUS_DRAFT) {
            $err['status'] = get_string('appraisalinvalid:status', 'corplms_appraisal');
        }
        ...
        if (empty($rolescananswer)) {
            $err['roles'] = get_string('appraisalinvalid:roles', 'corplms_appraisal');
        }
        ...
        foreach ($stages as $stage) {
            ...
            if ($checkstage->timedue && in_array($checkstage->timedue, $timesdue)) {
                $err['timedue'] = get_string('appraisalinvalid:stagesamedue', 'corplms_appraisal');
            }
    ...
    public function validate_roles($live = false) {
        ...
        if (!$learnercount) {
            $war['learners'] = get_string('appraisalinvalid:learners', 'corplms_appraisal');
            return $war;
        }
        ...
        foreach ($missingroles as $missingrole) {
            foreach ($rolesinvolved as $role) {
                ...
                if (empty($missingrole->$field)) {
                    ...
                    $a->role = get_string($allroles[$role], 'corplms_appraisal');
        ...
        if ($nummissing > APPRAISAL_VALIDATION_MAX_BAD_ROLES) {
            $war['missingrolesmore'] = get_string('xmoremissingroles', 'corplms_appraisal',
    ...
    public function close($formdata = null) {
        ...
        if (isset($formdata->sendalert) && $formdata->sendalert) {
            ...
            $alert->subject = get_string('closealerttitledefault', 'corplms_appraisal', $this);
            ...
            foreach ($roleusers as $roleuser) {
                ...
                $alert->fullmessage = get_string('closealertadminbody', 'corplms_appraisal', $formdata);
                $alert->fullmessagehtml = $alert->fullmessage;
    ...
    public function save_snapshot($filepath, $roleassignmentid, $time = 0) {
        ...
        $meta = array('contextid' => $context->id, 'component' => 'corplms_appraisal', 'filearea' => 'snapshot_'.$this->id,
            'itemid' => $roleassignmentid, 'filepath' => '/', 'filename' => $filename, 'timecreated' => $time,
            'userid' => $USER->id);
        $file = $fs->create_file_from_pathname($meta, $filepath);

        return moodle_url::make_pluginfile_url($context->id, 'corplms_appraisal', 'snapshot_'.$this->id, $roleassignmentid, '/',
    ...
    public static function list_snapshots($appraisalid, $roleassignmentid) {
        ...
        $files = $fs->get_area_files($context->id, 'corplms_appraisal', 'snapshot_'.$appraisalid, $roleassignmentid, 'timecreated',
                false);

        $snapshots = array();
        foreach ($files as $file) {
            $snapshot = new stdClass();
            $snapshot->url = moodle_url::make_pluginfile_url($context->id, 'corplms_appraisal', 'snapshot_'.$appraisalid,
    ...
    public function delete_all_snapshots() {
        $context = context_system::instance();
        $fs = get_file_storage();
        $fs->delete_area_files($context->id, 'corplms_appraisal', 'snapshot_', $this->id);
    }
    ...
    public function delete() {
        ...
        // Remove files.
        $fs = get_file_storage();
        $fs->delete_area_files($TEXTAREA_OPTIONS['context']->id, 'corplms_appraisal', 'appraisal', $this->id);
    ...
    public static function get_live_role_assignments($userid) {
        $manager = corplms_get_manager($userid);
        $teamleader = corplms_get_teamleader($userid);
        $appraiser = corplms_get_appraiser($userid);
    ...
    public static function display_status($status) {
        ...
        switch ($status) {
            case self::STATUS_ACTIVE:
                $result = get_string('active', 'corplms_appraisal');
                break;
            case self::STATUS_DRAFT:
                $result = get_string('draft', 'corplms_appraisal');
                break;
            case self::STATUS_CLOSED:
                $result = get_string('closed', 'corplms_appraisal');
                break;
            case self::STATUS_COMPLETED:
                $result = get_string('completed', 'corplms_appraisal');
                break;
        }
        return $result;
    }
        ...
        foreach ($stages as $stage) {
            ...
            foreach ($pages as $page) {
                ...
                foreach ($questionrecords as $questionrecord) {
                    ...
                    $a->type = $info['type'];
                    $item->name = get_string('questionandtype', 'corplms_question', $a);
    ...
    public static function delete_learner_assignments($userid) {
        ...
        foreach ($userassignments as $userassignment) {
            ...
            foreach ($roleassignments as $roleassignment) {
                ...
                $fs->delete_area_files($context->id, 'corplms_appraisal', 'snapshot_', $appraisalid, $roleassignmentid);
            }
    ...
    public static function get_manage_list() {
        ...
        foreach ($appraisals as $appraisal) {
            if ($appraisal->status == self::STATUS_DRAFT) {
                $assign = new corplms_assign_appraisal('appraisal', new appraisal($appraisal->id));
    ...
    public static function duplicate_appraisal($appraisalid, $daysoffset = 0) {
        ...
        require_capability('corplms/appraisal:cloneappraisal', $systemcontext);
        ...
        $assign = new corplms_assign_appraisal('appraisal', new appraisal($appraisalid));
    ...
    public static function check_feature_enabled() {
        if (corplms_feature_disabled('appraisals')) {
            print_error('appraisalsdisabled', 'corplms_appraisal');
        }
    }
...
class appraisal_stage {
    ...
    public function duplicate($appraisalid, $daysoffset = 0) {
        ...
        $data->descriptionformat = FORMAT_HTML;
        $data = file_prepare_standard_editor($data, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
            'corplms_appraisal', 'appraisal_stage', $srcstageid);

        $data = file_postupdate_standard_editor($data, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
            'corplms_appraisal', 'appraisal_stage', $newstage->id);
    ...
    public function validate($time = null) {
        ...
        if (empty($this->timedue) || ($this->timedue > 0 && $this->timedue < $time)) {
                $err['stagedue'.$this->id] = get_string('appraisalinvalid:stagedue', 'corplms_appraisal',
        ...
        if (empty($pages)) {
            $err['stageempty' . $this->id] = get_string('appraisalinvalid:stageempty', 'corplms_appraisal',
                        format_string($this->name));
        }
        ...
        if (empty($rolesinvolved)) {
            $err['stagenocananswer' . $this->id] =
                    get_string('appraisalinvalid:stagenoonecananswer', 'corplms_appraisal',
                        format_string($this->name));
        }
    ...
    public function get_may_answer() {
        ...
        foreach ($roles as $role) {
            $out[$role->appraisalrole] = get_string($strroles[$role->appraisalrole], 'corplms_appraisal');
        }
    ...
    public function complete_for_user($subjectid) {
        ...
        $event = \corplms_appraisal\event\appraisal_stage_completion::create(
            array(
    ...
    public function delete() {
        ...
        // Remove files.
        $fs = get_file_storage();
        $fs->delete_area_files($TEXTAREA_OPTIONS['context']->id, 'corplms_appraisal', 'appraisal_stage', $this->id);
...
class appraisal_page {
    ...
    public function save() {
        ...
        // Put default name.
        if (!$todb->name) {
            $todb->name = get_string('pagedefaultname', 'corplms_appraisal');
        }
    ...
    public function validate() {
        ...
        if (empty($questionrecords)) {
            $err['page' . $this->id] = get_string('appraisalinvalid:pageempty', 'corplms_appraisal',
                        format_string($this->name));
        }
        ...
        foreach ($questionrecords as $questionrecord) {
            ...
            if ($question->is_invalid_redisplay()) {
                $err['page' . $this->id . '_' . $question->id] = get_string('appraisalinvalid:redisplayfuture',
                        'corplms_appraisal', format_string($this->name));
            }
        }
    ...
    public function populate_roles_element(appraisal_role_assignment $roleassignment,
        ...
        foreach ($questroles as $eachrole => $rights) {
            if (isset($otherassignments[$eachrole]) && ($rights & appraisal::ACCESS_CANANSWER) == appraisal::ACCESS_CANANSWER) {
                ...
                $questioninfo->label = get_string('role_answer_' . $rolecodestrings[$eachrole], 'corplms_appraisal');
 ...
 */
function corplms_appraisal_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options=array()) {
    global $USER;
    $systemcontext = context_system::instance();
    ...
    $fs = get_file_storage();
    if (!$file = $fs->get_file($context->id, 'corplms_appraisal', $filearea, $assignmentid, '/', $filename)) {
        send_file_not_found();
    }
 ...
 */
function corplms_appraisal_install_example_appraisal() {
    global $CFG, $DB;
    require_once($CFG->dirroot . '/corplms/question/field/multichoice.class.php');
    ...
    $appraisal->name = get_string('example:appraisalname', 'corplms_appraisal');
    $appraisal->description = get_string('example:appraisaldescription', 'corplms_appraisal');
    $appraisal->status = 0;
    ...
    foreach (range(1, 3) as $num) {
        $stage = new stdClass();
        $stage->appraisalid = $appraisalid;
        $stage->name = get_string("example:stage{$num}name", 'corplms_appraisal');
        $stage->description = get_string("example:stage{$num}description", 'corplms_appraisal');
    ...
    foreach ($stageids as $stagenum => $stageid) {
        ...
        foreach (range(1, $pagesperstage[$stagenum]) as $pagenum) {
            $page = new stdClass();
            $page->appraisalstageid = $stageid;
            $page->name = get_string("example:stage{$stagenum}page{$pagenum}name", 'corplms_appraisal');
            $page->sortorder = $sort;
            $sort++;
    ...
    $scale = new stdClass();
    $scale->name = get_string('example:scaleyesnoname', 'corplms_appraisal');
    $admin = get_admin();
 ...
 */
function corplms_appraisal_cron() {
    global $CFG;
    require_once($CFG->dirroot . '/corplms/appraisal/cron.php');
    appraisal_cron();
}
