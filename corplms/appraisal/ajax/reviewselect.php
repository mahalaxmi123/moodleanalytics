...
require_once($CFG->dirroot.'/corplms/core/dialogs/dialog_content_plan.class.php');
require_once($CFG->dirroot.'/corplms/core/dialogs/dialog_content_goals.class.php');
require_once($CFG->dirroot.'/corplms/core/dialogs/dialog_content_required_learning.class.php');
require_once($CFG->dirroot.'/corplms/core/dialogs/dialog_content_plan_evidence.class.php');
...
if (!$roleassignment) {
    echo get_string('noassignments', 'corplms_appraisal');
} else {
    // Display page.
    if (!$nojs) {
        ...
        if ($datatype == 'goals') {
            ...
            $dialog = new corplms_dialog_content_goals($frameworkid, false, $subjectid);
            ...
            if (!$dialog->showcompany && !$dialog->showpersonal) {
                print_error('error:goalselectionmustallowsomething', 'corplms_question');
            ...
            $dialog = new corplms_dialog_content_required_learning($subjectid);
            $dialog->load_items();
        } else {
            if ($datatype == 'evidencefromplan') {
                $dialog = new corplms_dialog_content_plan_evidence('evidence', $planid, $showhidden, $subjectid);
            } else {
                $dialog = new corplms_dialog_content_plan($question->get_element()->component, $planid, $showhidden, $subjectid);
            }
        ...
        $dialog->lang_file = 'corplms_appraisal';
