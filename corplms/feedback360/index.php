...
if ($USER->id == $userid) {
    ...
    $viewrequestee = has_capability('corplms/feedback360:viewownreceivedfeedback360', $systemcontext);
    $viewrequested = has_capability('corplms/feedback360:viewownrequestedfeedback360', $systemcontext);
    $canmanage = has_capability('corplms/feedback360:manageownfeedback360', $systemcontext);
    if (!$viewrequestee && !$viewrequested) {
        // You can't see anything, you should't be accessing this page.
        print_error('error:accessdenied', 'corplms_feedback360');
    }
    ...
    $viewrequestee = has_capability('corplms/feedback360:viewstaffreceivedfeedback360', $usercontext);
    $viewrequested = has_capability('corplms/feedback360:viewstaffrequestedfeedback360', $usercontext);
    $canmanage = has_capability('corplms/feedback360:managestafffeedback', $usercontext);

    $userxfeedback = get_string('userxfeedback360', 'corplms_feedback360', fullname($user));
...
if ($viewrequestee) {
    ...
    if ($userid == $USER->id) {
        $user_title = get_string('feedback360aboutyou', 'corplms_feedback360');
    } else {
        $fullname = fullname($user);
        $user_title = get_string('feedback360aboutuser', 'corplms_feedback360', $fullname);
    }
...
if ($viewrequested) {
    ...
    if ($USER->id == $userid) {
        $colleagues_title = get_string('feedback360aboutcolleagues', 'corplms_feedback360');
    } else {
        $colleagues_title = get_string('viewuserxresponses', 'corplms_feedback360', fullname($user));
    }
