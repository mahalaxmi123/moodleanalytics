...
require_once($CFG->dirroot . '/corplms/feedback360/feedback360_forms.php');
...
$strrequestfeedback = get_string('requestfeedback360', 'corplms_feedback360');

// Set up the page.
$PAGE->set_url(new moodle_url('/corplms/feedback360/index.php'));
...
if ($USER->id == $userid) {
    ...
    $PAGE->set_corplms_menu_selected('feedback360');
...
$PAGE->requires->js('/corplms/feedback360/js/preview.js', false);
...
if ($action == 'form') {
    ...
    if (count($available_forms) == 0) {
        print_error('noavailableforms', 'corplms_feedback360');
    } else if (count($available_forms) == 1) {
        $form = array_pop($available_forms);
        redirect("{$CFG->wwwroot}/corplms/feedback360/request.php?userid={$userid}&action=users&formid={$form->assigid}");
    } else {
    ...
    if ($feedback360->status != feedback360::STATUS_ACTIVE) {
        print_error('error:feedbacknotactive', 'corplms_feedback360');
    }
    ...
    $PAGE->requires->strings_for_js(array('addsystemusers'), 'corplms_feedback360');

    // Include position user js modules.
    $jsmodule = array('name' => 'corplms_requestfeedback',
                      'fullpath' => '/corplms/feedback360/js/request.js',
                      'requires' => array('json')
                     );
    $PAGE->requires->js('/corplms/feedback360/js/delete.js', false);
    $PAGE->requires->js_init_call('M.corplms_requestfeedback.init', $args, false, $jsmodule);
    ...
    print_error('error:unrecognisedaction', 'corplms_feedback360', null, $action);
}

// Handle forms being submitted.
if ($mform->is_cancelled()) {
    $cancelurl = new moodle_url('/corplms/feedback360/index.php', array('userid' => $userid));
    redirect($cancelurl);
} else if ($data = $mform->get_data()) {
    if ($action == 'form') {
        $params = array('userid' => $userid, 'action' => 'users', 'formid' => $data->formid);
        $url = $CFG->wwwroot . '/corplms/feedback360/request.php';
        ...
        if (!empty($data->addsystemusers)) {
            ...
            $findurl = '/corplms/feedback360/request/find.php';
        ...
        if (!empty($newsystem) || !empty($cancelsystem) || !empty($newemail) || !empty($cancelemail) ||
            ...
            $url = new moodle_url('/corplms/feedback360/request.php', $params);
            redirect($url);
        } else {
            $params = array('userid' => $data->userid,
                'formid' => $data->formid,
            ...
            corplms_set_notification('No changes to be made', $url, array('class' => 'notifysuccess'));
        }
    } else if ($action == 'confirm') {
        ...
        if ($data->duenotifications && !empty($systemkeep)) {
            ...
            foreach ($systemkeep as $uid) {
                ...
                $event->userto = $userto;
                if ($asmanager) {
                    $event->subject = $stringmanager->get_string('managerupdatesubject', 'corplms_feedback360',
                            $strvars, $userto->lang);
                    $event->fullmessage = $stringmanager->get_string('managerupdatealert', 'corplms_feedback360',
                            $strvars, $userto->lang);
                    $event->fullmessagehtml = $stringmanager->get_string('managerupdatealert', 'corplms_feedback360',
                            $strvars, $userto->lang);
                } else {
                    $event->subject = $stringmanager->get_string('updatesubject', 'corplms_feedback360',
                            $strvars, $userto->lang);
                    $event->fullmessage = $stringmanager->get_string('updatealert', 'corplms_feedback360',
                            $strvars, $userto->lang);
                    $event->fullmessagehtml = $stringmanager->get_string('updatealert', 'corplms_feedback360',
                            $strvars, $userto->lang);
                }
                tm_alert_send($event);
            }
        ...
        if ($data->duenotifications && !empty($emailkeep)) {
            ...
            if ($asmanager) {
                $emailsubject = get_string('managerupdatesubject', 'corplms_feedback360', $strvars);
                $email_str = get_string('managerupdateemail', 'corplms_feedback360', $strvars);
                $sendas = $USER;
            } else {
                $emailsubject = get_string('updatesubject', 'corplms_feedback360', $strvars);
                $email_str = get_string('updateemail', 'corplms_feedback360', $strvars);
                $sendas = $userfrom;
            }
        ...
        if (empty($emailkeep) && empty($emailcancel) && empty($systemkeep) && empty($systemcancel)) {
            $successstr = get_string('requestcreatedsuccessfully', 'corplms_feedback360');
        } else {
            $successstr = get_string('requestupdatedsuccessfully', 'corplms_feedback360');
        }
        $returnurl = new moodle_url('/corplms/feedback360/index.php', array('userid' => $userid));
        corplms_set_notification($successstr, $returnurl, array('class' => 'notifysuccess'));
    } else {
        print_error('error:unrecognisedaction', 'corplms_feedback360', null, $action);
    }
}
