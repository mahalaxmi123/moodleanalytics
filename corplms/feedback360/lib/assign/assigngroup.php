...
$assignclassname = "corplms_assign_{$module}";
require_once($CFG->dirroot."/corplms/{$module}/lib.php");
...
if (!in_array($grouptype, $grouptypes)) {
    ...
    $a->module = $module;
    print_error('error:assignmentgroupnotallowed', 'corplms_core', null, $a);
}
...
if ($add) {
    ...
    if (!empty($listofvalues) && $grouptypeobj->validate_item_selector($listofvalues)) {
        ...
        $output = $PAGE->get_renderer("corplms_{$module}");
