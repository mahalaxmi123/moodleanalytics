...
$assign = new corplms_assign_feedback360($module, $feedback360);

// Capability checks.
$systemcontext = context_system::instance();
$canassign = has_capability('corplms/feedback360:assignfeedback360togroup', $systemcontext);
$canviewusers = has_capability('corplms/feedback360:viewassignedusers', $systemcontext);
...
// Setup the JS.
corplms_setup_assigndialogs($module, $itemid, $canviewusers);
...
echo $output->heading(get_string('assigncurrentgroups', 'corplms_feedback360'));

if ($canassign) {
    if ($feedback360->status == feedback360::STATUS_DRAFT) {
        $options = array_merge(array("" => get_string('assigngroup', 'corplms_core')), $assign->get_assignable_grouptype_names());
        ...
        echo get_string('feedback360closednochangesallowed', 'corplms_feedback360');
    } else {
        echo get_string('feedback360activenochangesallowed', 'corplms_feedback360');
    }
}
...
echo $output->heading(get_string('assigncurrentusers', 'corplms_feedback360'));
