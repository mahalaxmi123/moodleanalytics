...
require_once($CFG->dirroot . '/corplms/question/lib.php');
require_once($CFG->dirroot . '/corplms/feedback360/lib/assign/lib.php');
...
class feedback360 {
    ...
    public function delete() {
        ...
        $assign = new corplms_assign_feedback360('feedback360', $this);
        $assign->delete();

        // Delete files.
        $fs = get_file_storage();
        $fs->delete_area_files($TEXTAREA_OPTIONS['context']->id, 'corplms_feedback360', 'feedback360', $this->id);
    ...
    public function load() {
        ...
        if (!$feedback360) {
            throw new feedback360_exception(get_string('loadfeedback360failure', 'corplms_feedback360'), 1);
        }
    ...
    public static function get_manage_list($userid = 0) {
        ...
        require_capability('corplms/feedback360:managefeedback360', $context);
    ...
    public static function duplicate($feedback360id) {
        ...
        $context = context_system::instance();
        require_capability('corplms/feedback360:clonefeedback360', $context);
        ...
        $assign = new corplms_assign_feedback360('feedback360', new feedback360($feedback360id));
    ...
    public function set_status($newstatus) {
        ...
        if (!in_array($newstatus, $allowedstatus[$this->status])) {
            ...
            throw new feedback360_exception(get_string('error:cannotchangestatus', 'corplms_feedback360', $a));
        } else {
    ...
    public static function cancel_resp_assignment($resp_assignment, $asmanager = false) {
        ...
        if (!is_object($resp_assignment)) {
            print_error('error:unexpectedtype', 'corplms_feedback360');
        }
        ...
        if (!empty($resp_assignment->feedback360emailassignmentid)) {
            ...
            if ($asmanager) {
                $subject = get_string('managercancellationsubject', 'corplms_feedback360', $stringvars);
                $message = get_string('managercancellationemail', 'corplms_feedback360', $stringvars);
            } else {
                $subject = get_string('cancellationsubject', 'corplms_feedback360', $stringvars);
                $message = get_string('cancellationemail', 'corplms_feedback360', $stringvars);
            }
            // Send a cancellation email.
            $userto = \corplms_core\corplms_user::get_external_user($email);
            ...
            $event->userto = $userto;
            if ($asmanager) {
                $event->subject = $stringmanager->get_string('managercancellationsubject', 'corplms_feedback360',
                        $stringvars, $userto->lang);
                $event->fullmessage = $stringmanager->get_string('managercancellationalert', 'corplms_feedback360',
                        $stringvars, $userto->lang);
                $event->fullmessagehtml = $stringmanager->get_string('managercancellationalert', 'corplms_feedback360',
                        $stringvars, $userto->lang);
            } else {
                $event->subject = $stringmanager->get_string('cancellationsubject', 'corplms_feedback360',
                        $stringvars, $userto->lang);
                $event->fullmessage = $stringmanager->get_string('cancellationalert', 'corplms_feedback360',
                        $stringvars, $userto->lang);
                $event->fullmessagehtml = $stringmanager->get_string('cancellationalert', 'corplms_feedback360',
                        $stringvars, $userto->lang);
            }
    ...
    public static function display_status($status) {
        switch ($status) {
            case self::STATUS_DRAFT:
                $result = get_string('draft', 'corplms_feedback360');
                break;
            case self::STATUS_ACTIVE:
                $result = get_string('active', 'corplms_feedback360');
                break;
            case self::STATUS_CLOSED:
                $result = get_string('closed', 'corplms_feedback360');
                break;
            case self::STATUS_COMPLETED:
                $result = get_string('completed', 'corplms_feedback360');
                break;
        }
    ...
    public static function can_view_feedback360s($userid = null) {
        ...
        if (has_capability('corplms/feedback360:viewownreceivedfeedback360', $systemcontext, $userid)) {
        ...
        if (has_capability('corplms/feedback360:viewownrequestedfeedback360', $systemcontext, $userid)) {
        ...
        if (has_capability('corplms/feedback360:viewstaffreceivedfeedback360', $usercontext, $userid)) {
    ...
    private function create_answers_table() {
        ...
        if ($this->id < 1) {
            throw new feedback360_exception(get_string('error:feedbacktablecreation', 'corplms_feedback360'), 4);
        }
    ...
    public function activate() {
        if (!in_array($this->status, array(self::STATUS_DRAFT, self::STATUS_CLOSED))) {
            throw new feedback360_exception(get_string('error:activationstatus', 'corplms_feedback360'), 5);
        }
    ...
    public function validate() {
        ...
        if (!$is_question) {
            $errors['questions'] = get_string('error:questionsrequired', 'corplms_feedback360');
        }
        ...
        if ($learners == 0) {
            $errors['learners'] = get_string('error:learnersrequired', 'corplms_feedback360');
        }
        // Check recipients.
        if ($this->recipients < 1) {
            $errors['recipients'] = get_string('error:recipientsrequired', 'corplms_feedback360');
        }
    ...
    public static function check_managing_assigned($feedback360id, $userid) {
        ...
        $staff = corplms_get_staff($userid);
    ...
    public static function check_feature_enabled() {
        if (corplms_feature_disabled('feedback360')) {
            print_error('feedback360disabled', 'corplms_feedback360');
        }
    }
}
...
class feedback360_responder {
    ...
    public static function by_user($userid, $feedback360id, $subjectid) {
        ...
        if (!$data = $DB->get_record_sql($sql, array($userid, $feedback360id, $subjectid))) {
            print_error('error:respassignmentaccess', 'corplms_feedback360');
        }
    ...
    public static function update_system_assignments($new, $cancel, $userformid, $duedate, $asmanager = false) {
        ...
        foreach ($new as $userid) {
            ...
            $url = new moodle_url('/corplms/feedback360/feedback.php', $params);
            ...
            if ($asmanager) {
                $eventdata->subject = $stringmanager->get_string('manageremailrequestsubject', 'corplms_feedback360',
                        $taskvars, $userto->lang);
                $eventdata->fullmessage = $stringmanager->get_string('manageremailrequeststr', 'corplms_feedback360',
                        $taskvars, $userto->lang);
            } else {
                $eventdata->subject = $stringmanager->get_string('emailrequestsubject', 'corplms_feedback360',
                        $taskvars, $userto->lang);
                $eventdata->fullmessage = $stringmanager->get_string('emailrequeststr', 'corplms_feedback360',
                        $taskvars, $userto->lang);
            }
    ...
    public static function update_external_assignments($newassignments, $cancellations, $userformid, $duedate, $asmanager = false) {
        ...
        foreach ($newassignments as $email) {
            ...
            if ($asmanager) {
                $emailplain = get_string('manageremailrequeststr', 'corplms_feedback360', $emailvars);
                $emailhtml = get_string('manageremailrequesthtml', 'corplms_feedback360', $emailvars);
                $emailsubject = get_string('manageremailrequestsubject', 'corplms_feedback360', $emailvars);
            } else {
                $emailplain = get_string('emailrequeststr', 'corplms_feedback360', $emailvars);
                $emailhtml = get_string('emailrequesthtml', 'corplms_feedback360', $emailvars);
                $emailsubject = get_string('emailrequestsubject', 'corplms_feedback360', $emailvars);
            }
...
class feedback360_event_handler {
     ...
     *
     * @param \corplms_appraisal\event\appraisal_user_deleted $event
     */
 ...
 */
function corplms_feedback360_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options=array()) {
    global $USER;
    ...
    $fs = get_file_storage();
    if (!$file = $fs->get_file($context->id, 'corplms_feedback360', $filearea, $assignmentid, '/', $filename)) {
        send_file_not_found();
    }
...
class request_feedback_potential_user_selector extends user_selector_base {
    ...
    public function find_users($search) {
        ...
        if ($search) {
            $group = get_string('potentialusersmatching', 'corplms_feedback360', $search);
        } else {
            $group = get_string('potentialusers', 'corplms_feedback360');
        }
...
class request_feedback_current_user_selector extends user_selector_base {
    ...
    public function find_users($search) {
        ...
        if ($search) {
            $group = get_string('currentusersmatching', 'corplms_feedback360', $search);
        } else {
            $group = get_string('currentusers', 'corplms_feedback360');
        }
