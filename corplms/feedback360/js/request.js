...
M.corplms_requestfeedback = M.corplms_requestfeedback || {
    ...
    init: function(Y, args){
        ...
        if (typeof $ === 'undefined') {
            throw new Error('M.corplms_requestfeedback.init()-> jQuery dependency required for this module to function.');
        }

        // Select users to request feedback from, javascript dialog.
        (function() {
            var url = M.cfg.wwwroot+ '/corplms/feedback360/request';
            var saveurl = url + '/update.php';
            var findurl = url + '/find.php';
            var uid = M.corplms_requestfeedback.config.userid;
            var fid = M.corplms_requestfeedback.config.formid;
            feedback360Dialog(
                'systemrequest',
                M.util.get_string('addsystemusers', 'corplms_feedback360'),
                findurl + '?userid=' + uid,
...
feedback360Dialog = function(name, title, find_url, save_url) {
    ...
    var existing = M.corplms_requestfeedback.config.existing;
    ...
    corplmsDialogs[name].saveurl = save_url;
    corplmsDialogs[name].findurl = find_url;

    /**
     * Open dialog and load external page
     * @return  void
     */
    corplmsDialogs[name].open = function() {
...
feedback360_handler.prototype = new corplmsDialog_handler_treeview_multiselect();
