...
/**
 * Output renderer for corplms_feedback360s module
 */
class corplms_feedback360_renderer extends plugin_renderer_base {
    ...
    public function create_feedback360_button() {
        return $this->output->single_button(new moodle_url('/corplms/feedback360/general.php'),
            get_string('createfeedback360', 'corplms_feedback360'), 'get');
    }

    public function preview_feedback360_button($feedback360id) {
        $preview_params = array('feedback360id' => $feedback360id, 'preview' => 1);
        $preview_url = new moodle_url('/corplms/feedback360/feedback.php', $preview_params);
        $preview_str = get_string('preview', 'corplms_feedback360');
    ...
    public function request_feedback360_button($userid) {
        ...
        $request_url = new moodle_url('/corplms/feedback360/request.php', $request_params);
        $request_str = get_string('requestfeedback360', 'corplms_feedback360');
    ...
    public function feedback360_manage_table($feedback360s = array(), $userid = null) {
        ...
        if (empty($feedback360s)) {
            return get_string('nofeedback360s', 'corplms_feedback360');
        }

        $tableheader = array(get_string('name', 'corplms_feedback360'),
                             get_string('assignments', 'corplms_feedback360'),
                             get_string('status', 'corplms_feedback360'),
                             get_string('options', 'corplms_feedback360'));
        ...
        $stractivate = get_string('activate', 'corplms_feedback360');
        $strclose = get_string('close', 'corplms_feedback360');
        $strsettings = get_string('settings', 'corplms_feedback360');
        $strdelete = get_string('delete', 'corplms_feedback360');
        ...
        foreach ($feedback360s as $feedback360) {
            ...
            $activateurl = new moodle_url('/corplms/feedback360/activation.php',
                    array('id' => $feedback360->id, 'action' => 'activate'));
            $closeurl = new moodle_url('/corplms/feedback360/activation.php',
                    array('id' => $feedback360->id, 'action' => 'close'));
            $editurl = new moodle_url('/corplms/feedback360/general.php',
                    array('id' => $feedback360->id));
            $deleteurl = new moodle_url('/corplms/feedback360/manage.php',
                    array('id' => $feedback360->id, 'action' => 'delete'));
            $cloneurl = new moodle_url('/corplms/feedback360/manage.php',
                    array('id' => $feedback360->id, 'action' => 'copy'));

            $row = array();
            if (has_capability('corplms/feedback360:managefeedback360', $systemcontext, $userid)) {
                $row[] = html_writer::link($editurl, $name);
            } else {
                $row[] = $name;
            }

            $assign = new corplms_assign_feedback360('feedback360', new feedback360($feedback360->id));
            $countassignments = $assign->get_current_users_count();
            if ($feedback360->status == feedback360::STATUS_DRAFT) {
                $row[] = get_string('assignedtoxdraftusers', 'corplms_feedback360', $countassignments);
            } else {
                $row[] = get_string('assignedtoxusers', 'corplms_feedback360', $countassignments);
            }
            ...
            if (has_capability('corplms/feedback360:managefeedback360', $systemcontext, $userid)) {
                ...
                if ($feedback360->status == feedback360::STATUS_ACTIVE) {
                    $edit_error = get_string('error:feedback360noteditable', 'corplms_feedback360');
                    $edit = $this->output->pix_icon('/t/edit_gray', $edit_error, 'moodle', array('class' => 'disabled iconsmall'));

                    $delete_error = get_string('error:feedback360isactive', 'corplms_feedback360');
            ...
            $activate = '';
            if (has_capability('corplms/feedback360:manageactivation', $systemcontext, $userid)) {
                if ($feedback360->status == feedback360::STATUS_ACTIVE) {
    ...
    public function feedback360_management_tabs($feedback360id, $currenttab = 'general') {
        ...
        $systemcontext = context_system::instance();
        if (has_capability('corplms/feedback360:managefeedback360', $systemcontext)) {
            $row[] = new tabobject('general', $CFG->wwwroot . '/corplms/feedback360/general.php?id='
                    . $feedback360id, get_string('general'));
        }
        if (has_capability('corplms/feedback360:managepageelements', $systemcontext)) {
            $row[] = new tabobject('content', $CFG->wwwroot . '/corplms/feedback360/content.php?feedback360id='
                    . $feedback360id, get_string('content', 'corplms_feedback360'));
        }
        $capabilities = array('corplms/feedback360:viewassignedusers', 'corplms/feedback360:assignfeedback360togroup');
        if (has_any_capability($capabilities, $systemcontext)) {
            $row[] = new tabobject('assignments', $CFG->wwwroot . '/corplms/feedback360/assignments.php?id='
                    . $feedback360id, get_string('assignments', 'corplms_feedback360'));
        }
    ...
    public function display_assigned_groups($assignments, $itemid) {
        $tableheader = array(get_string('assigngrouptype', 'corplms_feedback360'),
                             get_string('assignsourcename', 'corplms_feedback360'),
                             get_string('assignincludechildren', 'corplms_feedback360'),
                             get_string('assignnumusers', 'corplms_feedback360'),
                             get_string('actions'));
        ...
        if (empty($assignments)) {
            $table->data[] = array(get_string('nogroupassignments', 'corplms_feedback360'));
        } else {
            foreach ($assignments as $assign) {
                ...
                if ($feedback360->status == feedback360::STATUS_DRAFT) {
                    $delete = $this->output->action_icon(
                            new moodle_url('/corplms/feedback360/assignments.php',
    ...
    public function display_feedback_header(feedback360_responder $resp, $subjectuser) {
        ...
        $r = new html_table_row(array($this->output->user_picture($subjectuser, array('link' => false)),
            get_string($titlestr, 'corplms_feedback360', $a)));
        ...
        if (!$resp->is_completed() && !$resp->is_fake()) {
            $savebutton = new single_button(new moodle_url('#'), get_string('saveprogress', 'corplms_feedback360'));
    ...
    public function display_preview_feedback_header(feedback360_responder $resp, $feedbackname) {
        $headerstr = get_string('previewheader', 'corplms_feedback360', $feedbackname);
        $subheader = get_string('previewsubheader', 'corplms_feedback360');
    ...
    public function display_userview_header($user) {
        ...
        if ($USER->id != $user->id) {
            ...
            $viewstr = html_writer::tag('strong', get_string('viewinguserxfeedback360', 'corplms_feedback360', $link));
    ...
    public function display_user_datatable() {
        ...
        $table->head = array(get_string('learner'), get_string('assignedvia', 'corplms_core'));
    ...
    public function feedback360_additional_actions($status, $id) {
        $activateurl = new moodle_url('/corplms/feedback360/activation.php', array('id' => $id, 'action' => 'activate'));
        $closeurl = new moodle_url('/corplms/feedback360/activation.php', array('id' => $id, 'action' => 'close'));

        $strstatusnow = feedback360::display_status($status);
        $strstatusat = get_string('statusat', 'corplms_feedback360');
        $feedback360 = new feedback360($id);

        $preview = $this->preview_feedback360_button($id);
        $activate = '';
        if ($feedback360->status == feedback360::STATUS_ACTIVE) {
            $activate = $this->output->action_link($closeurl, get_string('closenow', 'corplms_feedback360'));
        } else if ($feedback360->status == feedback360::STATUS_DRAFT || $feedback360->status == feedback360::STATUS_CLOSED) {
            $activate = $this->output->action_link($activateurl,  get_string('activatenow', 'corplms_feedback360'));
        }
    ...
    public function confirm_delete_feedback360 (feedback360 $feedback360) {
        $msg = get_string('deletefeedback360s', 'corplms_feedback360', $feedback360->name);

        if ($feedback360->status != feedback360::STATUS_DRAFT) {
            $questions = $feedback360->fetch_questions();
            if (count($questions) > 0) {
                $msg .= html_writer::empty_tag('br');
                $msg .= get_string('deletefeedback360questions', 'corplms_feedback360');
            }
            $assign = new corplms_assign_feedback360('feedback360', $feedback360);
            $assignments = $assign->get_current_users_count();
            if ($assignments > 0) {
                $msg .= html_writer::empty_tag('br');
                $msg .= get_string('deletefeedback360assignments', 'corplms_feedback360');
            }
        }
        ...
        $continue = new moodle_url('/corplms/feedback360/manage.php', $params);
        $cancel = new moodle_url('/corplms/feedback360/manage.php');
    ...
    public function confirm_question_delete(feedback360_question $question) {
        $msg = get_string('confirmdeletequestion', 'corplms_feedback360', $question->name);
        ...
        $continue = new moodle_url('/corplms/feedback360/content.php', $params);
        $cancel = new moodle_url('/corplms/feedback360/content.php', array('feedback360id' => $question->feedback360id));
    ...
    public function confirm_activation_feedback360 ($feedback360, $errors) {
        ...
        if (!empty($errors)) {
            $out = $this->heading(get_string('error:activationconfirmation', 'corplms_feedback360'));
            $out .= html_writer::tag('p', get_string('feedback360fixerrors', 'corplms_feedback360'));
            $errordesc = array();
            ...
            $buttons = array();
            $buttons[] = $this->output->single_button(new moodle_url('/corplms/feedback360/content.php',
                    array('feedback360id' => $feedback360->id)), get_string('backtofeedback360', 'corplms_feedback360',
                            $feedback360->name), 'get');
            $out .= html_writer::tag('div', implode(' ', $buttons), array('class' => 'buttons'));
            return $out;
        } else {
            $msg = get_string('confirmactivatefeedback360', 'corplms_feedback360', $feedback360->name);
            ...
            $continueurl = new moodle_url('/corplms/feedback360/activation.php', $params);
            $cancelurl = new moodle_url('/corplms/feedback360/manage.php');
    ...
    public function confirm_close_feedback360 ($feedback360) {
        $msg = get_string('confirmclosefeedback360', 'corplms_feedback360', $feedback360->name);
        $params = array('id' => $feedback360->id,
                            'action' => 'close',
                            'confirm' => 1,
                            'sesskey' => sesskey()
        ...
        if ($quests) {
            ...
            foreach ($quests as $quest) {
                ...
                if ($quest->id != $first->id) {
                    $posupurl = new moodle_url('/corplms/feedback360/content.php', array('action' => 'posup',
                ...
                if ($quest->id != $last->id) {
                    $posdownurl = new moodle_url('/corplms/feedback360/content.php', array('action' => 'posdown',
                            'id' => $quest->id, 'feedback360id' => $feedback360->id, 'sesskey' => sesskey()));
                ...
                $editurl = new moodle_url('/corplms/feedback360/content.php', array('action' => 'edit',
                    'id' => $quest->id, 'feedback360id' => $feedback360->id));
                $cloneurl = new moodle_url('/corplms/feedback360/content.php', array('action' => 'clone',
                    'id' => $quest->id, 'feedback360id' => $feedback360->id));
                $deleteurl = new moodle_url('/corplms/feedback360/content.php', array('action' => 'delete',
                    'id' => $quest->id, 'feedback360id' => $feedback360->id));;
    ...
    public function myfeedback_user_table($userid, $user_assignments, $canmanage) {
        ...
        $user_table->head = array(
            get_string('name', 'corplms_feedback360'),
            get_string('responses', 'corplms_feedback360'),
            get_string('duedate', 'corplms_feedback360')
        );
        if ($canmanage) {
            $user_table->head[] = get_string('options', 'corplms_feedback360');
        }
        ...
        foreach ($user_assignments as $user_assignment) {
            ...
            if (!empty($requests)) {
                ...
                $nameurl = new moodle_url('/corplms/feedback360/request/view.php',
                ...
                if ($canmanage) {
                    ...
                    $editurl = new moodle_url('/corplms/feedback360/request.php', $editparams);
                    ...
                    $remindurl = new moodle_url('/corplms/feedback360/request/remind.php', $remindparams);
                    $remindstr = get_string('remind', 'corplms_feedback360');
                    ...
                    $cancelurl = new moodle_url('/corplms/feedback360/request/stop.php', $cancelparams);
                    $cancelstr = get_string('stop', 'corplms_feedback360');
                    ...
                    if ($user_assignment->status == feedback360::STATUS_ACTIVE) {
                        ...
                        $options = get_string('closed', 'corplms_feedback360');
                    }
                }

                $respond = get_string('responsecount', 'corplms_feedback360', $res);
                if (!empty($newresponses)) {
                    $respond .= html_writer::tag('strong', get_string('responsecountnew', 'corplms_feedback360', $res));
                }
        ...
        if ($nodata) {
            $cell = new html_table_cell(get_string('nofeedback360requested', 'corplms_feedback360'));
    ...
    public function myfeedback_colleagues_table($userid, $resp_assignments) {
        ...
        if (empty($resp_assignments)) {
            $cell = new html_table_cell(get_string('nofeedback360togive', 'corplms_feedback360'));
            ...
            foreach ($resp_assignments as $resp_assignment) {
                ...
                $answerurl = new moodle_url('/corplms/feedback360/feedback.php', $answerparams);
                if (!empty($completed)) {
                    // Complete.
                    $status = get_string('completed', 'corplms_feedback360');

                    $revstr = ($USER->id == $userid) ? 'reviewnow' : 'reviewnowmanager';
                    $options = $this->output->action_link($answerurl, get_string($revstr, 'corplms_feedback360'));
                } else {
                    if (!$hidebutton) {
                        $options = $this->output->single_button($answerurl, get_string('answernow', 'corplms_feedback360'), 'get');
                    } else {
                        $options = '';
                    }
                    if (empty($resp_assignment->timedue)) {
                        ...
                        // Overdue.
                        $status = get_string('overdue', 'corplms_feedback360');
                    } else {
                        // Pending.
                        $status = get_string('pending', 'corplms_feedback360');
                    }
                }
    ...
    public function view_request_infotable($user_assignment, $resp_assignments) {
        ...
        $request_infotable->head = array(
            get_string('nameemail', 'corplms_feedback360'),
            get_string('completed', 'corplms_feedback360'),
            get_string('response', 'corplms_feedback360'),
        );
        ...
        foreach ($resp_assignments as $resp_assignment) {
            if (!empty($resp_assignment->timecompleted)) {
                ...
                if (empty($resp_assignment->viewed)) {
                    ...
                    $view_str = html_writer::tag('strong', get_string('viewresponse', 'corplms_feedback360'));
                } else {
                    $view_str = get_string('viewresponse', 'corplms_feedback360');
                }

                $responseparam = array('myfeedback' => 1, 'responseid' => $resp_assignment->id);
                $responseurl = new moodle_url('/corplms/feedback360/feedback.php', $responseparam);
                $responselink = html_writer::link($responseurl, $view_str);
            } else {
                $comp_str = get_string('notcompleted', 'corplms_feedback360');
                $responselink = '';
            }
            ...
            if (empty($resp_assignment->feedback360emailassignmentid)) {
                ...
                if (empty($resp_assignment->email)) {
                    $name_str = get_string('emailmissing', 'corplms_feedback360');
                } else {
    ...
    public function system_user_record($user, $userform, $resp) {
        ...
        $completestr = get_string('alreadyreplied', 'corplms_feedback360');
    ...
    public function external_user_record($email, $userform, $resp) {
        ...
        $deleteurl = new moodle_url('/corplms/feedback360/request/delete.php', $deleteparams);
    ...
    public function nojs_feedback_request_users($selected, $returnurl, $add_user_selector, $remove_user_selector) {
        ...
        $existing_cell = new html_table_cell(html_writer::tag('p', html_writer::tag('label',
                get_string('currentrequestees', 'corplms_feedback360'),
        ...
        $potential_cell = new html_table_cell(html_writer::tag('p', html_writer::tag('label',
                get_string('potentialrequestees', 'corplms_feedback360'),
        ...
        $backbutton_cell = new html_table_cell(html_writer::empty_tag('input',
                array('type' => 'submit', 'name' => 'cancel',
                    'value' => get_string('backtofeedbackrequest', 'corplms_feedback360'))));
