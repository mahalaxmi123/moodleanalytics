...
$returnurl = new moodle_url('/corplms/feedback360/index.php');
...
if ($isexternaluser) {
    ...
    $returnurl = new moodle_url('/corplms/feedback360/feedback.php', array('token' => $token));
    if ($respassignment) {
    ...
    $canmanage = has_capability('corplms/feedback360:managefeedback360', $systemcontext);
    ...
    if (!$canmanage && !$assigned && empty($manager)) {
        print_error('error:previewpermissions', 'corplms_feedback360');
    }
    ...
    if ($respassignment->subjectid != $USER->id) {
        ...
        if (corplms_is_manager($respassignment->subjectid)) {
            // Or their manager.
            $capability_context = context_user::instance($respassignment->subjectid);
            require_capability('corplms/feedback360:viewstaffreceivedfeedback360', $capability_context);
        ...
        $systemcontext = context_system::instance();
        require_capability('corplms/feedback360:viewownreceivedfeedback360', $systemcontext);
    ...
    if ($viewas != $USER->id) {
        if (corplms_is_manager($viewas)) {
            ...
            require_capability('corplms/feedback360:viewstaffrequestedfeedback360', $usercontext);
        } else {
...
if (!$respassignment) {
    corplms_set_notification(get_string('feedback360notfound', 'corplms_feedback360'),
            new moodle_url('/corplms/feedback360/index.php'), array('class' => 'notifyproblem'));
}

// Set up the page.
$pageurl = new moodle_url('/corplms/feedback360/index.php');
...
if ($isexternaluser) {
    $heading = get_string('feedback360', 'corplms_feedback360');

    $PAGE->set_title($heading);
    $PAGE->set_heading($heading);
    $PAGE->set_corplms_menu_selected('appraisals');
    $PAGE->navbar->add($heading);
    $PAGE->navbar->add(get_string('givefeedback', 'corplms_feedback360'));
} else if ($viewasown) {
    $heading = get_string('myfeedback', 'corplms_feedback360');
    ...
    $PAGE->navbar->add(get_string('viewresponse', 'corplms_feedback360'));
}
...
$backurl = null;
if ($viewanswer) {
    $backurl = new moodle_url('/corplms/feedback360/request/view.php',
...
if ($form->is_submitted() && !$respassignment->is_completed()) {
    ...
    if ($answers->action == 'saveprogress' || ($answers->action == 'submit' && $formisvalid)) {
        ...
        $message = get_string('progresssaved', 'corplms_feedback360');
        if ($answers->action == 'submit') {
            // Mark as answered.
            $respassignment->complete();
            $message = get_string('feedbacksubmitted', 'corplms_feedback360');
        }
        corplms_set_notification($message, $returnurl, array('class' => 'notifysuccess'));
    }
    if ($answers->action == 'submit' && !$formisvalid) {
        corplms_set_notification(get_string('error:submitform', 'corplms_feedback360'), null, array('class' => 'notifyproblem'));
    }
} else if (!$preview) {
...
local_js();
$jsmodule = array(
    'name' => 'corplms_feedback360_feedback',
    'fullpath' => '/corplms/feedback360/js/feedback.js',
    'requires' => array('json'));
$PAGE->requires->js_init_call('M.corplms_feedback360_feedback.init', array($form->_form->getAttribute('id')),
        false, $jsmodule);
