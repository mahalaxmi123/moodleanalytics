...
$defaults = file_prepare_standard_editor($defaults, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
        'corplms_feedback360', 'feedback360', $id);
...
if ($fromform = $mform->get_data()) {
    if (empty($fromform->submitbutton)) {
        corplms_set_notification(get_string('error:unknownbuttonclicked', 'corplms_feedback360'), $returnurl);
    }
    ...
    $todb = file_postupdate_standard_editor($todb, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
        'corplms_feedback360', 'feedback360', $feedback360->id);
    ...
    if ($id > 0) {
        ...
        $stageurl = new moodle_url('/corplms/feedback360/general.php', array('id' => $feedback360->id));
        corplms_set_notification(get_string('feedback360updated', 'corplms_feedback360'), $stageurl,
                array('class' => 'notifysuccess'));
    }
}
...
if ($feedback360->id) {
    ...
    echo $output->heading(get_string('createfeedback360heading', 'corplms_feedback360'));
}
