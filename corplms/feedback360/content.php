...
require_capability('corplms/feedback360:managepageelements', $systemcontext);

$output = $PAGE->get_renderer('corplms_feedback360');
...
$returnurl = new moodle_url('/corplms/feedback360/content.php', array('feedback360id' => $feedback360->id));
...
switch ($action) {
    case 'pos':
        ...
        corplms_set_notification(get_string('feedback360updated', 'corplms_feedback360'), $returnurl,
    ...
    case 'edit':
        if ($mnewform->is_submitted()) {
            ...
            if (!$newelement) {
                corplms_set_notification(get_string('error:choosedatatype', 'corplms_question'), $returnurl,
        ...
        if ($isdraft && $fromform = $meditform->get_data()) {
            ...
            if (is_ajax_request($_SERVER)) {
                ...
                corplms_set_notification(get_string('contentupdated', 'corplms_feedback360'), $returnurl,
                        array('class' => 'notifysuccess'));
            }
        } else {
    ...
    case 'delete':
        if ($question->id < 1) {
            corplms_set_notification(get_string('error:elementnotfound', 'corplms_question'), $returnurl,
                    array('class' => 'notifyproblem'));
            return;
        }
        ...
        if ($confirm == 1) {
            ...
            corplms_set_notification(get_string('deletedquestion', 'corplms_question'), $returnurl,
                    array('class' => 'notifysuccess'));
        }
        break;
    case 'clone':
    ...
    default:
        ...
        if ($isdraft) {
            ...
            $jsmodule = array(
                'name' => 'corplms_feedback360_content',
                'fullpath' => '/corplms/feedback360/js/content.js',
                'requires' => array('json'));

            $PAGE->requires->js_init_call('M.corplms_feedback360_content.init', array(), false, $jsmodule);
        }
}
