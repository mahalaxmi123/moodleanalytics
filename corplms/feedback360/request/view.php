...
if ($userid == $USER->id) {
    ...
    require_capability('corplms/feedback360:viewownreceivedfeedback360', $context);
    ...
    require_capability('corplms/feedback360:viewstaffreceivedfeedback360', $usercontext);
}


$feedback = $DB->get_record('feedback360', array('id' => $user_assignment->feedback360id));
$strviewrequest = get_string('viewrequest', 'corplms_feedback360');
...
$requested = get_string('requested', 'corplms_feedback360') .
        userdate($requested_time, get_string('strftimedate', 'langconfig'));
$timedue = '';
if (!empty($user_assignment->timedue)) {
    $timedue = get_string('timedue', 'corplms_feedback360') .
            userdate($user_assignment->timedue, get_string('strftimedate', 'langconfig'));
}

// Set up the page.
$PAGE->set_url(new moodle_url('/corplms/feedback360/request/view.php'), array('userassignment' => $assigid));
