...
$strdelrequest = get_string('removerequest', 'corplms_feedback360');
...
$returnurl = new moodle_url('/corplms/feedback360/request.php',
        array('action' => 'users', 'userid' => $userid, 'formid' => $userform));

// Set up the page.
$urlparams = array('userid' => $userid, 'userform' => $userform);
$PAGE->set_url(new moodle_url('/corplms/feedback360/request/delete.php'), $urlparams);
$PAGE->set_context($systemcontext);
$PAGE->set_pagelayout('admin');
$PAGE->set_corplms_menu_selected('myappraisals');
...
if ($delete) {
    ...
    if ($delete != md5($request_record->timeassigned)) {
        print_error('error:requestdeletefailure', 'corplms_feedback360');
    }
    ...
    corplms_set_notification(get_string('feedback360requestdeleted', 'corplms_feedback360'), $returnurl,
    ...
    $deleteurl = new moodle_url('/corplms/feedback360/request/delete.php', $delete_params);
    if (!empty($email)) {
        $username = $email;
    } else {
    ...
    echo $OUTPUT->confirm(get_string('removerequestconfirm', 'corplms_feedback360', $username), $deleteurl, $returnurl);
