...
$remindstr = get_string('remindresponders', 'corplms_feedback360');
...
if (!empty($confirmation)) {
    ...
    if ($confirmation == $valid) {
        ...
        if (!empty($userform->timedue)) {
            $duedate = userdate($userform->timedue, get_string('datepickerlongyearphpuserdate', 'corplms_core'));
            $remvars->timedue = get_string('byduedate' , 'corplms_feedback360', $duedate);
        } else {
            $remvars->timedue = '';
        }
        ...
        foreach ($resp_assignments as $resp_assignment) {
            ...
            if (!empty($resp_assignment->feedback360emailassignmentid)) {
                ...
                $remvars->link = html_writer::link($url, get_string('urlrequesturlmask', 'corplms_feedback360'));
                $remvars->url = $url->out();

                if ($asmanager) {
                    $emailplain = get_string('managerreminderemailbody', 'corplms_feedback360', $remvars);
                    $emailhtml = get_string('managerreminderemailbodyhtml', 'corplms_feedback360', $remvars);
                    $emailsubject = get_string('managerreminderemailsubject', 'corplms_feedback360', $remvars);
                } else {
                    $emailplain = get_string('reminderemailbody', 'corplms_feedback360', $remvars);
                    $emailhtml = get_string('reminderemailbodyhtml', 'corplms_feedback360', $remvars);
                    $emailsubject = get_string('reminderemailsubject', 'corplms_feedback360', $remvars);
                }

                // Send the feedback reminder email.
                $userto = \corplms_core\corplms_user::get_external_user($email_assignment->email);
                ...
                if ($asmanager) {
                    $eventdata->subject = $stringmanager->get_string('managerreminderemailsubject', 'corplms_feedback360',
                            $remvars, $userto->lang);
                    $eventdata->fullmessage = $stringmanager->get_string('managerreminderemailbody', 'corplms_feedback360',
                            $remvars, $userto->lang);
                    $eventdata->fullmessagehtml = $stringmanager->get_string('managerreminderemailbodyhtml', 'corplms_feedback360',
                            $remvars, $userto->lang);
                } else {
                    $eventdata->subject = $stringmanager->get_string('reminderemailsubject', 'corplms_feedback360',
                            $remvars, $userto->lang);
                    $eventdata->fullmessage = $stringmanager->get_string('reminderemailbody', 'corplms_feedback360',
                            $remvars, $userto->lang);
                    $eventdata->fullmessagehtml = $stringmanager->get_string('reminderemailbodyhtml', 'corplms_feedback360',
                            $remvars, $userto->lang);
                }
        ...
        // Redirect.
        $success = get_string('reminderssent', 'corplms_feedback360', format_string($feedback->name));
...
$strremind = get_string('reminderconfirm', 'corplms_feedback360') . $spacer . $system . $spacer . $external;
...
$rem_url = new moodle_url('/corplms/feedback360/request/remind.php', $rem_params);
