...
if (!$item = $hierarchy->get_item($id)) {
    print_error('itemdoesntexist', 'corplms_hierarchy', null, $prefix);
}
...
if ($canmanage) {
    ...
    $PAGE->set_heading('');
    $PAGE->set_url('/corplms/hierarchy/item/view.php', array('prefix' => $prefix, 'id' => $id));
...
if ($canmanageframeworks) {
    ...
    $button = $OUTPUT->single_button(new moodle_url('../index.php', $options), get_string($prefix.'returntoframework', 'corplms_hierarchy'), 'get');
