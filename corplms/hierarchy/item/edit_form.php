...
class item_edit_form extends moodleform {
    ...
    function definition() {
        ...
        $typename = ($type) ? $type->fullname : get_string('unclassified', 'corplms_hierarchy');
        ...
        $mform->addElement('text', 'framework', get_string($prefix.'framework', 'corplms_hierarchy'));
        ...
        if (count($parents) <= 1) {
            ...
            $mform->addElement('select', 'parentid', get_string('parent', 'corplms_hierarchy'), $parents, corplms_select_width_limiter());
            $mform->addHelpButton('parentid', $prefix.'parent', 'corplms_hierarchy');
        }
        $mform->setType('parentid', PARAM_INT);

        $mform->addElement('text', 'fullname', get_string('name'), 'maxlength="1024" size="50"');
        $mform->addRule('fullname', get_string($prefix.'missingname', 'corplms_hierarchy'), 'required', null);
        $mform->setType('fullname', PARAM_MULTILANG);

        if (HIERARCHY_DISPLAY_SHORTNAMES) {
            $mform->addElement('text', 'shortname', get_string($prefix.'shortname', 'corplms_hierarchy'), 'maxlength="100" size="20"');
            $mform->addHelpButton('shortname', $prefix.'shortname', 'corplms_hierarchy');
            $mform->addRule('shortname', get_string($prefix.'missingshortname', 'corplms_hierarchy'), 'required', null);
            $mform->setType('shortname', PARAM_MULTILANG);
        }

        $mform->addElement('text', 'idnumber', get_string($prefix.'idnumber', 'corplms_hierarchy'), 'maxlength="100"  size="10"');
        $mform->addHelpButton('idnumber', $prefix.'idnumber', 'corplms_hierarchy');
        $mform->setType('idnumber', PARAM_TEXT);

        // If we are in a dialog, hide the htmleditor. It messes with the jquery code
        if (!$dialog) {
            $mform->addElement('editor', 'description_editor', get_string('description', 'corplms_hierarchy'), null, $TEXTAREA_OPTIONS);
            $mform->addHelpButton('description_editor', $prefix.'description', 'corplms_hierarchy');
        ...
        if ($item->id) {
            ...
            $group[] = $mform->createElement('submit', 'changetype', get_string('changetype', 'corplms_hierarchy'));
            $mform->addGroup($group, 'typegroup', get_string('type', 'corplms_hierarchy'), array(' &nbsp; '), false);

            $mform->setDefault('type', $typename);
            $mform->addHelpButton('typegroup', $prefix.'type', 'corplms_hierarchy');
            ...
            if ($types) {
                ...
                $mform->addElement('select', 'typeid', get_string('type', 'corplms_hierarchy'), $select, corplms_select_width_limiter());
                $mform->addHelpButton('typeid', $prefix.'type', 'corplms_hierarchy');
            } else {
                // no types exist
        ...
        if (get_config('corplms_sync', "element_{$shortprefix}_enabled")) {
            $mform->addElement('advcheckbox', 'corplmssync', get_string('corplmssync', 'tool_corplms_sync').'?');
            $mform->addHelpButton('corplmssync', 'corplmssync', 'tool_corplms_sync');
        }
    ...
    function validation($itemnew, $files) {
        ...
        if (!empty($itemnew->idnumber) && corplms_idnumber_exists($shortprefix, $itemnew->idnumber, $itemnew->id)) {
