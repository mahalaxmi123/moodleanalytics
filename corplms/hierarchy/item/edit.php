...
if (!$id && !$frameworkid) {
    print_error('incorrectparameters', 'corplms_hierarchy');
}
...
if ($id == 0) {
    // creating new item
    require_capability('corplms/hierarchy:create'.$prefix, $context);
    ...
    require_capability('corplms/hierarchy:update'.$prefix, $context);

    if (!$item = $DB->get_record($shortprefix, array('id' => $id))) {
        print_error('incorrectid', 'corplms_hierarchy');
    }
...
$item = file_prepare_standard_editor($item, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
                                          'corplms_hierarchy', $shortprefix, $item->id);
...
if ($itemform->is_cancelled()) {
    ...
    redirect("{$CFG->wwwroot}/corplms/hierarchy/index.php?prefix=$prefix&amp;frameworkid={$item->frameworkid}&amp;page=$page");

// Update data
} else if ($itemnew = $itemform->get_data()) {

    if (isset($itemnew->changetype)) {
        redirect($CFG->wwwroot . "/corplms/hierarchy/type/change.php?prefix=$prefix&amp;frameworkid={$item->frameworkid}&amp;page={$page}&typeid={$itemnew->typeid}&amp;itemid={$itemnew->id}");
    }
    ...
    // Save
    // Class to hold corplms_set_notification info.
    ...
    if ($itemnew->id == 0) {
        ...
        if ($updateditem = $hierarchy->add_hierarchy_item($itemnew, $itemnew->parentid, $itemnew->frameworkid, false)) {
            ...
            $notification->url = "{$CFG->wwwroot}/corplms/hierarchy/item/view.php?prefix=$prefix&id={$updateditem->id}";
            $notification->params = array('class' => 'notifysuccess');
        } else {
            $notification->text = 'error:add';
            $notification->url = "{$CFG->wwwroot}/corplms/hierarchy/index.php?prefix=$prefix";
        ...
        $notification->url = "{$CFG->wwwroot}/corplms/hierarchy/item/view.php?prefix=$prefix&id={$itemnew->id}";
        $notification->params = array('class' => 'notifysuccess');
    }
    //fix the description field and redirect
    $itemnew = file_postupdate_standard_editor($itemnew, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'corplms_hierarchy', $shortprefix, $itemnew->id);
    $DB->set_field($shortprefix, 'description', $itemnew->description, array('id' => $itemnew->id));
    corplms_set_notification(get_string($notification->text . $prefix, 'corplms_hierarchy', format_string($itemnew->fullname)), $notification->url, $notification->params);
}
...
if ($item->id) {
    ...
    $PAGE->navbar->add(get_string('edit'.$prefix, 'corplms_hierarchy'));
} else {
    $PAGE->navbar->add(get_string('addnew'.$prefix, 'corplms_hierarchy'));
}
...
if ($item->id == 0) {
    echo $OUTPUT->heading(get_string('addnew'.$prefix, 'corplms_hierarchy'));
} else {
    echo $OUTPUT->heading(get_string('edit'.$prefix, 'corplms_hierarchy'));
}
