...
require_once($CFG->dirroot.'/corplms/hierarchy/item/bulkadd_form.php');
...
if ($mform->is_cancelled()) {
    ...
    redirect("{$CFG->wwwroot}/corplms/hierarchy/index.php?prefix=$prefix&amp;frameworkid={$frameworkid}&amp;page={$page}");
...
// Update data
    ...
    if (!$items_to_add) {
        corplms_set_notification(get_string('bulkaddfailed', 'corplms_hierarchy'), "{$CFG->wwwroot}/corplms/hierarchy/index.php?prefix=$prefix&amp;frameworkid={$frameworkid}&amp;page={$page}");
    }
    ...
    if ($new_ids = $hierarchy->add_multiple_hierarchy_items($formdata->parentid, $items_to_add, $frameworkid)) {
        ...
        corplms_set_notification(get_string('bulkaddsuccess', 'corplms_hierarchy', count($new_ids)), "{$CFG->wwwroot}/corplms/hierarchy/index.php?prefix=$prefix&amp;frameworkid={$frameworkid}&amp;page={$page}", array('class' => 'notifysuccess'));
    } else {
        corplms_set_notification(get_string('bulkaddfailed', 'corplms_hierarchy'), "{$CFG->wwwroot}/corplms/hierarchy/index.php?prefix=$prefix&amp;frameworkid={$frameworkid}&amp;page={$page}");
    }
}
...
$PAGE->navbar->add(get_string('addmultiplenew'.$prefix, 'corplms_hierarchy'));

/// Display page header
echo $OUTPUT->header();

echo $OUTPUT->heading(get_string('addmultiplenew'.$prefix, 'corplms_hierarchy'));
