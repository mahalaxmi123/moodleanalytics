...
require_once($CFG->dirroot.'/corplms/hierarchy/item/bulkactions_form.php');
...
if ($action == 'delete') {
    require_capability('corplms/hierarchy:delete'.$prefix, $context);
} else {
...
if ($selected_items = $DB->get_records_sql($sql, $selectedparams, $soffset, HIERARCHY_BULK_SELECTED_PER_PAGE)) {
    ...
    foreach ($selected_items as $id => $item) {
        if ($item->children == 0) {
            ...
            $displayed_selected_items[$id] = get_string($langstr, 'corplms_hierarchy', $a);
        }
    }
} else {
    $displayed_selected_items = array();
...
$formurl = new moodle_url('/corplms/hierarchy/item/bulkactions.php', $formparams);
// return to the hierarchy index page (when form is done)
$returnurl = new moodle_url('/corplms/hierarchy/index.php', array('prefix' => $prefix, 'frameworkid' => $frameworkid));
...
if ($confirmdelete) {
    ...
    if ($status) {
        ...
        $a->items = $items;
        $message = get_string('xitemsdeleted', 'corplms_hierarchy', $a);
        corplms_set_notification($message, $returnurl,
            array('class' => 'notifysuccess'));
    } else if ($deletecount == 0) {
        $message = get_string('error:nonedeleted', 'corplms_hierarchy', $items);
        corplms_set_notification($message, $formurl);
    } else {
        $a = new stdClass();
        $a->actually_deleted = $deletecount;
        $a->marked_for_deletion = count($unique_ids);
        $a->items = $items;
        $message = get_string('error:somedeleted', 'corplms_hierarchy', $a);
        corplms_set_notification($message, $returnurl);
    }
...
if ($confirmmove && $newparent !== false) {
    ...
    $status = true;
        ...
        if (!$status) {
            corplms_set_notification(get_string('error:failedbulkmove', 'corplms_hierarchy'), $formurl);
        }
    ...
    $a = new stdClass();
    $a->num = count($unique_ids);
    $a->items = ($a->num == 1) ? strtolower(get_string($prefix, 'corplms_hierarchy')) :
        strtolower(get_string($prefix . 'plural', 'corplms_hierarchy'));

    corplms_set_notification(get_string('xitemsmoved', 'corplms_hierarchy', $a),
...
$PAGE->navbar->add(get_string('bulk'.$action.$prefix, 'corplms_hierarchy'));
...
// Update data
    ...
    if (isset($formdata->add_items)) {
        if (!isset($formdata->available)) {
            ...
            corplms_set_notification(get_string('error:noitemsselected', 'corplms_hierarchy'), $formurl);
        }
    ...
    if (isset($formdata->movebutton) && isset($formdata->newparent)) {
        ...
        if ($formdata->newparent != 0) {
            ...
            if (!$parentitem = $DB->get_record($shortprefix, array('id' => $formdata->newparent))) {
                $invalidmove = true;
                $message = get_string('error:invalidparentformove', 'corplms_hierarchy');
            }
            ...
            foreach ($unique_ids as $itemid) {
                if ($hierarchy->is_child_of($parentitem, $itemid)) {
                    ...
                    $message = get_string('error:cannotmoveparentintochild', 'corplms_hierarchy', $a);
                }
            }
        }
...
echo $OUTPUT->heading(get_string('bulk'.$action.$prefix, 'corplms_hierarchy'));
