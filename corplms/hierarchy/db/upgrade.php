...
/**
 * DB upgrades for Corplms Hierarchies
 */
 ...
 */
function xmldb_corplms_hierarchy_upgrade($oldversion) {
    ...
    if ($oldversion < 2012071000) {
        ...
        corplms_upgrade_mod_savepoint(true, 2012071000, 'corplms_hierarchy');
    }

    //Update to set default proficient value in competency scale
    if ($oldversion < 2012071200) {
        $scaleid = $DB->get_field('comp_scale', 'id', array('name' => get_string('competencyscale', 'corplms_hierarchy')));
        ...
        corplms_upgrade_mod_savepoint(true, 2012071200, 'corplms_hierarchy');
    }
    ...
    if ($oldversion < 2012092600) {
        ...
        corplms_upgrade_mod_savepoint(true, 2012092600, 'corplms_hierarchy');
    }

    if ($oldversion < 2012110500) {
        // set positionsenabled default config
        if (get_config('corplms_hierarchy', 'positionsenabled') === false) {
            set_config('positionsenabled', '1,2,3', 'corplms_hierarchy');
        }

        corplms_upgrade_mod_savepoint(true, 2012110500, 'corplms_hierarchy');
    }
    ...
    if ($oldversion < 2013031400) {
        ...
        corplms_upgrade_mod_savepoint(true, 2013031400, 'corplms_hierarchy');
    }
    ...
    if ($oldversion < 2013041000) {
        ...
        corplms_upgrade_mod_savepoint(true, 2013041000, 'corplms_hierarchy');
    }
    ...
    if ($oldversion < 2013041300) {
        ...
        corplms_upgrade_mod_savepoint(true, 2013041300, 'corplms_hierarchy');
    }
    ...
    if ($oldversion < 2013080500) {
        ...
        $table->setComment('Corplms Goals');
        ...
        if (empty($existing_scales)) {
            ...
            $todb = new stdClass();
            $todb->name = get_string('goalscale', 'corplms_hierarchy');
            $todb->description = '';
            ...
            $goal_scale_vals = array(
                array('name'=>get_string('goalscaledefaultassigned', 'corplms_hierarchy'), 'scaleid' => $scaleid,
                      'sortorder' => 3, 'usermodified' => $USER->id, 'timemodified' => $now),
                array('name'=>get_string('goalscaledefaultstarted', 'corplms_hierarchy'), 'scaleid' => $scaleid,
                      'sortorder' => 2, 'usermodified' => $USER->id, 'timemodified' => $now),
                array('name'=>get_string('goalscaledefaultcompleted', 'corplms_hierarchy'), 'scaleid' => $scaleid,
        ...
        corplms_upgrade_mod_savepoint(true, 2013080500, 'corplms_hierarchy');
    }
    ...
    if ($oldversion < 2013080501) {
        ...
        upgrade_plugin_savepoint(true, 2013080501, 'corplms', 'hierarchy');
    }
    ...
    if ($oldversion < 2013080502) {
        ...
        // Hierarchy savepoint reached.
        upgrade_plugin_savepoint(true, 2013080502, 'corplms', 'hierarchy');
    }
    ...
    if ($oldversion < 2013101500) {
        ...
        // Hierarchy savepoint reached.
        upgrade_plugin_savepoint(true, 2013101500, 'corplms', 'hierarchy');
    }
    ...
    if ($oldversion < 2013103000) {
        ...
        // Savepoint reached.
        upgrade_plugin_savepoint(true, 2013103000, 'corplms', 'hierarchy');
    }
    ...
    if ($oldversion < 2013120200) {
        ...
        $fixapplied = get_config('corplms_hierarchy', 'managerpathfixapplied');
        if (empty($fixapplied)) {
            ...
            foreach ($POSITION_TYPES as $positiontype => $typestring) {
                $typename = get_string('type' . $typestring, 'corplms_hierarchy');
                ...
                if ($assignments) {
                    ...
                    foreach ($assignments as $assignment) {
                        $path = '/' . implode(corplms_get_lineage($manager_relations, $assignment->userid), '/');
            ...
            $transaction->allow_commit();
            set_config('managerpathfixapplied', 1, 'corplms_hierarchy');
        }

        upgrade_plugin_savepoint(true, 2013120200, 'corplms', 'hierarchy');
    }
    ...
    if ($oldversion < 2014030400) {
        ...
        upgrade_plugin_savepoint(true, 2014030400, 'corplms', 'hierarchy');
    }
    ...
    if ($oldversion < 2014041500) {
        ...
        upgrade_plugin_savepoint(true, 2014041500, 'corplms', 'hierarchy');
    }
    ...
    if ($oldversion < 2014070800) {
        ...
        upgrade_plugin_savepoint(true, 2014070800, 'corplms', 'hierarchy');
    }
    return true;
}
