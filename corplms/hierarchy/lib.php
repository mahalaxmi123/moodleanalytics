...
*/
function corplms_hierarchy_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, $options=array()) {
    $fs = get_file_storage();
    $relativepath = implode('/', $args);
    $fullpath = "/{$context->id}/corplms_hierarchy/$filearea/$args[0]/$args[1]";
 ...
 */
function corplms_hierarchy_cron() {
    global $CFG;

    require_once($CFG->dirroot.'/corplms/hierarchy/prefix/competency/cron.php');
    require_once($CFG->dirroot.'/corplms/hierarchy/prefix/goal/cron.php');

    competency_cron();
    if (!corplms_feature_disabled('goals')) {
        goal_cron();
    }
}
 ...
 * @returns true
 */
function corplms_hierarchy_install_default_comp_scale() {
    global $USER, $DB;
    $now = time();

    $todb = new stdClass;
    $todb->name = get_string('competencyscale', 'corplms_hierarchy');
    $todb->description = '';
    ...
    $comp_scale_vals = array(
        array('name'=>get_string('competent', 'corplms_hierarchy'), 'scaleid' => $scaleid, 'sortorder' => 1, 'usermodified' => $USER->id, 'timemodified' => $now, 'proficient' => 1),
        array('name'=>get_string('competentwithsupervision', 'corplms_hierarchy'), 'scaleid' => $scaleid, 'sortorder' => 2, 'usermodified' => $USER->id, 'timemodified' => $now),
        array('name'=>get_string('notcompetent', 'corplms_hierarchy'), 'scaleid' => $scaleid, 'sortorder' => 3, 'usermodified' => $USER->id, 'timemodified' => $now)
        );
...
class hierarchy_event_handler {
    ...
    public static function user_deleted(\core\event\user_deleted $event) {
        ...
        foreach ($POSITION_TYPES as $typeid => $typename) {
            $teammembers = corplms_get_staff($userid, $typeid);
...
class hierarchy {
        ...
        if ($id == 0) {
            ...
            if (!$framework = $DB->get_record_sql($sql, null, true)) {
                if ($noframeworkok) {
                    ...
                    print_error('noframeworks', 'corplms_hierarchy');
                }
            }
        } else {
            if (!$framework = $DB->get_record($this->shortprefix.'_framework', array('id' => $id))) {
                print_error('frameworkdoesntexist', 'corplms_hierarchy', '', $this->prefix);
            }
        }
    ...
    function get_item_descendants($id) {
        ...
        if ($path) {
            ...
            print_error('nopathfoundforid', 'corplms_hierarchy', '', (object)array('prefix' => $this->prefix, 'id' => $id));
        }
    }
    ...
    */
    ...
    function display_framework_selector($page = 'index.php', $simple = false, $return = false, $showhidden = false) {
        ...
        if (!$simple) {
            ...
            $select->label = get_string('switchframework', 'corplms_hierarchy');
    ...
    function display_add_item_button($page=0) {
        ...
        $url = new moodle_url('/corplms/hierarchy/item/edit.php', $options);
        return $OUTPUT->single_button($url, get_string('addnew'.$this->prefix, 'corplms_hierarchy'), 'get');
    }
    ...
    function display_add_multiple_items_button($page=0) {
        global $OUTPUT;
        $options = array('prefix' => $this->prefix, 'frameworkid' => $this->frameworkid, 'page' => $page);
        $url = new moodle_url('/corplms/hierarchy/item/bulkadd.php', $options);
    ...
    function display_bulk_actions_picker($can_add_item, $can_edit_item, $can_delete_item, $can_manage_type, $page=0) {
        ...
        $options = array();
        if ($can_add_item) {
            $options['/corplms/hierarchy/item/bulkadd.php?prefix='.$this->prefix.'&frameworkid='.$this->frameworkid.'&page='.$page] = get_string('add');
        }
        if ($can_delete_item) {
            $options['/corplms/hierarchy/item/bulkactions.php?action=delete&prefix='.$this->prefix.'&frameworkid='.$this->frameworkid] = get_string('delete');
        }
        if ($can_edit_item) {
            $options['/corplms/hierarchy/item/bulkactions.php?action=move&prefix='.$this->prefix.'&frameworkid='.$this->frameworkid] = get_string('move');
        }
        if ($can_manage_type) {
            $options['/corplms/hierarchy/type/index.php?prefix='.$this->prefix.'#bulkreclassify'] = get_string('reclassifyitems' ,'corplms_hierarchy');
        }

        if (count($options) > 0) {
            $out .= $OUTPUT->container_start('hierarchy-bulk-actions-picker');
            $select = new url_select($options, '', array('' => get_string('bulkactions', 'corplms_hierarchy')));
    ...
    function display_add_type_button($page=0) {
        global $OUTPUT;
        $options = array('prefix' => $this->prefix, 'frameworkid' => $this->frameworkid, 'page' => $page);
        $url = new moodle_url('/corplms/hierarchy/type/edit.php', $options);
        echo $OUTPUT->single_button($url, get_string('addtype', 'corplms_hierarchy'), 'get');
    }
    ...
    function reorder_hierarchy_item($id, $swapwith) {
        ...
        if (!$destid) {
            ...
            echo $OUTPUT->notification(get_string('error:couldnotmoveitemnopeer','corplms_hierarchy',$this->prefix));
            return false;
        }
    ...
    public function delete_hierarchy_item($id, $triggerevent = true) {
        ...
        $frameworkid = isset($this->frameworkid) ? $this->frameworkid :
            ...
            while ($delete_items = corplms_pop_n($delete_list, 1000)) {
        ...
        if ($triggerevent) {
            foreach ($deleted_list as $deleted_item) {
                ...
                $eventname = '\corplms_hierarchy\event\\' . $this->prefix . '_deleted';
    ...
    function get_item_data($item, $cols = NULL) {
        ...
        foreach ($cols as $datatype) {
            if ($datatype == 'description') {
                $value = file_rewrite_pluginfile_urls($item->$datatype, 'pluginfile.php', context_system::instance()->id, 'corplms_hierarchy', $this->shortprefix, $item->id);
            } else {
                $value = $item->$datatype;
            }
            $data[] = array(
                'title' => get_string($datatype.'view', 'corplms_hierarchy'),
                'value' => $value
            );
        }
        ...
        $typename = ($itemtype) ? $itemtype->fullname : get_string('unclassified', 'corplms_hierarchy');
        $data[] = array(
            'title' => get_string('type', 'corplms_hierarchy'),
            'value' => $typename,
        );
    ...
    static function get_short_prefix($prefixname) {
        ...
        $libpath = $CFG->dirroot.'/corplms/hierarchy/prefix/'.$cleanprefixname.'/lib.php';
        if (!file_exists($libpath)) {
            print_error('error:hierarchyprefixnotfound', 'corplms_hierarchy', $cleanprefixname);
        }
    ...
    static function load_hierarchy($prefix) {
        ...
        // Check file exists
        $libpath = $CFG->dirroot.'/corplms/hierarchy/prefix/'.$prefix.'/lib.php';
        if (!file_exists($libpath)) {
            print_error('error:hierarchyprefixnotfound', 'corplms_hierarchy', '', $prefix);
        }
    ...
    function display_hierarchy_item($record, $include_custom_fields=false, $indicate_depth=true, $cfields=array(), $types=array()) {
        ...
        $out .= $OUTPUT->action_link(new moodle_url('/corplms/hierarchy/item/view.php', array('prefix' => $this->prefix, 'id' => $record->id)), format_string($record->fullname), null, array('class' => $cssclass));
        if ($include_custom_fields) {
            $out .= html_writer::empty_tag('br');
            // Print description if available.
            if ($record->description) {
                $record->description = file_rewrite_pluginfile_urls($record->description, 'pluginfile.php', context_system::instance()->id, 'corplms_hierarchy', $this->shortprefix, $record->id);
            ...
            if ($record->typeid !=0 && is_array($types) && array_key_exists($record->typeid, $types)) {
                $out .= html_writer::tag('div', html_writer::tag('strong', get_string('type', 'corplms_hierarchy') . ': ') . format_string($types[$record->typeid]->fullname), array('class' => 'itemtype' . $cssclass));
            }
    ...
    function display_hierarchy_item_custom_field_data($record, $cfields) {
        ...
        foreach ($cfields as $cf) {
            ...
            require_once($CFG->dirroot.'/corplms/customfield/field/'.$cf->datatype.'/field.class.php');
    ...
    function add_hierarchy_item($item, $parentid, $frameworkid = null, $usetransaction = true, $triggerevent = true, $removedesc = true) {
        ...
        if ($triggerevent) {
            $eventname = '\corplms_hierarchy\event\\' . $this->prefix . '_added';
            $event = $eventname::create(
                array(
                    'objectid' => $newitem->id,
                    'context' => context_system::instance(),
    ...
    function update_hierarchy_item($itemid, $newitem, $usetransaction = true, $triggerevent = true, $removedesc = true) {
        ...
        if ($triggerevent) {
                $eventname = '\corplms_hierarchy\event\\' . $this->prefix . '_updated';
                $event = $eventname::create(
                    array(
     ...
     * local_js(array(CORPLMS_JS_PLACEHOLDER));
     *
    ...
    function display_search_box($query, $placeholdertext=null) {
        ...
        $out = $renderer->print_corplms_search('', $hiddenfields, $placeholdertext, $query, 'hierarchy-search-form', 'hierarchy-search-text-field');
        return $out;
    }
    ...
    function display_showhide_detail_button($displaymode, $query='', $page=0) {
        ...
        $buttontext = ($displaymode) ? get_string('showdetails', 'corplms_hierarchy') : get_string('hidedetails', 'corplms_hierarchy');
        $urlparams = array(
            'prefix' => $this->prefix,
            'frameworkid' => $this->frameworkid,
            'query' => $query,
    ...
    function export_select($baseurl=null) {
        global $CFG;
        require_once($CFG->dirroot.'/corplms/hierarchy/export_form.php');
        if (empty($baseurl)) {
    ...
    function export_data($format) {
        ...
        if ($searchactive) {
            $headings = array('hierarchyname' => get_string('name'), 'typename' => get_string('type','corplms_hierarchy'));
        } else {
            $headings = array('typename' => get_string('type', 'corplms_hierarchy'));
        }
        ...
        if ($searchactive) {
            ...
            $keywords = corplms_search_parse_keywords($query);
            ...
            list($searchsql, $searchparams) = corplms_search_get_keyword_where_clause($keywords, $dbfields);
            $where .= ' AND (' . $searchsql. ')';
    ...
    function parse_customfield($fieldid, $fieldvalue, $isexport=false) {
        ...
        $classfilepath = $CFG->dirroot . '/corplms/customfield/field/' . $datatype . '/field.class.php';
    ...
    function download_ods($fields, $query, $params, $maxdepth, $file=null, $searchactive=false) {
        ...
        if (!$searchactive) {
            for ($depth = 1 ; $depth <= $maxdepth ; $depth++) {
                $worksheet[0]->write($row, $col, get_string('depth', 'corplms_hierarchy', $depth));
                $col++;
            }
        }
    ...
    function download_csv($fields, $query, $params, $maxdepth, $file=null, $searchactive=false) {
        ...
        if (!$searchactive) {
            $headers = array();
            for ($depth = 1 ; $depth <= $maxdepth ; $depth++) {
                $headers[] = get_string('depth', 'corplms_hierarchy' ,$depth);
            }
        }
    ...
    public function output_delete_message($stats) {
        ...
        $a->children_string = $stats['children'] == 1 ? get_string('child', 'corplms_hierarchy') : get_string('children', 'corplms_hierarchy');
        ...
        $message .= get_string($langstr, 'corplms_hierarchy', $a) . html_writer::empty_tag('br');

        if ($stats['cf_data'] > 0) {
            $message .= get_string('deleteincludexcustomfields', 'corplms_hierarchy', $stats['cf_data']) . html_writer::empty_tag('br');
        }
    ...
    public function get_move_message($ids, $parentid) {
        ...
        if (is_array($ids) && count($ids) != 1) {
            $itemstr = get_string($this->prefix.'plural', 'corplms_hierarchy');
            $num = count($ids);
        } else {
            $itemstr = get_string($this->prefix, 'corplms_hierarchy');
            $num = 1;
        }

        $parentname = ($parentid == 0) ? get_string('top', 'corplms_hierarchy') :
        ...
        return get_string('confirmmoveitems', 'corplms_hierarchy', $a);
    }
    ...
    public function get_items_excluding_children($ids) {
        ...
        if ($items) {
            ...
            $items_by_depth = corplms_group_records($items, 'depthlevel');
            // sort ascending
    ...
    public function get_parent_list($items, $selected = array(), $inctop = true) {
        ...
        if ($inctop) {
            ...
            $out[0] = get_string('top', 'corplms_hierarchy');
        }
    ...
    protected function get_next_child_sortthread($parentid, $frameworkid = null) {
        ...
        if (!$maxthread || strlen($maxthread->sortthread) == 0) {
            if ($parentid == 0) {
                ...
                return corplms_int2vancode(1);
            } else {
                // parent has no children yet
                return $DB->get_field($this->shortprefix, 'sortthread', array('id' => $parentid, 'frameworkid' => $frameworkid)) . '.' . corplms_int2vancode(1);
            }
        }
    ...
    public function get_permissions($context = null, $user = null) {
        ...
        $cancreateframeworks = has_capability('corplms/hierarchy:create' . $prefix . 'frameworks', $context, $user);
        $canupdateframeworks = has_capability('corplms/hierarchy:update' . $prefix . 'frameworks', $context, $user);
        $candeleteframeworks = has_capability('corplms/hierarchy:delete' . $prefix . 'frameworks', $context, $user);
        $canmanageframeworks = $cancreateframeworks || $canupdateframeworks || $candeleteframeworks;
        $canviewframeworks = has_capability('corplms/hierarchy:view' . $prefix . 'frameworks', $context, $user);

        $cancreateitems = has_capability('corplms/hierarchy:create' . $prefix, $context, $user);
        $canupdateitems = has_capability('corplms/hierarchy:update' . $prefix, $context, $user);
        $candeleteitems = has_capability('corplms/hierarchy:delete' . $prefix, $context, $user);
        $canmanageitems = $cancreateitems || $canupdateitems || $candeleteitems;
        $canviewitems = has_capability('corplms/hierarchy:view' . $prefix, $context, $user);

        if (file_exists($CFG->dirroot.'/corplms/hierarchy/prefix/'.$prefix.'/scale/lib.php')) {
            $cancreatescales = has_capability('corplms/hierarchy:create' . $prefix . 'scale', $context, $user);
            $canupdatescales = has_capability('corplms/hierarchy:update' . $prefix . 'scale', $context, $user);
            $candeletescales = has_capability('corplms/hierarchy:delete' . $prefix . 'scale', $context, $user);
            $canmanagescales = $cancreatescales || $canupdatescales || $candeletescales;
            $canviewscales = has_capability('corplms/hierarchy:view' . $prefix . 'scale', $context, $user);
