...
class organisation extends hierarchy {
    ...
    function hierarchy_page_setup($page = '', $item) {
        ...
        $PAGE->requires->strings_for_js(array('assigncompetencies', 'assigngoals'), 'corplms_hierarchy');
        ...
        $jsmodule = array(
            'name' => 'corplms_organisationitem',
            'fullpath' => '/corplms/core/js/organisation.item.js',
            'requires' => array('json'));
        $PAGE->requires->js_init_call('M.corplms_organisationitem.init',
            $args, false, $jsmodule);
    }
    ...
    function display_extra_view_info($item, $frameworkid=0) {
        ...
        $can_edit = has_capability('corplms/hierarchy:updateorganisation', $sitecontext);
        ...
        if ($comptype == 'competencies') {
            ...
            $addurl = new moodle_url('/corplms/hierarchy/prefix/organisation/assigncompetency/find.php', array('assignto' => $item->id));
            ...
            $addurl = new moodle_url('/corplms/hierarchy/prefix/organisation/assigncompetencytemplate/find.php', array('assignto' => $item->id));
    ...
    */
    function print_comp_framework_picker($organisationid, $currentfw) {
        ...
        if (!empty($frameworks)) {
            ...
            $out .= get_string('filterframework', 'corplms_hierarchy') . $OUTPUT->single_select(
                new moodle_url('/corplms/hierarchy/item/view.php', array('id' => $organisationid, 'edit' => $edit, 'prefix' => 'organisation')),
                'framework',
                $fwoptions,
                $currentfw,
                null,
    ...
    public function output_delete_message($stats) {
        ...
        if ($stats['org_assignment'] > 0) {
            $message .= get_string('organisationdeleteincludexposassignments', 'corplms_hierarchy', $stats['org_assignment']) . html_writer::empty_tag('br');
        }

        if ($stats['assigned_comps'] > 0) {
            $message .= get_string('organisationdeleteincludexlinkedcompetencies', 'corplms_hierarchy', $stats['assigned_comps']). html_writer::empty_tag('br');
        }
