...
if (!$organisation = $organisations->get_item($assignto)) {
    print_error('positionnotfound', 'corplms_hierarchy');
}
...
foreach ($add as $addition) {
    ...
    if (!is_numeric($addition)) {
        print_error('baddatanonnumeric', 'corplms_hierarchy', 'id');
    }
    ...
    if ($nojs) {
        ...
        $row->cells[] = new html_table_cell($OUTPUT->action_link(new moodle_url('/corplms/hierarchy/prefix/competency/template/view.php', array('id' => $related->id)), $related->fullname));

        $row->cells[] = new html_table_cell($OUTPUT->action_icon(new moodle_url('/corplms/hierarchy/prefix/organisation/assigncompetency/remove.php', array('id' => $relationship->id, 'organisation' => $organisation->id)),
