...
require_capability('corplms/hierarchy:updateorganisation', $sitecontext);
...
if (!$assignment = $DB->get_record('org_competencies', array('id' => $id))) {
    print_error('assignmentnotexist', 'corplms_hierarchy');
}
...
$returnurl = $CFG->wwwroot.'/corplms/hierarchy/item/view.php?prefix=organisation&amp;id='.$organisation.'&amp;framework='.$frameworkid;
$deleteurl = $CFG->wwwroot.'/corplms/hierarchy/prefix/organisation/assigncompetency/remove.php?id='.$id.'&amp;organisation='.$organisation.'&amp;framework='.$frameworkid.'&amp;delete='.md5($assignment->timecreated).'&amp;sesskey='.$USER->sesskey;
...
if ($delete) {
    ...
    corplms_set_notification(get_string('organisationdeletedassignedcompetency','corplms_hierarchy'), $returnurl, array('class' => 'notifysuccess'));
    ...
    if (!$delete) {
        $strdelete = get_string('competencyassigndeletecheck', 'corplms_hierarchy');
