...
M.corplms_assignindividual = M.corplms_assignindividual || {
     ...
     init: function(Y, args){
        ...
        if (typeof $ === 'undefined') {
            throw new Error('M.corplms_assignindividual.init()-> jQuery dependency required for this module to function.');
        }

        // Goals dialog.
        // Note: assigntype=1 is referring to GOAL_ASSIGNMENT_INDIVIUDAL.
        (function() {
            var assignto = M.corplms_assignindividual.config.id;
            var fid = M.corplms_assignindividual.config.frameworkid;
            var url = M.cfg.wwwroot+ '/corplms/hierarchy/prefix/goal/assign/';
            var saveurl = url + 'assign.php';
            var findurl = url + 'find.php';

            // Dialog & handler for hierarchy picker.
            var thandler = new corplmsDialog_handler_assigngoaltreeview();
            var tbuttons = {};
            ...
            var tdialog = new corplmsDialog(
                ...
                {
                    buttons: tbuttons,
                    title: '<h2>' + M.util.get_string('assigngoals', 'corplms_hierarchy') + '</h2>'
                },
                findurl + '?assigntype=1&assignto=' + assignto + '&frameworkid=' + fid,
                thandler
            );
            tdialog.assigngoal_base_url = url;
            corplmsDialogs['assigngoaltreeview'] = tdialog;
            ...
            $(corplmsDialogs.assigngoaltreeview.dialog).bind('ajaxComplete', function(event) {
            ...
            $(document).on('click', '#show-assignedgoals-dialog', function(event) {
                ...
                // Open default url
                var select = $(this);
                var id = M.corplms_assignindividual.config.id;
                var sesskey = M.corplms_assignindividual.config.sesskey;

                var dialog = corplmsDialogs['assigngoaltreeview'];
                ...
                dialog.showhide_url = M.corplms_assignindividual.config.showhide_url;
                dialog.showhide_text = M.corplms_assignindividual.config.showhide_text;
                dialog.showhide_id = M.corplms_assignindividual.config.id;
                dialog.open();
...
corplmsDialog_handler_assigngoaltreeview = function() {};
corplmsDialog_handler_assigngoaltreeview.prototype = new corplmsDialog_handler_treeview_multiselect();
 ...
 * @return void
 */
corplmsDialog_handler_assigngoaltreeview.prototype._save = function() {
    // Serialize data
...
corplmsDialog_handler_assigngoaltreeview.prototype._display_details = function () {
...
corplmsDialog_handler_assigngoaltreeview.prototype._update = assigngoal_handler_responsefunc;
