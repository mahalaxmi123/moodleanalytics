...
$strfindgoals = get_string('findgoals', 'corplms_hierarchy');
...
// Set up the page.
$PAGE->set_url(new moodle_url('/corplms/hierarchy/prefix/goal/assign/find.php'), $urlparams);
...
if ($assigntype == GOAL_ASSIGNMENT_INDIVIDUAL) {
    ...
    if (!$moditem = $module->get_item($assignto)) {
        ...
        print_error($type->fullname . 'notfound', 'corplms_hierarchy');
    }
...
if (!$nojs) {
    ...
    if ($assigntype == GOAL_ASSIGNMENT_POSITION) {
        ...
        $dialog = new corplms_dialog_content_hierarchy_multi('goal', $frameworkid, false, true);
    }
    ...
    $admin = has_capability('corplms/hierarchy:managegoalassignments', $context);
    $manager = false;
    $self = false;

    if ($assigntype == GOAL_ASSIGNMENT_INDIVIDUAL) {
        $user_context = context_user::instance($assignto);
        $manager = corplms_is_manager($assignto) && has_capability('corplms/hierarchy:managestaffcompanygoal', $user_context);
        $self = $USER->id == $assignto && has_capability('corplms/hierarchy:manageowncompanygoal', $user_context);
    }

    if (!($admin || $manager || $self)) {
        print_error('error:findgoals', 'corplms_hierarchy');
    }
    ...
    if (!$framework = $hierarchy->get_framework($frameworkid)) {
        print_error('goalframeworknotfound', 'corplms_hierarchy');
    }
    ...
    echo $OUTPUT->header();
    $out = html_writer::tag('h2', get_string('assigngoal', 'corplms_hierarchy'));
    ...
    if (empty($frameworkid)) {
        ...
        echo build_nojs_frameworkpicker(
            $hierarchy,
            '/corplms/hierarchy/prefix/goal/assign/find.php',
            array(
                'returnurl' => $returnurl,
                'sesskey' => sesskey(),
                'nojs' => 1,
        ...
        $out .= html_writer::tag('p', get_string('clicktoassigngoal', 'corplms_hierarchy') . ' ' .
                                      get_string('clicktoviewchildrengoal', 'corplms_hierarchy'));
        $out .= html_writer::end_tag('div');

        $out .= html_writer::start_tag('div', array('class' => 'nojsselect'));
        $out .= build_nojs_treeview(
            $goals,
            get_string('nochildgoalsfound', 'corplms_hierarchy'),
            '/corplms/hierarchy/prefix/goal/assign/assign.php',
            array(
                'sesskey' => sesskey(),
                'returnurl' => $returnurl,
                'nojs' => 1,
