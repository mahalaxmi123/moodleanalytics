...
class corplms_assign_goal extends corplms_assign_core {
    ...
    public function __construct($module, $moduleinstance) {
        ...
        $this->basepath = $CFG->dirroot . '/corplms/hierarchy/prefix/goal/assign/';
    }
    ...
    public static function get_assignable_grouptypes() {
        ...
        $basepath = $CFG->dirroot . "/corplms/hierarchy/prefix/goal/assign/";
...
class pos_goal_assign_ui_picker_hierarchy extends corplms_assign_ui_picker_hierarchy {
    ...
    public function populate_selected_items_pane($elements, $overridden = false) {
        ...
        if (!$overridden) {
            $childmenu = array();
            $childmenu[0] = get_string('posincludechildrenno', 'corplms_hierarchy');
            $childmenu[1] = get_string('posincludechildrenyes', 'corplms_hierarchy');
...
class org_goal_assign_ui_picker_hierarchy extends corplms_assign_ui_picker_hierarchy {
    ...
    public function populate_selected_items_pane($elements, $overridden = false) {
        ...
        if (!$overridden) {
            $childmenu = array();
            $childmenu[0] = get_string('orgincludechildrenno', 'corplms_hierarchy');
            $childmenu[1] = get_string('orgincludechildrenyes', 'corplms_hierarchy');
            $selected = isset($this->params['includechildren']) ? $this->params['includechildren'] : '';
            $html = html_writer::select($childmenu, 'includechildren', $selected, array(),
                    array('id' => 'id_includechildren', 'class' => 'assigngrouptreeviewsubmitfield'));
        } else {
