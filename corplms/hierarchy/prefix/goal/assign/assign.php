...
$straddgoals = get_string('addgoals', 'corplms_hierarchy');
require_sesskey();

// You must have some form of managegoals permission to see this page.
$admin = has_capability('corplms/hierarchy:managegoalassignments', $sitecontext);
$manager = false;
$self = false;
...
if (!($admin || $manager || $self)) {
    print_error('error:addgoals', 'corplms_hierarchy');
}
...
if (empty($returnurl)) {
    switch ($assigntype) {
        case GOAL_ASSIGNMENT_INDIVIDUAL:
            $returnurl = "/corplms/hierarchy/prefix/goal/mygoals.php?userid={$assignto}";
            break;
        ...
        case GOAL_ASSIGNMENT_ORGANISATION:
            $returnurl = "/corplms/hierarchy/item/view.php?prefix={$type->fullname}&id={$assignto}";
            break;
    }
}
...
// Set up the page.
$PAGE->set_url(new moodle_url('/corplms/hierarchy/prefix/goal/assign/assign.php'), $urlparams);
...
if ($add) {
    ...
    foreach ($items as $item) {
        ...
        if (goal::currently_assigned($assigntype, $assignto, $item)) {
            print_error('error:alreadyassigned', 'corplms_hierarchy');
        }

        // Check id.
        if (!is_numeric($item)) {
            print_error('baddatanonnumeric', 'corplms_hierarchy');
        }
    ...
    if ($nojs) {
        ...
        switch ($assigntype) {
            ...
            case GOAL_ASSIGNMENT_ORGANISATION:
                $renderer = $PAGE->get_renderer('corplms_hierarchy');
                $addgoalurl = new moodle_url('/corplms/hierarchy/prefix/goal/assign/find.php',
