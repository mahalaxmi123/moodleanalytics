...
if (!$permissions = $goal->get_permissions(null, $userid)) {
    ...
    print_error('error:viewusergoals', 'corplms_hierarchy');
}

extract($permissions);

// Set up the page.
$PAGE->set_url(new moodle_url('/corplms/hierarchy/prefix/goal/mygoals.php'));
$PAGE->set_context($context);
$PAGE->set_pagelayout('standard');

if (corplms_is_manager($userid)) {
    $username = fullname($DB->get_record('user', array('id' => $userid)));
    $strmygoals = get_string('mygoalsteam', 'corplms_hierarchy', $username);
    $myteamurl = new moodle_url('/my/teammembers.php', array());
    $PAGE->set_corplms_menu_selected('myteam');
    $PAGE->navbar->add(get_string('myteam', 'corplms_core'), $myteamurl);
} else {
    $strmygoals = get_string('mygoals', 'corplms_hierarchy');
...
$showhide_url = new moodle_url('/corplms/hierarchy/prefix/goal/mygoals.php', $showhide_params);
$showhide_text = $display ? get_string('hidedetails', 'corplms_hierarchy') : get_string('showdetails', 'corplms_hierarchy');
...
$jsmodule = array(
    'name' => 'corplms_assignindividual',
    'fullpath' => '/corplms/hierarchy/prefix/goal/assign/individual_dialog.js',
    'requires' => array('json'));
$PAGE->requires->js_init_call('M.corplms_assignindividual.init', $args, false, $jsmodule);
...
if ($can_view_company) {
    ...
    if ($can_edit_company) {
        ...
        $addgoalurl = new moodle_url('/corplms/hierarchy/prefix/goal/assign/find.php', $add_params);
        $add_button_text = get_string('addgoalcompany', 'corplms_hierarchy');
    ...
    if ($can_edit_company) {
        ...
        $detailsurl = new moodle_url('/corplms/hierarchy/framework/index.php', array('prefix' => 'goal', 'readonly' => true));
        $company_edit .= html_writer::link($detailsurl, get_string('companygoaldetails', 'corplms_hierarchy'));
    }
    ...
    $company .= $OUTPUT->heading(get_string('companygoals', 'corplms_hierarchy'), 3) . $company_edit;
...
if ($can_view_personal) {
    ...
    if ($can_edit_personal) {
        ...
        $personal_edit_url = new moodle_url('/corplms/hierarchy/prefix/goal/item/edit_personal.php', array('userid' => $userid));
        $personal_edit = $OUTPUT->single_button($personal_edit_url, get_string('addgoalpersonal', 'corplms_hierarchy'), 'get');
    } else {
        $personal_edit = '';
    }
    ...
    $personal .= $OUTPUT->heading(get_string('personalgoals', 'corplms_hierarchy'), 3);
