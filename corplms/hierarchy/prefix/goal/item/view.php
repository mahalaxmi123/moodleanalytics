...
$mygoalsurl = new moodle_url('/corplms/hierarchy/prefix/goal/mygoals.php', array('userid' => $userid));
...
$edit_url = new moodle_url('/corplms/hierarchy/prefix/goal/item/edit_personal.php', $edit_params);
...
$PAGE->set_url(new moodle_url('/corplms/hierarchy/prefix/goal/item/view.php'), array('goalpersonalid' => $goalpersonalid));
$PAGE->set_context($context);
...
if ($USER->id == $userid) {
    ...
    require_capability("corplms/hierarchy:viewownpersonalgoal", $context);
} else {
    // Or a manager/admin viewing a staff memebers goals.
    require_capability("corplms/hierarchy:viewstaffpersonalgoal", $context);
}
...
// Set up the heading.
echo $OUTPUT->heading(get_string("personalgoal", 'corplms_hierarchy') . $edit_button);
echo html_writer::empty_tag('br');

// Name.
$namestr =  html_writer::tag('strong', get_string('goaltable:name', 'corplms_hierarchy')) . ': ' . format_string($name);
echo html_writer::tag('p', $namestr);

// Scale.
$scalename = !empty($scale) ? format_string($scale->name) : '';
$scalestr = html_writer::tag('strong', get_string('goaltable:scale', 'corplms_hierarchy')) . ': ' . $scalename;
echo html_writer::tag('p', $scalestr);

$scalevaluestr = html_writer::tag('strong', get_string('goaltable:scalevalue', 'corplms_hierarchy')) . ': ' . $scalevalue;
echo html_writer::tag('p', $scalevaluestr);

// Target Date.
$targetdate = '';
if (!empty($goalpersonal->targetdate)) {
    $targetdate = userdate($goalpersonal->targetdate, get_string('datepickerlongyearphpuserdate', 'corplms_core'), $CFG->timezone, false);
}
$targetstr = html_writer::tag('strong', get_string('goaltargetdate', 'corplms_hierarchy')) . ': ' . $targetdate;
...
$goalpersonal->description = file_rewrite_pluginfile_urls($goalpersonal->description, 'pluginfile.php', $context->id,
        'corplms_hierarchy', 'goal', $goalpersonalid);

echo html_writer::tag('p', html_writer::tag('strong', get_string('description', 'corplms_hierarchy')) . ': ');
