...
require_once($CFG->dirroot . '/corplms/hierarchy/prefix/goal/item/edit_form.php');
...
$pageurl = new moodle_url('/corplms/hierarchy/prefix/goal/item/edit_personal.php', array('userid' => $userid));
...
if (!empty($goalpersonalid)) {
    ...
    if (!$can_edit[$goalpersonal->assigntype]) {
        print_error('error:editgoals', 'corplms_hierarchy');
    }
} else {
    $goalpersonal = new stdClass();
    $goalpersonal->userid = $userid;
    $goalname = get_string('addgoalpersonal', 'corplms_hierarchy');
    ...
    if (!$can_edit[GOAL_ASSIGNMENT_SELF] && !$can_edit[GOAL_ASSIGNMENT_MANAGER] && !$can_edit[GOAL_ASSIGNMENT_ADMIN]) {
        print_error('error:createpersonalgoal', 'corplms_hierarchy');
    }
}
...
if ($mform->is_cancelled()) {
    // Cancelled.
    redirect("{$CFG->wwwroot}/corplms/hierarchy/prefix/goal/mygoals.php?userid={$userid}");
    ...
    if (isset($existingrecord)) {
        ...
        $fromform = file_postupdate_standard_editor($fromform, 'description', $TEXTAREA_OPTIONS, $context,
            'corplms_hierarchy', 'goal', $fromform->goalpersonalid);
        ...
        } else if (corplms_is_manager($todb->userid) && $can_edit[GOAL_ASSIGNMENT_MANAGER]) {
        ...
        $fromform = file_postupdate_standard_editor($fromform, 'description', $TEXTAREA_OPTIONS, $context,
            'corplms_hierarchy', 'goal', $todb->id);
    ...
    redirect("{$CFG->wwwroot}/corplms/hierarchy/prefix/goal/mygoals.php?userid={$todb->userid}");
}
