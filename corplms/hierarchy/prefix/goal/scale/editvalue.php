...
require_once($CFG->dirroot . '/corplms/hierarchy/prefix/goal/scale/lib.php');
...
if ($id == 0) {
    ...
    require_capability('corplms/hierarchy:creategoalscale', $sitecontext);
    ...
    // Editing scale value.
    require_capability('corplms/hierarchy:updategoalscale', $sitecontext);
...
$value = file_prepare_standard_editor($value, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
                                          'corplms_hierarchy', 'goal_scale_values', $value->id);
...
if ($valueform->is_cancelled()) {
    // Cancelled.
    redirect("$CFG->wwwroot/corplms/hierarchy/prefix/goal/scale/view.php?id={$value->scaleid}&amp;prefix=goal");
    ...
    if ($valuenew->id == 0) {
        ...
        if ($valuenew->id = $DB->insert_record('goal_scale_values', $valuenew)) {
            ...
            $notification->url = "$CFG->wwwroot/corplms/hierarchy/" . $urlpart;
    ...
    $valuenew = file_postupdate_standard_editor($valuenew, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
        'corplms_hierarchy', 'goal_scale_values', $valuenew->id);
