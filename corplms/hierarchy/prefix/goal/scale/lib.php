...
function goal_scale_display_table($scales) {
    ...
    // Cache permissions.
    $can_edit = has_capability('corplms/hierarchy:updategoalscale', $sitecontext);
    $can_delete = has_capability('corplms/hierarchy:deletegoalscale', $sitecontext);
    $can_add = has_capability('corplms/hierarchy:creategoalscale', $sitecontext);
    $can_view = has_capability('corplms/hierarchy:viewgoalscale', $sitecontext);
    ...
    if ($scales) {
        ...
        foreach ($scales as $scale) {
            ...
            $line = array();
            $line[] = $OUTPUT->action_link(new moodle_url('/corplms/hierarchy/prefix/goal/scale/view.php',
            ...
            if ($can_edit || $can_delete) {
                if ($can_edit) {
                    $buttons[] = $OUTPUT->action_icon(new moodle_url('/corplms/hierarchy/prefix/goal/scale/edit.php',
                ...
                if ($can_delete) {
                    if ($scale_used) {
                        $buttons[] = $OUTPUT->pix_icon('t/delete_grey',
                                get_string('error:nodeletegoalscaleinuse', 'corplms_hierarchy'), 'corplms_core',
                                array('class' => 'iconsmall disabled',
                                      'title' => get_string('error:nodeletegoalscaleinuse', 'corplms_hierarchy')));
                    } else if ($scale_assigned) {
                        $buttons[] = $OUTPUT->pix_icon('t/delete_grey',
                                get_string('error:nodeletegoalscaleassigned', 'corplms_hierarchy'), 'corplms_core',
                                array('class' => 'iconsmall disabled',
                                      'title' => get_string('error:nodeletegoalscaleassigned', 'corplms_hierarchy')));
                    } else {
                        $buttons[] = $OUTPUT->action_icon(new moodle_url('/corplms/hierarchy/prefix/goal/scale/delete.php',
    ...
    echo $OUTPUT->heading(get_string('goalscales', 'corplms_hierarchy'));
    ...
    if ($can_add) {
        $buttonurl = new moodle_url('/corplms/hierarchy/prefix/goal/scale/edit.php', array('prefix' => 'goal'));
        echo html_writer::tag('div',
            $OUTPUT->single_button($buttonurl, get_string('scalesgoalcustomcreate', 'corplms_hierarchy'), 'get')
            . $OUTPUT->help_icon('goalscalesgeneral', 'corplms_hierarchy'),
            array('class' => 'buttons'));
    }
}
