...
class goalscalevalue_edit_form extends moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('static', 'scalename', get_string('goalscale', 'corplms_hierarchy'));
        $mform->addHelpButton('scalename', 'goalscaleassign', 'corplms_hierarchy');

        $mform->addElement('text', 'name', get_string('goalscalevaluename', 'corplms_hierarchy'), 'maxlength="255" size="20"');
        $mform->addHelpButton('name', 'goalscalevaluename', 'corplms_hierarchy');
        $mform->addRule('name', get_string('missingscalevaluename', 'corplms_hierarchy'), 'required', null, 'client');
        $mform->setType('name', PARAM_MULTILANG);

        $mform->addElement('text', 'idnumber', get_string('goalscalevalueidnumber', 'corplms_hierarchy'),
            'maxlength="100"  size="10"');
        $mform->addHelpButton('idnumber', 'goalscalevalueidnumber', 'corplms_hierarchy');
        $mform->setType('idnumber', PARAM_TEXT);

        $mform->addElement('text', 'numericscore', get_string('goalscalevaluenumericalvalue', 'corplms_hierarchy'),
            'maxlength="12"  size="10"');
        $mform->addHelpButton('numericscore', 'goalscalevaluenumericalvalue', 'corplms_hierarchy');
        $mform->setType('numericscore', PARAM_RAW);

        if (goal_scale_is_used($scaleid)) {
            $note = html_writer::tag('span', get_string('proficientvaluefrozen', 'corplms_hierarchy'),
                array('class' => 'notifyproblem'));
            $freeze = true;
        } else if ($id != 0 && goal_scale_only_proficient_value($scaleid) == $id) {
            $note = html_writer::tag('span', get_string('proficientvaluefrozenonlyprof', 'corplms_hierarchy'),
                array('class' => 'notifyproblem'));
            $freeze = true;
        } else {
            $note = '';
        ...
        $mform->addElement('advcheckbox', 'proficient', get_string('goalscaleproficient', 'corplms_hierarchy'), $note);
        $mform->addHelpButton('proficient', 'goalscaleproficient', 'corplms_hierarchy');
        if ($freeze) {
            $mform->hardFreeze('proficient');
        }

        $mform->addElement('editor', 'description_editor', get_string('description'), null, $TEXTAREA_OPTIONS);
        $mform->addHelpButton('description_editor', 'goalscalevaluedescription', 'corplms_hierarchy');
    ...
    public function validation($valuenew, $files) {
        ...
        if (!empty($valuenew->idnumber) && corplms_idnumber_exists('goal_scale_values', $valuenew->idnumber, $valuenew->id)) {
            $err['idnumber'] = get_string('idnumberexists', 'corplms_core');
        }
