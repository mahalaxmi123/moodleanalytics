...
$returnurl = new moodle_url('/corplms/hierarchy/prefix/goal/scale/view.php', $returnparams);
$deleteparams = array('id' => $value->id, 'delete' => md5($value->timemodified), 'sesskey' => $USER->sesskey, 'prefix' => 'goal');
$deleteurl = new moodle_url('/corplms/hierarchy/prefix/goal/scale/deletevalue.php', $deleteparams);
...
if ($value->id == $scale->defaultid) {
    corplms_set_notification(get_string('error:nodeletegoalscalevaluedefault', 'corplms_hierarchy'), $returnurl);
}
...
corplms_set_notification(get_string('deletedgoalscalevalue', 'corplms_hierarchy', format_string($value->name)),
