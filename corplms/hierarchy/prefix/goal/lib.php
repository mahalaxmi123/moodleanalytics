...
class goal extends hierarchy {
    ...
    public function hierarchy_page_setup($page = '', $item = null) {
        ...
        // Selector setup.
        $jsmodule = array(
            'name' => 'corplms_assigngroups',
            'fullpath' => '/corplms/hierarchy/prefix/goal/assign/assigngroup_dialog.js',
            'requires' => array('json'));
        $args = array('args' => "{\"module\":\"goal\"}");
        $PAGE->requires->js_init_call('M.corplms_assigngroupdialog.init', $args, false, $jsmodule);
    }
    ...
    public function display_extra_view_info($item) {
        ...
        require_once($CFG->dirroot . '/corplms/hierarchy/prefix/goal/assign/lib.php');

        // Set up some variables.
        $renderer = $PAGE->get_renderer('corplms_hierarchy');
        $can_assign = has_capability('corplms/hierarchy:managegoalassignments', context_system::instance());
        $assignclass = new corplms_assign_goal('goal', $item);
        $options = array(
            "" => get_string('assigngroup', 'corplms_hierarchy'),
            "org" => get_string('addorganisations', 'corplms_hierarchy'),
            'pos' => get_string('addpositions', 'corplms_hierarchy'),
            'cohort' => get_string('addcohorts', 'corplms_hierarchy'),
        );
    ...
    public function get_item_data($item, $cols = null) {
        ...
        if (!empty($item->targetdate)) {
            $format_date = userdate($item->targetdate, get_string('datepickerlongyearphpuserdate', 'corplms_core'),
                    $CFG->timezone, false);
            $data[] = array(
                'title' => get_string('goaltargetdate', 'corplms_hierarchy'),
                'value' => $format_date,
            );
        }
        ...
        if (!empty($scaleid)) {
            $data[] = array(
                'title' => get_string('goalscale', 'corplms_hierarchy'),
    ...
    public function display_additional_item_form_fields($item, $cssclass) {
        ...
        if (!empty($item->targetdate)) {
            $targetdate = userdate($item->targetdate, get_string('datepickerlongyearphpuserdate', 'corplms_core'), $CFG->timezone, false);
            $out .= html_writer::tag('div', html_writer::tag('strong', get_string('goaltargetdate', 'corplms_hierarchy') . ': ') . $targetdate,
    ...
    public function output_delete_message($stats) {
        ...
        if ($stats['user_assignments'] > 0) {
            $message .= get_string('deleteincludexuserassignments', 'corplms_hierarchy',
    ...
    public static function delete_group_assignment($assigntype, $assignmentid, $type, $delete_params) {
        ...
        if ($assigntype == GOAL_ASSIGNMENT_INDIVIDUAL) {
            ...
            print_error('error:deletingindiviualassignments', 'corplms_hierarchy');
        }
    ...
    public function create_user_assignments($type, $assignment, $includechildren = null) {
        ...
        if (!empty($includechildren)) {
            if ($type == GOAL_ASSIGNMENT_AUDIENCE) {
                ...
                print_error('error:includechildrencohort', 'corplms_hierarchy');
            }
    ...
    public static function get_user_assignments($userid, $canedit = null, $display = false) {
        ...
        foreach ($assignments as $assignment) {
            ...
            if ($assignment->assigntype == GOAL_ASSIGNMENT_INDIVIDUAL && $canedit) {
                ...
                $del_url = new moodle_url('/corplms/hierarchy/prefix/goal/assign/remove.php', $del_params);
            ...
            if (!empty($assignment_info[$goalid])) {
                ...
                if (has_capability('corplms/hierarchy:viewgoal', context_system::instance())) {
                    // Put a link in so they can actually see what the goal is.
                    $goal_params = array('id' => $goalid, 'prefix' => 'goal');
                    $goal_url = new moodle_url('/corplms/hierarchy/item/view.php', $goal_params);
        ...
        if ($scope == self::SCOPE_PERSONAL) {
            ...
            switch ($assignment->assigntype) {
                case GOAL_ASSIGNMENT_SELF:
                    // Self: unassign?
                    $assignment_string = get_string('goalassignmentself', 'corplms_hierarchy');
                    break;
                case GOAL_ASSIGNMENT_MANAGER:
                    // Mananger: {$managername}.
                    $manager = fullname($DB->get_record('user', array('id' => $assignment->usercreated)));
                    $assignment_string = get_string('goalassignmentmanager', 'corplms_hierarchy', $manager);
                    break;
                case GOAL_ASSIGNMENT_ADMIN:
                    // Admin: {$adminname}.
                    $admin = fullname($DB->get_record('user', array('id' => $assignment->usercreated)));
                    $assignment_string = get_string('goalassignmentadmin', 'corplms_hierarchy', $admin);
                    break;
                default:
                    // Unrecognised assignment type.
                    print_error('invalidassigntype', 'corplms_hierarchy', 'personal');
            }
            ...
            switch ($assignment->assigntype) {
                case GOAL_ASSIGNMENT_INDIVIDUAL:
                    ...
                    if (!empty($assignment->extrainfo) && ($extrainfo = str_replace('OLD:', '', $assignment->extrainfo))) {
                        ...
                        $replace->type = get_string('assign' . $type->fullname, 'corplms_hierarchy');
                        $replace->name = format_string($type->modname);
                        $assignment_string = get_string('oldgoalassignment', 'corplms_hierarchy', $replace);
                    } else {
                        // This is your average individual assignment.
                        $user = $DB->get_record('user', array('id' => $assignment->usermodified));
                        $name = fullname($user);
                        $assignment_string = get_string('goalassignmentindividual', 'corplms_hierarchy', $name);
                    }
                ...
                case GOAL_ASSIGNMENT_AUDIENCE:
                    ...
                    $assignment_string = get_string('goalassignmentaudience', 'corplms_hierarchy',
                ...
                case GOAL_ASSIGNMENT_POSITION:
                    ...
                    $assignment_string = get_string('goalassignmentposition', 'corplms_hierarchy',
                ...
                case GOAL_ASSIGNMENT_ORGANISATION:
                    ...
                    $assignment_string = get_string('goalassignmentorganisation', 'corplms_hierarchy',
                            format_string($org->fullname));
                    break;
                default:
                    // Unrecognised assignment type.
                    print_error('invalidassigntype', 'corplms_hierarchy', 'company');
            }
        } else {
            // Unrecognised goal type.
            print_error('invalidgoaltype', 'corplms_hierarchy');
        }
     ...
     *
     * @param string $grp_type  The type as handed back by corplms_assign_goal class
    ...
    public static function can_view_goals($userid = null) {
        ...
        $view_personal = has_capability('corplms/hierarchy:viewownpersonalgoal', $usercontext);
        if ($view_personal) {
            $has_personal = $DB->record_exists('goal_personal', array('userid' => $userid, 'deleted' => 0));
            if ($has_personal) {
                return true;
            }
            $add_personal = has_capability('corplms/hierarchy:manageownpersonalgoal', $usercontext);
            if ($add_personal) {
                return true;
            }
        }
        ...
        $view_company = has_capability('corplms/hierarchy:viewowncompanygoal', $usercontext);
        if ($view_company) {
            $has_company = $DB->record_exists('goal_user_assignment', array('userid' => $userid));
            if ($has_company) {
                return true;
            }
            $add_company = has_capability('corplms/hierarchy:manageowncompanygoal', $usercontext);
            if ($add_company) {
                return true;
            }
        }
    ...
    public function get_permissions($context = null, $user = null) {
        ...
        if (!empty($user)) {
            ...
            if (has_capability('corplms/hierarchy:managegoalassignments', $syscontext)) {
            ...
            } else if (corplms_is_manager($userid)) {
                // Manager permissions.
                $permissions['can_view_personal'] = has_capability('corplms/hierarchy:viewstaffpersonalgoal', $context);
                $permissions['can_edit_personal'] = has_capability('corplms/hierarchy:managestaffpersonalgoal', $context);
                $permissions['can_view_company'] = has_capability('corplms/hierarchy:viewstaffcompanygoal', $context);
                $permissions['can_edit_company'] = has_capability('corplms/hierarchy:managestaffcompanygoal', $context);
                $permissions['can_edit'] = array(
                ...
                // User permissions.
                $permissions['can_view_personal'] = has_capability('corplms/hierarchy:viewownpersonalgoal', $context);
                $permissions['can_edit_personal'] = has_capability('corplms/hierarchy:manageownpersonalgoal', $context);
                $permissions['can_view_company'] = has_capability('corplms/hierarchy:viewowncompanygoal', $context);
                $permissions['can_edit_company'] = has_capability('corplms/hierarchy:manageowncompanygoal', $context);
                $permissions['can_edit'] = array(
                    GOAL_ASSIGNMENT_INDIVIDUAL => $permissions['can_edit_company'],
                    GOAL_ASSIGNMENT_SELF => $permissions['can_edit_personal'],
    ...
    public static function check_feature_enabled() {
        if (corplms_feature_disabled('goals')) {
            print_error('goalsdisabled', 'corplms_hierarchy');
        }
    }
}
