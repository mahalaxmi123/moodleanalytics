...
if (!$permissions = $goal->get_permissions(null, $userid)) {
    echo get_string('error:viewusergoals', 'corplms_hierarchy');
    exit;
}
...
if ($scope == goal::SCOPE_PERSONAL && !$can_edit[$goalitem->assigntype]) {
    echo get_string('error:updatescalevalue', 'corplms_hierarchy');
    exit;
}
...
$return = new moodle_url('/corplms/hierarchy/prefix/goal/mygoals.php', array('userid' => $userid));

if ($result) {
    if ($nojs) {
        $message = get_string('updatescalevaluesuccess', 'corplms_hierarchy');
        corplms_set_notification($message, $return, array('class' => 'notifysuccess'));
    }
    echo "OK";
} else {
    if ($nojs) {
        $message = get_string('updatescalevaluefailure', 'corplms_hierarchy');
        corplms_set_notification($message, $return, array('class' => 'notifyproblem'));
    }
    echo get_string('error:updatingscalevalue', 'corplms_hierarchy');
}
