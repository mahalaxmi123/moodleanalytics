...
$adminurl = $CFG->wwwroot.'/corplms/hierarchy/framework/edit.php?prefix='.$prefix.'&id='.$id;
...
if ($id == 0) {
    ...
    if (!goal_scales_available()) {
        ...
        // Display page header.
        echo $OUTPUT->header();
        notice(get_string('nogoalscales', 'corplms_hierarchy'), "{$CFG->wwwroot}/corplms/hierarchy/framework/index.php?prefix=goal" );
        echo $OUTPUT->footer();
        die(); // Agh.
    }
    ...
    if (!$framework = $DB->get_record($shortprefix.'_framework', array('id' => $id))) {
        print_error('incorrectframework', 'corplms_hierarchy', $prefix);
    }
...
if ($frameworkform->is_cancelled()) {
    ...
    $frameworknew = file_postupdate_standard_editor($frameworknew, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
        'corplms_hierarchy', $shortprefix.'_framework', $frameworknew->id);
    ...
    if (isset($frameworknew->scale)) {
        ...
        foreach ($scales_new as $key) {
            ...
            if (!$DB->insert_record($shortprefix.'_scale_assignments', $assignment)) {
                print_error('addscaleassignment', 'corplms_hierarchy');
            }
        }
    ...
    foreach ($scales_removed as $key) {
        if (!$DB->delete_records($shortprefix.'_scale_assignments', array('scaleid' => $key, 'frameworkid' => $frameworknew->id))) {
            print_error('deletescaleassignment', 'corplms_hierarchy');
        }
    }
    ...
    redirect("$CFG->wwwroot/corplms/hierarchy/framework/index.php?prefix=$prefix&id=" . $frameworknew->id);
    // Never reached.
}
