...
if (!$assignment = $DB->get_record('pos_competencies', array('id' => $id))) {
    print_error('posassignmentnotexist', 'corplms_hierarchy');
}
...
$returnurl = new moodle_url('/corplms/hierarchy/item/view.php', array('prefix' => 'position', 'id' => $position, 'framework' => $frameworkid));
$deleteurl = new moodle_url('/corplms/hierarchy/prefix/position/assigncompetency/remove.php',
...
if ($delete) {
    ...
    corplms_set_notification(get_string('positiondeletedassignedcompetency','corplms_hierarchy'), $returnurl, array('class' => 'notifysuccess'));
} else {
    /// Display page
    echo $OUTPUT->header();
