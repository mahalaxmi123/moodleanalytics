...
class position extends hierarchy {
    ...
    function hierarchy_page_setup($page = '', $item) {
        ...
        $PAGE->requires->strings_for_js(array('assigncompetencies', 'assigncompetencytemplate', 'assigngoals'), 'corplms_hierarchy');

        // Include position user js modules.
        $jsmodule = array(
                'name' => 'corplms_positionitem',
                'fullpath' => '/corplms/core/js/position.item.js',
                'requires' => array('json'));
        $PAGE->requires->js_init_call('M.corplms_positionitem.init',
            $args, false, $jsmodule);
    }
    ...
    function display_extra_view_info($item, $frameworkid=0) {
        ...
        $can_edit = has_capability('corplms/hierarchy:updateposition', $sitecontext);
        ...
        echo $OUTPUT->heading(get_string('assignedcompetencies', 'corplms_hierarchy'));
        ...
        if ($comptype == 'competencies') {
            ...
            $addurl = new moodle_url('/corplms/hierarchy/prefix/position/assigncompetency/find.php', array('assignto' => $item->id));
            ...
            $addurl = new moodle_url('/corplms/hierarchy/prefix/position/assigncompetencytemplate/find.php', array('assignto' => $item->id));
        ...
        if (corplms_feature_visible('goals') && !is_ajax_request($_SERVER)) {
            $addgoalparam = array('assignto' => $item->id, 'assigntype' => GOAL_ASSIGNMENT_POSITION, 'sesskey' => sesskey());
            $addgoalurl = new moodle_url('/corplms/hierarchy/prefix/goal/assign/find.php', $addgoalparam);
            echo html_writer::start_tag('div', array('class' => 'list-assigned-goals'));
            echo $OUTPUT->heading(get_string('goalsassigned', 'corplms_hierarchy'));
    ...
    */
    ...
    function user_positions_picker($user, $selected) {
        ...
        foreach ($positions as $type => $pos) {
            $text = get_string('type'.$POSITION_TYPES[$type], 'corplms_hierarchy').': '.$pos->fullname;
            $options[$pos->id] = $text;
        }
    ...
    function print_comp_framework_picker($positionid, $currentfw) {
      global $CFG, $DB, $OUTPUT;
        ...
        if (!empty($frameworks)) {
            ...
            $url = new moodle_url('/corplms/hierarchy/item/view.php', array('id' => $positionid, 'edit' => $edit, 'prefix' => 'position'));
            $options = $fwoptions;
            $selected = $currentfw;
            $formid = 'switchframework';
            $out .= get_string('filterframework', 'corplms_hierarchy') . $OUTPUT->single_select($url, 'framework', $options, $selected, null, $formid);

            $out .= html_writer::end_tag('div');
        } else {
            $out .= get_string('competencynoframeworks', 'corplms_hierarchy');
        }
        $out .= html_writer::end_tag('div');
    ...
    public function output_delete_message($stats) {
        ...
        if ($stats['pos_assignment'] > 0) {
            $message .= get_string('positiondeleteincludexposassignments', 'corplms_hierarchy', $stats['pos_assignment']) . html_writer::empty_tag('br');
        }

        if ($stats['assigned_comps'] > 0) {
            $message .= get_string('positiondeleteincludexlinkedcompetencies', 'corplms_hierarchy', $stats['assigned_comps']). html_writer::empty_tag('br');
        }
    ...
    public static function position_label($posassignment) {
        ...
        if ($posassignment->positionassignmentname) {
            ...
            $label .= get_string('type' . $POSITION_TYPES[$posassignment->positiontype], 'corplms_hierarchy');
        }
    ...
    */
    ...
    public function save($managerchanged = true) {
        ...
        if ($managerchanged) {
            ...
            $this->managerpath = '/' . implode(corplms_get_lineage($manager_relations, $this->userid), '/');
...
function pos_add_nav_positions_links($courseid, $userid, $usersetting) {
    ...
    $canview = false;
    if (!empty($USER->id) && ($userid == $USER->id) && has_capability('corplms/hierarchy:viewposition', $systemcontext)) {
    ...
    $positionsenabled = get_config('corplms_hierarchy', 'positionsenabled');
    if ($canview && $positionsenabled) {
        ...
        $positions = $usersetting->add(get_string('positions', 'corplms_hierarchy'), null, navigation_node::TYPE_CONTAINER);
        ...
        foreach ($POSITION_TYPES as $pcode => $ptype) {
            if (in_array($pcode, $enabled_positions)) {
                ...
                $positions->add(get_string('type' . $ptype, 'corplms_hierarchy'), $url, navigation_node::TYPE_USER);
            }
        }
    }
}
...
function pos_can_edit_position_assignment($userid) {
    ...
    if (has_capability('corplms/hierarchy:assignuserposition', context_system::instance())) {
        return true;
    }

    // can assign this particular user's position
    if (has_capability('corplms/hierarchy:assignuserposition', $personalcontext)) {
        return true;
    }

    // editing own position and have capability to assign own position
    if ($USER->id == $userid && has_capability('corplms/hierarchy:assignselfposition', context_system::instance())) {
        return true;
    }
...
function get_position_assignments($managerreqd = false, $userid = false) {
    ...
    $tempmanager = corplms_get_manager($userid, null, false, true);
