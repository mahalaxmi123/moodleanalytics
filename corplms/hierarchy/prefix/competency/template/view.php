...
$can_edit   = has_capability('corplms/hierarchy:update'.$hierarchy->prefix.'template', $sitecontext);
...
$sitecontext = context_system::instance();
require_capability('corplms/hierarchy:view'.$hierarchy->prefix, $sitecontext);
...
if ($competencies) {
    ...
    echo html_writer::tag('p', get_string('noassignedcompetenciestotemplate', 'corplms_hierarchy'));
}
...
if ($can_edit) {
    ...
    $action = new moodle_url('/corplms/hierarchy/prefix/' . $hierarchy->prefix . '/template/find_competency.php', array('templateid' => $item->id));
    $out .= html_writer::start_tag('form', array('action' => $action->out(), 'method' => 'get'));
    $out .= html_writer::start_tag('div');
    $out .= html_writer::empty_tag('input', array('type' => 'submit', 'id' => "show-assignment-dialog", 'value' => get_string('assignnewcompetency', 'corplms_hierarchy')));
