...
require_capability('corplms/hierarchy:delete'.$hierarchy->prefix.'template', $sitecontext);
...
if (!$delete) {
    ...
    $confirmurl = new moodle_url("/corplms/hierarchy/prefix/{$hierarchy->prefix}/template/delete.php", array('id' => $template->id, 'delete' => md5($template->timemodified), 'sesskey' => $USER->sesskey));
    $cancelurl = new moodle_url('/corplms/hierarchy/framework/view.php', array('prefix' => 'competency', 'frameworkid' => $template->frameworkid));
...
echo $OUTPUT->continue_button("{$CFG->wwwroot}/corplms/hierarchy/framework/view.php?prefix=competency&frameworkid={$template->frameworkid}");
echo $OUTPUT->footer();
