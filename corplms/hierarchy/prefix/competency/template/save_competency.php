...
// Setup page
admin_externalpage_setup('competencyframework', '', array(), '/corplms/hierarchy/prefix/competency/template/update_assignments.php');
...
if ($deleteexisting) {
    ...
    foreach ($removeditems as $ritem) {
        ...
        echo get_string('reloadpage', 'corplms_hierarchy');  // Indicate that a page reload is required
    }
}
...
foreach ($assignments as $assignment) {
    ...
    if ($nojs) {
        ...
        if ($editingon) {
            ...
            echo $OUTPUT->action_link(new moodle_url("/corplms/hierarchy/prefix/{$hierarchy->prefix}/template/remove_assignment.php", array('templateid' => $template->id, 'assignment' => $competency->id)),
