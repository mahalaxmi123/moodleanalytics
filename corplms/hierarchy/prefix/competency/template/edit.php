...
require_once($CFG->dirroot.'/corplms/hierarchy/prefix/competency/template/edit_form.php');
...
admin_externalpage_setup('competencymanage', '', array(), '/corplms/hierarchy/prefix/competency/template/edit.php');

$context = context_system::instance();

if ($id == 0) {
    // Creating new competency template
    require_capability('corplms/hierarchy:createcompetencytemplate', $context);
    ...
    require_capability('corplms/hierarchy:updatecompetencytemplate', $context);

    if (!$template = $DB->get_record('comp_template', array('id' => $id))) {
    print_error('incorrectcompetencytemplateid', 'corplms_hierarchy');
    }
}

// Load framework
if (!$framework = $DB->get_record('comp_framework', array('id' => $template->frameworkid))) {
    print_error('incorrectcompetencyframeworkid', 'corplms_hierarchy');
}
...
$template = file_prepare_standard_editor($template, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
                                          'corplms_hierarchy', 'comp_template', $template->id);
...
if ($form->is_cancelled()) {
    ...
    redirect("$CFG->wwwroot/corplms/hierarchy/framework/view.php?prefix=competency&frameworkid=".$framework->id);
...
// Update data
    ...
    if ($templatenew->id == 0) {
        ...
        if (!$templatenew->id = $DB->insert_record('comp_template', $templatenew)) {
            print_error('createcompetencytemplaterecord', 'corplms_hierarchy');
        }

    // Existing template
    } else {
        if (!$DB->update_record('comp_template', $templatenew)) {
            print_error('updatecompetencytemplaterecord', 'corplms_hierarchy');
        }
    }
    ...
    $templatenew = file_postupdate_standard_editor($templatenew, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'corplms_hierarchy', 'comp_template', $templatenew->id);
...
$PAGE->navbar->add(format_string($framework->fullname),
                    new moodle_url('corplms/hierarchy/framework/view.php', array('prefix' => 'competency', 'frameworkid' => $framework->id)));
if ($template->id == 0) {
    $heading = get_string('addnewtemplate', 'corplms_hierarchy');
    $PAGE->navbar->add($heading);
} else {
    $heading = get_string('editgeneric', 'corplms_hierarchy', format_string($template->fullname));
