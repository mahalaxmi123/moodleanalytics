...
if (!$template) {
    print_error('competencytemplatenotfound', 'corplms_hierarchy');

}

if (!$competency) {
    print_error('assignedcompetencynotfound', 'corplms_hierarchy');
}

// Check capabilities
require_capability('corplms/hierarchy:update'.$hierarchy->prefix.'template', $sitecontext);
...
// Cancel/return url
$return = "{$CFG->wwwroot}/corplms/hierarchy/prefix/competency/template/view.php?id={$template->id}";
...
if (!$delete) {
    ...
    $action = "{$CFG->wwwroot}/corplms/hierarchy/prefix/competency/template/remove_assignment.php?templateid={$template->id}&amp;assignment={$competency->id}&amp;delete=".md5($competency->timemodified)."&amp;sesskey={$USER->sesskey}";
