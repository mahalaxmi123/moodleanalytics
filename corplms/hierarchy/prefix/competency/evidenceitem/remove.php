...
if (!$competency = $DB->get_record('comp', array('id' => $item->competencyid))) {
    print_error('incorrectcompetencyid', 'corplms_hierarchy');
}

// Check capabilities
require_capability('corplms/hierarchy:update'.$hierarchy->prefix, $sitecontext);
...
if (empty($return)) {
    if (!$course) {
        $return = new moodle_url('/corplms/hierarchy/item/view.php', array('prefix' => $hierarchy->prefix, 'id' => $item->competencyid));
    } else {
...
if (!$delete) {
    if (!$course) {
        $message = get_string('evidenceitemremovecheck', 'corplms_hierarchy', $compname) . html_writer::empty_tag('br') . html_writer::empty_tag('br');
        $message .= $item->get_name() .' ('. $item->get_type().')';
    } else {
        $message = get_string('evidenceitemremovecheck', 'corplms_hierarchy', $item->get_name()) . html_writer::empty_tag('br') . html_writer::empty_tag('br');
    ...
    $action = new moodle_url("/corplms/hierarchy/prefix/{$hierarchy->prefix}/evidenceitem/remove.php", $actionurlparams);
...
$message = get_string('removed'.$hierarchy->prefix.'evidenceitem', 'corplms_hierarchy', format_string($compname .' ('. $item->get_type().')'));

corplms_set_notification($message, $return, array('class' => 'notifysuccess'));
