...
abstract class competency_evidence_type extends data_object {
    ...
    public static function factory($data) {
        ...
        if (!isset($data['itemtype']) || !isset($COMPETENCY_EVIDENCE_TYPES[$data['itemtype']])) {
            print_error('invalidevidencetype', 'corplms_hierarchy');
        }

        // Load class file
        require_once($CFG->dirroot.'/corplms/hierarchy/prefix/competency/evidenceitem/type/'.$data['itemtype'].'.php');
    ...
    public function add($competency) {
        ...
        $newid = parent::insert();
        if (!$newid) {
            print_error('insertevidenceitem', 'corplms_hierarchy');
        }
        ...
        if (!$DB->update_record('comp', $todb)) {
            print_error('updatecompetencyevidencecount', 'corplms_hierarchy');
        }
        return $newid;
    }
    ...
    public function delete($competency = null) {
        ...
        if (!parent::delete()) {
            print_error('deleteevidencetype', 'corplms_hierarchy');
        }
    ...
    public function get_type_name() {
        return get_string('evidence'.$this->itemtype, 'corplms_hierarchy');
    }
