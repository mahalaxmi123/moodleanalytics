...
$competencyscalestr = get_string('competencyscale', 'corplms_hierarchy');
$pageurl = new moodle_url('/corplms/hierarchy/prefix/competency/scale/view.php', array('id' => $id, 'prefix' => $prefix));
...
if (!$scale = $DB->get_record('comp_scale', array('id' => $id))) {
    print_error('incorrectcompetencyscaleid', 'corplms_hierarchy');
}
...
if ($canupdatescales || $candeletescales) {
    ...
    if ((!empty($moveup) or !empty($movedown))) {
        ...
        if (competency_scale_is_used($scale->id)) {
            $returnurl = new moodle_url('/corplms/hierarchy/prefix/competency/scale/view.php', array('id' => $scale->id, 'prefix' => 'competency'));
            print_error('error:noreorderscaleinuse', 'corplms_hierarchy', $returnurl);
        }
    ...
    if ($default) {
        ...
        if (!$DB->update_record('comp_scale', $s)) {
            print_error('updatecompetencyscale', 'corplms_hierarchy');
        } else {
            // Fetch the update scale record so it'll show up to the user.
            $scale = $DB->get_record('comp_scale', array('id' => $id));
            corplms_set_notification(get_string('scaledefaultupdated', 'corplms_hierarchy'), null, array('class' => 'notifysuccess'));
        }
    }
}
...
$PAGE->navbar->add(get_string("competencyframeworks", 'corplms_hierarchy'),
                    new moodle_url('/corplms/hierarchy/framework/index.php', array('prefix' => 'competency')));
$PAGE->navbar->add(format_string($scale->name));

echo $OUTPUT->header();

echo $OUTPUT->action_link(new moodle_url('/corplms/hierarchy/framework/index.php', array('prefix' => 'competency')), '&laquo; ' . get_string('allcompetencyscales', 'corplms_hierarchy'));

// Display info about scale
echo $OUTPUT->heading(get_string('scalex', 'corplms_hierarchy', format_string($scale->name)), 1);
$scale->description = file_rewrite_pluginfile_urls($scale->description, 'pluginfile.php', $sitecontext->id, 'corplms_hierarchy', 'comp_scale', $scale->id);
echo html_writer::tag('p', $scale->description);

// Display warning if scale is in use
if ($canupdatescales && $scale_used) {
    echo $OUTPUT->container(get_string('competencyscaleinuse', 'corplms_hierarchy'), 'notifysuccess');
}
...
if (isset($maxprof) && isset($minnoneprof) && $maxprof > $minnoneprof) {
    echo $OUTPUT->container(get_string('nonsensicalproficientvalues', 'corplms_hierarchy'), 'notifyproblem');
}

// Display scale values
if ($values) {
    if ($canupdatescales) {
        echo html_writer::start_tag('form', array('id' => 'compscaledefaultprofform', 'action' => new moodle_url('/corplms/hierarchy/prefix/competency/scale/view.php'), 'method' => 'POST'));
    ...
    if ($canupdatescales || $candeletescales) {
        $table->head[] = get_string('competencyscaledefault', 'corplms_hierarchy').' '.
            $OUTPUT->help_icon('competencyscaledefault', 'corplms_hierarchy');
        $table->align[] = 'center';

        $table->head[] = get_string('competencyscaleproficient', 'corplms_hierarchy').' '.
            $OUTPUT->help_icon('competencyscaleproficient', 'corplms_hierarchy');
        $table->align[] = 'center';
        ...
        if ($canupdatescales || $candeletescales) {
            ...
            if ($canupdatescales) {
                $buttons[] = $OUTPUT->action_icon(
                    new moodle_url('/corplms/hierarchy/prefix/competency/scale/editvalue.php',
            ...
            if (!$scale_used && $candeletescales) {
                ...
                if ($value->id == $scale->defaultid) {
                    $buttons[] = $OUTPUT->pix_icon('t/delete_grey', get_string('error:nodeletecompetencyscalevaluedefault', 'corplms_hierarchy'), 'corplms_core', array('class' => 'iconsmall action-icon'));
                // prevent deleting last proficient value
                } else if ($value->id == $onlyprof) {
                    $buttons[] = $OUTPUT->pix_icon('t/delete_grey', get_string('error:nodeletecompetencyscalevalueonlyprof', 'corplms_hierarchy'), 'corplms_core', array('class' => 'iconsmall action-icon'));
                } else {
                    $buttons[] = $OUTPUT->action_icon(
                        new moodle_url('/corplms/hierarchy/prefix/competency/scale/deletevalue.php',
                        array('id' => $value->id, 'prefix' => 'competency')),
            ...
            if ($count > 1 && !$scale_used && $canupdatescales) {
                $buttons[] = $OUTPUT->action_icon(new moodle_url('/corplms/hierarchy/prefix/competency/scale/view.php', array('id' => $scale->id, 'moveup' => $value->id, 'prefix' => 'competency')),
            ...
            if ($count < $numvalues && !$scale_used && $canupdatescales) {
                $buttons[] = $OUTPUT->action_icon(new moodle_url('/corplms/hierarchy/prefix/competency/scale/view.php', array('id' => $scale->id, 'movedown' => $value->id, 'prefix' => 'competency')),
    ...
    echo html_writer::empty_tag('br');
    echo html_writer::tag('div', get_string('noscalevalues','corplms_hierarchy'));
    echo html_writer::empty_tag('br');
}
...
$button_editscale = '';
if ($canupdatescales && !$scale_used) {
    $button_editscale = $OUTPUT->single_button(new moodle_url('/corplms/hierarchy/prefix/competency/scale/editvalue.php', array('scaleid' => $scale->id, 'prefix' => 'competency')),
            get_string('addnewscalevalue', 'corplms_hierarchy'), 'get');
}
