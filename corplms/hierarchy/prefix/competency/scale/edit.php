...
$scale = file_prepare_standard_editor($scale, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
                                          'corplms_hierarchy', 'comp_scale', $scale->id);
...
if ($mform->is_cancelled()) {
  ...
  redirect("$CFG->wwwroot/corplms/hierarchy/framework/index.php?prefix=competency");
  ...
  // Update data
    ...
    if (empty($scalenew->id)) {
        ...
        $notification->url = "$CFG->wwwroot/corplms/hierarchy/prefix/competency/scale/view.php?id={$scalenew->id}&amp;prefix=competency";
    ...
    $scalenew = file_postupdate_standard_editor($scalenew, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'corplms_hierarchy', 'comp_scale', $scalenew->id);
    $DB->set_field('comp_scale', 'description', $scalenew->description, array('id' => $scalenew->id));
    corplms_set_notification(get_string($notification->text, 'corplms_hierarchy', $scalenew->name),
