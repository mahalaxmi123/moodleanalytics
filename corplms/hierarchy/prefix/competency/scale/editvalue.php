...
require_once($CFG->dirroot . '/corplms/hierarchy/prefix/competency/scale/lib.php');
...
if ($id == 0) {
    ...
    require_capability('corplms/hierarchy:createcompetencyscale', $sitecontext);
    ...
    // Editing scale value
    require_capability('corplms/hierarchy:updatecompetencyscale', $sitecontext);
...
if ($value->id == 0 && $scale_used) {
    print_error('usedscale', 'corplms_hierarchy');
}
...
$value = file_prepare_standard_editor($value, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
                                          'corplms_hierarchy', 'comp_scale_values', $value->id);
...
if ($valueform->is_cancelled()) {
    ...
    redirect("$CFG->wwwroot/corplms/hierarchy/prefix/competency/scale/view.php?id={$value->scaleid}&amp;prefix=competency");
...
// Update data
    ...
    if ($valuenew->id == 0) {
        ...
        if ($valuenew->id = $DB->insert_record('comp_scale_values', $valuenew)) {
            ...
            $notification->url = "$CFG->wwwroot/corplms/hierarchy/prefix/competency/scale/view.php?id={$valuenew->scaleid}&amp;prefix=competency";
            $notification->params = array('class' => 'notifysuccess');
        } else {
            print_error('createscalevaluerecord', 'corplms_hierarchy');
        }
    ...
    // Updating scale value
        ...
        if ($DB->update_record('comp_scale_values', $valuenew)) {
          ...
          print_error('updatescalevaluerecord', 'corplms_hierarchy');
        }
    }
    //fix the description field and redirect
    $valuenew = file_postupdate_standard_editor($valuenew, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'corplms_hierarchy', 'comp_scale_values', $valuenew->id);
    $DB->set_field('comp_scale_values', 'description', $valuenew->description, array('id' => $valuenew->id));
    corplms_set_notification(get_string($notification->text, 'corplms_hierarchy', format_string($valuenew->name)),
...
if ($id == 0) {
    echo $OUTPUT->heading(get_string('addnewscalevalue', 'corplms_hierarchy'));
} else {
    echo $OUTPUT->heading(get_string('editscalevalue', 'corplms_hierarchy'));
}
