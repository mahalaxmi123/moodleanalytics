...
$returnurl = "{$CFG->wwwroot}/corplms/hierarchy/framework/index.php?prefix=competency";
$deleteurl = "{$CFG->wwwroot}/corplms/hierarchy/prefix/competency/scale/delete.php?id={$scale->id}&amp;delete=".md5($scale->timemodified)."&amp;sesskey={$USER->sesskey}&amp;prefix=competency";

// Can't delete if the scale is in use or assigned
if (competency_scale_is_used($id)) {
    print_error('error:nodeletecompetencyscaleinuse', 'corplms_hierarchy', $returnurl);
}
if (competency_scale_is_assigned($id)) {
    print_error('error:nodeletecompetencyscaleassigned', 'corplms_hierarchy', $returnurl);
}
...
// redirect
corplms_set_notification(get_string('deletedcompetencyscale', 'corplms_hierarchy', format_string($scale->name)), $CFG->wwwroot . '/corplms/hierarchy/framework/index.php?prefix=competency', array('class' => 'notifysuccess'));
