...
function competency_scale_display_table($scales) {
    ...
    // Cache permissions
    $can_edit = has_capability('corplms/hierarchy:updatecompetencyscale', $sitecontext);
    $can_delete = has_capability('corplms/hierarchy:deletecompetencyscale', $sitecontext);
    $can_add = has_capability('corplms/hierarchy:createcompetencyscale', $sitecontext);
    $can_view = has_capability('corplms/hierarchy:viewcompetencyscale', $sitecontext);
    ...
    $stroptions = get_string('options', 'corplms_core');
    ...
    if ($scales) {
        ...
        foreach ($scales as $scale) {
            ...
            $line = array();
            $line[] = $OUTPUT->action_link(new moodle_url('/corplms/hierarchy/prefix/competency/scale/view.php', array('id' => $scale->id, 'prefix' => 'competency')), format_string($scale->name));
            if ($scale_used) {
                $line[] = get_string('yes');
            } else if ($scale_assigned) {
                $line[] = get_string('assignedonly', 'corplms_hierarchy');
            } else {
                $line[] = get_string('no');
            }
            ...
            if ($can_edit || $can_delete) {
                if ($can_edit) {
                    $buttons[] = $OUTPUT->action_icon(new moodle_url('/corplms/hierarchy/prefix/competency/scale/edit.php', array('id' => $scale->id, 'prefix' => 'competency')),
                        new pix_icon('t/edit', $stredit), null, array('title' => $stredit));
                }

                if ($can_delete) {
                    if ($scale_used) {
                        $buttons[] = $OUTPUT->pix_icon('t/delete_grey', get_string('error:nodeletecompetencyscaleinuse', 'corplms_hierarchy'), 'corplms_core',
                            array('class' => 'iconsmall action-icon', 'title' => get_string('error:nodeletecompetencyscaleinuse', 'corplms_hierarchy')));
                    } else if ($scale_assigned) {
                        $buttons[] = $OUTPUT->pix_icon('t/delete_grey', get_string('error:nodeletecompetencyscaleassigned', 'corplms_hierarchy'), 'corplms_core',
                            array('class' => 'iconsmall action-icon', 'title' => get_string('error:nodeletecompetencyscaleassigned', 'corplms_hierarchy')));
                    } else {
                        $buttons[] = $OUTPUT->action_icon(new moodle_url('/corplms/hierarchy/prefix/competency/scale/delete.php', array('id' => $scale->id, 'prefix' => 'competency')),
    ...
    echo $OUTPUT->heading(get_string('competencyscales', 'corplms_hierarchy'));

    if ($scales) {
        echo html_writer::table($table);
    } else {
        echo html_writer::tag('p', get_string('noscalesdefined', 'corplms_hierarchy'));
    }

    if ($can_add) {
        echo html_writer::tag('div',
            $OUTPUT->single_button(new moodle_url('/corplms/hierarchy/prefix/competency/scale/edit.php',
            array('prefix' => 'competency')), get_string('scalescompcustomcreate', 'corplms_hierarchy'), 'get') .
            $OUTPUT->help_icon('competencyscalesgeneral', 'corplms_hierarchy'), array('class' => 'buttons'));
    }
}
