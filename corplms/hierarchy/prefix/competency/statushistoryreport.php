...
$PAGE->set_url(new moodle_url('/corplms/hierarchy/prefix/competency/statushistoryreport.php', $urlparms));
...
if (is_null($compid)) {
    $strsubheading = get_string($rolstatus . 'competenciessubhead', 'corplms_plan');
} else {
    $compname = format_string($DB->get_field('comp', 'fullname', array('id' => $compid)));
    $strsubheading = get_string('historyforcompetencyx', 'corplms_plan', $compname);
}
...
$PAGE->navbar->add($strheading, new moodle_url('/corplms/plan/record/index.php'));
