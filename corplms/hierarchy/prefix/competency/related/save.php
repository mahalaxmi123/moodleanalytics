...
// Setup page
admin_externalpage_setup('competencymanage', '', array(), '/corplms/hierarchy/prefix/competency/related/save.php');
...
if (!$competency = $hierarchy->get_item($compid)) {
    print_error('competencynotfound', 'corplms_hierarchy');
}
...
foreach ($relidlist as $relid) {
    // Check id
    if (!is_numeric($relid)) {
        print_error('baddatanonnumeric', 'corplms_hierarchy', null, $relid);
    }
