 ...
 */
require_once($CFG->dirroot.'/corplms/hierarchy/prefix/competency/evidence/evidence.php');
...
function hierarchy_can_add_competency_evidence($plan, $component, $userid, $competencyid) {
    ...
    if (!has_capability('corplms/plan:accessanyplan', $systemcontext) && ($plan->get_setting('view') < DP_PERMISSION_ALLOW)) {
        return array('error:nopermissions', 'corplms_plan');
    }

    if ($component->get_setting('setproficiency') != DP_PERMISSION_ALLOW) {
        return array('error:competencystatuspermission', 'corplms_plan');
    }

    // Validate whether the plan belongs to the specified user
    if (!$plan->userid == $userid) {
        return array('error:usernotfound','corplms_plan');
    }
...
function hierarchy_add_competency_evidence($competencyid, $userid, $prof, $component, $details, $reaggregate = true, $notify = true) {
    ...
    $count = $DB->count_records('block_corplms_stats', array('userid' => $currentuser, 'eventtype' => $event, 'data2' => $data2));
    ...
    if ($notify) {
        ...
        if ($isproficient && $count == 0) {
            corplms_stats_add_event($time, $currentuser, $event, '', $data2);
            //Send Alert
            $alert_detail = new stdClass();
            $alert_detail->itemname = $DB->get_field('comp', 'fullname', array('id' => $data2));
            $alert_detail->text = get_string('competencycompleted', 'corplms_plan');
        ...
        // check record exists for removal and is set to "not proficient"
            ...
            corplms_stats_remove_event($currentuser, $event, $data2);
        }
    }
