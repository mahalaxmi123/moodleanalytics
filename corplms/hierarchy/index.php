...
$canview       = has_capability('corplms/hierarchy:view' . $prefix . 'frameworks', $sitecontext);
$canmanagetype = (count($types) > 0) && has_capability('corplms/hierarchy:update' . $prefix . 'type', $sitecontext);

// Process actions.
if ($canupdateitems) {
    require_capability('corplms/hierarchy:update'.$prefix, $sitecontext);
...
if ($canmanage) {
    ...
    $detailsstr = get_string($prefix . 'details', 'corplms_hierarchy');
    $url_params = array('prefix' => $prefix, 'frameworkid' => $frameworkid);
    $PAGE->set_url(new moodle_url('/corplms/hierarchy/index', $url_params));
...
if ($searchactive) {
    ...
    $keywords = corplms_search_parse_keywords($search);
...
$table = new corplms_table($prefix.'-framework-index-'.$frameworkid);
...
if ($extrafields = $hierarchy->get_extrafields()) {
    foreach ($extrafields as $extrafield) {
        ...
        $row->value = new stdClass();
        $row->value->fullname = get_string($prefix . $extrafield, 'corplms_hierarchy');
        $headerdata[] = $row;
    }
}
...
$baseurl = new moodle_url('/corplms/hierarchy/index.php', $urlparams);
...
echo $OUTPUT->container($OUTPUT->action_link(
    new moodle_url('/corplms/hierarchy/framework/index.php', array('prefix' => $prefix)), '&laquo; ' .
            get_string($prefix.'backtoallframeworks', 'corplms_hierarchy')), 'back-link'
);
...
$framework->description = file_rewrite_pluginfile_urls($framework->description, 'pluginfile.php', $sitecontext->id,
        'corplms_hierarchy', $shortprefix.'_framework', $frameworkid);
...
$table->set_no_records_message(get_string('no'.$prefix, 'corplms_hierarchy'));
...
if ($searchactive) {
    if ($filteredcount > 0) {
        ...
        echo html_writer::tag('strong', get_string('showingxofyforsearchz', 'corplms_hierarchy', $a));
    } else {
        echo html_writer::start_tag('p');
        echo html_writer::tag('strong', get_string('noresultsforsearchx', 'corplms_hierarchy', $search));
    }
    echo $OUTPUT->action_link(new moodle_url('/corplms/hierarchy/index.php',
            array('prefix' => $prefix, 'frameworkid' => $frameworkid)), get_string('clearsearch', 'corplms_hierarchy'));
