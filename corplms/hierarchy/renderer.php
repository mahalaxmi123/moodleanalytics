...
/**
* Standard HTML output renderer for corplms_hierarchy module
*/
class corplms_hierarchy_renderer extends plugin_renderer_base {
    ...
    */
    public function print_competency_view_evidence($item, $evidence=null, $can_edit=false) {
        ...
        $out .= $this->output->heading(get_string('evidenceitems', 'corplms_hierarchy'));
        ...
        if (!empty($CFG->competencyuseresourcelevelevidence)) {
            $table->head[] = get_string('type', 'corplms_hierarchy');
            $table->head[] = get_string('activity');
        }
        if ($can_edit) {
            $table->head[] = get_string('linktype', 'corplms_plan');
            $table->head[] = get_string('options', 'corplms_hierarchy');
        }
        ...
        if ($evidence) {
            ...
            foreach ($evidence as $eitem) {
                ...
                if ($can_edit) {
                    ...
                    array( //$options
                    PLAN_LINKTYPE_MANDATORY => get_string('mandatory','corplms_hierarchy'),
                    PLAN_LINKTYPE_OPTIONAL => get_string('optional','corplms_hierarchy'),
                    ),
                    'linktype', //$name,
                    (isset($eitem->linktype) ? $eitem->linktype : PLAN_LINKTYPE_OPTIONAL), //$selected,
                    false, //$nothing,
                    array('onchange' => "\$.get(".
                                "'{$CFG->wwwroot}/corplms/plan/update-linktype.php".
                    ...
                    $link = $this->output->action_link(
                        new moodle_url('/corplms/hierarchy/prefix/competency/evidenceitem/remove.php', array('id' => $eitem->id)),
            ...
            // # cols varies
            $cols = $can_edit ? 4 : 3;
            $cell = new html_table_cell(html_writer::start_tag('i') . get_string('noevidenceitems', 'corplms_hierarchy') . html_writer::end_tag('i'));
            $cell->colspan = $cols;
        ...
        $can_edit = has_capability('corplms/hierarchy:updatecompetency', $context);
        // Display add evidence item button
        if ($can_edit) {
            $out .= html_writer::start_tag('div', array('class' => 'singlebutton'));

            $action = new moodle_url('/corplms/hierarchy/prefix/competency/evidenceitem/edit.php', array('id' => $item->id));
            $out .= html_writer::start_tag('form', array('action' => $action->out(), 'method' => 'get'));
            $out .= html_writer::start_tag('div');
            if (!empty($CFG->competencyuseresourcelevelevidence)) {
                $btnstr = get_string('assignnewevidenceitem', 'corplms_hierarchy');
            } else {
                $btnstr = get_string('assigncoursecompletions', 'corplms_hierarchy');
            }
    ...
    */
    public function print_competency_view_related($item, $can_edit=false, $related=null) {
        ...
        $out = $this->output->heading(get_string('relatedcompetencies', 'corplms_hierarchy'));

        $table = new html_table();
        $table->attributes = array('id' => 'list-related', 'class' => 'generaltable boxaligncenter');
        // Set up table header.
        $table->head = array(
            get_string('competencyframework', 'corplms_hierarchy'),
            get_string('name'),
        );

        if ($can_edit) {
            $table->head[] = get_string('options', 'corplms_plan');
        }

        // Now the rows if any.
        if ($related) {
            $sitecontext = context_system::instance();
            $can_manage_fw = has_capability('corplms/hierarchy:updatecompetencyframeworks', $sitecontext);

            $oddeven = 1;
            foreach ($related as $ritem) {
                $cells = array();
                $framework_text = ($can_manage_fw) ?
                    $this->output->action_link(new moodle_url('/corplms/hierarchy/index.php', array('prefix' => 'competency', 'frameworkid' => $ritem->fid)),
                    format_string($ritem->framework)) : format_string($ritem->framework);
                $oddeven = ++$oddeven % 2;

                $cells[] = new html_table_cell($framework_text);
                $link = html_writer::link(new moodle_url('/corplms/hierarchy/item/view.php', array('prefix' => 'competency', 'id' => $ritem->id)), format_string($ritem->fullname));
                $cells[] = new html_table_cell($link);

                if ($can_edit) {
                    $str_remove = get_string('remove');
                    $content = $this->output->action_link(
                        new moodle_url("/corplms/hierarchy/prefix/competency/related/remove.php", array('id' => $item->id, 'related' => $ritem->id)),
            ...
            // # cols varies
            $cols = $can_edit ? 4 : 3;
            $cell = new html_table_cell(html_writer::start_tag('i') . get_string('norelatedcompetencies', 'corplms_hierarchy') . html_writer::end_tag('i'));
            $cell->colspan = $cols;
            $row = new html_table_row(array($cell));
        ...
        if ($can_edit) {
            ...
            $action = new moodle_url('/corplms/hierarchy/prefix/competency/related/find.php', array('id' => $item->id, 'frameworkid' => $item->frameworkid));
            $out .= html_writer::start_tag('form', array('action' => $action->out(), 'method' => 'get'));
            $out .= html_writer::start_tag('div');
            $out .= html_writer::empty_tag('input', array('type' => 'submit', 'id' => "show-related-dialog", 'value' => get_string('assignrelatedcompetencies', 'corplms_hierarchy')));
            $out .= html_writer::empty_tag('input', array('type' => 'hidden', 'name' => "id", 'value' => $item->id));
            $out .= html_writer::empty_tag('input', array('type' => 'hidden', 'name' => "nojs", 'value' => '1'));
            $out .= html_writer::empty_tag('input', array('type' => 'hidden', 'name' => "returnurl", 'value' => qualified_me()));
            $out .= html_writer::empty_tag('input', array('type' => 'hidden', 'name' => "s", 'value' => sesskey()));
    ...
    public function print_goal_view_assignments($item, $can_edit = false, $assignments = null, $dialog_box = false) {
        ...
        $heading = '';
        if (!$dialog_box) {
            $heading = $this->output->heading(get_string('goalassignments', 'corplms_hierarchy'), 3);
        }

        // Initialise table and add header row.
        $table = new html_table();
        $table->head = array(
            get_string('goaltable:name', 'corplms_hierarchy'),
            get_string('goaltable:type', 'corplms_hierarchy'),
            get_string('goaltable:numusers', 'corplms_hierarchy')
        );
        ...
        $andchildstr = get_string('andchildren', 'corplms_hierarchy');
    ...
    */
    function print_hierarchy_items($framework, $prefix, $shortprefix, $displaytitle, $addurl, $itemid, $items, $can_edit=false){
        ...
        if ($displaytitle == 'assignedcompetencies') {
            ...
            $headers = array(
            get_string('type', 'corplms_hierarchy'),
            get_string('name', 'corplms_hierarchy')
            );
        ...
        if ($can_edit) {
            ...
            $str_remove = get_string('remove');
            $columns[] = 'linktype';
            $headers[] = get_string('linktype', 'corplms_plan');
            $columns[] = 'options';
            $headers[] = get_string('options', 'corplms_hierarchy');
        }
        $out = '';
        if (is_array($items) && count($items)) {
            ...
            $table->define_baseurl("{$CFG->wwwroot}/corplms/hierarchy/item/view.php?prefix={$displayprefix}&id={$itemid}");
            ...
            foreach ($items as $ritem) {
                $content = array();
                $content[] = empty($ritem->type) ? get_string('unclassified', 'corplms_hierarchy') : $ritem->type;

                if ($displaytitle == 'assignedcompetencies') {
                    $content[] = $this->output->action_link(new moodle_url('/corplms/hierarchy/item/view.php', array('prefix' => $displayprefix, 'id' => $ritem->id)), format_string($ritem->fullname));
                } elseif ($displaytitle == 'assignedcompetencytemplates') {
                    $content[] = $this->output->action_link(new moodle_url('/corplms/hierarchy/prefix/competency/template/view.php', array('id' => $ritem->id)), format_string($ritem->fullname));
                }
                ...
                if ($can_edit) {
                    ...
                    $content[] = $this->output->action_icon(
                        new moodle_url('/corplms/hierarchy/prefix/' . $prefix . '/assigncompetency/remove.php',
            ...
            $out .= get_string('no'.$displaytitle, 'corplms_hierarchy');
        ...
        if ($can_edit) {
            ...
            $add_button_text = get_string('add'.$displayprefix, 'corplms_hierarchy');
            $out .= html_writer::start_tag('div',
                    array('class' => 'buttons'));
            $out .= html_writer::start_tag('div',
    ...
    public function print_assigned_goals($prefix, $shortprefix, $addgoalurl, $itemid) {
        ...
        $remove = get_string('remove');
        $level_only = get_string('goalassignthislevelonly', 'corplms_hierarchy');
        $level_below = get_string('goalassignthislevelbelow', 'corplms_hierarchy');
        $can_edit = has_capability('corplms/hierarchy:managegoalassignments', context_system::instance());
        ...
        if (empty($assigned_goals)) {
            ...
            $out .= html_writer::tag('p', get_string('noassignedgoals', 'corplms_hierarchy'));
            $out .= html_writer::end_tag('div');
        } else {
            // Initialise table and add header row.
            $table = new html_table();
            $table->head = array(
                get_string('goaltable:name', 'corplms_hierarchy'),
                get_string('goaltable:assignmentlevel', 'corplms_hierarchy')
            );
            ...
            foreach ($assigned_goals as $goal) {
                ...
                $nameurl = new moodle_url('/corplms/hierarchy/item/view.php', array('prefix' => 'goal', 'id' => $goal->goalid));
                ...
                if ($goal->includechildren) {
                    if ($goal->$field == $itemid) {
                        ...
                        $parent_url = new moodle_url('/corplms/hierarchy/item/view.php', $parent_params);
                        $parent_link = html_writer::link($parent_url, format_string($goal->parentname));
                        $level = get_string('goalassignlevelparent', 'corplms_hierarchy', $parent_link);
                    }
                } else {
                    $level = $level_only;
                }
    ...
    public function mygoals_company_table($userid, $can_edit, $display = false) {
        ...
        $company_table->head = array(
            get_string('goaltable:name', 'corplms_hierarchy'),
            get_string('goaltable:status', 'corplms_hierarchy'),
            get_string('goaltable:assigned', 'corplms_hierarchy')
        );
        ...
        foreach ($assignments as $goalid => $assignment) {
            ...
            if ($can_edit) {
                ...
                $attributes = array(
                    ...
                    'onChange' => "\$.get(".
                        "'{$CFG->wwwroot}/corplms/hierarchy/prefix/goal/update-scalevalue.php" .
                ...
                $scaleurl = new moodle_url('/corplms/hierarchy/prefix/goal/update-scalevalue.php', array('nojs' => true));
    ...
    public function mygoals_personal_table($userid, $can_edit) {
        ...
        $personal_table->head = array(
            get_string('goaltable:name', 'corplms_hierarchy'),
            get_string('goaltable:due', 'corplms_hierarchy'),
            get_string('goaltable:status', 'corplms_hierarchy'),
            get_string('goaltable:assigned', 'corplms_hierarchy'),
            get_string('edit')
        );
        ...
        foreach ($goalpersonals as $goalpersonal) {
            ...
            if ($can_edit[$goalpersonal->assigntype]) {
                ...
                $edit_url = new moodle_url('/corplms/hierarchy/prefix/goal/item/edit_personal.php',
                ...
                $delete_url = new moodle_url('/corplms/hierarchy/prefix/goal/item/delete.php',
                ...
                $edit_button = $this->output->pix_icon('t/edit_gray',
                        get_string('error:editgoals', 'corplms_hierarchy'), 'moodle', array('class' => 'iconsmall action-icon'));
                $delete_button = $this->output->pix_icon('t/delete_gray',
                        get_string('error:deletegoalassignment', 'corplms_hierarchy'), 'moodle', array('class' => 'iconsmall action-icon'));
            }

            $duedate = !empty($goalpersonal->targetdate) ? userdate($goalpersonal->targetdate,
                    get_string('datepickerlongyearphpuserdate', 'corplms_core'),
                $CFG->timezone, false) : '';
            $assign = goal::get_assignment_string(goal::SCOPE_PERSONAL, $goalpersonal);
            $nameurl = new moodle_url('/corplms/hierarchy/prefix/goal/item/view.php', array('goalpersonalid' => $goalpersonal->id));
