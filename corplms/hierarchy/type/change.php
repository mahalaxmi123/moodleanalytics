...
require_once($CFG->dirroot.'/corplms/hierarchy/type/changelib.php');
...
if ($itemid) {
    ...
    $returnurl = $CFG->wwwroot . '/corplms/hierarchy/item/edit.php?prefix=' .
    ...
    $item_sql = '';
    $item_param = array();
    $returnurl = $CFG->wwwroot . '/corplms/hierarchy/type/index.php?prefix=' . $prefix . '&amp;page=' . $page;
...
if ($itemid && !$item) {
    print_error('error:invaliditemid', 'corplms_hierarchy');
}
...
if ($affected_item_count == 0) {
    corplms_set_notification(get_string('error:nonefound' . $optype, 'corplms_hierarchy'), $returnurl);
}
...
$cfs_by_type = corplms_group_records($rs, 'typeid');
...
if ($itemid) {
    ...
    $PAGE->navbar->add(get_string('edit'.$prefix, 'corplms_hierarchy'), new moodle_url('/corplms/hierarchy/item/edit.php', array('prefix' => $prefix, 'id' => $itemid)));
    $PAGE->navbar->add(get_string('changetype', 'corplms_hierarchy'));

} else {
    $PAGE->navbar->add(get_string('types', 'corplms_hierarchy'), new moodle_url('/corplms/hierarchy/type/index.php', array('prefix' => $prefix)));
    $PAGE->navbar->add(get_string('bulktypechanges', 'corplms_hierarchy'));
}
...
if ($itemid) {
    ...
    $itemstr = ($affected_item_count == 1) ? strtolower(get_string($prefix, 'corplms_hierarchy')) :
        strtolower(get_string($prefix.'plural', 'corplms_hierarchy'));
    $a->num = $affected_item_count;
    $a->items = $itemstr;
}
echo $OUTPUT->heading(get_string('reclassify1of2' . $optype, 'corplms_hierarchy', $a), 1);

echo $OUTPUT->container_start();
echo html_writer::start_tag('p');
echo get_string('reclassify1of2desc', 'corplms_hierarchy');
// if there's data to transfer, let people know they'll get the chance to move it
// in step 2
if ($current_type_cfs) {
    echo ' ' . get_string('reclassifytransferdata', 'corplms_hierarchy');
}
echo html_writer::end_tag('p');
echo $OUTPUT->container_end();

echo $OUTPUT->heading(get_string('currenttype', 'corplms_hierarchy'), 3, 'hierarchy-bulk-type');
...
$table->head = array('',
    get_string('name'),
    get_string('customfields', 'corplms_customfield'));
...
$row[] = ($cfs = hierarchy_get_formatted_custom_fields($current_type_cfs)) ?
    ...
    get_string('nocustomfields', 'corplms_hierarchy');
$table->data[] = $row;
...
foreach ($types as $type) {
    ...
    $row[] = $OUTPUT->single_button(new moodle_url('/corplms/hierarchy/type/changeconfirm.php', array('prefix' => $prefix, 'typeid' => $typeid, 'newtypeid' => $type->id, 'itemid' => $itemid, 'page' => $page)), get_string('choose'), 'get');
...
if ($typeid != 0) {
    $row = array();
    $row[] = $OUTPUT->single_button(new moodle_url('/corplms/hierarchy/type/changeconfirm.php', array('prefix' => $prefix, 'typeid' => $typeid, 'newtypeid' => 0, 'itemid' => $itemid)), get_string('choose'), 'get');
    $row[] = get_string('unclassified', 'corplms_hierarchy');
    $row[] = get_string('nocustomfields', 'corplms_hierarchy');
    $table->data[] = $row;
}


echo $OUTPUT->heading(get_string('newtype', 'corplms_hierarchy'), 3, 'hierarchy-bulk-type');
