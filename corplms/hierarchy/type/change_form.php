...
class type_change_form extends moodleform {
    ...
    function definition() {
        ...
        if (!$current_type_cfs || $affected_data_count == false) {
            ...
            $confirmtext = get_string('confirmproceed', 'corplms_hierarchy');
            $buttontext = get_string('reclassifyitems', 'corplms_hierarchy');
            ...
            $confirmtext = get_string('deletedataconfirmproceed', 'corplms_hierarchy', html_writer::alist($message));
            $buttontext = get_string('reclassifyitemsanddelete', 'corplms_hierarchy');
            ...
            $mform->addElement('html', get_string('choosewhattodowithdata', 'corplms_hierarchy'));
            ...
            if ($old_cfs && $new_cfs) {
                ...
                foreach ($current_type_cfs as $old_cf) {
                    ...
                    $options = array(0 => get_string('deletethisdata', 'corplms_hierarchy'));
                    foreach ($new_type_cfs as $new_cf) {
                        if (hierarchy_allowed_datatype_conversion($old_cf->datatype, $new_cf->datatype)) {
                            $options[$new_cf->id] = get_string('transfertox', 'corplms_hierarchy', $new_cfs[$new_cf->id]);
                        }
                    }

                    if (array_key_exists($old_cf->id, $affected_data_count) && count($options) > 1) {
                        $mform->addElement('select', 'field[' . $old_cf->id . ']', get_string('datainx', 'corplms_hierarchy', $old_cfs[$old_cf->id]), $options);
                        $mform->setType('field[' . $old_cf->id . ']', PARAM_INT);
                    } else if (array_key_exists($old_cf->id, $affected_data_count)) {
                        $mform->addElement('static', 'message[' . $old_cf->id . ']', get_string('datainx', 'corplms_hierarchy', $old_cfs[$old_cf->id]), get_string('deletethisdata', 'corplms_hierarchy'));
                        $mform->addElement('hidden', 'field[' . $old_cf->id . ']', 0);
                        $mform->setType('field[' . $old_cf->id . ']', PARAM_INT);
                    } else {
                        $mform->addElement('static', 'field[' . $old_cf->id . ']', get_string('datainx', 'corplms_hierarchy', $old_cfs[$old_cf->id]), get_string('nodata', 'corplms_hierarchy'));
                    }
                }
            }

            $confirmtext = '';
            $buttontext = get_string('reclassifyitemsandtransfer', 'corplms_hierarchy');
        }
        ...
        if ($current_type_cfs && $new_type_cfs) {
            ...
            foreach ($current_type_cfs as $oldcf) {
                ...
                if (in_array($newfield, $used)) {
                    $errors['field[' . $oldcf->id . ']'] = get_string('error:alreadyassigned', 'corplms_hierarchy');
                }
                ...
                if (!hierarchy_allowed_datatype_conversion($oldcf->datatype, $newcf->datatype)) {
                    ...
                    $errors['field[' . $oldcf->id . ']'] = get_string('error:cannotconvertfieldfromxtoy', 'corplms_hierarchy', $a);
