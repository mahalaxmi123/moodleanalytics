...
$returnurl = $CFG->wwwroot . '/corplms/hierarchy/type/index.php?prefix='. $prefix;
...
if (file_exists($CFG->dirroot.'/corplms/hierarchy/prefix/'.$prefix.'/type/edit.php')) {
    require_once($CFG->dirroot.'/corplms/hierarchy/prefix/'.$prefix.'/type/edit_form.php');
    require_once($CFG->dirroot.'/corplms/hierarchy/prefix/'.$prefix.'/type/edit.php');
    die;
} else {
    require_once($CFG->dirroot.'/corplms/hierarchy/type/edit_form.php');
}
...
if ($id == 0) {
    // creating new type
    require_capability('corplms/hierarchy:create'.$prefix.'type', $context);
    ...
    require_capability('corplms/hierarchy:update'.$prefix.'type', $context);
    if (!$type = $hierarchy->get_type_by_id($id)) {
        print_error('incorrecttypeid', 'corplms_hierarchy');
    }
}

// Include JS for icon preview
local_js(array(CORPLMS_JS_ICON_PREVIEW));
...
$type = file_prepare_standard_editor($type, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
                                          'corplms_hierarchy', $shortprefix.'_type', $type->id);
...
// update data
    ...
    $typenew = file_postupdate_standard_editor($typenew, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'corplms_hierarchy', $shortprefix.'_type', $typenew->id);
    $DB->set_field($shortprefix.'_type', 'description', $typenew->description, array('id' => $typenew->id));
    corplms_set_notification(get_string($notification->text, 'corplms_hierarchy', $typenew->fullname), $notification->url, $notification->params);
}


/// Display page header
$PAGE->navbar->add(get_string("{$prefix}types", 'corplms_hierarchy'), $returnurl);

if ($id == 0) {
    $PAGE->navbar->add(get_string('addtype', 'corplms_hierarchy'));
} else {
    $PAGE->navbar->add(get_string('editgeneric', 'corplms_hierarchy', format_string($type->fullname)));
}

echo $OUTPUT->header();

if ($type->id == 0) {
    echo $OUTPUT->heading(get_string('addtype', 'corplms_hierarchy'));
} else {
    echo $OUTPUT->heading(get_string('editgeneric', 'corplms_hierarchy', format_string($type->fullname)));
}
