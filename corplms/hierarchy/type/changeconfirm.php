...
require_once($CFG->dirroot.'/corplms/hierarchy/type/change_form.php');
...
if ($itemid) {
    ...
    $returnurl = new moodle_url("/corplms/hierarchy/item/edit.php", array('prefix' => $prefix, 'id' => $itemid, 'page' => $page));
    ...
    $returnurl = new moodle_url("/corplms/hierarchy/type/index.php", array('prefix' => $prefix, 'page' => $page));
    $optype = 'bulk';
...
if ($changeform->is_cancelled()) {
    ...
    // process
        ...
        if (isset($data->field)) {
            ...
            if (!empty($errors)) {
                foreach ($errors as $field => $val) {
                    corplms_set_notification("$field : " . $val);
                }
                $a = new stdClass();
                $a->from = hierarchy_get_type_name($typeid, $shortprefix);
                $a->to = hierarchy_get_type_name($newtypeid, $shortprefix);
                $DB->force_transaction_rollback();
                corplms_set_notification(get_string('error:couldnotreclassify' . $optype, 'corplms_hierarchy', $a), "$CFG->wwwroot/corplms/hierarchy/index.php?prefix=$prefix");
            }
        }
        ...
        if (!$status) {
            ...
            $a->from = hierarchy_get_type_name($typeid, $shortprefix);
            $a->to = hierarchy_get_type_name($newtypeid, $shortprefix);
            corplms_set_notification(get_string('error:couldnotreclassify' . $optype,
                'corplms_hierarchy', $a),
                "$CFG->wwwroot/corplms/hierarchy/type/index.php?prefix=$prefix");
        }
    ...
    $url = substr($returnurl, strlen($CFG->wwwroot . '/corplms/hierarchy/'));
    ...
    if ($itemid) {
        ...
        $a->items = ($affected_item_count == 1) ?
            strtolower(get_string($prefix, 'corplms_hierarchy')) :
            strtolower(get_string($prefix . 'plural', 'corplms_hierarchy'));
    }
    $a->from = hierarchy_get_type_name($typeid, $shortprefix);
    $a->to = hierarchy_get_type_name($newtypeid, $shortprefix);

    corplms_set_notification(get_string('reclassifysuccess' . $optype, 'corplms_hierarchy', $a),
...
echo $OUTPUT->heading(get_string('reclassifyingfromxtoy' . $optype, 'corplms_hierarchy', $a), 1);
