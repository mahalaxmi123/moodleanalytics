...
// @todo add capabilities
    ...
    $can_add = has_capability('corplms/hierarchy:create'.$prefix.'type', $sitecontext);
    $can_edit = has_capability('corplms/hierarchy:update'.$prefix.'type', $sitecontext);
    $can_delete = has_capability('corplms/hierarchy:delete'.$prefix.'type', $sitecontext);
    $can_edit_custom_fields = has_any_capability(array('corplms/hierarchy:update'.$prefix.'customfield', 'corplms/hierarchy:create'.$prefix.'customfield', 'corplms/hierarchy:delete'.$prefix.'customfield'), $sitecontext);
...
if ($types) {
    ...
    $table->head = array(get_string('name', 'corplms_hierarchy'),
        get_string($prefix . 'plural', 'corplms_hierarchy'),
        get_string("customfields", 'corplms_hierarchy'));
    ...
    foreach ($types as $type) {
        ...
        if ($can_edit_custom_fields) {
            $row[] = $OUTPUT->action_link(new moodle_url('/corplms/customfield/index.php', array('prefix' => $prefix, 'typeid' => $type->id)), format_string($type->fullname));
        } else {
        ...
        if ($can_edit) {
            $buttons[] = $OUTPUT->action_icon(
                new moodle_url('/corplms/hierarchy/type/edit.php', array('prefix' => $prefix, 'id' => $type->id)),
        ...
        if ($can_delete) {
            $buttons[] = $OUTPUT->action_icon(
                new moodle_url('/corplms/hierarchy/type/delete.php', array('prefix' => $prefix, 'id' => $type->id)),
...
echo $OUTPUT->heading(get_string('types', 'corplms_hierarchy') . ' ' . $OUTPUT->help_icon($prefix.'type', 'corplms_hierarchy', false));
...
if ($types) {
    ...
    echo html_writer::tag("p", get_string($prefix.'notypes', 'corplms_hierarchy'));
}
...
if ($unclassified) {
    $options[0] = get_string('unclassified', 'corplms_hierarchy');
}

if ($showbulkform) {
    echo html_writer::empty_tag('br');
    echo html_writer::tag('a', '', array('name' => 'bulkreclassify'));
    echo $OUTPUT->heading(get_string('bulktypechanges', 'corplms_hierarchy'));

    echo $OUTPUT->container(get_string('bulktypechangesdesc', 'corplms_hierarchy'));
