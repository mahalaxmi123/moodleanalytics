...
if ($canmanage) {
    ...
    $PAGE->set_url(new moodle_url('/corplms/hierarchy/framework/index', $url_params));
...
if ($canupdateframeworks) {
    ...
    if ($hide or $show or $moveup or $movedown) {
        require_capability('corplms/hierarchy:update'.$prefix.'frameworks', $sitecontext);
        // Hide an item.
        if ($hide) {
...
if ($frameworks) {
    ...
    $table->head = array(get_string('name', 'corplms_hierarchy'), get_string($prefix.'plural', 'corplms_hierarchy'));
    ...
    foreach ($frameworks as $framework) {
        ...
        $link_url = new moodle_url('/corplms/hierarchy/index.php', $link_params);
...
echo $OUTPUT->heading(get_string($prefix.'frameworks', 'corplms_hierarchy') . ' ' . $OUTPUT->help_icon($prefix.'frameworks', 'corplms_hierarchy', false));
...
if ($cancreateframeworks) {
    ...
    echo html_writer::tag('div', $OUTPUT->single_button(new moodle_url('edit.php', array('prefix' => $prefix)),
                get_string($prefix.'addnewframework', 'corplms_hierarchy'), 'get'), array('class' => 'hierarchy-index-buttons'));
}
...
if ($canviewframeworks) {
    if ($frameworks) {
        ...
        echo html_writer::tag('p', get_string($prefix.'noframeworks', 'corplms_hierarchy'));
...
if ($hierarchyhasscales) {
    include($CFG->dirroot.'/corplms/hierarchy/prefix/'.$prefix.'/scale/lib.php');
