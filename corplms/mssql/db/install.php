 ...
 */
function xmldb_corplms_mssql_install() {
    ...
    if ($DB->get_dbfamily() == 'mssql') {
        ...
        if (!$DB->get_record_sql('SELECT * FROM sys.assemblies WHERE name = ?',
            ...
            $sqllist = require($CFG->dirroot . '/corplms/mssql/db/mssqlgroupconcat.php');
            try {
                ...
                echo $OUTPUT->notification(get_string('mssqlgroupconcatfail', 'corplms_mssql',
                        $message));
 ...
 */
function xmldb_corplms_mssql_install_recovery() {
    xmldb_corplms_mssql_install();
}
 ...
 */
function corplms_mssql_environment_check(environment_results $result) {
    global $DB;
    $status = true;
