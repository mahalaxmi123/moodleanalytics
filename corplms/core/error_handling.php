...
/**
 * Setup error/exception handlers for Corplms
 *
 * @access  public
 * @return  void
 */
function corplms_setup_error_handlers() {
    set_error_handler('corplms_error_handler');
    set_exception_handler('corplms_exception_handler');
}


/**
 * Corplms error handler
 *
 * @access  public
 ...
 * @return  bool
 */
function corplms_error_handler($errno, $errstr, $errfile = '', $errline = 0, $errcontext = array()) {
    global $CFG, $DB, $CORPLMS;
    ...
    // Only log error in database if Corplms is installed and it would recorded at "DEVELOPER" level
    ...
    if (!empty($CFG->local_postinst_hasrun) && ($errno & DEBUG_DEVELOPER) && $insertcount < 100) {
        ...
        if (!in_array($hash, $previous_errors)) {
            ...
            $error->version = $CORPLMS->version;
            $error->build = $CORPLMS->build;
...
/**
 * Corplms exception handler
 *
 * @access  public
 * @param   $exception  Exception
 * @return  bool
 */
function corplms_exception_handler($exception) {
    ...
    $result = corplms_error_handler($errno, $errstr, $errfile, $errline);
 ...
 */
function corplms_crop_error_log() {
    global $DB;
