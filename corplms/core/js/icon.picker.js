 ...
 */
M.corplms_iconpicker = M.corplms_iconpicker || {
    ...
    init: function(Y, args){
        ...
        if (typeof $ === 'undefined') {
            throw new Error('M.corplms_iconpicker.init()-> jQuery dependency required for this module to function.');
        }
        ...
        $(btnids).each(function(idx, btnid) {
            var handler = new corplmsDialog_handler_selectable(M.corplms_iconpicker.config.selected_icon);
            ...
            var dialog = new corplmsDialog(
                    ...
                    {
                        buttons: buttonsObj,
                        title: '<h2>' + M.util.get_string('chooseicon', 'corplms_program') + '</h2>'
                    },
                    M.cfg.wwwroot + '/corplms/core/icons.php?type=' + M.corplms_iconpicker.config.type,
                    handler
                );
            ...
            corplmsDialogs['icon-dialog' + suffix] = dialog;
