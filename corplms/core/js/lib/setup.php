 ...
 */
define('CORPLMS_JS_DIALOG',         1);
define('CORPLMS_JS_TREEVIEW',       2);
define('CORPLMS_JS_DATEPICKER',     3);
define('CORPLMS_JS_PLACEHOLDER',    4);
define('CORPLMS_JS_ICON_PREVIEW',   5);
define('CORPLMS_JS_UI',             6);
define('CORPLMS_JS_DATATABLES',     7);
/**
...
function local_js($options = array()) {
    ...
    if (!core_useragent::is_ie() || core_useragent::check_ie_version(9)) {
        $PAGE->requires->js('/corplms/core/js/lib/jquery-2.1.0.min.js');
    } else {
        $PAGE->requires->js('/corplms/core/js/lib/jquery-1.9.1.min.js');
    }

    // If UI
    if (in_array(CORPLMS_JS_UI, $options)) {

        $PAGE->requires->js('/corplms/core/js/lib/jquery-ui-1.10.4.custom.min.js');

    }

    // If dialog
    if (in_array(CORPLMS_JS_DIALOG, $options)) {

        $PAGE->requires->js('/corplms/core/js/lib/jquery-ui-1.10.4.custom.min.js');

        // Load required strings into the JS global namespace in the form
        // M.str.COMPONENT.IDENTIFIER, eg; M.str.corplms_core['save']. Can also
        ...
        $PAGE->requires->strings_for_js(array('save', 'delete'), 'corplms_core');
        $PAGE->requires->strings_for_js(array('ok', 'cancel'), 'moodle');

        // Include the corplms_dialog JS module. Args supplied to the module's
        ...
        // which is then available via M.corplms_dialog.config.id once the module
        ...
        $jsmodule = array(
                'name' => 'corplms_dialog',
                'fullpath' => '/corplms/core/js/lib/corplms_dialog.js',
                'requires' => array('json'));
        $PAGE->requires->js_init_call('M.corplms_dialog.init', null, false, $jsmodule);
    }

    // If treeview enabled
    if (in_array(CORPLMS_JS_TREEVIEW, $options)) {

        $PAGE->requires->js('/corplms/core/js/lib/jquery.treeview.min.js');

    }

    // If datepicker enabled
    if (in_array(CORPLMS_JS_DATEPICKER, $options)) {
        ...
        $file = "/corplms/core/js/lib/i18n/jquery.ui.datepicker-{$lang}.js";
    ...
    if (in_array(CORPLMS_JS_PLACEHOLDER, $options)) {
        $PAGE->requires->js('/corplms/core/js/lib/jquery.placeholder.min.js');
        $PAGE->requires->js('/corplms/core/js/lib/load.placeholder.js');

    }

    // If Icon preview is enabled
    if (in_array(CORPLMS_JS_ICON_PREVIEW, $options)) {

        $PAGE->requires->js('/corplms/core/js/icon.preview.js');

    }

    if (in_array(CORPLMS_JS_DATATABLES, $options)) {
        $PAGE->requires->js('/corplms/core/js/lib/jquery.dataTables.min.js');
    }
}
...
function build_datepicker_js($selector, $dateformat=null) {
    ...
    if (empty($dateformat)) {
        $dateformat = get_string('datepickerlongyeardisplayformat', 'corplms_core');
    }
    $button_img = array('t/calendar', 'corplms_core');
    $args = array($selector, $dateformat, $button_img);
    $PAGE->requires->js_init_call('M.corplms_core.build_datepicker', $args);
}
...
function build_treeview($elements, $error_string, $hierarchy = null, $disabledlist = array()) {
    ...
    $maxitems = CORPLMS_DIALOG_MAXITEMS;
    ...
    if (is_array($elements) && !empty($elements)) {
        ...
        if(count($elements) > $maxitems) {
            ...
            $html .= get_string('error:morethanxitemsatthislevel', 'corplms_core', $maxitems);
            $html .= ' <a href="#search-tab" onclick="$(\'#tabs\').tabs(\'select\', 1);return false;">';
            $html .= get_string('trysearchinginstead', 'corplms_core');
            $html .= '</a>';
...
function build_nojs_treeview($elements, $error_string, $actionurl, $actionparams, $expandurl, $expandparams, $parents = array(), $disabledlist = array()) {
    ...
    if (is_array($elements) && !empty($elements)) {
        ...
        foreach ($elements as $element) {
            ...
            $cells = array();
            $cells[] = new html_table_cell($OUTPUT->single_button(new moodle_url($actionurl, $params), get_string('assign','corplms_hierarchy'), 'get', array('disabled' => array_key_exists($element->id, $disabledlist))));
...
function build_nojs_breadcrumbs($hierarchy, $parentid, $url, $urlparams, $allfws=true) {
    ...
    if ($allfws) {
        $first = false;
        $html .= html_writer::tag('li', html_writer::link($nofwurl, get_string('allframeworks','corplms_hierarchy')), array('class' => 'first'));
    }
    if ($parentid) {
...
function build_nojs_frameworkpicker($hierarchy, $url, $urlparams) {
    ...
    if ($fws = $DB->get_records($hierarchy->shortprefix.'_framework', null, 'sortorder')) {
        ...
        $out .= html_writer::tag('p', get_string('pickaframework','corplms_hierarchy'));
        ...
        print_error($hierarchy->prefix . 'noframeworks', 'corplms_hierarchy');
    }
}
...
function build_nojs_positionpicker($url, $urlparams) {
    ...
    if ($positions) {
        ...
        foreach ($positions as $position) {
            ...
            case 1:
                $html .= ' (' . get_string('typeprimary', 'corplms_hierarchy') . ')';
                break;
            case 2:
                $html .= ' (' . get_string('typesecondary', 'corplms_hierarchy') . ')';
                break;
            case 3:
                $html .= ' (' . get_string('typeaspirational', 'corplms_hierarchy') . ')';
                break;
            }
