...
*/
class corplms_core_renderer extends plugin_renderer_base {
    ...
    */
    public function corplms_print_active_users($activeusers) {
        $output = '';
        $output .= $this->output->box_start('generalbox adminwarning');
        $output .= get_string('numberofactiveusers', 'corplms_core', $activeusers);
     ...
     * @return string HTML to output.
     */
    public function corplms_print_errorlog_link($latesterror) {
        $output = '';
        $output .= $this->output->box_start('generalbox adminwarning');
        $output .= get_string('lasterroroccuredat', 'corplms_core', userdate($latesterror->timeoccured));
        $output .= $this->output->single_button(new moodle_url('/admin/index.php', array('geterrors' => 1)), get_string('downloaderrorlog', 'corplms_core'), 'post');
        $output .= $this->output->box_end();
        return $output;
    }

    /**
     * Outputs a block containing corplms copyright information
     *
     * @param string $corplms_release A corplms release version, for inclusion in the block
     *
     * @return string HTML to output.
     */
    public function corplms_print_copyright($corplms_release) {
        $output = '';
        $output .= $this->output->box_start('generalbox adminwarning corplms-copyright');
        $text = get_string('corplmslogo', 'corplms_core');
        $icon = new pix_icon('logo', $text, 'corplms_core',
            array('width' => 253, 'height' => 177, 'class' => 'corplmslogo'));
        $url = new moodle_url('http://www.corplmslms.com');
        $output .= $this->output->action_icon($url, $icon, null, array('target' => '_blank'));
        $output .= html_writer::empty_tag('br');
        $output .= html_writer::empty_tag('br');
        $text = get_string('version') . ' ' . $corplms_release;
        $url = new moodle_url('http://www.corplmslms.com');
        $attributes = array('href' => $url, 'target' => '_blank');
        $output .= html_writer::tag('a', $text, $attributes);
        $output .= html_writer::empty_tag('br');
        $output .= html_writer::empty_tag('br');
        $output .= get_string('corplmscopyright', 'corplms_core');
        $output .= $this->output->box_end();
        return $output;
    }
    ...
    */
    public function print_my_team_nav($numteammembers) {
        ...
        $text = get_string('viewmyteam','corplms_core');
        $icon = new pix_icon('teammembers', $text, 'corplms_core');
        $url = new moodle_url('/my/teammembers.php');
        $content = $this->output->action_icon($url, $icon);
        $content .= html_writer::link($url, $text);
        $content .= html_writer::tag('span', get_string('numberofstaff', 'corplms_core', $numteammembers));
        return $content;
    }
    ...
    */
    public function print_report_manager($reports, $canedit) {
        ...
        foreach ($reports as $report) {
            ...
            if (!isset($report->url)) {
                debugging(get_string('error:reporturlnotset', 'corplms_reportbuilder', $report->fullname), DEBUG_DEVELOPER);
                continue;
            }
            ...
            $icon = html_writer::empty_tag(
                    'img',
                    array('src' => $this->output->pix_url('report_icon', 'corplms_reportbuilder'),
                    'alt'=> $text)
            );
            ...
            if ($canedit) {
                $text = get_string('settings','corplms_core');
                $icon = html_writer::empty_tag('img', array('src' => $this->output->pix_url('/t/edit'),
                                                'alt'=> $text));
                $url = new moodle_url('/corplms/reportbuilder/general.php?id='.$report->id);
                $attributes = array('href' => $url);
    ...
    */
    public function print_scheduled_reports($scheduledreports, $showoptions=true) {
        ...
        $headers = array();
        $headers[] = get_string('reportname', 'corplms_reportbuilder');
        $headers[] = get_string('savedsearch', 'corplms_reportbuilder');
        $headers[] = get_string('format', 'corplms_reportbuilder');
        if (get_config('reportbuilder', 'exporttofilesystem') == 1) {
            $headers[] = get_string('exportfilesystemoptions', 'corplms_reportbuilder');
        }
        $headers[] = get_string('schedule', 'corplms_reportbuilder');
        if ($showoptions) {
            $headers[] = get_string('options', 'corplms_core');
        }
        $table->head = $headers;
        ...
        foreach ($scheduledreports as $sched) {
            ...
            if ($showoptions) {
                ...
                $url = new moodle_url('/corplms/reportbuilder/scheduled.php', array('id' => $sched->id));
                $attributes = array('href' => $url);
                ...
                $icon = html_writer::empty_tag('img', array('src' => $this->output->pix_url('/t/delete'),
                                                        'alt' => $text, 'class' =>'iconsmall'));
                $url = new moodle_url('/corplms/reportbuilder/deletescheduled.php', array('id' => $sched->id));
                $attributes = array('href' => $url);
                $cellcontent .= html_writer::tag('a', $icon, $attributes);
    ...
    public function print_my_courses($displaycourses, $userid) {
        ...
        if (count($displaycourses) > 0) {
            ...
            $headers[] = $cell;
            $cell = new html_table_cell(get_string('enrolled', 'corplms_core'));
            $cell->attributes['class'] = 'enroldate';
            $headers[] = $cell;
            $cell = new html_table_cell(get_string('started','corplms_core'));
            $cell->attributes['class'] = 'startdate';
            $headers[] = $cell;
            $cell = new html_table_cell(get_string('completed','corplms_core'));
            ...
            $url = new moodle_url('/corplms/plan/record/courses.php?userid='.$userid);
            $attributes = array('href' => $url);
            $content .= html_writer::tag('a', get_string('allmycourses','corplms_core'), $attributes);
            $content .= html_writer::end_tag('div');
        } else {
            $content = html_writer::start_tag('span', array('class' => 'noenrollments'));
            $content .= get_string('notenrolled','corplms_core');
            $content .= html_writer::end_tag('span');
        }
        ...
        $output .= get_string('mycoursecompletions','corplms_core');
    ...
    */
    public function print_corplms_search($action, $hiddenfields = null, $placeholder = '', $value = '', $formid = null, $inputid = null) {
    ...
    /**
     * Generate markup for corplms menu
     */
    public function print_corplms_menu($menudata, $parent=null, $selected_items=array()) {
        global $PAGE;
        $PAGE->requires->jquery();
        static $menuinited = false;
        if (!$menuinited) {
            $PAGE->requires->yui_module('moodle-corplms_core-corplmsmenu', 'M.coremenu.setfocus.init');
            $menuinited = true;
        }
        ...
        if (!$selected_items && $PAGE->corplms_menu_selected) {
            $relationships = array();
            foreach ($menudata as $item) {
                ...
                if ($item->parent) {
                    ...
                    if (!empty($relationships[$item->parent])) {
                        ...
                        throw new coding_exception('Corplms menu definition is incorrect');
                    }
                }
            }

            if (array_key_exists($PAGE->corplms_menu_selected, $relationships)) {
                $selected_items = $relationships[$PAGE->corplms_menu_selected];
            }
        }
        ...
        if ($numitems > 0) {
            ...
            foreach ($currentlevel as $menuitem) {
                ...
                $output .= $this->print_corplms_menu($menudata, $menuitem->name, $selected_items);
                $output .= html_writer::end_tag('li');
     ...
     */
    public function print_corplms_notifications() {
        $output = '';
        // Display notifications set with corplms_set_notification()
        $notices = corplms_get_notifications();
     ...
     */
    public function print_corplms_progressbar($percent, $size='medium', $showlabel=false, $tooltip='DEFAULTTOOLTIP') {
        ...
        case 'large' :
            ...
            $class = 'corplms_progress_bar_large';
            break;
        case 'medium' :
        default :
            ...
            $class = 'corplms_progress_bar_medium';
            break;
        }

        if ($tooltip == 'DEFAULTTOOLTIP') {
            $tooltip = get_string('xpercent', 'corplms_core', $percent);
        }

        $out = '';

        $out .= $this->pix_icon($bar_foreground, $tooltip, 'corplms_core', array('title' => $tooltip, 'style' => 'background-position: ' . $pixeloffset . 'px 0px;', 'class' => $class));
        if ($showlabel) {
            $out .= ' ' . get_string('xpercentcomplete', 'corplms_core', $percent) . html_writer::empty_tag('br');
        }
    ...
    /**
     * Renders a Corplms-style HTML comment template to be used by the comments engine
     *
     * @return string Corplms-style HTML comment template
     */
    ...
    public function print_icons_list($type = 'course') {
        ...
        $fs = get_file_storage();
        $files = $fs->get_area_files(context_system::instance()->id, 'corplms_core', $type, 0, 'itemid', false);
        ...
        foreach ($files as $file) {
            ...
            $url = moodle_url::make_pluginfile_url($file->get_contextid(), 'corplms_core',
        ...
        foreach (scandir($path) as $icon) {
            ...
            $out .= $this->output->pix_icon('/' . $type . 'icons/' . $iconid, $iconname, 'corplms_core',
    ...
    /**
     * Render Corplms information on user profile.
     *
     * @param $userid ID of a user.
     * @return string HTML to output.
     */
    public function print_corplms_user_profile($userid) {
        global $USER, $CFG;
        ...
        $canviewROL = has_capability('corplms/core:viewrecordoflearning', $usercontext);
        // Record of learning.
        if ($currentuser || corplms_is_manager($userid) || $canviewROL) {
            $strrol = get_string('recordoflearning', 'corplms_core');
            $urlrol = new moodle_url('/corplms/plan/record/index.php', array('userid' => $userid));
            echo html_writer::tag('dt', $strrol);
            echo html_writer::tag('dd', html_writer::link($urlrol, $strrol));
        }

        // Learning plans.
        if (corplms_feature_visible('learningplans') && dp_can_view_users_plans($userid)) {
            $strplans = get_string('learningplans', 'corplms_plan');
            $urlplans = new moodle_url('/corplms/plan/index.php', array('userid' => $userid));
    ...
    public function show_text_broken_rules($brokenrules = null) {
        ...
        if (is_null($brokenrules)) {
            $brokenrules = corplms_get_text_broken_rules();
        }

        if (!empty($brokenrules)) {
            $content = array();
            $warning = get_string('cohortbugneedfixing', 'corplms_cohort');
            ...
            $table->head = array(get_string('cohorts', 'corplms_cohort'), get_string('rules', 'corplms_cohort'));
            foreach ($content as $key => $value) {
                list($id, $name) = explode(',', $key);
                $cohortlink = html_writer::link(new moodle_url('/corplms/cohort/rules.php', array('id' => $id)), $name,
                    array('target' => '_blank'));
