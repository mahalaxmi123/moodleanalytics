 ...
 * @package corplms
 * @subpackage corplms_core/dialogs
 */

defined('CORPLMS_DIALOG_SEARCH') || die();

require_once("{$CFG->dirroot}/corplms/core/dialogs/search_form.php");
 ...
 *     corplms_dialog_get_search_clause()
 */
switch ($searchtype) {
    /**
     * User search
    ...
    case 'user':
        ...
        list($searchsql, $params) = corplms_search_get_keyword_where_clause($keywords, $fields);
    ...
    case 'coursecompletion':
        ...
        list($searchsql, $params) = corplms_search_get_keyword_where_clause($keywords, $fields, SQL_PARAMS_NAMED);
        ...
        list($visibilitysql, $visibilityparams) = corplms_visibility_where($USER->id, 'c.id', 'c.visible', 'c.audiencevisible');
    ...
    case 'program':
        ...
        list($searchsql, $params) = corplms_search_get_keyword_where_clause($keywords, $fields, SQL_PARAMS_NAMED);
        list($visibilitysql, $visibilityparams) = corplms_visibility_where(null,
                                                                          'p.id',
                                                                          'p.visible',
                                                                          'p.audiencevisible',
                                                                          'p',
    ...
    default:
        print_error('invalidsearchtable', 'corplms_core');
}
...
if (strlen($query)) {
    ...
    $strqueryerror = get_string('queryerror', 'corplms_core');
    ...
    if ($total) {
        if ($results) {
            ...
            $dialog = new corplms_dialog_content();
            $dialog->items = array();
            ...
            foreach ($results as $result) {
                ...
                if (isset($result->email)) {
                    ...
                    $item->fullname = get_string('assignindividual', 'corplms_program', $username);
                } else {
        ...
        $message = get_string('noresultsfor', 'corplms_core', $params);

        if (!empty($frameworkid)) {
            $params->framework = $DB->get_field($shortprefix.'_framework', 'fullname', array('id' => $frameworkid));
            $message = get_string('noresultsforinframework', 'corplms_hierarchy', $params);
        }
