...
class corplms_dialog_content_goals extends corplms_dialog_content {
    ...
    public function get_frameworks() {
        $list = array();
        if ($this->showcompany) {
            $list[goal::SCOPE_COMPANY] = get_string('companygoals', 'corplms_hierarchy');
        }
        if ($this->showpersonal) {
            $list[goal::SCOPE_PERSONAL] = get_string('personalgoals', 'corplms_hierarchy');
        }
        return $list;
    }
