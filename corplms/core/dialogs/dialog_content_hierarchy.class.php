 ...
 * @access  public
 */
class corplms_dialog_content_hierarchy extends corplms_dialog_content {
     ...
     *
     * @see     corplms_dialog_hierarchy::skip_access_checks
     *
     * @access  public
    ...
    public function __construct($prefix, $frameworkid = 0, $showhidden = false, $skipaccesschecks = false) {
        ...
        if (!$this->skip_access_checks) {
            require_login();
            require_capability("corplms/hierarchy:view{$prefix}", context_system::instance());
        }
        ...
        if (empty($this->framework)) {
            echo '<p>' . get_string($prefix.'noframeworkssetup', 'corplms_hierarchy') . '</p>';
            die();
        }
 ...
 * @access  public
 */
class corplms_dialog_content_hierarchy_multi extends corplms_dialog_content_hierarchy {
