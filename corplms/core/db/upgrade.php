...
/**
 * Local db upgrades for Corplms Core
 */
 ...
 */
function xmldb_corplms_core_upgrade($oldversion) {
    ...
    if ($oldversion < 2012052802) {
        ...
        // distinct from the corplms "Staff Manager"
        if ($managerroleid = $DB->get_field('role', 'id', array('shortname' => 'manager', 'name' => get_string('manager', 'role')))) {
            $todb = new stdClass();
            $todb->id = $managerroleid;
            $todb->name = get_string('sitemanager', 'corplms_core');
            $DB->update_record('role', $todb);
        }

        corplms_upgrade_mod_savepoint(true, 2012052802, 'corplms_core');
    }
    ...
    if ($oldversion < 2012061200) {
        ...
        corplms_upgrade_mod_savepoint(true, 2012061200, 'corplms_core');
    }

    /*
     * Move Corplms 1.1 dashlets to Corplms 2.x mymoodle
     */
    if ($oldversion < 2012062900) {
        ...
        $quicklinks_defaultinstances = $DB->get_fieldset_sql("
            ...
            WHERE di.userid = 0
                ...
                AND bi.blockname = 'corplms_quicklinks'
        ");
        ...
        if ($mypageid = $DB->get_field_sql('SELECT id FROM {my_pages} WHERE userid IS null AND private = 1')) {
            ...
            // List of Corplms blocks for default pages
            $defaultblocks = array('corplms_quicklinks', 'corplms_tasks', 'corplms_alerts', 'corplms_stats');

            // Install new Corplms blocks to default mymoodle page
            foreach ($defaultblocks as $block) {
                // put tasks and alerts in the middle, others on the side
                if ($block == 'corplms_tasks' || $block == 'corplms_alerts' || $block == 'corplms_recent_learning') {
        ...
        // get all corplms dashboard users (except deleted users)
        ...
        foreach ($dashusers as $user) {
            ...
            if (!($DB->record_exists('my_pages', array('userid' => $user->userid)))) {
                ...
                foreach ($dashletinstances as $instance) {
                    ...
                    // put tasks and alerts in the middle, others on the side
                    if ($instance->blockname == 'corplms_alerts' || $instance->blockname == 'corplms_tasks' || $instance->blockname == 'corplms_recent_learning') {
                    ...
                    if (!(in_array($instance->blockname, $userblocks))) {
                        ...
                        if ($instance->blockname == 'corplms_quicklinks') {
        ...
        $DB->delete_records('block_instances', array('pagetypepattern' => 'corplms_dashboard'));

        corplms_upgrade_mod_savepoint(true, 2012062900, 'corplms_core');
    }


    if ($oldversion < 2012080100) {
        // readd corplms specific course completion changes for anyone
        // who has already upgraded from moodle 2.2.2+
        corplms_readd_course_completion_changes();
        corplms_upgrade_mod_savepoint(true, 2012080100, 'corplms_core');
    }

    if ($oldversion < 2012080101) {
        // remove OAuth plugin
        // Google fusion export will use repository/gdrive integration instead
        uninstall_plugin('corplms', 'oauth');
        corplms_upgrade_mod_savepoint(true, 2012080101, 'corplms_core');
    }

    if ($oldversion < 2012081300) {
        //turn off forceunique for any filepicker corplms customfields
        ...
        corplms_upgrade_mod_savepoint(true, 2012081300, 'corplms_core');
    }
    ...
    if ($oldversion < 2012090500) {
        ...
        corplms_upgrade_mod_savepoint(true, 2012090500, 'corplms_core');
    }
    ...
    if ($oldversion < 2012102400) {
        ...
        foreach ($completions as $completion) {
            ...
            if (!$DB->record_exists('block_corplms_stats', $data)) {
                corplms_stats_add_event($completion->timecompleted, $data['userid'], STATS_EVENT_COURSE_COMPLETE, '', $data['data2']);
            }
        }
        $completions->close();
        corplms_upgrade_mod_savepoint(true, 2012102400, 'corplms_core');
    }

    if ($oldversion < 2012121200) {
        // remove hardcoded names and descriptions for corplms core roles
        ...
        corplms_upgrade_mod_savepoint(true, 2012121200, 'corplms_core');
    }
    ...
    if ($oldversion < 2013041000) {
        ...
        corplms_upgrade_mod_savepoint(true, 2013041000, 'corplms_core');
    }
    ...
    if ($oldversion < 2013041500) {
        ...
        if (!$availablelangs = $installer->get_remote_list_of_languages()) {
            $notice_error[] = get_string('cannotdownloadcorplmslanguageupdatelist', 'corplms_core');
        } else {
            ...
            foreach ($langdirs as $lang) {
                ...
                //if this installed lang is a properly configured language that also exists on the Corplms lang site, add it to the update list
        ...
        corplms_upgrade_mod_savepoint(true, 2013041500, 'corplms_core');
    }
    ...
    if ($oldversion < 2013042300) {
        ...
        corplms_upgrade_mod_savepoint(true, 2013042300, 'corplms_core');
    }
    ...
    if ($oldversion < 2013042600) {
        ...
        // Add corplms block instances to context
        $bisql = "SELECT id FROM {block_instances}
                  WHERE blockname IN ('corplms_stats', 'corplms_alerts', 'corplms_tasks')";
        $corplmsblocks = $DB->get_records_sql($bisql);
        foreach ($corplmsblocks as $block) {
            context_block::instance($block->id);
        }

        corplms_upgrade_mod_savepoint(true, 2013042600, 'corplms_core');
    }
    ...
    if ($oldversion < 2013061800) {
      ...
      LEFT OUTER JOIN {role} r on cc.role=r.id
        ...
        corplms_upgrade_mod_savepoint(true, 2013061800, 'corplms_core');
    }
    ...
    if ($oldversion < 2013070800) {
        ...
        corplms_upgrade_mod_savepoint(true, 2013070800, 'corplms_core');
    }
    ...
    if ($oldversion < 2013070801) {
        ...
        // Main savepoint reached.
        corplms_upgrade_mod_savepoint(true, 2013070801, 'corplms_core');
    }
    ...
    if ($oldversion < 2013070802) {
        ...
        corplms_upgrade_mod_savepoint(true, 2013070802, 'corplms_core');
    }
    ...
    if ($oldversion < 2013070803) {
        ...
        corplms_upgrade_mod_savepoint(true, 2013070803, 'corplms_core');
    }
    ...
    if ($oldversion < 2013070804) {
        ...
        // Main savepoint reached.
        corplms_upgrade_mod_savepoint(true, 2013070804, 'corplms_core');
    }
    ...
    if ($oldversion < 2013091500) {
        ...
        corplms_upgrade_mod_savepoint(true, 2013091500, 'corplms_core');
    }
    ...
    if ($oldversion < 2013092000) {
        ...
        upgrade_plugin_savepoint(true, 2013092000, 'corplms', 'core');
    }
    ...
    if ($oldversion < 2013092100) {
        ...
        // Main savepoint reached.
        corplms_upgrade_mod_savepoint(true, 2013092100, 'corplms_core');
    }
    ...
    if ($oldversion < 2013101100) {
        ...
        // Main savepoint reached.
        corplms_upgrade_mod_savepoint(true, 2013101100, 'corplms_core');
    }
    ...
    if ($oldversion < 2013101800) {
        ...
        corplms_upgrade_mod_savepoint(true, 2013101800, 'corplms_core');
    }
    ...
    if ($oldversion < 2013102300) {
        ...
        corplms_upgrade_mod_savepoint(true, 2013102300, 'corplms_core');
    }
    ...
    if ($oldversion < 2013102900) {
        ...
        corplms_upgrade_mod_savepoint(true, 2013102900, 'corplms_core');
    }
    ...
    if ($oldversion < 2013120300) {
        ...
        corplms_upgrade_mod_savepoint(true, 2013120300, 'corplms_core');
    }
    ...
    if ($oldversion < 2014010700) {
        ...
        corplms_upgrade_mod_savepoint(true, 2014010700, 'corplms_core');
    }
    ...
    if ($oldversion < 2014030800) {
        ...
        corplms_upgrade_mod_savepoint(true, 2014030800, 'corplms_core');
    }
    ...
    if ($oldversion < 2014031901) {
        ...
        corplms_upgrade_mod_savepoint(true, 2014031901, 'corplms_core');
    }

    if ($oldversion < 2014032000) {
        global $CFG;
        require_once($CFG->dirroot . '/corplms/core/corplms.php');

        // Set features as they were before (enable/disable).
        $featureslist = corplms_advanced_features_list();
        foreach ($featureslist as $feature) {
            $cfgsetting = "enable{$feature}";
            if (!isset($CFG->$cfgsetting)) {
                set_config($cfgsetting, CORPLMS_SHOWFEATURE);
            } else if ($CFG->$cfgsetting == 0) {
                set_config($cfgsetting, CORPLMS_HIDEFEATURE);
            }
        }

        corplms_upgrade_mod_savepoint(true, 2014032000, 'corplms_core');
    }
    ...
    if ($oldversion < 2014041500) {
        ...
        $DB->execute($sql, array('America/Indiana/Indianapolis', 'America/Indianapolis'));
        corplms_upgrade_mod_savepoint(true, 2014041500, 'corplms_core');
    }
    ...
    if ($oldversion < 2014051200) {
        ...
        $hasrun = get_config('corplms_core', 'completion_reaggregation_fix_has_run');
        if (empty($hasrun)) {

            $previousversion = get_config('corplms_core', 'previous_version');
            ...
            set_config('completion_reaggregation_fix_has_run', 1, 'corplms_core');
        }

        corplms_upgrade_mod_savepoint(true, 2014051200, 'corplms_core');
    }
    ...
    if ($oldversion < 2014081200) {
        ...
        corplms_upgrade_mod_savepoint(true, 2014081200, 'corplms_core');
    }

    if ($oldversion < 2014082700) {
        require_once(dirname(__FILE__) . '/upgradelib.php');
        corplms_core_fix_old_upgraded_mssql();
        upgrade_plugin_savepoint(true, 2014082700, 'corplms', 'core');
    }

    // This is Corplms 2.7 upgrade line.
    ...
    if ($oldversion < 2014100700) {
        ...
        corplms_upgrade_mod_savepoint(true, 2014100700, 'corplms_core');
    }
    ...
    if ($oldversion < 2014100701) {
        ...
        // Core savepoint reached.
        corplms_upgrade_mod_savepoint(true, 2014100701, 'corplms_core');
    }
    ...
    if ($oldversion < 2014100800) {
        ...
        corplms_upgrade_mod_savepoint(true, 2014100800, 'corplms_core');
    }

    if ($oldversion < 2014100902) {
        // Removing the themes from core and old Corplms.
        $themes = array('standard', 'standardold', 'clean', 'more', 'customcorplms', 'kiwifruit',
                        'standardcorplms', 'mymobilecorplms', 'canvas');
        ...
        corplms_upgrade_mod_savepoint(true, 2014100902, 'corplms_core');
    }

    if ($oldversion < 2014100903) {
        if (file_exists($CFG->dataroot.'/environment/environment.xml')) {
            // Corplms cannot use Moodle environment files and there is no update mechanism.
            unlink($CFG->dataroot.'/environment/environment.xml');
        }
        corplms_upgrade_mod_savepoint(true, 2014100903, 'corplms_core');
    }
    ...
    if ($oldversion < 2014101400) {
        ...
        corplms_upgrade_mod_savepoint(true, 2014101400, 'corplms_core');
    }
    ...
    if ($oldversion < 2014101401) {
        ...
        corplms_upgrade_mod_savepoint(true, 2014101401, 'corplms_core');
    }
    ...
    if ($oldversion < 2014101402) {
        ...
        corplms_upgrade_mod_savepoint(true, 2014101402, 'corplms_core');
    }

    if ($oldversion < 2014102000) {

        $table = new xmldb_table('corplms_navigation');
        ...
        $table->setComment('Corplms navigation menu');

        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        corplms_upgrade_mod_savepoint(true, 2014102000, 'corplms_core');
    }
