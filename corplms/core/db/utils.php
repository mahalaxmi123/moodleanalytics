...
/**
* Corplms Module upgrade savepoint, marks end of Corplms module upgrade blocks
...
* @return void
*/
function corplms_upgrade_mod_savepoint($result, $version, $modname) {
    global $DB;
...
/**
* corplms_set_charfield_nullable, for fixing 1.9 char fields where old definition was ISNULL=true and DEFAULT=""
*
* @global object $DB
...
* @return void
*/
function corplms_fix_nullable_charfield($table, $field, $previous, $length='255', $notnull=null, $default=null, $indexes=array(), $fieldtype=XMLDB_TYPE_CHAR) {
    global $DB, $CFG;
 ...
 *                 was nothing to do
 */
function corplms_data_object_duplicate_fix($tablename, $where_sql) {
    global $DB, $OUTPUT;
    ...
    echo $OUTPUT->notification(get_string('error:duplicaterecordsfound', 'corplms_core', $a));
    ...
    foreach ($duplicates as $dup) {
        error_log(get_string('error:duplicaterecordsdeleted', 'corplms_core', $tablename) . var_export((array)$dup, true));
        $ids[] = $dup->id;
    }
...
/**
 * Re-add changes to course completion for Corplms
 *
 ...
 *
 * 1. when corplms first is installed (to fix for anyone upgrading from 2.2.2+)
 * 2. in a corplms core upgrade (to fix for anyone who has already upgraded from 2.2.2+)
 *
 * These changes will only be applied if they haven't been run previously so it's okay
 * to call this function multiple times
 */
function corplms_readd_course_completion_changes() {
    global $CFG, $DB;
    $dbman = $DB->get_manager();
...
/**
 * Fix badges and completion capabilities when upgrading from Corplms 2.4.
 *
 * @return void
 */
function corplms_fix_existing_capabilities() {
    global $DB;

    // Get all existing corplms_core capabilities in the database.
    $cachedcaps = get_cached_capabilities('corplms_core');
    if ($cachedcaps) {
 ...
 */
function corplms_get_nonunique_idnumbers() {
    global $DB;
...
/**
 * Get cohort rules that were broken by expansion of options in Corplms 2.4.8.
 *
 * @return  array List of rules.
 */
function corplms_get_text_broken_rules() {
    global $DB, $CFG;
 ...
 *
 * @param array $rulestofix Array of rules to fix as generated by {@link corplms_get_text_broken_rules()}.
 *
 * @return  void
 */
function corplms_cohort_remap_rules($rulestofix) {
    global $DB, $USER;
 ...
 *
 * @param object $brokenrule Rule as returned by {@link corplms_get_text_broken_rules()}.
 * @return bool True if the rule can be fixed automatically, false otherwise.
 */
function corplms_cohort_is_rule_fixable($brokenrule) {
 ...
 *
 * @param $corplmsinfo Info obtained from corplms_version_info function.
 * @return void.
 */
function corplms_preupgrade($corplmsinfo) {
    global $CFG, $version;
    print_upgrade_part_start('corplms_core', false, true);
    // Save a copy of the version they are upgrading from.
    set_config('previous_version', $corplmsinfo->existingcorplmsversion, 'corplms_core');
    // Check for existence of the three upgrade scripts as necessary - first the any upgrade script.
    $upgradefile = "{$CFG->dirroot}/corplms/core/db/pre_any_upgrade.php";
    ...
    if ($version > $CFG->version) {
        $upgradefile = "{$CFG->dirroot}/corplms/core/db/pre_moodle_upgrade.php";
        if (file_exists($upgradefile)) {
            set_time_limit(0);
            require($upgradefile);
        }
    }
    // Is this a Corplms upgrade?
    ...
    if (version_compare($newversion, $oldversion, '>')) {
        $upgradefile = "{$CFG->dirroot}/corplms/core/db/pre_corplms_upgrade.php";
        if (file_exists($upgradefile)) {
            set_time_limit(0);
            require($upgradefile);
        }
    }
    print_upgrade_part_end('corplms_core', false, true);
}
 ...
 * @return void.
 */
function corplms_postupgrade($corplmsinfo) {
    print_upgrade_part_start('corplms_core', false, true);

    corplms_upgrade_menu();
