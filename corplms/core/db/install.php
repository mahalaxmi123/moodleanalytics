...
function xmldb_corplms_core_install() {
    global $CFG, $DB, $SITE;

    // switch to new default theme in corplms 2.6
    ...
    // Set up blocks.
    corplms_reset_mymoodle_blocks();

    // Set up frontpage.
    set_config('frontpage', '');
    set_config('frontpageloggedin', '');

    // Turn completion on in Corplms by default.
    require_once($CFG->dirroot . '/lib/completionlib.php');
    if(!completion_info::is_enabled_for_site()){
        set_config('corplmscoreinstallation', 1);
    ...
    // was never implemented and is no longer in corplms
    ...
    // Adding some corplms upgrade code from lib/db/upgrade.php to
