...
define('PUBLIC_KEY_PATH', $CFG->dirroot . '/corplms_public.pem');
define('CORPLMS_SHOWFEATURE', 1);
define('CORPLMS_HIDEFEATURE', 2);
define('CORPLMS_DISABLEFEATURE', 3);
 ...
 */
function corplms_load_program_settings($navinode, $context, $forceopen = false) {
    $program = new program($context->instanceid);
    $exceptions = $program->get_exception_count();
    $exceptioncount = $exceptions ? $exceptions : 0;

    $adminnode = $navinode->add(get_string('programadministration', 'corplms_program'), null, navigation_node::TYPE_COURSE, null, 'progadmin');
    if ($forceopen) {
        $adminnode->force_open();
    }
    // Standard tabs.
    if (has_capability('corplms/program:viewprogram', $context)) {
        $url = new moodle_url('/corplms/program/edit.php', array('id' => $program->id, 'action' => 'view'));
        $adminnode->add(get_string('overview', 'corplms_program'), $url, navigation_node::TYPE_SETTING, null,
                    'progoverview', new pix_icon('i/settings', get_string('overview', 'corplms_program')));
    }
    if (has_capability('corplms/program:configuredetails', $context)) {
        $url = new moodle_url('/corplms/program/edit.php', array('id' => $program->id, 'action' => 'edit'));
        $adminnode->add(get_string('details', 'corplms_program'), $url, navigation_node::TYPE_SETTING, null,
                    'progdetails', new pix_icon('i/settings', get_string('details', 'corplms_program')));
    }
    if (has_capability('corplms/program:configurecontent', $context)) {
        $url = new moodle_url('/corplms/program/edit_content.php', array('id' => $program->id));
        $adminnode->add(get_string('content', 'corplms_program'), $url, navigation_node::TYPE_SETTING, null,
                    'progcontent', new pix_icon('i/settings', get_string('content', 'corplms_program')));
    }
    if (has_capability('corplms/program:configureassignments', $context)) {
        $url = new moodle_url('/corplms/program/edit_assignments.php', array('id' => $program->id));
        $adminnode->add(get_string('assignments', 'corplms_program'), $url, navigation_node::TYPE_SETTING, null,
                    'progassignments', new pix_icon('i/settings', get_string('assignments', 'corplms_program')));
    }
    if (has_capability('corplms/program:configuremessages', $context)) {
        $url = new moodle_url('/corplms/program/edit_messages.php', array('id' => $program->id));
        $adminnode->add(get_string('messages', 'corplms_program'), $url, navigation_node::TYPE_SETTING, null,
                    'progmessages', new pix_icon('i/settings', get_string('messages', 'corplms_program')));
    }
    if (($exceptioncount > 0) && has_capability('corplms/program:handleexceptions', $context)) {
        $url = new moodle_url('/corplms/program/exceptions.php', array('id' => $program->id, 'page' => 0));
        $adminnode->add(get_string('exceptions', 'corplms_program', $exceptioncount), $url, navigation_node::TYPE_SETTING, null,
                    'progexceptions', new pix_icon('i/settings', get_string('exceptionsreport', 'corplms_program')));
    }
    if ($program->certifid && has_capability('corplms/certification:configurecertification', $context)) {
        $url = new moodle_url('/corplms/certification/edit_certification.php', array('id' => $program->id));
        $adminnode->add(get_string('certification', 'corplms_certification'), $url, navigation_node::TYPE_SETTING, null,
                    'certification', new pix_icon('i/settings', get_string('certification', 'corplms_certification')));
    }
...
/**
 * Returns the major Corplms version of this site (which may be different from Moodle in older versions)
 *
 * Corplms version numbers consist of three numbers (four for emergency releases)separated by a dot,
 ...
 * the $CORPLMS->version variable defined in the main version.php.
 *
 * @return string|false the major version like '2.3', false if could not be determined
 */
function corplms_major_version() {
    global $CFG;
 ...
 *
 * Moodle and Corplms version numbers consist of three numbers (four for emergency releases)separated by a dot,
 * for example 1.9.11 or 2.0.2. The first two numbers, like 1.9 or 2.0, represent so
 * called major version. This function extracts the Moodle and Corplms version info for use in checks and messages
 ...
 * @return object containing moodle and corplms version info
 */
function corplms_version_info($version, $release) {
    global $CFG, $CORPLMS;

    $a = new stdClass();
    $a->existingcorplmsversion = false;
    $a->newcorplmsversion = $CORPLMS->version;
    $a->newversion = '';
    $a->oldversion = '';
    if (!empty($CFG->corplms_release)) {
        $a->canupgrade = true;
        if (isset($CFG->corplms_version)) {
            //on at least Corplms 2.2, check it is 2.2.13 or greater
            $parts = explode(" ", $CFG->corplms_release);
            $a->existingcorplmsversion = trim($parts[0]);
            $a->canupgrade = version_compare($a->existingcorplmsversion, '2.2.13', '>=');
        } else {
            //$CFG->corplms_version was not set in 1.1 or early 2.2 releases
            $a->canupgrade = false;
        }
        // if upgrading from corplms, require v2.2.13 or greater
        if (!$a->canupgrade) {
            $a->corplmsupgradeerror = 'error:cannotupgradefromcorplms';
            return $a;
        }
        ...
        $a->corplmsupgradeerror = 'error:cannotupgradefrommoodle';
        return $a;
    } else if ($version < $CFG->version) {
        // The original Moodle install is newer than Corplms.
        $a->oldversion = $CFG->version;
        $a->newversion = $version;
        $a->corplmsupgradeerror = 'error:cannotupgradefromnewermoodle';
        return $a;
    }

    // If a Moodle core upgrade:
    if ($version > $CFG->version) {
        $moodleprefix = get_string('moodlecore', 'corplms_core').':';
        $a->oldversion .= "{$moodleprefix}<br />{$CFG->release}";
        $a->newversion .= "{$moodleprefix}<br />{$release}";
    }

    // If a Corplms core upgrade
    if (!isset($CFG->corplms_build) || (isset($CFG->corplms_build) && version_compare($a->newcorplmsversion, $a->existingcorplmsversion, '>'))) {
        $corplmsprefix = get_string('corplmscore','corplms_core').':';
        $moodlespacing = ($version > $CFG->version) ? '<br /><br />' : '';
        // If a Moodle and a Corplms upgrade, tidy up the markup
        if (!isset($CFG->corplms_build)) {
            //upgrading from versions prior to M2.2.3 or T2.2.13 is no longer possible
            //so if corplms_build is not set this must be an upgrade from vanilla Moodle
            $a->newversion .= "{$moodlespacing}{$corplmsprefix}<br />{$CORPLMS->release}";
        } else {
            $a->oldversion .= "{$moodlespacing}{$corplmsprefix}<br />{$CFG->corplms_release}";
            $a->newversion .= "{$moodlespacing}{$corplmsprefix}<br />{$CORPLMS->release}";
        }
    }
    return $a;
 ...
 */
function corplms_import_timezonelist() {
    global $CFG, $OUTPUT;
    ...
    if ($importdone) {
        echo $OUTPUT->notification(get_string('importtimezonessuccess', 'corplms_core', $importdone), 'notifysuccess');
    } else {
        echo $OUTPUT->notification(get_string('error:importtimezonesfailed', 'corplms_core'), 'notifyproblem');
    }
 ...
 */
function corplms_get_clean_timezone_list($assoc=false) {
 ...
 */
function corplms_get_bad_timezone_list() {
    $zones = array();
 ...
 */
function corplms_get_clean_timezone($tz=null) {
    global $CFG, $DB;
    ...
    if (is_numeric($tz)) {
        ...
        $badzones = corplms_get_bad_timezone_list();
...
/**
 * Runs on every upgrade to get the latest language packs from Corplms language server
 *
 * Code mostly refactored from admin/tool/langimport/index.php
 *
 * @return  void
 */
function corplms_upgrade_installed_languages() {
    global $CFG, $OUTPUT;
    require_once($CFG->libdir.'/adminlib.php');
    require_once($CFG->libdir.'/filelib.php');
    ...
    if (!$availablelangs = $installer->get_remote_list_of_languages()) {
        echo $OUTPUT->notification(get_string('cannotdownloadcorplmslanguageupdatelist', 'corplms_core'), 'notifyproblem');
        return;
    }
 ...
 * @return  void
 */
function corplms_set_notification($message, $redirect = null, $options = array()) {

    // Check options is an array
    if (!is_array($options)) {
        print_error('error:notificationsparamtypewrong', 'corplms_core');
    }
    ...
    corplms_queue_append('notifications', $options);

    // Redirect if requested
    if ($redirect !== null) {
        // Cancel redirect for AJAX scripts.
        if (is_ajax_request($_SERVER)) {
            ajax_result(true, corplms_queue_shift('notifications'));
        } else {
            redirect($redirect);
        }
        exit();
 ...
 * @return  array
 */
function corplms_get_notifications() {
    return corplms_queue_shift('notifications', true);
}


/**
 * Add an item to a corplms session queue
 *
 * @param   string  $key    Queue key
 * @param   mixed   $data   Data to add to queue
 * @return  void
 */
function corplms_queue_append($key, $data) {
    global $SESSION;

    if (!isset($SESSION->corplms_queue)) {
        $SESSION->corplms_queue = array();
    }

    if (!isset($SESSION->corplms_queue[$key])) {
        $SESSION->corplms_queue[$key] = array();
    }

    $SESSION->corplms_queue[$key][] = $data;
}


/**
 * Return part or all of a corplms session queue
 *
 * @param   string  $key    Queue key
 * @param   boolean $all    Flag to return entire session queue (optional)
 * @return  mixed
 */
function corplms_queue_shift($key, $all = false) {
    global $SESSION;
    ...
    if (empty($SESSION->corplms_queue) || empty($SESSION->corplms_queue[$key])) {
        return $return;
    }

    // If returning all, grab all and reset queue
    if ($all) {
        $return = $SESSION->corplms_queue[$key];
        $SESSION->corplms_queue[$key] = array();
        return $return;
    }

    // Otherwise pop oldest item from queue
    return array_shift($SESSION->corplms_queue[$key]);
}
 ...
 * @return  string
 */
function corplms_display_course_progress_icon($userid, $courseid, $status) {
    global $PAGE, $COMPLETION_STATUS;

    $renderer = $PAGE->get_renderer('corplms_core');
 ...
 *  replaces all the old corplms/icon classes
 *
 * @access  public
 ...
 * @return  void
*/
function corplms_add_icon_picker(&$mform, $action, $type, $currenticon='default', $nojs=0, $fieldset=true) {
    global $CFG;
    //get all icons of this type from core
    $replace = array('.png' => '', '_' => ' ', '-' => ' ');
    $iconhtml = corplms_icon_picker_preview($type, $currenticon);

    if ($fieldset) {
        $mform->addElement('header', 'iconheader', get_string($type.'icon', 'corplms_core'));
    }
    if ($nojs == 1) {
        $mform->addElement('static', 'currenticon', get_string('currenticon', 'corplms_core'), $iconhtml);
        if ($action=='add' || $action=='edit') {
            $path = $CFG->dirroot . '/corplms/core/pix/' . $type . 'icons';
            ...
            $mform->addElement('select', 'icon', get_string('icon', 'corplms_core'), $icons);
        ...
        $buttonhtml = '';
        if ($action=='add' || $action=='edit') {
            $buttonhtml = html_writer::empty_tag('input', array('type' => 'button', 'value' => get_string('chooseicon', 'corplms_program'), 'id' => 'show-icon-dialog'));
            $mform->addElement('hidden', 'icon');
            $mform->setType('icon', PARAM_TEXT);
        }
        $mform->addElement('static', 'currenticon', get_string('currenticon', 'corplms_core'), $iconhtml . $buttonhtml);
    }
    if ($fieldset) {
 ...
 */
function corplms_create_icon_picker(&$mform, $action, $type, $currenticon = '', $nojs = 0, $ind = '') {
    global $CFG;
    $return = array();
    if ($currenticon == '') {
        $currenticon = 'default';
    ...
    // Get all icons of this type from core.
    $replace = array('.png' => '', '_' => ' ', '-' => ' ');
    $iconhtml = corplms_icon_picker_preview($type, $currenticon, $ind);

    if ($nojs == 1) {
        $return['currenticon'.$ind] = $mform->createElement('static', 'currenticon',
                get_string('currenticon', 'corplms_core'), $iconhtml);
        if ($action == 'add' || $action == 'edit') {
            $path = $CFG->dirroot . '/corplms/core/pix/' . $type . 'icons';
            foreach (scandir($path) as $icon) {
                if ($icon == '.' || $icon == '..') {
            ...
            $return['icon'.$ind] = $mform->createElement('select', 'icon',
                    get_string('icon', 'corplms_core'), $icons);
            $mform->setDefault('icon', $currenticon);
        }
    } else {
        $linkhtml = '';
        if ($action == 'add' || $action == 'edit') {
            $linkhtml = html_writer::tag('a', get_string('chooseicon', 'corplms_program'),
        ...
        $return['currenticon' . $ind] = $mform->createElement('static', 'currenticon', '',
                get_string('icon', 'corplms_program') . $iconhtml . $linkhtml);
    }
    return $return;
}
 ...
 * @return string HTML
 */
function corplms_icon_picker_preview($type, $currenticon, $ind = '', $alt = '') {
    list($src, $alt) = corplms_icon_url_and_alt($type, $currenticon, $alt);
 ...
 * @return string HTML
 */
function corplms_icon_url_and_alt($type, $icon, $alt = '') {
...
/**
* print out the Corplms My Team nav section
*/
function corplms_print_my_team_nav() {
    global $CFG, $USER, $PAGE;

    $managerroleid = $CFG->managerroleid;

    // return users with this user as manager
    $staff = corplms_get_staff();
...
*/
function corplms_print_report_manager() {
    ...
    $context = context_system::instance();
    $canedit = has_capability('corplms/reportbuilder:managereports',$context);

    if (count($reportbuilder_permittedreports) > 0) {
        $renderer = $PAGE->get_renderer('corplms_core');
        $returnstr = $renderer->print_report_manager($reportbuilder_permittedreports, $canedit);
    } else {
        $returnstr = get_string('nouserreports', 'corplms_reportbuilder');
    }
    return $returnstr;
}
...
*/
function corplms_print_scheduled_reports($showoptions=true, $showaddform=true, $sqlclause=array()) {
    ...
    if (count($scheduledreports) > 0) {
        ...
        echo get_string('noscheduledreports', 'corplms_reportbuilder') . html_writer::empty_tag('br') . html_writer::empty_tag('br');
    }

    if ($showaddform) {
        $mform = new scheduled_reports_add_form($CFG->wwwroot . '/corplms/reportbuilder/scheduled.php', array());
        $mform->display();
    }
}

function corplms_print_my_courses() {
...
**/
function corplms_is_manager($userid, $managerid=null, $postype=null) {
    global $DB, $USER;
...
**/
function corplms_get_staff($userid=null, $postype=null) {
    global $CFG, $DB, $USER;
 ...
 */
function corplms_get_manager($userid, $postype=null, $skiptemp=false, $skipreal=false) {
    global $CFG, $DB;
 ...
 * @return mixed False if no manager. Manager user object from mdl_user if the user has a manager.
 */
function corplms_get_most_primary_manager($userid = false) {
    global $DB, $USER;
    ...
    if (!empty($enabletempmanagers)) {
        if ($tempmanager = corplms_get_manager($userid, null, false, true)) {
 ...
 */
function corplms_update_temporary_manager($userid, $managerid, $expiry) {
    global $CFG, $DB, $USER;
    ...
    $realmanager = corplms_get_manager($userid, null, true);
    ...
    $msg = new stdClass();
    $msg->userfrom = $USER;
    $msg->msgstatus = CORPLMS_MSG_STATUS_OK;
    $msg->contexturl = $CFG->wwwroot.'/user/positions.php?user='.$userid.'&courseid='.SITEID;
    $msg->contexturlname = get_string('xpositions', 'corplms_core', fullname($user));
    $msgparams = (object)array('staffmember' => fullname($user), 'tempmanager' => fullname($newtempmanager),
        'expirytime' => userdate($expiry, get_string('datepickerlongyearphpuserdate', 'corplms_core')), 'url' => $msg->contexturl);
    ...
    if (!empty($oldtempmanager) && $newtempmanager->id == $oldtempmanager->tempmanagerid) {
        if ($oldtempmanager->expirytime == $expiry) {
            ...
            // Notify staff member.
            $msg->userto = $user;
            $msg->subject = get_string('tempmanagerexpiryupdatemsgstaffsubject', 'corplms_core', $msgparams);
            $msg->fullmessage = get_string('tempmanagerexpiryupdatemsgstaff', 'corplms_core', $msgparams);
            $msg->fullmessagehtml = get_string('tempmanagerexpiryupdatemsgstaff', 'corplms_core', $msgparams);
            tm_alert_send($msg);

            // Notify real manager.
            if (!empty($realmanager)) {
                $msg->userto = $realmanager;
                $msg->subject = get_string('tempmanagerexpiryupdatemsgmgrsubject', 'corplms_core', $msgparams);
                $msg->fullmessage = get_string('tempmanagerexpiryupdatemsgmgr', 'corplms_core', $msgparams);
                $msg->fullmessagehtml = get_string('tempmanagerexpiryupdatemsgmgr', 'corplms_core', $msgparams);
            ...
            $msg->subject = get_string('tempmanagerexpiryupdatemsgtmpmgrsubject', 'corplms_core', $msgparams);
            $msg->fullmessage = get_string('tempmanagerexpiryupdatemsgtmpmgr', 'corplms_core', $msgparams);
            $msg->fullmessagehtml = get_string('tempmanagerexpiryupdatemsgtmpmgr', 'corplms_core', $msgparams);
            $msg->roleid = $CFG->managerroleid;
            tm_alert_send($msg);
    ...
    corplms_unassign_temporary_manager($userid);
    ...
    // Notify staff member.
    $msg->userto = $user;
    $msg->subject = get_string('tempmanagerassignmsgstaffsubject', 'corplms_core', $msgparams);
    $msg->fullmessage = get_string('tempmanagerassignmsgstaff', 'corplms_core', $msgparams);
    $msg->fullmessagehtml = get_string('tempmanagerassignmsgstaff', 'corplms_core', $msgparams);
    tm_alert_send($msg);

    // Notify real manager.
    if (!empty($realmanager)) {
        $msg->userto = $realmanager;
        $msg->subject = get_string('tempmanagerassignmsgmgrsubject', 'corplms_core', $msgparams);
        $msg->fullmessage = get_string('tempmanagerassignmsgmgr', 'corplms_core', $msgparams);
        $msg->fullmessagehtml = get_string('tempmanagerassignmsgmgr', 'corplms_core', $msgparams);
        $msg->roleid = $CFG->managerroleid;
        tm_alert_send($msg);
    }
    ...
    // Notify temp manager.
    $msg->userto = $newtempmanager;
    $msg->subject = get_string('tempmanagerassignmsgtmpmgrsubject', 'corplms_core', $msgparams);
    $msg->fullmessage = get_string('tempmanagerassignmsgtmpmgr', 'corplms_core', $msgparams);
    $msg->fullmessagehtml = get_string('tempmanagerassignmsgtmpmgr', 'corplms_core', $msgparams);
    $msg->roleid = $CFG->managerroleid;
    tm_alert_send($msg);
}
 ...
 * @throws Exception
 */
function corplms_unassign_temporary_manager($userid) {
    global $DB, $CFG;
 ...
 */
function corplms_get_teamleader($userid, $postype=null) {
    $manager = corplms_get_manager($userid, $postype);
    if ($manager) {
        // We use the default postype for the manager's manager.
        return corplms_get_manager($manager->id);
    } else {
        return false;
    }
}
 ...
 */
function corplms_get_appraiser($userid, $postype=null) {
    global $CFG, $DB;
    require_once($CFG->dirroot.'/corplms/hierarchy/prefix/position/lib.php');
    $postype = ($postype === null) ? POSITION_TYPE_PRIMARY : (int) $postype;
...
*/
function corplms_date_parse_from_format ($format, $date) {
 ...
 * @return  boolean
 */
function corplms_is_post_request() {
 ...
 * @return  void
 */
function corplms_errors_download() {
    global $DB;
    ...
    send_temp_file($content, 'corplms-error.log', true);
}
 ...
 * @return  string|void
 */
function print_corplms_search($value = '', $return = true, $type = 'all', $category = -1) {
    ...
    if ($category > 0) {
        ...
        if ($categoryname) {
            $strsearch = get_string('searchx', 'corplms_core', $categoryname);
        } else {
            $strsearch = get_string('search');
        }
    } else {
        if ($type == 'course') {
            $strsearch = get_string('searchallcourses', 'corplms_coursecatalog');
        } elseif ($type == 'program') {
            $strsearch = get_string('searchallprograms', 'corplms_coursecatalog');
        } elseif ($type == 'certification') {
            $strsearch = get_string('searchallcertifications', 'corplms_coursecatalog');
        } elseif ($type == 'category') {
            $strsearch = get_string('searchallcategories', 'corplms_coursecatalog');
        } else {
            $strsearch = get_string('search');
            $type = '';
        }
    ...
    $formid = 'searchcorplms';
    $inputid = 'navsearchbox';
    $value = s($value, true);
    $strsearch = s($strsearch);

    $renderer = $PAGE->get_renderer('corplms_core');
    $output = $renderer->print_corplms_search($action, $hiddenfields, $strsearch, $value, $formid, $inputid);
 ...
 */
function corplms_print_edit_button($settingname, $params = array()) {
...
*/
function corplms_get_nav_select_classes($navstructure, $primary_selected, $secondary_selected) {
...
/**
 * Builds Corplms menu, returns an array of objects that
 ...
 */
function corplms_build_menu() {
    $rs = \corplms_core\corplms\menu\menu::get_nodes();
    $tree = array();
    foreach ($rs as $id => $item) {
        // If parent exists and parent visibility is always hide then hide all children.
        if (!is_null($item->parentvisibility) && $item->parentvisibility == \corplms_core\corplms\menu\menu::HIDE_ALWAYS) {
            continue;
        // Otherwise, if parent is show when required, check if it is currently visible.
        } else if ($item->parentvisibility == \corplms_core\corplms\menu\menu::SHOW_WHEN_REQUIRED) {
            $classname = $item->parent;
            if (!is_null($classname) && class_exists($classname)) {
                $parentnode = new $classname(array());
                if ($parentnode->get_visibility() != \corplms_core\corplms\menu\menu::SHOW_ALWAYS) {
                    continue;
                }
            }
        }
        $node = \corplms_core\corplms\menu\menu::node_instance($item);
        ...
        if ($node->get_visibility() != \corplms_core\corplms\menu\menu::SHOW_ALWAYS) {
            continue;
        }
...
function corplms_upgrade_menu() {
    $CORPLMSMENU = new \corplms_core\corplms\menu\build();
    $plugintypes = core_component::get_plugin_types();
    foreach ($plugintypes as $plugin => $path) {
        $pluginname = core_component::get_plugin_list_with_file($plugin, 'db/corplmsmenu.php');
        if (!empty($pluginname)) {
...
/**
 * Install the Corplms MyMoodle blocks
 *
 * @return bool
 */
function corplms_reset_mymoodle_blocks() {
    global $DB, $SITE;
    ...
    $blockinstance->blockname = 'corplms_quicklinks';
    ...
    if ($block = block_instance('corplms_quicklinks', $blockinstance)) {
...
/**
 * Color functions used by corplms themes for auto-generating colors
 */
 ...
 */
function corplms_brightness($color, $percent) {
    ...
    if (!preg_match($pattern, $color, $matches)) {
        debugging("Bad hex color '{$color}' passed to corplms_brightness().", DEBUG_DEVELOPER);
        return $color;
    }
 ...
 *
 * This differs from {@link corplms_brightness} in that the scaling is
 ...
 * @return string 6 digit hex color code for resulting color
 */
function corplms_brightness_linear($color, $amount) {
    // convert 3 digit color codes into 6 digit form
    $pattern = '/^#([[:xdigit:]])([[:xdigit:]])([[:xdigit:]])$/';
    $color = preg_replace($pattern, '#$1$1$2$2$3$3', $color );
    ...
    $pattern = '/^#([[:xdigit:]]{2})([[:xdigit:]]{2})([[:xdigit:]]{2})$/';
    if (!preg_match($pattern, $color, $matches)) {
        debugging("Bad hex color '{$color}' passed to corplms_brightness_linear().", DEBUG_DEVELOPER);
        return $color;
    }
    $red = hexdec($matches[1]);
    $green = hexdec($matches[2]);
 ...
 * @return string 6 digit hex color code for resulting color
 */
function corplms_readable_text($color) {
    // convert 3 digit color codes into 6 digit form
    $pattern = '/^#([[:xdigit:]])([[:xdigit:]])([[:xdigit:]])$/';
    $color = preg_replace($pattern, '#$1$1$2$2$3$3', $color );
    ...
    $pattern = '/^#([[:xdigit:]]{2})([[:xdigit:]]{2})([[:xdigit:]]{2})$/';
    if (!preg_match($pattern, $color, $matches)) {
        debugging("Bad hex color '{$color}' passed to corplms_readable_text().", DEBUG_DEVELOPER);
        return $color;
    }
    $red = hexdec($matches[1]);
    $green = hexdec($matches[2]);
 ...
 */
function corplms_readable_text_shadow($color) {
    if (corplms_readable_text($color) == '#FFFFFF') {
 ...
 */
function corplms_desaturate($color) {
    // convert 3 digit color codes into 6 digit form
    $pattern = '/^#([[:xdigit:]])([[:xdigit:]])([[:xdigit:]])$/';
    $color = preg_replace($pattern, '#$1$1$2$2$3$3', $color );
 ...
 */
function corplms_theme_generate_autocolors($css, $theme, $substitutions) {
    ...
    foreach ($substitutions as $setting => $defaultcolor) {
        ...
        foreach ($autosettings as $suffix => $modification) {
            ...
            $function_name = 'corplms_' . array_shift($modification);
        ...
        if ($setting == 'headerbgc') {
            ...
            $replace[] = (corplms_readable_text($value) == '#000000' ? '#444444' : '#b3b3b3');

            $find[] = "[[setting:text-on-headerbgc]]";
            $replace[] = (corplms_readable_text($value) == '#000000' ? '#444444' : '#cccccc');
        }
    }
...
/**
 * Encrypt any string using corplms public key
 *
 * @param string $plaintext
 * @param string $key Public key If not set corplms public key will be used
 * @return string Encrypted data
 */
function encrypt_data($plaintext, $key = '') {
    global $CFG;
    require_once($CFG->dirroot.'/corplms/core/lib/phpseclib/Crypt/RSA.php');
 ...
 */
function corplms_get_icon($instanceid, $icontype) {
    global $DB, $OUTPUT, $PAGE;

    $component = 'corplms_core';
    $urlicon = '';

    if ($icontype == CORPLMS_ICON_TYPE_COURSE) {
...
/**
 * Corplms specific initialisation
 ...
 */
function corplms_setup() {
    global $CFG;
    if (is_ajax_request($_SERVER)) {
        $CFG->moodlepageclassfile = $CFG->dirroot.'/corplms/core/pagelib.php';
        $CFG->moodlepageclass = 'corplms_page';
    }
}
 ...
 */
function corplms_idnumber_exists($table, $idnumber, $itemid = 0) {
    global $DB;
...
/**
 * List of strings which can be used with 'corplms_feature_*() functions'.
 *
 ...
 */
function corplms_advanced_features_list() {
    return array(
        'goals',
        'appraisals',
        'feedback360',
...
/**
 * Check the state of a particular Corplms feature against the specified state.
 *
 * Used by the corplms_feature_*() functions to see if some Corplms functionality is visible/hidden/disabled.
 *
 * @param string $feature Name of the feature to check, must match options from {@link corplms_advanced_features_list()}.
 * @param integer $stateconstant State to check, must match one of CORPLMS_*FEATURE constants defined in this file.
 * @return bool True if the feature's config setting is in the specified state.
 */
function corplms_feature_check_state($feature, $stateconstant) {
    global $CFG;

    if (!in_array($feature, corplms_advanced_features_list())) {
        throw new coding_exception("'{$feature}' not supported by Corplms feature checking code.");
    }
 ...
 *
 * @param string $feature The name of the feature from the list in {@link corplms_feature_check_support()}.
 * @return bool True if the feature is set to be visible.
 */
function corplms_feature_visible($feature) {
    return corplms_feature_check_state($feature, CORPLMS_SHOWFEATURE);
}
 ...
 */
function corplms_feature_disabled($feature) {
    return corplms_feature_check_state($feature, CORPLMS_DISABLEFEATURE);
}
 ...
 */
function corplms_feature_hidden($feature) {
    return corplms_feature_check_state($feature, CORPLMS_HIDEFEATURE);
}

function corplms_seedlings_header() {
    ...
    if (!$seedlingsdismissed) {
        ...
        if ($USER->id) {
            $PAGE->requires->js('/corplms/core/module.js');
            $action = new component_action('click', 'M.corplms_core.dismiss_seedlings');
            $dismissicon = new pix_icon('t/delete', get_string('dismiss', 'corplms_core'), 'moodle', array('class' => 'seedlings-dismiss'));
            $html .= $OUTPUT->action_icon($CFG->wwwroot, $dismissicon, $action);
        }
        $html .= get_string('seedlingsmessage', 'corplms_core');
