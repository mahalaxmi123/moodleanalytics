...
require_once($CFG->dirroot . '/corplms/core/deprecatedlib.php');

/* Core event handler classes.
 *
 */
class corplms_core_event_handler {
    ...
    * @return  boolean
    */
    public static function user_enrolment(\corplms_core\event\user_enrolment $event) {
        global $CFG, $DB;
    ...
    * @return  boolean
    */
    public static function criteria_course_calc(\corplms_core\event\module_completion $event) {
        global $CFG, $DB;
        include_once($CFG->dirroot . '/completion/completion_completion.php');
 ...
 */
function corplms_site_version_tracking() {
    global $CFG, $PAGE, $CORPLMS;

    require_once($CFG->dirroot.'/corplms/core/js/lib/setup.php');
    local_js();

    //Params for JS
    $corplms_version = $CORPLMS->version;
    $major_version = substr($CORPLMS->version, 0, 3);
    ...
    $PAGE->requires->strings_for_js(array('unsupported_branch_text', 'supported_branch_text', 'supported_branch_old_release_text'), 'corplms_core', $major_version);
    $PAGE->requires->strings_for_js(array('old_release_text_singular', 'old_release_text_plural', 'old_release_security_text_singular', 'old_release_security_text_plural', 'corplmsreleaselink'), 'corplms_core');

    $args = array('args' => '{"corplms_version":"'.$corplms_version.'", "major_version":"'.$major_version.'", "protocol":"'.$protocol.'"}');

    $jsmodule = array(
        'name' => 'corplms_version_tracking',
        'fullpath' => '/corplms/core/js/version_tracking.js',
        'requires' => array('json'));
    $PAGE->requires->js_init_call('M.corplms_version_tracking.init', $args, false, $jsmodule);

}

function corplms_core_cron() {

    // Temporary manager tasks.
    corplms_update_temporary_managers();

    return true;
}

function corplms_update_temporary_managers() {
    global $CFG, $DB;
    ...
    if (empty($CFG->enabletempmanagers)) {
        ...
        if ($rs = $DB->get_recordset('temporary_manager', null, '', 'userid')) {
            ...
            foreach ($rs as $tmassignment) {
                corplms_unassign_temporary_manager($tmassignment->userid);
            }
        }
    ...
    if (!$CFG->tempmanagerrestrictselection) {
        ...
        if ($rs = $DB->get_recordset_sql($sql)) {
            ...
            foreach ($rs as $assignment) {
                corplms_unassign_temporary_manager($assignment->userid);
            }
        }
    }
    ...
    if (!empty($expiredmanagers)) {
        ...
        foreach ($expiredmanagers as $m) {
            corplms_unassign_temporary_manager($m->userid);
        }
...
/**
 * To download the file we upload in corplms_core filearea
 *
 * @param $course
 * @param $cm
 * @param $context
 ...
 */
function corplms_core_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, $options=array()) {
 ...
 */
function corplms_resize_images_filearea($contextid, $component, $filearea, $itemid, $width, $height, $replace=false) {
    global $CFG, $USER;
...
/**
 * Create recursively corplms menu table
 *
 * @param html_table $table to add data to.
 * @param corplms_core_menu $item to render
 ...
 */
function corplms_menu_table_load(html_table &$table, \corplms_core\corplms\menu\menu $item, $depth = 0, $up = false, $down = false) {
    global $OUTPUT;
    ...
    if ($item->id) {
        $node = \corplms_core\corplms\menu\menu::node_instance($item->get_property());
        if ($node === false) {
            // Bad node, don't display.
            return;
        }
        $dimmed = ($item->visibility ? '' : ' dimmed');
        $url = '/corplms/core/menu/index.php';
        ...
        $attributes['class'] = 'corplms_item_depth'.$depth.$dimmed;
        $itemtitle = html_writer::link(new moodle_url('/corplms/core/menu/edit.php',
                array('id' => $item->id)), $itemtitle, $attributes);

        $icons = array();
        // Edit category.
        $icons[] = $OUTPUT->action_icon(
                        new moodle_url('/corplms/core/menu/edit.php', array('id' => $item->id)),
                        new pix_icon('t/edit', $str->edit, 'moodle', array('class' => 'iconsmall')),
                        null, array('title' => $str->edit)
        );
        // Change visibility.
        if ($item->visibility != \corplms_core\corplms\menu\menu::HIDE_ALWAYS) {
            $icons[] = $OUTPUT->action_icon(
        ...
        // Delete item.
        if ($item->custom == \corplms_core\corplms\menu\menu::DB_ITEM) {
            $icons[] = $OUTPUT->action_icon(
                            new moodle_url('/corplms/core/menu/delete.php', array('id' => $item->id)),
        ...
        $table->data[] = new html_table_row(array(
                         ...
                         new html_table_cell(\corplms_core\corplms\menu\menu::get_visibility($item->visibility)),
    ...
    if ($items = $item->get_children()) {
        ...
        foreach ($items as $node) {
            ...
            corplms_menu_table_load($table, $node, $depth+1, $up, $down);
        }
    }
}
