...
/**
 * base assignment classes corplms_assign_core and corplms_assign_core_groups
...
class corplms_assign_core {
    ...
    public function __construct($module, $moduleinstance) {
        ...
        self::$module = $module;
        $this->basepath = $CFG->dirroot . "/corplms/{$module}/lib/assign/";
    }
    ...
    public function load_grouptype($grouptype) {
        $module = self::$module;
        $classname = "corplms_assign_{$module}_grouptype_{$grouptype}";
        $classfile = $this->basepath . "groups/{$grouptype}.class.php";

        // Check class file exists.
        if (!file_exists($classfile)) {
            print_error('error:assignmentprefixnotfound', 'corplms_core', '', $grouptype);
        }
    ...
    public static function get_assignable_grouptypes() {
        ...
        $basepath = $CFG->dirroot . "/corplms/{$module}/lib/assign/";
    ...
    public function get_group_instance_name($grouptype, $groupid) {
        ...
        if (!in_array($grouptype, $groups)) {
            print_error('error:invalidgrouptype', 'corplms_core');
        }
    ...
    public function delete_assigned_group($grouptype, $deleteid) {
        if (!in_array($grouptype, $this->get_assignable_grouptypes())) {
            print_error('error:assigncannotdeletegrouptypex', 'corplms_core', $grouptype);
        }
        if ($this->is_locked()) {
            print_error('error:assignmentmoduleinstancelocked', 'corplms_core');
        }
        $grouptypeobj = $this->load_grouptype($grouptype);
    ...
    public function get_group_assignedvia_details($userids) {
        ...
        foreach ($assignedgroups as $assignedgroup) {
            ...
            if (!empty($assignedgroup->includechildren)) {
                $string .= get_string('assignincludechildren', 'corplms_core');
            }
            $grouptypeobj = $this->load_grouptype($assignedgroup->grouptype);
...
abstract class corplms_assign_core_grouptype {
    ...
    public function __construct($assignobject) {
        // Store the whole assignment object from corplms_assign or child class of corplms_assign.
    ...
    public static function load_grouptype($assignobject) {
        $classname = "corplms_assign_{$assignobject->module}_group_{$assignobject->grouptype}";
        // Check group class file exists.
        $classfile = $assignobject->basepath . "groups/{$assignobject->grouptype}.class.php";
        if (!file_exists($classfile)) {
            print_error('error:assignmentprefixnotfound', 'corplms_core', '', $assignobject->grouptype);
        }
        // Load class file.
        require_once($classfile);
        // Check class exists.
...
class corplms_assign_ui_picker_cohort extends corplms_dialog_content {
    ...
    public function set_parameters($newparams = array()) {
        if (!is_array($newparams)) {
            print_error('error:assignmentbadparameters', 'corplms_core', null, null);
            die();
        }
 ...
 */
class corplms_assign_ui_picker_hierarchy extends corplms_dialog_content_hierarchy_multi {
    public $params = array(
        'equal' => 0,
        'includechildren' => 0,
        'listofvalues' => 1,
 ...
 * @param   $module string The Corplms module
 ...
 * @return  void
 */
function corplms_setup_assigndialogs($module, $itemid, $datatable = false, $notice = "") {
    global $CFG, $PAGE;
    ...
    $jselements = array(
        ...
        CORPLMS_JS_UI);
    if ($datatable) {
        $jselements[] = CORPLMS_JS_DATATABLES;
    }
    local_js(
        $jselements
    );
    $PAGE->requires->strings_for_js(array('assigngroup'), 'corplms_' . $module);
    $jsmodule = array(
        'name' => 'corplms_assigngroups',
        'fullpath' => '/corplms/core/lib/assign/assigngroup_dialog.js',
        'requires' => array('json'));
    ...
    if ($datatable) {
        $PAGE->requires->js_init_code('
                $(document).ready(function() {
                    ...
                    "sAjaxSource": "'.$CFG->wwwroot.'/corplms/'.$module.'/lib/assign/ajax.php",
                    "fnServerParams": function ( aoData ) {
                            aoData.push( { "name": "module", "value": "'.$module.'" } );
                            aoData.push( { "name": "itemid", "value": "'.$itemid.'" } );
                    },
                    "oLanguage" : {
                        "sEmptyTable":     "'.addslashes_js(get_string('datatable:sEmptyTable', 'corplms_core')).'",
                        "sInfo":           "'.addslashes_js(get_string('datatable:sInfo', 'corplms_core')).'",
                        "sInfoEmpty":      "'.addslashes_js(get_string('datatable:sInfoEmpty', 'corplms_core')).'",
                        "sInfoFiltered":   "'.addslashes_js(get_string('datatable:sInfoFiltered', 'corplms_core')).'",
                        "sInfoPostFix":    "'.addslashes_js(get_string('datatable:sInfoPostFix', 'corplms_core')).'",
                        "sInfoThousands":  "'.addslashes_js(get_string('datatable:sInfoThousands', 'corplms_core')).'",
                        "sLengthMenu":     "'.addslashes_js(get_string('datatable:sLengthMenu', 'corplms_core')).'",
                        "sLoadingRecords": "'.addslashes_js(get_string('datatable:sLoadingRecords', 'corplms_core')).'",
                        "sProcessing":     "'.addslashes_js(get_string('datatable:sProcessing', 'corplms_core')).'",
                        "sSearch":         "'.addslashes_js(get_string('datatable:sSearch', 'corplms_core')).'",
                        "sZeroRecords":    "'.addslashes_js(get_string('datatable:sZeroRecords', 'corplms_core')).'",
                        "oPaginate": {
                            "sFirst":    "'.addslashes_js(get_string('datatable:oPaginate:sFirst', 'corplms_core')).'",
                            "sLast":     "'.addslashes_js(get_string('datatable:oPaginate:sLast', 'corplms_core')).'",
                            "sNext":     "'.addslashes_js(get_string('datatable:oPaginate:sNext', 'corplms_core')).'",
                            "sPrevious": "'.addslashes_js(get_string('datatable:oPaginate:sPrevious', 'corplms_core')).'"
                        }
                    }
                } );
