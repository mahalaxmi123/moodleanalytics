...
/**
 * This file defines the Corplms dialog for adding groups of users
 */

M.corplms_assigngroupdialog = M.corplms_assigngroupdialog || {
    ...
    init: function(Y, args) {
        ...
        if (typeof $ == 'undefined') {
            throw new Error('M.corplms_assigngroupdialog.init()-> jQuery dependency required for this module.');
        }

        this.corplms_assigngroupdialog_init_dialogs();
    },

    corplms_assigngroupdialog_init_dialogs: function() {
        var url = M.cfg.wwwroot + '/corplms/' + M.corplms_assigngroupdialog.config.module + '/lib/assign/assigngroup.php';
        var saveurl = url;
        // Dialog & handler for hierarchy picker

        var thandler = new corplmsDialog_handler_assigngrouptreeview();
        var tbuttons = {};
        ...
        var tdialog = new corplmsDialog(
            ...
            {
                buttons: tbuttons,
                title: '<h2>' + M.util.get_string('assigngroup', 'corplms_' + M.corplms_assigngroupdialog.config.module) + '</h2>'
            },
            url,
            thandler
        );
        tdialog.assigngroup_base_url = url;
        corplmsDialogs['assigngrouptreeview'] = tdialog;
        ...
        $(document).on('change', 'select.group_selector', function(event) {
            ...
            var module = M.corplms_assigngroupdialog.config.module;
            var sesskey = M.corplms_assigngroupdialog.config.sesskey;

            var dialog = corplmsDialogs['assigngrouptreeview'];
...
var assigngroup_handler_responsefunc = function(response) {
    ...
    if (response.substr(0,4) == 'DONE') {
        ...
        if (notice !== null ) {
            notice.innerHTML = M.corplms_assigngroupdialog.config.notice;
        }
...
corplmsDialog_handler_assigngrouptreeview = function() {};
corplmsDialog_handler_assigngrouptreeview.prototype = new corplmsDialog_handler_treeview_multiselect();
 ...
 * @return void
 */
corplmsDialog_handler_assigngrouptreeview.prototype._save = function() {
    // Serialize data
...
corplmsDialog_handler_assigngrouptreeview.prototype._update = assigngroup_handler_responsefunc;
