...
class scheduler {
    ...
    public function next($timestamp = null, $is_cron = true) {
        ...
        $usertz = corplms_get_clean_timezone($this->subject->{$this->map['timezone']});
    ...
    public function get_formatted($user = null) {
        ...
        switch($this->subject->{$this->map['frequency']}) {
            case self::DAILY:
                $out .= get_string('daily', 'corplms_reportbuilder') . ' ' .  get_string('at', 'corplms_reportbuilder') . ' ';
                $out .= strftime('%I:%M%p' , mktime($schedule, 0, 0, $timemonth, $timeday, $timeyear));
                break;
            case self::WEEKLY:
                $out .= get_string('weekly', 'corplms_reportbuilder') . ' ' . get_string('on', 'corplms_reportbuilder') . ' ';
                $out .= $calendardays[$schedule]['fullname'];
                break;
            case self::MONTHLY:
                $out .= get_string('monthly', 'corplms_reportbuilder') . ' ' . get_string('onthe', 'corplms_reportbuilder') . ' ';
