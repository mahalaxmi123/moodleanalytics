...
// Constants defined to be used in corplms_search_for_value function.
define('CORPLMS_SEARCH_OP_EQUAL', 0);
define('CORPLMS_SEARCH_OP_NOT_EQUAL', 1);
define('CORPLMS_SEARCH_OP_GREATER_THAN', 2);
define('CORPLMS_SEARCH_OP_GREATER_THAN_OR_EQUAL', 3);
define('CORPLMS_SEARCH_OP_LESS_THAN', 4);
define('CORPLMS_SEARCH_OP_LESS_THAN_OR_EQUAL', 5);

// Type of icon.
define ('CORPLMS_ICON_TYPE_COURSE', 'course');
define ('CORPLMS_ICON_TYPE_PROGRAM', 'program');
 ...
 */
function corplms_pop_n(&$items, $number) {
    if (count($items) == 0) {
...
function sql_sequence($field, $items, $type=SQL_PARAMS_QM, $negate = false) {
    ...
    $out = array();
    while ($some_items = corplms_pop_n($items, 1000)) {
        list($sql, $params) = $DB->get_in_or_equal($items, $type, 'param', !$negate);
        $out[] =" $field " . $sql;
 ...
 *               form element
 */
function corplms_select_width_limiter() {
    return array(
        'class' => 'corplms-limited-width'
    );
}
 ...
 */
function corplms_group_records($rs, $field) {
    if (!$rs) {
        return false;
    }
    $out = array();
 ...
 */
function corplms_int2vancode($int = 0) {
 ...
 *
 * See {@link corplms_int2vancode()} for details
 *
 ...
 */
function corplms_vancode2int($char = '00') {
 ...
 */
function corplms_increment_vancode($char, $inc = 1) {
    return corplms_int2vancode(corplms_vancode2int($char) + (int) $inc);
}
 ...
 */
function corplms_get_lineage($items, $itemid, $pathsofar = array()) {
    ...
    if (!isset($items[$itemid]) || empty($items[$itemid])) {
        ...
        // keep going
        return corplms_get_lineage($items, $items[$itemid], $pathsofar);
    }
}
/**
 ...
 */
function corplms_search_for_value($arraytosearch, $property, $operator, $searchvalue) {
    $objectsfound = array();
    switch ($operator) {
        case CORPLMS_SEARCH_OP_EQUAL:
            ...
            break;
        case CORPLMS_SEARCH_OP_NOT_EQUAL:
            $objectsfound = array_filter($arraytosearch,
                function ($objtofind) use($searchvalue, $property) {
                    return $objtofind->{$property} != $searchvalue;
                }
            ...
            break;
        case CORPLMS_SEARCH_OP_GREATER_THAN:
            $objectsfound = array_filter($arraytosearch,
                function ($objtofind) use($searchvalue, $property) {
                    return $objtofind->{$property} > $searchvalue;
                }
            ...
            break;
        case CORPLMS_SEARCH_OP_GREATER_THAN_OR_EQUAL:
            $objectsfound = array_filter($arraytosearch,
                function ($objtofind) use($searchvalue, $property) {
                    return $objtofind->{$property} >= $searchvalue;
                }
            ...
            break;
        case CORPLMS_SEARCH_OP_LESS_THAN:
            $objectsfound = array_filter($arraytosearch,
                function ($objtofind) use($searchvalue, $property) {
                    return $objtofind->{$property} < $searchvalue;
                }
            ...
            break;
        case CORPLMS_SEARCH_OP_LESS_THAN_OR_EQUAL:
            $objectsfound = array_filter($arraytosearch,
                function ($objtofind) use($searchvalue, $property) {
                    return $objtofind->{$property} <= $searchvalue;
                }
