...
/**
 * Class to store, render and manage corplms navigation category
 *
...
class menu implements \renderable, \IteratorAggregate {
    ...
    // Custom field values.
    // Corplms menu default item - delete is forbidden
    const DEFAULT_ITEM = 0;
    // Corplmsmenu default classname - add sting to database
    const DEFAULT_CLASSNAME = '\corplms_core\corplms\menu\item';
    ...
    /**
    * @var corplms_core_menu stores pseudo category with id=0.
    * Use corplms_core_menu::get(0) to retrieve.
    */
    ...
    */
    ...
    public function __get($name) {
        ...
        if (array_key_exists($name, self::$menufields)) {
            if ($this->$name === false) {
                ...
                $rs = $DB->get_record('corplms_navigation', array('id' => $this->id), join(',', array_keys($notretrievedfields)), MUST_EXIST);
        ...
        debugging('Invalid corplms_core_menu property accessed! ' . $name, DEBUG_DEVELOPER);
        return null;
    }
    ...
    /**
     * Constructor.
     * Constructor is protected, use corplms_core_menu::get($id) to retrieve category.
     *
    ...
    /**
     * Returns corplms_core_menu object for requested category.
     ...
     * $data->url = '/corplms/core/tm_sync4.php';
     * $data->parentid = 0|19;
    ...
    public static function sync($data) {
        ...
        $data->id = $DB->insert_record('corplms_navigation', $data);

        // Update path (only possible after we know the category id.
        $data->path = $parent->path . '/' . $data->id;
        $DB->set_field('corplms_navigation', 'path', $data->path, array('id' => $data->id));
        // Add to log.
        \corplms_core\event\menuitem_sync::create_from_item($data->id)->trigger();
    ...
    public function create($data) {
        ...
        \corplms_core\event\menuitem_created::create_from_item($data->id)->trigger();
    ...
    public function update($data) {
        ...
        if (!$this->id) {
            ...
            throw new \moodle_exception('error:findingmenuitem', 'corplms_core');
        }

        if ((int)$data->parentid > 0) {
            // Check if the children exists.
            if ($this->get_children()) {
                throw new \moodle_exception('error:itemhaschildren', 'corplms_core');
            }
        }
        ...
        if ($this->custom == self::DEFAULT_ITEM) {
            // If Corplms default menu title changed.
        ...
        $DB->update_record('corplms_navigation', $data);

        \corplms_core\event\menuitem_updated::create_from_item($data->id)->trigger();
    ...
    public static function validation($data) {
        ...
        if (isset($data->title) && empty($data->title)) {
            $errors['title'] = get_string('error:menuitemtitlerequired', 'corplms_core');
        }
        if (\core_text::strlen($data->title) > 1024) {
            $errors['title'] = get_string('error:menuitemtitletoolong', 'corplms_core');
        }

        if ($data->custom == self::DB_ITEM) {
            if (isset($data->url) && empty($data->url)) {
                $errors['url'] = get_string('error:menuitemurlrequired', 'corplms_core');
            }
            if (\core_text::strlen($data->url) > 255) {
                $errors['url'] = get_string('error:menuitemurltoolong', 'corplms_core');
            }
            if (!empty($data->url) && $data->url[0] != '/' && !filter_var($data->url, FILTER_VALIDATE_URL)) {
                $errors['url'] = get_string('error:menuitemurlinvalid', 'corplms_core');
            }
        }

        if (isset($data->classname)) {
            if (\core_text::strlen($data->classname) > 255) {
                $errors['classname'] = get_string('error:menuitemclassnametoolong', 'corplms_core');
            }
        }

        if (isset($data->targetattr)) {
            if (\core_text::strlen($data->targetattr) > 100) {
                $errors['targetattr'] = get_string('error:menuitemtargetattrtoolong', 'corplms_core');
            }
        }
    ...
    /**
     * Retrieves number of records from corplms_navigation table
     *
     * @global \moodle_database $DB
    ...
    public static function get_records($whereclause, $params = array()) {
        ...
        $sql = "SELECT tn.". join(',tn.', $fields). " FROM {corplms_navigation} tn WHERE ". $whereclause." ORDER BY tn.sortorder";
        return $DB->get_records_sql($sql, $params);
    }

    /**
     * Retrieves number of records from corplms_navigation table where visibility true
     *
     * @global \moodle_database $DB
     * @return array of stdClass objects
     */
    public static function get_nodes() {
        ...
        $sql = "SELECT
                    tn.". join(',tn.', $fields). ",
                    (SELECT classname FROM {corplms_navigation} WHERE tn.parentid = id) AS parent,
                    (SELECT visibility FROM {corplms_navigation} WHERE tn.parentid = id) AS parentvisibility
                FROM
                    {corplms_navigation} tn
                WHERE
     ...
     *
     * @global \moodle_database $DB
     * @return corplms_core_menu[] Array of corplms_core_menu objects indexed by category id
     */
    ...
    public function delete() {
        ...
        // If category is from CorplmsMenu throw exception, can't delete it.
        if (!$this->custom) {
            throw new \moodle_exception('error:menuitemcannotberemoved', 'corplms_core', null, $this->title);
        }
        ...
        $DB->delete_records('corplms_navigation', array('id' => $this->id));
        // Add to log.
        \corplms_core\event\menuitem_deleted::create_from_item($this->id)->trigger();
    ...
    public static function get_visibility_list() {
        return array(
            self::HIDE_ALWAYS => get_string('menuitem:hide', 'corplms_core'),
            self::SHOW_ALWAYS => get_string('menuitem:show', 'corplms_core'),
            self::SHOW_WHEN_REQUIRED => get_string('menuitem:showwhenrequired', 'corplms_core'),
        );
    }
    ...
    /**
     * Set child node as a parent during corplms_upgrade_menu().
     *
     * @global \moodle_database $DB
     */
    public function set_parent() {
        ...
        $DB->set_field('corplms_navigation', 'parentid', '0', array('id' => (int)$this->id));
        // Add to log.
        \corplms_core\event\menuitem_setparent::create_from_item($this->id)->trigger();
    }

    /*
     * Change $custom property to delete a corplms menu item during corplms_upgrade_menu().
     */
    ...
    public static function make_menu_list($excludeid = 0) {
        ...
        $sql = 'SELECT tn.id, tn.title FROM {corplms_navigation} tn WHERE tn.id <> ? AND tn.parentid = 0 ORDER BY tn.sortorder';
    ...
    public static function change_sortorder($id = 0, $up = false) {
        ...
        $swapcategory = $DB->get_records_select('corplms_navigation', $select, $params, $sort, '*', 0, 1);
        $swapcategory = reset($swapcategory);
        if (!$swapcategory) {
            return false;
        }

        $DB->set_field('corplms_navigation', 'sortorder', $swapcategory->sortorder, array('id' => $data->id));
        $DB->set_field('corplms_navigation', 'timemodified', time(), array('id' => $data->id));
        $DB->set_field('corplms_navigation', 'sortorder', $data->sortorder, array('id' => $swapcategory->id));
        $DB->set_field('corplms_navigation', 'timemodified', time(), array('id' => $swapcategory->id));

        \corplms_core\event\menuitem_sortorder::create_from_item($data->id, $action)->trigger();
        return true;
    }
    ...
    public static function change_visibility($id = 0, $hide = false) {
        ...
        if ($hide) {
            $DB->set_field('corplms_navigation', 'visibility', self::HIDE_ALWAYS, array('id' => $itemid));
        } else {
            // Change visibility to show
            $item = self::get($itemid);
            if ($item->custom == self::DB_ITEM) {
                $DB->set_field('corplms_navigation', 'visibility', self::SHOW_ALWAYS, array('id' => $itemid));
            } else {
                $DB->set_field('corplms_navigation', 'visibility', self::SHOW_WHEN_REQUIRED, array('id' => $itemid));
            }
        }

        \corplms_core\event\menuitem_visibility::create_from_item($itemid, $hide)->trigger();
     ...
     * the class \corplms_core\corplms\menu\item, for other items it is
     * the item classname.
     *
    ...
    public static function node_instance($item) {
        ...
        if ($item->custom) {
            return new \corplms_core\corplms\menu\item($item);
        }
    ...
    private static function calculate_item_sortorder($defaultsort, $parentid = 0) {
        global $DB;
        $existingsortorders = $DB->get_records_menu('corplms_navigation', null, 'sortorder DESC', 'id,sortorder');
        ...
        if ($parentid > 0) {
            ...
            $existingchildren = $DB->get_records_menu('corplms_navigation', array('parentid' => $parentid), 'sortorder DESC', 'id,sortorder');
            ...
            $toplevelitems = $DB->get_records_menu('corplms_navigation', array('parentid' => 0), 'sortorder DESC', 'id,sortorder');
            $maxvalue = false;
