 ...
 *
 * Corplms navigation edit form page
 *
...
class edit_form extends moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('select', 'parentid', get_string('menuitem:formitemparent', 'corplms_core'), $options);
        $mform->setType('parentid', PARAM_INT);

        $mform->addElement('text', 'title', get_string('menuitem:formitemtitle', 'corplms_core'), 'maxlength="1024"');
        $mform->addRule('title', get_string('required'), 'required', null);
        $mform->setType('title', PARAM_MULTILANG);
        $mform->addHelpButton('title', 'menuitem:formitemtitle', 'corplms_core');
        ...
        $mform->addElement('select', 'visibility', get_string('menuitem:formitemvisibility', 'corplms_core'), $options);
        $mform->setType('visibility', PARAM_INT);

        if ($item->custom == menu::DB_ITEM) {
            $mform->addElement('text', 'url', get_string('menuitem:formitemurl', 'corplms_core'), 'maxlength="255"');
            $mform->setType('url', PARAM_TEXT);
            $mform->addHelpButton('url', 'menuitem:formitemurl', 'corplms_core');
        }

        $options = array(menu::TARGET_ATTR_SELF, menu::TARGET_ATTR_BLANK);
        $mform->addElement('advcheckbox', 'targetattr', get_string('menuitem:formitemtargetattr', 'corplms_core'), '', null, $options);
        $mform->addHelpButton('targetattr', 'menuitem:formitemtargetattr', 'corplms_core');

        if ($item->id > 0) {
            $strsubmit = get_string('savechanges');
        } else {
            $strsubmit = get_string('menuitem:addnew', 'corplms_core');
        }
