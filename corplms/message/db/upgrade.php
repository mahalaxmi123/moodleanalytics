...
function xmldb_corplms_message_upgrade($oldversion) {
    ...
    if ($oldversion < 2012012701) {
        ...
        $types = array('alert' => 'corplms_msg_send_alrt_emails', 'task' => 'corplms_msg_send_task_emails');
        foreach ($types as $type => $oldsetting) {

            // find old settings
            $prefs = $DB->get_records('user_preferences', array('name' => $oldsetting));
            foreach ($prefs as $pref) {
                $newpref = "corplms_{$type}";
                if ($pref->value == 1) {
                    $newpref .= ',email';
                }

                // set new ones
                set_user_preference("message_provider_corplms_message_{$type}_loggedin", $newpref, $pref->userid);
                set_user_preference("message_provider_corplms_message_{$type}_loggedoff", $newpref, $pref->userid);
    ...
    if ($oldversion < 2012012702) {
        //fix old 1.9 corplms_msg tables
        ...
        if ($dbman->table_exists($table)) {
            ...
            foreach ($msgs as $msg) {
                unset($msg->id);
                //fix contexturl to change /local/ to /corplms/ for corplms modules only
                $msg->contexturl = str_replace('/local/plan','/corplms/plan', $msg->contexturl);
                $msg->contexturl = str_replace('/local/program','/corplms/program', $msg->contexturl);
            ...
            $count = count($msgs);
            if ($count > 0) {
                $pbar = new progress_bar('migratecorplmsmessages', 500, true);
                $i = 0;
                ...
                foreach ($msgs as $msg) {
                    ...
                    $pbar->update($i, $count, "Migrating corplms messages - message $i/$count.");
                }
                $pbar->update($count, $count, "Migrated corplms messages - done!");
            }
            ...
            echo $OUTPUT->notification('corplms/message: Recreated existing alerts and tasks ('.count($msgs).')', 'notifysuccess');
        }
        ...
        echo $OUTPUT->notification('Dropping obsolete corplms_msg tables', 'notifysuccess');
    ...
    if ($oldversion < 2012120400) {
        ...
        corplms_upgrade_mod_savepoint(true, 2012120400, 'corplms_message');
    }

    if ($oldversion < 2013020700) {
        $reportlist = $DB->get_fieldset_select('report_builder', 'id', 'source = ?', array('corplmsmessages'));
        if ($reportlist) {
        ...
        corplms_upgrade_mod_savepoint(true, 2013020700, 'corplms_message');
    }
