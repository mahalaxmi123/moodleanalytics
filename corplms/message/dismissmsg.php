...
if ($isfacetoface && !$DB->record_exists('facetoface_sessions', array('id' => $eventdata->data['session']->id))) {
        $subject .= ' (' . html_writer::tag('strong', get_string('f2fsessiondeleted', 'block_corplms_tasks')) . ')';
} else if ($isfacetoface && !$canbook) {
        $subject .= ' (' . html_writer::tag('strong', get_string('f2fsessionfull', 'block_corplms_tasks')) . ')';
}
global $CORPLMS_MESSAGE_TYPES;
$msgtype = get_string($CORPLMS_MESSAGE_TYPES[$metadata->msgtype], 'corplms_message');
$icon = $OUTPUT->pix_icon('msgicons/' . $metadata->icon, format_string($msgtype), 'corplms_core', array('class' => 'msgicon',  'alt' => format_string($msgtype)));

echo html_writer::start_tag('div', array('id' => 'corplms-msgs-dismiss'));
...
echo html_writer::tag('dt', get_string('type', 'block_corplms_alerts'));
echo html_writer::tag('dd', $icon);

echo html_writer::tag('dt', get_string('from', 'block_corplms_alerts'));
echo html_writer::tag('dd', $fromname);

echo html_writer::tag('dt', get_string('statement', 'block_corplms_alerts'));
...
if ($eventdata && $eventdata->action != 'facetoface') {
    echo html_writer::tag('label', get_string('reasonfordecision', 'corplms_message'), array('for' => 'dismiss-reasonfordecision'));
