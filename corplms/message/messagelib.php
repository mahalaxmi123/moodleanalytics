...
require_once($CFG->dirroot.'/corplms/message/lib.php');
...
define('CORPLMS_MSG_STATUS_UNDECIDED', 0);
define('CORPLMS_MSG_STATUS_OK', 1);
define('CORPLMS_MSG_STATUS_NOTOK', 2);

// message type constants
define('CORPLMS_MSG_TYPE_UNKNOWN', 0);
define('CORPLMS_MSG_TYPE_COURSE', 1);
define('CORPLMS_MSG_TYPE_FORUM', 2);
define('CORPLMS_MSG_TYPE_GRADING', 3);
define('CORPLMS_MSG_TYPE_CHAT', 4);
define('CORPLMS_MSG_TYPE_LESSON', 5);
define('CORPLMS_MSG_TYPE_QUIZ', 6);
define('CORPLMS_MSG_TYPE_FACE2FACE', 7);
define('CORPLMS_MSG_TYPE_SURVEY', 8);
define('CORPLMS_MSG_TYPE_SCORM', 9);
define('CORPLMS_MSG_TYPE_LINK', 10);
define('CORPLMS_MSG_TYPE_PROGRAM', 11);

// Message email constants (used to override default message processor behaviour):
// Send email via processor as normal (default)
define('CORPLMS_MSG_EMAIL_YES', 0);
// Override to prevent the message sending an email (even if the user has asked for it)
define('CORPLMS_MSG_EMAIL_NO', 1);
...
define('CORPLMS_MSG_EMAIL_MANUAL', 2);

// message type shortnames
global $CORPLMS_MESSAGE_TYPES;
$CORPLMS_MESSAGE_TYPES = array(
    CORPLMS_MSG_TYPE_UNKNOWN => 'unknown',
    CORPLMS_MSG_TYPE_COURSE => 'course',
    CORPLMS_MSG_TYPE_PROGRAM => 'program',
    CORPLMS_MSG_TYPE_FORUM => 'forum',
    CORPLMS_MSG_TYPE_GRADING => 'grading',
    CORPLMS_MSG_TYPE_CHAT => 'chat',
    CORPLMS_MSG_TYPE_LESSON => 'lesson',
    CORPLMS_MSG_TYPE_QUIZ => 'quiz',
    CORPLMS_MSG_TYPE_FACE2FACE => 'face2face',
    CORPLMS_MSG_TYPE_SURVEY => 'survey',
    CORPLMS_MSG_TYPE_SCORM => 'scorm',
    CORPLMS_MSG_TYPE_LINK => 'link',
);

// list of supported categories
global $CORPLMS_MESSAGE_CATEGORIES;
$CORPLMS_MESSAGE_CATEGORIES = array_merge(array_keys($CORPLMS_MESSAGE_TYPES),
        array('facetoface', 'learningplan', 'objective', 'resource'));

// message urgency constants
define('CORPLMS_MSG_URGENCY_LOW', -4);
define('CORPLMS_MSG_URGENCY_NORMAL', 0);
define('CORPLMS_MSG_URGENCY_URGENT', 4);
 ...
 *  msgstatus - int Message Status see CORPLMS_MSG_STATUS* constants
 *  msgtype - int Message Type see CORPLMS_MSG_TYPE* constants
 *
...
function tm_message_send($eventdata) {
    ...
    if (empty($processor)) { //this user never had a preference, save default
     ...
     // Create a message object.
        $eventdatabl = new stdClass();
        $eventdatabl->component         = $eventdata->component;
        $eventdatabl->name              = $eventdata->name;
        $eventdatabl->userfrom          = $eventdata->userfrom;;
        $eventdatabl->userto            = $eventdata->userto;;
        $eventdatabl->notification      = $eventdata->notification;
        $eventdatabl->subject           = $eventdata->subject;;
        $eventdatabl->fullmessage       = $eventdata->fullmessage;
        $eventdatabl->fullmessageformat = $eventdata->fullmessageformat;;
        $eventdatabl->fullmessagehtml   = $eventdata->fullmessagehtml;;
        $eventdatabl->smallmessage      = $eventdata->smallmessage;
    // call core message processing - this will trigger either output_corplms_task output_corplms_alert
    $eventdata->savedmessageid = message_send($eventdatabl);
...
function tm_insert_metadata($eventdata, $processorid) {
    ...
    $eventdata->msgtype = isset($eventdata->msgtype) ? $eventdata->msgtype : CORPLMS_MSG_TYPE_UNKNOWN;
    $eventdata->msgstatus = isset($eventdata->msgstatus) ? $eventdata->msgstatus : CORPLMS_MSG_STATUS_UNDECIDED;
    $eventdata->urgency = isset($eventdata->urgency) ? $eventdata->urgency : CORPLMS_MSG_URGENCY_NORMAL;
...
function tm_alert_send($eventdata) {
    ...
    (!isset($eventdata->msgtype)) && $eventdata->msgtype = CORPLMS_MSG_TYPE_UNKNOWN;
    (!isset($eventdata->msgstatus)) && $eventdata->msgstatus = CORPLMS_MSG_STATUS_UNDECIDED;
    (!isset($eventdata->urgency)) && $eventdata->urgency = CORPLMS_MSG_URGENCY_NORMAL;
    (!isset($eventdata->sendemail)) && $eventdata->sendemail = CORPLMS_MSG_EMAIL_YES;

    $eventdata->component         = 'corplms_message';
    ...
    $alertemailpref = get_user_preferences('message_provider_corplms_message_alert_loggedoff', null, $eventdata->userto->id);
    if ($result && strpos($alertemailpref, 'email') !== false && $eventdata->sendemail == CORPLMS_MSG_EMAIL_MANUAL) {
        ...
        if ($eventdata->contexturl) {
            $eventdata->fullmessagehtml .= html_writer::empty_tag('br') . html_writer::empty_tag('br') . $string_manager->get_string('viewdetailshere', 'corplms_message', $eventdata->contexturl, $eventdata->userto->lang);
        }

        // Add footer to email
        $eventdata->fullmessagehtml .= html_writer::empty_tag('br') . html_writer::empty_tag('br') . $string_manager->get_string('alertfooter2', 'corplms_message', $CFG->wwwroot."/message/edit.php", $eventdata->userto->lang);
...
function tm_task_send($eventdata) {
    ...
    if (!empty($eventdata->contexturl)) {
        $eventdata->fullmessagehtml .= html_writer::empty_tag('br').html_writer::empty_tag('br').get_string('viewdetailshere', 'corplms_message', $eventdata->contexturl);
    }
 ...
 *          $a->plan = s($this->name);
 *          $event->subject = get_string('plan-request-manager-short', 'corplms_plan', $a);
 *          $event->fullmessage = get_string('plan-request-manager-long', 'corplms_plan', $a);
 *          $event->acceptbutton = get_string('approve', 'corplms_plan').' '.get_string('plan', 'corplms_plan');
 *          $event->accepttext = get_string('approveplantext', 'corplms_plan');
 *
 *
 * @param object $task information about the message (userfrom, userto, ...)
 * @return boolean success
...
function tm_workflow_send($eventdata) {
    ...
    $eventdata->msgtype = CORPLMS_MSG_TYPE_LINK; // tells us how to treat the display
    (!isset($eventdata->msgstatus)) && $eventdata->msgstatus = CORPLMS_MSG_STATUS_UNDECIDED;
    (!isset($eventdata->urgency)) && $eventdata->urgency = CORPLMS_MSG_URGENCY_NORMAL;
...
function tm_message_task_accept($id, $reasonfordecision) {
    ...
    if ($message) {
        // get the event data
        $eventdata = corplms_message_eventdata($id, 'onaccept');
...
function tm_message_task_link($id) {
    ...
    if ($message) {
        // get the event data
        $eventdata = corplms_message_eventdata($id, 'oninfo');
...
function tm_message_task_reject($id, $reasonfordecision) {
    ...
    if ($message) {
        // Get the event data.
        $eventdata = corplms_message_eventdata($id, 'onreject');
        if (!empty($reasonfordecision)) {
            $eventdata->data['reasonfordecision'] = $reasonfordecision;
        }
        // grab the onaccept handler
...
function tm_message_workflow_object($action) {
    ...
    require_once($CFG->dirroot.'/corplms/message/workflow/lib.php');
    $file = $CFG->dirroot.'/corplms/message/workflow/plugins/'.$action.'/workflow_'.$action.'.php';
    if (!file_exists($file)) {
    ...
    // create the object
    $ctlclass = 'corplms_message_workflow_'.$action;
...
/**
 * Set the default config values for corplms ouput types on install
 *
 * @param string $output
 */
function tm_set_preference_defaults() {
    set_config('corplms_task_provider_corplms_message_task_permitted', 'permitted', 'message');
    set_config('corplms_alert_provider_corplms_message_alert_permitted', 'permitted', 'message');
    set_config('corplms_alert_provider_corplms_message_task_permitted', 'disallowed', 'message');
    set_config('message_provider_corplms_message_alert_loggedin', 'corplms_alert,email', 'message');
    set_config('message_provider_corplms_message_alert_loggedoff', 'corplms_alert,email', 'message');
    set_config('message_provider_corplms_message_task_loggedin', 'corplms_task,email', 'message');
    set_config('message_provider_corplms_message_task_loggedoff', 'corplms_task,email', 'message');
    set_config('popup_provider_corplms_message_task_permitted', 'disallowed', 'message');
    set_config('email_provider_corplms_message_task_permitted', 'permitted', 'message');
    set_config('jabber_provider_corplms_message_task_permitted', 'disallowed', 'message');
    set_config('corplms_task_provider_corplms_message_alert_permitted', 'disallowed', 'message');
    set_config('popup_provider_corplms_message_alert_permitted', 'permitted', 'message');
    set_config('email_provider_corplms_message_alert_permitted', 'permitted', 'message');
    set_config('jabber_provider_corplms_message_alert_permitted', 'permitted', 'message');
}
...
function tm_message_set_default_message_preferences($user) {
    ...
    foreach ($providers as $providerid => $provider) {
        ...
        if ($provider->name == 'instantmessage') {
            ...
            //corplms alert
            $offlineprocessortouse = $onlineprocessortouse = 'corplms_alert,email';
        }
        else if ($provider->name == 'task') {
            //corplms task
            $offlineprocessortouse = $onlineprocessortouse = 'corplms_task,email';
        }
        else {
