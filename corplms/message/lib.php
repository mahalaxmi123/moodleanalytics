...
define('CORPLMS_MSG_ALERT_LIMIT', 5);
define('CORPLMS_MSG_TASK_LIMIT', 5);
 ...
 */
function corplms_message_msgstatus_text($msgstatus) {
    global $CFG;

    if ($msgstatus == CORPLMS_MSG_STATUS_OK) {
        $status = 'go';
        $text = get_string('statusok', 'block_corplms_alerts');
    }
    else if ($msgstatus == CORPLMS_MSG_STATUS_NOTOK) {
        $status = 'stop';
        $text = get_string('statusnotok', 'block_corplms_alerts');
    }
    else {
        $status = 'grey_undecided';
        $text = get_string('statusundecided', 'block_corplms_alerts');
    }
 ...
 * @return array('text' => , 'icon' => '')
 */
function corplms_message_urgency_text($urgency) {
    global $CFG;

    if ($urgency == CORPLMS_MSG_URGENCY_URGENT) {
        $level = 'stop';
        $text = get_string('urgent', 'block_corplms_alerts');
    }
    else {
        $level = 'go';
        $text = get_string('normal', 'block_corplms_alerts');
    }
 ...
 * @return array('text' => , 'icon' => '')
 */
function corplms_message_cssclass($msgtype) {
    global $CORPLMS_MESSAGE_TYPES;

    return $CORPLMS_MESSAGE_TYPES[$msgtype];
}
 ...
 * @return array('text' => '', 'icon' => '')
 */
function corplms_message_msgtype_text($msgtype) {
    global $CFG, $CORPLMS_MESSAGE_TYPES;

    if (array_key_exists($msgtype, $CORPLMS_MESSAGE_TYPES)) {
        $text = get_string($CORPLMS_MESSAGE_TYPES[$msgtype], 'corplms_message');
    } else {
        $text = get_string('unknown', 'corplms_message');
    }
 ...
 */
function corplms_message_eventdata($id, $event, $metadata=null) {
    global $DB;
 ...
 */
function corplms_message_dismiss_action($id) {
    global $CFG, $FULLME, $PAGE, $OUTPUT;

    $clean_fullme = clean_param($FULLME, PARAM_LOCALURL);
    // Button Lang Strings
    $PAGE->requires->string_for_js('cancel', 'moodle');
    $PAGE->requires->string_for_js('dismiss', 'corplms_message');
    $PAGE->requires->string_for_js('dismiss', 'block_corplms_alerts');

    // Include JS for generic dismiss dialog
    $args = array('id'=>$id, 'selector'=>'dismissmsg', 'clean_fullme'=>$clean_fullme, 'sesskey'=>sesskey());
    $PAGE->requires->js_init_call('M.corplms_message.create_dialog', $args);

    // TODO SCANMSG: Check this still outputs required markup in no/script render
    // Construct HTML for dismiss button
    $str = get_string('dismiss', 'block_corplms_alerts');
    $out = html_writer::empty_tag('input', array('id' => 'dismissmsg'.$id.'-dialog', 'type' => 'image', 'name' => 'tm_dismiss_msg', 'class' => 'iconsmall action', 'src' => $OUTPUT->pix_url('t/delete_grey', 'corplms_core'), 'title' => $str, 'alt' => $str));
    ...
    $out .= html_writer::tag('noscript',
        html_writer::tag('form',
            ...
            html_writer::empty_tag('input', array('type' => 'image', 'class' => 'iconsmall', 'src' => $OUTPUT->pix_url('t/delete_grey', 'corplms_core'), 'title' => $str, 'alt' => $str)),
        array('action' => $CFG->wwwroot . '/corplms/message/dismiss.php?id=' . $id, 'method' => 'post'))
    );
    return $out;
}
 ...
 */
function corplms_message_alert_popup($id, $extrabuttons=array(), $messagetype) {
    ...
    $eventdata = corplms_message_eventdata($id, 'onaccept', $metadata);
 ...
 */
function corplms_message_checkbox_all_none() {
    global $PAGE;
    $PAGE->requires->strings_for_js(array('all','none'), 'moodle');
    $PAGE->requires->js_init_call('M.corplms_message.select_all_none_checkbox');
}
 ...
 */
function corplms_message_action_button($action) {
    global $CFG, $FULLME, $PAGE;

    $clean_fullme = clean_param($FULLME, PARAM_LOCALURL);
    // Button Lang Strings
    $str = get_string($action, 'corplms_message');
    $PAGE->requires->string_for_js('cancel', 'moodle');

    $args = array('action'=>$action, 'action_str' => $str, 'clean_fullme'=>$clean_fullme, 'sesskey'=>sesskey());
    $PAGE->requires->js_init_call('M.corplms_message.create_action_dialog', $args);
}
 ...
 */
function corplms_message_accept_reject_action($id) {
    global $CFG, $FULLME, $PAGE;
    ...
    $msgacceptdata = corplms_message_eventdata($id, 'onaccept');

    $returnto = ($msgmeta->msgtype == CORPLMS_MSG_TYPE_LINK && isset($msgacceptdata->data['redirect'])) ? $msgacceptdata->data['redirect'] : $clean_fullme;

    // Validate redirect
    $return_host = parse_url($returnto);
    $site_host = parse_url($CFG->wwwroot);
    if ($return_host['host'] != $site_host['host']) {
        print_error('error:redirecttoexternal', 'corplms_message');
    }

    $subject = format_string($msg->subject);
    $onaccept_str = format_string(isset($msgacceptdata->acceptbutton) ? $msgacceptdata->acceptbutton : get_string('onaccept', 'block_corplms_tasks'));
    $onreject_str = get_string('onreject', 'block_corplms_tasks');
    ...
    if (!empty($msgmeta->onaccept)) {
        ...
        $PAGE->requires->js_init_call('M.corplms_message.create_accept_reject_dialog', $args);
        ...
        $out .= html_writer::tag('noscript',
            ...
            array('action' => $CFG->wwwroot . '/corplms/message/accept.php?id=' . $id, 'method' => 'post'))
        );
    }
    if (!empty($msgmeta->onreject)) {
        ...
        $out .= html_writer::tag('noscript',
            ...
            array('action' => $CFG->wwwroot . '/corplms/message/reject.php?id=' . $id, 'method' => 'post'))
        );
    }
    return $out;
}
 ...
 */
function corplms_message_install() {
    global $CFG;
    ...
    set_config('corplms_message_cron', 60);
    return true;
}
 ...
 */
function corplms_message_cron() {
    global $CFG;
    require_once($CFG->dirroot.'/corplms/message/cron.php');
    message_cron();
}
