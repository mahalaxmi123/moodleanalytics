...
$PAGE->set_url('/corplms/message/tasks.php');
$PAGE->set_pagelayout('noblocks');

$strheading = get_string('tasks', 'corplms_message');
...
if (empty($report->description)) {
    $report->description = get_string('task_description', 'corplms_message');
}
...
if ($countfiltered > 0) {
    ...
    $dismisslink = html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'dismiss', 'id' => 'corplms-dismiss',
            'disabled' => 'true', 'value' => get_string('dismiss', 'corplms_message'))) .
            html_writer::tag('noscript', get_string('noscript', 'corplms_message'));
    $out .= get_string('withselected', 'corplms_message');
    $out .= $dismisslink;
    $out .= $output->box_end();
    echo $out;
    echo corplms_message_checkbox_all_none();
}
