...
define('CORPLMS_MSG_CRON_WAIT_NUM', 10);
// how often to print errors (1 for every time, 2 every other time, etc)
define('CORPLMS_MSG_CRON_ERROR_FREQ', 10);

// age for expiring undismissed alerts - days
define('CORPLMS_MSG_CRON_DISMISS_ALERTS', 30);

// age for expiring undismissed tasks - days
define('CORPLMS_MSG_CRON_DISMISS_TASKS', 30);

// age for purging messages - days
define('CORPLMS_MSG_CRON_PURGE', 1);
...
function message_cron() {
    ...
    // dismiss old alerts
    $time = time() - (CORPLMS_MSG_CRON_DISMISS_ALERTS * (24 * 60 * 60));
    $msgs = tm_messages_get_by_time('corplms_alert', $time);
    $deleted = array();
    foreach ($msgs as $msg) {
    ...
    // dismiss old taskes
    $time = time() - (CORPLMS_MSG_CRON_DISMISS_TASKS * (24 * 60 * 60));
    $msgs = tm_messages_get_by_time('corplms_task', $time);
    foreach ($msgs as $msg) {
        tm_message_dismiss($msg->id);
        //store message ids for bulk delete
        if (!in_array($msg->id, $deleted)) {
