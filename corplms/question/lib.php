...
abstract class question_base {
    ...
    public function add_settings_form_elements(MoodleQuickForm $form, $readonly = false, $info = null) {
        if ($this->requires_name()) {
            if ($readonly) {
                $form->addElement('static', 'name', get_string('question', 'corplms_question'), 'size="50"');
            } else {
                $form->addElement('text', 'name', get_string('question', 'corplms_question'), 'size="50"');
            }
            $form->addRule('name', null, 'required');
            $form->setType('name', PARAM_MULTILANG);
            $form->addHelpButton('name', 'question', 'corplms_question');
        }
    ...
    public function define_validate_all($data, $files) {
        ...
        if ($this->requires_name()) {
            if (empty($data->name)) {
                $err['name'] = get_string('customfieldrequired', 'corplms_customfield');
            }
        }
    ...
    public function add_field_form_elements(MoodleQuickForm $form) {
        ...
        if ($this->cananswer) {
            ...
            if (!empty($this->viewers)) {
                $viewersstring = '<small class="visibleto">' . get_string('visibleto', 'corplms_question') .
...
class MoodleQuickForm_staticcallback extends MoodleQuickForm_static {
    ...
    public function MoodleQuickForm_staticcallback($elementname = null, $elementlabel = null, $callback = null) {
        ...
        $this->_text = html_writer::tag('em', get_string('notanswered', 'corplms_question'));
    }
...
MoodleQuickForm::registerElementType('staticcallback', "$CFG->dirroot/corplms/question/lib.php", 'MoodleQuickForm_staticcallback');
