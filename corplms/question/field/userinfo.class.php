...
class question_userinfo extends question_base{
    ...
    public function __construct($storage, $subjectid = 0, $answerid = 0) {
        ...
        $this->info_fields = array(
            ...
            'profile_managername' => get_string('managername', 'corplms_question'),
            'profile_position' => get_string('position', 'corplms_cohort'),
            'profile_organisation' => get_string('organisation', 'corplms_cohort')
        );
    ...
    public static function get_info() {
        return array('group' => question_manager::GROUP_OTHER,
                     'type' => get_string('questiontypeuserinfo', 'corplms_question'));
    }
    ...
    protected function define_validate($data, $files) {
        ...
        if (!$atleastone) {
            $err['errorplaceholder'] = get_string('error:userinfoatleastone', 'corplms_question');
        }
    ...
    protected function add_field_specific_settings_elements(MoodleQuickForm $form, $readonly, $moduleinfo) {
        ...
        $form->addElement('header', 'infoheader', get_string('infotodisplay', 'corplms_question') . $requiredstr);
            ...
            if (isset($settings[$field]) && $settings[$field]) {
                switch ($field) {
                    ...
                    case 'profile_managername':
                        if ($manager = corplms_get_manager($user->id)) {
