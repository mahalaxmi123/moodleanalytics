...
M.corplms_question_multichoice = M.corplms_question_multichoice || {
  Y : null,
  ...
  init : function(Y, args) {
    ...
    if ( typeof $ === 'undefined') {
      throw new Error('M.corplms_question_multichoice.init()-> jQuery dependency required for this module to function.');
    }

    var savedchoices = M.corplms_question_multichoice.config.savedchoices;
    var numVisible = 0;
    var $container = $('#id_'+M.corplms_question_multichoice.config.jsid);
    var max = M.corplms_question_multichoice.config.max;
    ...
    $allOptions.each(function(){
        ...
        if ($($this.find('span')).length > 0) {
            ...
            var $makeDefaultLink = $('<a href="#">' + M.util.get_string('defaultmake', 'corplms_question') + '</a>');
            $makeDefault.append($makeDefaultLink);
            $makeDefaultLink.on('click', function(){
                if (M.corplms_question_multichoice.config.oneAnswer == 1) {
            ...
            var $unselect = $('<span class="unselectlink js-hide">' + M.util.get_string('defaultselected', 'corplms_question') + ' </span>');
            var $unselectLink = $('<a href="#">' + M.util.get_string('defaultunselect', 'corplms_question') + '</a>');
