...
class question_goals extends reviewrating {
    ...
    public static function get_info() {
        return array('group' => question_manager::GROUP_REVIEW,
                     'type' => get_string('questiontypegoals', 'corplms_question'));
    }

    public function __construct($storage, $subjectid = 0, $answerid = 0) {
        $this->buttonlabel = get_string('choosegoalsreview', 'corplms_question');
    ...
    protected function add_field_specific_settings_elements(MoodleQuickForm $form, $readonly, $moduleinfo) {
        ...
        $selection[] = $form->createElement('static', 'selectcompanyheader', null,
                html_writer::tag('b', get_string('goalscompany', 'corplms_question')));
        $selection[] = $form->createElement('radio', 'selectcompany', null,
                get_string('selectcompanyusercanchoose', 'corplms_question'), self::SELECT_COMPANY_USERCANCHOOSE);
        $selection[] = $form->createElement('radio', 'selectcompany', null,
                get_string('selectcompanyaddall', 'corplms_question'), self::SELECT_COMPANY_ADDALL);
        $selection[] = $form->createElement('radio', 'selectcompany', null,
                get_string('selectcompanydonotreview', 'corplms_question'), 0);

        // Personal.
        $selection[] = $form->createElement('static', 'selectpersonalheader', null,
                html_writer::tag('b', get_string('goalspersonal', 'corplms_question')));
        $selection[] = $form->createElement('radio', 'selectpersonal', null,
                get_string('selectpersonalusercanchoose', 'corplms_question'), self::SELECT_PERSONAL_USERCANCHOOSE);
        $selection[] = $form->createElement('radio', 'selectpersonal', null,
                get_string('selectpersonaladdall', 'corplms_question'), self::SELECT_PERSONAL_ADDALL);
        $selection[] = $form->createElement('radio', 'selectpersonal', null,
                get_string('selectpersonaldonotreview', 'corplms_question'), 0);

        $form->addGroup($selection, 'selection', get_string('goalselection', 'corplms_question'),
                array('<br/>'), true);
    ...
    public function define_validate($data, $files) {
        ...
        if (!$selection) {
            $err['selection'] = get_string('error:allowselectgoals', 'corplms_question');
        }
    ...
    protected function add_rating_selector(MoodleQuickForm $form, $item) {
        ...
        if (($scalevalueid == 0) || ($item->scope == goal::SCOPE_PERSONAL && $goalitem->scaleid == 0)) {
            // There is no scale.
            $form->addElement('static', '', get_string('goalstatus', 'corplms_question'),
                    html_writer::tag('em', get_string('goalhasnoscale', 'corplms_question')));
        } else {
            ...
            if (!$this->viewonly && $caneditstatus) {
                ...
                $form->addElement('select', $name, get_string('goalstatus', 'corplms_question'), $options,
                        array('class' => 'rating_selector rating_item_goal_' . $item->itemid . '_' . $item->scope));
                $form->setDefault($name, $scalevalueid);
            } else {
                $form->addElement('static', '', get_string('goalstatus', 'corplms_question'), format_string($scalevalue->name));
            }
        }
    }
            ...
            foreach ($items as $item) {
                if (!isset($item->fullname)) {
                    $item->fullname = html_writer::tag('em',
                            get_string('reviewgoalsassignmissing', 'corplms_question'));
                    $item->ismissing = true;
                }
            }
            return $items;
