...
/**
 * Output renderer for corplms_question module
 */
class corplms_question_renderer extends plugin_renderer_base {
    ...
    public function add_review_item($form, $itemgroup, review $review) {
        ...
        if ($review->can_select_items() && $review->can_delete_item($itemgroup)) {
            $deleteurl = new moodle_url("/corplms/$prefix/ajax/removeitem.php",
        ...
        if (isset($anyitem->planname)) {
            ...
            $title = get_string('reviewnamewithplan', 'corplms_question', $a);
        } else {
                ...
                if ($multifield) {
                    ...
                    foreach ($scalevalues as $scalevalue) {
                        ...
                        if ($currentuseritems[$scalevalue->id]->content != '') {
                            ...
                            $content .= html_writer::tag('em', get_string('notanswered', 'corplms_question'));
                        }
                    }
                } else {
                    $content = format_string($currentuseritems[0]->content);
                }
                $form->addElement(new MoodleQuickForm_static('', get_string('youranswer', 'corplms_question'), $content));
            } else {
                if ($multifield) {
                    $youranswerlabel = get_string('youranswer', 'corplms_question');
                    foreach ($scalevalues as $scalevalue) {
                    ...
                    $formelement = $form->addElement(
                            ...
                            get_string('youranswer', 'corplms_question'), $text_area_options));
                }
            }
            if (!empty($review->viewers)) {
                $viewersstring = '<small class="visibleto-review">' . get_string('visibleto', 'corplms_question') .
