...
/**
 * Local db upgrades for Corplms completion import.
 */
 ...
 */
function xmldb_corplms_completionimport_upgrade($oldversion) {
    global $DB;
    ...
    if ($oldversion < 2014030400) {
        ...
        $table = new xmldb_table('corplms_compl_import_course');
        $index = new xmldb_index('compimpcou_username_ix', XMLDB_INDEX_NOTUNIQUE, array('username'));
        if (!$dbman->index_exists($table, $index)) {
            $dbman->add_index($table, $index);
        }

        $table = new xmldb_table('corplms_compl_import_cert');
