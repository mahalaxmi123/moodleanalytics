...
function get_tablename($importname) {
    $tablenames = array(
        'course' => 'corplms_compl_import_course',
        'certification' => 'corplms_compl_import_cert'
    );
 ...
 * @param string $importtable eg: "{corplms_compl_import_course}" or "i"
...
function check_fields_exist($filename, $importname) {
    ...
    $errors = array();
    $pluginname = 'corplms_completionimport_' . $importname;
    ...
    if (!is_readable($filename)) {
        $errors[] = get_string('unreadablefile', 'corplms_completionimport', $filename);
    } else if (!$handle = fopen($filename, 'r')) {
        $errors[] = get_string('erroropeningfile', 'corplms_completionimport', $filename);
    } else {
        // Read the first line.
        $csvfields = fgetcsv($handle, 0, $csvseparator, $csvdelimiter);
        if (empty($csvfields)) {
            $errors[] = get_string('emptyfile', 'corplms_completionimport', $filename);
        } else {
            ...
            foreach ($csvfields as $key => $value) {
                ...
                if (!in_array($value, $columnnames)) {
                    ...
                    $errors[] = get_string('unknownfield', 'corplms_completionimport', $field);
                }
            }
            ...
            foreach ($columnnames as $columnname) {
                if (!in_array($columnname, $csvfields)) {
                    ...
                    $errors[] = get_string('missingfield', 'corplms_completionimport', $field);
                }
            }
        }
        fclose($handle);
...
function import_csv($tempfilename, $importname, $importtime) {
    ...
    require_once($CFG->libdir . '/csvlib.class.php');
    require_once($CFG->dirroot . '/corplms/completionimport/csv_iterator.php');
        ...
        if (!empty($csvdateformat)) {
            ...
            if ($timecompleteds->valid()) {
                foreach ($timecompleteds as $timecompleted) {
                    if (!corplms_date_parse_from_format($csvdateformat, $timecompleted->completiondate)) {
                        $sql = "UPDATE {{$tablename}}
                                SET importerrormsg = " . $DB->sql_concat('importerrormsg', ':errorstring') . "
                                WHERE id = :importid";
...
function create_evidence_item($item, $evidencetype, $csvdateformat, $tablename, $shortnamefield, $idnumberfield, $importname) {
    ...
    $timecompleted = null;
    $timestamp = corplms_date_parse_from_format($csvdateformat, $item->completiondate);
    if (!empty($timestamp)) {
    ...
    if (!empty($item->$shortnamefield)) {
        $itemname = get_string('evidence_shortname_' . $importname, 'corplms_completionimport', $item->$shortnamefield);
    } else if (!empty($item->$idnumberfield)) {
        $itemname = get_string('evidence_idnumber_' . $importname, 'corplms_completionimport', $item->$idnumberfield);
    }

    $description = '';
    foreach ($item as $field => $value) {
        if (!in_array($field, array('userid'))) {
            $description .= html_writer::tag('p', get_string('evidence_' . $field, 'corplms_completionimport', $value));
        }
    }
    ...
    if ($courses->valid()) {
        ...
        foreach ($courses as $course) {
            if (empty($enrolid) || ($enrolid != $course->enrolid) || (($enrolcount % BATCH_INSERT_MAX_ROW_COUNT) == 0)) {
                ...
                if (!empty($stats)) {
                    // Batch import block_corplms_stats.
                    $DB->insert_records_via_batch('block_corplms_stats', $stats);
                    $stats = array();
                }
            ...
            $timecompleted = null;
            $timestamp = corplms_date_parse_from_format($csvdateformat, $course->completiondate);
            if (!empty($timestamp)) {
                $timecompleted = $timestamp;
            }
            ...
            $completion->rpl = get_string('rpl', 'corplms_completionimport', $course->grade);
            ...
            // Create block_corplms_stats records
            $stat = new stdClass();
...
function import_certification($importname, $importtime) {
    ...
    if ($programs->valid()) {
        foreach ($programs as $program) {
            ...
            // Do recert times.
            $timecompleted = corplms_date_parse_from_format($csvdateformat, $program->completiondate);
            if (!$timecompleted) {
                $timecompleted = now();
            }
...
function display_report_link($importname, $importtime) {
    ...
    echo $OUTPUT->heading(get_string('importresults', 'corplms_completionimport'));
    if ($totals->totalrows) {
        echo html_writer::tag('p', get_string('importerrors', 'corplms_completionimport', $totals->totalerrors));
        echo html_writer::tag('p', get_string('importevidence', 'corplms_completionimport', $totals->totalevidence));
        echo html_writer::tag('p', get_string('import' . $importname, 'corplms_completionimport',
                $totals->totalrows - $totals->totalerrors - $totals->totalevidence));
        echo html_writer::tag('p', get_string('importtotal', 'corplms_completionimport', $totals->totalrows));

        $viewurl = new moodle_url('/corplms/completionimport/viewreport.php',
                array('importname' => $importname, 'timecreated' => $importtime, 'importuserid' => $USER->id));
        $viewlink = html_writer::link($viewurl, format_string(get_string('report_' . $importname, 'corplms_completionimport')));
        echo html_writer::tag('p', $viewlink);
    } else {
        echo html_writer::tag('p', get_string('importnone', 'corplms_completionimport'));
    }
...
function get_temppath() {
    ...
    $temppath = $CFG->dataroot . DIRECTORY_SEPARATOR . 'temp' . DIRECTORY_SEPARATOR . 'corplms_completionimport';
    if (!file_exists($temppath)) {
        if (!mkdir($temppath, $CFG->directorypermissions, true)) {
            echo $OUTPUT->notification(get_string('cannotcreatetemppath', 'corplms_completionimport', $temppath), 'notifyproblem');
            return false;
        }
    }
...
function move_sourcefile($filename, $tempfilename) {
    ...
    $handle = false;
    if (!is_readable($filename)) {
        echo $OUTPUT->notification(get_string('unreadablefile', 'corplms_completionimport', $filename), 'notifyproblem');
        return false;
    } else if (!$handle = fopen($filename, 'r')) {
        echo $OUTPUT->notification(get_string('erroropeningfile', 'corplms_completionimport', $filename), 'notifyproblem');
        return false;
    } else if (!flock($handle, LOCK_EX | LOCK_NB)) {
        echo $OUTPUT->notification(get_string('fileisinuse', 'corplms_completionimport', $filename), 'notifyproblem');
        fclose($handle);
        return false;
    }
    ...
    if (!rename($filename, $tempfilename)) {
        ...
        echo $OUTPUT->notification(get_string('cannotmovefiles', 'corplms_completionimport', $a), 'notifyproblem');
        return false;
    }
...
function import_completions($tempfilename, $importname, $importtime, $quiet = false) {
    ...
    if ($errors = check_fields_exist($tempfilename, $importname)) {
        ...
        if (!$quiet) {
            echo $OUTPUT->notification(get_string('missingfields', 'corplms_completionimport'), 'notifyproblem');
    ...
    if ($errors = import_csv($tempfilename, $importname, $importtime)) {
        ...
        if (!$quiet) {
            echo $OUTPUT->notification(get_string('csvimportfailed', 'corplms_completionimport'), 'notifyproblem');
            echo html_writer::alist($errors);
        }
        unlink($tempfilename);
        return false;
    ...
    unlink($tempfilename);
    if (!$quiet) {
        echo $OUTPUT->notification(get_string('csvimportdone', 'corplms_completionimport'), 'notifysuccess');
    }
    ...
    if (!empty($errors)) {
        if (!$quiet) {
            echo $OUTPUT->notification(get_string('error:' . $functionname, 'corplms_completionimport'), 'notifyproblem');
            echo html_writer::alist($errors);
        }
        return false;
    }
    if (!$quiet) {
        echo $OUTPUT->notification(get_string('dataimportdone_' . $importname, 'corplms_completionimport'), 'notifysuccess');
    }
...
function reset_import($importname) {
    ...
    if ($DB->delete_records($tablename, array('importuserid' => $USER->id))) {
        echo $OUTPUT->notification(get_string('resetcomplete', 'corplms_completionimport', $importname), 'notifysuccess');
    } else {
        echo $OUTPUT->notification(get_string('resetfailed', 'corplms_completionimport', $importname), 'notifyproblem');
    }
}
