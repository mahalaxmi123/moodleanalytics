...
$value = file_prepare_standard_editor($value, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
                                     'corplms_plan', 'dp_priority_scale_value', $value->id);
...
if ($valueform->is_cancelled()) {
    ...
    redirect("$CFG->wwwroot/corplms/plan/priorityscales/view.php?id={$value->priorityscaleid}");
...
// Update data
    ...
    if ($valuenew->id == 0) {
        ...
        $notification = get_string('priorityscalevalueadded', 'corplms_plan', format_string($valuenew->name));
    } else {
        ...
        $notification = get_string('priorityscalevalueupdated', 'corplms_plan', format_string($valuenew->name));
    }
    $valuenew = file_postupdate_standard_editor($valuenew, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'corplms_plan', 'dp_priority_scale_value', $valuenew->id);
    ...
    corplms_set_notification($notification,
                            "$CFG->wwwroot/corplms/plan/priorityscales/view.php?id={$valuenew->priorityscaleid}",
...
if ($id == 0) {
    echo $OUTPUT->heading(get_string('addnewpriorityvalue', 'corplms_plan'));
} else {
    echo $OUTPUT->heading(get_string('editpriorityvalue', 'corplms_plan'));
}
