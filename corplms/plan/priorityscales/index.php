...
if ($delete) {
    if (!$scale = $DB->get_record('dp_priority_scale', array('id' => $delete))) {
        print_error('error:invalidpriorityscaleid', 'corplms_plan');
    }
    if (dp_priority_scale_is_used($delete)) {
        print_error('error:nodeletepriorityscaleinuse', 'corplms_plan');
    }
    if (dp_priority_scale_is_assigned($delete)) {
        print_error('error:nodeletepriorityscaleassigned', 'corplms_plan');
    }
    ...
    if ($confirm) {
        ...
        corplms_set_notification(get_string('deletedpriorityscale', 'corplms_plan', format_string($scale->name)), $CFG->wwwroot.'/corplms/plan/priorityscales/index.php', array('class' => 'notifysuccess'));

    } else {
        $returnurl = new moodle_url('/corplms/plan/priorityscales/index.php');
        $deleteurl = new moodle_url('/corplms/plan/priorityscales/index.php', array('delete' => $delete, 'confirm' => '1', 'sesskey' => sesskey()));

        echo $OUTPUT->header();
        $strdelete = get_string('deletecheckpriority', 'corplms_plan');
