...
$priority = file_prepare_standard_editor($priority, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
                                         'corplms_plan', 'dp_priority_scale', $priority->id);
...
if ($mform->is_cancelled()) {
    ...
    redirect("$CFG->wwwroot/corplms/plan/priorityscales/index.php");
...
// Update data
    ...
    if (empty($prioritynew->id)) {
        ...
        $notification = get_string('priorityscaleadded', 'corplms_plan', format_string($prioritynew->name));
        $transaction->allow_commit();
    } else {
        // Existing priority
        $DB->update_record('dp_priority_scale', $prioritynew);
        add_to_log(SITEID, 'priorityscales', 'updated', "view.php?id=$prioritynew->id");
        $notification = get_string('priorityscaleupdated', 'corplms_plan', format_string($prioritynew->name));
    }
    $prioritynew = file_postupdate_standard_editor($prioritynew, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'corplms_plan', 'dp_priority_scale', $prioritynew->id);
    ...
    corplms_set_notification($notification,
        "$CFG->wwwroot/corplms/plan/priorityscales/view.php?id={$prioritynew->id}",
        array('class' => 'notifysuccess'));
}

/// Print Page
$PAGE->navbar->add(get_string("priorityscales", 'corplms_plan'), new moodle_url('/corplms/plan/priorityscales/index.php'));

if ($id == 0) { // Add
    $PAGE->navbar->add(get_string('priorityscalecreate', 'corplms_plan'));
    $heading = get_string('priorityscalecreate', 'corplms_plan');
} else {    //Edit
    $PAGE->navbar->add(get_string('editpriority', 'corplms_plan', format_string($priority->name)));
    $heading = get_string('editpriority', 'corplms_plan', format_string($priority->name));
}
