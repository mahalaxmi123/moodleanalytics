...
class rb_source_dp_program_recurring extends rb_base_source {
    ...
    protected function define_columnoptions() {
        ...
        $columnoptions[] = new rb_column_option(
            'program',
            'fullname',
            get_string('programname', 'corplms_program'),
            "prog.fullname",
            array('joins' => 'prog',
                  'dbdatatype' => 'char',
                  'outputformat' => 'text')
        ...
        $columnoptions[] = new rb_column_option(
            'program',
            'proglinkicon',
            get_string('prognamelinkedicon', 'corplms_program'),
            "prog.fullname",
            array(
                'joins' => 'prog',
                'displayfunc' => 'link_program_icon',
                'defaultheading' => get_string('programname', 'corplms_program'),
                'extrafields' => array(
                    'programid' => "prog.id",
        ...
        $columnoptions[] = new rb_column_option(
            'program',
            'id',
            get_string('programid', 'corplms_program'),
            "base.programid"
        );
        ...
        $columnoptions[] = new rb_column_option(
            ...
            'courselink',
            get_string('coursenamelink', 'corplms_program'),
        ...
        $columnoptions[] = new rb_column_option(
            'program_completion_history',
            'status',
            get_string('completionstatus', 'corplms_program'),
            "base.status",
            array(
        ...
        $columnoptions[] = new rb_column_option(
            'program_completion_history',
            'timecompleted',
            get_string('completiondate', 'corplms_program'),
            "base.timecompleted",
            array(
        ...
        $columnoptions[] = new rb_column_option(
            'program_completion_history',
            'timedue',
            get_string('duedate', 'corplms_program'),
            "base.timedue",
            array(
                'displayfunc' => 'completion_date',
                'dbdatatype' => 'timestamp',
    ...
    function rb_display_program_completion_status($status,$row) {
        ...
        if ($status == STATUS_PROGRAM_COMPLETE) {
            return get_string('complete', 'corplms_program');
        } else if ($status == STATUS_PROGRAM_INCOMPLETE) {
            return $OUTPUT->error_text(get_string('incomplete', 'corplms_program'));
        } else {
            return get_string('unknownstatus', 'corplms_program');
        }
    ...
    function rb_display_completion_date($time) {
        if ($time == 0) {
            ...
            return userdate($time, get_string('datepickerlongyearphpuserdate', 'corplms_core'));
        }
    }
    ...
    function rb_display_link_course_name($courseid) {
        ...
        if ($course = $DB->get_record('course', array('id' => $courseid))) {
            ...
            $html = get_string('coursenotfound', 'corplms_plan');
        }
    ...
    protected function define_filteroptions() {
        ...
        $filteroptions[] = new rb_filter_option(
                'program_completion_history',
                'timedue',
                get_string('programduedate', 'corplms_program'),
                'date'
            );
        $filteroptions[] = new rb_filter_option(
                'program_completion_history',
