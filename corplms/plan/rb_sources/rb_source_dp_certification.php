...
class rb_source_dp_certification extends rb_base_source {
    ...
    protected function define_columnoptions() {
        ...
        $columnoptions[] = new rb_column_option(
                'base',
                'fullname',
                get_string('certificationname', 'corplms_program'),
                'base.fullname',
                array(
                    'joins' => 'base',
                    'dbdatatype' => 'char',
        ...
        $columnoptions[] = new rb_column_option(
                ...
                array(
                    ...
                    'defaultheading' => get_string('certificationname', 'corplms_program'),
        ...
        $columnoptions[] = new rb_column_option(
                'base',
                'shortname',
                get_string('programshortname', 'corplms_program'),
                'base.shortname',
                array(
                    'joins' => 'base',
                    'dbdatatype' => 'char',
        ...
        $columnoptions[] = new rb_column_option(
                'base',
                'idnumber',
                get_string('programidnumber', 'corplms_program'),
                'base.idnumber',
                array(
                    'joins' => 'base',
                    'dbdatatype' => 'char',
        ...
        $columnoptions[] = new rb_column_option(
                'certif_completion',
                'timedue',
                get_string('certificationduedate', 'corplms_program'),
    ...
    public function post_config(reportbuilder $report) {
        ...
        $report->set_post_config_restrictions(corplms_visibility_where($reportfor,
        ...
        if (!empty($time)) {
            ...
            if ($time < time()) {
                ...
                if ($row->status != CERTIFSTATUS_EXPIRED) {
                    $extra = $OUTPUT->notification(get_string('windowopen', 'corplms_certification'), 'notifysuccess');
                } else {
                    $extra = $OUTPUT->notification(get_string('status_expired', 'corplms_certification'), 'notifyproblem');
                }
            } else {
                ...
                if ($days_remaining == 1) {
                    $extra = $OUTPUT->notification(get_string('windowopenin1day', 'corplms_certification'), 'notifynotice');
                } else if ($days_remaining < 10 && $days_remaining > 0) {
                    $extra = $OUTPUT->notification(get_string('windowopeninxdays', 'corplms_certification', $days_remaining), 'notifynotice');
                }
            }
    ...
    public function rb_display_historylink($name, $row) {
        global $OUTPUT;
        return $OUTPUT->action_link(new moodle_url('/corplms/plan/record/certifications.php',
    ...
    function rb_display_certif_certifpath($certifpath, $row) {
        ...
        if ($certifpath && isset($CERTIFPATH[$certifpath])) {
            return get_string($CERTIFPATH[$certifpath], 'corplms_certification');
        }
    }
    ...
    function rb_display_certif_status($status, $row) {
        ...
        if ($status && isset($CERTIFSTATUS[$status])) {
            ...
            return get_string($CERTIFSTATUS[$status], 'corplms_certification') .' '. $unassigned;
        }
    }
    ...
    function rb_display_certif_renewalstatus($renewalstatus, $row) {
        ...
        if ($renewalstatus && isset($CERTIFRENEWALSTATUS[$renewalstatus])) {
            return get_string($CERTIFRENEWALSTATUS[$renewalstatus], 'corplms_certification');
        } else {
            return get_string($CERTIFRENEWALSTATUS[CERTIFRENEWALSTATUS_NOTDUE], 'corplms_certification');
        }
    }
    ...
    function rb_filter_certifpath() {
        ...
        foreach ($CERTIFPATH as $code => $cpstring) {
            $out[$code] = get_string($cpstring, 'corplms_certification');
        }
        return $out;
    }
    ...
    function rb_filter_status() {
        ...
        foreach ($CERTIFSTATUS as $code => $statusstring) {
            $out[$code] = get_string($statusstring, 'corplms_certification');
        }
        return $out;
    }
