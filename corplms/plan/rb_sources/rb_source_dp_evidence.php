...
class rb_source_dp_evidence extends rb_base_source {
    ...
    public function rb_display_evidenceview($evidence, $row) {
        $url = new moodle_url('/corplms/plan/record/evidence/view.php', array('id' => $row->evidence_id ));
    ...
    public function rb_display_actionlinks($evidenceid, $row) {
        ...
        $canaccess = has_capability('corplms/plan:accessanyplan', $usercontext);
        $canedit = has_capability('corplms/plan:editsiteevidence', $usercontext);
        if ($row->readonly && !($canaccess || $canedit)) {
            $out .= get_string('evidence_readonly', 'corplms_plan');
        } else if ($USER->id == $row->userid ||
                corplms_is_manager($row->userid) ||
                $canaccess || $canedit) {

            $out .= $OUTPUT->action_icon(
                        new moodle_url('/corplms/plan/record/evidence/edit.php',
    ...
    public function rb_display_description($description, $row) {
        $description = file_rewrite_pluginfile_urls($description, 'pluginfile.php',
                context_system::instance()->id, 'corplms_plan', 'dp_plan_evidence', $row->evidence_id );
