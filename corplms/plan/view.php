...
$PAGE->set_url(new moodle_url('/corplms/plan/view.php', array('id' => $id)));
...
if (!has_capability('corplms/plan:accessanyplan', $context) && ($plan->get_setting('view') < DP_PERMISSION_ALLOW)) {
...
$plan = file_prepare_standard_editor($plan, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
                                    'corplms_plan', 'dp_plan', $plan->id);
$form = new plan_edit_form($currenturl, array('plan' => $plan, 'action' => $action));

if ($form->is_cancelled()) {
    corplms_set_notification(get_string('planupdatecancelled', 'corplms_plan'), $viewurl, array('class' => 'notifysuccess'));
}
...
if ($data = $form->get_data()) {
    if (isset($data->edit)) {
        ...
        if ($plan->delete()) {
            corplms_set_notification(get_string('plandeletesuccess', 'corplms_plan', $plan->name), "{$CFG->wwwroot}/corplms/plan/index.php?userid={$plan->userid}", array('class' => 'notifysuccess'));
        }
        ...
        if ($plan->set_status(DP_PLAN_STATUS_COMPLETE, DP_PLAN_REASON_MANUAL_COMPLETE)) {
            ...
            corplms_set_notification(get_string('plancompletesuccess', 'corplms_plan', $plan->name), $viewurl, array('class' => 'notifysuccess'));
        } else {
            corplms_set_notification(get_string('plancompletefail', 'corplms_plan', $plan->name), $viewurl);
        }
        ...
        // Save plan data
        $data = file_postupdate_standard_editor($data, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'corplms_plan', 'dp_plan', $data->id);
        $DB->update_record('dp_plan', $data);
        corplms_set_notification(get_string('planupdatesuccess', 'corplms_plan'), $viewurl, array('class' => 'notifysuccess'));
    }
