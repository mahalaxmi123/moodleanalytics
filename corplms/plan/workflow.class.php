...
abstract class dp_base_workflow {
    ...
    function __construct() {
        ...
        foreach ($properties as $property) {
            if (!property_exists($this, $property)) {
                ...
                throw new Exception(get_string('error:propertymustbeset', 'corplms_plan', $msg));
            }
        }
        // reserve the name 'custom' for use by the system
        if ($this->classname == 'custom') {
            throw new Exception(get_string('error:cantcreatecustomworkflow', 'corplms_plan'));
        }

        // get name and description lang string based on name
        $this->name = get_string($this->classname . 'workflowname', 'corplms_plan');
        $this->description = get_string($this->classname . 'workflowdesc', 'corplms_plan');
    }
    ...
    function copy_to_db($templateid) {
        ...
        $returnurl = $CFG->wwwroot . '/corplms/plan/template/workflow?id=' . $templateid;

        if (!$templateid) {
            print_error('error:templateid', 'corplms_plan');
        }
