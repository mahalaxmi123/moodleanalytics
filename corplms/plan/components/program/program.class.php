...
class dp_program_component extends dp_base_component {
    ...
    public function get_search_info(stdClass $search_info, array $keywords, $parentid = 0, $approved = null) {
        ...
        if ($keywords) {
            list($searchsql, $searchparams) = corplms_search_get_keyword_where_clause($keywords, array('p.fullname'),
                SQL_PARAMS_NAMED);
    ...
    public function process_settings_update($ajax = false) {
        ...
        if (!empty($duedates) && $cansetduedates) {
            $datepickerlongyearplaceholder = get_string('datepickerlongyearplaceholder', 'corplms_core');
        ...
        if (!empty($stored_records)) {
            ...
            if ($this->plan->reviewing_pending) {
                ...
                if (!$ajax) {
                    corplms_set_notification(get_string('programsupdated', 'corplms_plan').$issuesnotification, $currenturl, array('class' => 'notifysuccess'));
                }
            }
        }
    ...
    public function assign_new_item($itemid, $checkpermissions = true, $manual = true) {
        ...
        if ($checkpermissions) {
            if (!$permission = $this->can_update_items()) {
                print_error('error:cannotupdateprograms', 'corplms_plan');
            }
        } else {
    ...
    public function setup_picker() {
        ...
        if ($this->can_update_items()) {
            ...
            $PAGE->requires->string_for_js('addprograms', 'corplms_plan');

            $jsmodule = array(
                'name' => 'corplms_plan_program_find',
                'fullpath' => '/corplms/plan/components/program/find.js',
                'requires' => array('json'));
            $PAGE->requires->js_init_call('M.corplms_plan_program_find.init', array('args' => '{"plan_id":'.$this->plan->id.'}'), false, $jsmodule);
        }
    }
    ...
    protected function display_list_item_progress($item) {
        $program = new program($item->programid);
        return $this->is_item_approved($item->approved) ? $program->display_progress($this->plan->userid) : get_string('unapproved', 'corplms_plan');
    }
    ...
    public function display_item_name($item) {
        ...
        $img = html_writer::empty_tag('img', array('src' => corplms_get_icon($item->programid, CORPLMS_ICON_TYPE_PROGRAM),
            'class' => 'course_icon', 'alt' => ''));
        if ($approved && $accessible) {
            $link = $OUTPUT->action_link(
                    new moodle_url('/corplms/plan/components/' . $this->component . '/view.php',array('id' => $this->plan->id, 'itemid' => $item->id, 'userid' => $extraparams)),
    ...
    function display_program_detail($progassid) {
        ...
        if (!$item) {
            return get_string('programnotfound', 'corplms_plan');
        }

        $out = '';
        $itemicon = (!empty($item->icon)) ? $item->icon : 'default';
        $icon = html_writer::empty_tag('img', array('src' => corplms_get_icon($item->id, CORPLMS_ICON_TYPE_PROGRAM),
            'class' => 'course_icon', 'alt' => ''));
    ...
    public function progress_stats() {
        ...
        $progress_str = "{$completedcount}/" . count($programs) . " " .
            get_string('programscomplete', 'corplms_program') . ", {$inprogresscount} " .
