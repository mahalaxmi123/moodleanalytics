...
class dp_competency_component extends dp_base_component {
    ...
    public function __construct($plan) {
        ...
        $this->defaultname = get_string('competencies', 'corplms_plan');
    }
    ...
    public function get_search_info(stdClass $search_info, $keywords, $parentid = 0, $approved = null) {
        ...
        if ($keywords) {
            list($searchsql, $searchparams) = corplms_search_get_keyword_where_clause($keywords, array('c.fullname'),
                SQL_PARAMS_NAMED);
    ...
    public function process_action_hook() {
        ...
        if ($delete && $confirm) {
            if (!confirm_sesskey()) {
                corplms_set_notification(get_string('confirmsesskeybad', 'error'), $currenturl);
            }
            if ($this->remove_competency_assignment($delete)) {
                ...
                corplms_set_notification(get_string('canremoveitem', 'corplms_plan'), $currenturl, array('class' => 'notifysuccess'));
            } else {
                corplms_set_notification(get_string('cannotremoveitem', 'corplms_plan'), $currenturl);
            }
        }
    }
    ...
    public function plan_create_hook() {
        if ($this->get_setting('autoassignorg')) {
            ...
            if (!$this->assign_from_org()) {
                error(get_string('unabletoassigncompsfromorg', 'corplms_plan'));
            }
        }
        if ($this->get_setting('autoassignpos')) {
            // From position
            if (!$this->assign_from_pos()) {
                error(get_string('unabletoassigncompsfrompos', 'corplms_plan'));
            }
        }
    }
    ...
    public function setup_picker() {
        ...
        if ($this->can_update_items() || hierarchy_can_add_competency_evidence($this->plan, $this, $this->plan->viewas, null)) {
            ...
            $PAGE->requires->string_for_js('addcompetencys', 'corplms_plan');

            $jsmodule = array(
                'name' => 'corplms_plan_competency_find',
                'fullpath' => '/corplms/plan/components/competency/find.js',
                'requires' => array('json'));
            $PAGE->requires->js_init_call('M.corplms_plan_competency_find.init', array('args' => '{"plan_id":'.$this->plan->id.'}'), false, $jsmodule);
        }
    }
    ...
    public function post_header_hook() {
        ...
        if ($delete) {
            ...
            if ($courses) {
                ...
                echo $OUTPUT->heading(get_string('deletelinkedcoursesheader', 'corplms_plan', s(format_string($compname))));

                if ($USER->id == $this->plan->userid) {
                    echo html_writer::tag('p', get_string('deletelinkedcoursesinstructionslearner', 'corplms_plan'));
                } else {
                    if ($planowner = $DB->get_record('user', array('id' => $this->plan->userid), 'firstname, lastname')) {
                        $planowner_name = fullname($planowner);
                    }

                    echo html_writer::tag('p', get_string('deletelinkedcoursesinstructionsmanager', 'corplms_plan', $planowner_name));
                }

                $form = html_writer::start_tag('form', array('method' => 'get', 'action' => "{$CFG->wwwroot}/corplms/plan/component.php"));
                ...
                $form .= $OUTPUT->container(html_writer::empty_tag("input", array('type' => "submit", 'value' => s(get_string('deletelinkedcoursessubmit', 'corplms_plan')))), 'buttons');
                ...
                echo $OUTPUT->confirm(get_string('confirmitemdelete', 'corplms_plan'), $continueurl, $currenturl);
                echo $OUTPUT->footer();
                die();
    ...
    function get_course_evidence_items($competencies) {
        ...
        $out = corplms_group_records($rs, 'competencyid');
        $rs->close();
    ...
    function display_approval_list($pendingitems) {
        ...
        foreach ($pendingitems as $item) {
            ...
            if (array_key_exists($item->competencyid, $evidence)) {
                ...
                $name .= html_writer::tag('strong', get_string('includerelatedcourses', 'corplms_plan'));
                foreach ($evidence[$item->competencyid] as $course) {
                    $message = '';
                    if ($this->plan->get_component('course')->is_item_assigned($course->courseid)) {
                        $message = ' (' . get_string('alreadyassignedtoplan', 'corplms_plan') . ')';
                    }
                    $name .= html_writer::empty_tag('br');
    ...
    function display_linked_competencies($list, $mandatory_list = null) {
        ...
        if (!$this->plan->is_complete() && $this->can_update_items()) {
            $tableheaders[] = get_string('remove', 'corplms_plan', get_string('competencies', 'corplms_plan'));
            $tablecolumns[] = 'remove';
        }
                ...
                if (!$this->plan->is_complete() && $this->can_update_items()) {
                    if (!empty($mandatory_list) && in_array($ca->id, $mandatory_list)) {
                        ...
                        $row[] = html_writer::checkbox('delete_linked_comp_assign['.$ca->id.']', '1', false,
                            get_string('mandatory', 'corplms_plan'), array('disabled' => 'true'));
                    } else {
                        $row[] = html_writer::checkbox('delete_linked_comp_assign['.$ca->id.']', '1', false);
                    }
                }
    ...
    public function display_item_name($item) {
        ...
        $img = $OUTPUT->pix_icon("/msgicons/" . $icon, '', 'corplms_core', array('class' => 'competency-state-icon'));
    ...
    function display_competency_detail($caid) {
        ...
        if (!$item) {
            return get_string('error:competencynotfound', 'corplms_plan');
        }
        ...
        $icon = $this->determine_item_icon($item);
        $icon = $OUTPUT->pix_icon("/msgicons/" . $icon, format_string($item->fullname), 'corplms_core', array('class' => "competency_state_icon"));
        ...
        if ($priorityenabled && !empty($item->priority)) {
            $cells[] = new html_table_cell(get_string('priority', 'corplms_plan') . ': ' . $this->display_priority_as_text($item->priority, $item->priorityname, $priorityvalues));
        }
        if ($duedateenabled && !empty($item->duedate)) {
            $cells[] = new html_table_cell(get_string('duedate', 'corplms_plan') . ': ' . $this->display_duedate_as_text($item->duedate) . html_writer::empty_tag('br') . $this->display_duedate_highlight_info($item->duedate));
        }
        if ($status = $this->get_status($item->competencyid)) {
            $cells[] = new html_table_cell(get_string('status', 'corplms_plan'). ': ' . format_string($status));
        }
        ...
        $item->description = file_rewrite_pluginfile_urls($item->description, 'pluginfile.php',
            context_system::instance()->id, 'corplms_hierarchy', 'comp', $item->id);
            ...
            if ($status) {
                ...
                foreach ($stored_records as $itemid => $record) {
                    ...
                    if (!empty($record->priority) && $oldrecords[$itemid]->priority != $record->priority) {
                        ...
                        $updates .= $compheader;
                        $compprinted = true;
                        $updates .= get_string('priority', 'corplms_plan').' - '.get_string('changedfromxtoy',
                            'corplms_plan', (object)array('before' => $oldpriority, 'after' => $newpriority)).html_writer::empty_tag('br');
                    }
                    if (!empty($record->duedate) && $oldrecords[$itemid]->duedate != $record->duedate) {
                        ...
                        $updates .= get_string('duedate', 'corplms_plan').' - '.
                            get_string('changedfromxtoy', 'corplms_plan', (object)array('before' => empty($oldrecords[$itemid]->duedate) ? '' :
                    ...
                    if (!empty($record->approved) && $oldrecords[$itemid]->approved != $record->approved) {
                        ...
                        $text = $compheader;
                        $text .= get_string('approval', 'corplms_plan').' - '.
                            get_string('changedfromxtoy', 'corplms_plan', (object)array('before' => dp_get_approval_status_from_code($oldrecords[$itemid]->approved),
                ...
                if ($status) {
                    ...
                    if (!empty($badduedates)) {
                        $issuesnotification .= $this->get_setting('duedatemode') == DP_DUEDATES_REQUIRED ?
                            html_writer::empty_tag('br').get_string('noteduedateswrongformatorrequired', 'corplms_plan') : html_writer::empty_tag('br').get_string('noteduedateswrongformat', 'corplms_plan');
                    }

                    // Do not create notification or redirect if ajax request
                    if (!$ajax) {
                        corplms_set_notification(get_string('competenciesupdated', 'corplms_plan').$issuesnotification, $currenturl, array('class' => 'notifysuccess'));
                    }
                } else {
                    // Do not create notification or redirect if ajax request
                    if (!$ajax) {
                        corplms_set_notification(get_string('error:competenciesupdated', 'corplms_plan'), $currenturl);
                    }
                }
            }
        }
    ...
    function remove_competency_assignment($caid) {
        ...
        if (!$this->can_delete_item($item)) {
            print_error('error:nopermissiondeletemandatorycomp', 'corplms_plan');
        }
    ...
    public function assign_new_item($itemid, $checkpermissions = true, $manual = true) {
        ...
        if ($checkpermissions) {
            if (!$permission = $this->can_update_items()) {
                print_error('error:cannotupdatecompetencies', 'corplms_plan');
            }
        } else {
    ...
    public function get_competency_menu($item) {
        ...
        $output = html_writer::select($formatscale,
                                    ...
                                    array(($item->profscalevalueid ? '' : 0) => ($item->profscalevalueid ? '' : get_string('notset', 'corplms_hierarchy'))),
                                    $attributes);
    ...
    public function display_comp_add_evidence_icon($item, $returnurl=false) {
        global $OUTPUT;
        if ($this->can_update_competency_evidence($item)) {
            $straddevidence = get_string('setstatusicon', 'corplms_plan');
            return $OUTPUT->action_icon(new moodle_url('/corplms/plan/components/competency/add_evidence.php',
    ...
    public function display_course_picker($competencyid) {
        ...
        $btntext_addfromplan = get_string('addlinkedcourses', 'corplms_plan');
        $btntext_addfromcomp = get_string('addlinkedcoursescompetency', 'corplms_plan');
    ...
    public function progress_stats() {
        ...
        $progress_str =  get_string('xofy', 'corplms_core', $a) . " " .
            get_string('competenciesachieved', 'corplms_plan') . "\n";
