...
$form = $OUTPUT->heading(get_string('confirmlinkedcourses', 'corplms_plan'));
$form .= html_writer::tag('p', get_string('confirmlinkedcoursesdesc', 'corplms_plan'));
...
foreach ($evidence as $compid => $linkedcourses) {
    $form .= get_string('competency', 'corplms_hierarchy') . ' '. format_string($compnames[$compid]) . ':' . html_writer::empty_tag('br');
