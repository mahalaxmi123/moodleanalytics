...
class dp_course_component extends dp_base_component {
    ...
    public function get_assigned_items($approved = null, $orderby='', $limitfrom='', $limitnum='') {
        ...
        list($visibilitysql, $visibilityparams) = corplms_visibility_where($this->plan->userid,
                                                                          'c.id',
                                                                          'c.visible',
                                                                          'c.audiencevisible',
                                                                          'c',
    ...
    public function process_action_hook() {
        ...
        if ($delete && $confirm) {
            ...
            if (!$deleteitem = $this->get_assignment($delete)) {
                print_error('error:couldnotfindassigneditem', 'corplms_plan');
            }

            // Check mandatory permissions
            if (!$this->can_delete_item($deleteitem)) {
                print_error('error:nopermissiondeletemandatorycourse', 'corplms_plan');
            }
            ...
            if ($this->unassign_item($deleteitem)) {
                ...
                print_error('error:couldnotunassignitem', 'corplms_plan');
            }
        }
    }
    ...
    public function setup_picker() {
        ...
        if ($this->can_update_items()) {
            ...
            $PAGE->requires->string_for_js('addcourses', 'corplms_plan');

            $jsmodule = array(
                'name' => 'corplms_plan_course_find',
                'fullpath' => '/corplms/plan/components/course/find.js',
                'requires' => array('json'));
            $PAGE->requires->js_init_call('M.corplms_plan_course_find.init', array('args' => '{"plan_id":'.$this->plan->id.'}'), false, $jsmodule);
        }
    }
    ...
    function display_linked_courses($list, $mandatory_list = null) {
        ...
        $tableheaders = array(
            get_string('coursename', 'corplms_plan'),
        );
        $tablecolumns = array(
            'fullname',
        );
        ...
        $tableheaders[] = get_string('progress', 'corplms_plan');
        $tablecolumns[] = 'progress';

        if (!$this->plan->is_complete() && $this->can_update_items()) {
            $tableheaders[] = get_string('remove', 'corplms_plan', get_string('courses'));
            $tablecolumns[] = 'remove';
        }
    ...
    public function display_item_name($item) {
        ...
        if ($approved) {
            $class = '';
            $action_link = $OUTPUT->action_link(new moodle_url('/course/view.php', array('id' => $item->courseid)), get_string('launchcourse', 'corplms_plan'), null, array('class' => 'link-as-button'));
        ...
        $img = html_writer::empty_tag('img', array('src' => corplms_get_icon($item->courseid, CORPLMS_ICON_TYPE_COURSE),
            'class' => 'course_icon', 'alt' => ''));
        $url = new moodle_url('/corplms/plan/components/' . $this->component . '/view.php', array('id' => $this->plan->id, 'itemid' => $item->id));
    ...
    function display_course_detail($caid) {
        ...
        if (!$item) {
            return get_string('coursenotfound', 'corplms_plan');
        }
        ...
        if ($this->is_item_approved($item->approved)) {
            $action_link = $OUTPUT->action_link(new moodle_url('/course/view.php', array('id' => $item->courseid)), get_string('launchcourse', 'corplms_plan'));
            $out = $OUTPUT->container($action_link, "plan-launch-course-button");
        }

        $icon = html_writer::empty_tag('img', array('src' => corplms_get_icon($item->courseid, CORPLMS_ICON_TYPE_COURSE),
            'class' => 'course_icon', 'alt' => ''));
        $out .= $OUTPUT->heading($icon . format_string($item->fullname), 3);
        $cell = array();

        if ($priorityenabled && !empty($item->priority)) {
            $cell[] = new html_table_cell(get_string('priority', 'corplms_plan') . ': ' . $this->display_priority_as_text($item->priority, $item->priorityname, $priorityvalues));
        }
        if ($duedateenabled && !empty($item->duedate)) {
            $cell[] = new html_table_cell(get_string('duedate', 'corplms_plan') . ': ' . $this->display_duedate_as_text($item->duedate) . html_writer::empty_tag('br') . $this->display_duedate_highlight_info($item->duedate));
        }
        if ($progressbar = $this->display_status_as_progress_bar($item)) {
            unset($completionstatus);
            $cell[] = new html_table_cell(get_string('progress', 'corplms_plan'));
    ...
    function display_status_as_progress_bar($item) {
        return corplms_display_course_progress_icon($this->plan->userid, $item->courseid, $item->coursecompletion);
    }
        ...
        if (!empty($stored_records)) {
                ...
                foreach ($stored_records as $itemid => $record) {
                    ...
                    if (!empty($record->priority) && $oldrecords[$itemid]->priority != $record->priority) {
                        ...
                        $updates .= get_string('priority', 'corplms_plan').' - '.
                            get_string('changedfromxtoy', 'corplms_plan', (object)array('before' => $oldpriority, 'after' => $newpriority)).html_writer::empty_tag('br');
                    }
            ...
            if ($this->plan->reviewing_pending) {
                ...
                if ($status) {
                    ...
                    if (!$ajax) {
                        corplms_set_notification(get_string('coursesupdated', 'corplms_plan').$issuesnotification, $currenturl, array('class' => 'notifysuccess'));
                    }
                } else {
                    // Do not create notification or redirect if ajax request
                    if (!$ajax) {
                        corplms_set_notification(get_string('coursesnotupdated', 'corplms_plan'), $currenturl);
                    }
                }
            }
    ...
    function get_list_headers() {
        ...
        foreach ($headers->headers as $i => $h) {
            if ($h == get_string('status', 'corplms_plan')) {
                // Replace 'Status' header with 'Progress'
                $headers->headers[$i] = get_string('progress', 'corplms_plan');
                break;
            }
        }
    ...
    protected function display_list_item_actions($item) {
        ...
        if ($cansetcompletion && $approved && $CFG->enablecourserpl) {
            $strrpl = get_string('addrpl', 'corplms_plan');
            $proficient = $OUTPUT->action_icon(new moodle_url('/corplms/plan/components/course/rpl.php', array('id' => $this->plan->id, 'courseid' => $item->courseid)),
    ...
    public function progress_stats() {
        ...
        $progress_str = "{$completedcount}/" . count($courses) . " " .
            get_string('coursescomplete', 'corplms_plan') . ", {$inprogresscount} " .
            get_string('inprogress', 'corplms_plan') . "\n";
    ...
    public function display_competency_picker($courseid) {
        ...
        $btntext = get_string('addlinkedcompetencies', 'corplms_plan');
