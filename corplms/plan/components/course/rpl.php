...
require_once($CFG->dirroot . '/corplms/plan/components/course/rpl_form.php');
...
$PAGE->set_url(new moodle_url('/corplms/plan/components/course/rpl.php', array('id' => $id, 'courseid' => $courseid)));
...
if ($component->get_setting('setcompletionstatus') != DP_PERMISSION_ALLOW) {
    print_error('error:coursecompletionpermission', 'corplms_plan');
}
...
$mform = new corplms_course_rpl_form(null, compact('id','rplid','rpltext','courseid','userid'));
...
if ($fromform = $mform->get_data()) {
    ...
    if (strlen($rpl)) {
        ...
        $alert_detail->text = get_string('completedviarpl', 'corplms_plan', $completion->rpl);
    ...
    corplms_set_notification(
        get_string('rplupdated', 'corplms_plan'),
        $returnurl,
