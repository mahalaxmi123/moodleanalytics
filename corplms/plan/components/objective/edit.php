...
$PAGE->set_url(new moodle_url('/corplms/plan/components/objective/edit.php', array('id' => $planid)));
...
if ($objectiveid) {
    $objviewurl = "{$CFG->wwwroot}/corplms/plan/components/objective/view.php?id={$planid}&amp;itemid={$objectiveid}";
} else {
...
$objective = file_prepare_standard_editor($objective, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
                                          'corplms_plan', 'dp_plan_objective', $objective->itemid);
$mform = $component->objective_form($objectiveid);
if (isset($objective->duedate)) {
    $objective->duedate = userdate($objective->duedate, get_string('datepickerlongyearphpuserdate', 'corplms_core'), $CFG->timezone, false);
}
...
if ($deleteyes) {
    ...
    if (!$component->delete_objective($objectiveid)) {
        print_error('error:objectivedeleted', 'corplms_plan');
    } else {
        corplms_set_notification(get_string('objectivedeleted', 'corplms_plan'), $objallurl, array('class' => 'notifysuccess'));
    }
} else if ($deleteno) {
    redirect($objallurl);
    ...
    if (empty($data->itemid)) {
        $result = $component->create_objective(
                ...
                !empty($data->duedate) ? corplms_date_parse_from_format(get_string('datepickerlongyearparseformat', 'corplms_core'), $data->duedate) : null,
                isset($data->scalevalueid) ? $data->scalevalueid : null
        );
        if (!$result) {
            print_error('error:objectiveupdated', 'corplms_plan');
        } else {
            $data->itemid = $result;
            $notification = get_string('objectivecreated', 'corplms_plan');
        ...
        $record->duedate = !empty($data->duedate)? corplms_date_parse_from_format(get_string('datepickerlongyearparseformat', 'corplms_core'), $data->duedate):null;
        ...
        $notification = get_string('objectiveupdated', 'corplms_plan');
    }
    $data = file_postupdate_standard_editor($data, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'corplms_plan', 'dp_plan_objective', $data->itemid);
    $DB->set_field('dp_plan_objective', 'description', $data->description, array('id' => $data->itemid));
    corplms_set_notification($notification, $objviewurl, array('class' => 'notifysuccess'));
}
...
$PAGE->navbar->add($fullname, new moodle_url('/corplms/plan/view.php', array('id' => $planid)));
$PAGE->navbar->add(get_string($component->component, 'corplms_plan'));
...
switch($action) {
    case 'add':
        echo $OUTPUT->heading(get_string('addnewobjective', 'corplms_plan'));
        print $component->display_back_to_index_link();
        $mform->display();
        break;
    case 'delete':
        echo $OUTPUT->heading(get_string('deleteobjective', 'corplms_plan'));
        print $component->display_back_to_index_link();
        ...
        echo $OUTPUT->confirm(get_string('deleteobjectiveareyousure', 'corplms_plan'),
                new moodle_url('/corplms/plan/components/objective/edit.php',
                    array('id' => $planid, 'itemid' => $objectiveid, 'deleteyes' => 'Yes', 'sesskey' => sesskey())),
                new moodle_url('/corplms/plan/components/objective/edit.php',
                        array('id' => $planid, 'itemid' => $objectiveid, 'deleteno' => 'No')));
        break;
    case 'edit':
        echo $OUTPUT->heading(get_string('editobjective', 'corplms_plan', $objective->fullname));
        print $component->display_back_to_index_link();
        $mform->display();
        break;
}
