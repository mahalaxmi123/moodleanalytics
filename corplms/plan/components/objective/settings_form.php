...
require_once($CFG->dirroot.'/corplms/plan/components/objective/objective.class.php');
...
function dp_objective_component_build_settings_form(&$mform, $customdata) {
    ...
    $mform->addElement('header', 'objectivesettings', get_string('objectivesettings', 'corplms_plan'));
    $mform->addHelpButton('objectivesettings', 'advancedsettingsobjectivesettings', 'corplms_plan', '', true);
    ...
    $mform->addElement('select', 'objectivescale', get_string('objectivescale', 'corplms_plan'), $objectivemenu);
    if (!empty($customdata['templateinuse'])) {
        ...
        $mform->addElement('static', 'objectivescalesdisabledtemplateinuse', null, get_string('objectivescalesdisabledtemplateinuse', 'corplms_plan'));
    }

    $mform->setDefault('objectivescale', $defaultobjectivescale);

    //Permissions
    $mform->addElement('header', 'objectivepermissions', get_string('objectivepermissions', 'corplms_plan'));
    $mform->addHelpButton('objectivepermissions', 'advancedsettingsobjectivepermissions', 'corplms_plan', '', true);
...
function dp_objective_component_process_settings_form($fromform, $id) {
    ...
    $currenturl = new moodle_url('/corplms/plan/template/advancedworkflow.php', array('id' => $id, 'component' => 'objective'));
    ...
    corplms_set_notification(get_string('update_objective_settings', 'corplms_plan'), $currenturl, array('class' => 'notifysuccess'));
}
