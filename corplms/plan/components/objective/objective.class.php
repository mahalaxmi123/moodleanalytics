...
class dp_objective_component extends dp_base_component {
    ...
    public function __construct($plan) {
        ...
        $this->defaultname = get_string('objectives', 'corplms_plan');
    }
    ...
    public function get_search_info(stdClass $search_info, array $keywords, $parentid = 0, $approved = null) {
        ...
        if ($keywords) {
            list($searchsql, $searchparams) = corplms_search_get_keyword_where_clause($keywords, array('a.fullname'),
                SQL_PARAMS_NAMED);
    ...
    public function setup_picker() {
        ...
        if ($this->can_update_items()) {
            ...
            $jsmodule = array(
                            'name' => 'corplms_plan_component',
                            'fullpath' => '/corplms/plan/component.js',
                            'requires' => array('json'));
            $PAGE->requires->js_init_call('M.corplms_plan_component.init', array('args' => '{"plan_id":'.$this->plan->id.', "page":"'.$paginated.'", "component_name":"'.$component_name.'", "sesskey":"'.$sesskey.'"}'), false, $jsmodule);
    ...
    function display_linked_objectives($list) {
        ...
        $objectivename = get_string('objective', 'corplms_plan');
        ...
        $tableheaders = array(
            get_string('name'),
            get_string('status', 'corplms_plan'),
        );
        $tablecolumns = array(
            'fullname',
            'proficiency',
        ...
        if ($showpriorities) {
            $tableheaders[] = get_string('priority', 'corplms_plan');
    ...
    function display_item_name($item) {
        ...
        $img = $OUTPUT->pix_icon("/msgicons/" . $icon, format_string($item->fullname), 'corplms_core', array('class' => 'objective-state-icon'));
        $link = $OUTPUT->action_link(
                new moodle_url('/corplms/plan/components/' . $this->component . '/view.php', array('id' => $this->plan->id, 'itemid' => $item->id)),
    ...
    function objective_form($objectiveid=null) {
        global $CFG, $DB;
        require_once($CFG->dirroot.'/corplms/plan/components/objective/edit_form.php');
        $customdata = array(
            'plan' => $this->plan,
            'objective' => $this
        );
        if (empty($objectiveid)) {
            ...
            if (!$objective = $DB->get_record('dp_plan_objective', array('id' => $objectiveid))) {
                print_error('error:objectiveidincorrect', 'corplms_plan');
            }
    ...
    public function process_settings_update($ajax = false) {
        ...
        if (!empty($duedates) && $cansetduedates) {
            ...
            foreach ($duedates as $id => $duedate) {
                ...
                if ($duedate == '' || $duedate == get_string('datepickerlongyearplaceholder', 'corplms_core')) {
                    ...
                    $datepattern = get_string('datepickerlongyearregexphp', 'corplms_core');
                    if (preg_match($datepattern, $duedate, $matches) == 0) {
                        // skip badly formatted date strings
                        continue;
                    }
                    $duedateout = corplms_date_parse_from_format(get_string('datepickerlongyearparseformat', 'corplms_core'), $duedate);
                }
                $todb = new stdClass();
                $todb->id = $id;
        ...
        if (!empty($proficiencies) && $cansetprofs) {
            foreach ($proficiencies as $id => $proficiency) {
                ...
                $count = $DB->count_records('block_corplms_stats', array('userid' => $currentuser, 'eventtype' => STATS_EVENT_OBJ_ACHIEVED, 'data2' => $id));
                $scalevalue = $DB->get_record('dp_objective_scale_value', array('id' => $proficiency));
                if (empty($scalevalue)) {
                    print_error('error:priorityscalevalueidincorrect', 'corplms_plan');
                    // checks objective can only be achieved once.
                } else if ($scalevalue->achieved == 1 && $count < 1) {
                    corplms_stats_add_event(time(), $currentuser, STATS_EVENT_OBJ_ACHIEVED, '', $id);
                    // checks objective exists for removal
                } else if ($scalevalue->achieved == 0 && $count > 0) {
                    corplms_stats_remove_event($currentuser, STATS_EVENT_OBJ_ACHIEVED, $id);
                }
            }
        }
        ...
        if (!empty($stored_records)) {
            ...
            foreach ($stored_records as $itemid => $record) {
                ...
                if (!empty($record->priority) && array_key_exists($itemid, $orig_objectives) &&
                    ...
                    $updates .= $objheader;
                    $objprinted = true;
                    $updates .= get_string('priority', 'corplms_plan').' - '.
                        get_string('changedfromxtoy', 'corplms_plan',
                ...
                if (!empty($record->duedate) && array_key_exists($itemid, $orig_objectives) &&
                    ...
                    $objprinted = true;
                    $updates .= $stringmanager->get_string('duedate', 'corplms_plan', null, $currentuserobj->lang).' - '.
                            get_string('changedfromxtoy', 'corplms_plan',
                ...
                if (!empty($record->scalevalueid) && array_key_exists($itemid, $orig_objectives) &&
                    ...
                    $updates .= $objprinted ? '' : $objheader;
                    $objprinted = true;
                    $updates .= $stringmanager->get_string('status', 'corplms_plan', null, $currentuserobj->lang).' - '.
                        get_string('changedfromxtoy', 'corplms_plan',
                ...
                if (!empty($record->approved) && array_key_exists($itemid, $orig_objectives) &&
                    ...
                    $text = $objheader;
                    $text .= $stringmanager->get_string('approval', 'corplms_plan', null, $currentuserobj->lang).' - '.
                            get_string('changedfromxtoy', 'corplms_plan',
            ...
            if ($this->plan->reviewing_pending) {
                ...
                corplms_set_notification(get_string('objectivesupdated', 'corplms_plan'), $currenturl, array('class' => 'notifysuccess'));
            }
        }
    ...
    function send_deletion_alert($objective) {
        ...
        $event->contexturl = new moodle_url("/corplms/plan/view.php", array('id' => $this->plan->id));
        ...
        if ($USER->id == $this->plan->userid) {
            ...
            if ($this->plan->is_active()) {
                ...
                if ($manager = corplms_get_manager($this->plan->userid)) {
                    $event->userto = $manager;
                    $event->subject = $stringmanager->get_string('objectivedeleteshortmanager', 'corplms_plan', fullname($USER), $manager->lang);
                    $event->fullmessage = $stringmanager->get_string('objectivedeletelongmanager', 'corplms_plan', $a, $manager->lang);
                    $event->fullmessagehtml = $stringmanager->get_string('objectivedeletelongmanagerhtml', 'corplms_plan', $a, $manager->lang);
                    tm_alert_send($event);
                }
            }
        }
        // notify user that someone else did it
            ...
            $event->userto = $userto;
            $event->subject = $stringmanager->get_string('objectivedeleteshortlearner', 'corplms_plan', $a->objective, $userto->lang);
            $event->fullmessage = $stringmanager->get_string('objectivedeletelonglearner', 'corplms_plan', $a, $userto->lang);
            $event->fullmessagehtml = $stringmanager->get_string('objectivedeletelonglearnerhtml', 'corplms_plan', $a, $userto->lang);
            tm_alert_send($event);
        }
    }
    ...
    function send_creation_alert($objid, $fullname) {
        ...
        $userfrom = $DB->get_record('user', array('id' => $USER->id));
        $event->userfrom = $userfrom;
        $event->contexturl = new moodle_url('/corplms/plan/components/objective/view.php', array('id' => $this->plan->id, 'itemid' => $objid));
        $event->icon = 'objective-add';
        $a = new stdClass();
        $a->objective = $fullname;
        $a->objectivehtml = $OUTPUT->action_link($event->contexturl, $fullname);
        $url = new moodle_url('/corplms/plan/view.php', array('id' => $this->plan->id));
        $a->plan = $this->plan->name;
        ...
        if ($USER->id == $this->plan->userid) {
            ...
            if ($this->plan->is_active()) {
                ...
                if ($manager = corplms_get_manager($this->plan->userid)) {
                    $event->userto = $manager;
                    $event->subject = $stringmanager->get_string('objectivenewshortmanager', 'corplms_plan', fullname($USER), $manager->lang);
                    $event->fullmessage = $stringmanager->get_string('objectivenewlongmanager', 'corplms_plan', $a, $manager->lang);
                    $event->fullmessagehtml = $stringmanager->get_string('objectivenewlongmanagerhtml', 'corplms_plan', $a, $manager->lang);
                    tm_alert_send($event);
                }
            }
        }
        // notify user that someone else did it
            ...
            $userto = $DB->get_record('user', array('id' => $this->plan->userid));
            $event->userto = $userto;
            $event->subject = $stringmanager->get_string('objectivenewshortlearner', 'corplms_plan', $fullname, $userto->lang);
            $event->fullmessage = $stringmanager->get_string('objectivenewlonglearner', 'corplms_plan', $a, $userto->lang);
            $event->fullmessagehtml = $stringmanager->get_string('objectivenewlonglearnerhtml', 'corplms_plan', $a, $userto->lang);
            tm_alert_send($event);
        }
    }
    ...
    function send_edit_alert($objective, $field) {
        ...
        $userfrom = $DB->get_record('user', array('id' => $USER->id));
        $event->userfrom = $userfrom;
        $event->contexturl = new moodle_url("/corplms/plan/components/objective/view.php", array('id' => $this->plan->id, 'itemid' => $objective->id));
        ...
        if ($USER->id == $this->plan->userid) {
            ...
            if ($this->plan->is_active()) {
                // notify their manager
                if ($manager = corplms_get_manager($this->plan->userid)) {
                    $a->field = $stringmanager->get_string('objective'.$field, 'corplms_plan', $manager->lang);
                    $event->userto = $manager;
                    $event->subject = $stringmanager->get_string('objectiveeditshortmanager', 'corplms_plan', fullname($USER), $manager->lang);
                    $event->fullmessage = $stringmanager->get_string('objectiveeditlongmanager', 'corplms_plan', $a, $manager->lang);
                    $event->fullmessagehtml = $stringmanager->get_string('objectiveeditlongmanagerhtml', 'corplms_plan', $a, $manager->lang);
                    tm_alert_send($event);
                }
            }
        } else {
            // notify user that someone else did it
            $userto = $DB->get_record('user', array('id' => $this->plan->userid));
            $a->field = $stringmanager->get_string('objective'.$field, 'corplms_plan', null, $userto->lang);
            $event->userto = $userto;
            $event->subject = $stringmanager->get_string('objectiveeditshortlearner', 'corplms_plan', $a->objective, $userto->lang);
            $event->fullmessage = $stringmanager->get_string('objectiveeditlonglearner', 'corplms_plan', $a, $userto->lang);
            $event->fullmessagehtml = $stringmanager->get_string('objectiveeditlonglearnerhtml', 'corplms_plan', $a, $userto->lang);
            tm_alert_send($event);
        }
    }
    ...
    function send_status_alert($objective) {
        ...
        if ($USER->id == $this->plan->userid) {
            ...
            if ($this->plan->is_active()) {
                ...
                if ($manager = corplms_get_manager($this->plan->userid)) {
                    $event->userto = $manager;
                    $event->subject = $stringmanager->get_string('objective'.$status.'shortmanager', 'corplms_plan', fullname($USER), $manager->lang);
                    $event->fullmessage = $stringmanager->get_string('objective'.$status.'longmanager', 'corplms_plan', $a, $manager->lang);
                    $event->fullmessagehtml = $stringmanager->get_string('objective'.$status.'longmanagerhtml', 'corplms_plan', $a, $manager->lang);
                    tm_alert_send($event);
                }
            }
        } else {
            ...
            $userto = $DB->get_record('user', array('id' => $this->plan->userid));
            $event->userto = $userto;
            $event->subject = $stringmanager->get_string('objective'.$status.'shortlearner', 'corplms_plan', $a->objective, $userto->lang);
            $event->fullmessage = $stringmanager->get_string('objective'.$status.'longlearner', 'corplms_plan', $a, $userto->lang);
            $event->fullmessagehtml = $stringmanager->get_string('objective'.$status.'longlearnerhtml', 'corplms_plan', $a, $userto->lang);
            tm_alert_send($event);
        }
    }
    ...
    protected function display_list_item_actions($item) {
        ...
        if ($this->can_delete_item($item)) {
            $deleteurl = new moodle_url('/corplms/plan/components/objective/edit.php',
                array('id' => $this->plan->id, 'itemid' => $item->id, 'd' => 1));
            $strdelete = get_string('delete', 'corplms_plan');
    ...
    public function display_picker() {
        ...
        if ($permission >= DP_PERMISSION_ALLOW) {
            $btntext = get_string('addnewobjective', 'corplms_plan');
        } else {
            $btntext = get_string('requestednewobjective', 'corplms_plan');
        }

        $button = $OUTPUT->single_button(new moodle_url("/corplms/plan/components/objective/edit.php", array('id' => $this->plan->id)), $btntext, 'get');
    ...
    public function display_course_picker($objectiveid) {
        ...
        $btntext = get_string('addlinkedcourses', 'corplms_plan');
    ...
    public function display_objective_detail($objectiveid) {
        ...
        if (!$item) {
            return get_string('error:objectivenotfound', 'corplms_plan');
        }
        ...
        $icon = $this->determine_item_icon($item);
        $icon = $OUTPUT->pix_icon("/msgicons/" . $icon, format_string($item->fullname), 'corplms_core', array('class' => 'objective_state_icon'));
        ...
        if (!$plancompleted && ($canupdate = $this->can_update_items())) {
            ...
            if ($this->will_an_update_revoke_approval( $objectiveid )) {
                $buttonlabel = get_string('editdetailswithapproval', 'corplms_plan');
            } else {
                $buttonlabel = get_string('editdetails', 'corplms_plan');
            }
            $out .= $OUTPUT->container($OUTPUT->single_button(new moodle_url("/corplms/plan/components/objective/edit.php", array('id' => $this->plan->id, 'itemid' => $objectiveid)), $buttonlabel), 'add-linked-course');
        }

        $row = new html_table_row();
        if ($priorityenabled && !empty($item->priority)) {
            $row->cells[] = get_string('priority', 'corplms_plan') . ': ' . $this->display_priority_as_text($item->priority, $item->priorityname, $priorityvalues);
        }
        if ($duedateenabled && !empty($item->duedate)) {
            $cell = get_string('duedate', 'corplms_plan') . ': ' . $this->display_duedate_as_text($item->duedate);
            if (!$item->achieved) {
        ...
        if (!empty($item->profname)) {
            $row->cells[] = get_string('status', 'corplms_plan') .": \n" . "  {$item->profname}\n";
        }
        ...
        $table->data = array($row);
        $out .= html_writer::table($table);
        $item->description = file_rewrite_pluginfile_urls($item->description, 'pluginfile.php', context_system::instance()->id, 'corplms_plan', 'dp_plan_objective', $item->id);
    ...
    public function progress_stats() {
        ...
        $progress_str = "{$completedcount}/" . count($objectives) . " " .
            get_string('objectivesmet', 'corplms_plan') . "\n";
