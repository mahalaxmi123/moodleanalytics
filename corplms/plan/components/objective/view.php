...
$objectivename = get_string($componentname, 'corplms_plan');
$coursename = get_string('courseplural', 'corplms_plan');
$currenturl = new moodle_url('/corplms/plan/components/objective/view.php', array('id' => $id, 'itemid' => $caid));
...
if ($canupdate) {
    ...
    $jsmodule = array(
                                'name' => 'corplms_plan_objective_find_course',
                                'fullpath' => '/corplms/plan/components/objective/find-course.js',
                                'requires' => array('json'));
    $PAGE->requires->js_init_call('M.corplms_plan_objective_find_course.init', array('args' => '{"plan_id":'.$id.',"objective_id":'.$caid.'}'), false, $jsmodule);
...
if ($data = data_submitted() && $canupdate) {
    ...
    if ($action === 'removelinkedcourses' && !$plan->is_complete()) {
        ...
        if ($deletions) {
            corplms_set_notification(get_string('selectedlinkedcoursesremovedfromobjective', 'corplms_plan'), $currenturl, array('class' => 'notifysuccess'));
        } else {
            redirect($currenturl);
        }
        die();
...
if ($data = $mform->get_data()) {
    if (isset($data->edit)) {
        redirect("{$CFG->wwwroot}/corplms/plan/components/objective/edit.php?id={$id}&itemid={$caid}");
    } else if (isset($data->delete)) {
        redirect("{$CFG->wwwroot}/corplms/plan/components/objective/edit.php?id={$id}&itemid={$caid}&d=1");
    }
}
...
$PAGE->navbar->add(get_string($component->component, 'corplms_plan'), $component->get_url());
