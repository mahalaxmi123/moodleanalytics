...
$currenturl = new moodle_url('/corplms/plan/components/objective/approval.php', array('id' => $id, 'itemid' => $caid, 'action' => $action));
...
if ($confirm) {
    ...
    if (!$canapproveobjective) {
        ...
        corplms_set_notification(get_string('nopermission', 'corplms_plan'),
            $returnurl);
        die();
    }
    ...
    corplms_set_notification(get_string('request'.$action, 'corplms_plan'), $returnurl, array('class' => 'notifysuccess'));
...
$PAGE->navbar->add(get_string('itemapproval', 'corplms_plan'));
...
echo $OUTPUT->confirm(get_string('confirmrequest'.$action, 'corplms_plan'), $currenturl.'&amp;confirm=1&amp;sesskey='.sesskey(), $returnurl);
