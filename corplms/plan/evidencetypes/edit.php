...
if (empty($id)) {
    ...
    if (!$item = $DB->get_record('dp_evidence_type', array('id' => $id))) {
        print_error('error:evidencetypedidincorrect', 'corplms_plan');
    }
}

$item->descriptionformat = FORMAT_HTML;
$item = file_prepare_standard_editor($item, 'description',
        $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'corplms_plan', 'dp_evidence_type', $item->id);

$mform = new edit_evidencetype_form(null, array('id' => $id));
$mform->set_data($item);

if ($mform->is_cancelled()) {
    redirect(new moodle_url('/corplms/plan/evidencetypes/index.php'));
    ...
    if (empty($data->id)) {
        ...
        if (!$data->id = $DB->insert_record('dp_evidence_type', $data)) {
            print_error(get_string('error:createnewevidencetype', 'corplms_plan'));
        }
    } else {
        $action = 'updated';
        if (!$DB->update_record('dp_evidence_type', $data)) {
            print_error('error:updateevidencetype', 'corplms_plan');
        }
    }

    // save and relink embedded images
    $data = file_postupdate_standard_editor($data, 'description',
            $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'corplms_plan', 'dp_evidence_type', $data->id);
    $DB->update_record('dp_evidence_type', $data);

    corplms_set_notification(get_string('evidencetype'.$action, 'corplms_plan',
            format_string(stripslashes($data->name))),
        new moodle_url('/corplms/plan/evidencetypes/view.php', array('id' => $data->id)),
...
// Print Page
if (empty($id)) { // Add
    $heading = get_string('evidencetypecreate', 'corplms_plan');
} else {    // Edit
    $heading = get_string('editevidencetype', 'corplms_plan', format_string($item->name));
}
