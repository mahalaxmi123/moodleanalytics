...
require_once($CFG->dirroot . '/corplms/plan/development_plan.class.php');
require_once($CFG->dirroot . '/corplms/plan/role.class.php');
require_once($CFG->dirroot . '/corplms/plan/component.class.php');
require_once($CFG->dirroot . '/corplms/plan/workflow.class.php');
require_once($CFG->dirroot . '/corplms/program/lib.php'); // needed to display required learning in plans menu
...
*/
function corplms_plan_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, $options=array()) {
    $fs = get_file_storage();
    $relativepath = implode('/', $args);
    $fullpath = "/{$context->id}/corplms_plan/$filearea/$args[0]/$args[1]";
...
function dp_can_view_users_plans($ownerid) {
    ...
    if (has_capability('corplms/plan:accessanyplan', $systemcontext)) {
        return true;
    }

    // If the user cannot view any plans
    if (!has_capability('corplms/plan:accessplan', $systemcontext)) {
        return false;
    }
...
function dp_get_alert_receivers($contextuser, $type) {
    ...
    $users = get_users_by_capability($contextuser, "corplms/plan:receive{$type}alerts");
...
function dp_add_permissions_select(&$form, $name, $requestable) {
    ...
    $select_options[DP_PERMISSION_ALLOW] = get_string('allow', 'corplms_plan');
    $select_options[DP_PERMISSION_DENY] = get_string('deny', 'corplms_plan');

    if ($requestable) {
        $select_options[DP_PERMISSION_REQUEST] = get_string('request', 'corplms_plan');
        $select_options[DP_PERMISSION_APPROVE] = get_string('approve', 'corplms_plan');
    }
...
function dp_add_permissions_table_headings(&$form) {
    ...
    $out .= html_writer::start_tag('tr') . html_writer::tag('th', get_string('action', 'corplms_plan'));

    foreach ($DP_AVAILABLE_ROLES as $role) {
        $out .= html_writer::tag('th', get_string($role, 'corplms_plan'));
    }
...
function dp_get_rol_tabs_visible($userid) {
    ...
    if (($assigned_progs > 0 || $show_program_tab) && corplms_feature_visible('programs')) {
        $visible[] = 'programs';
    }
    ...
    if (($certification_progs > 0 || $unassignedcertifications) && corplms_feature_visible('certifications')) {
    ...
    if ($visible = dp_get_rol_tabs_visible($userid)) {
        foreach ($visible as $element) {
            if ($element !== 'evidence') {
                $row[] = new tabobject(
                        $element,
                        new moodle_url("/corplms/plan/record/{$element}.php", $params),
                        get_string($rolstatus . $element, 'corplms_plan')
                );
            } else {
                $row[] = new tabobject(
                        'evidence',
                        new moodle_url('/corplms/plan/record/evidence/index.php', $params),
                        get_string($rolstatus . 'evidence', 'corplms_plan')
                );
            }
        }
    }
...
function dp_print_workflow_diff($diff_array) {
    global $OUTPUT;
    $columns[] = 'component';
    $headers[] = get_string('component', 'corplms_plan');
    $columns[] = 'setting';
    $headers[] = get_string('setting', 'corplms_plan');
    $columns[] = 'role';
    $headers[] = get_string('role', 'corplms_plan');
    $columns[] = 'before';
    $headers[] = get_string('before', 'corplms_plan');
    $columns[] = 'after';
    $headers[] = get_string('after', 'corplms_plan');

    $baseurl = new moodle_url( '/corplms/plan/template/workflow.php');
    $table = new flexible_table('Templates');
    $table->define_columns($columns);
    $table->define_headers($headers);
    $table->define_baseurl($baseurl);
    $return = html_writer::start_tag('p');
    $return .= $OUTPUT->heading(get_string('changes', 'corplms_plan'), 3);

    $table->setup();

    $permission_options = array(DP_PERMISSION_ALLOW => get_string('allow', 'corplms_plan'),
        DP_PERMISSION_DENY => get_string('deny', 'corplms_plan'),
        DP_PERMISSION_REQUEST => get_string('request', 'corplms_plan'),
        DP_PERMISSION_APPROVE => get_string('approve', 'corplms_plan')
    );

    $duedate_options = array(DP_DUEDATES_NONE => get_string('none'),
        DP_DUEDATES_OPTIONAL => get_string('optional', 'corplms_plan'),
        DP_DUEDATES_REQUIRED => get_string('required', 'corplms_plan')
    );

    $priority_options = array(DP_PRIORITY_NONE => get_string('none'),
        DP_PRIORITY_OPTIONAL => get_string('optional', 'corplms_plan'),
        DP_PRIORITY_REQUIRED => get_string('required', 'corplms_plan')
    );
    ...
    foreach ($diff_array as $item => $values) {
        ...
        if ($parts[0] == 'perm') {
            if ($parts[1] != 'plan') {
                ...
                $compname = $configsetting ? $configsetting : get_string($parts[1], 'corplms_plan');
                $tablerow[] = $compname;
            } else {
                $tablerow[] = get_string($parts[1], 'corplms_plan');
            }
            $tablerow[] = get_string($parts[2], 'corplms_plan');
            $tablerow[] = get_string($parts[3], 'corplms_plan');
            ...
            $tablerow[] = get_string($parts[2], 'corplms_plan');
            $tablerow[] = get_string('na', 'corplms_plan');
            switch($parts[2]) {
                case 'duedatemode':
...
function dp_display_plans($userid, $statuses=array(DP_PLAN_STATUSAPPROVED), $cols=array('enddate', 'status', 'completed'), $firstcolheader='') {
    ...
    if (empty($firstcolheader)) {
        $tableheaders[] = get_string('plan', 'corplms_plan');
    } else {
        $tableheaders[] = $firstcolheader;
    }

    if (in_array('enddate', $cols)) {
        $tableheaders[] = get_string('duedate', 'corplms_plan');
        $tablecols[] = 'enddate_'.$statuses_undrsc;
    }
    if (in_array('status', $cols)) {
        $tableheaders[] = get_string('status', 'corplms_plan');
        $tablecols[] = 'status_'.$statuses_undrsc;
    }
    if (in_array('completed', $cols)) {
        $tableheaders[] = get_string('completed', 'corplms_plan');
        $tablecols[] = 'timemodified_'.$statuses_undrsc;
    }

    // Actions
    $tableheaders[] = get_string('actions', 'corplms_plan');
    $tablecols[] = 'actioncontrols';

    $baseurl = $CFG->wwwroot . '/corplms/plan/index.php';
    if ($userid != $USER->id) {
...
function dp_display_plans_menu($userid, $selectedid=0, $role='learner', $rolpage='courses', $rolstatus='none', $showrol=true, $selectedprogid=0, $showrequired=true) {
    ...
    $list = array();
    $attr = array();
    $enableplans = corplms_feature_visible('learningplans');

    $out = $OUTPUT->container_start(null, 'dp-plans-menu');

    if ($role == 'manager') {
        // Print out the All team members link
        $out .= $OUTPUT->heading(get_string('teammembers', 'corplms_plan'), 3, 'main');
        $class = $userid == 0 ? 'dp-menu-selected' : '';
        $out .= html_writer::alist(array($OUTPUT->action_link(new moodle_url('/my/teammembers.php'), get_string('allteammembers', 'corplms_plan'))), array('class' => $class));
        if ($userid) {
            // Display who we are currently viewing if appropriate
            $out .= $OUTPUT->heading(get_string('currentlyviewing', 'corplms_plan'), 3, 'main');
            // TODO: make this more efficient
            $user = $DB->get_record('user', array('id' => $userid));
            $class = $selectedid == 0 ? 'dp-menu-selected' : '';
            $out .= html_writer::alist(array($OUTPUT->action_link(new moodle_url('/corplms/plan/index.php', array('userid' => $userid)), "$user->firstname $user->lastname")), array('class' => $class));
        }
    }
    ...
    if ($enableplans && $plans = dp_get_plans($userid, array(DP_PLAN_STATUS_APPROVED))) {
        if ($role == 'manager') {
            ...
            $out .= $OUTPUT->heading(get_string('activeplans', 'corplms_plan'), 4, 'dp-plans-menu-sub-header');
        }
        else {
            $out .= $OUTPUT->heading(get_string('activeplans', 'corplms_plan'), 3, 'main');
        }

        $list = array();
        foreach ($plans as $p) {
            $attr['class'] = $p->id == $selectedid ? 'dp-menu-selected' : '';
            $list[] = $OUTPUT->action_link(new moodle_url('/corplms/plan/view.php', array('id' => $p->id)), $p->name);
        }
    ...
    if ($enableplans && $plans = dp_get_plans($userid, array(DP_PLAN_STATUS_UNAPPROVED, DP_PLAN_STATUS_PENDING))) {
        if ($role == 'manager') {
            $out .= $OUTPUT->container_start(null, 'dp-plans-menu-section');
            $out .= $OUTPUT->heading(get_string('unapprovedplans', 'corplms_plan'), 4, 'dp-plans-menu-sub-header');
        }
        else {
            $out .= $OUTPUT->heading(get_string('unapprovedplans', 'corplms_plan'), 3, 'main');
        }
    ...
    if ($enableplans && $plans = dp_get_plans($userid, DP_PLAN_STATUS_COMPLETE)) {
        if ($role == 'manager') {
            $out .= $OUTPUT->container_start(null, 'dp-plans-menu-section');
            $out .= $OUTPUT->heading(get_string('completedplans', 'corplms_plan'), 4, 'dp-plans-menu-sub-header');
        }
        else {
            $out .= $OUTPUT->heading(get_string('completedplans', 'corplms_plan'), 3, 'main');
        }
    ...
    if ($showrequired) {
        ...
        if ($programs || $certifications) {
            $canviewprograms = corplms_feature_visible('programs');
            $canviewcertifications = corplms_feature_visible('certifications');
            $extraparams = array();
            $headingclass = 'main';
            if ($role == 'manager') {
            ...
            $out .= $OUTPUT->heading(get_string('requiredlearning', 'corplms_program'), 3, $headingclass);

            if ($programs && $canviewprograms) {
                $list = dp_display_plans_menu_required($programs, $extraparams);
                $out .= $OUTPUT->heading(get_string('programs', 'corplms_program'), 5);
                $out .= html_writer::alist($list, $attr);
            }
            if ($certifications && $canviewcertifications) {
                $list = dp_display_plans_menu_required($certifications, $extraparams, count($list));
                $out .= $OUTPUT->heading(get_string('certifications', 'corplms_program'), 5);
                $out .= html_writer::alist($list, $attr);
            }
            if ($role == 'manager') {
                $out .= $OUTPUT->container_end();
...
function dp_display_plans_menu_required($programs, $extraparams, $progcount=0) {
    ...
    foreach ($programs as $p) {
        ...
        if (count($list) + $progcount >= DP_MAX_PROGS_TO_DISPLAY) {
            $list[] = $OUTPUT->action_link(new moodle_url('/corplms/program/required.php', $urlparams), get_string('viewallrequiredlearning', 'corplms_program'));
            break;
        }
        ...
        $urlparams['id'] = $p->id;
        $list[] = $OUTPUT->action_link(new moodle_url('/corplms/program/required.php', $urlparams), $p->fullname);
    }
    return($list);
}
...
function dp_display_user_message_box($planuser) {
    ...
    $r = new html_table_row(array(
        ...
        get_string('youareviewingxsplans', 'corplms_plan', $a),
    ));
...
function dp_record_status_picker($pagename, $status, $userid=null) {
    ...
    foreach (array('all','active','completed') as $s) {
        ...
        $options[$s] = get_string($s . 'learning', 'corplms_plan');
    }

    $label = html_writer::tag('strong', get_string('filterbystatus', 'corplms_plan')) . '&nbsp;';

    // display status pulldown
    $form = $OUTPUT->single_select(
        new moodle_url("/corplms/plan/record/{$pagename}.php", array('userid' => $userid, 'status' => '')),
        'status',
        $options,
        $selected
    );
...
function dp_record_status_menu($pagename, $status, $userid=null) {
    ...
    $out = $OUTPUT->heading(get_string('recordoflearning', 'corplms_core'), 3, 'main');

    // generate options for menu display
    $filter = array();
    $items = array();
    foreach (array('all','active','completed') as $s) {
        $filter[$s] = get_string($s . 'learning', 'corplms_plan');
        $class = $status == $s ? "dp-menu-selected" : '';
        $items[] = $OUTPUT->action_link(new moodle_url("/corplms/plan/record/{$pagename}.php", array('userid' => $userid, 'status' => $s)), $filter[$s], null, array('class' => $class));
    }
...
function dp_get_plan_base_navlinks($userid) {
    ...
    if ($userid == $USER->id) {
        ...
        $PAGE->navbar->add(get_string('learningplans', 'corplms_plan'), new moodle_url('/corplms/plan/index.php'));
        return true;
    }
    ...
    if ($user) {
        ...
        $PAGE->navbar->add(get_string('xslearningplans', 'corplms_plan', fullname($user)), new moodle_url('/corplms/plan/index.php', array('userid' => $userid)));
    } else {
        $PAGE->navbar->add(get_string('unknownuserslearningplans', 'corplms_plan'), new moodle_url('/corplms/plan/index.php', array('userid' => $userid)));
    }
}
...
function dp_get_approval_status_from_code($code) {
    switch ($code) {
        case DP_APPROVAL_DECLINED:
            $status = get_string('declined', 'corplms_plan');
            break;
        case DP_APPROVAL_UNAPPROVED:
            $status = get_string('unapproved', 'corplms_plan');
            break;
        case DP_APPROVAL_REQUESTED:
            $status = get_string('pendingapproval', 'corplms_plan');
            break;
        case DP_APPROVAL_APPROVED:
            $status = get_string('approved', 'corplms_plan');
            break;
        default:
            $status = get_string('unknown', 'corplms_plan');
            break;
    }
...
function dp_create_template($templatename, $enddate, &$error) {
    ...
    foreach ($DP_AVAILABLE_COMPONENTS as $component) {
        ...
        if (!is_readable($classfile)) {
            ...
            $error = get_string('noclassfileforcomponent', 'corplms_plan', $string_properties);
            return false;
        }
        ...
        if (!class_exists($class)) {
            ...
            $string_properties->component = $component;
            $error = get_string('noclassforcomponent', 'corplms_plan', $string_properties);
            return false;
        }
        $cn = new stdClass();
    ...
    $classfile = $CFG->dirroot . "/corplms/plan/workflows/{$workflow}/{$workflow}.class.php";
    if (!is_readable($classfile)) {
        $string_properties = new stdClass();
        $string_properties->classfile = $classfile;
        $string_properties->workflow = $workflow;
        $error = get_string('noclassfileforworkflow', 'corplms_plan', $string_properties);
        return false;
    }
    include_once($classfile);
    // check class exists
    ...
    if (!class_exists($class)) {
        ...
        $string_properties->workflow = $workflow;
        $error = get_string('noclassforworkflow', 'corplms_plan', $string_properties);
        return false;
    }
    ...
    if (!$workflow_class->copy_to_db($newtemplateid)) {
        $error = get_string('error:newdptemplate', 'corplms_plan');
        return false;
    }
...
function dp_plan_item_updated($userid, $component, $componentid) {
    ...
    $component_include = $CFG->dirroot . '/corplms/plan/components/' . $component . '/' . $component . '.class.php';
...
function corplms_plan_comment_permissions($details) {
    global $DB;
    ...
    $plan = new development_plan($planid);
    if (!has_capability('corplms/plan:accessanyplan', $details->context) && ($plan->get_setting('view') < DP_PERMISSION_ALLOW)) {
...
function corplms_plan_comment_template() {
    global $OUTPUT, $PAGE;

    // Use the corplms default comment template
 ...
 * @return boolean
 */
function corplms_plan_comment_validate($comment_param) {
    global $DB;
    // comment itemid and comment area already validated as part of permission check (corplms_plan_comment_permissions)
...
function corplms_plan_comment_add($comment) {
    global $CFG, $DB, $USER;
    ...
    switch ($comment->commentarea) {
        case 'plan_overview':
            ...
            $contexturl = new moodle_url('/corplms/plan/view.php', array('id' => $plan->id.'#comments'));
        ...
        case 'plan_course_item':
            ...
            if (!$record = $DB->get_record_sql($sql, $params)) {
                print_error('commenterror:itemnotfound', 'corplms_plan');
            }
            ...
            $msgobj->plan = $plan->name;
            $msgobj->planowner = fullname($DB->get_record('user', array('id' => $plan->userid)));
            $msgobj->component = get_string('course', 'corplms_plan');
            $msgobj->componentname = $record->fullname;
            $msgobj->comment = format_text($comment->content);
            $msgobj->commentby = fullname($commentuser);
            $msgobj->commentdate = userdate($comment->timecreated);
            $contexturl = new moodle_url('/corplms/plan/components/course/view.php', array('id' => $plan->id, 'itemid' => $comment->itemid.'#comments'));
        ...
        case 'plan_competency_item':
            ...
            $msgobj->plan = $plan->name;
            $msgobj->planowner = fullname($DB->get_record('user', array('id' => $plan->userid)));
            $msgobj->component = get_string('competency', 'corplms_plan');
            $msgobj->componentname = $record->fullname;
            $msgobj->comment = format_text($comment->content);
            $msgobj->commentby = fullname($commentuser);
            $msgobj->commentdate = userdate($comment->timecreated);
            $contexturl = new moodle_url('/corplms/plan/components/competency/view.php', array('id' => $plan->id, 'itemid' => $comment->itemid.'#comments'));
            $contexturlname = $record->fullname;
        ...
        case 'plan_objective_item':
            ...
            $msgobj->plan = $plan->name;
            $msgobj->planowner = fullname($DB->get_record('user', array('id' => $plan->userid)));
            $msgobj->component = get_string('objective', 'corplms_plan');
            $msgobj->componentname = $record->fullname;
            $msgobj->comment = format_text($comment->content);
            $msgobj->commentby = fullname($commentuser);
            $msgobj->commentdate = userdate($comment->timecreated);
            $contexturl = new moodle_url('/corplms/plan/components/objective/view.php', array('id' => $plan->id, 'itemid' => $comment->itemid.'#comments'));
            $contexturlname = $record->fullname;
        ...
        case 'plan_program_item':
            ...
            $params = array($comment->itemid);
            if (!$record = $DB->get_record_sql($sql, $params)) {
                print_error('comment_error:itemnotfound', 'corplms_plan');
            }
            $plan = $DB->get_record('dp_plan', array('id' => $record->planid));
            ...
            $msgobj->plan = $plan->name;
            $msgobj->planowner = fullname($DB->get_record('user', array('id' => $plan->userid)));
            $msgobj->component = get_string('program', 'corplms_plan');
            $msgobj->componentname = $record->fullname;
            $msgobj->comment = format_text($comment->content);
            $msgobj->commentby = fullname($commentuser);
            $msgobj->commentdate = userdate($comment->timecreated);

            $contexturl = new moodle_url('/corplms/plan/components/program/view.php', array('id' => $plan->id, 'itemid' => $comment->itemid.'#comments'));
            $contexturlname = $record->fullname;
            $icon = 'program-newcomment';

            break;
        default:
            print_error('commenterror:unsupportedcomment', 'corplms_plan');
            break;
    }
    ...
    $manager = corplms_get_manager($plan->userid);
    ...
    foreach ($subscribers as $sid) {
        ...
        if ($comment->commentarea == 'plan_overview') {
            $subject = $stringmanager->get_string('commentmsg:planoverview', 'corplms_plan', $msgobj, $userto->lang);
            $fullmsg = $stringmanager->get_string('commentmsg:planoverviewdetail', 'corplms_plan', $msgobj, $userto->lang);
        } else {
            $subject = $stringmanager->get_string('commentmsg:componentitem', 'corplms_plan', $msgobj, $userto->lang);
            $fullmsg = $stringmanager->get_string('commentmsg:componentitemdetail', 'corplms_plan', $msgobj, $userto->lang);
        }
...
function plan_mark_competency_default($competencyid, $userid, $component) {
    ...
    require_once($CFG->dirroot.'/corplms/hierarchy/prefix/competency/evidence/lib.php');

    $details = new stdClass();
    $details->assessmenttype = get_string('automateddefault', 'corplms_plan');
 ...
 * Run the plan cron
 */
function corplms_plan_cron() {
    global $CFG;

    // Run cron if Learning plans are enabled.
    if (!corplms_feature_disabled('learningplans')) {
        require_once($CFG->dirroot . '/corplms/plan/cron.php');
        plan_cron();
    }
}
...
function check_learningplan_enabled() {
    if (corplms_feature_disabled('learningplans')) {
        print_error('learningplansdisabled', 'corplms_plan');
    }
}
