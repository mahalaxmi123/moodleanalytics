...
$PAGE->set_url(new moodle_url('/corplms/plan/action.php', $pageparams));
...
if (!empty($approve)) {
    if (in_array($plan->get_setting('approve'), array(DP_PERMISSION_ALLOW, DP_PERMISSION_APPROVE))) {
       ...
       corplms_set_notification(get_string('planapproved', 'corplms_plan', $plan->name), $referer, array('class' => 'notifysuccess'));
    } else {
        if (empty($ajax)) {
            corplms_set_notification(get_string('nopermission', 'corplms_plan'), $referer, array('class' => 'notifysuccess'));
        }
    }
}
...
if (!empty($decline)) {
    if (in_array($plan->get_setting('approve'), array(DP_PERMISSION_ALLOW, DP_PERMISSION_APPROVE))) {
        ...
        corplms_set_notification(get_string('plandeclined', 'corplms_plan', $plan->name), $referer, array('class' => 'notifysuccess'));
    } else {
        if (empty($ajax)) {
            corplms_set_notification(get_string('nopermission', 'corplms_plan'), $referer);
        }
    }
}
        ...
        if ($plan->get_setting('approve') == DP_PERMISSION_REQUEST) {
            ...
            if ($USER->id == $plan->userid) {
                if ($manager) {
                    ...
                    corplms_set_notification(get_string('nomanager', 'corplms_plan'), $referer);
                }
            }
            corplms_set_notification(get_string('approvalrequestsent', 'corplms_plan', $plan->name), $referer, array('class' => 'notifysuccess'));
...
if (!empty($delete)) {
    if ($plan->get_setting('delete') == DP_PERMISSION_ALLOW) {
        ...
        if (!$confirm && empty($ajax)) {
            ...
            $PAGE->set_title(get_string('confirmdeleteplantitle', 'corplms_plan', $plan->name));
            echo $OUTPUT->header();
            echo $OUTPUT->heading(get_string('deleteplan', 'corplms_plan'));
            $confirmurl = new moodle_url(qualified_me());
            $confirmurl->param('confirm', 'true');
            $confirmurl->param('referer', $referer);
            $strdelete = get_string('checkplandelete', 'corplms_plan', $plan->name);
            ...
            corplms_set_notification(get_string('plandeletesuccess', 'corplms_plan', $plan->name), $referer, array('class' => 'notifysuccess'));
        }
    } else {
        if (empty($ajax)) {
            corplms_set_notification(get_string('nopermission', 'corplms_plan'), $referer);
...
if (!empty($complete)) {
    if ($plan->get_setting('completereactivate') >= DP_PERMISSION_ALLOW) {
        ...
        if (!$confirm && empty($ajax)) {
            ...
            $confirmurl->param('confirm', 'true');
            $confirmurl->param('referer', $referer);
            $strcomplete = get_string('checkplancomplete11', 'corplms_plan', $plan->name);
            $strbreak = html_writer::empty_tag('br') . html_writer::empty_tag('br');
            ...
            corplms_set_notification(get_string('plancompletesuccess', 'corplms_plan', $plan->name), $referer, array('class' => 'notifysuccess'));
        }
    } else {
        if (empty($ajax)) {
            corplms_set_notification(get_string('nopermission', 'corplms_plan'), $referer);
...
///
if (!empty($reactivate)) {
    require_once($CFG->dirroot . '/corplms/plan/reactivate_form.php');
    ...
    if ($plan->get_setting('completereactivate') >= DP_PERMISSION_ALLOW) {
        ...
        if ($data = $form->get_data()) {
            ...
            if (!$plan->reactivate_plan($new_date)) {
                corplms_set_notification(get_string('planreactivatefail', 'corplms_plan', $plan->name), $referer);
            } else {
                add_to_log(SITEID, 'plan', 'reactivated', "view.php?id={$plan->id}", $plan->name);
                corplms_set_notification(get_string('planreactivatesuccess', 'corplms_plan', $plan->name), $referer, array('class' => 'notifysuccess'));
            }
        }

        echo $OUTPUT->header();
        echo $OUTPUT->heading(get_string('planreactivate', 'corplms_plan'), 2, 'reactivateheading');
...
if (empty($ajax)) {
    corplms_set_notification(get_string('error:incorrectparameters', 'corplms_plan'), $referer);
}
