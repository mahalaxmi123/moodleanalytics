...
require_once($CFG->dirroot . '/corplms/plan/edit_form.php');
...
$PAGE->set_url('/corplms/plan/add.php', array('userid' => $userid));
...
if ($userid != $USER->id) {
    ...
    if (corplms_is_manager($userid) || is_siteadmin()) {
        $role = 'manager';
    } else {
        print_error('error:nopermissions', 'corplms_plan');
    }
...
$allplansurl = "{$CFG->wwwroot}/corplms/plan/index.php?userid={$userid}";
...
if ($data = $form->get_data()) {
    if (isset($data->submitbutton)) {
        ...
        $viewurl = "{$CFG->wwwroot}/corplms/plan/view.php?id={$newid}";
        add_to_log(SITEID, 'plan', 'created', "view.php?id={$newid}", $plan->name);

        // Free memory
        unset($plan);

        corplms_set_notification(get_string('plancreatesuccess', 'corplms_plan'), $viewurl, array('class' => 'notifysuccess'));
    }
}
...
/// Display
///
$heading = get_string('createnewlearningplan', 'corplms_plan');
$pagetitle = format_string(get_string('learningplan', 'corplms_plan').': '.$heading);
dp_get_plan_base_navlinks($userid);
$PAGE->navbar->add($heading);

$jsmodule = array(
    'name' => 'corplms_plan_template',
    'fullpath' => '/corplms/plan/templates.js',
    'requires' => array('json'));

$json_templates = json_encode($templates);
$args = array('args' => '{"templates":' . $json_templates . '}');

$PAGE->requires->js_init_call('M.corplms_plan_template.init', $args, false, $jsmodule);
...
echo html_writer::tag('p', get_string('createplan_instructions', 'corplms_plan'));
