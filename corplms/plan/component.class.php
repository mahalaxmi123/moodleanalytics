...
abstract class dp_base_component {
    ...
    public function __construct($plan) {
        ...
        foreach ($properties as $property) {
            if (!property_exists($this, $property) && !property_exists(get_class($this), $property)) {
                ...
                throw new Exception(get_string('error:propertymustbeset', 'corplms_plan', $string_properties));
            }
        }
    }
    ...
    public function get_url() {
        ...
        return new moodle_url('/corplms/plan/component.php', array('id' => $this->plan->id, 'c' => $this->component));
    }
    ...
    public function display_list($restrict = null) {
        ...
        if (!$count = $this->count_assigned_items($restrict)) {
            $plural = mb_strtolower(get_string($this->component.'plural', 'corplms_plan'), "UTF-8");
            return html_writer::tag('span', get_string('nox', 'corplms_plan', $plural), array('class' => 'noitems-assign'.$this->component));
        }
    ...
    protected function get_list_headers() {
        ...
        $tableheaders = array(
            get_string($this->component.'name', 'corplms_plan'),
        ...
        if (($this->component == 'competency' || $this->component == 'objective')
            ...
            $tableheaders[] = get_string('numberoflinkedcourses', 'corplms_plan');
            $tablecolumns[] = 'linkedcourses';
            $tablehide[] = 'linkedcourses';
        }

        $tableheaders[] = get_string('numberoflinkedevidence', 'corplms_plan');
    ...
    public function display_approval_list($pendingitems) {
        ...
        foreach ($pendingitems as $item) {
            ...
            $controls .= get_string('reasonfordecision', 'corplms_message');
    ...
    public function update_assigned_items($items) {
        ...
        if ($items) {
            foreach ($items as $itemid) {
                ...
                if (!is_numeric($itemid)) {
                    throw new Exception(get_string('baddata', 'corplms_plan'));
                }
                // Check if not already assigned
                if (!isset($assigned_ids[$itemid])) {
                    $result = $this->assign_new_item($itemid);
                    if (!$result) {
                        print_error('error:couldnotassignnewitem', 'corplms_plan');
                    }
                    $updates .= get_string('addedx', 'corplms_plan', $result->fullname).html_writer::empty_tag('br');
                }
        ...
        foreach ($assigned as $item) {
            ...
            $updates .= get_string('removedx', 'corplms_plan', $assigned[$item->id]->fullname).html_writer::empty_tag('br');
        }
    ...
    function send_component_update_alert($update_info='') {
        ...
        $a->planhtml = $OUTPUT->action_link(new moodle_url('/corplms/plan/view.php', array('id' => $this->plan->id)),
            $this->plan->name, null, array('title' => $this->plan->name));
        $a->component = get_string($this->component.'plural', 'corplms_plan');
        ...
        if ($USER->id == $this->plan->userid) {
            ...
            if ($this->plan->is_active()) {
                if ($manager = corplms_get_manager($this->plan->userid)) {
                    ...
                    $event->subject = $stringmanager->get_string('componentupdateshortmanager', 'corplms_plan', $a, $manager->lang);
                    $event->fullmessage = $stringmanager->get_string('componentupdatelongmanager', 'corplms_plan', $a, $manager->lang);
                    $event->fullmessagehtml = $stringmanager->get_string('componentupdatelongmanagerhtml', 'corplms_plan', $a, $manager->lang);
                    tm_alert_send($event);
                }
            }
        } else {
            ...
            $event->userto = $userto;
            $event->subject = $stringmanager->get_string('componentupdateshortlearner', 'corplms_plan', $a->component, $userto->lang);
            $event->fullmessage = $stringmanager->get_string('componentupdatelonglearner', 'corplms_plan', $a, $userto->lang);
            $event->fullmessagehtml = $stringmanager->get_string('componentupdatelonglearnerhtml', 'corplms_plan', $a, $userto->lang);
            tm_alert_send($event);
        }
    }
        ...
        if ($USER->id == $this->plan->userid) {
            ...
            if ($this->plan->is_active()) {
                if ($manager = corplms_get_manager($this->plan->userid)) {
                    $event->userto = $manager;
                    $a->user = fullname($USER);
                    $event->subject = $stringmanager->get_string('component'.$type.'shortmanager', 'corplms_plan', $a, $manager->lang);
                    $event->fullmessage = $stringmanager->get_string('component'.$type.'longmanager', 'corplms_plan', $a, $manager->lang);
                    $event->fullmessagehtml = $stringmanager->get_string('component'.$type.'longmanagerhtml', 'corplms_plan', $a, $manager->lang);
                    if (!empty($reasonfordecision)) {
                        $breakline = html_writer::empty_tag('br') . html_writer::empty_tag('br');
                        $decision = $stringmanager->get_string('reasongivenfordecision', 'corplms_plan', $reasonfordecision, $manager->lang);
            ...
            $userto = $DB->get_record('user', array('id' => $this->plan->userid));
            $event->userto = $userto;
            $event->subject = $stringmanager->get_string('component'.$type.'shortlearner', 'corplms_plan', $a, $userto->lang);
            $event->fullmessage = $stringmanager->get_string('component'.$type.'longlearner', 'corplms_plan', $a, $userto->lang);
            $event->fullmessagehtml = $stringmanager->get_string('component'.$type.'longlearnerhtml', 'corplms_plan', $a, $userto->lang);
            if (!empty($reasonfordecision)) {
                $breakline = html_writer::empty_tag('br') . html_writer::empty_tag('br');
                $decision = $stringmanager->get_string('reasongivenfordecision', 'corplms_plan', $reasonfordecision, $userto->lang);
                $event->fullmessage .= $breakline . $decision;
                $event->fullmessagehtml .= $breakline . $decision;
            }
            tm_alert_send($event);
        ...
        if ($USER->id == $this->plan->userid) {
            ...
            if ($this->plan->is_active()) {
                if ($manager = corplms_get_manager($this->plan->userid)) {
                    $event->userto = $manager;
                    $a->user = fullname($USER);
                    $event->subject = $stringmanager->get_string('componentcompleteshortmanager', 'corplms_plan', $a, $manager->lang);
                    $event->fullmessage = $stringmanager->get_string('componentcompletelongmanager', 'corplms_plan', $a, $manager->lang);
                    $event->fullmessagehtml = $stringmanager->get_string('componentcompletelongmanagerhtml', 'corplms_plan', $a, $manager->lang);
                    tm_alert_send($event);
                }
            }
        } else {
            ...
            $userto = $DB->get_record('user', array('id' => $this->plan->userid));
            $event->userto = $userto;
            $event->subject = $stringmanager->get_string('componentcompleteshortlearner', 'corplms_plan', $a, $userto->lang);
            $event->fullmessage = $stringmanager->get_string('componentcompletelonglearner', 'corplms_plan', $a, $userto->lang);
            $event->fullmessagehtml = $stringmanager->get_string('componentcompletelonglearnerhtml', 'corplms_plan', $a, $userto->lang);
            tm_alert_send($event);
        }
    }
    ...
    protected function display_list_item_comments($item) {
        ...
        if ($count = $comment->count()) {
            ...
            $tooltip = get_string('latestcommentby', 'corplms_plan').' '.$latestcomment->firstname.' '.get_string('on', 'corplms_plan').' '.userdate($latestcomment->ctimecreated).': '.format_string(substr($latestcomment->ccontent, 0, 50));
            $tooltip = format_string(strip_tags($tooltip));
            $commentclass = 'comments-icon-some';
        } else {
            $tooltip = get_string('nocomments', 'corplms_plan');
            $commentclass = 'comments-icon-none';
        }
        return $OUTPUT->action_link(new moodle_url("/corplms/plan/components/{$this->component}/view.php", array('id' => $this->plan->id, 'itemid' => $item->id), 'comments'),
    ...
    public function display_picker() {
        ...
        $add_button_text = get_string('add'.$this->component.'s', 'corplms_plan');
    ...
    function display_duedate_as_form($duedate, $name, $inputclass='', $itemid) {
        global $CFG;
        $duedatestr = !empty($duedate) ? userdate($duedate, get_string('datepickerlongyearphpuserdate', 'corplms_core'), $CFG->timezone, false) : '';
        return html_writer::empty_tag('input',
            array('id' => $name, 'type' => "text", 'name' => $name, 'placeholder' => get_string('datepickerlongyearplaceholder', 'corplms_core'),
    ...
    function display_priority_picker($name, $priorityid, $itemid, $priorityvalues, $prioritydefaultid, $priorityrequired=false) {
        ...
        $choose = ($priorityrequired) ? null : get_string('none', 'corplms_plan');
    ...
    function display_back_to_index_link() {
        ...
        global $OUTPUT;
        $url = new moodle_url('/corplms/plan/component.php', array('id' => $this->plan->id, 'c' => $this->component));
        $link = $OUTPUT->action_link($url,
            get_string('backtoallx', 'corplms_plan', get_string("{$this->component}plural", 'corplms_plan')));
    ...
    function display_approval($obj, $canapprove) {
        ...
        case DP_APPROVAL_DECLINED:
            $out .= html_writer::tag('span', get_string('declined', 'corplms_plan'), array('class' => 'plan_highlight'));
            break;
        case DP_APPROVAL_UNAPPROVED:
            $out .= $OUTPUT->pix_icon('/learning_plan_alert', get_string('unapproved', 'corplms_plan'), 'corplms_plan');
            $out .= get_string('unapproved', 'corplms_plan');
            if ($canapprove) {
                $out .= ' '.$this->display_approval_options($obj, $approvalstatus);
            }
            break;
        case DP_APPROVAL_REQUESTED:
            $out .= html_writer::tag('span', get_string('pendingapproval', 'corplms_plan'), array('class' => 'plan_highlight'));
        ...
        case DP_APPROVAL_APPROVED:
            $out .= get_string('approved', 'corplms_plan');
        }
    ...
    function display_approval_options($obj, $approvalstatus) {
        ...
        $options = array(
            DP_APPROVAL_APPROVED => get_string('approve', 'corplms_plan'),
            DP_APPROVAL_DECLINED => get_string('decline', 'corplms_plan'),
        );
