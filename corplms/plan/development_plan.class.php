...
require_once($CFG->dirroot . '/corplms/message/eventdata.class.php');
...
class development_plan {
    ...
    function __construct($id, $viewas=null) {
        ...
        if (!$plan) {
            throw new PlanException(get_string('planidnotfound', 'corplms_plan', $id));
        }
    ...
    function load_roles() {
        ...
        foreach ($DP_AVAILABLE_ROLES as $role) {
            ...
            $classfile = $CFG->dirroot .
                "/corplms/plan/roles/{$role}/{$role}.class.php";

            if (!is_readable($classfile)) {
                $string_params = new stdClass();
                $string_params->classfile = $classfile;
                $string_params->role = $role;
                throw new PlanException(get_string('noclassfileforrole', 'corplms_plan', $string_params));
            }
            ...
            if (!class_exists($class)) {
                ...
                $string_params->role = $role;
                throw new PlanException(get_string('noclassforrole', 'corplms_plan', $string_params));
            }
    ...
    function load_components() {
        ...
        foreach ($DP_AVAILABLE_COMPONENTS as $component) {
            // include each class file
            $classfile = $CFG->dirroot .
                "/corplms/plan/components/{$component}/{$component}.class.php";
            if (!is_readable($classfile)) {
                $string_params = new stdClass();
                $string_params->classfile = $classfile;
                $string_params->component = $component;
                throw new PlanException(get_string('noclassfileforcomponent', 'corplms_plan', $string_params));
            }
            include_once($classfile);
            ...
            if (!class_exists($class)) {
                ...
                $string_params->class = $class;
                $string_params->component = $component;
                throw new PlanException(get_string('noclassforcomponent', 'corplms_plan', $string_params));
            }
    ...
    public function get_components() {
        ...
        if (corplms_feature_disabled('programs')) {
            $active_components = corplms_search_for_value($active_components, 'component', CORPLMS_SEARCH_OP_NOT_EQUAL, 'program');
        }
    ...
    function get_component_setting($component, $action) {
        ...
        print_error('error:settingdoesnotexist', 'corplms_plan', '', (object)array('component' => $component, 'action' => $action));
    }
    ...
    function initialize_settings() {
        ...
        $issuperuser = has_capability('corplms/plan:manageanyplan', context_system::instance());
        ...
        foreach ($components as $component) {
            ...
            $this->settings[$component->component.'_name'] = $name ? $name :
                get_string($component->component, 'corplms_plan');
        }
    ...
    function display_summary_widget() {
        ...
        $link = html_writer::link(new moodle_url('/corplms/plan/view.php', array('id' => $this->id)), $this->name);
        ...
        if (!corplms_feature_visible('programs')) {
            $components = corplms_search_for_value($components, 'component', CORPLMS_SEARCH_OP_NOT_EQUAL, 'program');
        }

        foreach ($components as $c) {
            $component = $this->get_component($c->component);
            $compname = get_string($component->component.'plural', 'corplms_plan');
        ...
        if ($pendingitems) {
            ...
            $link = new moodle_url('/corplms/plan/approve.php', array('id' => $this->id));
            $a->link = $link->out();
            $content .= html_writer::tag('span', get_string('pendingitemsx', 'corplms_plan', $a),
                    array('class' => 'dp-summary-widget-pendingitems-text'));
        }
        $out .= $OUTPUT->container($content, 'dp-summary-widget-components');
        $description = file_rewrite_pluginfile_urls($this->description, 'pluginfile.php', context_system::instance()->id, 'corplms_plan', 'dp_plan', $this->id);
    ...
    function display_add_plan_icon() {
        ...
        return $OUTPUT->action_icon(new moodle_url('/corplms/plan/edit.php'), new pix_icon('t/add', get_string('addplan', 'corplms_plan')), null, array('title' => get_string('addplan', 'corplms_plan')));
    }
    ...
    function display_enddate_highlight_info($enddate) {
        ...
        if (isset($enddate)) {
            if (($enddate < $now) && ($now - $enddate < 60*60*24)) {
                ...
                $out .= html_writer::tag('span', get_string('duetoday', 'corplms_plan'), array('class' => 'plan_highlight'));
            } else if ($enddate < $now) {
                $out .= html_writer::empty_tag('br');
                $out .= html_writer::tag('span', get_string('overdue', 'corplms_plan'), array('class' => 'plan_highlight'));
            } else if ($enddate - $now < 60*60*24*7) {
                $days = ceil(($enddate - $now)/(60*60*24));
                $out .= html_writer::empty_tag('br');
                $out .= html_writer::tag('span', get_string('dueinxdays', 'corplms_plan', $days), array('class' => 'plan_highlight'));
            }
        }
        return $out;
    }
    ...
    public function display_progress() {
        ...
        if ($this->status == DP_PLAN_STATUS_UNAPPROVED || $this->status == DP_PLAN_STATUS_PENDING) {
            $out = get_string('planstatusunapproved', 'corplms_plan');
            if ($this->status == DP_PLAN_STATUS_UNAPPROVED) {
                // Approval request
                if ($this->get_setting('approve') == DP_PERMISSION_REQUEST) {
                    $out .= html_writer::empty_tag('br');
                    $url = new moodle_url('/corplms/plan/action.php', array('id' => $this->id, 'approvalrequest' => 1, 'sesskey' => sesskey()));
                    $out .= $OUTPUT->action_link($url, get_string('requestapproval', 'corplms_plan'));
                }
            } else {
                $out .= html_writer::empty_tag('br');
                $out .= get_string('approvalrequested', 'corplms_plan');
            }
        ...
        // Get corplms core renderer
        $corplms_renderer = $PAGE->get_renderer('corplms_core');

        // Get relevant progress bar and return for display
        return $corplms_renderer->print_corplms_progressbar($overall_progress, 'medium', false, $tooltipstr);
    }
    ...
    function display_completeddate() {
        ...
        if ($this->status != DP_PLAN_STATUS_COMPLETE) {
            return get_string('notcompleted', 'corplms_plan');
        }
        ...
        return ($latestmodification->status != DP_PLAN_STATUS_COMPLETE) ? get_string('notcompleted', 'corplms_plan') : userdate($latestmodification->timemodified, get_string('datepickerlongyearphpuserdate', 'corplms_core'), $CFG->timezone, false);
    }
    ...
    function display_actions() {
        ...
        if ($this->status == DP_PLAN_STATUS_UNAPPROVED || $this->status == DP_PLAN_STATUS_PENDING) {
            ...
            if (in_array($this->get_setting('approve'), array(DP_PERMISSION_ALLOW, DP_PERMISSION_APPROVE))) {
                echo $OUTPUT->action_icon(new moodle_url('/corplms/plan/action.php', array('id' => $this->id, 'approve' => 1, 'sesskey' => sesskey())), new pix_icon('/t/go', get_string('approve', 'corplms_plan')));
                echo $OUTPUT->action_icon(new moodle_url('/corplms/plan/action.php', array('id' => $this->id, 'decline' => 1, 'sesskey' => sesskey())), new pix_icon('/t/stop', get_string('decline', 'corplms_plan')));
            }
        }
        ...
        if ($this->status == DP_PLAN_STATUS_APPROVED && $this->get_setting('completereactivate') >= DP_PERMISSION_ALLOW  && $this->get_setting('manualcomplete')) {
            echo $OUTPUT->action_icon(new moodle_url('/corplms/plan/action.php', array('id' => $this->id, 'complete' => 1, 'sesskey' => sesskey())), new pix_icon('/i/star', get_string('plancomplete', 'corplms_plan'), 'corplms_plan'));
        }

        // Reactivate
        if ($this->status == DP_PLAN_STATUS_COMPLETE && $this->get_setting('completereactivate') >= DP_PERMISSION_ALLOW) {
            echo $OUTPUT->action_icon(new moodle_url('/corplms/plan/action.php', array('id' => $this->id, 'reactivate' => 1, 'sesskey' => sesskey())), new pix_icon('/i/star_grey', get_string('planreactivate', 'corplms_plan'), 'corplms_plan'));
        }

        // Delete
        if ($this->get_setting('delete') == DP_PERMISSION_ALLOW) {
            echo $OUTPUT->action_icon(new moodle_url('/corplms/plan/action.php', array('id' => $this->id, 'delete' => 1, 'sesskey' => sesskey())), new pix_icon('/i/invalid', get_string('delete')));
        }
...
//                DP_APPROVAL_APPROVED,
    ...
    function display_completed_plan_message() {
        ...
        case DP_PLAN_REASON_MANUAL_COMPLETE:
            $message = get_string('plancompleted', 'corplms_plan');
            break;

        case DP_PLAN_REASON_AUTO_COMPLETE_ITEMS:
            $message = get_string('planautocompleteditems', 'corplms_plan');
            break;

        case DP_PLAN_REASON_AUTO_COMPLETE_DATE:
            $message = get_string('planautocompleteddate', 'corplms_plan');
            break;
        }
        ...
        if ($this->get_setting('completereactivate') == DP_PERMISSION_ALLOW) {
            $url = new moodle_url('/corplms/plan/action.php', array('id' => $this->id, 'reactivate' => 1, 'sesskey' => sesskey()));
            $extramessage = html_writer::tag('p', get_string('reactivateplantext', 'corplms_plan', $url->out()));
        }
    ...
    function display_unapproved_plan_message() {
        ...
        $out = '';
        $out .= html_writer::start_tag('form', array('action' => new moodle_url('/corplms/plan/action.php'), 'method' => 'post', 'class' => 'approvalform'));
        ...
        $out .= get_string('plannotapproved', 'corplms_plan');

        if ($canapproveplan) {
            $out .= html_writer::start_div();
            $out .= get_string('reasonfordecision', 'corplms_message');
            $out .= html_writer::empty_tag('input', array('type' => 'text', 'name' => 'reasonfordecision'));
            $out .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'approve', 'value' => get_string('approve', 'corplms_plan')));
            $out .= '&nbsp;' . html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'decline', 'value' => get_string('decline', 'corplms_plan')));
            $out .= html_writer::end_div();
        } else if ($canrequestapproval) {
            if ($this->status == DP_PLAN_STATUS_UNAPPROVED) {
                $out .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'approvalrequest', 'value' => get_string('sendapprovalrequest', 'corplms_plan')));
            } else {
                $out .= " " . get_string('approvalrequested', 'corplms_plan');
            }
        }
    ...
    function display_pending_items($pendinglist=null) {
        ...
        foreach ($DP_AVAILABLE_COMPONENTS as $componentname) {
            ...
            if ($enabled && !empty($pendinglist[$component->component])) {
                ...
                $a->name = (get_string($langkey, 'corplms_plan') ? get_string($langkey, 'corplms_plan') : $name);
                $a->link = $component->get_url()->out();
                $list[] = get_string('xitemspending', 'corplms_plan', $a);
            }
        ...
        if (count($list)) {
            ...
            $description .= html_writer::tag('p', get_string($descriptor, 'corplms_plan'));
            $description .= html_writer::alist($list);
            $description .= $OUTPUT->container_end();

            $url = new moodle_url('/corplms/plan/approve.php', array('id' => $this->id));
            $actionbutton = $approval ? $OUTPUT->single_button($url, get_string('review', 'corplms_plan'), 'get') : '';
    ...
    public function display_unapproved_items($unapproved) {
        ...
        foreach ($unapproved as $component => $items) {
            ...
            $a->name = get_string($langkey, 'corplms_plan');
            $list[] = get_string('xitemsunapproved', 'corplms_plan', $a);
        }

        // put the heading on now we know how many
        $out = $OUTPUT->container(html_writer::tag('p', get_string(($totalitems > 1 ? 'planhasunapproveditems' : 'planhasunapproveditem'), 'corplms_plan')) . html_writer::alist($list), 'plan_box_wrap');

        // Show request button if plan is active
        if ($this->status == DP_PLAN_STATUS_APPROVED) {
            $out .= html_writer::start_tag('form', array('action' => new moodle_url('/corplms/plan/action.php'), 'method' => 'post'));
            $out .= html_writer::empty_tag('input', array('type' => 'hidden', 'name' => 'id', 'value' => $this->id));
            $out .= html_writer::empty_tag('input', array('type' => 'hidden', 'name' => 'sesskey', 'value' => sesskey()));
            $out .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'approvalrequest', 'value' => get_string('sendapprovalrequest', 'corplms_plan')));
            $out .= html_writer::end_tag('form');
    ...
    static public function display_viewing_users_plan($userid) {
        ...
        $table = new html_table();
        $table->attributes['class'] = 'invisiblepadded';
        $cells = array($OUTPUT->user_picture($user), get_string('youareviewingxsplan', 'corplms_plan', $a));
    ...
    function get_manager() {
        return corplms_get_manager($this->userid);
    }
    ...
    function send_manager_plan_approval_request() {
        ...
        $manager = corplms_get_manager($this->userid);
        ...
        if ($manager && $learner) {
            ...
            $event->subject =      $stringmanager->get_string('plan-request-manager-short', 'corplms_plan', $a, $managerlang);
            $event->fullmessage =  $stringmanager->get_string('plan-request-manager-long', 'corplms_plan', $a, $managerlang);
            $event->acceptbutton = $stringmanager->get_string('approve', 'corplms_plan', null, $managerlang) . ' ' . $stringmanager->get_string('plan', 'corplms_plan', null, $managerlang);
            $event->accepttext =   $stringmanager->get_string('approveplantext', 'corplms_plan', null, $managerlang);
            $event->rejectbutton = $stringmanager->get_string('decline', 'corplms_plan', null, $managerlang) . ' ' . $stringmanager->get_string('plan', 'corplms_plan', null, $managerlang);
            $event->rejecttext =   $stringmanager->get_string('declineplantext', 'corplms_plan', null, $managerlang);
            $event->infobutton =   $stringmanager->get_string('review', 'corplms_plan', null, $managerlang) . ' ' .  $stringmanager->get_string('plan', 'corplms_plan', null, $managerlang);
            $event->infotext =     $stringmanager->get_string('reviewplantext', 'corplms_plan', null, $managerlang);
            $event->data = $data;
    ...
    public function send_manager_item_approval_request($unapproved) {
        ...
        if (!$manager || !$learner) {
            print_error('error:couldnotloadusers', 'corplms_plan');
            die();
        }
        ...
        foreach ($unapproved as $component => $items) {
            ...
            if ($items) {
                ...
                $message_data[] = count($items).' '. $stringmanager->get_string($comp->component, 'corplms_plan', null, $manager->lang);
            }
        }
        ...
        //ensure the message is actually coming from $learner, default to support
        $event->userfrom = ($USER->id == $learner->id) ? $learner : core_user::get_support_user();
        $event->contexturl = "{$CFG->wwwroot}/corplms/plan/approve.php?id={$this->id}";
        $event->contexturlname = $this->name;
        $event->icon = 'learningplan-request';
        ...
        $event->subject = $stringmanager->get_string('item-request-manager-short', 'corplms_plan', $a, $manager->lang);
        $event->fullmessage = $stringmanager->get_string('item-request-manager-long', 'corplms_plan', $a, $manager->lang);
        unset($event->acceptbutton);
        unset($event->onaccept);
        unset($event->rejectbutton);
        unset($event->onreject);
        $event->infobutton = $stringmanager->get_string('review', 'corplms_plan', null, $manager->lang).' '.$stringmanager->get_string('items', 'corplms_plan', null, $manager->lang);
        $event->infotext = $stringmanager->get_string('reviewitemstext', 'corplms_plan', null, $manager->lang);
        $event->data = $data;
     ...
     * @param string $icon filename of icon (in theme/corplms/pix/msgicons/)
     * @param string $subjectstring lang string in corplms_plan
     * @param string $fullmessagestring lang string in corplms_plan
     * @return boolean
     */
    public function send_alert($tolearner, $icon, $subjectstring, $fullmessagestring) {
        ...
        if ($learner && $manager) {
            ...
            $a->learner = fullname($learner);
            $stringmanager = get_string_manager();
            $event->subject = $stringmanager->get_string($subjectstring,'corplms_plan',$a, $userto->lang);
            $event->fullmessage = $stringmanager->get_string($fullmessagestring,'corplms_plan',$a, $userto->lang);
    ...
    function send_approved_alert($reasonfordecision) {
        ...
        $event->contexturl = $CFG->wwwroot.'/corplms/plan/view.php?id='.$this->id;
        $event->subject = $stringmanager->get_string('planapproved', 'corplms_plan', $this->name, $userto->lang);
        $event->fullmessage = $stringmanager->get_string('approvedplanrequest', 'corplms_plan', $this->name, $userto->lang);

        if (!empty($reasonfordecision)) {
            $event->fullmessage .= html_writer::empty_tag('br') . html_writer::empty_tag('br');
            $event->fullmessage .= $stringmanager->get_string('reasonapprovedplanrequest', 'corplms_plan', $reasonfordecision, $userto->lang);
        }
    ...
    function send_declined_alert($reasonfordecision = '') {
        ...
        $event->contexturl = $CFG->wwwroot.'/corplms/plan/view.php?id='.$this->id;
        $event->subject = format_string($stringmanager->get_string('plandeclined', 'corplms_plan', $this->name, $userto->lang));
        $event->fullmessage = $event->subject;
        $event->fullmessage = $stringmanager->get_string('declinedplanrequest', 'corplms_plan', $this->name, $userto->lang);

        if (!empty($reasonfordecision)) {
            $event->fullmessage .= html_writer::empty_tag('br') . html_writer::empty_tag('br');
            $event->fullmessage .= $stringmanager->get_string('reasondeclinedplanrequest', 'corplms_plan', $reasonfordecision, $userto->lang);
        }
    ...
    function send_completion_alert() {
        ...
        if ($manager && $manager->id != $USER->id) {
            ...
            $a->learner = fullname($learner);
            $a->plan = $this->name;
            $event->subject = $stringmanager->get_string('plan-complete-manager-short','corplms_plan',$a, $manager->lang);
            $event->fullmessage = $stringmanager->get_string('plan-complete-manager-long','corplms_plan',$a, $manager->lang);
            tm_alert_send($event);
        }
        ...
        $event->icon = 'learningplan-complete';
        $event->contexturl = $CFG->wwwroot.'/corplms/plan/view.php?id='.$this->id;
        $msg = $stringmanager->get_string('plancompletesuccess', 'corplms_plan', $this->name, $learner->lang);
        $event->subject = $msg;
    ...
    public function get_display_url(){
        ...
        if ($DB->record_exists('dp_plan', array('id' => $this->id))) {
            return "{$CFG->wwwroot}/corplms/plan/view.php?id={$this->id}";
        } else {
            //If plan doesnt exist show plan index page for that user
            return "{$CFG->wwwroot}/corplms/plan/index.php?userid={$this->userid}";
        }
    }
    ...
    public function display_tabs($currenttab) {
        ...
        $row[] = new tabobject(
                'plan',
                "{$CFG->wwwroot}/corplms/plan/view.php?id={$this->id}",
                get_string('overview', 'corplms_plan')
        );
        ...
        if ($components) {
            foreach ($components as $component) {
                ...
                $row[] = new tabobject(
                    $component->component,
                    $component->get_url(),
                    $componentname = get_string("{$component->component}plural", 'corplms_plan')
                );
            }
        }
        ...
        if ($pitems = $this->num_pendingitems()) {
            $row[] = new tabobject(
                'pendingitems',
                "{$CFG->wwwroot}/corplms/plan/approve.php?id={$this->id}",
                get_string('pendingitems', 'corplms_plan').' ('.$pitems.')'
            );
        }
    ...
    public function print_header($currenttab, $navlinks = array(), $printinstructions=true) {
        global $CFG;
        require("{$CFG->dirroot}/corplms/plan/header.php");
    }
