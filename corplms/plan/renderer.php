...
/**
* Standard HTML output renderer for corplms_plan module
*/
class corplms_plan_renderer extends plugin_renderer_base {
     ...
     */
    public function corplms_print_approval_form($requested_items, $require_approval) {
        $output = '';
        ...
        foreach ($require_approval as $componentname => $component) {
            ...
            $output .= $this->output->heading(get_string($component->component.'plural', 'corplms_plan'));

            $output .= $component->display_approval_list($requested_items[$componentname]);
        }

        $output .= html_writer::empty_tag('br');
        $submit_attributes = array('type' => 'submit', 'name' => 'submitbutton', 'value' => get_string('updatesettings', 'corplms_plan'));
    ...
    */
    function print_add_plan_button($userid) {
        $action = new moodle_url('/corplms/plan/add.php', array('userid' => $userid));
        $title = get_string('createnewlearningplan', 'corplms_plan');
