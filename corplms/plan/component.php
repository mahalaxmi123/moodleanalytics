...
if (!in_array($componentname, array_keys($plan->get_components()))) {
    print_error('error:invalidcomponent', 'corplms_plan');
}
...
$PAGE->navbar->add(get_string($component->component.'plural', 'corplms_plan'));
$PAGE->set_context($systemcontext);
$PAGE->set_url(new moodle_url('/corplms/plan/component.php', array('id' => $id, 'c' => $componentname)));
...
if ($component->can_update_settings(false)) {
    ...
    $button = html_writer::empty_tag('input', array('type' => "submit", 'name' => "submitbutton", 'value' => get_string('updatesettings', 'corplms_plan')));
