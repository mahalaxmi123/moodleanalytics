...
if ($currentcomponent == 'competency') {
    local_js();
    $PAGE->requires->js('/corplms/plan/components/competency/competency.settings.js');
}
...
if ($fromform = $mform->get_data()) {
    ...
    if ($component == 'plan') {
        ...
        if (!is_readable($formfile)) {
            ...
            throw new PlanException(get_string('noclassfileforcomponent', 'corplms_plan', $string_properties));
        }
        ...
        if (!class_exists($class)) {
            ...
            $string_properties->component = $component;
            throw new PlanException(get_string('noclassforcomponent', 'corplms_plan', $string_properties));
        }
    }
    if ($templateinuse) {
    ...
    redirect(new moodle_url('/corplms/plan/template/advancedworkflow.php', array('id' => $id, 'component' => $component)));
}
...
echo $OUTPUT->single_button(new moodle_url('/corplms/plan/template/workflow.php', array('id' => $id)), get_string('simpleworkflow', 'corplms_plan'), 'get');
