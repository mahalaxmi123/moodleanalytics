...
$PAGE->navbar->add(get_string('templateinstances', 'corplms_plan'));

echo $OUTPUT->header();

echo $OUTPUT->heading(get_string('templateinstances', 'corplms_plan'));
...
$columns[] = 'startdate';
$headers[] = get_string('startdate', 'corplms_plan');
$columns[] = 'enddate';
$headers[] = get_string('enddate', 'corplms_plan');
$columns[] = 'status';
$headers[] = get_string('status', 'corplms_plan');

$table = new flexible_table('Template_instances');
$table->define_columns($columns);
$table->define_headers($headers);
$table->define_baseurl(new moodle_url('/corplms/plan/template/templateinstances.php', array('id' => $id)));
...
foreach ($instances as $instance) {
    $tablerow = array();
    $tablerow[] = $OUTPUT->action_link(new moodle_url('/corplms/plan/view.php', array('id' => $instance->id)), $instance->name);
    ...
    case DP_PLAN_STATUS_COMPLETE:
        $status = get_string('complete', 'corplms_plan');
        break;
    }
    $tablerow[] = $status;
