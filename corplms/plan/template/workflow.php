...
if ($confirm) {
    $workflow = $confirm;
    $returnurl = $CFG->wwwroot . '/corplms/plan/template/workflow.php?id=' . $id;

    $classfile = $CFG->dirroot .
        "/corplms/plan/workflows/{$workflow}/{$workflow}.class.php";
    ...
    if (!$wf->copy_to_db($template->id)) {
        corplms_set_notification(get_string('error:update_workflow_settings', 'corplms_plan'), $returnurl);
    }
    ...
    corplms_set_notification(get_string('update_workflow_settings', 'corplms_plan'), $returnurl, array('class' => 'notifysuccess'));
...
if ($mform->is_cancelled()) {
    // user cancelled form
    redirect($CFG->wwwroot . '/corplms/plan/template/workflow.php?id=' . $id);
}
else if ($mform->no_submit_button_pressed()) {
    // user pressed advanced options button
    redirect($CFG->wwwroot . '/corplms/plan/template/advancedworkflow.php?id='.$id);
}

if ($fromform = $mform->get_data()) {
    $workflow = $fromform->workflow;
    $returnurl = $CFG->wwwroot . '/corplms/plan/template/workflow.php?id=' . $id;
    $changeurl = $CFG->wwwroot . '/corplms/plan/template/workflow.php?id=' . $id . '&amp;confirm=' . $workflow;
    if ($workflow != 'custom') {
        ...
        if ($template->workflow != $workflow) {
            ...
            if (!$diff) {
                $differences = html_writer::tag('p', get_string('nochanges', 'corplms_plan'));
            } else {
            ...
            $scales_locked = '';
            if ($template_in_use) {
                $scales_locked = html_writer::tag('p', html_writer::tag('b', get_string('scaleslocked', 'corplms_plan')));
            }

            $changeworkflowconfirm = get_string('changeworkflowconfirm', 'corplms_plan', get_string($fromform->workflow.'workflowname', 'corplms_plan')) . $scales_locked . $differences;
