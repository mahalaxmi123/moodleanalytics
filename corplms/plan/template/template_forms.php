...
class dp_template_general_settings_form extends moodleform {
    ...
    function definition() {
        ...
        $mform->addElement('header', 'generalsettings', get_string('generalsettings', 'corplms_plan'));

        $mform->addElement('text', 'templatename', get_string('name', 'corplms_plan'), 'maxlength="255"');
        $mform->setType('templatename', PARAM_TEXT);
        $mform->setDefault('templatename', $templatename);
        $mform->addRule('templatename', null, 'required');

        $mform->addElement('date_selector', 'enddate', get_string('enddate', 'corplms_plan'));
        $mform->setType('enddate', PARAM_TEXT);
        $mform->setDefault('enddate', $template->enddate);
        $mform->addRule('enddate', null, 'required');
        $mform->addHelpButton('enddate', 'templateenddate', 'corplms_plan', '', true);
...
class dp_template_new_form extends moodleform {
    function definition() {
        ...
        $mform->addElement('header', 'newtemplate', get_string('newtemplate', 'corplms_plan'));
...
class dp_template_workflow_form extends moodleform {
    function definition() {
        ...
        $mform->addElement('header', 'workflowsettings', get_string('workflowsettings', 'corplms_plan'));
        ...
        foreach ($DP_AVAILABLE_WORKFLOWS as $workflow) {
            ...
            if (!is_readable($classfile)) {
                ...
                throw new PlanException(get_string('noclassfileforworkflow', 'corplms_plan', $string_parameters));
            }
            ...
            if (!class_exists($classname)) {
                ...
                $string_parameters->workflow = $workflow;
                throw new PlanException(get_string('noclassforworkflow', 'corplms_plan', $string_parameters));
            }
            $wf = new $classname();
        ...
        $radiogroup[] =& $mform->createElement('radio', 'workflow', '', html_writer::tag('b', get_string('customworkflowname', 'corplms_plan')) . html_writer::empty_tag('br') . get_string('customworkflowdesc', 'corplms_plan'), 'custom');
        ...
        $mform->addElement('submit', 'advancedsubmitbutton', get_string('advancedworkflow', 'corplms_plan'));
...
class dp_template_advanced_workflow_form extends moodleform {
    function definition() {
        ...
        if ($component == 'plan') {
            ...
            require_once("{$CFG->dirroot}/corplms/plan/settings_form.php");
        } else {
            // Include each components form file
            // Component path
            $cpath = "{$CFG->dirroot}/corplms/plan/components/{$component}";
            ...
            if (!is_readable($formfile)) {
                ...
                throw new PlanException(get_string('noclassfileforcomponent', 'corplms_plan', $string_parameters));
            }
            ...
            if (!class_exists($class)) {
                ...
                $string_parameters->component = $component;
                throw new PlanException(get_string('noclassforcomponent', 'corplms_plan', $string_parameters));
            }
        }
