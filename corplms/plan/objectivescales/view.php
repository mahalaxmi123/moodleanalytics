...
require_capability('corplms/plan:manageobjectivescales', $context);

if (!$objective = $DB->get_record('dp_objective_scale', array('id' => $id))) {
    print_error('error:objectivescaleidincorrect', 'corplms_plan');
}
...
if ($delete) {
    if (!$value = $DB->get_record('dp_objective_scale_value', array('id' => $delete))) {
        print_error('error:invalidobjectivescalevalueid', 'corplms_plan');
    }
    if ($scale_used) {
        print_error('error:nodeleteobjectivescalevalueinuse', 'corplms_plan');
    }

    if ($value->id == $objective->defaultid) {
        print_error('error:nodeleteobjectivescalevaluedefault', 'corplms_plan');
    }
    ...
    if ($confirm) {
        ...
        corplms_set_notification(get_string('deletedobjectivescalevalue', 'corplms_plan', format_string($objective->name)), $CFG->wwwroot.'/corplms/plan/objectivescales/view.php?id='.$objective->id, array('class' => 'notifysuccess'));

    } else {
        $returnurl = new moodle_url('/corplms/plan/objectivescales/view.php', array('id' => $objective->id));
        $deleteurl = new moodle_url('/corplms/plan/objectivescales/view.php', array('id' => $objective->id, 'delete' => $delete, 'confirm' => '1', 'sesskey' => sesskey()));

        echo $OUTPUT->header();
        $strdelete = get_string('deletecheckobjectivevalue', 'corplms_plan');
...
if ((!empty($moveup) or !empty($movedown))) {
    ...
    if ($scale_used) {
        $returnurl = "{$CFG->wwwroot}/corplms/plan/objectivescales/view.php?id={$objective->id}";
        print_error('error:noreorderobjectiveinuse', 'corplms_plan', $returnurl);
    }
...
if ($default) {
    ...
    corplms_set_notification(get_string('objectivescaledefaultupdated', 'corplms_plan'), null, array('class' => 'notifysuccess'));
...
echo $OUTPUT->single_button('/corplms/plan/objectivescales/index.php', get_string('allobjectivescales', 'corplms_plan'), 'get');

// Display info about scale
echo $OUTPUT->heading(get_string('objectivescalex', 'corplms_plan', format_string($objective->name)));
$objective->description = file_rewrite_pluginfile_urls($objective->description, 'pluginfile.php', $context->id, 'corplms_plan', 'dp_objective_scale', $objective->id);
echo html_writer::tag('p', format_text($objective->description, FORMAT_HTML));

// Display warning if scale is in use
if ($scale_used) {
    echo $OUTPUT->container(get_string('objectivescaleinuse', 'corplms_plan'), 'notifymessage');
}
...
if (isset($max_achieved) && isset($min_notachieved) && $max_achieved > $min_notachieved) {
    echo $OUTPUT->container(get_string('nonsensicalachievedvalues', 'corplms_plan'), 'notifyproblem');
}

// Display objective scale values
if ($values) {
    echo html_writer::start_tag('form', array('id' => "objscaleupdateform", 'action' => new moodle_url('/corplms/plan/objectivescales/view.php', array('id' => $id)), 'method' => "POST"));
    ...
    $table->head[] = get_string('defaultvalue', 'corplms_plan').' '.
        $OUTPUT->help_icon('objectivescaledefault', 'corplms_plan', false);
    $table->align[] = 'center';

    $table->head[] = get_string('achieved', 'corplms_plan').' '.
        $OUTPUT->help_icon('objectivescalevalueachieved', 'corplms_plan', false);
    $table->align[] = 'center';
    ...
    foreach ($values as $value) {
        ...
        $buttons[] = $OUTPUT->action_icon(new moodle_url('/corplms/plan/objectivescales/editvalue.php', array('id' => $value->id)), new pix_icon('t/edit', $str_edit));

        if (!$scale_used) {
            if ($value->id == $objective->defaultid) {
                $buttons[] = $OUTPUT->pix_icon('t/delete_grey', get_string('error:nodeleteobjectivescalevaluedefault', 'corplms_plan'), 'corplms_core',
                    array('class' => 'iconsmall action-icon', 'title' => get_string('error:nodeleteobjectivescalevaluedefault', 'corplms_plan')));
            } else {
                $buttons[] = $OUTPUT->action_icon(new moodle_url('/corplms/plan/objectivescales/view.php', array('id' => $objective->id, 'delete' => $value->id)), new pix_icon('t/delete', $str_delete));
            }
        } else {
            $buttons[] = $OUTPUT->pix_icon('t/delete_grey', get_string('error:nodeleteobjectivescalevalueinuse', 'corplms_plan'), 'corplms_core',
                array('class' => 'iconsmall action-icon', 'title' => get_string('error:nodeleteobjectivescalevalueinuse', 'corplms_plan')));
        }

        // If value can be moved up
        if ($count > 1 && !$scale_used) {
            $buttons[] = $OUTPUT->action_icon(new moodle_url('/corplms/plan/objectivescales/view.php', array('id' => $objective->id, 'moveup' => $value->id)), new pix_icon('t/up', $str_moveup));
        } else {
            $buttons[] = $spacer;
        }
        ...
        if ($count < $numvalues && !$scale_used) {
            $buttons[] = $OUTPUT->action_icon(new moodle_url('/corplms/plan/objectivescales/view.php', array('id' => $objective->id, 'movedown' => $value->id)), new pix_icon('t/down', $str_movedown));
        } else {
            $buttons[] = $spacer;
        }
    ...
    echo html_writer::empty_tag('br') . $OUTPUT->container(get_string('noobjectivescalevalues', 'corplms_plan')) . html_writer::empty_tag('br');
...
if (!$scale_used) {
    ...
    $button = $OUTPUT->single_button(new moodle_url('/corplms/plan/objectivescales/editvalue.php', $options), get_string('addnewobjectivevalue', 'corplms_plan'), 'get');
}
