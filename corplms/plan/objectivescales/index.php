...
if ($delete) {
    if (!$scale = $DB->get_record('dp_objective_scale', array('id' => $delete))) {
        print_error('error:invalidobjectivescaleid', 'corplms_plan');
    }
    if (dp_objective_scale_is_used($delete)) {
        print_error('error:nodeleteobjectivescaleinuse', 'corplms_plan');
    }
    if (dp_objective_scale_is_assigned($delete)) {
        print_error('error:nodeleteobjectivescaleassigned', 'corplms_plan');
    }
    ...
    if ($confirm) {
        ...
        corplms_set_notification(get_string('deletedobjectivescale', 'corplms_plan', format_string($scale->name)), $CFG->wwwroot.'/corplms/plan/objectivescales/index.php', array('class' => 'notifysuccess'));

    } else {
        $returnurl = new moodle_url('/corplms/plan/objectivescales/index.php');
        $deleteurl = new moodle_url('/corplms/plan/objectivescales/index.php', array('delete' => $delete, 'confirm' => '1', 'sesskey' => sesskey()));

        echo $OUTPUT->header();
        $strdelete = get_string('deletecheckobjective', 'corplms_plan');
