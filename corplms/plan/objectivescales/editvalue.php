...
if (!$id && !$objectivescaleid) {
    print_error('error:incorrectparameters', 'corplms_plan');
}
...
if ($id == 0) {
    ...
    if (!$value = $DB->get_record('dp_objective_scale_value', array('id' => $id))) {
        print_error('error:objectivescalevalueidincorrect', 'corplms_plan');
    }
}
...
if ($value->id == 0 && $scale_used) {
    print_error('error:cannotaddscalevalue', 'corplms_plan');
}
...
$value = file_prepare_standard_editor($value, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
                                      'corplms_plan', 'dp_objective_scale_value', $value->id);
...
if ($valueform->is_cancelled()) {
    ...
    redirect("$CFG->wwwroot/corplms/plan/objectivescales/view.php?id={$value->objscaleid}");
...
// Update data
    ...
    if ($valuenew->id == 0) {
        ...
        $notification = get_string('objectivescalevalueadded', 'corplms_plan', format_string($valuenew->name));
    } else {
       ...
       $notification = get_string('objectivescalevalueupdated', 'corplms_plan', format_string($valuenew->name));
    }
    $valuenew = file_postupdate_standard_editor($valuenew, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'corplms_plan', 'dp_objective_scale_value', $valuenew->id);
    $DB->set_field('dp_objective_scale_value', 'description', $valuenew->description, array('id' => $valuenew->id));
    corplms_set_notification($notification, "$CFG->wwwroot/corplms/plan/objectivescales/view.php?id={$valuenew->objscaleid}", array('class' => 'notifysuccess'));
}
...
if ($id == 0) {
    echo $OUTPUT->heading(get_string('addnewobjectivevalue', 'corplms_plan'));
} else {
    echo $OUTPUT->heading(get_string('editobjectivevalue', 'corplms_plan'));
}
