...
if ($id == 0) {
    ...
    if (!$objective = $DB->get_record('dp_objective_scale', array('id' => $id))) {
        print_error('error:objectivescaledidincorrect', 'corplms_plan');
    }
}
...
$objective = file_prepare_standard_editor($objective, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
                                         'corplms_plan', 'dp_objective_scale', $objective->id);
...
if ($mform->is_cancelled()) {
    ...
    redirect("$CFG->wwwroot/corplms/plan/objectivescales/index.php");
...
// Update data
    ...
    if (empty($objectivenew->id)) {
        ...
        $notification = get_string('objectivescaleadded', 'corplms_plan', format_string($objectivenew->name));
        $transaction->allow_commit();
    } else {
        // Existing objective
        $DB->update_record('dp_objective_scale', $objectivenew);
        add_to_log(SITEID, 'objectivescales', 'updated', "view.php?id=$objectivenew->id");
        $notification = get_string('objectivescaleupdated', 'corplms_plan', format_string($objectivenew->name));
    }

    //update description
    $objectivenew = file_postupdate_standard_editor($objectivenew, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'corplms_plan', 'dp_objective_scale', $objectivenew->id);
    ...
    corplms_set_notification($notification,
        "$CFG->wwwroot/corplms/plan/objectivescales/view.php?id={$objectivenew->id}",
        array('class' => 'notifysuccess'));
}

/// Print Page
$PAGE->navbar->add(get_string("objectivescales", 'corplms_plan'), new moodle_url('/corplms/plan/objectivescales/index.php'));
if ($id == 0) { // Add
    $PAGE->navbar->add(get_string('objectivesscalecreate', 'corplms_plan'));
    $heading = get_string('objectivesscalecreate', 'corplms_plan');
} else {    //Edit
    $PAGE->navbar->add(get_string('editobjective', 'corplms_plan', format_string($objective->name)));
    $heading = get_string('editobjective', 'corplms_plan', format_string($objective->name));
}
