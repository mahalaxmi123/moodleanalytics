...
if (has_capability('corplms/plan:canselectplantemplate', context_system::instance())) {
...
// Display plan list
//
$PAGE->set_url('/corplms/plan/index.php');
...
$heading = get_string('learningplans', 'corplms_plan');
$pagetitle = get_string('learningplans', 'corplms_plan');
...
if ($planuser == $USER->id) {
    $planinstructions = get_string('planinstructions', 'corplms_plan') . ' ';
    add_to_log(SITEID, 'plan', 'view all', "index.php?userid={$planuser}");
} else {
    $user = $DB->get_record('user', array('id' => $planuser));
    $userfullname = fullname($user);
    $planinstructions = get_string('planinstructionsuser', 'corplms_plan', $userfullname) . ' ';
    add_to_log(SITEID, 'plan', 'view all', "index.php?userid={$planuser}", $userfullname);
}
if ($canaddplan) {
    $planinstructions .= get_string('planinstructions_add', 'corplms_plan');
}

echo $OUTPUT->container($planinstructions, 'instructional_text');;

if ($canaddplan) {
    $renderer = $PAGE->get_renderer('corplms_plan');
...
echo dp_display_plans($planuser, array(DP_PLAN_STATUS_APPROVED), array('enddate', 'status'), get_string('activeplans', 'corplms_plan'));
echo $OUTPUT->container_end();
...
echo dp_display_plans($planuser, array(DP_PLAN_STATUS_UNAPPROVED, DP_PLAN_STATUS_PENDING),
    array('status'), get_string('unapprovedplans', 'corplms_plan'));
echo $OUTPUT->container_end();

echo $OUTPUT->container_start('', 'dp-plans-list-completed-plans');
echo dp_display_plans($planuser, DP_PLAN_STATUS_COMPLETE, array('completed'), get_string('completedplans', 'corplms_plan'));
echo $OUTPUT->container_end();
