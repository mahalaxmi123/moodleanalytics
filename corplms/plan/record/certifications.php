...
if (!empty($certifid) && (!$certification = $DB->get_record('prog', array('certifid' => $certifid), 'fullname'))) {
    print_error(get_string('error:incorrectcertifid', 'corplms_certification', null, $certifid));
}

$context = context_system::instance();

$PAGE->set_context($context);
$PAGE->set_url(new moodle_url('/corplms/plan/record/certifications.php', $pageparams));
...
if ($history) {
    if (!empty($certifid)) {
        $strsubheading = get_string('certificationshistoryforsubhead', 'corplms_plan', $certification->fullname);
    } else {
        $strsubheading = get_string('certificationshistorysubhead', 'corplms_plan');
    }
    $shortname = 'plan_certifications_history';
    $data['certifid'] = $certifid;
} else {
    $strsubheading = get_string($rolstatus.'certificationssubhead', 'corplms_plan');
