...
if (!empty($programid) && (!$program = $DB->get_record('prog', array('id' => $programid), 'fullname'))) {
    print_error(get_string('programnotfound', 'corplms_plan'));
}

$context = context_system::instance();

$PAGE->set_context($context);
$PAGE->set_url(new moodle_url('/corplms/plan/record/programs.php', $pageparams));
...
$strsubheading = get_string($rolstatus.'programssubhead', 'corplms_plan');
...
if ($history) {
    ...
    if (!empty($programid)) {
        $strsubheading = get_string('programscompletionhistoryforsubhead', 'corplms_plan', $program->fullname);
    } else {
        $strsubheading = get_string('programscompletionhistorysubhead', 'corplms_plan');
    }
}
