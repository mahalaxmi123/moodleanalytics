...
function evidence_display_attachment($userid, $evidenceid) {
    ...
    $fs = get_file_storage();
    $files = $fs->get_area_files($context->id, 'corplms_plan', 'attachment', $evidenceid, null, FALSE);

    if (!empty($files)) {
        foreach ($files as $file) {
            $filename = $file->get_filename();
            $path = '/'.$file->get_contextid().'/corplms_plan/attachment'.$file->get_filepath().$file->get_itemid().'/'.$filename;
...
function display_evidence_detail($evidenceid, $delete = false) {
    ...
    if (!$item = $DB->get_record_sql($sql, array($evidenceid))) {
        return get_string('error:evidencenotfound', 'corplms_plan');
    }
    ...
    if (($USER->id == $item->userid && has_capability('corplms/plan:editownsiteevidence', $usercontext) ||
            $USER->id != $item->userid && has_capability('corplms/plan:editsiteevidence', $usercontext)) && !$delete) {
        // Can edit
        $buttonlabel = get_string('editdetails', 'corplms_plan');
        $editurl = new moodle_url('/corplms/plan/record/evidence/edit.php',
    ...
    if (!empty($item->description)) {
        $item->description = file_rewrite_pluginfile_urls($item->description, 'pluginfile.php', context_system::instance()->id, 'corplms_plan', 'dp_plan_evidence', $item->id);
        $out .= html_writer::tag('p', get_string('description') . ' : ' . format_text($item->description, FORMAT_HTML));
    }
    if (!empty($item->evidencetypename)) {
        $out .=  html_writer::tag('p', get_string('evidencetype', 'corplms_plan') . ' : ' . $item->evidencetypename);
    }
    if (!empty($item->institution)) {
        $out .=  html_writer::tag('p', get_string('evidenceinstitution', 'corplms_plan') . ' : ' . $item->institution);
    }
    if (!empty($item->datecompleted)) {
        $out .=  html_writer::tag('p', get_string('evidencedatecompleted', 'corplms_plan') . ' : ' . userdate($item->datecompleted, get_string('datepickerlongyearphpuserdate', 'corplms_core')));
    }
    if (!empty($item->evidencelink)) {
        $evidencelink = $OUTPUT->action_link(new moodle_url($item->evidencelink), $item->evidencelink);
        $out .=  html_writer::tag('p', get_string('evidencelink', 'corplms_plan') . ' : ' . $evidencelink);
    }

    $attachments = evidence_display_attachment($item->userid, $evidenceid);
    if (!empty($attachments)) {
        $out .= $OUTPUT->heading(get_string('attachment', 'corplms_plan'), 4);
...
function list_evidence_in_use($evidenceid) {
    ...
    if ($items = $DB->get_records_sql($sql, array($evidenceid, $evidenceid, $evidenceid, $evidenceid))) {
        $out .= $OUTPUT->heading(get_string('evidenceinuseby', 'corplms_plan'), 4);

        $tableheaders = array(
            get_string('planname', 'corplms_plan'),
            get_string('component', 'corplms_plan'),
        ...
        $url = new moodle_url('/corplms/plan/record/evidence/index.php');
        ...
        foreach ($items as $item) {
            $row = array();
            $row[] = $item->planname;
            $row[] = get_string($item->component, 'corplms_plan');
            $row[] = $item->itemname;
            $table->add_data($row);
        }
