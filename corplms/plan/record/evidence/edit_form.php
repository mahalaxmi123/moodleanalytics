...
class plan_evidence_edit_form extends moodleform {
    ...
    function definition() {
        ...
        $mform->addElement('text', 'name', get_string('evidencename', 'corplms_plan'));
        ...
        if ($selectoptions) {
            ...
            $mform->addElement('select', 'evidencetypeid', get_string('evidencetype', 'corplms_plan'), $selector + $selectoptions);
            ...
            $mform->addElement('static', 'evidencetypeiderror',
                    get_string('evidencetype', 'corplms_plan'), get_string('noevidencetypesdefined', 'corplms_plan'));
        }

        $mform->addElement('text', 'evidencelink', get_string('evidencelink', 'corplms_plan'));
        $mform->setType('evidencelink', PARAM_URL);

        $mform->addElement('text', 'institution', get_string('evidenceinstitution', 'corplms_plan'));
        $mform->setType('institution', PARAM_TEXT);

        $mform->addElement('date_selector', 'datecompleted',
                get_string('evidencedatecompleted', 'corplms_plan'), array('optional' => true));

        $mform->addElement('filemanager', 'attachment_filemanager',
                get_string('attachment', 'corplms_plan'), null, $fileoptions);

        $this->add_action_buttons(true, empty($this->_customdata['id']) ?
                get_string('addevidence', 'corplms_plan') : get_string('updateevidence', 'corplms_plan'));
    }
    ...
    function validation($data, $files) {
        ...
        if (!empty($evidencelink)) {
            ...
            if (empty($evidencelinktotest)) {
                $errors['evidencelink'] = get_string('evidencelinkerror', 'corplms_plan');
            }
        }
