...
require_once($CFG->dirroot.'/corplms/plan/lib.php'); // Is this needed?
...
$PAGE->set_url('/corplms/plan/record/evidence/index.php', array('userid' => $userid, 'format' => $format, 'status' => $rolstatus));
...
// Display the page.
$strsubheading = get_string('allevidence', 'corplms_plan');
$PAGE->navbar->add(get_string($menunavitem, 'corplms_core'), $url);
...
print $OUTPUT->single_button(
        new moodle_url("/corplms/plan/record/evidence/edit.php",
                array('id' => 0, 'userid' => $userid)), get_string('addevidence', 'corplms_plan'), 'get');
