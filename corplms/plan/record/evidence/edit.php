...
$PAGE->set_url('/corplms/plan/record/evidence/edit.php',
...
$canaccess = has_capability('corplms/plan:accessanyplan', $systemcontext);
$canedit = has_capability('corplms/plan:editsiteevidence', $systemcontext);

if (!empty($evidenceid)) {
    // Editing or deleting, check record exists
    if (!$item = $DB->get_record('dp_plan_evidence', array('id' => $evidenceid))) {
        print_error('error:evidenceidincorrect', 'corplms_plan');
    } else {
        // Check if its readonly
        if ($item->readonly && !($canaccess || $canedit)) {
            print_error('evidence_readonly', 'corplms_plan');
        }
...
if ($USER->id != $userid && !corplms_is_manager($userid) && !($canaccess || $canedit)) {
    print_error('error:cannotviewpage', 'corplms_plan');
}
...
$indexurl = new moodle_url('/corplms/plan/record/evidence/index.php', array('userid' => $userid));
$backlink = html_writer::tag('p', $OUTPUT->action_link($indexurl,
        get_string('backtoallx', 'corplms_plan', get_string("evidenceplural", 'corplms_plan'))));
...
if (!empty($evidenceid) || $deleteflag) {
    ...
    $itemurl = new moodle_url('/corplms/plan/record/evidence/view.php', array('id' => $evidenceid));
} else {
...
if ($deleteflag && $deleteconfirmed) {
    ...
    if (!$result) {
        print_error('error:evidencedeleted', 'corplms_plan');
    } else {
        $fs = get_file_storage();
        $fs->delete_area_files($TEXTAREA_OPTIONS['context']->id, 'corplms_plan', 'attachment', $item->id);

        add_to_log(SITEID, 'plan', 'deleted evidence',
                new moodle_url('/corplms/plan/record/evidence/edit.php', array('id' => $item->id)),
                "{$item->name} (ID:{$item->id})");
        corplms_set_notification(get_string('evidencedeleted', 'corplms_plan'),
...
$item = file_prepare_standard_editor($item, 'description',
        $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'corplms_plan', 'dp_plan_evidence', $item->id);

$fileoptions = $FILEPICKER_OPTIONS;
$fileoptions['maxfiles'] = 10;

$item = file_prepare_standard_filemanager($item, 'attachment',
        $fileoptions, $FILEPICKER_OPTIONS['context'], 'corplms_plan', 'attachment', $item->id);
...
if ($data = $mform->get_data()) {
    ...
    $data = file_postupdate_standard_editor($data, 'description',
            $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'corplms_plan', 'dp_plan_evidence', $data->id);

    // process files, update the data record
    $data = file_postupdate_standard_filemanager($data, 'attachment',
            $fileoptions, $FILEPICKER_OPTIONS['context'], 'corplms_plan', 'attachment', $data->id);

    $DB->update_record('dp_plan_evidence', $data);

    add_to_log(SITEID, 'plan', "{$result} evidence",
            new moodle_url('/corplms/plan/record/evidence/edit.php', array('id' => $data->id)),
            "{$item->name} (ID:{$data->id})");
    corplms_set_notification(get_string('evidence' . $result, 'corplms_plan'), $itemurl, array('class' => 'notifysuccess'));
...
$PAGE->navbar->add(get_string('allevidence', 'corplms_plan'), new moodle_url('/corplms/plan/record/evidence/index.php', array('userid' => $userid)));
$PAGE->navbar->add(get_string($action . 'evidence', 'corplms_plan'));
...
switch($action){
    case 'add':
    case 'edit':
        echo $OUTPUT->heading(get_string($action . 'evidence', 'corplms_plan'));
        echo $backlink;
        $mform->display();
        break;
    ...
    case 'delete':
        ...
        $deleteurl = new moodle_url('/corplms/plan/record/evidence/edit.php', $params);
        echo list_evidence_in_use($item->id);
        echo $OUTPUT->confirm(get_string('deleteevidenceareyousure', 'corplms_plan'), $deleteurl, $indexurl);
        break;
}
