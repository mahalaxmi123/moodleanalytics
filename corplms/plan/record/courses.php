...
if (!empty($courseid) && (!$course = $DB->get_record('course', array('id' => $courseid), 'fullname'))) {
    print_error(get_string('coursenotfound', 'corplms_plan'));
}

$context = context_system::instance();

$PAGE->set_context($context);
$PAGE->set_url(new moodle_url('/corplms/plan/record/courses.php', $pageparams));
...
$strsubheading = get_string($rolstatus.'coursessubhead', 'corplms_plan');
...
if ($history) {
    ...
    if (!empty($courseid)) {
        $strsubheading = get_string('coursescompletionhistoryforsubhead', 'corplms_plan', $course->fullname);
    } else {
        $strsubheading = get_string('coursescompletionhistorysubhead', 'corplms_plan');
    }
}
