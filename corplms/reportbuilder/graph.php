...
$PAGE->requires->yui_module('moodle-corplms_reportbuilder-graphicalreporting', 'M.reportbuilder.graphicalreport.init');

$id = required_param('reportid', PARAM_INT);

admin_externalpage_setup('rbmanagereports');

/** @var corplms_reportbuilder_renderer|core_renderer $output */
$output = $PAGE->get_renderer('corplms_reportbuilder');

$returnurl = new moodle_url('/corplms/reportbuilder/graph.php', array('reportid' => $id));
...
if ($fromform = $mform->get_data()) {
    ...
    corplms_set_notification(get_string('graph_updated', 'corplms_reportbuilder'), $returnurl, array('class' => 'notifysuccess'));
}
