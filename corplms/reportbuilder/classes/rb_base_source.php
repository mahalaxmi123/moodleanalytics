 ...
 */
    public function __construct() {
        ...
        $this->usedcomponents[] = 'corplms_reportbuilder';
        ...
        foreach ($properties as $property) {
            if (!property_exists($this, $property)) {
                ...
                throw new ReportBuilderException(get_string('error:propertyxmustbesetiny', 'corplms_reportbuilder', $a));
            }
        }
        ...
        foreach ($this->columnoptions as $columnoption) {
            if (isset($columnoption->extrafields) && is_array($columnoption->extrafields)) {
                foreach ($columnoption->extrafields as $extrakey => $extravalue) {
                    if ($extrakey == 'id') {
                        throw new ReportBuilderException(get_string('error:columnextranameid', 'corplms_reportbuilder', $extravalue), 101);
                    }
                }
            }
        }
    ...
    public function pre_display_actions() {
        if ($this->needsredirect && isset($this->redirecturl)) {
            if (isset($this->redirectmessage)) {
                corplms_set_notification($this->redirectmessage, $this->redirecturl, array('class' => 'notifymessage'));
            } else {
    ...
    private function validate_joinlist() {
        ...
        foreach ($joinlist as $item) {
            ...
            if (in_array($item->name, $joins_used)) {
                ...
                throw new ReportBuilderException(get_string('error:joinxusedmorethanonceiny', 'corplms_reportbuilder', $a));
            } else {
            ...
            if (in_array($item->name, $reserved_words)) {
                ...
                $a->join = $item->name;
                $a->source = get_class($this);
                throw new ReportBuilderException(get_string('error:joinxisreservediny', 'corplms_reportbuilder', $a));
            }
        }
            ...
            if (isset($item->dependencies) &&
                ...
                foreach ($item->dependencies as $dep) {
                    ...
                    if (!in_array($dep, $joins_used)) {
                        ...
                        $a->source = get_class($this);
                        $a->dependency = $dep;
                        throw new ReportBuilderException(get_string('error:joinxhasdependencyyinz', 'corplms_reportbuilder', $a));
                    }
                }
    ...
    public function new_column_from_option($type, $value, $transform, $aggregate, $heading=null, $customheading = true, $hidden=0) {
        ...
        if ($coloption =
            ...
            if (!reportbuilder::check_joins($joinlist, $coloption->joins)) {
                ...
                $a->source = get_class($this);
                throw new ReportBuilderException(get_string('error:joinsfortypexandvalueynotfoundinz', 'corplms_reportbuilder', $a));
            }
            ...
            $a->value = $value;
            $a->source = get_class($this);
            throw new ReportBuilderException(get_string('error:columnoptiontypexandvalueynotfoundinz', 'corplms_reportbuilder', $a));
        }
    }
    ...
    public function process_data_row(stdClass $row, $format, reportbuilder $report) {
        ...
        foreach ($report->columns as $column) {
            ...
            if (!property_exists($row, $field)) {
                $results[] = get_string('unknown', 'corplms_reportbuilder');
                continue;
            }
            ...
            $results[] = \corplms_reportbuilder\rb\display\legacy::display($row->$field, $format, $row, $column, $report);
        }
    ...
    function rb_display_nice_time_in_timezone($date, $row) {
        if ($date && is_numeric($date)) {
            if (empty($row->timezone)) {
                $targetTZ = corplms_get_clean_timezone();
                $tzstring = get_string('nice_time_unknown_timezone', 'corplms_reportbuilder');
            } else {
                $targetTZ = $row->timezone;
                $tzstring = get_string(strtolower($targetTZ), 'timezones');
            }
            $date = userdate($date, get_string('nice_time_in_timezone_format', 'corplms_reportbuilder'), $targetTZ) . ' ';
    ...
    function rb_display_nice_date_in_timezone($date, $row) {
        if ($date && is_numeric($date)) {
            ...
            $date = userdate($date, get_string('nice_date_in_timezone_format', 'corplms_reportbuilder'), $targetTZ) . ' ';
            return $date;
        } else {
            return '';
        }
    ...
    public function rb_display_course_expand($course, $row, $isexport = false) {
        ...
        $attr = array('class' => corplms_get_style_visibility($row, 'course_visible', 'course_audiencevisible'));
    ...
    public function rb_display_program_expand($program, $row, $isexport = false) {
        ...
        $attr = array('class' => corplms_get_style_visibility($row, 'prog_visible', 'prog_audiencevisible'));
        $alturl = new moodle_url('/corplms/program/view.php', array('id' => $row->prog_id));
    ...
    public function rb_expand_course_details() {
        ...
        if ($enrolled) {
            ...
            if ($complete) {
                ...
                $formdata['status'] = get_string('coursestatuscomplete', 'corplms_reportbuilder');
                $formdata['progress'] = get_string('coursecompletedon', 'corplms_reportbuilder', $coursecompletedon);
                if ($grade) {
                    if (!isset($grade->finalgrade)) {
                        $formdata['grade'] = '-';
                    } else {
                        $formdata['grade'] = get_string('xpercent', 'corplms_core', $grade->finalgrade);
                    }
                }
            } else {
                $formdata['status'] = get_string('coursestatusenrolled', 'corplms_reportbuilder');
                ...
                $progress = corplms_display_course_progress_icon($userid, $courseid,
            ...
            $formdata['action'] =  get_string('launchcourse', 'corplms_program');
        } else {
            $formdata['status'] = get_string('coursestatusnotenrolled', 'corplms_reportbuilder');
            ...
            if ($cansignup) {
                $formdata['enroltype'] = get_string('courseenrolavailable', 'corplms_reportbuilder');
                ...
                $formdata['action'] = get_string('viewcourse', 'corplms_program');
                $formdata['url'] = new moodle_url('/course/view.php', array('id' => $courseid));
            } else {
                $formdata['enroltype'] = $enrolmethodstr;
    ...
    private function get_inline_enrolment_elements(array $inlineenrolments) {
        ...
        foreach ($inlineenrolments as $inlineenrolment) {
            ...
            if (count($inlineenrolments) > 1) {
                $enrollabel = get_string('enrolusing', 'corplms_reportbuilder', $plugin->get_instance_name($instance->id));
            } else {
                $enrollabel = get_string('enrol', 'corplms_reportbuilder');
            }
            $name = $instance->id;
    ...
    public function rb_expand_prog_details() {
        global $CFG, $DB, $USER;
        require_once($CFG->dirroot . '/corplms/reportbuilder/report_forms.php');
        require_once($CFG->dirroot . '/corplms/program/renderer.php');
    ...
    public function get_dp_status_sql($userid, $courseid) {
        global $CFG;
        require_once($CFG->dirroot.'/corplms/plan/rb_sources/rb_source_dp_course.php');
    ...
    function rb_display_link_course_icon($course, $row, $isexport = false) {
        ...
        $cssclass = corplms_get_style_visibility($row, 'course_visible', 'course_audiencevisible');
        $icon = html_writer::empty_tag('img', array('src' => corplms_get_icon($courseid, CORPLMS_ICON_TYPE_COURSE),
    ...
    function rb_display_course_icon($icon, $row, $isexport = false) {
        ...
        $courseicon = html_writer::empty_tag('img', array('src' => corplms_get_icon($row->course_id, CORPLMS_ICON_TYPE_COURSE),
    ...
    function rb_display_course_type_icon($type, $row, $isexport = false) {
        ...
        if ($isexport) {
            switch ($type) {
                case CORPLMS_COURSE_TYPE_ELEARNING:
                    return get_string('elearning', 'rb_source_dp_course');
                case CORPLMS_COURSE_TYPE_BLENDED:
                    return get_string('blended', 'rb_source_dp_course');
                case CORPLMS_COURSE_TYPE_FACETOFACE:
        ...
        $icon = $OUTPUT->pix_icon('/msgicons/' . $image . '-regular', $alt, 'corplms_core', array('title' => $alt));
    ...
    function rb_display_link_course_category($category, $row, $isexport = false) {
        ...
        switch ($viewtype) {
            case 'program':
            case 'certification':
                $url = new moodle_url('/corplms/program/index.php', array('categoryid' => $catid, 'viewtype' => $viewtype));
                break;
            default:
    ...
    public function rb_display_planlink($planname, $row, $isexport = false) {
        ...
        $url = new moodle_url('/corplms/plan/view.php', array('id' => $row->plan_id));
    ...
    public function rb_display_plan_status($status, $row) {
        ...
        switch ($status) {
            case DP_PLAN_STATUS_UNAPPROVED:
                return get_string('unapproved', 'corplms_plan');
                break;
            case DP_PLAN_STATUS_APPROVED:
                return get_string('approved', 'corplms_plan');
                break;
            case DP_PLAN_STATUS_COMPLETE:
                return get_string('complete', 'corplms_plan');
                break;
        }
    }
    ...
    public function rb_display_plan_item_status($status) {
        ...
        case DP_APPROVAL_DECLINED:
            return get_string('declined', 'corplms_plan');
        case DP_APPROVAL_UNAPPROVED:
            return get_string('unapproved', 'corplms_plan');
        case DP_APPROVAL_REQUESTED:
            return get_string('pendingapproval', 'corplms_plan');
    ...
    function rb_display_deleted_status($status, $row) {
        switch($status) {
            case 1:
                return get_string('deleteduser', 'corplms_reportbuilder');
            case 2:
                return get_string('suspendeduser', 'corplms_reportbuilder');
            default:
                return get_string('activeuser', 'corplms_reportbuilder');
        }
    }
        ...
        if (isset($row->hierarchytype) && isset($this->hierarchymap[$row->hierarchytype])) {
            ...
            foreach ($paths as $path) {
                ...
                if (isset($map[$path])) {
                    ...
                    $displaypath .= get_string('unknown', 'corplms_reportbuilder');
                }
                $parentid = $path;
            }
        }
    ...
    function rb_display_language_code($code, $row) {
        ...
        if (empty($code)) {
            return get_string('notspecified', 'corplms_reportbuilder');
        }
        ...
        if (isset($languages[$code])) {
            ...
            if (isset($langcodes[$langcode])) {
                ...
                return get_string('uninstalledlanguage', 'corplms_reportbuilder', $a);
            } else {
                return get_string('unknownlanguage', 'corplms_reportbuilder', $code);
            }
        }
    }
    ...
    function rb_display_user_email($email, $row, $isexport = false) {
        ...
        if ($maildisplay != 1) {
            return get_string('useremailprivate', 'corplms_reportbuilder');
        }
    ...
    function rb_display_link_program_icon($program, $row) {
        ...
        $class = 'course_icon ' . corplms_get_style_visibility($programobj, 'program_visible', 'program_audiencevisible');
        $icon = html_writer::empty_tag('img', array('src' => corplms_get_icon($programid, CORPLMS_ICON_TYPE_PROGRAM),
            'class' => $class, 'alt' => ''));
        $link = $OUTPUT->action_link(
            new moodle_url('/corplms/program/view.php', array('id' => $programid)),
    ...
    function rb_display_grade_string($item, $row) {
        ...
        if ($item === null) {
            ...
            $a->grade = $usergrade;
            $a->pass = $passgrade;
            return get_string('gradeandgradetocomplete', 'corplms_reportbuilder', $a);
        }
    }
    ...
    function rb_filter_organisations_list($report) {
        ...
        require_once($CFG->dirroot . '/corplms/hierarchy/prefix/organisation/lib.php');
    ...
    function rb_filter_competency_type_list() {
        ...
        $unclassified_option = array(0 => get_string('unclassified', 'corplms_hierarchy'));
    ...
    public function rb_filter_course_types() {
        global $CORPLMS_COURSE_TYPES;
        $coursetypeoptions = array();
        foreach ($CORPLMS_COURSE_TYPES as $k => $v) {
            $coursetypeoptions[$v] = get_string($k, 'corplms_core');
        }
    ...
    protected function add_user_fields_to_columns(&$columnoptions,
        ...
        $columnoptions[] = new rb_column_option(
            $groupname,
            'fullname',
            get_string('userfullname', 'corplms_reportbuilder'),
        ...
        $columnoptions[] = new rb_column_option(
            $groupname,
            'namelink',
            get_string('usernamelink', 'corplms_reportbuilder'),
            $DB->sql_fullname("$join.firstname", "$join.lastname"),
            array(
                'joins' => $join,
                'displayfunc' => 'link_user',
                'defaultheading' => get_string('userfullname', 'corplms_reportbuilder'),
        ...
        $columnoptions[] = new rb_column_option(
            $groupname,
            'namelinkicon',
            get_string('usernamelinkicon', 'corplms_reportbuilder'),
            $DB->sql_fullname("$join.firstname", "$join.lastname"),
            array(
                'joins' => $join,
        ...
        $columnoptions[] = new rb_column_option(
            $groupname,
            'email',
            get_string('useremail', 'corplms_reportbuilder'),
        ...
        if (!empty($CFG->showuseridentity) &&
            ...
            $columnoptions[] = new rb_column_option(
                $groupname,
                'emailunobscured',
                get_string('useremailunobscured', 'corplms_reportbuilder'),
                "$join.email",
                array(
                    'joins' => $join,
                    'displayfunc' => 'user_email_unobscured',
                    'defaultheading' => get_string('useremail', 'corplms_reportbuilder'),
        ...
        $columnoptions[] = new rb_column_option(
            $groupname,
            'lastlogin',
            get_string('userlastlogin', 'corplms_reportbuilder'),
        ...
        $columnoptions[] = new rb_column_option(
            $groupname,
            'firstaccess',
            get_string('userfirstaccess', 'corplms_reportbuilder'),
            "$join.firstaccess",
            array(
                'joins' => $join,
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'lang',
            get_string('userlang', 'corplms_reportbuilder'),
            "$join.lang",
            array(
                'joins' => $join,
        ...
        $fields = array(
            'firstname' => get_string('userfirstname', 'corplms_reportbuilder'),
            'firstnamephonetic' => get_string('userfirstnamephonetic', 'corplms_reportbuilder'),
            'middlename' => get_string('usermiddlename', 'corplms_reportbuilder'),
            'lastname' => get_string('userlastname', 'corplms_reportbuilder'),
            'lastnamephonetic' => get_string('userlastnamephonetic', 'corplms_reportbuilder'),
            'alternatename' => get_string('useralternatename', 'corplms_reportbuilder'),
            'username' => get_string('username', 'corplms_reportbuilder'),
            'idnumber' => get_string('useridnumber', 'corplms_reportbuilder'),
            'phone1' => get_string('userphone', 'corplms_reportbuilder'),
            'institution' => get_string('userinstitution', 'corplms_reportbuilder'),
            'department' => get_string('userdepartment', 'corplms_reportbuilder'),
            'address' => get_string('useraddress', 'corplms_reportbuilder'),
            'city' => get_string('usercity', 'corplms_reportbuilder'),
        );
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'id',
            get_string('userid', 'corplms_reportbuilder'),
            "$join.id",
            array('joins' => $join)
        );
        ...
        $columnoptions[] = new rb_column_option(
            $groupname,
            'country',
            get_string('usercountry', 'corplms_reportbuilder'),
            "$join.country",
            array(
                'joins' => $join,
        ...
        $columnoptions[] = new rb_column_option(
            $groupname,
            'deleted',
            get_string('userstatus', 'corplms_reportbuilder'),
        ...
        $columnoptions[] = new rb_column_option(
            $groupname,
            'timecreated',
            get_string('usertimecreated', 'corplms_reportbuilder'),
            "$join.timecreated",
            array(
                'joins' => $join,
                'displayfunc' => 'nice_datetime',
        ...
        $columnoptions[] = new rb_column_option(
            $groupname,
            'timemodified',
            get_string('usertimemodified', 'corplms_reportbuilder'),
            "$join.timemodified",
            array(
                'joins' => $join,
                'displayfunc' => 'nice_datetime',
    ...
    protected function add_user_fields_to_filters(&$filteroptions, $groupname = 'user') {
        ...
        $fields = array(
            'fullname' => get_string('userfullname', 'corplms_reportbuilder'),
            'firstname' => get_string('userfirstname', 'corplms_reportbuilder'),
            'firstnamephonetic' => get_string('userfirstnamephonetic', 'corplms_reportbuilder'),
            'middlename' => get_string('usermiddlename', 'corplms_reportbuilder'),
            'lastname' => get_string('userlastname', 'corplms_reportbuilder'),
            ...
            'address' => get_string('useraddress', 'corplms_reportbuilder'),
            'city' => get_string('usercity', 'corplms_reportbuilder'),
            'email' => get_string('useremail', 'corplms_reportbuilder'),
        );
        foreach ($fields as $field => $name) {
        ...
        $filteroptions[] = new rb_filter_option(
            ...
            'select',
            array(
                'selectchoices' => array(0 => get_string('activeonly', 'corplms_reportbuilder'),
                                         1 => get_string('deletedonly', 'corplms_reportbuilder'),
                                         2 => get_string('suspendedonly', 'corplms_reportbuilder')),
                'attributes' => $select_width_options,
                'simplemode' => true,
            )
        );
    ...
    protected function add_course_fields_to_columns(&$columnoptions, $join='course') {
        ...
        $columnoptions[] = new rb_column_option(
            'course',
            'fullname',
            get_string('coursename', 'corplms_reportbuilder'),
            "$join.fullname",
            array('joins' => $join,
                  'dbdatatype' => 'char',
                  'outputformat' => 'text')
        ...
        $columnoptions[] = new rb_column_option(
            'course',
            'courselink',
            get_string('coursenamelinked', 'corplms_reportbuilder'),
            "$join.fullname",
            array(
                'joins' => $join,
                'displayfunc' => 'link_course',
                'defaultheading' => get_string('coursename', 'corplms_reportbuilder'),
        ...
        $columnoptions[] = new rb_column_option(
            'course',
            'courseexpandlink',
            get_string('courseexpandlink', 'corplms_reportbuilder'),
            "$join.fullname",
            array(
                'joins' => $join,
        ...
        $columnoptions[] = new rb_column_option(
            'course',
            'courselinkicon',
            get_string('coursenamelinkedicon', 'corplms_reportbuilder'),
            "$join.fullname",
            array(
                'joins' => $join,
        ...
        $columnoptions[] = new rb_column_option(
            'course',
            'visible',
            get_string('coursevisible', 'corplms_reportbuilder'),
            "$join.visible",
            array(
                'joins' => $join,
                'displayfunc' => 'yes_no'
        ...
        $columnoptions[] = new rb_column_option(
            'course',
            'audvis',
            get_string('audiencevisibility', 'corplms_reportbuilder'),
        ...
        $columnoptions[] = new rb_column_option(
            'course',
            'icon',
            get_string('courseicon', 'corplms_reportbuilder'),
            "$join.icon",
            array(
                'joins' => $join,
                'displayfunc' => 'course_icon',
                'defaultheading' => get_string('courseicon', 'corplms_reportbuilder'),
                'extrafields' => array(
        ...
        $columnoptions[] = new rb_column_option(
            'course',
            'shortname',
            get_string('courseshortname', 'corplms_reportbuilder'),
            "$join.shortname",
            array('joins' => $join,
                  'dbdatatype' => 'char',
                  'outputformat' => 'text')
        ...
        $columnoptions[] = new rb_column_option(
            'course',
            'idnumber',
            get_string('courseidnumber', 'corplms_reportbuilder'),
            "$join.idnumber",
            array('joins' => $join,
                  'dbdatatype' => 'char',
                  'outputformat' => 'text')
        ...
        $columnoptions[] = new rb_column_option(
            'course',
            'id',
            get_string('courseid', 'corplms_reportbuilder'),
            "$join.id",
            array('joins' => $join)
        );
        $columnoptions[] = new rb_column_option(
            'course',
            'startdate',
            get_string('coursestartdate', 'corplms_reportbuilder'),
            "$join.startdate",
            array(
                'joins' => $join,
                'displayfunc' => 'nice_date',
        ...
        $columnoptions[] = new rb_column_option(
            'course',
            'name_and_summary',
            get_string('coursenameandsummary', 'corplms_reportbuilder'),
        ...
        $columnoptions[] = new rb_column_option(
            'course',
            'summary',
            get_string('coursesummary', 'corplms_reportbuilder'),
        ...
        $columnoptions[] = new rb_column_option(
            'course',
            'coursetypeicon',
            get_string('coursetypeicon', 'corplms_reportbuilder'),
            "$join.coursetype",
            array(
                'joins' => $join,
                'displayfunc' => 'course_type_icon',
                'defaultheading' => get_string('coursetypeicon', 'corplms_reportbuilder'),
            )
        );
        $columnoptions[] = new rb_column_option(
            'course',
            'coursetype',
            get_string('coursetype', 'corplms_reportbuilder'),
            "$join.coursetype",
            array(
                'joins' => $join,
                'displayfunc' => 'course_type',
                'defaultheading' => get_string('coursetype', 'corplms_reportbuilder'),
            )
        );
        // add language option
        $columnoptions[] = new rb_column_option(
            'course',
            'language',
            get_string('courselanguage', 'corplms_reportbuilder'),
            "$join.lang",
            array(
                'joins' => $join,
                'displayfunc' => 'language_code'
     ...
     * @param string $langfile Source for translation, corplms_program or corplms_certification
     *
     * @return True
     */
    protected function add_program_fields_to_columns(&$columnoptions, $join = 'program', $langfile = 'corplms_program') {
        global $DB;
        ...
        $columnoptions[] = new rb_column_option(
            'prog',
            'availablefrom',
            get_string('availablefrom', 'corplms_program'),
            "$join.availablefrom",
            array(
                'joins' => $join,
                'displayfunc' => 'nice_date',
        ...
        $columnoptions[] = new rb_column_option(
            'prog',
            'availableuntil',
            get_string('availableuntil', 'corplms_program'),
            "$join.availableuntil",
            array(
                'joins' => $join,
                'displayfunc' => 'nice_date',
        ...
        $columnoptions[] = new rb_column_option(
            'prog',
            'visible',
            get_string('programvisible', 'corplms_program'),
            "$join.visible",
            array(
                'joins' => $join,
                'displayfunc' => 'yes_no'
     ...
     * @return True
     */
    protected function add_program_fields_to_filters(&$filteroptions, $langfile = 'corplms_program') {
        $filteroptions[] = new rb_filter_option(
            'prog',
            'fullname',
            get_string('programname', $langfile),
    ...
    protected function add_course_category_fields_to_columns(&$columnoptions,
        ...
        $columnoptions[] = new rb_column_option(
            'course_category',
            'name',
            get_string('coursecategory', 'corplms_reportbuilder'),
            "$catjoin.name",
            array('joins' => $catjoin,
                  'dbdatatype' => 'char',
                  'outputformat' => 'text')
        ...
        $columnoptions[] = new rb_column_option(
            'course_category',
            'namelink',
            get_string('coursecategorylinked', 'corplms_reportbuilder'),
            "$catjoin.name",
            array(
                'joins' => $catjoin,
                'displayfunc' => 'link_course_category',
                'defaultheading' => get_string('category', 'corplms_reportbuilder'),
        ...
        $columnoptions[] = new rb_column_option(
            'course_category',
            'id',
            get_string('coursecategoryid', 'corplms_reportbuilder'),
            "$coursejoin.category",
    ...
    protected function add_course_category_fields_to_filters(&$filteroptions) {
        ...
        $filteroptions[] = new rb_filter_option(
            'course_category',
            'path',
            get_string('coursecategorymultichoice', 'corplms_reportbuilder'),
            'category',
            array(),
    ...
    protected function add_position_fields_to_columns(&$columnoptions,
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'organisationid',
            get_string('usersorgid', 'corplms_reportbuilder'),
        ...
        $columnoptions[] = new rb_column_option(
            ...
            get_string('usersorgidnumber', 'corplms_reportbuilder'),
            "$org.idnumber",
            array('joins' => $org,
                  'selectable' => true,
                  'dbdatatype' => 'char',
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'organisationpath',
            get_string('usersorgpathids', 'corplms_reportbuilder'),
            "$org.path",
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'organisation',
            get_string('usersorgname', 'corplms_reportbuilder'),
            "$org.fullname",
            array('joins' => $org,
                  'dbdatatype' => 'char',
                  'outputformat' => 'text')
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'org_type',
            get_string('organisationtype', 'corplms_reportbuilder'),
            'org_type.fullname',
            array(
                'joins' => 'org_type',
                'dbdatatype' => 'char',
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'org_type_id',
            get_string('organisationtypeid', 'corplms_reportbuilder'),
            'organisation.typeid',
            array('joins' => $org, 'selectable' => false)
        );
        $columnoptions[] = new rb_column_option(
            'user',
            'positionid',
            get_string('usersposid', 'corplms_reportbuilder'),
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'positionidnumber',
            get_string('usersposidnumber', 'corplms_reportbuilder'),
            "$pos.idnumber",
            array('joins' => $pos,
                  'selectable' => true,
                  'dbdatatype' => 'char',
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'positionpath',
            get_string('userspospathids', 'corplms_reportbuilder'),
            "$pos.path",
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'position',
            get_string('userspos', 'corplms_reportbuilder'),
            "$pos.fullname",
            array('joins' => $pos,
                  'dbdatatype' => 'char',
                  'outputformat' => 'text')
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'pos_type',
            get_string('positiontype', 'corplms_reportbuilder'),
            'pos_type.fullname',
            array(
                'joins' => 'pos_type',
                 'dbdatatype' => 'char',
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'pos_type_id',
            get_string('positiontypeid', 'corplms_reportbuilder'),
            'position.typeid',
            array('joins' => $pos, 'selectable' => false)
        );
        $columnoptions[] = new rb_column_option(
            'user',
            'title',
            get_string('usersjobtitle', 'corplms_reportbuilder'),
            "$posassign.fullname",
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'posstartdate',
            get_string('posstartdate', 'corplms_reportbuilder'),
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'posenddate',
            get_string('posenddate', 'corplms_reportbuilder'),
    ...
    protected function add_position_fields_to_filters(&$filteroptions) {
        ...
        $filteroptions[] = new rb_filter_option(
            'user',
            'organisationid',
            get_string('participantscurrentorgbasic', 'corplms_reportbuilder'),
            'select',
            array(
        ...
        $filteroptions[] = new rb_filter_option(
            'user',
            'organisationpath',
            get_string('participantscurrentorg', 'corplms_reportbuilder'),
            'hierarchy',
            array(
                'hierarchytype' => 'org',
            )
        ...
        $filteroptions[] = new rb_filter_option(
            'user',
            'positionid',
            get_string('participantscurrentposbasic', 'corplms_reportbuilder'),
            'select',
            array(
        ...
        $filteroptions[] = new rb_filter_option(
            'user',
            'positionpath',
            get_string('participantscurrentpos', 'corplms_reportbuilder'),
            'hierarchy',
            array(
                'hierarchytype' => 'pos',
            )
    ...
    protected function add_custom_fields_for($cf_prefix, $join, $joinfield,
        ...
        if (!$seek) {
            ...
            $a->join = $join;
            $a->source = get_class($this);
            throw new ReportBuilderException(get_string('error:missingdependencytable', 'corplms_reportbuilder', $a));
        }
        ...
        foreach ($items as $record) {
            ...
            if ($record->datatype == 'multiselect') {
                ...
                require_once($CFG->dirroot . '/corplms/customfield/definelib.php');
                require_once($CFG->dirroot . '/corplms/customfield/field/multiselect/field.class.php');
                require_once($CFG->dirroot . '/corplms/customfield/field/multiselect/define.class.php');
                ...
                $columnoptions[] = new rb_column_option(
                        $cf_prefix,
                        $value.'_icon',
                        get_string('multiselectcolumnicon', 'corplms_customfield', $name),
                        "$joinname.data",
                ...
                $columnoptions[] = new rb_column_option(
                        $cf_prefix,
                        $value.'_text',
                        get_string('multiselectcolumntext', 'corplms_customfield', $name),
                        "$joinname.data",
                        array('joins' => $joinname,
    ...
    protected function add_manager_fields_to_columns(&$columnoptions,
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'managername',
            get_string('usersmanagername', 'corplms_reportbuilder'),
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'managerfirstname',
            get_string('usersmanagerfirstname', 'corplms_reportbuilder'),
            "$manager.firstname",
            array('joins' => $manager,
                  'dbdatatype' => 'char',
                  'outputformat' => 'text')
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'managerlastname',
            get_string('usersmanagerlastname', 'corplms_reportbuilder'),
            "$manager.lastname",
            array('joins' => $manager,
                  'dbdatatype' => 'char',
                  'outputformat' => 'text')
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'managerid',
            get_string('usersmanagerid', 'corplms_reportbuilder'),
            "$manager.id",
            array('joins' => $manager)
        );
        $columnoptions[] = new rb_column_option(
            'user',
            'manageridnumber',
            get_string('usersmanageridnumber', 'corplms_reportbuilder'),
            "$manager.idnumber",
            array('joins' => $manager,
                  'dbdatatype' => 'char',
                  'outputformat' => 'text')
    ...
    protected function add_manager_fields_to_filters(&$filteroptions) {
        $filteroptions[] = new rb_filter_option(
            'user',
            'managername',
            get_string('managername', 'corplms_reportbuilder'),
            'text'
        );
        $filteroptions[] = new rb_filter_option(
            'user',
    ...
    protected function add_tag_fields_to_columns($type, &$columnoptions, $tagids='tagids', $tagnames='tagnames') {
        ...
        $columnoptions[] = new rb_column_option(
            'tags',
            'tagids',
            get_string('tagids', 'corplms_reportbuilder'),
            "$tagids.idlist",
        ...
        $columnoptions[] = new rb_column_option(
            'tags',
            'tagnames',
            get_string('tags', 'corplms_reportbuilder'),
            "$tagnames.namelist",
        ...
        foreach ($tags as $tag) {
            ...
            $columnoptions[] = new rb_column_option(
                'tags',
                $join,
                get_string('taggedx', 'corplms_reportbuilder', $name),
    ...
    protected function add_tag_fields_to_filters($type, &$filteroptions) {
        ...
        $filteroptions[] = new rb_filter_option(
            'tags',         // type
            'tagids',           // value
            get_string('tags', 'corplms_reportbuilder'), // label
    ...
    protected function add_cohort_user_fields_to_columns(&$columnoptions,
        ...
        $columnoptions[] = new rb_column_option(
            'cohort',
            'usercohortids',
            get_string('usercohortids', 'corplms_reportbuilder'),
            "$cohortids.idlist",
    ...
    protected function add_cohort_course_fields_to_columns(&$columnoptions, $cohortenrolledids='cohortenrolledcourse') {
        $columnoptions[] = new rb_column_option(
            ...
            get_string('enrolledcoursecohortids', 'corplms_reportbuilder'),
    ...
    protected function add_cohort_program_fields_to_columns(&$columnoptions, $cohortenrolledids='cohortenrolledprogram') {
        $columnoptions[] = new rb_column_option(
            ...
            get_string('enrolledprogramcohortids', 'corplms_reportbuilder'),
            "$cohortenrolledids.idlist",
            array('joins' => $cohortenrolledids, 'selectable' => false)
        );
    ...
    protected function add_cohort_user_fields_to_filters(&$filteroptions) {
        ...
        $filteroptions[] = new rb_filter_option(
            'cohort',
            'usercohortids',
            get_string('userincohort', 'corplms_reportbuilder'),
            'cohort'
        );
        return true;
    }
    ...
    protected function add_cohort_course_fields_to_filters(&$filteroptions) {
        ...
        $filteroptions[] = new rb_filter_option(
            'cohort',
            'enrolledcoursecohortids',
            get_string('courseenrolledincohort', 'corplms_reportbuilder'),
            'cohort'
        );
    ...
    protected function add_cohort_program_fields_to_filters(&$filteroptions) {
        ...
        $filteroptions[] = new rb_filter_option(
            'cohort',
            'enrolledprogramcohortids',
            get_string('programenrolledincohort', 'corplms_reportbuilder'),
            'cohort'
        );
    ...
    protected function get_advanced_aggregation_classes($type) {
        ...
        foreach (scandir("{$CFG->dirroot}/corplms/reportbuilder/classes/rb/{$type}") as $filename) {
            ...
            $classname = "\\corplms_reportbuilder\\rb\\{$type}\\$name";
            if (!class_exists($classname)) {
    ...
    public function columns_page_requires() {
        \corplms_reportbuilder\rb\aggregate\base::require_column_heading_strings();
        \corplms_reportbuilder\rb\transform\base::require_column_heading_strings();
    }
