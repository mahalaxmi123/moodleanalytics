...
class rb_current_pos_content extends rb_base_content {
    ...
    public function text_restriction($title, $reportid) {
        ...
        case 0:
            return $title . ' ' . get_string('is', 'corplms_reportbuilder') .
                ': "' . $posname . '"';
        case 1:
            return $title . ' ' . get_string('is', 'corplms_reportbuilder') .
                ': "' . $posname . '" ' . get_string('orsubpos', 'corplms_reportbuilder');
        case 2:
            return $title . ' ' . get_string('isbelow', 'corplms_reportbuilder') .
                ': "' . $posname . '"';
        default:
            return '';
        }
    ...
    public function form_template(&$mform, $reportid, $title) {
        ...
        $mform->addElement('header', 'current_pos_header',
            get_string('showbyx', 'corplms_reportbuilder', lcfirst($title)));
        $mform->setExpanded('current_pos_header');
        $mform->addElement('checkbox', 'current_pos_enable', '',
            get_string('currentposenable', 'corplms_reportbuilder'));
        ...
        $radiogroup[] =& $mform->createElement('radio', 'current_pos_recursive',
            '', get_string('showrecordsinposandbelow', 'corplms_reportbuilder'), 1);
        $radiogroup[] =& $mform->createElement('radio', 'current_pos_recursive',
            '', get_string('showrecordsinpos', 'corplms_reportbuilder'), 0);
        $radiogroup[] =& $mform->createElement('radio', 'current_pos_recursive',
            '', get_string('showrecordsbelowposonly', 'corplms_reportbuilder'), 2);
        $mform->addGroup($radiogroup, 'current_pos_recursive_group',
            get_string('includechildpos', 'corplms_reportbuilder'), html_writer::empty_tag('br'), false);
        ...
        $mform->disabledIf('current_pos_recursive_group', 'current_pos_enable',
            'notchecked');
        $mform->addHelpButton('current_pos_header', 'reportbuildercurrentpos', 'corplms_reportbuilder');
    }
...
class rb_current_org_content extends rb_base_content {
    ...
    public function text_restriction($title, $reportid) {
        ...
        case 1:
            return $title . ' ' . get_string('is', 'corplms_reportbuilder') .
                ': "' . $orgname . '" ' . get_string('orsuborg', 'corplms_reportbuilder');
        case 2:
            return $title . ' ' . get_string('isbelow', 'corplms_reportbuilder') .
                ': "' . $orgname . '"';
        default:
    ...
    public function form_template(&$mform, $reportid, $title) {
        ...
        $mform->addElement('checkbox', 'current_org_enable', '',
            get_string('currentorgenable', 'corplms_reportbuilder'));
        ...
        $radiogroup[] =& $mform->createElement('radio', 'current_org_recursive',
            '', get_string('showrecordsinorgandbelow', 'corplms_reportbuilder'), 1);
        $radiogroup[] =& $mform->createElement('radio', 'current_org_recursive',
            '', get_string('showrecordsinorg', 'corplms_reportbuilder'), 0);
        $radiogroup[] =& $mform->createElement('radio', 'current_org_recursive',
            '', get_string('showrecordsbeloworgonly', 'corplms_reportbuilder'), 2);
        $mform->addGroup($radiogroup, 'current_org_recursive_group',
            get_string('includechildorgs', 'corplms_reportbuilder'), html_writer::empty_tag('br'), false);
        ...
        $mform->disabledIf('current_org_recursive_group', 'current_org_enable',
            'notchecked');
        $mform->addHelpButton('current_org_header', 'reportbuildercurrentorg', 'corplms_reportbuilder');
    }
...
class rb_completed_org_content extends rb_base_content {
    ...
    public function text_restriction($title, $reportid) {
        ...
        $orgid = $DB->get_field('pos_assignment', 'organisationid', array('userid' => $userid, 'type' => 1));
        if (empty($orgid)) {
            return $title . ' ' . get_string('is', 'corplms_reportbuilder') . ' "UNASSIGNED"';
        }
        $orgname = $DB->get_field('org', 'fullname', array('id' => $orgid));
    ...
    public function form_template(&$mform, $reportid, $title) {
        ...
        $mform->addElement('checkbox', 'completed_org_enable', '',
            get_string('completedorgenable', 'corplms_reportbuilder'));
        ...
        $mform->addHelpButton('completed_org_header', 'reportbuildercompletedorg', 'corplms_reportbuilder');
    }
...
class rb_user_content extends rb_base_content {
    ...
    public function text_restriction($title, $reportid) {
        ...
        if (($who & self::USER_OWN) == self::USER_OWN) {
            $strings[] = $title . ' ' . get_string('is', 'corplms_reportbuilder') . ' "' .
                fullname($user) . '"';
        }

        if (($who & self::USER_DIRECT_REPORTS) == self::USER_DIRECT_REPORTS) {
            $strings[] = $title . ' ' . get_string('reportsdirectlyto', 'corplms_reportbuilder') . ' "' .
                fullname($user) . '"';
        }

        if (($who & self::USER_INDIRECT_REPORTS) == self::USER_INDIRECT_REPORTS) {
            $strings[] = $title . ' ' . get_string('reportsindirectlyto', 'corplms_reportbuilder') . ' "' .
                fullname($user) . '"';
        }

        if (empty($strings)) {
            return $title . ' ' . get_string('isnotfound', 'corplms_reportbuilder');
        }

        return implode(get_string('or', 'corplms_reportbuilder'), $strings);
    ...
    public function form_template(&$mform, $reportid, $title) {
        ...
        $mform->addElement('header', 'user_header', get_string('showbyx',
            'corplms_reportbuilder', lcfirst($title)));
        $mform->setExpanded('user_header');
        $mform->addElement('checkbox', 'user_enable', '',
            get_string('showbasedonx', 'corplms_reportbuilder', lcfirst($title)));
        ...
        $checkgroup[] =& $mform->createElement('advcheckbox', 'user_who['.self::USER_OWN.']', '',
            get_string('userownrecords', 'corplms_reportbuilder'), null, array(0, 1));
        ...
        $checkgroup[] =& $mform->createElement('advcheckbox', 'user_who['.self::USER_DIRECT_REPORTS.']', '',
            get_string('userdirectreports', 'corplms_reportbuilder'), null, array(0, 1));
        ...
        $checkgroup[] =& $mform->createElement('advcheckbox', 'user_who['.self::USER_INDIRECT_REPORTS.']', '',
            get_string('userindirectreports', 'corplms_reportbuilder'), null, array(0, 1));
        ...
        $checkgroup[] =& $mform->createElement('advcheckbox', 'user_who['.self::USER_TEMP_REPORTS.']', '',
            get_string('usertempreports', 'corplms_reportbuilder'), null, array(0, 1));
        $mform->setType('user_who['.self::USER_TEMP_REPORTS.']', PARAM_INT);

        $mform->addGroup($checkgroup, 'user_who_group',
            get_string('includeuserrecords', 'corplms_reportbuilder'), html_writer::empty_tag('br'), false);
        ...
        $mform->addHelpButton('user_header', 'reportbuilderuser', 'corplms_reportbuilder');
    }
...
class rb_date_content extends rb_base_content {
    ...
    public function text_restriction($title, $reportid) {
        ...
        switch ($settings['when']) {
        case 'past':
            return $title . ' ' . get_string('occurredbefore', 'corplms_reportbuilder') . ' ' .
                userdate(time(), '%c'). $includenulls;
        case 'future':
            return $title . ' ' . get_string('occurredafter', 'corplms_reportbuilder') . ' ' .
                userdate(time(), '%c'). $includenulls;
        case 'last30days':
            return $title . ' ' . get_string('occurredafter', 'corplms_reportbuilder') . ' ' .
                userdate(time() - 60*60*24*30, '%c') . get_string('and', 'corplms_reportbuilder') .
                get_string('occurredbefore', 'corplms_reportbuilder') . userdate(time(), '%c') .
                $includenulls;

        case 'next30days':
            return $title . ' ' . get_string('occurredafter', 'corplms_reportbuilder') . ' ' .
                userdate(time(), '%c') . get_string('and', 'corplms_reportbuilder') .
                get_string('occurredbefore', 'corplms_reportbuilder') .
                userdate(time() + 60*60*24*30, '%c') . $includenulls;
        case 'currentfinancial':
            return $title . ' ' . get_string('occurredthisfinancialyear', 'corplms_reportbuilder') .
                $includenulls;
        case 'lastfinancial':
            return $title . ' ' . get_string('occurredprevfinancialyear', 'corplms_reportbuilder') .
                $includenulls;
        default:
    ...
    public function form_template(&$mform, $reportid, $title) {
        ...
        $mform->addElement('checkbox', 'date_enable', '',
            get_string('showbasedonx', 'corplms_reportbuilder',
            lcfirst($title)));
        ...
        $radiogroup[] =& $mform->createElement('radio', 'date_when', '',
            get_string('thepast', 'corplms_reportbuilder'), 'past');
        $radiogroup[] =& $mform->createElement('radio', 'date_when', '',
            get_string('thefuture', 'corplms_reportbuilder'), 'future');
        $radiogroup[] =& $mform->createElement('radio', 'date_when', '',
            get_string('last30days', 'corplms_reportbuilder'), 'last30days');
        $radiogroup[] =& $mform->createElement('radio', 'date_when', '',
            get_string('next30days', 'corplms_reportbuilder'), 'next30days');
        $radiogroup[] =& $mform->createElement('radio', 'date_when', '',
            get_string('currentfinancial', 'corplms_reportbuilder'), 'currentfinancial');
        $radiogroup[] =& $mform->createElement('radio', 'date_when', '',
            get_string('lastfinancial', 'corplms_reportbuilder'), 'lastfinancial');
        $mform->addGroup($radiogroup, 'date_when_group',
            get_string('includerecordsfrom', 'corplms_reportbuilder'), html_writer::empty_tag('br'), false);
        ...
        $mform->addHelpButton('date_header', 'reportbuilderdate', 'corplms_reportbuilder');

        $mform->addElement('checkbox', 'date_incnulls',
            get_string('includeemptydates', 'corplms_reportbuilder'));
...
class rb_tag_content extends rb_base_content {
    ...
    public function text_restriction($title, $reportid) {
        ...
        if (count($include_text) > 0) {
            $includestr = $title . ' ' . get_string('istaggedwith', 'corplms_reportbuilder') .
                ' ' . implode(get_string($include_logic, 'corplms_reportbuilder'), $include_text);
        } else {
            $includestr = '';
        }
        if (count($exclude_text) > 0) {
            $excludestr = $title . ' ' . get_string('isnttaggedwith', 'corplms_reportbuilder') .
                ' ' . implode(get_string($exclude_logic, 'corplms_reportbuilder'), $exclude_text);
        } else {
            $excludestr = '';
        }

        if ($includestr && $excludestr) {
            return $includestr . get_string('and', 'corplms_reportbuilder') . $excludestr;
        } else if ($includestr) {
            return $includestr;
        } else if ($excludestr) {
            return $excludestr;
    ...
    public function form_template(&$mform, $reportid, $title) {
        ...
        $mform->addElement('header', 'tag_header',
            get_string('showbytag', 'corplms_reportbuilder'));
        $mform->setExpanded('tag_header');
        $mform->addHelpButton('tag_header', 'reportbuildertag', 'corplms_reportbuilder');

        $mform->addElement('checkbox', 'tag_enable', '',
            get_string('tagenable', 'corplms_reportbuilder'));
        ...
        if (!empty($tags)) {
            $checkgroup = array();
            $opts = array(1 => get_string('anyofthefollowing', 'corplms_reportbuilder'),
                          0 => get_string('allofthefollowing', 'corplms_reportbuilder'));
            $mform->addElement('select', 'tag_include_logic', get_string('includetags', 'corplms_reportbuilder'), $opts);
        ...
        if (!empty($tags)) {
            ...
            $opts = array(1 => get_string('anyofthefollowing', 'corplms_reportbuilder'),
                          0 => get_string('allofthefollowing', 'corplms_reportbuilder'));
            $mform->addElement('select', 'tag_exclude_logic', get_string('excludetags', 'corplms_reportbuilder'), $opts);
...
class rb_prog_availability_content extends rb_base_content {
     ...
     */
    public function sql_restriction($field, $reportid) {
        // The restriction snippet based on the available fields was moved to corplms_visibility_where.
     ...
     */
    public function text_restriction($title, $reportid) {
        return get_string('contentavailability', 'corplms_program');
    }
    ...
    public function form_template(&$mform, $reportid, $title) {
        ...
        $mform->addElement('checkbox', 'prog_availability_enable', '',
            get_string('contentavailability', 'corplms_program'));
        $mform->setDefault('prog_availability_enable', $enable);
        $mform->disabledIf('prog_availability_enable', 'contentenabled', 'eq', 0);
        $mform->addHelpButton('prog_availability_header', 'contentavailability', 'corplms_program');
...
include_once($CFG->dirroot . '/corplms/reportbuilder/classes/rb_trainer_content.php');
