...
class rb_trainer_content extends rb_base_content {
    ...
    function text_restriction($title, $reportid) {
        ...
        case 'own':
            return $title . ' ' . get_string('is', 'corplms_reportbuilder') . ' "' .
                fullname($user) . '"';
        case 'reports':
            return $title . ' ' . get_string('reportsto', 'corplms_reportbuilder') . ' "' .
                fullname($user) . '"';
        case 'ownandreports':
            return $title . ' ' . get_string('is', 'corplms_reportbuilder') . ' "' .
                fullname($user) . '"' . get_string('or', 'corplms_reportbuilder') .
                get_string('reportsto', 'corplms_reportbuilder') . ' "' . fullname($user) . '"';
        default:
    ...
    function form_template(&$mform, $reportid, $title) {
        ...
        $radiogroup[] =& $mform->createElement('radio', 'trainer_who', '',
            get_string('trainerownrecords', 'corplms_reportbuilder'), 'own');
        $radiogroup[] =& $mform->createElement('radio', 'trainer_who', '',
            get_string('trainerstaffrecords', 'corplms_reportbuilder'), 'reports');
        $radiogroup[] =& $mform->createElement('radio', 'trainer_who', '',
            get_string('both', 'corplms_reportbuilder'), 'ownandreports');
        $mform->addGroup($radiogroup, 'trainer_who_group',
            get_string('includetrainerrecords', 'corplms_reportbuilder'), html_writer::empty_tag('br'), false);
        ...
        $mform->addHelpButton('trainer_header', 'reportbuildertrainer', 'corplms_reportbuilder');
    }
