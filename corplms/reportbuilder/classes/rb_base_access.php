...
class rb_role_access extends rb_base_access {
    ...
    */
    ...
    function form_template(&$mform, $reportid) {
        ...
        $mform->addElement('header', 'accessbyroles', get_string('accessbyrole', 'corplms_reportbuilder'));
        ...
        if (!empty($roles)) {
            $contextoptions = array('site' => get_string('systemcontext', 'corplms_reportbuilder'), 'any' => get_string('anycontext', 'corplms_reportbuilder'));

            // set context for role-based access
            $mform->addElement('select', 'role_context', get_string('context', 'corplms_reportbuilder'), $contextoptions);
            $mform->setDefault('role_context', $context);
            $mform->disabledIf('role_context', 'accessenabled', 'eq', 0);
            $mform->addHelpButton('role_context', 'reportbuildercontext', 'corplms_reportbuilder');
            ...
            $mform->addGroup($rolesgroup, 'roles', get_string('roleswithaccess', 'corplms_reportbuilder'), html_writer::empty_tag('br'), false);
            $mform->disabledIf('roles', 'accessenabled', 'eq', 0);
            $mform->addHelpButton('roles', 'reportbuilderrolesaccess', 'corplms_reportbuilder');
        } else {
            $mform->addElement('html', html_writer::tag('p', get_string('error:norolesfound', 'corplms_reportbuilder')));
        }
