...
class corplms_reportbuilder_display_testcase extends advanced_testcase {
    ...
    public function test_aggregation() {
        ...
        $fs = get_file_storage();
        $file = array('contextid' => $syscontext->id, 'component' => 'corplms_customfield', 'filearea' => 'course_filemgr',
                        'itemid' => $f->data, 'filepath' => '/', 'filename' => 'readme.txt');
        $fileurl = 'http://www.example.com/moodle/pluginfile.php/'.$syscontext->id.'/corplms_customfield/course_filemgr/'.
                        $f->data.'/readme.txt';
        ...
        $this->assertSame('<a class="icon" href="'.$fileurl.'"><img alt="File" class="smallicon" title="File" src="http://www.example.com/moodle/theme/image.php/_s/corplms/core/1/f/text" />readme.txt</a>', $processed[1]);
        $this->assertSame('volba1', $processed[2]);
        $this->assertSame('<img src="http://www.example.com/moodle/theme/image.php/_s/corplms/corplms_core/1//courseicons/business-modelling" id="icon_preview" class="course_icon" alt="volba1" title="volba1" />', $processed[3]);
