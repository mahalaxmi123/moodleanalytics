...
class corplms_reportbuilder_rb_goal_status_history_embedded_testcase extends advanced_testcase {
    /**
    ...
    public function test_is_capable() {
        ...
        assign_capability('corplms/hierarchy:viewgoalreport', CAP_ALLOW, $roleuser->id, $syscontext);
        $syscontext->mark_dirty();
        $this->assertTrue($embeddedobject->is_capable($userid, $report),
                'user with capability corplms/hierarchy:viewgoalreport cannot access report');
        assign_capability('corplms/hierarchy:viewgoalreport', CAP_INHERIT, $roleuser->id, $syscontext);
    }
}
