...
class corplms_reportbuilder_rb_appraisal_status_embedded_testcase extends advanced_testcase {
    /**
    ...
    public function test_is_capable() {
        ...
        assign_capability('corplms/appraisal:manageappraisals', CAP_ALLOW, $roleuser->id, $syscontext);
        $syscontext->mark_dirty();
        $this->assertTrue($embeddedobject->is_capable($userid, $report),
                'user with capability corplms/appraisal:manageappraisals cannot access report');
        assign_capability('corplms/appraisal:manageappraisals', CAP_INHERIT, $roleuser->id, $syscontext);
    }
}
