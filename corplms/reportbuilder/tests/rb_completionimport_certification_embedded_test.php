...
class corplms_reportbuilder_rb_completionimport_certification_embedded_testcase extends advanced_testcase {
    /**
    ...
    public function test_is_capable() {
        ...
        assign_capability('corplms/completionimport:import', CAP_ALLOW, $roleuser->id, $syscontext);
        $syscontext->mark_dirty();
        $this->assertTrue($embeddedobject->is_capable($userid, $report),
                'user with capability corplms/completionimport:import cannot access report');
        assign_capability('corplms/completionimport:import', CAP_INHERIT, $roleuser->id, $syscontext);
    }
}
