 ...
 */
    ...
    require_once($CFG->dirroot . '/corplms/reportbuilder/groupslib.php');
    require_once($CFG->dirroot . '/corplms/reportbuilder/groups_forms.php');
    ...
    $returnurl = $CFG->wwwroot . '/corplms/reportbuilder/groups.php';
    ...
    if ($d && $confirm) {
        ...
        if (delete_group($id)) {
            corplms_set_notification(get_string('groupdeleted', 'corplms_reportbuilder'), $returnurl, array('class' => 'notifysuccess'));
        } else {
            corplms_set_notification(get_string('error:groupnotdeleted', 'corplms_reportbuilder'), $returnurl);
        }
    } else if ($d) {
        ...
        if ($reports) {
            ...
            corplms_set_notification(get_string('error:grouphasreports', 'corplms_reportbuilder'), $returnurl);
            die;
        } else {
            // prompt to delete
            echo $output->header();
            echo $output->heading(get_string('activitygroups', 'corplms_reportbuilder'));
            echo $output->confirm(get_string('groupconfirmdelete', 'corplms_reportbuilder'),
                new moodle_url('/corplms/reportbuilder/groups.php',
    ...
    if ($fromform = $mform->get_data()) {
        ...
        if ($newid = create_group($fromform, $errorcode)) {
            redirect($CFG->wwwroot . '/corplms/reportbuilder/groupsettings.php?id=' .
                $newid);
            die;
        } else {
            corplms_set_notification(get_string($errorcode, 'corplms_reportbuilder'), $returnurl);
            die;
        }
    ...
    echo html_writer::tag('p', get_string('activitygroupdesc', 'corplms_reportbuilder'));
