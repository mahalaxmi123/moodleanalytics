    ...
    function definition() {
        ...
        foreach ($activities as $activity) {
            $course = ($activity->fullname === null) ?
                get_string('coursenotset', 'corplms_reportbuilder') : $activity->fullname;
            $grouped_activities[$course][$activity->id] = $activity->name;
        }

        $mform->addElement('header', 'general', get_string('newgroup', 'corplms_reportbuilder'));

        // get all official tags
        $tags = $DB->get_records_menu('tag', array('tagtype' => 'official'), 'id', 'id, name');
        if (!$tags) {
            $mform->addElement('html', html_writer::tag('p', get_string('notags', 'corplms_reportbuilder')));
            return;
        }

        $mform->addElement('text', 'name', get_string('groupname', 'corplms_reportbuilder'),
            'maxlength="255"');
        $mform->setType('name', PARAM_TEXT);
        $mform->addRule('name', null, 'required');
        $mform->addHelpButton('name', 'reportbuildergroupname', 'corplms_reportbuilder');

        $mform->addElement('select', 'assignvalue', get_string('grouptag',
            'corplms_reportbuilder'), $tags);
        // invalid if not set
        $mform->addHelpButton('assignvalue', 'reportbuildergrouptag', 'corplms_reportbuilder');
        ...
        $baseselect =& $mform->addElement('selectgroups', 'baseitem',
            get_string('basedon', 'corplms_reportbuilder'), $grouped_activities, $attributes);
        $mform->addHelpButton('baseitem', 'reportbuilderbaseitem', 'corplms_reportbuilder');
