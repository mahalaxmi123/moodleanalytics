...
$returnurl = $CFG->wwwroot . '/corplms/reportbuilder/index.php';
...
if ($d && $confirm) {
    ...
    if (delete_report($id)) {
        \corplms_reportbuilder\event\report_deleted::create_from_report($report, $em)->trigger();
        corplms_set_notification(get_string($type . 'report', 'corplms_reportbuilder'), $returnurl, array('class' => 'notifysuccess'));

    } else {
        corplms_set_notification(get_string('no' . $type . 'report', 'corplms_reportbuilder'), $returnurl);
    }
} else if ($d) {
    echo $output->header();
    echo $output->heading(get_string('reportbuilder', 'corplms_reportbuilder'));
    if ($em) {
        $continueurl = new moodle_url('/corplms/reportbuilder/index.php', array('id' => $id, 'd' => '1', 'em' => $em,
            'confirm' => '1', 'sesskey' => $USER->sesskey));
        echo $output->confirm(get_string('reportconfirm'.$type, 'corplms_reportbuilder'), $continueurl, $returnurl);
    } else {
        $continueurl = new moodle_url('/corplms/reportbuilder/index.php', array('id' => $id, 'd' => '1', 'em' => $em,
            'confirm' => '1', 'sesskey' => $USER->sesskey));
        echo $output->confirm(get_string('reportconfirm'.$type, 'corplms_reportbuilder'), $continueurl, $returnurl);
    ...
    if ($cache) {
        ...
        corplms_set_notification(get_string('reportcacheinitialize', 'corplms_reportbuilder'), $returnurl, array('class' => 'notifysuccess'));
    } else {
        corplms_set_notification(get_string('error:reportcacheinitialize', 'corplms_reportbuilder'), $returnurl);
    }
}
...
if ($fromform = $mform->get_data()) {
    ...
    if (empty($fromform->submitbutton)) {
        corplms_set_notification(
            get_string('error:unknownbuttonclicked', 'corplms_reportbuilder'),
            $returnurl);
    }
    ...
    try {
        ...
        \corplms_reportbuilder\event\report_created::create_from_report($report, false)->trigger();
        $transaction->allow_commit();
        redirect($CFG->wwwroot . '/corplms/reportbuilder/general.php?id='.$newid);
    } catch (ReportBuilderException $e) {
        $transaction->rollback($e);
        trigger_error($e->getMessage(), E_USER_WARNING);
    } catch (Exception $e) {
        $transaction->rollback($e);
        redirect($returnurl, get_string('error:couldnotcreatenewreport', 'corplms_reportbuilder'));
    }
}
...
echo $output->heading(get_string('usergeneratedreports', 'corplms_reportbuilder'));
...
// Embedded reports
echo $output->heading(get_string('embeddedreports', 'corplms_reportbuilder'));
