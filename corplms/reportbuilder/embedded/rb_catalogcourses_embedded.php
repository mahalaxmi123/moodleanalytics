...
require_once($CFG->dirroot.'/corplms/reportbuilder/embedded/rb_findcourses_embedded.php');

class rb_catalogcourses_embedded extends rb_base_embedded {
    public function __construct($data) {
        $this->url = '/corplms/coursecatalog/courses.php';
        $this->source = 'courses';
        $this->shortname = 'catalogcourses';
        $this->fullname = get_string('catalogcourses', 'corplms_coursecatalog');
        ...
        $this->columns = array(
            array(
                ...
                'heading' => get_string('coursename', 'corplms_reportbuilder')
            ),
            array(
                'type' => 'course',
                'value' => 'startdate',
                'heading' => get_string('report:startdate', 'corplms_reportbuilder')
            ),
            array(
                'type' => 'course',
                'value' => 'mods',
                'heading' => get_string('content', 'corplms_reportbuilder')
            )
        );
        ...
        $this->filters = array(
            array(
                ...
                'fieldname' => get_string('type', 'corplms_reportbuilder')
            ),
            array(
                'type' => 'course',
                'value' => 'mods',
                'region' => rb_filter_type::RB_FILTER_REGION_SIDEBAR,
                'fieldname' => get_string('content', 'corplms_reportbuilder')
            )
        );
    ...
    public function get_extrabuttons() {
        ...
        if (has_capability('moodle/course:create', $catcontext)) {
            ...
            $createbutton = new single_button($createurl, get_string('addcourse', 'corplms_coursecatalog'), 'get');
