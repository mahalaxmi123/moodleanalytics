...
class rb_cohort_associations_enrolled_embedded extends rb_base_embedded {
    ...
    public function __construct($data) {
        $this->url = '/corplms/cohort/enrolledlearning.php';
        $this->source = 'cohort_associations';
        $this->shortname = 'cohort_associations_enrolled';
        $this->fullname = get_string('cohortenrolledreportname', 'corplms_cohort');
        $this->columns = array(
            array(
                'type' => 'associations',
                'value' => 'nameiconlink',
                'heading' => get_string('associationname', 'corplms_cohort')
            ),
            array(
                'type' => 'associations',
                'value' => 'type',
                'heading' => get_string('associationtype', 'corplms_cohort')
            ),
            array(
                'type' => 'associations',
                'value' => 'programcompletionlink',
                'heading' => get_string('associationprogramcompletionlink', 'corplms_cohort')
            ),
            array(
                'type' => 'associations',
                'value' => 'actionsenrolled',
                'heading' => get_string('associationactionsenrolled', 'corplms_cohort')
            ),
        );
