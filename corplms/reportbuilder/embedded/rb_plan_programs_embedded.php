...
class rb_plan_programs_embedded extends rb_base_embedded {
    ...
    public function __construct($data) {
        ...
        $this->url = '/corplms/plan/record/programs.php';
        $this->source = 'dp_program';
        $this->shortname = 'plan_programs';
        $this->fullname = get_string('recordoflearningprograms', 'corplms_plan');
        $this->columns = array(
            array(
                'type' => 'program',
                'value' => 'proglinkicon',
                'heading' => get_string('programname', 'corplms_program'),
            ),
            array(
                'type' => 'program',
                'value' => 'mandatory',
                'heading' => get_string('mandatory', 'corplms_program'),
            ),
            array(
                'type' => 'program',
                'value' => 'recurring',
                'heading' => get_string('recurring', 'corplms_program'),
            ),
            array(
                'type' => 'program',
                'value' => 'timedue',
                'heading' => get_string('duestatus', 'corplms_program'),
            ),
            array(
            ...
            array(
                ...
                'value' => 'status',
                'heading' => get_string('progress', 'corplms_program'),
            ),
            array(
                'type' => 'program_completion',
                'value' => 'starteddate',
        ...
        if (!has_capability('corplms/program:viewhiddenprograms', $context)) {
    ...
    public function is_capable($reportfor, $report) {
        ...
        return ($reportfor == $subjectid ||
                corplms_is_manager($subjectid, $reportfor) ||
                has_capability('corplms/plan:accessanyplan', context_system::instance(), $reportfor) ||
                has_capability('corplms/core:viewrecordoflearning', context_user::instance($reportfor), $reportfor));
    }
}
