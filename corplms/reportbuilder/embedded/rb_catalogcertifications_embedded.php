 ...
 */
require_once($CFG->dirroot.'/corplms/reportbuilder/embedded/rb_findprograms_embedded.php');

class rb_catalogcertifications_embedded extends rb_base_embedded {
    public function __construct($data) {
        $this->url = '/corplms/coursecatalog/certifications.php';
        $this->source = 'certification';
        $this->shortname = 'catalogcertifications';
        $this->fullname = get_string('catalogcertifications', 'corplms_coursecatalog');

        $this->columns = array(
            array(
                'type' => 'prog',
                'value' => 'progexpandlink',
                'heading' => get_string('certifname', 'corplms_certification')
            ),
            array(
                'type' => 'prog',
                'value' => 'summary',
                'heading' => get_string('programsummary', 'corplms_certification')
            )
        );
    ...
    public function get_extrabuttons() {
        ...
        if (has_capability('corplms/certification:createcertification', $catcontext)) {
            $createurl = new moodle_url("/corplms/program/add.php", array('category' => $defaultcat, 'iscertif' => 1));
            $createbutton = new single_button($createurl, get_string('addcertification', 'corplms_coursecatalog'), 'get');
