 ...
 * @package corplms
 * @subpackage corplms_reportbuilder
 */
...
/**
* Output renderer for corplms_reportbuilder module
*/
class corplms_reportbuilder_renderer extends plugin_renderer_base {
    ...
    public function user_generated_reports_table($reports=array()) {
        ...
        if (empty($reports)) {
            return get_string('noreports', 'corplms_reportbuilder');
        }

        $tableheader = array(get_string('name', 'corplms_reportbuilder'),
                             get_string('source', 'corplms_reportbuilder'),
                             get_string('options', 'corplms_reportbuilder'));
        $data = array();
        foreach ($reports as $report) {
            try {
                $row = array();
                $strsettings = get_string('settings', 'corplms_reportbuilder');
                $strdelete = get_string('delete', 'corplms_reportbuilder');
                $viewurl = new moodle_url(reportbuilder_get_report_url($report));
                $editurl = new moodle_url('/corplms/reportbuilder/general.php', array('id' => $report->id));
                $deleteurl = new moodle_url('/corplms/reportbuilder/index.php', array('id' => $report->id, 'd' => 1));
    ...
    public function embedded_reports_table($reports=array()) {
        ...
        if (empty($reports)) {
            return get_string('noembeddedreports', 'corplms_reportbuilder');
        }
        ...
        $tableheader = array(get_string('name', 'corplms_reportbuilder'),
                             ...
                             get_string('options', 'corplms_reportbuilder'));
        $strsettings = get_string('settings', 'corplms_reportbuilder');
        $strreload = get_string('restoredefaults', 'corplms_reportbuilder');
        ...
        foreach ($reports as $report) {
            ...
            $editurl = new moodle_url('/corplms/reportbuilder/general.php', array('id' => $report->id));
            $reloadurl = new moodle_url('/corplms/reportbuilder/index.php', array('id' => $report->id, 'em' => 1, 'd' => 1));
    ...
    public function activity_groups_table($groups) {
        ...
        if (empty($groups)) {
            return html_writer::tag('p', get_string('nogroups', 'corplms_reportbuilder'));
        }

        $tableheader = array(get_string('name', 'corplms_reportbuilder'),
                             get_string('tag'),
                             get_string('baseitem', 'corplms_reportbuilder'),
                             get_string('activities', 'corplms_reportbuilder'),
                             get_string('reports', 'corplms_reportbuilder'),
                             get_string('options', 'corplms_reportbuilder'));
        ...
        foreach ($groups as $group) {
            ...
            $strsettings = get_string('settings', 'corplms_reportbuilder');
            $strdelete = get_string('delete', 'corplms_reportbuilder');
            $strcron = get_string('refreshdataforthisgroup', 'corplms_reportbuilder');

            $settingsurl = new moodle_url('/corplms/reportbuilder/groupsettings.php', array('id' => $group->id));
            ...
            $deleteurl = new moodle_url('/corplms/reportbuilder/groups.php', array('id' => $group->id, 'd' => 1));
            $deleteattr = array('title' => $strdelete);
            $delete = $this->output->action_icon($deleteurl, new pix_icon('t/delete', $strdelete), null, $deleteattr);

            $cronurl = new moodle_url('/corplms/reportbuilder/runcron.php', array('group' => $group->id, 'sesskey' => $USER->sesskey));
            $options['height'] = 500;
            $options['width'] = 750;
            ...
            $url = new moodle_url('/corplms/reportbuilder/groupsettings.php', array('id' => $group->id));
    ...
    public function activity_group_activities_table($activities) {
        ...
        $tableheader = array(get_string('course'),
                         get_string('feedback'),
                         get_string('lastchecked', 'corplms_reportbuilder'),
                         get_string('disabled', 'corplms_reportbuilder'));
        ...
        foreach ($activities as $activity) {
            ...
            if ($activity->course !== null) {
                ...
                $row[] = get_string('coursenotset', 'corplms_reportbuilder');
            }
            ...
            if ($activity->lastchecked !== null) {
                ...
                $row[] = get_string('notyetchecked', 'corplms_reportbuilder');
            }
    ...
    public function activity_group_reports_table($reports) {
        if (empty($reports)) {
            return html_writer::tag('p', get_string('noreportscount', 'corplms_reportbuilder'));
        }

        echo html_writer::tag('p', get_string('reportcount', 'corplms_reportbuilder', count($reports)));
    ...
    public function export_select($id, $sid = 0) {
        ...
        require_once($CFG->dirroot . '/corplms/reportbuilder/export_form.php');
    ...
    public function view_report_link($reporturl) {
        ...
        return html_writer::link($url, get_string('viewreport', 'corplms_reportbuilder'));
    }
    ...
    public function cache_pending_notification($report = 0) {
        ...
        if ($report instanceof reportbuilder) {
            ...
            if ($status == RB_CACHE_FLAG_FAIL) {
                $notice = $this->container(get_string('cachegenfail','corplms_reportbuilder'), 'notifyproblem clearfix');
            } else if ($status == RB_CACHE_FLAG_GEN) {
                $time = userdate($report->cacheschedule->genstart);
                $notice = $this->container(get_string('cachegenstarted','corplms_reportbuilder', $time), 'notifynotice clearfix');
            } else if ($status == RB_CACHE_FLAG_CHANGED) {
                $context = context_system::instance();
                if ($report->_id > 0 && has_capability('corplms/reportbuilder:managereports', $context)) {
                ...
                $notice = $this->container(get_string('cachepending','corplms_reportbuilder', $button),
    ...
    public function cachenow_button($reportid, $icon = false) {
        ...
        if (!$cachenowinit) {
            ...
            $PAGE->requires->strings_for_js(array('cachenow_title'), 'corplms_reportbuilder');
            $PAGE->requires->string_for_js('ok', 'moodle');
            $strcache = get_string('cachenow', 'corplms_reportbuilder');
            local_js(array(CORPLMS_JS_DIALOG));
            $jsmodule = array(
                'name' => 'corplms_reportbuilder_cachenow',
                'fullpath' => '/corplms/reportbuilder/js/cachenow.js',
                'requires' => array('json'));
            $args = array('args'=>json_encode(array('reportid' => $reportid)));
            $PAGE->requires->js_init_call('M.corplms_reportbuilder_cachenow.init', $args, false, $jsmodule);
        }
    ...
    public function view_all_reports_link() {
        $url = new moodle_url('/corplms/reportbuilder/');
        return '&laquo; ' . html_writer::link($url, get_string('allreports', 'corplms_reportbuilder'));
    }
    ...
    public function save_button($report) {
        ...
        if ($search || $hasrequiredurlparams) {
            ...
            return $this->output->single_button(new moodle_url('/corplms/reportbuilder/save.php', $params),
                    get_string('savesearch', 'corplms_reportbuilder'), 'get');
        } else {
            return '';
        }
    }
    ...
    public function showhide_button($reportid, $reportshortname) {
        ...
        $html .= html_writer::empty_tag('input', array('type' => 'button',
            ...
            'value' => get_string('showhidecolumns', 'corplms_reportbuilder')
        ));
    ...
    public function manage_search_button($report) {
        ...
        $html .= html_writer::empty_tag('input', array('type' => 'button',
            ...
            'value' => get_string('managesavedsearches', 'corplms_reportbuilder')
        ));
        $html .= html_writer::end_tag('form');
        $html .= html_writer::end_tag('div');
    ...
    public function print_description($description, $reportid) {
        ...
        $description = file_rewrite_pluginfile_urls($description, 'pluginfile.php', $sitecontext->id, 'corplms_reportbuilder', 'report_builder', $reportid);
    ...
    public function print_result_count_string($countfiltered, $countall) {
        ...
        if ($countfiltered == $countall) {
            $heading = get_string('x' . $resultstr, 'corplms_reportbuilder', $countall);
        } else {
            $a = new stdClass();
            $a->filtered = $countfiltered;
            $a->unfiltered = $countall;
            $heading = get_string('xofy' . $resultstr, 'corplms_reportbuilder', $a);
        }
    ...
    public function saved_searches_table($searches, $report) {
        $tableheader = array(get_string('name', 'corplms_reportbuilder'),
                             get_string('publicsearch', 'corplms_reportbuilder'),
                             get_string('options', 'corplms_reportbuilder'));
        $data = array();
        ...
        foreach ($searches as $search) {
            $editurl = new moodle_url('/corplms/reportbuilder/savedsearches.php',
                array('id' => $search->reportid, 'action' => 'edit', 'sid' => $search->id));
            $deleteurl = new moodle_url('/corplms/reportbuilder/savedsearches.php',
