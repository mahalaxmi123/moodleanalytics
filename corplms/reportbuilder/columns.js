 ...
 */
M.corplms_reportbuildercolumns = M.corplms_reportbuildercolumns || {

    Y: null,
    // optional php params and defaults defined here, args passed to init method
    // below will override these values
    config: {},
    loadingimg: '<img src="'+M.util.image_url('i/ajaxloader', 'moodle')+'" alt="'+M.util.get_string('saving', 'corplms_reportbuilder')+'" class="iconsmall" />',
    advoptionshtml : '',
    ...
    init: function(Y, config){
        ...
        if (typeof $ === 'undefined') {
            throw new Error('M.corplms_reportbuildercolumns.init()-> jQuery dependency required for this module to function.');
        }
        ...
        if (colName == '0') {
            ...
            if (advancedSelectorVal != '') {
                if ($.inArray(advancedSelectorVal, module.config.rb_allowed_advanced[colName]) == -1) {
                    ...
                    newHeading = M.util.get_string(strName, 'corplms_reportbuilder', newHeading);
                }
            }
    ...
    rb_init_addbutton: function(colselector){
        ...
        addbutton.bind('click', function(e) {
            ...
            e.preventDefault();
            $.ajax({
                url: M.cfg.wwwroot + '/corplms/reportbuilder/ajax/column.php',
                type: "POST",
                data: data,
                beforeSend: function() {
    ...
    rb_init_deletebuttons: function() {
        ...
        $('.reportbuilderform table .deletecolbtn').bind('click', function(e) {
            ...
            confirmed = confirm(M.util.get_string('confirmcoldelete', 'corplms_reportbuilder'));
    ...
    rb_get_btn_hide: function(reportid, colid) {
        return $('<a href="' + M.cfg.wwwroot + '/corplms/reportbuilder/columns.php?id=' + reportid + '&cid='+colid+'&h=1" class="hidecolbtn action-icon"><img src="' + M.util.image_url('t/hide', 'moodle') +'" alt="' + M.util.get_string('hide', 'corplms_reportbuilder') + '" class="iconsmall" /></a>');
    },

    rb_get_btn_show: function(reportid, colid) {
        return $('<a href="' + M.cfg.wwwroot + '/corplms/reportbuilder/columns.php?id=' + reportid + '&cid='+colid+'&h=0" class="showcolbtn action-icon"><img src="' + M.util.image_url('t/show', 'moodle') +'" alt="' + M.util.get_string('show', 'corplms_reportbuilder') + '" class="iconsmall" /></a>');
    },

    rb_get_btn_delete: function(reportid, colid) {
        return $('<a href="' + M.cfg.wwwroot + '/corplms/reportbuilder/columns.php?id=' + reportid + '&cid='+colid+'&d=1" class="deletecolbtn action-icon"><img src="' + M.util.image_url('t/delete', 'moodle') +'" alt="' + M.util.get_string('delete', 'corplms_reportbuilder') + '" class="iconsmall" /></a>');
    },

    rb_get_btn_up: function(reportid, colid) {
        return $('<a href="' + M.cfg.wwwroot + '/corplms/reportbuilder/columns.php?id=' + reportid + '&cid='+colid+'&m=up" class="movecolupbtn action-icon"><img src="' + M.util.image_url('t/up', 'moodle') +'" alt="' + M.util.get_string('moveup', 'corplms_reportbuilder') + '" class="iconsmall" /></a>');
    },

    rb_get_btn_down: function(reportid, colid) {
        return $('<a href="' + M.cfg.wwwroot + '/corplms/reportbuilder/columns.php?id=' + reportid + '&cid='+colid+'&m=down" class="movecoldownbtn action-icon"><img src="' + M.util.image_url('t/down', 'moodle') +'" alt="' + M.util.get_string('movedown', 'corplms_reportbuilder') + '" class="iconsmall" /></a>');
    },

    rb_get_btn_add: function(reportid) {
        return $('<a href="' + M.cfg.wwwroot + '/corplms/reportbuilder/columns.php?id=' + reportid + '" class="addcolbtn"><input type="button" value="' + M.util.get_string('add', 'corplms_reportbuilder') + '" /></a>');
    }
};
