...
$returnurl = new moodle_url('/corplms/reportbuilder/columns.php', array('id' => $id));
...
if ($h !== null && isset($cid)) {
    if ($report->showhide_column($cid, $h)) {
        \corplms_reportbuilder\event\report_updated::create_from_report($report, 'columns')->trigger();
        corplms_set_notification(get_string('column_vis_updated', 'corplms_reportbuilder'), $returnurl, array('class' => 'notifysuccess'));
    } else {
        corplms_set_notification(get_string('error:column_vis_not_updated', 'corplms_reportbuilder'), $returnurl);
    }
}
...
if ($d and $cid) {
    if ($confirm and confirm_sesskey()) {
        if ($report->delete_column($cid)) {
            \corplms_reportbuilder\event\report_updated::create_from_report($report, 'columns')->trigger();
            corplms_set_notification(get_string('column_deleted', 'corplms_reportbuilder'), $returnurl, array('class' => 'notifysuccess'));
        } else {
            corplms_set_notification(get_string('error:column_not_deleted', 'corplms_reportbuilder'), $returnurl);
        }
    }
    echo $output->header();

    $confirmurl = new moodle_url('/corplms/reportbuilder/columns.php', array('d' => '1', 'id' => $id, 'cid' => $cid,
                                                                            'confirm' => '1', 'sesskey' => $USER->sesskey));
    echo $output->confirm(get_string('confirmcolumndelete', 'corplms_reportbuilder'), $confirmurl, $returnurl);
...
if ($m and $cid and confirm_sesskey()) {
    if ($report->move_column($cid, $m)) {
        \corplms_reportbuilder\event\report_updated::create_from_report($report, 'columns')->trigger();
        corplms_set_notification(get_string('column_moved', 'corplms_reportbuilder'), $returnurl, array('class' => 'notifysuccess'));
    } else {
        corplms_set_notification(get_string('error:column_not_moved', 'corplms_reportbuilder'), $returnurl);
    }
}
...
if ($fromform = $mform->get_data()) {
    ...
    if (corplms_reportbuilder_build_columns($fromform, $report, $allowedadvanced, $grouped)) {
        reportbuilder_set_status($id);
        $report = new reportbuilder($id);
        \corplms_reportbuilder\event\report_updated::create_from_report($report, 'columns')->trigger();
        corplms_set_notification(get_string('columns_updated', 'corplms_reportbuilder'), $returnurl, array('class' => 'notifysuccess'));
    } else {
        corplms_set_notification(get_string('error:columns_not_updated', 'corplms_reportbuilder'), $returnurl);
    }
...
$jsmodule = array(
    'name' => 'corplms_reportbuildercolumns',
    'fullpath' => '/corplms/reportbuilder/columns.js');
$PAGE->requires->js_init_call('M.corplms_reportbuildercolumns.init', array($config), false, $jsmodule);
 ...
 */
function corplms_reportbuilder_build_columns($fromform, reportbuilder $report, $allowedadvanced, $grouped) {
    global $DB;
