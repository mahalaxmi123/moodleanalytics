...
$returnurl = $CFG->wwwroot . '/corplms/reportbuilder/groupsettings.php';
...
echo $output->single_button($CFG->wwwroot . '/corplms/reportbuilder/groups.php', get_string('backtoallgroups', 'corplms_reportbuilder'), 'get');

echo $output->heading(get_string('activitygroupingx', 'corplms_reportbuilder', $group->name));

echo $output->heading(get_string('assignedactivities', 'corplms_reportbuilder'), 3);

$info = new stdClass();
$info->count = count($activities);
$info->tag = $tag->name;
echo html_writer::tag('p', get_string('groupcontents', 'corplms_reportbuilder', $info));

if (count($activities)) {
    echo $output->activity_group_activities_table($activities);
}

echo $output->heading(get_string('baseactivity', 'corplms_reportbuilder'), 3);

$info = new stdClass();
$info->url = $CFG->wwwroot . '/mod/feedback/view.php?id=' . $baseitem->cmid;
$info->activity = $baseitem->feedback;
echo html_writer::tag('p', get_string('baseitemdesc', 'corplms_reportbuilder', $info));

echo $output->heading(get_string('reports', 'corplms_reportbuilder'), 3);
