...
class scheduled_reports_new_form extends moodleform {
    function definition() {
        ...
        $mform->addElement('header', 'general', get_string('scheduledreportsettings', 'corplms_reportbuilder'));

        $mform->addElement('static', 'report', get_string('report', 'corplms_reportbuilder'), $report->fullname);
        if (empty($savedsearches)) {
            $mform->addElement('static', '', get_string('data', 'corplms_reportbuilder'),
                    html_writer::div(get_string('scheduleneedssavedfilters', 'corplms_reportbuilder', $report->report_url()),
                            'notifyproblem'));
        } else {
            $mform->addElement('select', 'savedsearchid', get_string('data', 'corplms_reportbuilder'), $savedsearches);
        }
        $mform->addElement('select', 'format', get_string('export', 'corplms_reportbuilder'), $exportformatselect);

        if ($exporttofilesystemenabled) {
            $exporttosystemarray = array();
            $exporttosystemarray[] =& $mform->createElement('radio', 'emailsaveorboth', '',
                    get_string('exporttoemail', 'corplms_reportbuilder'), REPORT_BUILDER_EXPORT_EMAIL);
            $exporttosystemarray[] =& $mform->createElement('radio', 'emailsaveorboth', '',
                    get_string('exporttoemailandsave', 'corplms_reportbuilder'), REPORT_BUILDER_EXPORT_EMAIL_AND_SAVE);
            $exporttosystemarray[] =& $mform->createElement('radio', 'emailsaveorboth', '',
                    get_string('exporttosave', 'corplms_reportbuilder'), REPORT_BUILDER_EXPORT_SAVE);
            $mform->setDefault('emailsaveorboth', $exporttofilesystem);
            $mform->addGroup($exporttosystemarray, 'exporttosystemarray',
                    get_string('exportfilesystemoptions', 'corplms_reportbuilder'), array('<br />'), false);
        } else {
            $mform->addElement('hidden', 'emailsaveorboth', REPORT_BUILDER_EXPORT_EMAIL);
            $mform->setType('emailsaveorboth', PARAM_TEXT);
        }

        $mform->addElement('scheduler', 'schedulegroup', get_string('schedule', 'corplms_reportbuilder'),
...
class scheduled_reports_add_form extends moodleform {
    function definition() {
        ...
        if (!empty($reportselect)) {
            $mform->addElement('select', 'reportid', get_string('addnewscheduled', 'corplms_reportbuilder'), $reportselect);
            $mform->addElement('submit', 'submitbutton', get_string('addscheduledreport', 'corplms_reportbuilder'));
