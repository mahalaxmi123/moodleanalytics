...
include_once($CFG->dirroot . '/corplms/reportbuilder/classes/rb_base_content.php');
...
class report_builder_new_form extends moodleform {
    ...
    function definition() {
        ...
        $mform->addElement('header', 'general', get_string('newreport', 'corplms_reportbuilder'));
        $sources = reportbuilder::get_source_list();
        if (count($sources) > 0) {

            $mform->addElement('text', 'fullname', get_string('reportname', 'corplms_reportbuilder'), 'maxlength="255"');
            $mform->setType('fullname', PARAM_TEXT);
            $mform->addRule('fullname', null, 'required');
            $mform->addHelpButton('fullname', 'reportbuilderfullname', 'corplms_reportbuilder');

            $pick = array(0 => get_string('selectsource', 'corplms_reportbuilder'));
            $select = array_merge($pick, $sources);
            $mform->addElement('select', 'source', get_string('source', 'corplms_reportbuilder'), $select);
            // invalid if not set
            $mform->addRule('source', get_string('error:mustselectsource', 'corplms_reportbuilder'), 'regex', '/[^0]+/');
            $mform->addHelpButton('source', 'reportbuildersource', 'corplms_reportbuilder');

            $mform->addElement('advcheckbox', 'hidden', get_string('hidden', 'corplms_reportbuilder'), '', null, array(0, 1));
            $mform->addHelpButton('hidden', 'reportbuilderhidden', 'corplms_reportbuilder');
            $this->add_action_buttons(true, get_string('createreport', 'corplms_reportbuilder'));

        } else {
            $mform->addElement('html', get_string('error:nosources', 'corplms_reportbuilder'));
        }
    }
...
class report_builder_edit_form extends moodleform {
    function definition() {
        ...
        $mform->addElement('header', 'general', get_string('reportsettings', 'corplms_reportbuilder'));

        $mform->addElement('text', 'fullname', get_string('reporttitle', 'corplms_reportbuilder'), array('size' => '30'));
        $mform->setType('fullname', PARAM_TEXT);
        $mform->addRule('fullname', null, 'required');
        $mform->addHelpButton('fullname', 'reportbuilderfullname', 'corplms_reportbuilder');
        ...
        $mform->addHelpButton('description_editor', 'reportbuilderdescription', 'corplms_reportbuilder');

        $mform->addElement('static', 'reportsource', get_string('source', 'corplms_reportbuilder'), $report->src->sourcetitle);
        $mform->addHelpButton('reportsource', 'reportbuildersource', 'corplms_reportbuilder');
        ...
        $mform->addElement('text', 'recordsperpage', get_string('recordsperpage', 'corplms_reportbuilder'), array('size' => '6', 'maxlength' => 4));
        $mform->setType('recordsperpage', PARAM_INT);
        $mform->addRule('recordsperpage', null, 'numeric');
        $mform->addHelpButton('recordsperpage', 'reportbuilderrecordsperpage', 'corplms_reportbuilder');

        $reporttype = ($report->embeddedurl === null) ? get_string('usergenerated', 'corplms_reportbuilder') :
            get_string('embedded', 'corplms_reportbuilder');

        $mform->addElement('static', 'reporttype', get_string('reporttype', 'corplms_reportbuilder'), $reporttype);
...
class report_builder_edit_filters_form extends moodleform {
    function definition() {
        ...
        $strmovedown = get_string('movedown', 'corplms_reportbuilder');
        $strmoveup = get_string('moveup', 'corplms_reportbuilder');
        $strdelete = get_string('delete', 'corplms_reportbuilder');
        ...
        $mform->addElement('header', 'standardfilter', get_string('standardfilter', 'corplms_reportbuilder'));
        $mform->addHelpButton('standardfilter', 'standardfilter', 'corplms_reportbuilder');
        ...
        if (isset($report->filteroptions) && is_array($report->filteroptions) && count($report->filteroptions) > 0) {
            ...
            $mform->addElement('html', $OUTPUT->container(get_string('standardfilterdesc', 'corplms_reportbuilder')) .
                    html_writer::empty_tag('br'));

            $mform->addElement('html', $OUTPUT->container_start('reportbuilderform') . html_writer::start_tag('table') .
                    html_writer::start_tag('tr') . html_writer::tag('th', get_string('searchfield', 'corplms_reportbuilder')) .
                    html_writer::tag('th', get_string('customisename', 'corplms_reportbuilder'), array('colspan' => 2)) .
                    html_writer::tag('th', get_string('advanced', 'corplms_reportbuilder')) .
                    html_writer::tag('th', get_string('options', 'corplms_reportbuilder')) . html_writer::end_tag('tr'));
            ...
            if (isset($report->filters) && is_array($report->filters) && count($report->filters) > 0) {
                ...
                foreach ($filters as $index => $filter) {
                    ...
                    $mform->addElement('html', html_writer::end_tag('td') . html_writer::start_tag('td'));
                    $deleteurl = new moodle_url('/corplms/reportbuilder/filters.php',
                    ...
                    if ($i != 1) {
                        $moveupurl = new moodle_url('/corplms/reportbuilder/filters.php',
                    ...
                    if ($i != $standardfiltercount) {
                        $movedownurl = new moodle_url('/corplms/reportbuilder/filters.php',
                ...
                $mform->addElement('html', html_writer::tag('p', get_string('nofiltersyet', 'corplms_reportbuilder')));
            }
            ...
            $mform->addElement('header', 'sidebarfilter', get_string('sidebarfilter', 'corplms_reportbuilder'));
            $mform->addHelpButton('sidebarfilter', 'sidebarfilter', 'corplms_reportbuilder');
            $mform->setExpanded('sidebarfilter');

            $mform->addElement('html', $OUTPUT->container(get_string('sidebarfilterdesc', 'corplms_reportbuilder')) .
                    html_writer::empty_tag('br'));
            ...
            $mform->addElement('html', get_string('nofilteraskdeveloper', 'corplms_reportbuilder', $report->source));
        }

        // Toolbar search options.
        $mform->addElement('header', 'toolbarsearch', get_string('toolbarsearch', 'corplms_reportbuilder'));
        $mform->addHelpButton('toolbarsearch', 'toolbarsearch', 'corplms_reportbuilder');
        $mform->setExpanded('toolbarsearch');

        $mform->addElement('advcheckbox', 'toolbarsearchdisabled', get_string('toolbarsearchdisabled', 'corplms_reportbuilder'));
        $mform->setDefault('toolbarsearchdisabled', !$report->toolbarsearch);
        $mform->addHelpButton('toolbarsearchdisabled', 'toolbarsearchdisabled', 'corplms_reportbuilder');

        if (count($allsearchcolumns) > 0) {
            $searchcolumns = $report->searchcolumns;

            $mform->addElement('html', $OUTPUT->container(get_string('toolbarsearchdesc', 'corplms_reportbuilder')) .
                    html_writer::empty_tag('br'));
            ...
            if (isset($report->searchcolumns) && is_array($report->searchcolumns) && count($report->searchcolumns) > 0) {
                ...
                $mform->addElement('html', html_writer::tag('p', get_string('nosearchcolumnsyet', 'corplms_reportbuilder')));
            }
            ...
            $mform->addElement('html', get_string('nosearchcolumnsaskdeveloper', 'corplms_reportbuilder', $report->source));
        }
...
class report_builder_edit_columns_form extends moodleform {
    ...
    function definition() {
        ...
        $mform->addElement('header', 'reportcolumns', get_string('reportcolumns', 'corplms_reportbuilder'));

        $mform->addHelpButton('reportcolumns', 'reportbuildercolumns', 'corplms_reportbuilder');

        if (isset($report->columnoptions) && is_array($report->columnoptions) && count($report->columnoptions) > 0) {


            $mform->addElement('html', $OUTPUT->container(get_string('help:columnsdesc', 'corplms_reportbuilder')) .
                html_writer::empty_tag('br'));


            $mform->addElement('html', $OUTPUT->container_start('reportbuilderform') . html_writer::start_tag('table') .
                html_writer::start_tag('tr') . html_writer::tag('th', get_string('column', 'corplms_reportbuilder')) .
                html_writer::tag('th', get_string('advancedcolumnheading', 'corplms_reportbuilder')) .
                html_writer::tag('th', get_string('customiseheading', 'corplms_reportbuilder'), array('colspan' => 2)) .
                html_writer::tag('th', get_string('options', 'corplms_reportbuilder') . html_writer::end_tag('tr')));
            ...
            if ($goodcolumns) {
                ...
                foreach ($goodcolumns as $cid => $column) {
                    ...
                    if (!isset($column->required) || !$column->required) {
                        ...
                        // show/hide link
                        if ($column->hidden == 0) {
                            $hideurl = new moodle_url('/corplms/reportbuilder/columns.php',
                            ...
                            $showurl = new moodle_url('/corplms/reportbuilder/columns.php',
                        ...
                        // delete link
                        $delurl = new moodle_url('/corplms/reportbuilder/columns.php',
                        ...
                        // move up link
                        if ($i != 1) {
                            $moveupurl = new moodle_url('/corplms/reportbuilder/columns.php',
                        ...
                        // move down link
                        if ($i != $colcount) {
                            $movedownurl = new moodle_url('/corplms/reportbuilder/columns.php',
                ...
                $mform->addElement('html', html_writer::tag('p', get_string('nocolumnsyet', 'corplms_reportbuilder')));
            }

            $mform->addElement('html', html_writer::start_tag('tr') . html_writer::start_tag('td'));
            $newcolumnsselect = array_merge(
                array(
                    get_string('new') => array(0 => get_string('addanothercolumn', 'corplms_reportbuilder'))
                ),
                $columnsselect);
            ...
            if ($badcolumns) {
                $mform->addElement('header', 'badcols', get_string('badcolumns', 'corplms_reportbuilder'));
                $mform->addElement('html', html_writer::tag('p', get_string('badcolumnsdesc', 'corplms_reportbuilder')));

                $mform->addElement('html',
                    $OUTPUT->container_start('reportbuilderform') . html_writer::start_tag('table') . html_writer::start_tag('tr') .
                    html_writer::tag('th', get_string('type', 'corplms_reportbuilder')) .
                    html_writer::tag('th', get_string('value', 'corplms_reportbuilder')) .
                    html_writer::tag('th', get_string('heading', 'corplms_reportbuilder')) .
                    html_writer::tag('th', get_string('options', 'corplms_reportbuilder')) . html_writer::end_tag('tr'));
                foreach ($badcolumns as $bad) {
                    $deleteurl = new moodle_url('/corplms/reportbuilder/columns.php',
            ...
            $mform->addElement('header', 'sorting', get_string('sorting', 'corplms_reportbuilder'));
            $mform->addHelpButton('sorting', 'reportbuildersorting', 'corplms_reportbuilder');

            $pick = array('' => get_string('noneselected', 'corplms_reportbuilder'));
            $select = array_merge($pick, $columnoptions);
            $mform->addElement('select', 'defaultsortcolumn', get_string('defaultsortcolumn', 'corplms_reportbuilder'), $select);
            $mform->setDefault('defaultsortcolumn', $report->defaultsortcolumn);


            $radiogroup = array();
            $radiogroup[] =& $mform->createElement('radio', 'defaultsortorder', '', get_string('ascending', 'corplms_reportbuilder'), SORT_ASC);
            $radiogroup[] =& $mform->createElement('radio', 'defaultsortorder', '', get_string('descending', 'corplms_reportbuilder'), SORT_DESC);
            $mform->addGroup($radiogroup, 'radiogroup', get_string('defaultsortorder', 'corplms_reportbuilder'), html_writer::empty_tag('br'), false);
            $mform->setDefault('defaultsortorder', $report->defaultsortorder);
        } else {
            $mform->addElement('html', get_string('error:nocolumns', 'corplms_reportbuilder', $report->source));
        }
        ...
        foreach ($data as $key => $value) {
            ...
            if (preg_match('/^column\d+/', $key) or ($key === 'newcolumns' and $value)) {
                if (isset($usedcols[$value])) {
                    $errors[$key] = get_string('norepeatcols', 'corplms_reportbuilder');
                } else {
                    $usedcols[$value] = true;
                }
                continue;
            ...
            if (preg_match('/^heading(\d+)/', $key, $matches)) {
                ...
                if ($data['customheading'.$cid]) {
                    if (trim($value) == '') {
                        $errors[$key] = get_string('noemptycols', 'corplms_reportbuilder');
                    }
                }
                continue;
            }
...
class report_builder_edit_graph_form extends moodleform {
    public function definition() {
        ...
        $types = array(
            '' => get_string('none'),
            'column' => get_string('graphtypecolumn', 'corplms_reportbuilder'),
            'line' => get_string('graphtypeline', 'corplms_reportbuilder'),
            'bar' => get_string('graphtypebar', 'corplms_reportbuilder'),
            'pie' => get_string('graphtypepie', 'corplms_reportbuilder'),
            'scatter' => get_string('graphtypescatter', 'corplms_reportbuilder'),
            'area' => get_string('graphtypearea', 'corplms_reportbuilder'),
        );
        $mform->addElement('select', 'type', get_string('graphtype', 'corplms_reportbuilder'), $types);

        $optionoptions = array(
            'C' => get_string('graphorientationcolumn', 'corplms_reportbuilder'),
            'R' => get_string('graphorientationrow', 'corplms_reportbuilder'),
        );
        $mform->addElement('select', 'orientation', get_string('graphorientation', 'corplms_reportbuilder'), $optionoptions);
        $mform->addHelpButton('orientation', 'graphorientation', 'corplms_reportbuilder');

        $mform->addElement('header', 'serieshdr', 'Data');

        $catoptions = array('none' => get_string('graphnocategory', 'corplms_reportbuilder'));
        $legendoptions = array();
        ...
        $mform->addElement('select', 'category', get_string('graphcategory', 'corplms_reportbuilder'), $catoptions);
        $mform->disabledIf('category', 'type', 'eq', '');
        $mform->disabledIf('category', 'orientation', 'noteq', 'C');

        $mform->addElement('select', 'legend', get_string('graphlegend', 'corplms_reportbuilder'), $legendoptions);
        ...
        $mform->addElement('select', 'series', get_string('graphseries', 'corplms_reportbuilder'), $series, array('multiple' => true));
        $mform->disabledIf('series', 'type', 'eq', '');

        $mform->addElement('advcheckbox', 'stacked', get_string('graphstacked', 'corplms_reportbuilder'));
        ...
        $mform->addElement('textarea', 'settings', get_string('graphsettings', 'corplms_reportbuilder'), array('rows' => 10));
        $mform->addHelpButton('settings', 'graphsettings', 'corplms_reportbuilder');
...
class report_builder_edit_content_form extends moodleform {
    function definition() {
        ...
        $mform->addElement('header', 'contentheader', get_string('contentcontrols', 'corplms_reportbuilder'));

        if (count($contentoptions)) {
            if ($report->embeddedurl !== null) {
                $mform->addElement('html', html_writer::tag('p', get_string('embeddedcontentnotes', 'corplms_reportbuilder')));
            }

            $radiogroup = array();
            $radiogroup[] =& $mform->createElement('radio', 'contentenabled', '', get_string('nocontentrestriction', 'corplms_reportbuilder'), 0);
            $radiogroup[] =& $mform->createElement('radio', 'contentenabled', '', get_string('withcontentrestrictionany', 'corplms_reportbuilder'), 1);
            $radiogroup[] =& $mform->createElement('radio', 'contentenabled', '', get_string('withcontentrestrictionall', 'corplms_reportbuilder'), 2);
            $mform->addGroup($radiogroup, 'radiogroup', get_string('restrictcontent', 'corplms_reportbuilder'), html_writer::empty_tag('br'), false);
            $mform->addHelpButton('radiogroup', 'reportbuildercontentmode', 'corplms_reportbuilder');
            ...
            $mform->addElement('html',
                ...
                'corplms_reportbuilder', $report->source));
        }
    }
}
...
class report_builder_edit_access_form extends moodleform {
    function definition() {
        ...
        $mform->addElement('header', 'access', get_string('accesscontrols', 'corplms_reportbuilder'));

        if ($report->embeddedurl !== null) {
            $mform->addElement('html', html_writer::tag('p', get_string('embeddedaccessnotes', 'corplms_reportbuilder')));
        }
        $radiogroup = array();
        $radiogroup[] =& $mform->createElement('radio', 'accessenabled', '', get_string('norestriction', 'corplms_reportbuilder'), 0);
        $radiogroup[] =& $mform->createElement('radio', 'accessenabled', '', get_string('withrestriction', 'corplms_reportbuilder'), 1);
        $mform->addGroup($radiogroup, 'radiogroup', get_string('restrictaccess', 'corplms_reportbuilder'), html_writer::empty_tag('br'), false);
        $mform->setDefault('accessenabled', $DB->get_field('report_builder', 'accessmode', array('id' => $id)));
        $mform->addHelpButton('radiogroup', 'reportbuilderaccessmode', 'corplms_reportbuilder');
...
class report_builder_edit_performance_form extends moodleform {
    function definition() {
        ...
        $mform->addElement('header', 'general', get_string('initialdisplay_heading', 'corplms_reportbuilder'));
        $initial_display_attributes = sizeof($report->filters) < 1 ? array('disabled' => 'disabled', 'group' => null) : null;
        $initial_display_sidenote = is_null($initial_display_attributes) ? '' : get_string('initialdisplay_disabled', 'corplms_reportbuilder');
        $mform->addElement('advcheckbox', 'initialdisplay', get_string('initialdisplay', 'corplms_reportbuilder'),
        ...
        $mform->addHelpButton('initialdisplay', 'initialdisplay', 'corplms_reportbuilder');

        $mform->addElement('header', 'general', get_string('reportbuildercache_heading', 'corplms_reportbuilder'));
        if (!empty($CFG->enablereportcaching)) {
            ...
            $caching_sidenote = is_null($caching_attributes) ? '' :
                    get_string('reportbuildercache_disabled', 'corplms_reportbuilder');
            $mform->addElement('advcheckbox', 'cache', get_string('cache', 'corplms_reportbuilder'),
                    $caching_sidenote, $caching_attributes, array(0, 1));
            $mform->setType('cache', PARAM_INT);
            $mform->addHelpButton('cache', 'reportbuildercache', 'corplms_reportbuilder');

            $mform->addElement('scheduler', 'schedulegroup', get_string('reportbuildercachescheduler', 'corplms_reportbuilder'));
            $mform->disabledIf('schedulegroup', 'cache');
            $mform->addHelpButton('schedulegroup', 'reportbuildercachescheduler', 'corplms_reportbuilder');

            $mform->addElement('static', 'servertime', get_string('reportbuildercacheservertime', 'corplms_reportbuilder'),
                    date_format_string(time(), get_string('strftimedaydatetime', 'langconfig')));
            $mform->addHelpButton('servertime', 'reportbuildercacheservertime', 'corplms_reportbuilder');

            $usertz = corplms_get_clean_timezone();
            $cachetime = isset($report->cacheschedule->lastreport) ? $report->cacheschedule->lastreport : 0;
            $cachedstr = get_string('lastcached','corplms_reportbuilder', userdate($cachetime, '', $usertz));
            $notcachedstr = get_string('notcached','corplms_reportbuilder');
            $lastcached = ($cachetime > 0) ? $cachedstr : $notcachedstr;

            if ($report->cache) {
                $mform->addElement('static', 'cachenowselector', get_string('reportbuilderinitcache', 'corplms_reportbuilder'),
                    html_writer::tag('span', $lastcached. ' ') .
                    $output->cachenow_button($id)
                );
            } else {
                $mform->addElement('advcheckbox', 'generatenow', get_string('cachenow', 'corplms_reportbuilder'), '', null, array(0, 1));
                $mform->setType('generatenow', PARAM_INT);
                $mform->addHelpButton('generatenow', 'cachenow', 'corplms_reportbuilder');
            ...
            $mform->addElement('static', 'reportcachingdisabled', '', get_string('reportcachingdisabled', 'corplms_reportbuilder', $enablelink->out()));
        }
...
function validate_shortname($data) {
    ...
    if (count($foundreports)) {
        ...
        if (!empty($foundreports)) {
            $errors['shortname'] = get_string('shortnametaken', 'corplms_reportbuilder');
        }
    }
    return $errors;
...
function validate_unique_filters($data) {
    ...
    foreach ($currentfilters as $filt) {
        ...
        if (isset($data[$field])) {
            if (array_key_exists($data[$field], $used_filters)) {
                $errors[$field] = get_string('norepeatfilters', 'corplms_reportbuilder');
            } else {
    ...
    if (isset($data['newfilter'])) {
        if (array_key_exists($data['newfilter'], $used_filters)) {
            $errors['newfilter'] = get_string('norepeatfilters', 'corplms_reportbuilder');
        }
    }
    return $errors;
}
...
class report_builder_save_form extends moodleform {
    function definition() {
        ...
        if ($data->sid) {
            $mform->addElement('header', 'savesearch', get_string('editingsavedsearch', 'corplms_reportbuilder'));
        } else {
            $mform->addElement('header', 'savesearch', get_string('createasavedsearch', 'corplms_reportbuilder'));
        }
        $mform->addElement('static', 'description', '', get_string('savedsearchdesc', 'corplms_reportbuilder'));
        $mform->addElement('static', 'params', get_string('currentsearchparams', 'corplms_reportbuilder'), $params);
        $mform->addElement('text', 'name', get_string('searchname', 'corplms_reportbuilder'));
        $mform->setType('name', PARAM_TEXT);
        $mform->addRule('name', get_string('missingsearchname', 'corplms_reportbuilder'), 'required', null, 'server');
        $mform->addElement('advcheckbox', 'ispublic', get_string('publicallyavailable', 'corplms_reportbuilder'), '', null, array(0, 1));
        $mform->addElement('hidden', 'id');
        $mform->setType('id', PARAM_INT);
...
class report_builder_standard_search_form extends moodleform {
    function definition() {
        ...
        if ($fields && is_array($fields) && count($fields) > 0) {
            $mform->addElement('header', 'newfilterstandard', get_string('searchby', 'corplms_reportbuilder'));
            ...
            $submitgroup[] =& $mform->createElement('submit', 'addfilter',
                    get_string('search', 'corplms_reportbuilder'));
            $submitgroup[] =& $mform->createElement('submit', 'clearstandardfilters',
                    get_string('clearform', 'corplms_reportbuilder'));
...
class report_builder_sidebar_search_form extends moodleform {
    public function definition() {
        ...
        if ($fields && is_array($fields) && count($fields) > 0) {
            ...
            $mform->addElement('header', 'newfiltersidebar', get_string('filterby', 'corplms_reportbuilder'));
...
class report_builder_toolbar_search_form extends moodleform {
    public function definition() {
        ...
        $mform->addElement('text', 'toolbarsearchtext',
                get_string('searchby', 'corplms_reportbuilder'));
...
class report_builder_course_expand_form extends moodleform {
    public function definition() {
        ...
        if ($enroltype != '') {
            $mform->addElement('static', 'enroltype', get_string('courseenroltype', 'corplms_reportbuilder'), $enroltype);
        }
        if ($progress != '') {
            $mform->addElement('static', 'progress', get_string('courseprogress', 'corplms_reportbuilder'), $progress);
        }
        if ($enddate != '') {
            $mform->addElement('static', 'enddate', get_string('courseenddate', 'corplms_reportbuilder'), $enddate);
        }
        if ($grade != '') {
        ...
        if (count($inlineenrolmentelements) > 0) {
            ...
            $corplmsrenderer = $PAGE->get_renderer('corplms_core', null);
            $notifications = $corplmsrenderer->print_corplms_notifications();
...
class report_builder_program_expand_form extends moodleform {
    public function definition() {
        ...
        if ($prog['summary']) {
            $mform->addElement('static', 'summary', get_string('summary', 'corplms_program'), $prog['summary']);
        }

        if ($prog['certifid']) {
            $type = 'certification';
            if ($prog['assigned']) {
                $mform->addElement('static', 'status', get_string('status'), get_string('youareassigned', 'corplms_certification'));
            }
        } else {
            $type = 'program';
            if ($prog['assigned']) {
                $mform->addElement('static', 'status', get_string('status'), get_string('youareassigned', 'corplms_program'));
            }
        }

        $url = new moodle_url('/corplms/program/view.php', array('id' => $prog['id']));
        $mform->addElement('static', 'view', '', html_writer::link($url, get_string('view' . $type, 'corplms_' . $type),
            array('class' => 'link-as-button')));
    }
}
