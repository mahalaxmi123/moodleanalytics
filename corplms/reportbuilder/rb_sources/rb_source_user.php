...
class rb_source_user extends rb_base_source {
    ...
    protected function define_joinlist() {
        ...
        $joinlist = array(
            new rb_join(
                'corplms_stats_comp_achieved',
                'LEFT',
                "(SELECT userid, count(data2) AS number
                    FROM {block_corplms_stats}
                    WHERE eventtype = 4
                    GROUP BY userid)",
                'base.id = corplms_stats_comp_achieved.userid',
                REPORT_BUILDER_RELATION_ONE_TO_ONE
            ),
            new rb_join(
                'corplms_stats_courses_started',
                'LEFT',
                "(SELECT userid, COUNT(DISTINCT data2) as number
                    FROM {block_corplms_stats}
                    WHERE eventtype = 2
                    GROUP BY userid)",
                'base.id = corplms_stats_courses_started.userid',
                REPORT_BUILDER_RELATION_ONE_TO_ONE
            ),
            new rb_join(
                'corplms_stats_courses_completed',
                'LEFT',
                "(SELECT userid, count(DISTINCT data2) AS number
                    FROM {block_corplms_stats}
                    WHERE eventtype = 3
                    GROUP BY userid)",
                'base.id = corplms_stats_courses_completed.userid',
                REPORT_BUILDER_RELATION_ONE_TO_ONE
            ),
            new rb_join(
    ...
    protected function define_columnoptions() {
        ...
        $columnoptions[] = new rb_column_option(
                        ...
                        'corplms_stats_comp_achieved.number',
                        array(
                            'displayfunc' => 'count',
                            'joins' => 'corplms_stats_comp_achieved',
                            'dbdatatype' => 'integer',
                        )
        );
        ...
        $columnoptions[] = new rb_column_option(
                        ...
                        'corplms_stats_courses_started.number',
                        array(
                            'displayfunc' => 'count',
                            'joins' => 'corplms_stats_courses_started',
                            'dbdatatype' => 'integer',
                        )
        );
        ...
        $columnoptions[] = new rb_column_option(
                        ...
                        'corplms_stats_courses_completed.number',
                        array(
                            'displayfunc' => 'count',
                            'joins' => 'corplms_stats_courses_completed',
                            'dbdatatype' => 'integer',
                        )
        );
        ...
        $columnoptions[] = new rb_column_option(
                        'user',
                        'extensionswithlink',
                        get_string('extensions', 'corplms_program'),
    ...
    protected function define_contentoptions() {
        ...
        $contentoptions = array(
            ...
            new rb_content_option(
                'current_pos',
                get_string('currentpos', 'corplms_reportbuilder'),
                'position.path',
                'position'
            ),
            new rb_content_option(
                'current_org',
                get_string('currentorg', 'corplms_reportbuilder'),
                'organisation.path',
                'organisation'
            ),
            new rb_content_option(
    ...
    public function rb_display_learning_icons($itemid, $row) {
        ...
        $disp .= html_writer::start_tag('a', array('href' => $CFG->wwwroot . '/corplms/plan/record/index.php?userid='.$itemid));
        $disp .= html_writer::empty_tag('img',
            array('src' => $OUTPUT->pix_url('record', 'corplms_core'), 'title' => get_string('learningrecords', 'corplms_core')));
        ...
        if ($this->staff_f2f) {
            ...
            $disp .= html_writer::empty_tag('img',
                array('src' => $OUTPUT->pix_url('bookings', 'corplms_core'), 'title' => get_string('f2fbookings', 'corplms_core')));
            $disp .= html_writer::end_tag('a');
        }

        // Individual Development Plans icon
        if (has_capability('corplms/plan:accessplan', $systemcontext)) {
            $disp .= html_writer::start_tag('a', array('href' => $CFG->wwwroot . '/corplms/plan/index.php?userid='.$itemid));
            $disp .= html_writer::empty_tag('img',
                array('src' => $OUTPUT->pix_url('plan', 'corplms_core'), 'title' => get_string('learningplans', 'corplms_plan')));
            $disp .= html_writer::end_tag('a');
        }
    ...
    function rb_display_extension_link($extensioncount, $row, $isexport) {
        ...
        if (isset($row->user_id) && !$isexport) {
            return html_writer::link("{$CFG->wwwroot}/corplms/program/manageextensions.php?userid={$row->user_id}", $extensioncount);
        } else {
    ...
    function rb_display_user_with_links($user, $row, $isexport = false) {
        ...
        $appraisalstr = get_string('appraisals', 'corplms_appraisal');
        $feedback360str = get_string('feedback360', 'corplms_feedback360');
        $goalstr = get_string('goalplural', 'corplms_hierarchy');
        $rol_link = html_writer::link("{$CFG->wwwroot}/corplms/plan/record/index.php?userid={$userid}", $recordstr);
        $required_link = html_writer::link(new moodle_url('/corplms/program/required.php',
                array('userid' => $userid)), $requiredstr);
        $plan_link = html_writer::link("{$CFG->wwwroot}/corplms/plan/index.php?userid={$userid}", $planstr);
        ...
        $appraisal_link = html_writer::link("{$CFG->wwwroot}/corplms/appraisal/index.php?subjectid={$userid}", $appraisalstr);
        $feedback_link = html_writer::link("{$CFG->wwwroot}/corplms/feedback360/index.php?userid={$userid}", $feedback360str);
        $goal_link = html_writer::link("{$CFG->wwwroot}/corplms/hierarchy/prefix/goal/mygoals.php?userid={$userid}", $goalstr);

        require_once($CFG->dirroot . '/corplms/plan/lib.php');
        $show_plan_link = corplms_feature_visible('learningplans') && dp_can_view_users_plans($userid);
        ...
        $tempman = corplms_get_manager($userid, null, false, true);
        if ((!$tempman || $tempman->id != $USER->id) && corplms_feature_visible('appraisals')) {
            $links .= html_writer::tag('li', $appraisal_link);
        }

        if (corplms_feature_visible('feedback360')) {
            $links .= html_writer::tag('li', $feedback_link);
        }

        if (corplms_feature_visible('goals')) {
