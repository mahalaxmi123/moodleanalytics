...
class rb_filter_hierarchy extends rb_filter_type {
    ...
    function setupForm(&$mform) {
        ...
        $objs[] =& $mform->createElement('static', 'selectorbutton',
            ...
            html_writer::empty_tag('input', array('type' => 'button',
                ...
                'value' => get_string('choose' . $type, 'corplms_reportbuilder'),
        ...
        $mform->addHelpButton($grp->_name, 'reportbuilderdialogfilter', 'corplms_reportbuilder');
        if ($advanced) {
    ...
    function get_label($data) {
        ...
        $a->value    = '"'.s($itemname).'"';
        if ($recursive) {
            $a->value .= get_string('andchildren', 'corplms_hierarchy');
        }
    ...
    public function include_js() {
        ...
        $code[] = CORPLMS_JS_TREEVIEW;
        local_js($code);

        $jsdetails = new stdClass();
        $jsdetails->initcall = 'M.corplms_reportbuilder_filterdialogs.init';
        $jsdetails->jsmodule = array('name' => 'corplms_reportbuilder_filterdialogs',
            'fullpath' => '/corplms/reportbuilder/filter_dialogs.js');
        $jsdetails->strings = array(
            'corplms_hierarchy' => array('chooseposition', 'selected', 'chooseorganisation', 'currentlyselected', 'selectcompetency'),
            'corplms_reportbuilder' => array('chooseorgplural', 'chooseposplural', 'choosecompplural'),
        );
        $title = $this->type . '-' . $this->value;
        $currentlyselected = json_encode(dialog_display_currently_selected(get_string('currentlyselected', 'corplms_hierarchy'), $title));
