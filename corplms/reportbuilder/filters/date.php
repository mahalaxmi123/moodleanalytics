...
class rb_filter_date extends rb_filter_type {
    ...
    function setupForm(&$mform) {
        ...
        if ($includebetween) {
            ...
            $objs[] =& $mform->createElement('checkbox', $this->name.'daysbeforechkbox', null,
                    get_string('dateisbetween', 'corplms_reportbuilder'));
            $objs[] =& $mform->createElement('text', $this->name.'daysbefore', null, 'size="2"');
            $mform->setType($this->name.'daysbefore', PARAM_INT);
            $objs[] =& $mform->createElement('static', null, null, get_string('isbeforetoday', 'corplms_reportbuilder'));
            $objs[] =& $mform->createElement('static', null, null,
                    html_writer::span(get_string('isrelativetotoday', 'corplms_reportbuilder')));
            $objs[] =& $mform->createElement('static', null, null, html_writer::empty_tag('br'));
            ...
            $objs[] =& $mform->createElement('static', null, null, get_string('isaftertoday', 'corplms_reportbuilder'));
            $objs[] =& $mform->createElement('static', null, null,
                    html_writer::span(get_string('isrelativetotoday', 'corplms_reportbuilder')));
        }
    ...
    function get_label($data) {
        ...
        if ($daysafter and $daysbefore) {
            return get_string('datelabelisdaysbetween', 'corplms_reportbuilder', $a);

        } else if ($daysafter) {
            return get_string('datelabelisdaysafter', 'corplms_reportbuilder', $a);

        } else if ($daysbefore) {
            return get_string('datelabelisdaysbefore', 'corplms_reportbuilder', $a);
        }
        return '';
    }
}
