...
$returnurl = new moodle_url('/corplms/reportbuilder/filters.php', array('id' => $id));
...
$PAGE->requires->strings_for_js(array('saving', 'confirmfilterdelete', 'confirmsearchcolumndelete', 'delete', 'moveup',
    'movedown', 'add', 'initialdisplay_error'), 'corplms_reportbuilder');
...
$jsmodule = array(
    'name' => 'corplms_reportbuilderfilters',
    'fullpath' => '/corplms/reportbuilder/filters.js',
    'requires' => array('json'));
$PAGE->requires->js_init_call('M.corplms_reportbuilderfilters.init', $args, false, $jsmodule);


// Delete fields or columns.
if ($d and $confirm) {
    if (!confirm_sesskey()) {
        corplms_set_notification(get_string('error:bad_sesskey', 'corplms_reportbuilder'), $returnurl);
    }
    if (isset($fid)) {
        if ($report->initialdisplay && count($report->filters) <= 1) {
                corplms_set_notification(get_string('initialdisplay_error', 'corplms_reportbuilder'), $returnurl);
        } else {
            if ($report->delete_filter($fid)) {
                \corplms_reportbuilder\event\report_updated::create_from_report($report, 'filters')->trigger();
                corplms_set_notification(get_string('filterdeleted', 'corplms_reportbuilder'), $returnurl,
                    array('class' => 'notifysuccess'));
            } else {
                corplms_set_notification(get_string('error:filter_not_deleted', 'corplms_reportbuilder'), $returnurl);
            }
        }
        ...
        if ($report->initialdisplay && count($report->filters) <= 1) {
            ...
            if ($report->delete_search_column($searchcolumnid)) {
                \corplms_reportbuilder\event\report_updated::create_from_report($report, 'filters')->trigger();
                corplms_set_notification(get_string('searchcolumndeleted', 'corplms_reportbuilder'), $returnurl,
                    array('class' => 'notifysuccess'));
            } else {
                corplms_set_notification(get_string('error:search_column_not_deleted', 'corplms_reportbuilder'), $returnurl);
            }
        }
    }
}
...
if ($d) {
    ...
    if (isset($fid)) {
        $confirmurl = new moodle_url('/corplms/reportbuilder/filters.php',
            array('d' => '1', 'id' => $id, 'fid' => $fid, 'confirm' => '1', 'sesskey' => $USER->sesskey));
        echo $output->confirm(get_string('confirmfilterdelete', 'corplms_reportbuilder'), $confirmurl, $returnurl);
    } else if (isset($searchcolumnid)) {
        $confirmurl = new moodle_url('/corplms/reportbuilder/filters.php',
            array('d' => '1', 'id' => $id, 'searchcolumnid' => $searchcolumnid, 'confirm' => '1', 'sesskey' => $USER->sesskey));
        echo $output->confirm(get_string('confirmsearchcolumndelete', 'corplms_reportbuilder'), $confirmurl, $returnurl);
    }
...
if ($m && isset($fid)) {
    if ($report->move_filter($fid, $m)) {
        \corplms_reportbuilder\event\report_updated::create_from_report($report, 'filters')->trigger();
        corplms_set_notification(get_string('filtermoved', 'corplms_reportbuilder'), $returnurl, array('class' => 'notifysuccess'));
    } else {
        corplms_set_notification(get_string('error:filter_not_moved', 'corplms_reportbuilder'), $returnurl);
    }
}
...
if ($mform->is_cancelled()) {
    redirect(new moodle_url('/corplms/reportbuilder/index.php'));
}
if ($fromform = $mform->get_data()) {

    if (empty($fromform->submitbutton)) {
        print_error('error:unknownbuttonclicked', 'corplms_reportbuilder', $returnurl);
    }
    if (build_filters($id, $fromform)) {
        ...
        \corplms_reportbuilder\event\report_updated::create_from_report($report, 'filters')->trigger();
        corplms_set_notification(get_string('filters_updated', 'corplms_reportbuilder'), $returnurl,
            array('class' => 'notifysuccess'));
    } else {
        corplms_set_notification(get_string('error:filters_not_updated', 'corplms_reportbuilder'), $returnurl);
    }
