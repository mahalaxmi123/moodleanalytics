...
M.corplms_reportbuilder_savedsearches = M.corplms_reportbuilder_savedsearches || {
    ...
    init: function(Y, args) {
        ...
        if (typeof $ === 'undefined') {
            throw new Error('M.corplms_reportbuilder_savevedsearch.init()-> jQuery dependency required for this module to function.');
        }

        var corplmsDialog_saved_search_handler = function() {};

        corplmsDialog_saved_search_handler.prototype = new corplmsDialog_handler();

        corplmsDialog_saved_search_handler.prototype.every_load = function() {
        ...
        if (typeof managebutton.attr('id') != 'undefined') {
            ...
            var handler = new corplmsDialog_saved_search_handler();
            var name = 'searchlist';
            ...
            corplmsDialogs[name] = new corplmsDialog(
                ...
                {
                    buttons: buttons,
                    title: '<h2>' + M.util.get_string('managesavedsearches', 'corplms_reportbuilder') + '</h2>'
                },
