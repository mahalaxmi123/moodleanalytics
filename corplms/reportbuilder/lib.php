...
require_once($CFG->libdir . '/corplmstablelib.php');
require_once($CFG->dirroot . '/corplms/core/lib.php');
require_once($CFG->dirroot . '/corplms/reportbuilder/classes/rb_base_source.php');
require_once($CFG->dirroot . '/corplms/reportbuilder/classes/rb_base_content.php');
require_once($CFG->dirroot . '/corplms/reportbuilder/classes/rb_base_access.php');
require_once($CFG->dirroot . '/corplms/reportbuilder/classes/rb_base_preproc.php');
require_once($CFG->dirroot . '/corplms/reportbuilder/classes/rb_base_embedded.php');
require_once($CFG->dirroot . '/corplms/reportbuilder/classes/rb_join.php');
require_once($CFG->dirroot . '/corplms/reportbuilder/classes/rb_column.php');
require_once($CFG->dirroot . '/corplms/reportbuilder/classes/rb_column_option.php');
require_once($CFG->dirroot . '/corplms/reportbuilder/classes/rb_filter_option.php');
require_once($CFG->dirroot . '/corplms/reportbuilder/classes/rb_param.php');
require_once($CFG->dirroot . '/corplms/reportbuilder/classes/rb_param_option.php');
require_once($CFG->dirroot . '/corplms/reportbuilder/classes/rb_content_option.php');
...
class reportbuilder {
    ...
    public function __construct($id=null, $shortname=null, $embed_deprecated=false, $sid=null, $reportfor=null,
        ...
        if ($id != null) {
            ...
            print_error('noshortnameorid', 'corplms_reportbuilder');
        }
        ...
        if (!$report) {
            ...
            if ($embed) {
                ...
                if (! $id = reportbuilder_create_embedded_record($shortname, $embed, $error)) {
                    print_error('error:creatingembeddedrecord', 'corplms_reportbuilder', '', $error);
                }
        ...
        if (!$report) {
            print_error('reportwithidnotfound', 'corplms_reportbuilder', '', $id);
        }
    ...
    function include_js() {
        ...
        if (!$graph) {
            ...
            $jsdetails->initcall = 'M.corplms_reportbuilder_showhide.init';
            $jsdetails->jsmodule = array('name' => 'corplms_reportbuilder_showhide',
                'fullpath' => '/corplms/reportbuilder/showhide.js');
            $jsdetails->args = array('hiddencols' => $this->js_get_hidden_columns());
            $jsdetails->strings = array(
                'corplms_reportbuilder' => array('showhidecolumns'),
                'moodle' => array('ok')
            );
            $js[] = $jsdetails;
            ...
            // Add saved search.js.
            $jsdetails = new stdClass();
            $jsdetails->initcall = 'M.corplms_reportbuilder_savedsearches.init';
            $jsdetails->jsmodule = array('name' => 'corplms_reportbuilder_savedsearches',
                'fullpath' => '/corplms/reportbuilder/saved_searches.js');
            $jsdetails->strings = array(
                'corplms_reportbuilder' => array('managesavedsearches'),
                'form' => array('close')
            );
            $js[] = $jsdetails;
        }
        ...
        foreach ($this->filters as $filter) {
            ...
            $filterpath = $CFG->dirroot.'/corplms/reportbuilder/filters/'.$filtertype.'.php';
        ...
        if ($saved = $DB->get_record('report_builder_saved', array('id' => $this->_sid))) {
            ...
            if ($saved->ispublic != 0 || $saved->userid == $this->reportfor) {
                ...
                if (defined('FULLME') and FULLME === 'cron') {
                    ...
                    print_error('savedsearchnotfoundornotpublic', 'corplms_reportbuilder');
                }
                return false;
            }
        } else {
    ...
    function fetch_sql_filters($extrasql='', $extraparams=array()) {
        ...
        if ($extrasql != '') {
            if (strpos($extrasql, '?')) {
                print_error('extrasqlshouldusenamedparams', 'corplms_reportbuilder');
            }
            $where_sqls[] = $extrasql;
        }
    ...
    private function get_toolbar_sql_filter($toolbarsearchtext) {
        ...
        $keywords = corplms_search_parse_keywords($toolbarsearchtext);
        $columns = $this->get_search_columns();
        ...
        return corplms_search_get_keyword_where_clause($keywords, $dbfields, SQL_PARAMS_NAMED);
    }
    ...
    private function get_toolbar_text_filter($toolbarsearchtext) {
        ...
        if ($numberoffields == 0) {
            ...
            return get_string('toolbarsearchtextiscontainedinsingle', 'corplms_reportbuilder', $a);

        } else {
            $result = get_string('toolbarsearchtextiscontainedinmultiple', 'corplms_reportbuilder', $toolbarsearchtext);
            $columnnames = array();
            foreach ($columns as $column) {
                $columnobject = self::get_single_item($this->columnoptions, $column->type, $column->value);
    ...
    public function format_column_heading(rb_column $column, $plaintext) {
        if ($column->customheading) {
            ...
            if ($column->grouping === 'none') {
                if ($column->transform) {
                    $heading = get_string("transformtype{$column->transform}_heading", 'corplms_reportbuilder', $heading);
                } else if ($column->aggregate) {
                    $heading = get_string("aggregatetype{$column->aggregate}_heading", 'corplms_reportbuilder', $heading);
                }
            }
        }
    ...
    function report_url() {
        ...
        if ($this->embeddedurl === null) {
            return $CFG->wwwroot . '/corplms/reportbuilder/report.php?id=' . $this->_id;
        } else {
    ...
    public function display_sidebar_search() {
        ...
        local_js();
        $jsmodule = array(
            'name' => 'corplms_reportbuilder_instantfilter',
            'fullpath' => '/corplms/reportbuilder/js/instantfilter.js',
            'requires' => array('json'));
        $PAGE->requires->js_init_call('M.corplms_reportbuilder_instantfilter.init', array('id' => $this->_id), true, $jsmodule);
    }
    ...
    */
        ...
        if (isset($this->contentoptions) && is_array($this->contentoptions)) {
            foreach ($this->contentoptions as $option) {
                ...
                if (class_exists($classname)) {
                    ...
                    print_error('contentclassnotexist', 'corplms_reportbuilder', '', $classname);
                }
            }
        }
    ...
    function get_restriction_descriptions($which='all') {
        ...
        if ($this->contentmode != REPORT_BUILDER_CONTENT_MODE_NONE) {
            ...
            if ($this->contentmode == REPORT_BUILDER_CONTENT_MODE_ALL) {
                ...
                $content_restrictions[] = implode(get_string('or', 'corplms_reportbuilder'), $res);
            }
        }
    ...
    function get_single_join($join, $usage) {
        ...
        if ($match = $this->get_joinlist_item($join)) {
            ...
            print_error('joinnotinjoinlist', 'corplms_reportbuilder', '', (object)array('join' => $join, 'usage' => $usage));
            return false;
        }
    }
    ...
    function sort_joins($unsortedjoins) {
        ...
        if (count($items) != 0) {
            print_error('couldnotsortjoinlist', 'corplms_reportbuilder');
        }
    ...
    function get_full_count() {
        ...
        if (empty($this->_fullcount)) {
            ...
            try {
                ...
                print_error('error:problemobtainingreportdata', 'corplms_reportbuilder', '', $debuginfo);
            }
        }
    ...
    public function get_filtered_count($nocache = false) {
        ...
        if (empty($this->_filteredcount) || $nocache) {
            ...
            try {
                ...
                $debuginfo = $CFG->debugdeveloper ? $e->debuginfo : '';
                print_error('error:problemobtainingcachedreportdata', 'corplms_reportbuilder', '', $debuginfo);
            }
        }
    ...
    function export_data($format) {
        ...
        // Log export event.
        \corplms_reportbuilder\event\report_exported::create_from_report($this, $format)->trigger();
    ...
    function display_table() {
        ...
        if (count($columns) == 0) {
            echo html_writer::tag('p', get_string('error:nocolumnsdefined', 'corplms_reportbuilder'));
            return;
        }

        $graphrecord = $DB->get_record('report_builder_graph', array('reportid' => $this->_id));
        if (!empty($graphrecord->type)) {
            $graph = new \corplms_reportbuilder\local\graph($graphrecord, $this, false);
        } else {
            $graph = null;
        }
        ...
        if ($cache) {
            ...
            $html .= get_string('report:cachelast', 'corplms_reportbuilder', $lastreport);
            $html .= html_writer::empty_tag('br');
            $html .= get_string('report:cachenext', 'corplms_reportbuilder', $nextreport);
            $html .= html_writer::end_tag('div');
            echo $html;
        }

        // Start the table.
        $table = new corplms_table($shortname);
        ...
        if ($initiallyhidden) {
            $table->set_no_records_message(get_string('initialdisplay_pending', 'corplms_reportbuilder'));
        } else {
            if ($this->is_report_filtered()) {
                $table->set_no_records_message(get_string('norecordswithfilter', 'corplms_reportbuilder'));
            } else {
                $table->set_no_records_message(get_string('norecordsinreport', 'corplms_reportbuilder'));
            }
        ...
        if ($graph and $graphdata = $graph->fetch_svg()) {
            if (core_useragent::check_browser_version('MSIE', '6.0') and !core_useragent::check_browser_version('MSIE', '9.0')) {
                ...
                $svgurl = new moodle_url('/corplms/reportbuilder/ajax/graph.php', array('id' => $this->_id, 'sid' => $this->_sid));
                $svgurl = $svgurl . '#toolbar=0&navpanes=0&scrollbar=0&statusbar=0&viewrect=20,20,400,300';
                $nopdf = get_string('error:nopdf', 'corplms_reportbuilder');
        ...
        local_js();
        $jsmodule = array(
            'name' => 'corplms_reportbuilder_expand',
            'fullpath' => '/corplms/reportbuilder/js/expand.js',
            'requires' => array('json'));
        $PAGE->requires->js_init_call('M.corplms_reportbuilder_expand.init', array(), true, $jsmodule);
    ...
    public function display_redirect_link() {
        if (isset($this->src->redirecturl)) {
            if (isset($this->src->redirectmessage)) {
                ...
                $message = '&laquo; ' . get_string('selectitem', 'corplms_reportbuilder');
            }
    ...
    function view_saved_menu() {
        ...
        if ($this->embedded) {
            ...
            $common = new moodle_url('/corplms/reportbuilder/report.php', array('id' => $id));
        }
        ...
        if (count($savedoptions) > 0) {
            ...
            $select->label = get_string('viewsavedsearch', 'corplms_reportbuilder');
    ...
    function edit_button() {
        ...
        if (has_capability('corplms/reportbuilder:managereports', $context)) {
            return $OUTPUT->single_button(new moodle_url('/corplms/reportbuilder/general.php', array('id' => $this->_id)), get_string('editthisreport', 'corplms_reportbuilder'), 'get');
        } else {
            return '';
        }
    }
    ...
    function download_ods($fields, $query, $params, $count, $restrictions = array(), $file = null, $cache = array()) {
        ...
        if (is_array($restrictions) && count($restrictions) > 0) {
            $worksheet[0]->write($row, 0, get_string('reportcontents', 'corplms_reportbuilder'));
            $row++;
        ...
        if ($cache) {
            ...
            $worksheet[0]->write($row, 0, get_string('report:cachelast', 'corplms_reportbuilder', $a));
            $row++;
        }
    ...
    function download_pdf($fields, $query, $params, $count, $restrictions = array(), $portrait = true, $file = null, $cache = array()) {
        ...
        $recordscount = get_string('x' . $resultstr, 'corplms_reportbuilder', $count);
        ...
        if (is_array($restrictions) && count($restrictions) > 0) {
            $pdf->Write(0, get_string('reportcontents', 'corplms_reportbuilder'), '', 0, 'L', true, 0, false, false, 0);
            foreach ($restrictions as $restriction) {
        ...
        if ($cache) {
            $usertz = corplms_get_clean_timezone();
            $a = userdate($cache['lastreport'], '', $usertz);
            $lastcache = get_string('report:cachelast', 'corplms_reportbuilder', $a);
        ...
        foreach ($records as $record) {
            ...
            if (REPORT_BUILDER_EXPORT_PDF_MEMORY_LIMIT <= $mramuse) {
                ...
                $records->close();
                // Notice message.
                print_error('exportpdf_mramlimitexceeded', 'corplms_reportbuilder', '', REPORT_BUILDER_EXPORT_PDF_MEMORY_LIMIT);
            }
        }
        $html .= '</tbody></table>';
    ...
    function download_fusion() {
        $jump = new moodle_url('/corplms/reportbuilder/fusionexporter.php', array('id' => $this->_id, 'sid' => $this->_sid));
        redirect($jump->out());
        die;
    }
    ...
    public function get_filters_select($onlyinstant = false) {
        ...
        foreach ($filters as $filter) {
            if (!$onlyinstant || in_array($filter->filtertype, array('date', 'select', 'multicheck'))) {
                ...
                } else if (get_string_manager()->string_exists($langstr, 'corplms_reportbuilder')) {
                    // How about in report builder?
                    $section = get_string($langstr, 'corplms_reportbuilder');
                } else {
    ...
    public function get_all_filters_select() {
        ...
        $allstandardfilters = array_merge(
                array(get_string('new') => array(0 => get_string('addanotherfilter', 'corplms_reportbuilder'))),
        ...
        $allsearchcolumns = array_merge(
            array(get_string('new') => array(0 => get_string('addanothersearchcolumn', 'corplms_reportbuilder'))),
    ...
    function print_feedback_results() {
        ...
        if ($this->is_initially_hidden()) {
            return get_string('initialdisplay_pending', 'corplms_reportbuilder');
        }
        ...
        if (!$groupid) {
            $out .= get_string('activitygroupnotfound', 'corplms_reportbuilder');
        }
        ...
        foreach ($data as $item) {
            ...
            $out .= html_writer::tag('p', get_string('resultsfromfeedback', 'corplms_reportbuilder', $item->responses_number));
    ...
    function get_feedback_standard_answer($qid, $item) {
        ...
        $answer = 'q' . $qid . '_list';
        if (isset($item->$count)) {
            $out .= html_writer::tag('p', get_string('numresponses', 'corplms_reportbuilder', $item->$count));
        }
    ...
    function get_feedback_option_answer($qid, $options, $item) {
        ...
        if (isset($item->$numresp)) {
            $out .= html_writer::tag('p', get_string('numresponses', 'corplms_reportbuilder', $item->$numresp));
        }
 ...
 */
function corplms_reportbuilder_cron() {
    global $CFG;
    require_once($CFG->dirroot . '/corplms/reportbuilder/cron.php');
    reportbuilder_cron();
}
...
function reportbuilder_purge_cache($cache, $unschedule = false) {
    ...
    if (!is_object($cache) || !isset($cache->reportid)) {
        error_log(get_string('error:cachenotfound', 'corplms_reportbuilder'));
        return false;
    }
    if ($cache->cachetable) {
...
function reportbuilder_send_scheduled_report($sched) {
    ...
    if (!$user = $DB->get_record('user', array('id' => $sched->userid))) {
        error_log(get_string('error:invaliduserid', 'corplms_reportbuilder'));
        return false;
    }

    if (!$report = $DB->get_record('report_builder', array('id' => $sched->reportid))) {
        error_log(get_string('error:invalidreportid', 'corplms_reportbuilder'));
        return false;
    }
    ...
    if (!reportbuilder::is_capable($sched->reportid, $sched->userid)) {
        error_log(get_string('error:nopermissionsforscheduledreport', 'corplms_reportbuilder', $sched));
        return false;
    }
    ...
    $messagedetails->exporttype = $strmgr->get_string($export_codes[$sched->format] . 'format', 'corplms_reportbuilder', null, $user->lang);
    ...
    $subject = $report->fullname . ' ' . $strmgr->get_string('report', 'corplms_reportbuilder', null, $user->lang);

    if ($sched->savedsearchid != 0) {
        if (!$savename = $DB->get_field('report_builder_saved', 'name', array('id' => $sched->savedsearchid))) {
            mtrace(get_string('error:invalidsavedsearchid', 'corplms_reportbuilder'));
        } else {
            $messagedetails->savedtext = $strmgr->get_string('savedsearchmessage', 'corplms_reportbuilder', $savename, $user->lang);
        }
    } else {
        $messagedetails->savedtext = '';
    }

    $message = $strmgr->get_string('scheduledreportmessage', 'corplms_reportbuilder', $messagedetails, $user->lang);
    ...
    if (!unlink($CFG->dataroot . DIRECTORY_SEPARATOR . $attachment)) {
        mtrace(get_string('error:failedtoremovetempfile', 'corplms_reportbuilder'));
    }
...
function reportbuilder_get_export_filename($report, $userid, $scheduleid) {
    ...
    $reportfilename = format_string($report->fullname) . '_' .
            userdate(time(), get_string('datepickerlongyearphpuserdate', 'corplms_core')) . '_' . $scheduleid;
    ...
    if (!empty($path)) {
        ...
        if (preg_match($pattern, $path)) {
            mtrace(get_string('error:notapathexportfilesystempath', 'corplms_reportbuilder'));
        } else if (!is_dir($path)) {
            mtrace(get_string('error:notdirexportfilesystempath', 'corplms_reportbuilder'));
        } else if (!is_writable($path)) {
            mtrace(get_string('error:notwriteableexportfilesystempath', 'corplms_reportbuilder'));
        }
    }
...
function reportbuilder_get_report_url($report) {
    ...
    if ($report->embedded == 0) {
        return $CFG->wwwroot . '/corplms/reportbuilder/report.php?id=' . $report->id;
    } else {
...
function reportbuilder_get_embedded_report_object($embedname, $data=array()) {
    ...
    $sourcepaths[] = $CFG->dirroot . '/corplms/reportbuilder/embedded/';
 ...
 *
 * Looks in the corplms/reportbuilder/embedded/ directory and creates a new
...
function reportbuilder_create_embedded_record($shortname, $embed, &$error) {
    ...
    try {
        ...
        \corplms_reportbuilder\event\report_created::create_from_report($report, true)->trigger();
...
function reportbuilder_get_export_options() {
    ...
    foreach ($options as $key => $value) {
        $select[$value] = get_string('export' . $alloptions[$value], 'corplms_reportbuilder');
    }
...
*/
function corplms_reportbuilder_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, $options=array()) {
    $fs = get_file_storage();
    $relativepath = implode('/', $args);
    $fullpath = "/{$context->id}/corplms_reportbuilder/$filearea/$args[0]/$args[1]";
