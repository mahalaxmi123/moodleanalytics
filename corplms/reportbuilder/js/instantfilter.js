...
M.corplms_reportbuilder_instantfilter = M.corplms_reportbuilder_instantfilter || {
    ...
    init: function(Y, args) {
        ...
        if (typeof $ === 'undefined') {
            throw new Error('M.corplms_reportbuilder_instantfilter.init()-> jQuery dependency required for this module to function.');
        }
        ...
        $('.rb-sidebar input, .rb-sidebar select').change(function (event) {
            ...
            if ($(event.target).siblings('.instantfilterwait').length === 0) {
                loadingimg = '<img src="' + M.util.image_url('wait', 'corplms_reportbuilder') +
                        '" class="iconsmall instantfilterwait" />';
                $(loadingimg).insertAfter($(event.target).parent().children().last());
            }
            xhr = M.corplms_reportbuilder_instantfilter.refresh_results();
        });
    },
    ...
    refresh_results: function (callback) {
        // Make the ajax call.
        return $.post(
            M.cfg.wwwroot + '/corplms/reportbuilder/ajax/instantreport.php',
