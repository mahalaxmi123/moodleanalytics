...
$returnurl = $CFG->wwwroot . "/corplms/reportbuilder/general.php?id=$id";
...
$record = file_prepare_standard_editor($record, 'description', $TEXTAREA_OPTIONS, context_system::instance(),
    'corplms_reportbuilder', 'report_builder', $record->id);
...
if ($fromform = $mform->get_data()) {
    ...
    $todb = file_postupdate_standard_editor($todb, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
        'corplms_reportbuilder', 'report_builder', $todb->id);

    $DB->update_record('report_builder', $todb);

    $report = new reportbuilder($id);
    \corplms_reportbuilder\event\report_updated::create_from_report($report, 'general')->trigger();
