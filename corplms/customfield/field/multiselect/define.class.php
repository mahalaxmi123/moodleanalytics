...
class customfield_define_multiselect extends customfield_define_base {
    ...
    public function define_form_specific(&$form) {
        $title =  get_string('multiselectoptions', 'corplms_customfield');
        $this->define_add_js($form, '_specificsettings', false);
        $form->addElement('static', 'cautiondatele', '',
                get_string('multiselectdeleteunlink', 'corplms_customfield'));
        for ($menuind = 0; $menuind < self::MAX_CHOICES; $menuind++) {
            $group = array();
            $group[] = $form->createElement('text', 'option');
            $icon = corplms_create_icon_picker($form, 'edit', 'course', '', 0, $menuind);
            ...
            $group[] = $form->createElement('advcheckbox', 'default', '',
                    get_string('defaultselected', 'corplms_customfield'), array('class' => 'makedefault'));
        ...
        $form->addElement('static', 'addoptionelem', '',
                html_writer::link('#', get_string('addanotheroption', 'corplms_question'),
                        array('id' => "addoptionlink_specificsettings", 'class' => 'addoptionlink')));
        $form->addHelpButton('multiselectitem[0]', 'customfieldmultiselectoptions', 'corplms_customfield');
    }
        ...
        foreach ($data->multiselectitem as $ind => $item) {
            ...
            if ($item['option'] != '') {
                if (in_array($item['option'], $addoptions)) {
                    $err["multiselectitem[$ind]"] = get_string('menunotuniqueoptions', 'corplms_customfield');
                }
                $addoptions[] = $item['option'];
            }
        }
        if ($itemerr) {
            $err['multiselectitem[0]'] = get_string('menunooptions', 'corplms_customfield');
        }
        return $err;
    }
    ...
    public function define_load_preprocess($data) {
        if (isset($data->param1) && $data->param1 != '') {
            ...
            foreach ($data->multiselectitem as $key => $item) {
                list($src, $alt) = corplms_icon_url_and_alt('course', $item['icon']);
    ...
    public function define_add_js($form, $jsid = '', $limitone = true) {
        ...
        $jsmodule = array(
            'name' => 'corplms_customfield_multiselect',
            'fullpath' => '/corplms/customfield/field/multiselect/multiselect.js',
            'requires' => array('json')
        );

        $PAGE->requires->js_init_call('M.corplms_customfield_multiselect.init', $args, false, $jsmodule);
