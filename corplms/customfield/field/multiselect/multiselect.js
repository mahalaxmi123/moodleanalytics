...
M.corplms_customfield_multiselect = M.corplms_customfield_multiselect || {
  Y : null,
  ...
  init : function(Y, args) {
    ...
    if (typeof $ === 'undefined') {
      throw new Error('M.corplms_customfield_multiselect.init()-> jQuery dependency required for this module to function.');
    }

    var savedchoices = M.corplms_customfield_multiselect.config.savedchoices;
    var numVisible = 0;
    var $container = $('#id_' + M.corplms_customfield_multiselect.config.jsid);
    var max = M.corplms_customfield_multiselect.config.max;
    ...
    $allOptions.each(function(){
        ...
        if ($($this.find('span')).length > 0) {
            ...
            var $makeDefaultLink = $('<a href="#">' + M.util.get_string('defaultmake', 'corplms_customfield') + '</a>');
            $makeDefault.append($makeDefaultLink);
            $makeDefaultLink.on('click', function(){
                if (M.corplms_customfield_multiselect.config.oneAnswer == 1) {
            ...
            var $unselectLink = $('<a href="#" class="js-hide">' + M.util.get_string('defaultselected', 'corplms_customfield') + '</a>');
