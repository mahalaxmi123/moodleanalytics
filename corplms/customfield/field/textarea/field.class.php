...
class customfield_textarea extends customfield_base {
    ...
    function edit_field_add(&$mform) {
        ...
        if ($this->is_locked()) {
            if ($this->data == $this->field->defaultdata) {
                $data = file_rewrite_pluginfile_urls($this->field->defaultdata, 'pluginfile.php', $context->id, 'corplms_customfield', 'textarea', $this->fieldid);
            } else {
                $data = file_rewrite_pluginfile_urls($this->data, 'pluginfile.php', $context->id, 'corplms_customfield', $this->prefix, $this->dataid);
            }
    ...
    */
    function edit_save_data($itemnew, $prefix, $tableprefix) {
        ...
        $itemnew = file_postupdate_standard_editor($itemnew, $shortinputname, $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'corplms_customfield', $prefix, $data->id);
    ...
    * Display the data for this field
    ...
    static function display_item_data($data, $extradata=array()) {
        ...
        $context = context_system::instance();
        $data = file_rewrite_pluginfile_urls($data, 'pluginfile.php', $context->id, 'corplms_customfield', $extradata['prefix'], $extradata['itemid']);
