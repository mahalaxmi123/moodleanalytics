...
function customfield_edit_icons($field, $fieldcount, $typeid=0, $prefix, $can_edit, $can_delete) {
    ...
    if ($can_edit) {
        ...
        $editstr = $OUTPUT->action_icon(new moodle_url('/corplms/customfield/index.php', $params), new pix_icon('t/edit', $stredit), null, array('title' => $stredit));
    } else {
    ...
    if ($can_delete) {
        ...
        $deletestr = $OUTPUT->action_icon(new moodle_url('/corplms/customfield/index.php', $params), new pix_icon('t/delete', $strdelete), null, array('title' => $strdelete));
    } else {
    ...
    if ($field->sortorder > 1 && $can_edit) {
        ...
        $upstr = $OUTPUT->action_icon(new moodle_url('/corplms/customfield/index.php', $params), new pix_icon('t/up', $strmoveup), null, array('title' => $strmoveup));
    } else {
    ...
    if ($field->sortorder < $fieldcount && $can_edit) {
        ...
        $params['dir'] = 'down';
        $params['sesskey'] = sesskey();
        $downstr = $OUTPUT->action_icon(new moodle_url('/corplms/customfield/index.php', $params), new pix_icon('t/down', $strmovedown), null, array('title' => $strmovedown));
    } else {
...
function customfield_list_datatypes() {
    ...
    if ($dirlist = get_directory_list($CFG->dirroot.'/corplms/customfield/field', '', false, true, false)) {
        foreach ($dirlist as $type) {
            $datatypes[$type] = get_string('customfieldtype'.$type, 'corplms_customfield');
...
function customfield_edit_field($id, $datatype, $typeid=0, $redirect, $tableprefix, $prefix, $navlinks=false) {
    ...
    require_once($CFG->dirroot.'/corplms/customfield/index_field_form.php');
    $field->descriptionformat = FORMAT_HTML;
    $field = file_prepare_standard_editor($field, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'corplms_customfield', 'textarea', $field->id);
    if ($field->datatype == 'textarea') {
        $field->defaultdataformat = FORMAT_HTML;
        $field = file_prepare_standard_editor($field, 'defaultdata', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'corplms_customfield', 'textarea', $field->id);
    }
    ...
    if ($fieldform->is_cancelled()) {
        ...
        if ($data = $fieldform->get_data()) {
            require_once($CFG->dirroot.'/corplms/customfield/field/'.$datatype.'/define.class.php');
        ...
        if (empty($id)) {
            $strheading = get_string('createnewfield', 'corplms_customfield', $datatypes[$datatype]);
        } else {
            $strheading = get_string('editfield', 'corplms_customfield', format_string($field->fullname));
        }
        ...
        if ($navlinks == false) {
            ...
            $PAGE->navbar->add(get_string($prefix.'plural', 'corplms_customfield'));
            $PAGE->navbar->add(get_string($prefix.'depthcustomfields', 'corplms_customfield'));
        }
...
function customfield_reorder_fields($tableprefix) {
    ...
    if ($types = corplms_group_records($rs, 'typeid')) {
