...
/***** The following methods must be overwritten by child classes *****/
    ...
    function edit_field_add(&$mform) {
        print_error('error:abstractmethod', 'corplms_customfield');
    }
...
/***** The following methods may be overwritten by child classes *****/
        ...
        if ($this->is_unique()) {
            ...
            switch ($this->field->datatype) {
                ...
                case 'textarea':
                    ...
                    $itemnew = file_postupdate_standard_editor($itemnew, $shortinputname, $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'corplms_customfield', $prefix, $itemnew->id);
    ...
    function edit_field_set_required(&$mform) {
        if ($this->is_required()) {
            $mform->addRule($this->inputname, get_string('customfieldrequired', 'corplms_customfield'), 'required', null, 'client');
        }
    }
...
function customfield_load_data(&$item, $prefix, $tableprefix) {
    ...
    foreach ($fields as $field) {
        require_once($CFG->dirroot.'/corplms/customfield/field/'.$field->datatype.'/field.class.php');
        ...
        if ($field->datatype == 'textarea' && !$formfield->is_locked()) {
            ...
            if ($formfield->data == $formfield->field->defaultdata) {
                ...
                $item = file_prepare_standard_editor($item, $shortinputname, $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
                                'corplms_customfield', 'textarea', $formfield->fieldid);
            } else {
                $item = file_prepare_standard_editor($item, $shortinputname, $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
                                'corplms_customfield', $prefix, $formfield->dataid);
            }
        }
    }
}
