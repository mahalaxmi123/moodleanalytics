...
abstract class course_set {
    ...
    protected function get_completion_type_string() {
        ...
        case COMPLETIONTYPE_ANY:
            $completiontypestr = get_string('or', 'corplms_program');
            break;
        case COMPLETIONTYPE_ALL:
            $completiontypestr = get_string('and', 'corplms_program');
            break;
        default:
            return false;
            break;
    ...
    public function get_default_label() {
        return get_string('untitledset', 'corplms_program');
    }
    ...
    public function save_set() {
        ...
        if ($this->label == $this->get_default_label()) {
            $this->label = get_string('legend:courseset', 'corplms_program', $this->sortorder);
        }
        ...
        if ($completion = $DB->get_record('prog_completion',
            ...
            if ($update_success = $DB->update_record('prog_completion', $completion)) {
                if ($eventtrigger) {
                    ...
                    $event = \corplms_program\event\program_courseset_completed::create(
                        array(
    ...
    public function get_nextsetoperator_select_form_template(&$mform, &$template_values, $formdataobject, $prefix, $updateform=true) {
        ...
        if ($updateform) {
            if (isset($this->islastset) && $this->islastset) {
                ...
                $options = array(
                    NEXTSETOPERATOR_THEN => get_string('then', 'corplms_program'),
                    NEXTSETOPERATOR_OR => get_string('or', 'corplms_program')
                );
                $mform->addElement('select', $prefix.'nextsetoperator', get_string('label:nextsetoperator', 'corplms_program'), $options);
    ...
    protected function get_courseset_divider_text($previous_sets=array(), $next_sets=array(), $userid=0, $viewinganothersprogram=false) {
        ...
        $separator = ' ' . get_string('or', 'corplms_program') . ' ';
        if ($previous_sets[count($previous_sets)-1]->nextsetoperator == NEXTSETOPERATOR_OR) {
            ...
            if ($viewinganothersprogram) {
                if (!$user = $DB->get_record('user', array('id' => $userid))) {
                    print_error('error:invaliduser', 'corplms_program');
                }
                $out .= fullname($user) . ' ' . get_string('youmustcompleteormanager', 'corplms_program', implode($separator, $sets));
            } else {
                if ($userid) {
                    $out .= get_string('youmustcompleteorlearner', 'corplms_program', implode($separator, $sets));
                } else {
                    $out .= get_string('youmustcompleteorviewing', 'corplms_program', implode($separator, $sets));
                }
            }
        } else {
            $a = new stdClass();
            ...
            $a->proceedto = ' ' . get_string('anothercourse', 'corplms_program');
            ...
            if ($viewinganothersprogram) {
                ...
                $out .= fullname($user) . ' ' . get_string('youmustcompletebeforeproceedingtomanager', 'corplms_program', $a);
            } else {
                if ($userid) {
                    $out .= get_string('youmustcompletebeforeproceedingtolearner', 'corplms_program', $a);
                } else {
                    $out .= get_string('youmustcompletebeforeproceedingtoviewing', 'corplms_program', $a);
                }
            }
        }
        return $out;
...
class multi_course_set extends course_set {
    ...
    public function save_courses() {
        ...
        $program_plugin = enrol_get_plugin('corplms_program');
    ...
    public function display($userid=null, $previous_sets=array(), $next_sets=array(), $accessible=true, $viewinganothersprogram=false) {
        ...
        switch ($this->completiontype) {
            case COMPLETIONTYPE_ALL:
                $out .= html_writer::tag('p', html_writer::tag('strong', get_string('completeallcourses', 'corplms_program')));
                break;
            case COMPLETIONTYPE_ANY:
                $out .= html_writer::tag('p', html_writer::tag('strong', get_string('completeanycourse', 'corplms_program')));
                break;
        }
        ...
        if ($this->timeallowed > 0) {
            $out .= html_writer::tag('p', get_string('allowtimeforset', 'corplms_program', $timeallowance));
        } else {
            $out .= html_writer::tag('p', get_string('allowtimeforsetinfinity', 'corplms_program'));
        }

        if (count($this->courses) > 0) {
            $table = new html_table();
            $table->head = array(get_string('coursename', 'corplms_program'), get_string('actions'));
            $table->colclasses = array('coursename', 'launchcourse');
            $table->attributes['class'] = 'fullwidth generaltable';
            if ($userid) {
                $table->head[] = get_string('status', 'corplms_program');
                $table->colclasses[] = 'status';
                $completeheading = false;
            }

            // Get label for launch/view course button - applies to all courses.
            $launchviewlabel = get_string('launchcourse', 'corplms_program');
            if (!empty($this->certifpath)) {
                ...
                if ($certifpath_user == CERTIFPATH_RECERT && !certif_iswindowopen($certificationid, $userid)) {
                    $launchviewlabel = get_string('viewcourse', 'corplms_program');
                }
            }
            ...
            foreach ($this->courses as $course) {
                ...
                $coursedetails = html_writer::empty_tag('img', array('src' => corplms_get_icon($course->id, CORPLMS_ICON_TYPE_COURSE),
                    'class' => 'course_icon', 'alt' => ''));

                $showcourseset = false;
                if ($userid) {
                    $showcourseset = (is_enrolled($coursecontext, $userid) || corplms_course_is_viewable($course->id, $userid))
                                     && $accessible;
                }
                ...
                if (is_siteadmin($USER->id)) {
                    ...
                    $dimmed = corplms_get_style_visibility($course);
                    ...
                    $launch = html_writer::tag('div', $OUTPUT->single_button(new moodle_url('/course/view.php', array('id' => $course->id)),
                                    get_string('launchcourse', 'corplms_program'), null), array('class' => 'prog-course-launch'));
                } else {
                    ...
                    if ($showcourseset) {
                        ...
                        $launch = html_writer::tag('div', $OUTPUT->single_button(null, get_string('notavailable', 'corplms_program'), null,
                ...
                if ($userid) {
                    ...
                    $cells[] = new html_table_cell(corplms_display_course_progress_icon($userid, $course->id, $status));

                    if ($showcourseset && corplms_is_manager($userid) &&
                            has_capability('corplms/program:markstaffcoursecomplete', $usercontext)) {
                        $completion = new completion_info($course);
                        $indicator = ($completion->is_course_complete($userid) ? 'y' : 'n');
                        $url = new moodle_url('/corplms/program/content/completecourse.php',
                        ...
                        if (!$completeheading) {
                            $table->head[] = get_string('markcompletheading', 'corplms_program');
            ...
            $out .= html_writer::tag('p', get_string('nocourses', 'corplms_program'));
        }
        ...
        if (!isset($this->islastset) || $this->islastset === false) {
            switch ($this->nextsetoperator) {
                case NEXTSETOPERATOR_THEN:
                    ...
                    $out .= html_writer::tag('div', get_string('then', 'corplms_program'), array('class' => 'operator-then'));
                ...
                case NEXTSETOPERATOR_OR:
                    $out .= html_writer::start_tag('div', array('class' => 'nextsetoperator'));
                    $out .= html_writer::tag('div', get_string('or', 'corplms_program'), array('class' => 'operator-or'));
                    $out .= html_writer::tag('div', $this->get_courseset_divider_text($previous_sets, $next_sets, $userid,
                        $viewinganothersprogram), array('class' => 'nextsethelp'));
                    $out .= html_writer::end_tag('div');
                    break;
    ...
    public function display_form_element() {
        ...
        $completiontypestr = $this->completiontype == COMPLETIONTYPE_ALL ? get_string('and', 'corplms_program') : get_string('or', 'corplms_program');
        $courses = $this->courses;
        ...
        if (count($courses)) {
            ...
            $out .= get_string('nocourses', 'corplms_program');
        }
        ...
        if (!isset($this->islastset) || $this->islastset === false) {
            if ($this->nextsetoperator != 0) {
                $out .= html_writer::start_tag('div', array('class' => 'nextsetoperator'));
                $operatorstr = $this->nextsetoperator == NEXTSETOPERATOR_THEN ? get_string('then', 'corplms_program') : get_string('or', 'corplms_program');
                $out .= $operatorstr;
                $out .= html_writer::end_tag('div');
            }
        }
    ...
    public function print_courses() {
        ...
        $out .= html_writer::tag('div', get_string('courses', 'corplms_program'). ':', array('class' => 'fitemtitle'));
        if (isset($this->courses) && is_array($this->courses) && count($this->courses)>0) {

            if (!$completiontypestr = $this->get_completion_type_string()) {
                print_error('unknowncompletiontype', 'corplms_program', '', $this->sortorder);
            }
            ...
            foreach ($this->courses as $course) {
                ...
                $content .= html_writer::start_tag('div', array('class' => 'corplms-item-group delete_item'));
                $content .= html_writer::start_tag('a',
                                array('class' => 'corplms-item-group-icon coursedeletelink', 'href' => 'javascript:;',
            ...
            $out .= html_writer::tag('div', get_string('nocourses', 'corplms_program'), array('class' => 'felement'));
            $out .= html_writer::empty_tag('input', array('type' => 'hidden', 'name' => $prefix.'courses', 'value' => ''));
        }
    ...
    public function get_courseset_form_template(&$mform, &$template_values, &$formdataobject, $updateform=true) {
        ...
        $helpbutton = $OUTPUT->help_icon('multicourseset', 'corplms_program');
        $legend = ((isset($this->label) && ! empty($this->label)) ? $this->label : get_string('untitledset', 'corplms_program',
        ...
        if ($updateform) {
            ...
            $mform->addElement('submit', $prefix.'moveup', get_string('moveup', 'corplms_program'), $attributes);
            ...
            $mform->addElement('submit', $prefix.'movedown', get_string('movedown', 'corplms_program'), $attributes);
            ...
            $mform->addElement('submit', $prefix.'delete', get_string('delete', 'corplms_program'),
        ...
        $helpbutton = $OUTPUT->help_icon('setlabel', 'corplms_program');
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fitem'));
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fitemtitle'));
        $templatehtml .= html_writer::tag('label', get_string('label:setname', 'corplms_program') . ' ' . $helpbutton,
        ...
        if ($updateform) {
            $completiontypeoptions = array(
                COMPLETIONTYPE_ANY => get_string('onecourse', 'corplms_program'),
                COMPLETIONTYPE_ALL => get_string('allcourses', 'corplms_program'),
            );
            $onchange = 'return M.corplms_programcontent.changeCompletionTypeString(this, '.$prefix.');';
            $mform->addElement('select', $prefix.'completiontype', get_string('label:learnermustcomplete', 'corplms_program'),
        ...
        $helpbutton = $OUTPUT->help_icon('completiontype', 'corplms_program');
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fitem'));
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fitemtitle'));
        $templatehtml .= html_writer::tag('label', get_string('label:learnermustcomplete', 'corplms_program') . ' ' . $helpbutton,
        ...
        $helpbutton = $OUTPUT->help_icon('minimumtimerequired', 'corplms_program');
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fitem'));
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fitemtitle'));
        $templatehtml .= html_writer::tag('label', get_string('label:minimumtimerequired', 'corplms_program') . ' ' . $helpbutton,
        ...
        if (count($courseoptions) > 0) {
            if ($updateform) {
                ...
                $mform->addElement('submit', $prefix.'addcourse', get_string('addcourse', 'corplms_program'),
                                 array('onclick' => "return M.corplms_programcontent.amendCourses('$prefix')"));
            ...
            $templatehtml .= html_writer::tag('p', get_string('nocoursestoadd', 'corplms_program'));
        }
    ...
    public function get_courses_form_template(&$mform, &$template_values, &$formdataobject, $updateform=true) {
        ...
        $templatehtml = '';
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fitem'));
        $templatehtml .= html_writer::tag('div', get_string('courses', 'corplms_program'). ':', array('class' => 'fitemtitle'));
        if (isset($this->courses) && is_array($this->courses) && count($this->courses)>0) {
            ...
            $templatehtml .= html_writer::tag('div', get_string('nocourses', 'corplms_program'), array('class' => 'felement'));
        }
    ...
    public function get_course_text($courseset) {
        if ($courseset->completiontype == COMPLETIONTYPE_ALL) {
            return get_string('allcoursesfrom', 'corplms_program') . ' "' . format_string($courseset->label) . '"';
        }
        else {
            return get_string('onecoursesfrom', 'corplms_program') . ' "' . format_string($courseset->label) . '"';
        }
    }
    ...
    private function get_course_warnings($course) {
        ...
        if (!empty($modinfo->instances['facetoface'])) {
            ...
            if ($DB->record_exists_select('facetoface', 'multiplesessions = 0 AND id ' . $sql, $params)) {
                $content .= $OUTPUT->notification(get_string('multiplefacetofacewarning', 'corplms_program'), 'notifyproblem');
            }
        }
        return $content;
    }
...
class competency_course_set extends course_set {
    ...
    public function display($userid=null,$previous_sets=array(),$next_sets=array(),$accessible=true, $viewinganothersprogram=false) {
        ...
        if ($courses && count($courses) > 0) {
            $table = new html_table();
            $table->head = array(get_string('coursename', 'corplms_program'), '');
            $table->colclasses = array('coursename', 'launchcourse');
            $table->attributes['class'] = 'fullwidth generaltable';
            if ($userid) {
                $table->head[] = get_string('status', 'corplms_program');
            ...
            foreach ($courses as $course) {
                ...
                $cells[] = new html_table_cell($coursedetails);
                if ($accessible && corplms_course_is_viewable($course->id, $userid)) {
                    $launch = html_writer::tag('div', $OUTPUT->single_button(new moodle_url('/course/view.php', array('id' => $course->id)),
                                     get_string('launchcourse', 'corplms_program'), null), array('class' => 'prog-course-launch'));
                } else {
                    $launch = html_writer::tag('div', $OUTPUT->single_button(null, get_string('notavailable', 'corplms_program'), null,
    ...
    public function display_form_element() {
        ...
        $completiontypestr = $this->get_completion_type() == COMPLETIONTYPE_ALL ? get_string('and', 'corplms_program') : get_string('or', 'corplms_program');
        $courses = $this->get_competency_courses();
        ...
        if ($this->nextsetoperator != 0) {
            ...
            $operatorstr = $this->nextsetoperator == NEXTSETOPERATOR_THEN ?
                                            get_string('then', 'corplms_program') : get_string('or', 'corplms_program');
            $out .= $operatorstr;
            $out .= html_writer::end_tag('div');
        }
    ...
    public function get_courseset_form_template(&$mform, &$template_values, &$formdataobject, $updateform=true) {
        ...
        $helpbutton = $OUTPUT->help_icon('competencycourseset', 'corplms_program');
        $legend = ((isset($this->label) && ! empty($this->label)) ? $this->label : get_string('legend:courseset', 'corplms_program',
                        $this->sortorder)) . ' ' . $helpbutton;
        $templatehtml .= html_writer::tag('legend', $legend);
        $templatehtml .= html_writer::start_tag('div', array('class' => 'setbuttons'));
        ...
        if ($this->competencyid > 0) {
            if ($competency = $DB->get_record('comp', array('id' => $this->competencyid))) {
                $templatehtml .= html_writer::start_tag('div', array('class' => 'fitem'));
                $templatehtml .= html_writer::start_tag('div', array('class' => 'fitemtitle'));
                $templatehtml .= html_writer::tag('label', get_string('label:competencyname', 'corplms_program'));
                $templatehtml .= html_writer::end_tag('div');
        ...
        if ($this->competencyid > 0) {
            ...
            if (count($course_competencies) > 0) {
                if ($updateform) {
                    ...
                    $mform->addElement('submit', $prefix.'addcompetency', get_string('addcompetency', 'corplms_program'));
                    $template_values['%'.$prefix.'competencyid%'] = array('name'=>$prefix.'competencyid', 'value'=>null);
                ...
                $templatehtml .= '%'.$prefix.'competencyid%'."\n";
                $templatehtml .= html_writer::tag('p', get_string('nocompetenciestoadd', 'corplms_program'));
...
class recurring_course_set extends course_set {
    ...
    public function get_courseset_form_template(&$mform, &$template_values, &$formdataobject, $updateform=true) {
        ...
        $helpbutton = $OUTPUT->help_icon('recurringcourseset', 'corplms_program');
        $legend = ((isset($this->label) && ! empty($this->label)) ? $this->label : get_string('legend:recurringcourseset', 'corplms_program',
                        $this->sortorder)) . ' ' . $helpbutton;
        $templatehtml .= html_writer::tag('legend', $legend);
        ...
        if (is_object($this->course)) {
            if (isset($this->course->fullname)) {
                $templatehtml .= html_writer::start_tag('div', array('class' => 'fitem'));
                $templatehtml .= html_writer::start_tag('div', array('class' => 'fitemtitle'));
                $templatehtml .= html_writer::tag('label', get_string('coursename', 'corplms_program'));
                $templatehtml .= html_writer::end_tag('div');
                ...
                if (count($courseoptions) > 0) {
                    ...
                    $templatehtml .= '%'.$prefix.'courseid%'."\n";
                    $templatehtml .= $OUTPUT->help_icon('recurringcourse', 'corplms_program');
                    $formdataobject->{$prefix.'courseid'} = $this->course->id;
                } else {
                    $templatehtml .= html_writer::tag('p', get_string('nocoursestoselect', 'corplms_program'));
                }
                $templatehtml .= html_writer::end_tag('div');
        ...
        $helpbutton = $OUTPUT->help_icon('recurrence', 'corplms_program');
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fitem'));
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fitemtitle'));
        $templatehtml .= html_writer::tag('label', get_string('label:recurrence', 'corplms_program') . ' ' . $helpbutton,
            array('for' => $prefix.'recurrencetimenum'));
        $templatehtml .= html_writer::end_tag('div');
        $templatehtml .= html_writer::tag('div', get_string('repeatevery', 'corplms_program') . ' %' . $prefix .
        ...
        $helpbutton = $OUTPUT->help_icon('coursecreation', 'corplms_program');
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fitem'));
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fitemtitle'));
        $templatehtml .= html_writer::tag('label', get_string('label:recurcreation', 'corplms_program') . ' ' . $helpbutton,
            array('for' => $prefix.'recurcreatetimenum'));
        $templatehtml .= html_writer::end_tag('div');
        $templatehtml .= html_writer::tag('div', get_string('createcourse', 'corplms_program') . ' %' .
            $prefix . 'recurcreatetimenum% %' . $prefix . 'recurcreatetimeperiod% '.
            get_string('beforecourserepeats', 'corplms_program'), array('class' => 'felement'));
        $templatehtml .= html_writer::end_tag('div');
        ...
        if ($updateform) {
            $mform->addElement('submit', $prefix.'update', get_string('update', 'corplms_program'),
