...
class prog_assignments {
    ...
    public function count_active_user_assignments() {
        global $CFG, $DB;
        require_once($CFG->dirroot . '/corplms/program/program.class.php');
    ...
    public function display_form_label() {
        $out = '';
        $out .= get_string('instructions:assignments1', 'corplms_program');
        return $out;
    }
    ...
    public function display_form_element() {
        ...
        if (count($this->assignments)) {
            ...
            $table->head = array(
                get_string('overview', 'corplms_program'),
                get_string('numlearners', 'corplms_program')
            );
            $table->data = array();
            ...
            foreach ($assignmentdata as $categorytype => $data) {
                ...
                $row = array();
                $row[] = $data['typecount'].' '.get_string($categoryclassname, 'corplms_program');
                $row[] = $data['users'];
            ...
            $helpbutton = $OUTPUT->help_icon('totalassignments', 'corplms_program');
            $table->data[] = array(
                html_writer::tag('strong', get_string('totalassignments', 'corplms_program')),
            ...
            $out .= get_string('noprogramassignments', 'corplms_program');
        }
    ...
    public static function get_confirmation_template() {
        ...
        $table = new html_table();
        $table->head = array('', get_string('added', 'corplms_program'), get_string('removed', 'corplms_program'));
        $table->data = array();
        ...
        $data = array();
        $data['html'] = html_writer::tag('div', get_string('youhavemadefollowingchanges', 'corplms_program') . html_writer::empty_tag('br') . html_writer::empty_tag('br') . $tableHTML . html_writer::empty_tag('br') . get_string('tosaveassignments','corplms_program'));
...
abstract class prog_assignment_category {
            ...
            foreach ($itemids as $itemid) {
                ...
                if ($object->completiontime != COMPLETION_TIME_NOT_SET) {
                    if ($object->completionevent == COMPLETION_EVENT_NONE) {
                        // Convert fixed dates.
                        $object->completiontime = corplms_date_parse_from_format(get_string('datepickerlongyearparseformat', 'corplms_core'), $object->completiontime);
                    } else {
                ...
                if (isset($object->id)) {
                    ...
                    if ($original_object->includechildren != $object->includechildren ||
                        ...
                        if (!$DB->update_record('prog_assignment', $object)) {
                            print_error('error:updatingprogramassignment', 'corplms_program');
                        }
                    }
                } else {
    ...
    function get_completion($item) {
        global $CFG, $OUTPUT;
        $completion_string = get_string('setcompletion', 'corplms_program');
        ...
        if ($item->completionevent == COMPLETION_EVENT_NONE) {
            ...
            if ($item->completiontime != COMPLETION_TIME_NOT_SET) {
                ...
                $item->completiontime = trim(userdate($item->completiontime,
                    get_string('datepickerlongyearphpuserdate', 'corplms_core'), $CFG->timezone, false));
        ...
        if ($show_deletecompletionlink) {
            $html .= $OUTPUT->action_icon('#', new pix_icon('t/delete', get_string('removecompletiondate', 'corplms_program')), null, array('class' => 'deletecompletiondatelink'));
        }
        return $html;
    }
    ...
    public function build_first_table_cell($name, $id, $itemid) {
        global $OUTPUT;
        $output = html_writer::start_tag('div', array('class' => 'corplms-item-group'));
        $output .= format_string($name);
        $output .= $OUTPUT->action_icon('#', new pix_icon('t/delete', get_string('delete')), null,
            array('class' => 'deletelink corplms-item-group-icon'));
    ...
    public static function build_completion_string($completiontime, $completionevent, $completioninstance) {
        ...
        if (isset($COMPLETION_EVENTS_CLASSNAMES[$completionevent])) {
            ...
            if (isset($TIMEALLOWANCESTRINGS[$parts[1]])) {
                $a->period = get_string($TIMEALLOWANCESTRINGS[$parts[1]], 'corplms_program');
            } else {
                return '';
            }
            ...
            return get_string('completewithinevent', 'corplms_program', $a);
        }
        else {
            ...
            if (preg_match($datepattern, $completiontime, $matches) == 0) {
                ...
                return get_string('completebytime', 'corplms_program', $completiontime);
            }
        }
    }
...
class organisations_category extends prog_assignment_category {
    ...
    function __construct() {
        ...
        $this->name = get_string('organisations', 'corplms_program');
        $this->buttonname = get_string('addorganisationstoprogram', 'corplms_program');
    }
    ...
    function build_table($programid) {
        ...
        $this->headers = array(
            get_string('organisationname', 'corplms_program'),
            get_string('allbelow', 'corplms_program'),
            get_string('complete','corplms_program'),
            get_string('numlearners','corplms_program')
        );
    ...
    function get_js($programid) {
        $title = addslashes_js(get_string('addorganisationstoprogram', 'corplms_program'));
        $url = 'find_hierarchy.php?type=organisation&table=org&programid='.$programid;
        return "M.corplms_programassignment.add_category({$this->id}, 'organisations', '{$url}', '{$title}');";
    }
}
...
class positions_category extends prog_assignment_category {
    ...
    function __construct() {
        ...
        $this->name = get_string('positions', 'corplms_program');
        $this->buttonname = get_string('addpositiontoprogram', 'corplms_program');
    }

    function build_table($programid) {
        global $DB;
        $this->headers = array(
            get_string('positionsname', 'corplms_program'),
            get_string('allbelow', 'corplms_program'),
            get_string('complete','corplms_program'),
            get_string('numlearners','corplms_program')
        );
    ...
    function get_js($programid) {
        $title = addslashes_js(get_string('addpositiontoprogram', 'corplms_program'));
        $url = 'find_hierarchy.php?type=position&table=pos&programid='.$programid;
        return "M.corplms_programassignment.add_category({$this->id}, 'positions', '{$url}', '{$title}');";
    }
}
...
class cohorts_category extends prog_assignment_category {
    ...
    function __construct() {
        ...
        $this->name = get_string('cohorts', 'corplms_program');
        $this->buttonname = get_string('addcohortstoprogram', 'corplms_program');
    }

    function build_table($programid) {
        global $DB;
        $this->headers = array(
            get_string('cohortname', 'corplms_program'),
            get_string('type', 'corplms_program'),
            get_string('complete','corplms_program'),
            get_string('numlearners','corplms_program')
        );
    ...
    function get_js($programid) {
        $title = addslashes_js(get_string('addcohortstoprogram', 'corplms_program'));
        $url = 'find_cohort.php?programid='.$programid;
        return "M.corplms_programassignment.add_category({$this->id}, 'cohorts', '{$url}', '{$title}');";
    }
    protected function _add_assignment_hook($object) {
        return true;
    }
...
class managers_category extends prog_assignment_category {
    ...
    function __construct() {
        ...
        $this->name = get_string('managementhierarchy', 'corplms_program');
        $this->buttonname = get_string('addmanagerstoprogram', 'corplms_program');
    }

    function build_table($programid) {
        global $DB;
        $this->headers = array(
            get_string('managername', 'corplms_program'),
            get_string('for', 'corplms_program'),
            get_string('complete','corplms_program'),
            get_string('numlearners','corplms_program')
        );
    ...
    function build_row($item) {
        ...
        $options = array(
            0 => get_string('directteam', 'corplms_program'),
            1 => get_string('allbelowlower', 'corplms_program'));
    ...
    function get_js($programid) {
        $title = addslashes_js(get_string('addmanagerstoprogram', 'corplms_program'));
        $url = 'find_manager_hierarchy.php?programid='.$programid;
        return "M.corplms_programassignment.add_category({$this->id}, 'managers', '{$url}', '{$title}');";
    }
}
...
class individuals_category extends prog_assignment_category {
    ...
    function __construct() {
        ...
        $this->name = get_string('individuals', 'corplms_program');
        $this->buttonname = get_string('addindividualstoprogram', 'corplms_program');
    }

    function build_table($programid) {
        global $DB;
        $this->headers = array(
            get_string('individualname', 'corplms_program'),
            get_string('userid', 'corplms_program'),
            get_string('complete','corplms_program')
        );
    ...
    function get_js($programid) {
        $title = addslashes_js(get_string('addindividualstoprogram', 'corplms_program'));
        $url = 'find_individual.php?programid='.$programid;
        return "M.corplms_programassignment.add_category({$this->id}, 'individuals', '{$url}', '{$title}');";
    }
}
...
class prog_assigment_completion_first_login extends prog_assignment_completion_type {
    ...
    public function get_name() {
        return get_string('firstlogin', 'corplms_program');
    }
    public function get_script() {
        return "
            corplmsDialogs['completionevent'].clear();
        ";
    }
...
class prog_assigment_completion_position_start_date extends prog_assignment_completion_type {
    ...
    public function get_name() {
        return get_string('positionstartdate', 'corplms_program');
    }
    public function get_script() {
        global $CFG;

        return "
            corplmsDialogs['completionevent'].default_url = '$CFG->wwwroot/corplms/program/assignment/completion/find_position.php?';
            corplmsDialogs['completionevent'].open();
    ...
    public function get_completion_string() {
        return get_string('startinposition', 'corplms_program');
    }
    public function get_timestamp($userid, $assignobject) {
        // Lazy load data when required.
...
class prog_assigment_completion_program_completion extends prog_assignment_completion_type {
    ...
    public function get_name() {
        return get_string('programcompletion', 'corplms_program');
    }
    public function get_script() {
        global $CFG;

        return "
            corplmsDialogs['completionevent'].default_url = '$CFG->wwwroot/corplms/program/assignment/completion/find_program.php?';
            corplmsDialogs['completionevent'].open();
    ...
    public function get_completion_string() {
        return get_string('completionofprogram', 'corplms_program');
    }
    public function get_timestamp($userid, $assignobject) {
        // Lazy load data when required.
        if (!isset($this->timestamps)) {
...
class prog_assigment_completion_course_completion extends prog_assignment_completion_type {
    ...
    public function get_name() {
        return get_string('coursecompletion', 'corplms_program');
    }
    public function get_script() {
        global $CFG;

        return "
            corplmsDialogs['completionevent'].default_url = '$CFG->wwwroot/corplms/program/assignment/completion/find_course.php?';
            corplmsDialogs['completionevent'].open();
    ...
    public function get_completion_string() {
        return get_string('completionofcourse', 'corplms_program');
    }
    public function get_timestamp($userid, $assignobject) {
        // Lazy load data when required.
        if (!isset($this->timestamps)) {
...
class prog_assigment_completion_profile_field_date extends prog_assignment_completion_type {
    ...
    public function get_name() {
        return get_string('profilefielddate', 'corplms_program');
    }
    public function get_script() {
        global $CFG;

        return "
            corplmsDialogs['completionevent'].default_url = '$CFG->wwwroot/corplms/program/assignment/completion/find_profile_field.php?';
            corplmsDialogs['completionevent'].open();
    ...
    public function get_completion_string() {
        return get_string('dateinprofilefield', 'corplms_program');
    }
    public function get_timestamp($userid, $assignobject) {
        // Lazy load data when required.
        if (!isset($this->timestamps)) {
...
class prog_assigment_completion_enrollment_date extends prog_assignment_completion_type {
    ...
    public function get_name() {
        return get_string('programenrollmentdate', 'corplms_program');
    }
    public function get_script() {
        return "
            corplmsDialogs['completionevent'].clear();
