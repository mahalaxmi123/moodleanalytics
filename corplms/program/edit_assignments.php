...
$PAGE->set_url(new moodle_url('/corplms/program/edit_assignments.php', array('id' => $id)));
...
$PAGE->requires->strings_for_js(array('setcompletion', 'removecompletiondate', 'youhaveunsavedchanges',
                ...
                'saveallchanges','confirmassignmentchanges','chooseitem'), 'corplms_program');
$PAGE->requires->string_for_js('loading', 'admin');
$PAGE->requires->string_for_js('none', 'moodle');
$display_selected = json_encode(dialog_display_currently_selected(get_string('selected', 'corplms_hierarchy'), 'completion-event-dialog'));
...
$PAGE->requires->js_init_call('M.corplms_programassignment.init',$args, false, $jsmodule);
...
if ($data = data_submitted()) {
    ...
    $event = \corplms_program\event\program_assignmentsupdated::create(
        array(
            'objectid' => $id,
    ...
    if (isset($data->savechanges)) {
        corplms_set_notification(get_string('programassignmentssaved', 'corplms_program'), 'edit_assignments.php?id='.$id,
