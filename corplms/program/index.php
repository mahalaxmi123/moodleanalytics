...
if (!empty($CFG->enhancedcatalog) && !$categoryid) {
    if ($viewtype == 'program') {
        redirect(new moodle_url('/corplms/coursecatalog/programs.php'));
    } else {
        redirect(new moodle_url('/corplms/coursecatalog/certifications.php'));
    }
}
...
if ($categoryid) {
    $url = new moodle_url('/corplms/program/index.php', array('categoryid' => $categoryid, 'viewtype' => $viewtype));
    ...
    $categoryid = 0;
    $PAGE->set_url('/corplms/program/index.php', array('viewtype' => $viewtype));
...
$PAGE->set_corplms_menu_selected('findlearning');
