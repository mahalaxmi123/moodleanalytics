...
require_once($CFG->dirroot . '/corplms/certification/lib.php'); // For the constants
...
function prog_can_view_users_required_learning($learnerid) {
    ...
    if (has_capability('corplms/program:accessanyprogram', $systemcontext)) {
        return true;
    }

    // If the user cannot view any programs
    if (!has_capability('corplms/program:viewprogram', $systemcontext)) {
        return false;
    }
    ...
    if (corplms_is_manager($learnerid)) {
        return true;
    }
...
function prog_display_required_programs($userid) {
    ...
    $tableheaders = array(get_string('programname', 'corplms_program'));
    $tablecols = array('progname');

    // Due date
    $tableheaders[] = get_string('duedate', 'corplms_program');
    $tablecols[] = 'duedate';

    // Progress
    $tableheaders[] = get_string('progress', 'corplms_program');
    $tablecols[] = 'progress';

    $baseurl = $CFG->wwwroot . '/corplms/program/required.php?userid='.$userid;
...
function prog_display_certification_programs($userid) {
    ...
    // Set up table
    $tablename = 'progs-list';
    $tableheaders = array(get_string('certificationname', 'corplms_program'));
    $tablecols = array('progname');
    ...
    $baseurl = new moodle_url('/corplms/program/required.php', array('userid' => $userid));
...
function prog_add_base_navlinks() {
    ...
    $PAGE->navbar->add(get_string('browsecategories', 'corplms_program'), new moodle_url('/corplms/program/index.php'));
}
...
function prog_add_required_learning_base_navlinks($userid) {
    ...
    if ($userid == $USER->id) {
        $PAGE->navbar->add(get_string('mylearning', 'corplms_core'), '/my/');
        $PAGE->navbar->add(get_string('requiredlearning', 'corplms_program'), new moodle_url('/corplms/program/required.php'));
        return true;
    }
    ...
    if ($user) {
        ...
        $PAGE->navbar->add(get_string('xsrequiredlearning', 'corplms_program', fullname($user)), new moodle_url('/corplms/program/required.php', array('userid' => $userid)));
    } else {
        $PAGE->navbar->add(get_string('unknownusersrequiredlearning', 'corplms_program'), new moodle_url('/corplms/program/required.php', array('userid' => $userid)));
    }
    ...
    if ($bread_info = $DB->get_records_sql($sql, $params)) {
        foreach ($bread_info as $crumb) {
            $cat_bread[] = array('name' => format_string($crumb->name),
                                 'link' => new moodle_url("/corplms/program/index.php",
...
function prog_get_programs_page($categoryid="all", $sort="sortorder ASC",
    ...
    // Visibility.
    list($visibilitysql, $visibilityparams) = corplms_visibility_where(null, 'p.id', 'p.visible', 'p.audiencevisible', 'p', $type);
    $params = array_merge($params, $visibilityparams);
    ...
    if (!empty($programids)) {
            ...
            foreach ($programids as $programid) {
                ...
                if (!$program  = $DB->get_record("prog", array("id" => $programid))) {
                    echo $OUTPUT->notification(get_string('error:findingprogram', 'corplms_program'));
                } else {
                    ...
                    if (!$DB->update_record('prog', $program)) {
                        echo $OUTPUT->notification(get_string('error:prognotmoved', 'corplms_program'));
                    }
...
function prog_get_tab_link($userid) {
    ...
    if ($dbman->table_exists($progtable)) {
        ...
        if ($requiredlearningcount == 1) {
            ...
            return $CFG->wwwroot . '/corplms/program/required.php?id=' . $program->id;
        } else if ($requiredlearningcount > 1) {
            return $CFG->wwwroot . '/corplms/program/required.php';
        }
    }
    ...
    if (!empty($extensions)) {
        ...
        foreach ($extensions as $id => $action) {
            ...
            if (!$extension = $DB->get_record('prog_extension', array('id' => $id))) {
                print_error('error:couldnotloadextension', 'corplms_program');
            }

            if (!corplms_is_manager($extension->userid)) {
                print_error('error:notusersmanager', 'corplms_program');
            }
            ...
            if ($action == PROG_EXTENSION_DENY) {
                ...
                $userfrom = corplms_is_manager($extension->userid, $USER->id) ? $USER : core_user::get_support_user();
                ...
                $messagedata->subject          = $stringmanager->get_string('extensiondenied', 'corplms_program', null, $userto->lang);
                $messagedata->contexturl       = $CFG->wwwroot.'/corplms/program/required.php?id='.$extension->programid;
                $messagedata->contexturlname   = $stringmanager->get_string('launchprogram', 'corplms_program', null, $userto->lang);
                $messagedata->fullmessage      = $stringmanager->get_string('extensiondeniedmessage', 'corplms_program', $program->fullname, $userto->lang);
                $messagedata->icon             = 'program-decline';
                $messagedata->msgtype          = CORPLMS_MSG_TYPE_PROGRAM;
                ...
                if (!empty($reasonfordecision[$id])) {
                    ...
                    $messagedata->fullmessage  .= $stringmanager->get_string('reasondeniedmessage', 'corplms_program', $reasonfordecision[$id], $userto->lang);
                }
                ...
                if ($result = tm_alert_send($messagedata)) {
                    ...
                    print_error('error:failedsendextensiondenyalert', 'corplms_program');
                }
                ...
                if (!$extension_program->set_timedue($extension->userid, $extension->extensiondate)) {
                    ...
                    $userto = $DB->get_record('user', array('id' => $extension->userid));
                    if (!$userto) {
                        print_error('error:failedtofinduser', 'corplms_program', $extension->userid);
                    }
                    ...
                    $messagedata->userto           = $userto;
                    $messagedata->userfrom         = $userfrom;
                    $messagedata->subject          = $stringmanager->get_string('extensiongranted', 'corplms_program', null, $userto->lang);
                    $messagedata->contexturl       = $CFG->wwwroot.'/corplms/program/required.php?id='.$extension->programid;
                    $messagedata->contexturlname   = $stringmanager->get_string('launchprogram', 'corplms_program', null, $userto->lang);
                    $messagedata->fullmessage      = $stringmanager->get_string('extensiongrantedmessage', 'corplms_program', userdate($extension->extensiondate, get_string('strftimedate', 'langconfig'), $CFG->timezone), null, $userto->lang);
                    ...
                    if (!empty($reasonfordecision[$id])) {
                        // Add reason to the message.
                        $messagedata->fullmessage  .= html_writer::empty_tag('br') . html_writer::empty_tag('br');
                        $messagedata->fullmessage  .= $stringmanager->get_string('reasonapprovedmessage', 'corplms_program', $reasonfordecision[$id], $userto->lang);
                    }
                    ...
                    if ($result = tm_alert_send($messagedata)) {
                        ...
                        print_error('error:failedsendextensiongrantalert','corplms_program');
                    }
                 }
            }
        }
...
function prog_assignments_firstlogin($user) {
    ...
    $pending_by_program = corplms_group_records($rs, 'programid');
 ...
 */
function corplms_program_cron() {
    global $CFG;
    require_once($CFG->dirroot . '/corplms/program/cron.php');
    program_cron();
}
...
function corplms_program_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, $options=array()) {
...
function prog_can_delete_programs($categoryid) {
    ...
    foreach ($programcontexts as $ctxrecord) {
        ...
        if (!has_capability('corplms/program:deleteprogram', $programcontext)) {
            return false;
        }
    }
...
class program_in_list implements IteratorAggregate {
    ...
    public function get_program_overviewfiles() {
        ...
        $fs = get_file_storage();
        $context = context_program::instance($this->id);
        $files = $fs->get_area_files($context->id, 'corplms_program', 'overviewfiles', false, 'filename', false);
        if (count($files)) {
...
function prog_format_seconds($seconds, $timeonly = false) {
    ...
    $str_years = get_string('xyears', 'corplms_program', $years);
    $seconds = $seconds % DURATION_YEAR;

    $months = floor($seconds / DURATION_MONTH);
    $str_months = get_string('xmonths', 'corplms_program', $months);
    $seconds = $seconds % DURATION_MONTH;

    $weeks = floor($seconds / DURATION_WEEK);
    $str_weeks = get_string('xweeks', 'corplms_program', $weeks);
    $seconds = $seconds % DURATION_WEEK;

    $days = floor($seconds / DURATION_DAY);
    $str_days = get_string('xdays', 'corplms_program', $days);
    ...
    $output .= get_string('minprogramtimerequired', 'corplms_program');
    $output .= $timestring;
 ...
 *
 * @param enrol_corplms_program_plugin $program_plugin
 * @param int $programid
 * @param boolean debugging
 */
function prog_update_available_enrolments(enrol_corplms_program_plugin $program_plugin, $programid, $debugging = false) {
    global $DB;
...
function check_program_enabled() {
    if (corplms_feature_disabled('programs')) {
        print_error('programsdisabled', 'corplms_program');
    }
}
...
function check_certification_enabled() {
    if (corplms_feature_disabled('certifications')) {
        print_error('certificationsdisabled', 'corplms_certification');
    }
}
