...
function xmldb_corplms_program_install() {
    global $CFG, $DB;
    ...
    if (!isset($CFG->corplms_program_cron)) {
        // hack to get cron working via admin/cron.php
        set_config('corplms_program_cron', 60);
    }
...
/**
* This function is called both when Moodle/Corplms is first installed or when
* the program module is installed into an existing Corplms instance.
*
...
function prog_setup_initial_plan_settings() {
    global $CFG, $DB;
    require_once($CFG->dirroot . '/corplms/plan/priorityscales/lib.php');
