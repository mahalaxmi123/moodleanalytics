...
if (count($failed_ids) == 0) {
    corplms_set_notification(get_string('successfullyresolvedexceptions', 'corplms_program'), null, array('class' => 'notifysuccess'));
}
else {
    corplms_set_notification(get_string('failedtoresolve', 'corplms_program') . ': ' . implode(', ', $failed_ids));
}
