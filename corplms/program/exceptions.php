...
$baseurl = new moodle_url('/corplms/program/exceptions.php', $pageparams);

if (!has_capability('corplms/program:handleexceptions', $programcontext)) {
...
echo $OUTPUT->heading(get_string('programexceptions', 'corplms_program'));
echo html_writer::start_tag('p') . get_string('instructions:programexceptions', 'corplms_program') . html_writer::end_tag('p');
...
$PAGE->requires->string_for_js('confirmresolution', 'corplms_program');
...
$jsmodule = array(
     'name' => 'corplms_programexceptions',
     'fullpath' => '/corplms/program/exceptions.js',
     'requires' => array('json')
     );

$PAGE->requires->js_init_call('M.corplms_programexceptions.init',$args, false, $jsmodule);
