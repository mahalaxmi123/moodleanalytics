...
if (!has_capability('corplms/program:configuremessages', $programcontext)) {
    print_error('error:nopermissions', 'corplms_program');
}

$PAGE->set_url(new moodle_url('/corplms/program/edit_messages.php', array('id' => $id)));
...
if ($rawdata = data_submitted()) {
    ...
    if (!$programmessagemanager->setup_messages($rawdata)) {
        print_error('error:setupprogrammessages', 'corplms_program');
    }

    if (isset($rawdata->addmessage)) {
        if (!$programmessagemanager->add_message($rawdata->messagetype)) {
            echo $OUTPUT->notification(get_string('error:unableaddmessagetypeunrecog', 'corplms_program'));
        }
    } else if (isset($rawdata->update)) {
        $programmessagemanager->update_messages();
        echo $OUTPUT->notification(get_string('progmessageupdated', 'corplms_program'));
        ...
        if (!$programmessagemanager->delete_message($messagenumber)) {
            echo $OUTPUT->notification(get_string('error:unabledeletemessagenotfound', 'corplms_program'));
        }
...
if ($data = $messageseditform->get_data()) {
    ...
    if (isset($data->savechanges)) {
        ...
        if (!$programmessagemanager->save_messages($data)) {
            ...
            corplms_set_notification(get_string('programmessagessaved', 'corplms_program'), 'edit_messages.php?id='.$id, array('class' => 'notifysuccess'));
        }
    }
...
$jsmodule = array(
     'name' => 'corplms_programmessages',
     'fullpath' => '/corplms/program/messages/program_messages.js',
     'requires' => array('json'));
$PAGE->requires->js_init_call('M.corplms_programmessages.init',$args, false, $jsmodule);
