 ...
 * @subpackage program
 */
M.corplms_programexceptions = M.corplms_programexceptions || {
    ...
    init: function(Y, args){
        ...
        if (!this.config.id) {
            throw new Error('M.corplms_programexceptions.init()-> Required config \'id\' not available.');
        }
        ...
        $('#selectiontype').change(function() {
            ...
            var url = M.cfg.wwwroot+'/corplms/program/exception/updateselections.php?id='+module.config.id,
        ...
        var buttonsObj = {};
            ...
            buttonsObj[M.util.get_string('ok', 'moodle')] = function() { dialog.save(M.cfg.wwwroot+'/corplms/program/exception/resolve.php?id='+module.config.id); };
        var dialog = new corplmsDialog(
                'applyaction',
                'applyactionbutton',
                {
                    buttons: buttonsObj,
                    title: '<h2>'+M.util.get_string('confirmresolution', 'corplms_program')+'</h2>',
                            height: '250'
                },
                M.cfg.wwwroot+'/corplms/program/exception/confirm_resolution.php',
                handler
            );
        ...
        dialog._update = function(response) {
            this.hide();
            window.location.href = M.cfg.wwwroot+'/corplms/program/exceptions.php?id='+module.config.id+'&search='+module.config.search_term;
        };

        corplmsDialogs['applyaction'] = dialog;
    },

    /**
     * wrapper for Corplms 1.9 globally declared Items() function, when called sets an
    ...
    /**
     * wrapper for Corplms 1.9 globally declared Item() function
