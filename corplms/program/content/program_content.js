 ...
 * @subpackage program
 */
M.corplms_programcontent = M.corplms_programcontent || {
    ...
    init: function(Y, args){
        ...
        if (!this.config.id) {
            throw new Error('M.corplms_programcontent.init()-> Required config \'id\' not available.');
        }

        // check jQuery dependency and continue with setup
        if (typeof $ === 'undefined') {
            throw new Error('M.corplms_programcontent.init()-> jQuery dependency required for this module to function.');
        }

        /**
         * Dialog and dialog handler definitions, corplmsDialog_program_cat is
        ...
        */
        ...
        // Create handler
        var corplmsDialog_handler_addmulticourse = function(contenturl) {
            this.contenturl = contenturl;
        };

        // Set the dialog handler as a multi select - corplms/core/js/lib/corplms_dialog.js
        corplmsDialog_handler_addmulticourse.prototype = new corplmsDialog_handler_treeview_multiselect();


        // Adapt the handler's save function
        corplmsDialog_handler_addmulticourse.prototype._save = function() {
            ...
            if (courseids.length === 0) {
                alert(M.util.get_string('error:courses_nocourses', 'corplms_program'));
                return false;
            }
        ...
        // Define the dialog
        var corplmsDialog_addmulticourse = function() {

            this.url = M.cfg.wwwroot+'/corplms/program/content/';
            this.find_url = 'find_courses.php?id='+module.config.id;
            this.ajax_url = 'get_html.php?id='+module.config.id;

            // Setup the handler
            var handler = new corplmsDialog_handler_addmulticourse(this.url + this.ajax_url);
            ...
            // Call the parent dialog object and link us (corplms_dialog.js)
            corplmsDialog.call(
                this,
                ...
                {                 // config
                    buttons: buttonsObj,
                    title: '<h2>'+M.util.get_string('addcourseset', 'corplms_program')+'</h2>',
                },
                default_url,
                handler
            );
        ...
        // Create handler
        var corplmsDialog_handler_addcompetency = function(contenturl) {
            this.contenturl = contenturl;
        };

        // Set the dialog handler as a single select
        corplmsDialog_handler_addcompetency.prototype = new corplmsDialog_handler_treeview_singleselect();

        // Adapt the handler's save function
        corplmsDialog_handler_addcompetency.prototype._save = function() {

            // retrieve the competency id
            var id = $('#treeview_selected_val_addcompetency').val();
            if (id === "0") {
                alert(M.util.get_string('error:nocompetency', 'corplms_program'));
                return false;
            }
        ...
        // Define the dialog
        var corplmsDialog_addcompetency = function() {
            ...
            // Setup the handler
            var handler = new corplmsDialog_handler_addcompetency(this.url + this.ajax_url);
            ...
            corplmsDialog.call(
                ...
                {
                    buttons: buttonsObj,
                    title: '<h2>'+ M.util.get_string('addcompetency', 'corplms_program') + module.config.display_selected_addcompetency +'</h2>'
                },
                default_url,
                handler
            );
        ...
        // Create handler
        corplmsDialog_handler_addrecurringcourse = function(contenturl) {
            this.contenturl = contenturl;
        }

        // Set the dialog handler as a single select
        corplmsDialog_handler_addrecurringcourse.prototype = new corplmsDialog_handler_treeview_singleselect();

        // Adapt the handler's save function
        corplmsDialog_handler_addrecurringcourse.prototype._save = function() {

            // retrieve the course id
            var courseid = $('#treeview_selected_val_addrecurringcourse').val();
            if (courseid === "0") {
                alert(M.util.get_string('error:recur_nocourse', 'corplms_program'));
                return false;
            }
        ...
        // Define the dialog
        corplmsDialog_addrecurringcourse = function() {
            ...
            // Setup the handler
            var handler = new corplmsDialog_handler_addrecurringcourse(this.url + this.ajax_url);
            ...
            corplmsDialog.call(
                ...
                {
                    buttons: buttonsObj,
                    title: '<h2>'+ M.util.get_string('addcourse', 'corplms_program') + module.config.display_selected_addrecurringcourse +'</h2>'
                },
                default_url,
                handler
            );
        ...
        corplmsDialog_handler_amendmulticourse = function(contenturl) {
            this.contenturl = contenturl;
        ...
        // Set the dialog handler as a multiselect (./corplms/core/js/lib/corplms_dialog.js)
        corplmsDialog_handler_amendmulticourse.prototype = new corplmsDialog_handler_treeview_multiselect();


        // Adapt the handler's save function
        corplmsDialog_handler_amendmulticourse.prototype._save = function() {
        ...
        // Define the dialog
        corplmsDialog_amendmulticourse = function() {

            var url = M.cfg.wwwroot+'/corplms/program/content/';
            ...
            // Setup the handler
            this.handler = new corplmsDialog_handler_amendmulticourse(url + this.ajax_url);

            var self = this;
            var buttonsObj = {};
            buttonsObj[M.util.get_string('cancel', 'corplms_program')] = function() { self.handler._cancel(); };
            buttonsObj[M.util.get_string('ok','corplms_program')] = function() { self.handler._save(); };
            ...
            corplmsDialog.call(
                ...
                {
                    buttons: buttonsObj,
                    title: '<h2>'+ M.util.get_string('addcourses', 'corplms_program') +'</h2>'
                },
                this.default_url,
                this.handler
            );
        ...
        corplmsDialog_savechanges = function() {
            ...
            var self = this;
            var buttonsObj = {};
            buttonsObj[M.util.get_string('editcontent', 'corplms_program')] = function() { handler._cancel(); };
            ...
            corplmsDialog.call(
              ...
              {
                  buttons: buttonsObj,
                  title: '<h2>'+ M.util.get_string('confirmcontentchanges', 'corplms_program') +'</h2>'
              },
        ...
        corplmsDialogs['addmulticourse'] = new corplmsDialog_addmulticourse();
        corplmsDialogs['addcompetency'] = new corplmsDialog_addcompetency();
        corplmsDialogs['addrecurringcourse'] = new corplmsDialog_addrecurringcourse();
        corplmsDialogs['amendcourses'] = new corplmsDialog_amendmulticourse();
        corplmsDialogs['savechanges'] = new corplmsDialog_savechanges();
    ...
    initCoursesets: function(){
        ...
        $('div.courseadder input:submit').val(M.util.get_string('addcourses', 'corplms_program'));
    ...
    changeCompletionTypeString: function(el, prefix){
        if (el.value == this.config.COMPLETIONTYPE_ANY) {
            var completiontypestr = M.util.get_string('or', 'corplms_program');
        } else {
            var completiontypestr = M.util.get_string('and', 'corplms_program');
        }
    ...
    addContent: function(suf){
        ...
        if (contenttype == this.config.CONTENTTYPE_MULTICOURSE) {
            corplmsDialogs['addmulticourse'].suf = suf; // store on dialog object
            corplmsDialogs['addmulticourse'].open();
        } else if (contenttype == this.config.CONTENTTYPE_COMPETENCY) {
            corplmsDialogs['addcompetency'].open();
        } else if (contenttype == this.config.CONTENTTYPE_RECURRING) {
            corplmsDialogs['addrecurringcourse'].open();
        }
    ...
    amendCourses: function(coursesetprefix){
        ...
        var dialog_url = M.cfg.wwwroot+'/corplms/program/content/find_courses.php?id='+this.config.id+'&selectedcourseids=' + selectedcourses_encoded;

        var mydialog = corplmsDialogs['amendcourses'];
    ...
    deleteCourse: function(coursesetid, coursesetprefix, coursetodelete_id){
        ...
        var contenturl = M.cfg.wwwroot+'/corplms/program/content/get_html.php?id='+M.corplms_programcontent.config.id;
        ...
        $.getJSON(contenturl +
            ...
            M.corplms_programcontent.initCoursesets();
            ...
            $('.edit-program .coursedeletelink').click(function(){
                M.corplms_programcontent.deleteCourse(this.getAttribute('data-coursesetid'),
                                                     this.getAttribute('data-coursesetprefix'),
                                                     this.getAttribute('data-coursetodelete_id'));
                return true;
            });
    ...
    handleSaveChanges: function(){
        ...
        if (window.currentassignmentcount !== undefined) {
            html = M.util.get_string('affectedusercount', 'corplms_program') + window.currentassignmentcount;
        }
        html = html + '<'+'p>' + M.util.get_string('tosavecontent', 'corplms_program') +'<'+'/p>';
        dialog.open(html);
