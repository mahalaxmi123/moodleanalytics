...
if ($action == 'delete') {
    redirect(new moodle_url('/corplms/program/delete.php', array('id' => $id, 'category' => $category)));
}
...
$PAGE->set_url(new moodle_url('/corplms/program/edit.php', array('id' => $id, 'action' => $action)));
...
if ($action == 'edit') {
    ...
    $PAGE->requires->string_for_js('youhaveunsavedchanges', 'corplms_program');
    $args = array('args'=>'{"id":'.$id.'}');
    $jsmodule = array(
            'name' => 'corplms_programedit',
            'fullpath' => '/corplms/program/program_edit.js',
            'requires' => array('json'));
    $PAGE->requires->js_init_call('M.corplms_programedit.init',$args, false, $jsmodule);
    ...
    if (!empty($CFG->audiencevisibility)) {
        if (empty($program->id)) {
            ...
            $visibleselected = corplms_cohort_get_visible_learning($program->id, $instancetype);
...
if (!$progcategory = $DB->get_record('course_categories', array('id' => $program->category))) {
    print_error('error:determineprogcat', 'corplms_program');
}
...
$editcontenturl = "{$CFG->wwwroot}/corplms/program/edit_content.php?id={$program->id}";
$editassignmentsurl = "{$CFG->wwwroot}/corplms/program/edit_assignments.php?id={$program->id}";
$editmessagesurl = "{$CFG->wwwroot}/corplms/program/edit_messages.php?id={$program->id}";
$editcertificationsurl = "{$CFG->wwwroot}/corplms/certification/edit_certification.php?id={$program->id}";
...
if ($overviewfilesoptions) {
    file_prepare_standard_filemanager($program, 'overviewfiles', $overviewfilesoptions, $programcontext, 'corplms_program', 'overviewfiles', 0);
}
...
if ($detailsform->is_cancelled()) {
    corplms_set_notification(get_string('programupdatecancelled', 'corplms_program'), $viewurl, array('class' => 'notifysuccess'));
}
...
if ($data = $detailsform->get_data()) {
    if (isset($data->edit)) {
        ...
        file_postupdate_standard_editor($data, 'summary', $TEXTAREA_OPTIONS, $programcontext, 'corplms_program', 'summary', 0);
        ...
        if ($overviewfilesoptions = prog_program_overviewfiles_options($data->id)) {
            file_postupdate_standard_filemanager($data, 'overviewfiles', $overviewfilesoptions, $programcontext, 'corplms_program', 'overviewfiles', 0);
        }

        file_postupdate_standard_editor($data, 'endnote', $TEXTAREA_OPTIONS, $programcontext, 'corplms_program', 'endnote', 0);
        $DB->set_field('prog', 'endnote', $data->endnote, array('id' => $data->id));

        // Visible audiences.
        if (!empty($CFG->audiencevisibility) && has_capability('corplms/coursecatalog:manageaudiencevisibility', $systemcontext)) {
            $visiblecohorts = corplms_cohort_get_visible_learning($program->id, $instancetype);
            ...
            if ($newvisible = array_diff($newvisible, array_keys($visiblecohorts))) {
                ...
                foreach ($newvisible as $cohortid) {
                    corplms_cohort_add_association($cohortid, $program->id, $instancetype, COHORT_ASSN_VALUE_VISIBLE);
                }
            }
        }

        $event = \corplms_program\event\program_updated::create(
            array(
                'objectid' => $program->id,
        ...
        corplms_set_notification(get_string('programdetailssaved', 'corplms_program'), $nexturl, array('class' => 'notifysuccess'));
    }
