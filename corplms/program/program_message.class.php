 ...
 * corplms_alert_send method which is used extensively by the program
...
class prog_message_data {
    ...
    public function __construct($messagedata) {
        ...
        $this->msgtype   = isset($messagedata['msgtype']) ? $messagedata['msgtype'] : CORPLMS_MSG_TYPE_UNKNOWN;
        $this->urgency   = isset($messagedata['urgency']) ? $messagedata['urgency'] : CORPLMS_MSG_URGENCY_NORMAL;
...
abstract class prog_message {
    ...
    if (!$this->studentrole) {
        print_error('error:failedtofindstudentrole', 'corplms_program');
    }
    if (!$this->managerrole) {
        print_error('error:failedtofindmanagerrole', 'corplms_program');
    }
    ...
    public function set_replacementvars($recipient, $options = array()) {
        ...
        if ($manager = corplms_get_manager($recipient->id)) {
        ...
        if (in_array('duedate', $placeholders) || in_array('completioncriteria', $placeholders)) {
            $formatdate = get_string('datepickerlongyearphpuserdate', 'corplms_core');
        ...
        foreach ($placeholders as $placeholder) {
            switch ($placeholder) {
                ...
                case 'duedate';
                    ...
                    $duedate = get_string('duedatenotset', 'corplms_program');
                ...
                case 'completioncriteria';
                    ...
                    if ($progassignment->completionevent == COMPLETION_EVENT_NONE) {
                        $ccriteria = get_string('completioncriterianotdefined', 'corplms_program');
    ...
    /**
     * Sends a generic alert message using the Corplms message/alert framework
     *
    ...
    public static function send_generic_alert($messagedata) {
        ...
        (!isset($messagedata->msgtype))     && $messagedata->msgtype    = CORPLMS_MSG_TYPE_UNKNOWN;
        (!isset($messagedata->urgency))     && $messagedata->urgency    = CORPLMS_MSG_URGENCY_NORMAL;
    ...
    public function get_generic_basic_fields_template(&$mform, &$template_values, &$formdataobject, $updateform=true) {
        ...
        $helpbutton = $OUTPUT->help_icon('messagesubject', 'corplms_program');
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fline'));
        $templatehtml .= html_writer::tag('div', html_writer::tag('label', get_string('label:subject', 'corplms_program') . ' ' . $helpbutton, array('for' => $prefix.'messagesubject')), array('class' => 'flabel'));
        ...
        $helpbutton = $OUTPUT->help_icon('mainmessage', 'corplms_program');
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fline'));
        $templatehtml .= html_writer::tag('div', html_writer::tag('label', get_string('label:message', 'corplms_program') . ' ' . $helpbutton, array('for' => $prefix.'mainmessage')), array('class' => 'flabel'));
    ...
    public function get_generic_manager_fields_template(&$mform, &$template_values, &$formdataobject, $updateform=true) {
        ...
        $helpbutton = $OUTPUT->help_icon('notifymanager', 'corplms_program');
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fline'));
        $templatehtml .= html_writer::tag('div', html_writer::tag('label', get_string('label:sendnoticetomanager', 'corplms_program') . ' ' . $helpbutton, array('for' => $prefix.'notifymanager')), array('class' => 'flabel'));
        ...
        $helpbutton = $OUTPUT->help_icon('managermessage', 'corplms_program');
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fline'));
        $templatehtml .= html_writer::tag('div', html_writer::tag('label', get_string('label:noticeformanager', 'corplms_program') . ' ' . $helpbutton, array('for' => $prefix.'managermessage')), array('class' => 'flabel'));
    ...
    public function get_generic_trigger_fields_template(&$mform, &$template_values, &$formdataobject, $updateform=true) {
        ...
        $helpbutton = $OUTPUT->help_icon('trigger', 'corplms_program');
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fline'));
        $templatehtml .= html_writer::tag('div', html_writer::tag('label', get_string('label:trigger', 'corplms_program') . ' ' . $helpbutton, array('for' => $prefix.'triggernum')), array('class' => 'flabel'));
...
abstract class prog_noneventbased_message extends prog_message {
    ...
    public function __construct($programid, $messageob=null, $uniqueid=null) {
        ...
        $studentmessagedata = array(
            ...
            'contexturl'        => $CFG->wwwroot.'/corplms/program/view.php?id='.$this->programid,
            'contexturlname'    => get_string('launchprogram', 'corplms_program'),
        );
    ...
    public function send_message($recipient, $sender=null, $options=array()) {
        ...
        $manager = corplms_get_manager($recipient->id);
        ...
        $studentdata->msgtype = CORPLMS_MSG_TYPE_PROGRAM;
        ...
        if ($result && $this->notifymanager && $manager) {
            ...
            $managerdata->contexturl = $CFG->wwwroot.'/corplms/program/view.php?id='.$this->programid.'&amp;userid='.$recipient->id;
            $managerdata->icon = 'program-regular';
            $managerdata->msgtype = CORPLMS_MSG_TYPE_PROGRAM;
    ...
    public function get_message_form_template(&$mform, &$template_values, &$formdataobject, $updateform=true) {
        ...
        $helpbutton = $OUTPUT->help_icon($this->helppage, 'corplms_program');
...
class prog_enrolment_message extends prog_noneventbased_message {
   ...
   public function __construct($programid, $messageob=null, $uniqueid=null) {
        ...
        $this->sortorder = 1;
        $this->fieldsetlegend = get_string('legend:enrolmentmessage', 'corplms_program');

        $managermessagedata = array(
            'roleid'            => $this->managerrole,
            'subject'           => get_string('learnerenrolled', 'corplms_program'),
...
class prog_exception_report_message extends prog_noneventbased_message {
    ...
    public function __construct($programid, $messageob=null, $uniqueid=null) {
        ...
        $this->sortorder = 2;
        $this->fieldsetlegend = get_string('legend:exceptionreportmessage', 'corplms_program');

        $studentmessagedata = array(
            'roleid'            => $this->studentrole,
            'subject'           => $this->messagesubject,
            'fullmessage'       => $this->mainmessage,
            'contexturl'        => $CFG->wwwroot.'/corplms/program/exceptions.php?id='.$this->programid,
            'contexturlname'    => get_string('viewexceptions', 'corplms_program'),
        );
...
class prog_unenrolment_message extends prog_noneventbased_message {
    ...
    public function __construct($programid, $messageob=null, $uniqueid=null) {
        ...
        $this->fieldsetlegend = get_string('legend:unenrolmentmessage', 'corplms_program');
        ...
        $managermessagedata = array(
            'roleid'            => $this->managerrole,
            'subject'           => get_string('learnerunenrolled', 'corplms_program'),
            'fullmessage'       => $this->managermessage,
            'contexturl'        => $CFG->wwwroot.'/corplms/program/view.php?id='.$this->programid,
            'contexturlname'    => get_string('launchprogram', 'corplms_program'),
        );
...
class prog_program_completed_message extends prog_noneventbased_message {
    ...
    public function __construct($programid, $messageob=null, $uniqueid=null) {
        ...
        $this->fieldsetlegend = get_string('legend:programcompletedmessage', 'corplms_program');

        $managermessagedata = array(
            'roleid'            => $this->managerrole,
            'subject'           => get_string('programcompleted', 'corplms_program'),
            'fullmessage'       => $this->managermessage,
            'contexturl'        => $CFG->wwwroot.'/corplms/program/view.php?id='.$this->programid,
            'contexturlname'    => get_string('launchprogram', 'corplms_program'),
        );
...
class prog_courseset_completed_message extends prog_noneventbased_message {
    ...
    public function __construct($programid, $messageob=null, $uniqueid=null) {
        ...
        $this->fieldsetlegend = get_string('legend:coursesetcompletedmessage', 'corplms_program');

        $managermessagedata = array(
            'roleid'            => $this->managerrole,
            'subject'           => get_string('coursesetcompleted', 'corplms_program'),
            'fullmessage'       => $this->managermessage,
            'contexturl'        => $CFG->wwwroot.'/corplms/program/view.php?id='.$this->programid,
            'contexturlname'    => get_string('launchprogram', 'corplms_program'),
        );
...
class prog_program_due_message extends prog_eventbased_message {
    ...
    public function __construct($programid, $messageob=null, $uniqueid=null) {
        ...
        $this->fieldsetlegend = get_string('legend:programduemessage', 'corplms_program');
        $this->triggereventstr = get_string('beforeprogramisdue', 'corplms_program');

        $managermessagedata = array(
            'roleid'            => $this->managerrole,
            'subject'           => get_string('programdue', 'corplms_program'),
            'fullmessage'       => $this->managermessage,
            'contexturl'        => $CFG->wwwroot.'/corplms/program/view.php?id='.$this->programid,
            'contexturlname'    => get_string('launchprogram', 'corplms_program'),
        );
...
class prog_courseset_due_message extends prog_eventbased_message {
    ...
    public function __construct($programid, $messageob=null, $uniqueid=null) {
        ...
        $this->fieldsetlegend = get_string('legend:coursesetduemessage', 'corplms_program');
        $this->triggereventstr = get_string('beforesetisdue', 'corplms_program');

        $managermessagedata = array(
            'roleid'            => $this->managerrole,
            'subject'           => get_string('coursesetdue', 'corplms_program'),
            'fullmessage'       => $this->managermessage,
            'contexturl'        => $CFG->wwwroot.'/corplms/program/view.php?id='.$this->programid,
            'contexturlname'    => get_string('launchprogram', 'corplms_program'),
        );
...
class prog_program_overdue_message extends prog_eventbased_message {
    ...
    public function __construct($programid, $messageob=null, $uniqueid=null) {
        ...
        $this->fieldsetlegend = get_string('legend:programoverduemessage', 'corplms_program');
        $this->triggereventstr = get_string('afterprogramisdue', 'corplms_program');

        $managermessagedata = array(
            'roleid'            => $this->managerrole,
            'subject'           => get_string('programoverdue', 'corplms_program'),
            'fullmessage'       => $this->managermessage,
            'contexturl'        => $CFG->wwwroot.'/corplms/program/view.php?id='.$this->programid,
            'contexturlname'    => get_string('launchprogram', 'corplms_program'),
        );
...
class prog_courseset_overdue_message extends prog_eventbased_message {
    ...
    public function __construct($programid, $messageob=null, $uniqueid=null) {
        ...
        $this->fieldsetlegend = get_string('legend:coursesetoverduemessage', 'corplms_program');
        $this->triggereventstr = get_string('aftersetisdue', 'corplms_program');

        $managermessagedata = array(
            'roleid'            => $this->managerrole,
            'subject'           => get_string('coursesetoverdue', 'corplms_program'),
            'fullmessage'       => $this->managermessage,
            'contexturl'        => $CFG->wwwroot.'/corplms/program/view.php?id='.$this->programid,
            'contexturlname'    => get_string('launchprogram', 'corplms_program'),
        );
...
class prog_learner_followup_message extends prog_eventbased_message {
    ...
    public function __construct($programid, $messageob=null, $uniqueid=null) {
        ...
        $this->fieldsetlegend = get_string('legend:learnerfollowupmessage', 'corplms_program');
        $this->triggereventstr = get_string('afterprogramiscompleted', 'corplms_program');
        $this->notifymanager = false;
    }
...
class prog_extension_request_message extends prog_noneventbased_message {
    ...
    public function __construct($programid, $userid, $messageob=null, $uniqueid=null, $data) {
        ...
        $this->fieldsetlegend = get_string('legend:extensionrequestmessage', 'corplms_program');
        $this->userid = $userid;
        ...
        $managermessagedata = array(
            ...
            'subject'           => $this->messagesubject,
            'fullmessage'       => $this->mainmessage,
            'contexturl'        => $CFG->wwwroot.'/corplms/program/manageextensions.php?userid='.$this->userid,
            'contexturlname'    => get_string('manageextensionrequests', 'corplms_program'),
        );
...
class prog_recert_windowopen_message extends prog_eventbased_message {
    ...
    public function __construct($programid, $messageob=null, $uniqueid=null) {
        ...
        $this->fieldsetlegend = get_string('legend:recertwindowopenmessage', 'corplms_certification');
        $managermessagedata = array(
            'roleid'            => $this->managerrole,
            'subject'           => get_string('recertwindowopen', 'corplms_certification'),
            'fullmessage'       => $this->managermessage,
            'contexturl'        => $CFG->wwwroot.'/corplms/program/view.php?id='.$this->programid,
            'contexturlname'    => get_string('launchprogram', 'corplms_program'),
        );
    ...
    public function get_message_form_template(&$mform, &$template_values, &$formdataobject, $updateform=true) {
        ...
        $helpbutton = $OUTPUT->help_icon($this->helppage, 'corplms_certification');
...
class prog_recert_windowdueclose_message extends prog_eventbased_message {
    ...
    public function __construct($programid, $messageob=null, $uniqueid=null) {
        ...
        $this->fieldsetlegend = get_string('legend:recertwindowdueclosemessage', 'corplms_certification');
        $this->triggereventstr = get_string('beforewindowduetoclose', 'corplms_certification');
        $managermessagedata = array(
            'roleid'            => $this->managerrole,
            'subject'           => get_string('recertwindowdueclose', 'corplms_certification'),
            'fullmessage'       => $this->managermessage,
            'contexturl'        => $CFG->wwwroot.'/corplms/program/view.php?id='.$this->programid,
            'contexturlname'    => get_string('launchprogram', 'corplms_program'),
        );
...
class prog_recert_failrecert_message extends prog_eventbased_message {
    ...
    public function __construct($programid, $messageob=null, $uniqueid=null) {
        ...
        $this->fieldsetlegend = get_string('legend:recertfailrecertmessage', 'corplms_certification');
        $managermessagedata = array(
            'roleid'            => $this->managerrole,
            'subject'           => get_string('recertfailrecert', 'corplms_certification'),
            'fullmessage'       => $this->managermessage,
            'contexturl'        => $CFG->wwwroot.'/corplms/program/view.php?id='.$this->programid,
            'contexturlname'    => get_string('launchprogram', 'corplms_program'),
        );
