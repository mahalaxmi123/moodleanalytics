...
/**
* Standard HTML output renderer for corplms_core module
*/
class corplms_program_renderer extends plugin_renderer_base {
    ...
    */
    public function get_cancel_button($params=null, $url='') {
        if (empty($url)) {
            $url = "/corplms/program/edit.php";
        }
        $link = new moodle_url($url, $params);
        $output = $this->output->action_link($link, get_string('cancelprogrammanagement', 'corplms_program'), null, array('id' => 'cancelprogramedits'));
    ...
    */
    public function render_current_status($data) {
        ...
        $programstatusstring = get_string($data->statusstr, 'corplms_program');

        $learnerinfo = html_writer::empty_tag('br') . html_writer::start_tag('span', array('class' => 'assignmentcount'));
        if ($data->exceptions > 0) {
            $learnerinfo .= get_string('learnersassignedexceptions', 'corplms_program', $data);
        } else {
            $learnerinfo .= get_string('learnersassigned', 'corplms_program', $data);
        }
        ...
        if ($data->audiencevisibilitywarning) {
            $coursevisibilityinfo .= get_string('audiencevisibilityconflictmessage', 'corplms_program');
        }
    ...
    */
    public function assignment_category_display($assignment_class, $headers, $buttonname, $data) {
        ...
        $html .= html_writer::start_tag('div', array('class' => 'total_user_count')) . get_string('total', 'corplms_program') . ': ';
    ...
    public function display_edit_assignment_form($id, $categories, $certificationpath) {
        ...
        $out .= $this->output->heading(get_string('programassignments', 'corplms_program'), 3);
        ...
        $out .= html_writer::tag('p', get_string('instructions:programassignments', 'corplms_program'));
        ...
        if ($js != '') {
            $jsmodule = array(
                'name' => 'corplms_programassignment',
                'fullpath' => '/corplms/program/assignment/program_assignment.js',
        ...
        if (!empty($dropdown_options)) {
            ...
            $out .= get_string('addnew', 'corplms_program');
            $out .= html_writer::select($dropdown_options, 'category_select_dropdown', array('initialvalue' => 1));
            $out .= get_string('toprogram', 'corplms_program');
            $out .= html_writer::tag('button', get_string('add'));
            $out .= html_writer::end_tag('div');
        }
        $out .= html_writer::start_tag('div', array('class' => 'overall_total'));
        $out .= $this->output->help_icon('totalassignments', 'corplms_program');
        $out .= ' ' . get_string('totalassignments', 'corplms_program') . ': ';
    ...
    */
    public function display_user_message_box($user, $a) {
        ...
        $cells[] = $cell;
        $cell = new html_table_cell(html_writer::start_tag('strong') . get_string('youareviewingxsrequiredlearning', 'corplms_program', $a) . html_writer::end_tag('strong'));
        $cells[] = $cell;
    ...
    */
    public function print_exceptions_link($url, $excount) {
        ...
        $out .= get_string('unresolvedexceptions', 'corplms_program', $excount);
        $out .= html_writer::end_tag('span');
        $out .= html_writer::start_tag('span', array('class' => 'exceptionslink'));
        $out .= html_writer::link($url, get_string('viewexceptions', 'corplms_program'));
        $out .= html_writer::end_tag('span');
    ...
    */
    public function print_search($programid, $previoussearch='', $resultcount = 0) {
        $url = new moodle_url('/corplms/program/exceptions.php', array('id' => $programid));
        $out = html_writer::start_tag('form', array('action' => $url->out(), 'method' => 'get'));
        $out .= html_writer::tag('label', get_string('searchforindividual', 'corplms_program'), array('for' => 'exception_search'));
        ...
        if ($previoussearch != '' && $resultcount > 0) {
            ...
            $out .= html_writer::tag('p', get_string('xresultsfory', 'corplms_core', $a));
        }
        return $out;
    }
    ...
    */
    public function print_exceptions_form($numexceptions, $numselectedexceptions, $programid, $selectiontype, $tabledata) {
        ...
        if ($numexceptions == 0) {
            $out .= html_writer::start_tag('p') . get_string('noprogramexceptions', 'corplms_program') . html_writer::end_tag('p');
        } else {
            ...
            $out .= html_writer::empty_tag('input', array('type' => 'submit', 'id' => 'applyactionbutton', 'name' => "submit", 'value' => get_string('proceed', 'corplms_program')));
            $out .= html_writer::end_tag('div');

            $out .= html_writer::start_tag('div') . html_writer::start_tag('p');
            $out .= html_writer::start_tag('span', array('id' => 'numselectedexceptions')) . $numselectedexceptions . html_writer::end_tag('span') . ' ' .get_string('learnersselected', 'corplms_program');
            ...
            $table->id = 'exceptions';
            $table->head = array(
                get_string('header:hash', 'corplms_program'),
                get_string('header:learners', 'corplms_program'),
                get_string('header:id','corplms_program'),
                get_string('header:issue','corplms_program'),
            );
    ...
    public function get_exceptiontype_selector($selectiontype) {
        ...
        require_once($CFG->dirroot . '/corplms/program/program_exceptions.class.php');

        $out = '';
        $options = array();
        $options[SELECTIONTYPE_NONE] = get_string('select', 'corplms_program');
        $options[SELECTIONTYPE_ALL] = get_string('alllearners', 'corplms_program');
        $options[SELECTIONTYPE_TIME_ALLOWANCE] = get_string('alltimeallowanceissues', 'corplms_program');
        $options[SELECTIONTYPE_ALREADY_ASSIGNED] = get_string('allcurrentlyassignedissues', 'corplms_program');
        $options[SELECTIONTYPE_COMPLETION_TIME_UNKNOWN] = get_string('allcompletiontimeunknownissues', 'corplms_program');
        $options[SELECTIONTYPE_DUPLICATE_COURSE] = get_string('allduplicatecourseissues', 'corplms_program');
        $out .= html_writer::start_tag('div');
    ...
    public function get_exceptionaction_selector() {
        ...
        $out = '';
        $options = array();
        $options[SELECTIONACTION_NONE] = get_string('action', 'corplms_program');
        $options[SELECTIONACTION_AUTO_TIME_ALLOWANCE] = get_string('setrealistictimeallowance', 'corplms_program');
        $options[SELECTIONACTION_OVERRIDE_EXCEPTION] = get_string('overrideandaddprogram', 'corplms_program');
        $options[SELECTIONACTION_DISMISS_EXCEPTION] = get_string('dismissandtakenoaction', 'corplms_program');
        $out .= html_writer::start_tag('div');
    ...
    */
    public function display_set_completion() {
        ...
        $out .= html_writer::start_tag('label', array('for' => 'completiontime')) . get_string('completeby', 'corplms_program') . html_writer::end_tag('label');
        $out .= html_writer::start_tag('div', array('class' => 'datepicker-wrapper'));
        $out .= html_writer::empty_tag('input', array('class' => 'completiontime', 'type' => 'text', 'name' => "completiontime", 'placeholder' => get_string('datepickerlongyearplaceholder', 'corplms_core')));
        $out .= html_writer::end_tag('div');
        $out .= ' ' . html_writer::start_tag('button', array('class' => 'fixeddate')) .
            get_string('setfixedcompletiondate', 'corplms_program') . html_writer::end_tag('button');
        $out .= html_writer::end_tag('div');

        $out .= html_writer::start_tag('div', array('id' => 'prog-completion-or-string'));
        $out .= get_string('or', 'corplms_program');
        $out .= html_writer::end_tag('div');

        $out .= html_writer::start_tag('div', array('id' => 'prog-completion-relative-date'));
        $out .= html_writer::start_tag('label', array('for' => 'timeamount')) . get_string('completewithin', 'corplms_program') .
        ...
        $out .= ' ' . get_string('of', 'corplms_program') . ' ';
        ...
        $out .= html_writer::start_tag('button', array('class' => 'relativeeventtime')) . get_string('settimerelativetoevent', 'corplms_program') . html_writer::end_tag('button');
        $out .= html_writer::end_tag('div');
    ...
    */
    public function display_set_extension() {
        $out = '';
        $out .= html_writer::start_tag('div');
        $out .= html_writer::start_tag('label', array('for' => 'extensiontime')) . get_string('extenduntil', 'corplms_program') . html_writer::end_tag('label');
        $out .= html_writer::empty_tag('input', array('class' => 'extensiontime', 'type' => 'text', 'name' => 'extensiontime', 'id' => 'extensiontime', 'size' => '20', 'maxlength' => '10', 'placeholder' => get_string('datepickerlongyearplaceholder', 'corplms_core')));
        $out .= html_writer::end_tag('div');
        $out .= html_writer::empty_tag('br');

        $out .= html_writer::start_tag('div');
        $out .= html_writer::start_tag('label', array('for' => 'extensionreason')) . get_string('reasonforextension', 'corplms_program') . html_writer::end_tag('label');
    ...
    protected function coursecat_category(programcat_helper $chelper, $coursecat, $depth, $type = 'program') {
        global $CFG;
        require_once($CFG->dirroot . '/corplms/coursecatalog/lib.php');
        ...
        $categorycount = corplms_get_category_item_count($coursecat->id, $type);
        ...
        if ($type == 'program') {
            $createcapability = 'corplms/program:createprogram';
        } else {
            $createcapability = 'corplms/certification:createcertification';
        }
        ...
        $categoryname = html_writer::link(new moodle_url('/corplms/program/index.php',
                        array('categoryid' => $coursecat->id, 'viewtype' => $type)),
                        $categoryname);
        $categoryname .= html_writer::tag('span', ' (' . $categorycount . ')',
                        array('title' => get_string('numberofprograms', 'corplms_program')));
    ...
    protected function coursecat_subcategories(programcat_helper $chelper, $coursecat, $depth, $type = 'program') {
        ...
        if ($totalcount > count($subcategories)) {
            if ($paginationurl) {
                ...
                if ($viewmoreurl->compare(new moodle_url('/corplms/program/index.php'), URL_MATCH_BASE)) {
    ...
    protected function coursecat_category_content(programcat_helper $chelper, $coursecat, $depth, $type = 'program') {
        ...
        if ($chelper->get_show_programs() > corplms_program_renderer::COURSECAT_SHOW_PROGRAMS_COUNT) {
            $programs = array();
     ...
     *
     * Invoked from /corplms/program/index.php
     *
    ...
    public function program_category($category, $viewtype = 'program', $subcategoryforajax = false) {
        ...
        if (!$subcategoryforajax) {
            ...
            $label = ($viewtype == 'program') ? get_string('programcategories', 'corplms_program') :
                                                get_string('certifcategories', 'corplms_certification');
            if (!$coursecat->id) {
                if (can_edit_in_category()) {
                    // Add 'Manage' button instead of program search form.
                    $titlebutton = ($viewtype == 'program' ? get_string('manageprograms', 'admin') :
                                                             get_string('managecertifications', 'corplms_certification'));
                    $managebutton = $this->single_button(new moodle_url('/corplms/program/manage.php', array('viewtype' => $viewtype)),
                                    $titlebutton, 'get');
                ...
                if (coursecat::count_all() == 1) {
                    ...
                    $strfulllistofprograms = get_string('fulllistofprograms', 'corplms_program');
                    $this->page->set_title("$site->shortname: $strfulllistofprograms");
                } else {
                    $this->page->set_title($label);
                    $this->page->navbar->add($label, new moodle_url('/corplms/program/index.php', array('viewtype' => $viewtype)));
                }
            } else {
                ...
                $select = new single_select(new moodle_url('/corplms/program/index.php', array('viewtype' => $viewtype)), 'categoryid',
        ...
        $baseurl = new moodle_url('/corplms/program/index.php');
        if ($coursecat->id) {
        ...
        if ($browse === 'categories' || !$hasitems && $coursecat->id != 0) {
            ...
            $programdisplayoptions['viewmoretext'] = new lang_string("viewall{$viewtype}s", "corplms_{$viewtype}");
        } else {
        ...
        if (!$subcategoryforajax) {
            ...
            if ($viewtype == 'program' && has_capability('corplms/program:createprogram', $context)) {
                // Print link to create a new program, for the 1st available category.
                if ($coursecat->id) {
                    $url = new moodle_url('/corplms/program/add.php', array('category' => $coursecat->id, 'returnto' => 'category'));
                } else {
                    $url = new moodle_url('/corplms/program/add.php', array('category' => $CFG->defaultrequestcategory, 'returnto' => 'topcat'));
                }
                $output .= $this->single_button($url, get_string('addnewprogram', 'corplms_program'), 'get');
            } else if (has_capability('corplms/certification:createcertification', $context)) {
                if ($coursecat->id) {
                    $url = new moodle_url('/corplms/certification/add.php', array('category' => $coursecat->id, 'returnto' => 'category'));
                } else {
                    $url = new moodle_url('/corplms/certification/add.php', array('category' => $CFG->defaultrequestcategory, 'returnto' => 'topcat'));
                }
                $output .= $this->single_button($url, get_string('addnewcertification', 'corplms_certification'), 'get');
            }
    ...
    function program_search_form($type = 'program', $value = '', $format = 'plain') {
        ...
        if ($type == 'program') {
            $strsearchprograms = get_string('searchprograms', 'corplms_program');
        } else {
            $strsearchprograms = get_string('searchcertifications', 'corplms_certification');
        }
        $searchurl = new moodle_url('/corplms/program/search.php');
        ...
        if (!empty($searchcriteria)) {
            ...
            if (!$totalcount) {
                if (!empty($searchcriteria['search'])) {
                    $content .= html_writer::tag('p', get_string('noprogramsfound', 'corplms_program', $searchcriteria['search']));
                } else {
                    $content .= html_writer::tag('p', get_string('novalidprograms', 'corplms_program'));
                }
            } else {
     ...
     * please use {@link corplms_program_renderer::program_info_box()}
     *
     * @param programcat_helper $chelper various display options
    ...
    protected function coursecat_programbox(programcat_helper $chelper, $program, $additionalclasses = '') {
        ...
        $programicon = corplms_get_icon($program->id, CORPLMS_ICON_TYPE_PROGRAM);
        $dimmed = corplms_get_style_visibility($program);
        $programnamelink = html_writer::link(new moodle_url('/corplms/program/view.php', array('id' => $program->id)),
        ...
        if ($chelper->get_show_programs() < self::COURSECAT_SHOW_PROGRAMS_EXPANDED) {
            if ($program->has_summary() || $program->has_program_overviewfiles()) {
                $url = new moodle_url('/corplms/program/info.php', array('id' => $program->id));
    ...
    public function coursecat_programbox_content(programcat_helper $chelper, $program) {
        ...
        foreach ($program->get_program_overviewfiles() as $file) {
            ...
            $url = moodle_url::make_pluginfile_url($file->get_contextid(), 'corplms_program',
        ...
        if ($chelper->get_show_programs() == self::COURSECAT_SHOW_PROGRAMS_EXPANDED_WITH_CAT) {
            ...
            if ($cat = coursecat::get($program->category, IGNORE_MISSING)) {
                ...
                $content .= get_string('category').': '.
                                html_writer::link(new moodle_url('/corplms/program/index.php', array('categoryid' => $cat->id)),
    ...
    /**
     * Serves requests to /corplms/program/category.ajax.php
     *
    ...
    public function program_description_ajax(program $program) {
        ...
        $summary = file_rewrite_pluginfile_urls($program->summary, 'pluginfile.php', $context->id, 'corplms_program', 'summary', 0);
    ...
    protected function include_js() {
        ...
        if (!$jsloaded) {
            ...
            $this->page->requires->yui_module('moodle-corplms_program-categoryexpander', 'M.program.categoryexpander.init');
            $jsloaded = true;
        }
    }
}
...
class programcat_helper extends coursecat_helper {
    ...
    protected $showprograms = 20; /* corplms_program_renderer::COURSECAT_SHOW_PROGRAMS_EXPANDED */
    ...
    public function set_show_programs($showprograms) {
        ...
        $this->programsdisplayoptions['summary'] = $showprograms >= corplms_program_renderer::COURSECAT_SHOW_PROGRAMS_AUTO;
        return $this;
    }
