 ...
 * @subpackage program
 */
M.corplms_programassignment = M.corplms_programassignment || {
    ...
    corplmsDialog_program_cat: null,
    ...
    init: function(Y, args){
        ...
        if (!this.config.id) {
            throw new Error('M.corplms_programassignment.init()-> Required config \'id\' not available.');
        }

        // check jQuery dependency and continue with setup
        if (typeof $ === 'undefined') {
            throw new Error('M.corplms_programassignment.init()-> jQuery dependency required for this module to function.');
        }
         ...
         */
        this.corplmsDialog_program_cat = function(category, catid, name, find_url, title) {

            // Store some bits
            this.category = category;
            this.catid = catid;
            this.name = name;
            this.url = M.cfg.wwwroot+'/corplms/program/assignment/';
            ...
            var self = this;
            var buttonsObj = {};
            buttonsObj[M.util.get_string('cancel','corplms_program')] = function() { self.hide(); };
            buttonsObj[M.util.get_string('ok','corplms_program')] = function() { self.category.save(); };
        ...
        var corplmsDialog_completion_handler = function() {};

        corplmsDialog_completion_handler.prototype = new corplmsDialog_handler();

        corplmsDialog_completion_handler.prototype.first_load = function() {
            M.corplms_core.build_datepicker(module.Y, 'input[name="completiontime"]', M.util.get_string('datepickerlongyeardisplayformat', 'corplms_core'));
        ...
        var corplmsDialog_completion = function() {
        ...
        var corplmsDialog_savechanges = function() {
            ...
            var self = this;
            var buttonsObj = {};
            buttonsObj[M.util.get_string('editassignments','corplms_program')] = function() { handler._cancel() };
            ...
            {
                buttons: buttonsObj,
                title: '<h2>'+M.util.get_string('confirmassignmentchanges', 'corplms_program')+'</h2>'
            },
        ...
        corplmsDialog_completion_event = function() {
            ...
            var buttonsObj = {};
            buttonsObj[M.util.get_string('cancel','corplms_program')] = function() { handler._cancel() };
            buttonsObj[M.util.get_string('ok','corplms_program')] = function() { self.save() };
            ...
            {
                buttons: buttonsObj,
                title: '<h2>'+M.util.get_string('chooseitem', 'corplms_program') + module.config.display_selected_completion_event +'</h2>'
            },
        ...
        $('#category_select button').click(function() {
            var url = M.cfg.wwwroot+'/corplms/program/assignment/get_items.php?progid='+module.config.id;
    ...
    handleSaveChanges: function(event){
        ...
        corplmsDialogs['savechanges'].open(html);
        corplmsDialogs['savechanges'].save = function() {
            corplmsDialogs['savechanges'].savechanges = true;
            this.hide();
...
function category(id, name, find_url, title) {
    ...
    this.dialog_additem = new M.corplms_programassignment.corplmsDialog_program_cat(this, id, name, find_url, title);
...
function item(category, element, isexistingitem) {
    ...
    // Update the hidden inputs
        ...
        $.get(url, function(data) {
            if (data == 'error') {
                ...
                if (data == M.util.get_string('setcompletion', 'corplms_program')) {
                    ...
                    if (deletecompletiondatelink.length == 0) {
                        var newlink = $('<a href="#" class="deletecompletiondatelink"><img class="smallicon" src="'+M.util.image_url('t/delete', 'moodle')+'" alt="'+M.util.get_string('removecompletiondate', 'corplms_program')+'" title="'+M.util.get_string('removecompletiondate', 'corplms_program')+'" /></a>');
                        self.completionlink.parent().append(newlink);
                    }
                }
                self.completionlink.html(data);
                if (typeof completiontime === 'undefined' || completiontime == M.corplms_programassignment.config.COMPLETION_TIME_NOT_SET) {
                ...
                if (typeof completionevent === 'undefined' || completionevent == M.corplms_programassignment.config.COMPLETION_EVENT_NONE) {
    ...
    this.element.on('click', '.deletecompletiondatelink', function(event){
        event.preventDefault();
        self.update_completiontime(M.corplms_programassignment.config.COMPLETION_TIME_NOT_SET, 0, 0);
    });
    ...
    this.element.on('click', '.completionlink', function(event){
        ...
        event.preventDefault();
        corplmsDialogs['completion'].item = self;
        corplmsDialogs['completion'].default_url = self.category.url + 'set_completion.php';
