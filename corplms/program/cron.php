...
function program_hourly_cron() {
    ...
    $lasthourlycron = get_config(null, 'corplms_program_lasthourlycron');
    ...
    if (!set_config('corplms_program_lasthourlycron', $timenow)) {
...
function program_daily_cron() {
    ...
    $lastdailycron = get_config(null, 'corplms_program_lastdailycron');
    ...
    if (!set_config('corplms_program_lastdailycron', $timenow)) {
            ...
            if ($newcourse = $DB->get_record('course', array('id' => $recurrence_rec->nextcourseid))) {
                ...
                foreach ($users as $user) {
                    ...
                    if ($user->completionstatus == STATUS_PROGRAM_INCOMPLETE) {
                        ...
                        $messagedata->subject = $stringmanager->get_string('z:incompleterecurringprogramsubject', 'corplms_program', null, $user->lang);
                        $messagedata->fullmessage = $stringmanager->get_string('z:incompleterecurringprogrammessage', 'corplms_program', null, $user->lang);
                        $messagedata->contexturl = $CFG->wwwroot . '/course/view.php?id=' . $course->id;
                        $messagedata->contexturlname = $stringmanager->get_string('launchcourse', 'corplms_program', null, $user->lang);
