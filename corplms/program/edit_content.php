...
if (!has_capability('corplms/program:configurecontent', $programcontext)) {
    print_error('error:nopermissions', 'corplms_program');
}

$PAGE->set_url(new moodle_url('/corplms/program/edit_content.php', array('id' => $id)));
...
$overviewurl = $CFG->wwwroot."/corplms/program/edit.php?id={$id}&action=view";
...
if ($rawdata = data_submitted()) {
    ...
    if (!$programcontent->setup_content($rawdata)) {
        print_error('error:unabletosetupprogcontent', 'corplms_program');
    }
    ...
    if (isset($rawdata->{'addcontent'.$addsuffix})) {
        if (!$programcontent->add_set($rawdata->{'contenttype'.$addsuffix})) {
            echo $OUTPUT->notification(get_string('error:unabletoaddset', 'corplms_program'));
        }
    } else if (isset($rawdata->{'update'.$updatesuffix})) {
        $programcontent->update_content();
        echo $OUTPUT->notification(get_string('contentupdatednotsaved', 'corplms_program'));
    } else if ($setnumber = $programcontent->check_set_action('delete', $rawdata)) {
        if (!$programcontent->delete_set($setnumber)) {
            echo $OUTPUT->notification(get_string('error:deleteset', 'corplms_program'));
        } else {
            $submitdata = true;
        }
        ...
        if (!$programcontent->add_course($setnumber, $rawdata)) {
            echo $OUTPUT->notification(get_string('error:setunabletoaddcourse', 'corplms_program'));
        } else {
            $submitdata = true;
        }
        ...
        if (!$programcontent->add_competency($setnumber, $rawdata)) {
            echo $OUTPUT->notification(get_string('error:setunableaddcompetency', 'corplms_program'));
        } else {
            $submitdata = true;
        }
        ...
        if (!$programcontent->delete_course($action->setnumber, $action->courseid, $rawdata)) {
            echo $OUTPUT->notification(get_string('error:setunabletodeletecourse', 'corplms_program', $action->setnumber));
        } else {
            $submitdata = true;
        }
    }
...
if ($contenteditform->is_cancelled()) {
    corplms_set_notification(get_string('programupdatecancelled', 'corplms_program'), $overviewurl, array('class' => 'notifysuccess'));
}
...
if ($data = $contenteditform->get_data()) {
    if (isset($data->savechanges) || $submitdata) {
        ...
        if (!$programcontent->setup_content($data)) {
            print_error('error:setupprogcontent', 'corplms_program');
        }
        ...
        if (!$programcontent->save_content()) {
            corplms_set_notification(get_string('programupdatefail', 'corplms_program'), $currenturl);
        } else {
            // log this request
            ...
            if (isset($data->savechanges) || $submitdata) {
                corplms_set_notification(get_string('programcontentsaved', 'corplms_program'),
...
$PAGE->requires->strings_for_js(array('addcourseset', 'addcourses', 'cancel', 'ok', 'addcompetency',
            ...
            'corplms_program');
$selected_addrecurringcourse = json_encode(dialog_display_currently_selected(get_string('selected', 'corplms_hierarchy'), 'addrecurringcourse'));
$selected_addcompetency = json_encode(dialog_display_currently_selected(get_string('selected', 'corplms_hierarchy'), 'addcompetency'));
...
$jsmodule = array(
     'name' => 'corplms_programcontent',
     'fullpath' => '/corplms/program/content/program_content.js',
     'requires' => array('json','event-delegate')
     );
$PAGE->requires->js_init_call('M.corplms_programcontent.init', $args, false, $jsmodule);
