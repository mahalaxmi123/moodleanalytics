...
class prog_messages_manager {
        ...
        if (count($messages) > 0) {
            foreach ($messages as $message) {
                ...
                if (!array_key_exists($message->messagetype, $this->message_classnames)) {
                    throw new ProgramMessageException(get_string('meesagetypenotfound', 'corplms_program'));
                }
            ...
            $enrolment_message->messagesubject = get_string('defaultenrolmentmessage_subject', 'corplms_program');
            $enrolment_message->mainmessage = get_string('defaultenrolmentmessage_message', 'corplms_program');
            ...
            $exception_report_message->messagesubject = get_string('defaultexceptionreportmessage_subject', 'corplms_program');
            $exception_report_message->mainmessage = get_string('defaultexceptionreportmessage_message', 'corplms_program');
    ...
    public function display_form_label() {
        $out = '';
        $out .= get_string('instructions:messages1', 'corplms_program');
        return $out;
    }
    ...
    public function display_form_element() {
        ...
        if (count($this->messages)) {
            ...
            foreach ($this->messages as $message) {
                ...
                $component = (substr($messageclassname, 0, 11) == 'prog_recert' ? 'corplms_certification' : 'corplms_program');
                $out .= html_writer::tag('p', get_string($messageclassname, $component), array('class' => $styleclass));
                $messagecount++;
            }
        } else {
            $out .= get_string('noprogrammessages', 'corplms_program');
        }
    ...
    public function get_message_form_template(&$mform, &$template_values, $messages=null, $updateform=true) {
        ...
        if ($updateform) {
            $mform->addElement('submit', 'update', get_string('update', 'corplms_program'));
        ...
        $templatehtml .= $OUTPUT->heading(get_string('programmessages', 'corplms_program'));
        $templatehtml .= html_writer::tag('p', get_string('instructions:programmessages', 'corplms_program'));

        $templatehtml .= html_writer::start_tag('div', array('id' => 'messages'));

        if ($nummessages == 0) { // if there are no messages yet
            $templatehtml .= html_writer::tag('p', get_string('noprogrammessages', 'corplms_program'));
        } else {
        ...
        if ($canaddmessage) {
            ...
            if ($updateform) {
                $messageoptions = array(
                    MESSAGETYPE_ENROLMENT => get_string('enrolment', 'corplms_program'),
                    MESSAGETYPE_EXCEPTION_REPORT => get_string('exceptionsreport', 'corplms_program'),
                    MESSAGETYPE_UNENROLMENT => get_string('unenrolment', 'corplms_program'),
                    MESSAGETYPE_PROGRAM_DUE => get_string('programdue', 'corplms_program'),
                    MESSAGETYPE_PROGRAM_OVERDUE => get_string('programoverdue', 'corplms_program'),
                    MESSAGETYPE_PROGRAM_COMPLETED => get_string('programcompleted', 'corplms_program'),
                    MESSAGETYPE_COURSESET_DUE => get_string('coursesetdue', 'corplms_program'),
                    MESSAGETYPE_COURSESET_OVERDUE => get_string('coursesetoverdue', 'corplms_program'),
                    MESSAGETYPE_COURSESET_COMPLETED => get_string('coursesetcompleted', 'corplms_program'),
                    MESSAGETYPE_LEARNER_FOLLOWUP => get_string('learnerfollowup', 'corplms_program')
                );

                // add extra messages if a certification-program
                $prog = $DB->get_record('prog', array('id' => $this->programid));
                if ($prog->certifid > 0) {
                    $messageoptions[MESSAGETYPE_RECERT_WINDOWOPEN]     = get_string('recertwindowopen', 'corplms_certification');
                    $messageoptions[MESSAGETYPE_RECERT_WINDOWDUECLOSE] = get_string('recertwindowdueclose', 'corplms_certification');
                    $messageoptions[MESSAGETYPE_RECERT_FAILRECERT]     = get_string('recertfailrecert', 'corplms_certification');
                }

                $mform->addElement('select', 'messagetype', get_string('addnew', 'corplms_program'), $messageoptions, array('id'=>'messagetype'));
                $mform->setType('messagetype', PARAM_INT);
                $template_values['%messagetype%'] = array('name'=>'messagetype', 'value'=>null);
            }
            $templatehtml .= html_writer::tag('label', get_string('addnew', 'corplms_program'), array('for' => 'messagetype'));
            $templatehtml .= '%messagetype%';
            $templatehtml .= get_string('toprogram', 'corplms_program');
        ...
        if ($updateform) {
            $mform->addElement('cancel', 'cancel', get_string('cancel', 'corplms_program'));
