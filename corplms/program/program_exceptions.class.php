...
class prog_exceptions_manager {
    ...
    function __construct($programid) {
        ...
        $this->exceptiontype_descriptors = array(
            EXCEPTIONTYPE_TIME_ALLOWANCE    => get_string('timeallowance', 'corplms_program'),
            EXCEPTIONTYPE_ALREADY_ASSIGNED  => get_string('currentlyassigned', 'corplms_program'),
            EXCEPTIONTYPE_COMPLETION_TIME_UNKNOWN  => get_string('completiontimeunknown', 'corplms_program'),
            EXCEPTIONTYPE_UNKNOWN => get_string('unknownexception', 'corplms_program'),
            EXCEPTIONTYPE_DUPLICATE_COURSE  => get_string('duplicatecourse', 'corplms_program')
        );
    ...
    public function handle_exception($exceptionid, $action) {
        ...
        if (!$exception = $DB->get_record('prog_exception', array('id' => $exceptionid))) {
            throw new ProgramExceptionException(get_string('exceptionnotfound', 'corplms_program'));
        }

        if (!array_key_exists($exceptiontype, $this->exceptiontype_classnames)) {
            throw new ProgramExceptionException(get_string('exceptiontypenotfound', 'corplms_program'));
        }
