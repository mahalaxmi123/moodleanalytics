...
class prog_content {
    ...
    function __construct($programid) {
        ...
        foreach ($sets as $set) {
            if (!array_key_exists($set->contenttype, $this->courseset_classnames)) {
                throw new ProgramContentException(get_string('contenttypenotfound', 'corplms_program'));
            }
    ...
    public function save_content() {
        ...
        $event = \corplms_program\event\program_contentupdated::create(
            array(
    ...
    public function add_set($contenttype) {
        ...
        $courseset->label = get_string('legend:courseset', 'corplms_program', $courseset->sortorder);
    ...
    public function get_content_form_template(&$mform, &$template_values, $coursesets=null, $updateform=true, $iscertif=false, $certifpath=CERTIFPATH_CERT) {
        ...
        if ($iscertif) {
            ...
            $templatehtml .= html_writer::start_tag('legend', array('class' => 'ftoggler', 'id' => 'certifpath'))
                . get_string(($certifpath == CERTIFPATH_CERT ? 'oricertpath' : 'recertpath'), 'corplms_certification')
                . html_writer::end_tag('legend');
            $templatehtml .= html_writer::start_tag('p')
                . get_string(($certifpath == CERTIFPATH_CERT ? 'oricertpathdesc' : 'recertpathdesc'), 'corplms_certification')
                . html_writer::end_tag('p');

            if ($certifpath == CERTIFPATH_RECERT && $numcoursesets == 0) {
                // ask for cert content to be copied to recert
                $mform->addElement('advcheckbox', 'sameascert'.$suffix, get_string('sameascert', 'corplms_certification'),
                ...
                $templatehtml .= html_writer::start_tag('label', array('for' => 'sameascert'.$suffix))
                                . get_string('sameascert', 'corplms_certification') . html_writer::end_tag('label');
                $templatehtml .= '%sameascert'.$suffix.'%';
            }
        }

        $templatehtml .= $OUTPUT->heading(get_string('programcontent', 'corplms_program'));
        ...
        if ($iscertif) {
            $templatehtml .= html_writer::start_tag('p') . get_string('certificationcontent', 'corplms_certification') . html_writer::end_tag('p');
        } else {
            $templatehtml .= html_writer::start_tag('p') . get_string('instructions:programcontent', 'corplms_program') . html_writer::end_tag('p');
        }
        ...
        if ($numcoursesets == 0) { // if there's no content yet
            $templatehtml .= html_writer::start_tag('p') . get_string('noprogramcontent', 'corplms_program') . html_writer::end_tag('p');
        } else {
            foreach ($coursesets as $courseset) {
        ...
        if (!$recurring) {
            ...
            if ($updateform) {
                ...
                if ($iscertif) {
                    ...
                    $contentoptions = array(
                        CONTENTTYPE_MULTICOURSE => get_string('setofcourses', 'corplms_program'),
                    );
                } else {
                    $contentoptions = array(
                        CONTENTTYPE_MULTICOURSE => get_string('setofcourses', 'corplms_program'),
                        CONTENTTYPE_COMPETENCY => get_string('competency', 'corplms_program')
                    );
                    if ($numcoursesets == 0) { // don't allow recurring course to be added if the program already has other content
                        $contentoptions[CONTENTTYPE_RECURRING] = get_string('recurringcourse', 'corplms_program');
                    }
                }

                $mform->addElement('select', 'contenttype'.$suffix, get_string('addnew', 'corplms_program'), $contentoptions,
            ...
            $templatehtml .= html_writer::start_tag('label', array('for' => 'contenttype'.$suffix)) . get_string('addnew', 'corplms_program')
            ...
            if ($iscertif) {
                $helpbutton = $OUTPUT->help_icon('addcertifprogramcontent', 'corplms_certification');
            } else {
                $helpbutton = $OUTPUT->help_icon('addprogramcontent', 'corplms_program');
            }
     ...
     *
     * @param int $operator Constant to search for a value in corplms_search_for_value function
    ...
    public function get_visibility_coursesets($operator, $visibility) {
        ...
        foreach ($this->coursesets as $set) {
            if (get_class($set) === $this->courseset_classnames[CONTENTTYPE_MULTICOURSE]) {
                $courseaudiencevisibility += corplms_search_for_value($set->courses, 'audiencevisible', $operator, $visibility);
            } else if (get_class($set) === $this->courseset_classnames[CONTENTTYPE_COMPETENCY]){
                $courses = $set->get_competency_courses();
                $courseaudiencevisibility += corplms_search_for_value($courses, 'audiencevisible', $operator, $visibility);
            }
        }
