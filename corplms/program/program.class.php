...
require_once($CFG->dirroot . '/corplms/program/program_content.class.php');
require_once($CFG->dirroot . '/corplms/program/program_courseset.class.php');
require_once($CFG->dirroot . '/corplms/program/program_assignments.class.php');
require_once($CFG->dirroot . '/corplms/program/program_messages.class.php');
require_once($CFG->dirroot . '/corplms/program/program_message.class.php');
require_once($CFG->dirroot . '/corplms/program/program_exceptions.class.php');
require_once($CFG->dirroot . '/corplms/program/program_exception.class.php');
require_once($CFG->dirroot . '/corplms/program/program_user_assignment.class.php');
...
class program {
    ...
    function __construct($id) {
        ...
        if (!$program) {
            throw new ProgramException(get_string('programidnotfound', 'corplms_program', $id));
        }
    ...
    public function delete() {
        ...
        $event = \corplms_program\event\program_deleted::create(
            array(
                'objectid' => $this->id,
    ...
    public function update_learner_assignments() {
        ...
        if (!empty($message_queue)) {
            ...
            foreach ($message_queue as $userid => $eventdata) {
                ...
                $event = \corplms_program\event\program_assigned::create(
                    array(
    ...
    public function unassign_learners($userids) {
        ...
        foreach ($userids as $userid) {
            ...
            $event = \corplms_program\event\program_unassigned::create(
                array(
                    'objectid' => $this->id,
                    'context' => context_program::instance($this->id),
                    'userid' => $userid,
    ...
    public function make_timedue($userid, $assignment_record, $timedue) {
        ...
        if (!isset($COMPLETION_EVENTS_CLASSNAMES[$assignment_record->completionevent])) {
            throw new ProgramException(get_string('eventnotfound', 'corplms_program', $assignment_record->completionevent));
        }
    ...
    public function update_program_complete($userid, $completionsettings) {
        ...
        if ($completion = $DB->get_record('prog_completion', array('programid' => $this->id, 'userid' => $userid, 'coursesetid' => 0))) {
            ...
            if ($progcompleted_eventtrigger) {
                ...
                $event = \corplms_program\event\program_completed::create(
                    array(
                        'objectid' => $this->id,
                        'context' => context_program::instance($this->id),
                        'userid' => $userid,
    ...
    public function display($userid=null) {
        ...
        if (!$this->is_accessible()) {
            ...
            return html_writer::tag('p', get_string('programnotcurrentlyavailable', 'corplms_program'));
        }
        ...
        if ($userid && $userid != $USER->id) {
            ...
            if (!$user = $DB->get_record('user', array('id' => $userid))) {
                print_error('error:failedtofinduser', 'corplms_program', $userid);
            }
            $user->fullname = fullname($user);
            $user->wwwroot = $CFG->wwwroot;
            $message .= html_writer::tag('p', get_string('viewingxusersprogram', 'corplms_program', $user));
        }
        ...
        if ($userassigned) {
            ...
            if (count($user_assignments) > 0) {
                if ($viewinganothersprogram) {
                    $message .= html_writer::tag('p', get_string('assignmentcriteriamanager', 'corplms_program'));
                } else {
                    $message .= html_writer::tag('p', get_string('assignmentcriterialearner', 'corplms_program'));
                }
                ...
                if ($certifcompletion->certifpath == CERTIFPATH_CERT) {
                    if ($certifcompletion->renewalstatus == CERTIFRENEWALSTATUS_EXPIRED) {
                        ...
                        if ($timedue < $pasttimedue) {
                            $timedue = $pasttimedue;
                            $out .= get_string('certexpired', 'corplms_certification');
                        }
                    } else {
                        $out .= get_string('certinprogress', 'corplms_certification');
                    }
                } else {
                    $timedue = $certifcompletion->timeexpires;
                    if ($now > $certifcompletion->timewindowopens) {
                        $out .= get_string('recertwindowopen', 'corplms_certification');
                        $out .= get_string('recertwindowexpiredate', 'corplms_certification',
                                userdate($certifcompletion->timeexpires, get_string('strftimedatetime', 'langconfig')));
                    } else {
                        $out .= get_string('certcomplete', 'corplms_certification');
                        $out .= get_string('recertwindowopendate', 'corplms_certification',
        ...
        if ($userassigned) {
            ...
            if ($this->assigned_to_users_required_learning($userid) && $prog_completion->timedue != COMPLETION_TIME_NOT_SET && $prog_completion->timecompleted == 0) {
                ...
                if (!$extension = $DB->get_record('prog_extension', array('userid' => $userid, 'programid' => $this->id, 'status' => 0))) {
                    if (!$viewinganothersprogram && corplms_get_manager($userid)) {
                        // Show extension request link if it is their assignment and they have a manager to request it from.
                        $url = new moodle_url('/corplms/program/view.php', array('id' => $this->id, 'extrequest' => '1'));
                        $request = ' ' . html_writer::link($url, get_string('requestextension', 'corplms_program'), array('id' => 'extrequestlink'));
                    }
                } else {
                    // Show pending text if they have already requested an extension.
                    $request = ' ' . get_string('pendingextension', 'corplms_program');
                }
            }
            ...
            if ($prog_completion) {
                $startdatestr = ($prog_completion->timestarted != 0
                                ...
                                : get_string('nostartdate', 'corplms_program'));
                if ($iscertif) {
                ...
                $out .= html_writer::tag('div', get_string('startdate', 'corplms_program') . ': '
                                . $startdatestr, array('class' => 'item'));
                $out .= html_writer::tag('div', get_string('duedate', 'corplms_program').': '
                                . $duedatestr, array('class' => 'item'));
                $out .= html_writer::tag('div', get_string('progress', 'corplms_program') . ': '
 ...
 *
        $summary = file_rewrite_pluginfile_urls($this->summary, 'pluginfile.php',
            context_program::instance($this->id)->id, 'corplms_program', 'summary', 0);
 ...
 t2-feature-certification */
        ...
        if ($iscertif) {
            if (is_siteadmin()) {
                $out .= $OUTPUT->heading(get_string('oricertpath', 'corplms_certification'), 2);
                ...
                $out .= $OUTPUT->heading(get_string('recertpath', 'corplms_certification'), 2);
                $out .= html_writer::end_tag('div');
        ...
        if ($prog_completion && $prog_completion->status == STATUS_PROGRAM_COMPLETE) {
            ...
            $out .= $OUTPUT->heading(get_string('programends', 'corplms_program'), 2);
    ...
    function display_courseset($certifpath, $userid, $viewinganothersprogram) {
        ...
        if (count($courseset_groups) == 0) {
            $out .= html_writer::tag('p', get_string('nocoursecontent', 'corplms_program'), array('class' => 'nocontent'));
    ...
    function display_summary_widget($userid=null) {
        ...
        $params['id'] = $this->id;
        $url = new moodle_url("/corplms/program/required.php", $params);
        $summary = file_rewrite_pluginfile_urls($this->summary, 'pluginfile.php',
                context_program::instance($this->id)->id, 'corplms_program', 'summary', 0);
    ...
    function display_duedate($duedate, $userid, $certifpath = null, $certstatus = null) {
        ...
        if (empty($duedate) || $duedate == COMPLETION_TIME_NOT_SET) {
            if ($certifpath == null && $certstatus == null) {
                ...
                return get_string('duedatenotset', 'corplms_program');
            } else if ($certifpath == CERTIFPATH_CERT) {
                if ($certstatus == CERTIFSTATUS_EXPIRED) {
                    // This certification has expired.
                    return $OUTPUT->error_text(get_string('overdue', 'corplms_program'));
                } else {
                ...
                print_error('error:recertduedatenotset', 'corplms_program');
            }
    ...
    function display_duedate_highlight_info($duedate) {
        ...
        if (isset($duedate)) {
            ...
            if (($duedate < $now) && ($now - $duedate < 60*60*24)) {
                $out .= get_string('duetoday', 'corplms_plan');
            } else if ($duedate < $now) {
                $out .= $OUTPUT->error_text(get_string('overdue', 'corplms_plan'));
            } else if ($duedate - $now < 60*60*24*7) {
                $days = ceil(($duedate - $now)/(60*60*24));
                $out .= get_string('dueinxdays', 'corplms_plan', $days);
            }
    ...
    public function display_progress($userid) {
        ...
        if (!$prog_completion) {
            $out = get_string('notassigned', 'corplms_program');
            return $out;
        ...
        return $renderer->print_corplms_progressbar($overall_progress, 'medium', false, $tooltipstr);
    }
    ...
    public function display_timedue_date($completionstatus, $time, $format = '') {
        ...
        if ($time == 0 || $time == COMPLETION_TIME_NOT_SET) {
            return get_string('noduedate', 'corplms_plan');;
        }

        if (empty($format)) {
            $format = get_string('datepickerlongyearphpuserdate', 'corplms_core');
        }
        ...
        if ($completionstatus != STATUS_PROGRAM_COMPLETE) {
            ...
            if ($days_remaining == 1) {
                $days = get_string('onedayremaining', 'corplms_program');
            } else if ($days_remaining < 10 && $days_remaining > 0) {
                $days = get_string('daysremaining', 'corplms_program', $days_remaining);
            } else if ($time < time()) {
                $days = get_string('overdue', 'corplms_plan');
            }
        }
        if ($days != '') {
    ...
    public function display_link_program_icon($programname, $program_id, $program_icon, $userid = null) {
        ...
        if ($program->is_required_learning($userid) && $program->is_accessible()) {
            $html = $OUTPUT->action_link(
                new moodle_url('/corplms/program/required.php', array('id' => $programid, 'userid' => $userid)),
                $icon . $programname
            );
    ...
    public function display_exceptions_link() {
        ...
        if ($exceptionscount && $exceptionscount>0) {
            $url = new moodle_url('/corplms/program/exceptions.php', array('id' => $this->id));
            $renderer = $PAGE->get_renderer('corplms_program');
    ...
    public function display_current_status() {
        ...
        if (!empty($CFG->audiencevisibility)) {
            $coursesnovisible = $this->content->get_visibility_coursesets(CORPLMS_SEARCH_OP_NOT_EQUAL, COHORT_VISIBLE_ALL);
    ...
    public function is_viewable($user = null) {
        ...
        list($visibilitysql, $visibilityparams) = corplms_visibility_where($user->id,
                                                                            'p.id',
                                                                            'p.visible',
                                                                            'p.audiencevisible',
                                                                            'p',
    ...
    public function is_accessible($user = null) {
        ...
        if ($this->certifid) {
            $capability = 'corplms/certification:viewhiddencertifications';
        } else {
            $capability = 'corplms/program:viewhiddenprograms';
        }

        $syscontext = context_system::instance();
        $canmanagevisibility = has_capability('corplms/coursecatalog:manageaudiencevisibility', $syscontext) ||
...
class program_utilities {
    ...
    public static function duration_explode($duration) {
        ...
        if (array_key_exists($ob->period, $TIMEALLOWANCESTRINGS)) {
            $ob->periodstr = strtolower(get_string($TIMEALLOWANCESTRINGS[$ob->period], 'corplms_program'));
        } else {
            $ob->periodstr = '';
        }
    ...
    public static function print_duration_selector($prefix, $periodelementname, $periodvalue, $numberelementname, $numbervalue, $includehours=true) {
        ...
        if ($includehours) {
            $timeallowances[TIME_SELECTOR_HOURS] = get_string('hours', 'corplms_program');
        }
        $timeallowances[TIME_SELECTOR_DAYS] = get_string('days', 'corplms_program');
        $timeallowances[TIME_SELECTOR_WEEKS] = get_string('weeks', 'corplms_program');
        $timeallowances[TIME_SELECTOR_MONTHS] = get_string('months', 'corplms_program');
        $timeallowances[TIME_SELECTOR_YEARS] = get_string('years', 'corplms_program');
    ...
    public static function get_standard_time_allowance_options($infinity=false) {
        $timeallowances = array(
            TIME_SELECTOR_DAYS => get_string('days', 'corplms_program'),
            TIME_SELECTOR_WEEKS => get_string('weeks', 'corplms_program'),
            TIME_SELECTOR_MONTHS => get_string('months', 'corplms_program'),
            TIME_SELECTOR_YEARS => get_string('years', 'corplms_program')
        );
        if ($infinity) {
            $timeallowances[TIME_SELECTOR_INFINITY] = get_string('infinity', 'corplms_program');
        }
