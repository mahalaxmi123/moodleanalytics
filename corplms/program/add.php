...
$actualurl = new moodle_url('/corplms/program/add.php', array('category' => $categoryid, 'iscertif' => $iscertif));

// Integrate into the admin tree only if the user can create programs at the top level,
// otherwise the admin block does not appear to this user, and you get an error.
if ($iscertif) {
    if (has_capability('corplms/certification:createcertification', $systemcontext)) {
        admin_externalpage_setup('managecertifications', '', null, $actualurl);
    } else {
        $PAGE->set_context($systemcontext);
        $PAGE->set_url($actualurl);
        $PAGE->set_title(get_string('createnewcertification', 'corplms_certification'));
        $PAGE->set_heading(get_string('createnewcertification', 'corplms_certification'));
    }
} else {
    if (has_capability('corplms/program:createprogram', $systemcontext)) {
        admin_externalpage_setup('programmgmt', '', null, $actualurl);
    } else {
        $PAGE->set_context($systemcontext);
        $PAGE->set_url($actualurl);
        $PAGE->set_title(get_string('createnewprogram', 'corplms_program'));
        $PAGE->set_heading(get_string('createnewprogram', 'corplms_program'));
    }
}
...
$PAGE->requires->string_for_js('chooseicon', 'corplms_program');
$iconjsmodule = array(
        'name' => 'corplms_iconpicker',
        'fullpath' => '/corplms/core/js/icon.picker.js',
        'requires' => array('json'));

$iconargs = array('args' => '{"selected_icon":"default", "type":"program"}');

$PAGE->requires->js_init_call('M.corplms_iconpicker.init', $iconargs, false, $iconjsmodule);

// Visible audiences.
if (!empty($CFG->audiencevisibility)) {
    $PAGE->requires->strings_for_js(array('programcohortsvisible'), 'corplms_cohort');
    $jsmodule = array(
                    'name' => 'corplms_visiblecohort',
                    'fullpath' => '/corplms/cohort/dialog/visiblecohort.js',
                    'requires' => array('json'));
    $args = array('args'=>'{"visibleselected":"", "type":"program"}');
    $PAGE->requires->js_init_call('M.corplms_visiblecohort.init', $args, true, $jsmodule);
}
...
if ($categoryid) { // creating new program in this category
    ...
    if (!$iscertif) {
        require_capability('corplms/program:createprogram', context_coursecat::instance($category->id));
    } else {
        require_capability('corplms/certification:createcertification', context_coursecat::instance($category->id));
    }
...
if ($data = $form->get_data()) {
    ...
    if (isset($data->savechanges)) {
        ...
        $data = file_postupdate_standard_editor($data, 'summary', $editoroptions, $editoroptions['context'], 'corplms_program', 'summary', 0);
        $data = file_postupdate_standard_editor($data, 'endnote', $editoroptions, $editoroptions['context'], 'corplms_program', 'endnote', 0);
        if ($overviewfilesoptions = prog_program_overviewfiles_options($newid)) {
            // Save the course overviewfiles
            $data = file_postupdate_standard_filemanager($data, 'overviewfiles', $overviewfilesoptions, $editoroptions['context'], 'corplms_program', 'overviewfiles', 0);
        }
        ...
        // Visible audiences.
        if (!empty($CFG->audiencevisibility)) {
            $visiblecohorts = corplms_cohort_get_visible_learning($newid, $instancetype);
            ...
            if ($todelete = array_diff(array_keys($visiblecohorts), $newvisible)) {
                ...
                foreach ($todelete as $cohortid) {
                    corplms_cohort_delete_association($cohortid, $visiblecohorts[$cohortid]->associd,
            ...
            if ($newvisible = array_diff($newvisible, array_keys($visiblecohorts))) {
                ...
                foreach ($newvisible as $cohortid) {
                    corplms_cohort_add_association($cohortid, $newid, $instancetype, COHORT_ASSN_VALUE_VISIBLE);
                }
            }
        }
        ...
        if (has_capability('corplms/program:configuredetails', $programcontext)) {
            $viewurl = "{$CFG->wwwroot}/corplms/program/edit.php?id={$newid}&amp;action=edit";
        } else {
            $viewurl = "{$CFG->wwwroot}/corplms/program/view.php?id={$newid}";
        }
        ...
        if ($data->iscertif) {
            ...
            if (has_capability('corplms/certification:configuredetails', $programcontext)) {
                $viewurl = "{$CFG->wwwroot}/corplms/program/edit.php?id={$newid}";
            } else {
                $viewurl = "{$CFG->wwwroot}/corplms/program/view.php?id={$newid}";
            }

            $successmsg = get_string('certifprogramcreatesuccess', 'corplms_certification');
        } else {
            $successmsg = get_string('programcreatesuccess', 'corplms_program');
        }
        ...
        $event = \corplms_program\event\program_created::create(
            array(
                'objectid' => $newid,
      ...
      corplms_set_notification($successmsg, $viewurl, array('class' => 'notifysuccess'));
    }
}
...
///
if (!$iscertif) {
    $heading = get_string('createnewprogram', 'corplms_program');
    $pagetitle = format_string(get_string('program', 'corplms_program').': '.$heading);
} else {
    $heading = get_string('createnewcertifprog', 'corplms_certification');
    $pagetitle = format_string(get_string('certifprog', 'corplms_certification').': '.$heading);
}
