...
class program_content_edit_form extends moodleform {
    ...
    function add_errors() {
        ...
        if (!empty($mform->_errors)) {
            ...
            $mform->setConstant('errors', get_string('errorsinform', 'corplms_program'));
    ...
    function validation($data, $files) {
        ...
        foreach ($data as $elementname=>$elementvalue) {
            ...
            if (preg_match('/[0-9]timeallowednum/', $elementname)) {
                ...
                if ($timeallowednum < 0) {
                    $errors[$elementname] = get_string('error:timeallowednum_nonzero', 'corplms_program');
                }
            }
            ...
            if (preg_match('/[0-9]recurrencetimenum/', $elementname)) {
                $timeallowednum = $elementvalue;
                if ($timeallowednum <= 0) {
                    $errors[$elementname] = get_string('error:recurrence_nonzero', 'corplms_program');
                }
            }
            ...
            if (preg_match('/[0-9]recurcreatetimenum/', $elementname)) {
                $timeallowednum = $elementvalue;
                if ($timeallowednum <= 0) {
                    $errors[$elementname] = get_string('error:coursecreationrepeat_nonzero', 'corplms_program');
                }
            }
            ...
            if (preg_match('/[0-9]courses/', $elementname)) {
                ...
                if (empty($courses)) {
                    $errors[$elementname] = get_string('error:courses_nocourses', 'corplms_program');
                }
            }
        }
        return $errors;
