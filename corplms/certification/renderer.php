 ...
 */
class corplms_certification_renderer extends plugin_renderer_base {
    ...
    public function print_certification($data) {
        ...
        $out .= html_writer::empty_tag('img', array('src' => corplms_get_icon($data->pid, CORPLMS_ICON_TYPE_PROGRAM),
            'class' => 'course_icon', 'alt' => ''));
        $url = new moodle_url('/corplms/program/view.php', array('id' => $data->progid));
        $attributes = array('title' => get_string('viewprogram', 'corplms_program'), 'class' => $linkcss);
    ...
    public function get_cancel_button($params=null, $url='') {
        if (empty($url)) {
            $url = "/corplms/program/edit.php"; // Back to program edit.
        }
        $link = new moodle_url($url, $params);
        $output = $this->output->action_link($link, get_string('cancelcertificationmanagement', 'corplms_certification'),
