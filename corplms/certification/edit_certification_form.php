...
class edit_certification_form extends moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('header', 'editdetailshdr', get_string('editdetailshdr', 'corplms_certification'));
        $mform->addElement('html', html_writer::start_tag('p', array('class' => 'instructions')) .
                             get_string('editdetailsdesc', 'corplms_certification') . html_writer::end_tag('p'));

        // Active period num.
        $mform->addElement('html', html_writer::start_tag('p', array('class' => 'subheader')) .
                             get_string('editdetailsactivep', 'corplms_certification') . html_writer::end_tag('p'));
        $mform->addElement('html', html_writer::start_tag('p', array('class' => 'instructions')) .
                             get_string('editdetailsvalid', 'corplms_certification') . html_writer::end_tag('p'));
        $activegrp = array();
        ...
        $dateperiodoptions = array(
            'day' => get_string('days', 'corplms_certification'),
            'week' => get_string('weeks', 'corplms_certification'),
            'month' => get_string('months', 'corplms_certification'),
            'year' => get_string('years', 'corplms_certification'),
        );
        $activegrp[] = $mform->createElement('select', 'activeperiod', '', $dateperiodoptions);
        $mform->setDefault('activeperiod', $active[1]);
        $mform->addGroup($activegrp, 'activegrp', get_string('editdetailsactive', 'corplms_certification'), ' ', false);
        $mform->addHelpButton('activegrp', 'editdetailsactive', 'corplms_certification');

        $mform->registerRule('activeperiod_validation', 'function', 'activeperiod_validation');
        $mform->addRule('activegrp',
                get_string('error:minimumactiveperiod', 'corplms_certification'),
                'activeperiod_validation',
                $mform);

        // Recert window period num.
        $mform->addElement('html', html_writer::start_tag('p', array('class' => 'subheader')) .
                             get_string('editdetailsrcwin', 'corplms_certification') . html_writer::end_tag('p'));
        $windowgrp = array();
        ...
        $mform->addGroup($windowgrp, 'windowgrp', get_string('editdetailswindow', 'corplms_certification'), ' ', false);
        $mform->addHelpButton('windowgrp', 'editdetailswindow', 'corplms_certification');

        $mform->registerRule('windowperiod_validation', 'function', 'windowperiod_validation');
        $mform->addRule('windowgrp',
                get_string('error:minimumwindowperiod', 'corplms_certification', $timeallowance->timestring),
                'windowperiod_validation',
                $timeallowance->seconds);

        if ($timeallowance->seconds > 0) {
            $mform->addElement('html', html_writer::tag('p',
                    get_string('timeallowance', 'corplms_certification', $timeallowance),
        ...
        $mform->addElement('select', 'recertifydatetype', get_string('editdetailsrcopt', 'corplms_certification'), $recertoptions);
        $mform->setDefault('recertifydatetype', $recertifydatetype);
        $mform->addHelpButton('recertifydatetype', 'editdetailsrcopt', 'corplms_certification');
    ...
    public function validation($data, $files) {
        ...
        foreach ($data as $elementname => $elementvalue) {
            ...
            if ($elementname == 'activenum' || $elementname == 'windownum') {
                if ($elementvalue < 0) {
                    $errors[$elementname] = get_string('error:mustbepositive', 'corplms_certification');
                }
            }
        }
        return $errors;
