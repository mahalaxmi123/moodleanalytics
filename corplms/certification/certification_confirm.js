 ...
 */
M.corplms_certificationconfirm = M.corplms_certificationconfirm  || {
    ...
    init: function(Y, args){
        ...
        if (!this.config.id) {
            throw new Error('M.corplms_certificationconfirm init()-> Required config \'id\' not available.');
        }

        // check jQuery dependency and continue with setup
        if (typeof $ === 'undefined') {
            throw new Error('M.corplms_certificationconfirm.init()-> jQuery dependency required for this module to function.');
        }
        ...
        this.corplmsDialog_savechanges = function() {
            ...
            var buttonsObj = {};
            buttonsObj[M.util.get_string('editcertif', 'corplms_certification')] = function() { handler._cancel() };
            buttonsObj[M.util.get_string('saveallchanges', 'corplms_certification')] = function() { self.save() };
            ...
            {
                buttons: buttonsObj,
                title: '<h2>'+ M.util.get_string('confirmchanges', 'corplms_certification') +'</h2>'
            },
        ...
        window.onbeforeunload = function(e) {
            ...
            var str = M.util.get_string('youhaveunsavedchanges', 'corplms_certification');
    ...
    handleSaveChanges: function(){
        ...
        dialog.open(M.util.get_string('tosaveall', 'corplms_certification'));
