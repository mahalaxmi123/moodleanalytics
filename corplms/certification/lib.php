...
require_once($CFG->dirroot.'/corplms/program/program_assignments.class.php'); // For assignment category literals.
...
$CERTIFRECERT = array(
    ...
    CERTIFRECERT_COMPLETION => get_string('editdetailsrccmpl', 'corplms_certification'),
    CERTIFRECERT_EXPIRY => get_string('editdetailsrcexp', 'corplms_certification'),
);
...
class certification_event_handler {
     ...
     *
     * @param \corplms_program\event\program_assigned $event
     */
    public static function assigned(\corplms_program\event\program_assigned $event) {
        global $DB;
     ...
     *
     * @param \corplms_program\event\program_unassigned $event
     */
    public static function unassigned(\corplms_program\event\program_unassigned $event) {
        global $DB;
     ...
     *
     * @param \corplms_program\event\program_completed $event
     */
    public static function completed(\corplms_program\event\program_completed $event) {
        global $DB;
 ...
 */
function corplms_certification_cron() {
    global $CFG;
    require_once($CFG->dirroot . '/corplms/certification/cron.php');
    certification_cron();
}
...
function write_certif_completion($certificationid, $userid, $certificationpath = CERTIFPATH_CERT,
    ...
    if (!$certification) {
        print_error('error:incorrectcertifid', 'corplms_certification', null, $certificationid);
    }
...
function copy_certif_completion_to_hist($certificationid, $userid, $unassigned = false) {
    ...
    if (!$certificationcompletion) {
        print_error('error:incorrectid', 'corplms_certification');
    }
...
function send_certif_message($progid, $userid, $msgtype) {
    ...
    list($visibilitysql, $visibilityparams) = corplms_visibility_where(null, 'p.id', 'p.visible',
...
function certif_get_certifications_search($searchterms, $sort='fullname ASC', $page=0, $recordsperpage=50, &$totalcount,
    ...
    foreach ($rs as $certification) {
        ...
        if ($certification->visible || has_capability('corplms/certification:viewhiddencertifications',
...
function certif_print_certifications($category) {
    ...
    if ($certifications) {
        ...
        echo $OUTPUT->heading(get_string('nocertifications', 'corplms_certification'));
        $context = context_system::instance();
        if (has_capability('corplms/certification:createcertification', $context)) {
            $options = array();
            $options['category'] = $category->id;
            echo html_writer::start_tag('div', array('class' => 'addcertificationbutton'));
            echo $OUTPUT->single_button(new moodle_url('/corplms/certification/add.php', $options), get_string("addnewcertification", 'corplms_certification'), 'get');
...
function certif_print_certification($certification, $highlightterms = '') {
    ...
    $data->learningcomptypestr = get_string($CERTIFTYPE[$certification->learningcomptype], 'corplms_certification');
    $data->fullname = $certification->fullname;
    $data->summary = file_rewrite_pluginfile_urls($certification->summary, 'pluginfile.php',
        context_program::instance($certification->pid)->id, 'corplms_program', 'summary', 0);
...
function reset_certifcomponent_completions($certifcompletion, $courses=null) {
    ...
    if ($pcp = $DB->get_record('prog_completion', array('programid' => $prog->id, 'userid' => $userid, 'coursesetid' => 0))) {
        ...
        print_error('error:missingprogcompletion', 'corplms_certification', '', $certifcompletion);
    }
...
function certif_delete_messagelog($progid, $userid, $messagetype) {
    ...
    if ($manager = corplms_get_manager($userid)) {
        $userids[] = $manager->id;
    }
...
function get_timeexpires($base, $activeperiod) {
    if (empty($activeperiod)) {
        print_error('error:nullactiveperiod', 'corplms_certification');
    }
...
function get_timewindowopens($timeexpires, $windowperiod) {
    if (empty($windowperiod)) {
        print_error('error:nullwindowperiod', 'corplms_certification');
    }
...
function certif_can_delete_certifications($categoryid) {
    ...
    foreach ($programcontexts as $ctxrecord) {
        ...
        if (!has_capability('corplms/certification:deletecertification', $programcontext)) {
            return false;
        }
    }
