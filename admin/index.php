...
if (version_compare(phpversion(), '5.4.4') < 0) {
    ...
    echo "Corplms 2.7 or later requires at least PHP 5.4.4 (currently using version $phpversion).<br />";
    echo "Please upgrade your server software or install older Corplms version.";
    die();
}
...
if (!function_exists('iconv')) {
    ...
    echo 'Corplms requires the iconv PHP extension. Please install or enable the iconv extension.';
    die();
}
...
// Corplms upgrade version checks - only certain upgrade paths are permitted
// do this early to ensure upgrade hasn't started yet
//
// we also need to prevent attempts to downgrade from Moodle release that
// is later than current corplms version (e.g. Moodle 2.3 -> Corplms 2.2)
// This is already handled by the core upgrade code as it would detected a
// core downgrade and throw and exception

//setup corplms version variables
$corplmsinfo = corplms_version_info($version, $release);
if (!empty($corplmsinfo->corplmsupgradeerror)){
    print_error($corplmsinfo->corplmsupgradeerror, 'corplms_core', '', $corplmsinfo);
}
...
if (!core_tables_exist()) {
    ...
    if (empty($agreelicense)) {
        ...
        $PAGE->set_title($strinstallation.' - Corplms '.$CORPLMS->release);
    ...
    if (empty($confirmrelease)) {
        ...
        $PAGE->set_heading($strinstallation . ' - Corplms ' . $CORPLMS->release);
        $PAGE->set_cacheable(false);

        /** @var core_admin_renderer $output */
        $output = $PAGE->get_renderer('core', 'admin');
        echo $output->install_environment_page($maturity, $envstatus, $environment_results, $CORPLMS->release);
        die();
    }
...
// Always force autoupdates off in Corplms.
...
if (!$cache && ($version > $CFG->version
        || (isset($CFG->corplms_build) && version_compare($corplmsinfo->newcorplmsversion, $corplmsinfo->existingcorplmsversion, '>')))) {  // upgrade
    ...
    if (empty($confirmupgrade)) {
        $strdatabasechecking = get_string('databasechecking', '', $corplmsinfo);

        $PAGE->set_title($stradministration);
        $PAGE->set_heading($strdatabasechecking);
        $PAGE->set_cacheable(false);

        echo $output->upgrade_confirm_page($corplmsinfo, $maturity, $testsite);
        die();
        ...
        echo $output->upgrade_environment_page($CORPLMS->release, $envstatus, $environment_results);
        die();
        ...
        corplms_preupgrade($corplmsinfo);
        // Launch main upgrade.
...
if ( (!isset($CFG->corplms_release) || $CFG->corplms_release <> $CORPLMS->release)
    || (!isset($CFG->corplms_build) || $CFG->corplms_build <> $CORPLMS->build)
    || (!isset($CFG->corplms_version) || $CFG->corplms_version <> $CORPLMS->version)) {
    // Also set Corplms release (human readable version)
    set_config("corplms_release", $CORPLMS->release);
    set_config("corplms_build", $CORPLMS->build);
    set_config("corplms_version", $CORPLMS->version);
}
...
if (!empty($id) and $id == $CFG->siteidentifier) {
    // No registration of Corplms on Moodle.org!
...
echo $output->admin_notifications_page($maturity, $insecuredataroot, $errorsdisplayed, $cronoverdue, $dbproblems,
                                       ...
                                       $registered, $cachewarnings, $latesterror, $activeusers, $CORPLMS->release);
