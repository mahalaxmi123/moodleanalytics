 ...
 *
 * @author Moises Burgos <moises.burgos@corplmslms.com>
 * @package corplms
 * @subpackage admin_user
 */

/**
 * Script for bulk enabling/disabling corplms sync checkbox for users
 */
...
if ($confirm) {
    ...
    while ($users = corplms_pop_n($SESSION->bulk_users, 1000)) {
        ...
        if (!empty($users)) {
            ...
            foreach ($rs as $user) {
                if (!$DB->set_field('user', 'corplmssync', $enable, array('id' => $user->id))) {
                    $errornotifications .= $OUTPUT->notification(get_string('togglecorplmssyncerror', 'corplms_core', fullname($user, true)), 'notifynotice');
                    $errorcounter++;
                } else {
                    $usercount++;
                }
    ...
    if(!empty($badusernotification)) {
        echo $OUTPUT->notification(get_string('togglecorplmssyncerror:noidnumber', 'corplms_core', implode(',', $badusernotification)), 'notifynotice');
    }
    ...
    while ($users = corplms_pop_n($bulkusers, 1000)) {
    ...
    $formcontinue = $OUTPUT->single_button(new moodle_url('user_bulk_togglecorplmssync.php', array('confirm' => 1)), get_string('continue'));
    $formcancel = $OUTPUT->single_button(new moodle_url('user_bulk.php'), get_string('cancel'), 'get');

    echo $OUTPUT->box_start('generalbox', 'notice');
    echo html_writer::tag('p', get_string('enabledisablecorplmssync', 'corplms_core', $usernames));
