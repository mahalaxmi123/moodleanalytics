...
define('SITE_REGISTRATION_EMAIL', 'registrations@corplmslms.com');

/**
 *  Collect information to be sent to register.corplmslms.com
 *
...
function get_registration_data() {
    ...
    $data['corplmsversion'] = $CORPLMS->version;
    $data['corplmsbuild'] = $CORPLMS->build;
    $data['corplmsrelease'] = $CORPLMS->release;
...
/**
 * Send registration data to corplmslms.com
 *
...
function send_registration_data($data) {
    ...
    $recdata = $ch->post('https://register.corplmslms.com/register/report.php', $data, $options);
    if ($recdata === '') {
...
/**
 * Send registration data to corplmslms.com using curl
 *
 * @param array $data Associative array of data to send
...
function send_registration_data_email($data) {
    ...
    $message = get_string('siteregistrationemailbody', 'corplms_core', $data['sitefullname']);
