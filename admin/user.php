<?php
    ...
    $strundelete = get_string('undelete', 'corplms_core');
    ...
    if (has_capability('corplms/core:seedeletedusers', $sitecontext)) {
    ...
    if ($confirmuser and confirm_sesskey()) {
        ...
        if (!has_capability('corplms/core:undeleteuser', $sitecontext)) {
            print_error('undeleteusernoperm', 'corplms_core');
        }
        if (!$user = $DB->get_record('user', array('id' => $undelete))) {
            print_error('userdoesnotexist', 'corplms_core');
        }
        if (preg_match($preg_emailhash, $user->email)) {
            // ensure we're not trying to undelete a legacy-deleted (hash in email) user
            print_error('cannotundeleteuser', 'corplms_core');
        }

        if ($confirm != md5($undelete)) {
            echo $OUTPUT->header();
            $fullname = fullname($user, true);
            echo $OUTPUT->heading(get_string('undeleteuser', 'corplms_core'));
            $optionsyes = array('undelete' => $undelete, 'confirm' => md5($undelete), 'sesskey' => sesskey());
            echo $OUTPUT->confirm(get_string('undeletecheckfull', 'corplms_core', "'$fullname'"), new moodle_url($returnurl, $optionsyes), $returnurl);
            echo $OUTPUT->footer();
            die;
            ...
            if (undelete_user($user)) {
                ...
                corplms_set_notification(get_string('undeletedx', 'corplms_core', fullname($user, true)), $returnurl, array('class' => 'notifysuccess'));
            } else {
                corplms_set_notification(get_string('undeletednotx', 'corplms_core', fullname($user, true)), $returnurl);
            }
        }
