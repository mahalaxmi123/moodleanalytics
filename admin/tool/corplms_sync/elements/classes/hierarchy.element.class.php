...
abstract class corplms_sync_hierarchy extends corplms_sync_element {
    ...
    function config_form(&$mform) {
        $mform->addElement('header', 'crud', get_string('allowedactions', 'tool_corplms_sync'));
    ...
    function sync() {
        ...
        $this->addlog(get_string('syncstarted', 'tool_corplms_sync'), 'info', "{$elname}sync");
        if (!$synctable = $this->get_source_sync_table()) {
            throw new corplms_sync_exception($elname, "{$elname}sync", 'couldnotgetsourcetable');
        }

        // Create a clone of the temporary table
        if (!$synctable_clone = $this->get_source_sync_table_clone($synctable)) {
            throw new corplms_sync_exception($elname, "{$elname}sync", 'couldnotcreateclonetable');
        }

        if (!$this->check_sanity($synctable, $synctable_clone)) {
            $this->get_source()->drop_table($synctable_clone);
            throw new corplms_sync_exception($elname, "{$elname}sync", 'sanitycheckfailed');
        }
        ...
        $sql = "SELECT s.*
                ...
                LEFT OUTER JOIN {{$synctable_clone}} ss ON (ii.idnumber = ss.idnumber)
                          WHERE (ii.corplmssync=1 AND ss.idnumber IS NULL)
        ...
        if (!empty($this->config->allow_delete)) {
           ...
           LEFT OUTER JOIN {{$synctable}} s ON i.idnumber = s.idnumber
                     WHERE i.corplmssync=1 AND s.idnumber IS NULL";
            $rs = $DB->get_recordset_sql($sql);

            foreach ($rs as $r) {
                if (!$this->hierarchy->delete_hierarchy_item($r->id)) {
                    $this->addlog(get_string('cannotdeletex', 'tool_corplms_sync',
                        "{$elname} {$r->idnumber}"), 'warn', "{$elname}sync");
                } else {
                    $this->addlog(get_string('deletedx', 'tool_corplms_sync',
        ...
        $this->addlog(get_string('syncfinished', 'tool_corplms_sync'), 'info', "{$elname}sync");
    ...
    function sync_item($newitem, $synctable) {
        ...
        if (!$newitem->frameworkid = $DB->get_field("{$elname}_framework", 'id', array('idnumber' => $newitem->frameworkidnumber))) {
            throw new corplms_sync_exception($elname, 'syncitem', 'frameworkxnotfound',
        ...
        if (!empty($newitem->parentidnumber)
            ...
            if (!$newparent = $DB->get_record_sql($sql, array($newitem->parentidnumber), IGNORE_MULTIPLE)) {
                throw new corplms_sync_exception($elname, 'syncitem', 'parentxnotfound',
                    $newitem->parentidnumber);
            }
            try {
                $this->sync_item($newparent, $synctable);
            } catch (corplms_sync_exception $e) {
                throw new corplms_sync_exception($elname, 'syncitem', 'cannotsyncitemparent',
        ...
        if (!$dbitem = $DB->get_record($elname, array('idnumber' => $newitem->idnumber))) {  // TODO: make this scale
            ...
            if (!$hitem = $this->hierarchy->add_hierarchy_item($newitem, $newitem->parentid, $newitem->frameworkid, true, true, false)) {
                throw new corplms_sync_exception($elname, 'syncitem', 'cannotcreatex',
            ...
            $this->addlog(get_string('createdx', 'tool_corplms_sync', "{$elname} {$hitem->idnumber}"), 'info', 'syncitem');

            return true;
        }

        if ((!empty($newitem->timemodified) && $newitem->timemodified == $dbitem->timemodified) || !$dbitem->corplmssync) {
            // This record is not enabled for corplms syncing OR
        ...
        if (!$this->hierarchy->update_hierarchy_item($dbitem->id, $newitem, false, true, false)) {
            throw new corplms_sync_exception($elname, 'syncitem', 'cannotupdatex',
                "{$elname} {$newitem->idnumber}");
        }
        ...
        $this->addlog(get_string('updatedx', 'tool_corplms_sync', "{$elname} {$newitem->idnumber}"), 'info', 'syncitem');
    ...
    function check_sanity($synctable, $synctable_clone) {
        ...
        if ($rs->valid()) {
            foreach ($rs as $r) {
                $this->addlog(get_string('frameworkxnotexist', 'tool_corplms_sync', $r->frameworkidnumber), 'error', 'checksanity');
            }
            $rs->close();
            return false;
        }
        ...
        if ($rs->valid()) {
            foreach ($rs as $r) {
                $this->addlog(get_string('duplicateidnumberx', 'tool_corplms_sync', $r->idnumber), 'warn', 'checksanity');
            }
            $rs->close();
        }
       ...
       LEFT OUTER JOIN {{$elname}} i
        ...
        if ($rs->valid()) {
            foreach ($rs as $r) {
                $this->addlog(get_string('parentxnotexistinfile', 'tool_corplms_sync', $r->parentidnumber), 'error', 'checksanity');
            }
            $rs->close();
            return false;
        }
        ...
        if ($rs->valid()) {
            foreach ($rs as $r) {
                $this->addlog(get_string('typexnotexist', 'tool_corplms_sync', $r->typeidnumber), 'error', 'checksanity');
            }
            $rs->close();
            return false;
        }
        ...
        if (!empty($nodes)) {
            $this->addlog(get_string('circularreferror', 'tool_corplms_sync',
            ...
            foreach ($rs as $r) {
                ...
                if ($customfielddata = json_decode($r->customfields, true)) {
                    ...
                    if (empty($r->typeidnumber)) {
                        ...
                        $this->addlog(get_string('customfieldsnotype', 'tool_corplms_sync', "({$elname}:{$r->idnumber})"), 'error', 'checksanity');
                        return false;
                    }
                    if (!$typeid = $DB->get_field($elname.'_type', 'id', array('idnumber' => $r->typeidnumber))) {
                        $this->addlog(get_string('typexnotfound', 'tool_corplms_sync', $r->typeidnumber), 'error', 'checksanity');
                        return false;
                    }
                    foreach ($customfields as $c) {
                        ...
                        if (!$DB->record_exists($elname.'_type_info_field', array('typeid' => $typeid, 'shortname' => $shortname))) {
                            $this->addlog(get_string('customfieldnotexist', 'tool_corplms_sync',
