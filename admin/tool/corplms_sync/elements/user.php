...
require_once($CFG->dirroot.'/admin/tool/corplms_sync/elements/classes/element.class.php');
require_once($CFG->dirroot.'/corplms/customfield/fieldlib.php');
require_once($CFG->dirroot.'/corplms/hierarchy/prefix/position/lib.php');

class corplms_sync_element_user extends corplms_sync_element {
    ...
    function config_form(&$mform) {
        $mform->addElement('selectyesno', 'sourceallrecords', get_string('sourceallrecords', 'tool_corplms_sync'));
        $mform->addElement('static', 'sourceallrecordsdesc', '', get_string('sourceallrecordsdesc', 'tool_corplms_sync'));

        // User email settings.
        $mform->addElement('selectyesno', 'allowduplicatedemails', get_string('allowduplicatedemails', 'tool_corplms_sync'));
        $mform->addElement('text', 'defaultsyncemail', get_string('defaultemailaddress', 'tool_corplms_sync'), array('size' => 50));
        $mform->addElement('static', 'emailsettingsdesc', '', get_string('emailsettingsdesc', 'tool_corplms_sync'));
        ...
        $mform->addElement('selectyesno', 'ignoreexistingpass', get_string('ignoreexistingpass', 'tool_corplms_sync'));
        $mform->addElement('static', 'ignoreexistingpassdesc', '', get_string('ignoreexistingpassdesc', 'tool_corplms_sync'));
        $mform->addElement('selectyesno', 'forcepwchange', get_string('forcepwchange', 'tool_corplms_sync'));
        $mform->addElement('static', 'forcepwchangedesc', '', get_string('forcepwchange', 'tool_corplms_sync'));

        $mform->addElement('header', 'crudheading', get_string('allowedactions', 'tool_corplms_sync'));
        $mform->addElement('checkbox', 'allow_create', get_string('create', 'tool_corplms_sync'));
        $mform->setDefault('allow_create', 1);
        $mform->addElement('checkbox', 'allow_update', get_string('update', 'tool_corplms_sync'));
        $mform->setDefault('allow_update', 1);
        $mform->addElement('checkbox', 'allow_delete', get_string('delete', 'tool_corplms_sync'));
    ...
    function config_save($data) {
        ...
        if (!empty($data->source_user)) {
            ...
            $url = new moodle_url('/admin/tool/corplms_sync/admin/sourcesettings.php', array('element' => $this->get_name(), 'source' => $source->get_name()));
            if ($source->has_config()) {
                ...
                if ($import_deleted_new != $import_deleted_old) {
                    ...
                    corplms_set_notification(get_string('checkuserconfig', 'tool_corplms_sync', $url->out()), null, array('class'=>'notifynotice'));
                }
            }
        }
    }
    ...
    function sync() {
        ...
        $this->addlog(get_string('syncstarted', 'tool_corplms_sync'), 'info', 'usersync');
        ...
        try {
            ...
            corplms_sync_log($this->get_name(), $msg, 'error', 'unknown');
            return false;
        }
            ...
            if (empty($this->config->sourceallrecords)) {
                ...
                $sql = "SELECT DISTINCT u.id, u.idnumber, u.auth
                   ...
                   INNER JOIN {user} u ON (s.idnumber = u.idnumber AND u.idnumber != '')
                        WHERE u.corplmssync=1 AND u.deleted = 0 AND s.deleted = 1";
            } else {
               ...
               LEFT OUTER JOIN {{$synctable}} s ON (u.idnumber = s.idnumber AND u.idnumber != '')
                         WHERE u.corplmssync=1 AND s.idnumber IS NULL AND u.deleted=0";
            }
                ...
                foreach ($rs as $user) {
                    ...
                    try {
                        ...
                        if (array_search($user->idnumber, $invalidids) === false) {
                            ...
                            $this->addlog(get_string('deleteduserx', 'tool_corplms_sync', $user->idnumber), 'info', 'deleteuser');
                        }
                    } catch (Exception $e) {
                        throw new corplms_sync_exception('user', 'deleteuser', 'cannotdeleteuserx', $user->idnumber, $e->getMessage());
                    }
                }
                $rs->close();
            }
            ...
            if ($rscreateaccounts->valid()) {
                ...
                foreach ($rscreateaccounts as $suser) {
                    try {
                        ...
                        $this->addlog(get_string('createduserx', 'tool_corplms_sync', $suser->idnumber), 'info', 'createuser');
                    } catch (Exception $e) {
                        $this->addlog(get_string('cannotcreateuserx', 'tool_corplms_sync', $suser->idnumber), 'error', 'createuser');
                    }
                }
            ...
            foreach ($rsupdateaccounts as $suser) {
                ...
                if (!empty($this->config->allow_create) && !empty($user->deleted)) {
                    ...
                    if (undelete_user($user)) {
                        ...
                        $this->addlog(get_string('reviveduserx', 'tool_corplms_sync', $suser->idnumber), 'info', 'updateusers');
                    } else {
                        $this->addlog(get_string('cannotreviveuserx', 'tool_corplms_sync', $suser->idnumber), 'warn', 'updateusers');
                    }
                }
                ...
                try {
                    ...
                    throw new corplms_sync_exception('user', 'updateusers', 'cannotupdateuserx', $user->idnumber, $e->getMessage());
                }
                ...
                if (empty($this->config->ignoreexistingpass) && isset($suser->password) && trim($suser->password) !== '') {
                    ...
                    if ($userauth->can_change_password()) {
                        if (!$userauth->user_update_password($user, $suser->password)) {
                            $this->addlog(get_string('cannotsetuserpassword', 'tool_corplms_sync', $user->idnumber), 'warn', 'updateusers');
                        }
                    } else {
                        $this->addlog(get_string('cannotsetuserpasswordnoauthsupport', 'tool_corplms_sync', $user->idnumber), 'warn', 'updateusers');
                    }
                    unset($userauth);
                }
                ...
                $this->addlog(get_string('updateduserx', 'tool_corplms_sync', $suser->idnumber), 'info', 'updateusers');

                $transaction->allow_commit();

                if ($suspenduser) {
                    $event = \corplms_core\event\user_suspended::create(
                        array(
                            'objectid' => $user->id,
        ...
        foreach ($assign_sync_users as $suser) {
            try {
                ...
                throw new corplms_sync_exception('user', 'syncuserassignments', 'cannotcreateuserassignments', $suser->idnumber, $e->getMessage());
            }
        }
        ...
        $this->addlog(get_string('syncfinished', 'tool_corplms_sync'), 'info', 'usersync');
    ...
    function create_user($suser) {
        ...
        try {
            ...
            $transaction->rollback($e);
            throw new corplms_sync_exception('user', 'createusers', 'cannotcreateuserx', $user->idnumber);
        }
        ...
        if ($userauth->can_change_password()) {
            if (!isset($suser->password) || trim($suser->password) === '') {
                ...
                // Set user password.
                if (!$userauth->user_update_password($user, $suser->password)) {
                    $this->addlog(get_string('cannotsetuserpassword', 'tool_corplms_sync', $user->idnumber), 'warn', 'createusers');
    ...
    function get_duplicated_values($synctable, $synctable_clone, $field, $identifier) {
        ...
        foreach ($rs as $r) {
            $this->addlog(get_string($identifier, 'tool_corplms_sync', $r), 'error', 'checksanity');
            $invalidids[] = $r->id;
        }
        $rs->close();
     ...
     *                     language string in lang/en/tool_corplms_sync.php
     *
    ...
    function get_invalid_roles($synctable, $synctable_clone, $role) {
       ...
       LEFT OUTER JOIN {user} u
        ...
        $rs = $DB->get_recordset_sql($sql, $params);
        foreach ($rs as $r) {
            $this->addlog(get_string($role.'xnotexist', 'tool_corplms_sync', $r), 'error', 'checksanity');
            $invalidids[] = $r->id;
        }
        $rs->close();
    ...
    public function validate_custom_fields($synctable) {
            ...
            if (!empty($customfields)) {
                foreach ($customfields as $name => $value) {
                    ...
                    if ($this->customfieldsdb[$name]['required'] && trim($value) == '') {
                        $this->addlog(get_string('fieldrequired', 'tool_corplms_sync', (object)array('idnumber' => $r->idnumber, 'fieldname' => $name)), 'error', 'checksanity');
                        ...
                        if (!in_array(strtolower($value), $this->customfieldsdb[$name]['menu_options'])) {
                            $this->addlog(get_string('optionxnotexist', 'tool_corplms_sync', (object)array('idnumber' => $r->idnumber, 'option' => $value, 'fieldname' => $name)), 'error', 'checksanity');
                            $invalidids[] = intval($r->id);
                            break;
                        }
                        ...
                        if ($cfdata) {
                            $this->addlog(get_string('fieldduplicated', 'tool_corplms_sync', (object)array('idnumber' => $r->idnumber, 'fieldname' => $name, 'value' => $value)), 'error', 'checksanity');
                            $invalidids[] = intval($r->id);
                            break;
                        } else {
        ...
        foreach ($unique_fields as $fieldname => $fielddata) {
            ...
            foreach ($error_ids as $id) {
                ...
                $this->addlog(get_string('fieldmustbeunique', 'tool_corplms_sync', $log_data), 'error', 'checksanity');
            }
