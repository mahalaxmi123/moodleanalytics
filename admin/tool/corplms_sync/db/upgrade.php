...
/**
 * DB upgrades for Corplms Sync
 */

function xmldb_tool_corplms_sync_upgrade($oldversion) {
    ...
    if ($oldversion < 2012101100) {
        ...
        $sql = "UPDATE {config_plugins}
            ...
            WHERE plugin = 'corplms_sync_source_user_csv'
        ...
        upgrade_plugin_savepoint(true, 2012101100, 'tool', 'corplms_sync');
    }

    //manual modifying permissions in $DB to retain any existing permissions
    if ($oldversion < 2012121200) {
        $oldname = 'tool/corplms_sync:setfilesdirectory';
        $newname = 'tool/corplms_sync:setfileaccess';
        ...
        upgrade_plugin_savepoint(true, 2012121200, 'tool', 'corplms_sync');
    }

    if ($oldversion < 2013031400) {
        $table = new xmldb_table('corplms_sync_log');
        ...
        $params = array('shortname' => 'corplmssynclog', 'source' => 'corplms_sync_log', 'embedded' => 1);
        if ($report = $DB->get_record('report_builder', $params)) {
            ...
            $todb->value = 'runid';
            $todb->heading = get_string('runid', 'tool_corplms_sync');
        ...
        upgrade_plugin_savepoint(true, 2013031400, 'tool', 'corplms_sync');
    }

    if ($oldversion < 2013092000) {
        // Set the lastnotify flag so emails with entire log file don't get sent out on first run.
        set_config('lastnotify', time(), 'corplms_sync');

        upgrade_plugin_savepoint(true, 2013092000, 'tool', 'corplms_sync');
    }
    ...
    if ($oldversion < 2013101500) {
        ...
        $params = array('plugin' => 'corplms_sync_element_user', 'name' => 'removeuser');
        if ($oldsetting = $DB->get_record('config_plugins', $params)) {
            $DB->delete_records('config_plugins', $params);
        }

        foreach ($actions as $action) {
            $params = array('plugin' => 'corplms_sync_element_user', 'name' => $action);
            if (!$DB->record_exists('config_plugins', $params)) {
                $newsetting = new stdClass();
                $newsetting->plugin = 'corplms_sync_element_user';
        ...
        foreach (array('org', 'pos') as $element) {
            $params = array('plugin' => "corplms_sync_element_{$element}", 'name' => 'removeitems');
            if ($oldsetting = $DB->get_record('config_plugins', $params)) {
                $DB->delete_records('config_plugins', $params);
            }

            foreach ($actions as $action) {
                $params = array('plugin' => "corplms_sync_element_{$element}", 'name' => $action);
                if (!$DB->record_exists('config_plugins', $params)) {
                    $newsetting = new stdClass();
                    $newsetting->plugin = "corplms_sync_element_{$element}";
                    $newsetting->name   = $action;
                    $newsetting->value  = $action == 'allow_delete' ? !empty($oldsetting->value) : 1; // Keep the previously set value.
                    $DB->insert_record('config_plugins', $newsetting);
                }
        ...
        upgrade_plugin_savepoint(true, 2013101500, 'tool', 'corplms_sync');
    }
