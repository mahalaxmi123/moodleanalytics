 ...
 *
 * Unit tests for admin/tool/corplms_sync
 */
...
global $CFG;
require_once($CFG->dirroot . '/admin/tool/corplms_sync/sources/source_org_csv.php');
require_once($CFG->dirroot . '/admin/tool/corplms_sync/admin/forms.php');
require_once($CFG->dirroot . '/admin/tool/corplms_sync/elements/user.php');
require_once($CFG->dirroot . '/admin/tool/corplms_sync/sources/source_user_csv.php');

class corplms_sync_elements_test extends PHPUnit_Framework_TestCase {
    ...
    public function test_elements_path() {
        ...
        $source = new corplms_sync_source_org_csv();
    ...
    public function test_user_sync() {
        ...
        foreach (array(1, 0) as $sourceallrecords) {
            ...
            $source = new corplms_sync_source_user_csv();
            if (!$sourceallrecords) {
                                ...
                                foreach (array(1, 0) as $syncidnumber) {
                                    ...
                                    foreach ($syncdeleteoptions as $syncdeleted) {
                                        ...
                                        if ($userexists) {
                                            ...
                                            $userrecord->corplmssync = $usersync;
                                            if ($useridnumber) {
                                            ...
                                            $expectedresult->corplmssync = $userrecord->corplmssync;
                                            if ($useridnumber) {
                                        ...
                                        if ($syncexists) {
                                            ...
                                            if ($userexists) {
                                                ...
                                                if ($syncidnumber && !$syncdeleted) {
                                                    ...
                                                    $expectedresult->idnumber = $syncrecord->idnumber;
                                                    $expectedresult->deleted = $syncrecord->deleted;
                                                    $expectedresult->corplmssync = true;
                                                    $expectedcount++;
                                                } else {
            ...
            $element = $this->getMock('corplms_sync_element_user',
            ...
            foreach ($expectedresults as $expectedresult) {
                ...
                if (!empty($expectedresult->corplmssync)) {
                    $this->assertEquals($expectedresult->corplmssync, $finalresult->corplmssync,
                            "Unexpected result for corplmssync\n{$settings}");
                }
            }
