...
require_once($CFG->dirroot.'/admin/tool/corplms_sync/sources/classes/source.user.class.php');
require_once($CFG->dirroot.'/admin/tool/corplms_sync/lib.php');

class corplms_sync_source_user_csv extends corplms_sync_source_user {
    ...
    function config_form(&$mform) {
        ...
        if (empty($filepath) && get_config('corplms_sync', 'fileaccess') == FILE_ACCESS_DIRECTORY) {
            $mform->addElement('html', html_writer::tag('p', get_string('nofilesdir', 'tool_corplms_sync')));
            return false;
        }
        ...
        $info = get_string('csvimportfilestructinfo', 'tool_corplms_sync', implode($delimiter, $filestruct));
        $mform->addElement('html', html_writer::tag('div', html_writer::tag('p', $info, array('class' => "informationbox"))));

        // Add some source file details
        $mform->addElement('header', 'fileheader', get_string('filedetails', 'tool_corplms_sync'));
        $mform->setExpanded('fileheader');
        if (get_config('corplms_sync', 'fileaccess') == FILE_ACCESS_DIRECTORY) {
            $mform->addElement('static', 'nameandloc', get_string('nameandloc', 'tool_corplms_sync'),
                html_writer::tag('strong', $filepath));
        } else {
            $link = "{$CFG->wwwroot}/admin/tool/corplms_sync/admin/uploadsourcefiles.php";
            $mform->addElement('static', 'uploadfilelink', get_string('uploadfilelink', 'tool_corplms_sync', $link));
        }

        $encodings = textlib::get_encodings();
        $mform->addElement('select', 'csvuserencoding', get_string('csvencoding', 'tool_corplms_sync'), $encodings);
        ...
        $delimiteroptions = array(
            ',' => get_string('comma', 'tool_corplms_sync'),
            ';' => get_string('semicolon', 'tool_corplms_sync'),
            ':' => get_string('colon', 'tool_corplms_sync'),
            '\t' => get_string('tab', 'tool_corplms_sync'),
            '|' => get_string('pipe', 'tool_corplms_sync')
        );

        $mform->addElement('select', 'delimiter', get_string('delimiter', 'tool_corplms_sync'), $delimiteroptions);
    ...
    function import_data($temptable) {
        ...
        if ($fileaccess == FILE_ACCESS_DIRECTORY) {
            if (!$this->filesdir) {
                throw new corplms_sync_exception($this->get_element_name(), 'populatesynctablecsv', 'nofilesdir');
            }
            $filepath = $this->get_filepath();
            if (!file_exists($filepath)) {
                throw new corplms_sync_exception($this->get_element_name(), 'populatesynctablecsv', 'nofiletosync', $filepath, null, 'warn');
            }
            ...
            if (!$file = is_readable($filepath)) {
                throw new corplms_sync_exception($this->get_element_name(), 'populatesynctablecsv', 'cannotreadx', $filepath);
            }

            // Move file to store folder
            $storedir = $this->filesdir.'/csv/store';
            if (!corplms_sync_make_dirs($storedir)) {
                throw new corplms_sync_exception($this->get_element_name(), 'populatesynctablecsv', 'cannotcreatedirx', $storedir);
            }
            ...
            $fieldid = get_config('corplms_sync', 'sync_user_itemid');

            // Check the file exist
            if (!$fs->file_exists($systemcontext->id, 'corplms_sync', 'user', $fieldid, '/', '')) {
                throw new corplms_sync_exception($this->get_element_name(), 'populatesynctablecsv', 'nofileuploaded', $this->get_element_name(), null, 'warn');
            }

            // Get the file
            $fsfiles = $fs->get_area_files($systemcontext->id, 'corplms_sync', 'user', $fieldid, 'id DESC', false);
            $fsfile = reset($fsfiles);

            // Set up the temp dir
            $tempdir = $CFG->tempdir . '/corplmssync/csv';
            ...
            $fs->delete_area_files($systemcontext->id, 'corplms_sync', 'user', $itemid);
        ...
        if (!$file = fopen($storefilepath, 'r')) {
            throw new corplms_sync_exception($this->get_element_name(), 'populatesynctablecsv', 'cannotopenx', $storefilepath);
        }
        ...
        foreach ($fields as $field) {
            ...
            if (strlen($invalidchars)) {
                ...
                throw new corplms_sync_exception($this->get_element_name(), 'mapfields', 'csvnotvalidinvalidchars', $errorvar);
            }
        }
        ...
        foreach ($fieldmappings as $m => $f) {
            if (!in_array($m, $fields)) {
                if ($f == $m) {
                    throw new corplms_sync_exception($this->get_element_name(), 'mapfields', 'csvnotvalidmissingfieldx', $f);
                } else {
                    throw new corplms_sync_exception($this->get_element_name(), 'mapfields', 'csvnotvalidmissingfieldxmappingx', (object)array('field' => $f, 'mapping' => $m));
                }
            }
        }
        ...
        foreach ($this->fields as $f) {
            ...
            if (!in_array($f, $fields)) {
                throw new corplms_sync_exception($this->get_element_name(), 'importdata', 'csvnotvalidmissingfieldx', $f);
            }
        }
        ...
        $dbpersist = CORPLMS_SYNC_DBROWS;  // # of rows to insert into db at a time
        $rowcount = 0;
        $fieldcount = new object();
        $fieldcount->headercount = count($fields);
        $fieldcount->rownum = 0;
        $csvdateformat = (isset($CFG->csvdateformat)) ? $CFG->csvdateformat : get_string('csvdateformatdefault', 'corplms_core');
        $encoding = textlib::strtoupper($this->get_config('csvuserencoding'));
        $badtimezones = false;
        $goodtimezones = corplms_get_clean_timezone_list();
        ...
        while ($csvrow = fgetcsv($file, 0, $this->config->delimiter)) {
            ...
            if (is_array($csvrow) && current($csvrow) === null) {
                ...
                $this->addlog(get_string('fieldcountmismatch', 'tool_corplms_sync', $fieldcount), 'error', 'populatesynctablecsv');
            ...
            $csvrow = corplms_sync_clean_fields($csvrow, $encoding);
            ...
            if (empty($csvrow['timemodified'])) {
                ...
                $parsed_date = corplms_date_parse_from_format($csvdateformat, trim($csvrow['timemodified']));
                if ($parsed_date) {
            ...
            foreach ($posdates as $posdate) {
                if (isset($dbrow[$posdate])) {
                    if (empty($dbrow[$posdate])) {
                        ...
                        // Try to parse the contents - if parse fails assume a unix timestamp and leave unchanged
                        $parsed_date = corplms_date_parse_from_format($csvdateformat, trim($csvrow[$posdate]));
                        if ($parsed_date) {
                ...
                foreach (array_keys($this->customfields) as $cf) {
                    if (!empty($this->config->{'import_'.$cf})) {
                        ...
                        switch ($datatype) {
                            case 'datetime':
                                // Try to parse the contents - if parse fails assume a unix timestamp and leave unchanged.
                                $parsed_date = corplms_date_parse_from_format($csvdateformat, $value);
                                if ($parsed_date) {
                                    $value = $parsed_date;
                                } else {
            ...
            if ($rowcount >= $dbpersist) {
                ...
                // Bulk insert
                try {
                    corplms_sync_bulk_insert($temptable, $datarows);
                } catch (dml_exception $e) {
                    throw new corplms_sync_exception($this->get_element_name(), 'populatesynctablecsv', 'couldnotimportallrecords', $e->getMessage());
                }
        ...
        fclose($file);
        if ($badtimezones) {
            $OUTPUT->notification(get_string('badusertimezonemessage', 'tool_corplms_timezonefix'), 'notifynotice');
        }
