...
require_once($CFG->dirroot.'/admin/tool/corplms_sync/sources/databaselib.php');

class corplms_sync_source_user_database extends corplms_sync_source_user {
    ...
    function config_form(&$mform) {
        ...
        if (!$db_table) {
            $description = get_string('dbconnectiondetails', 'tool_corplms_sync');
        } else {
            $db_table = $this->config->{'database_dbtable'};
            $dbstruct = implode(', ', $dbstruct);
            $description = get_string('tablemustincludexdb', 'tool_corplms_sync', $db_table);
        ...
        // Database details
        $mform->addElement('select', 'database_dbtype', get_string('dbtype', 'tool_corplms_sync'), $db_options);
        $mform->addElement('text', 'database_dbname', get_string('dbname', 'tool_corplms_sync'));
        $mform->addRule('database_dbname', get_string('err_required', 'form'), 'required');
        $mform->setType('database_dbname', PARAM_ALPHANUMEXT);
        $mform->addElement('text', 'database_dbhost', get_string('dbhost', 'tool_corplms_sync'));
        $mform->setType('database_dbhost', PARAM_ALPHANUMEXT);
        $mform->addElement('text', 'database_dbuser', get_string('dbuser', 'tool_corplms_sync'));
        $mform->addRule('database_dbuser', get_string('err_required', 'form'), 'required');
        $mform->setType('database_dbuser', PARAM_ALPHANUMEXT);
        $mform->addElement('password', 'database_dbpass', get_string('dbpass', 'tool_corplms_sync'));
        $mform->setType('database_dbpass', PARAM_TEXT);

        // Table name
        $mform->addElement('text', 'database_dbtable', get_string('dbtable', 'tool_corplms_sync'));
        $mform->addRule('database_dbtable', get_string('err_required', 'form'), 'required');
        $mform->setType('database_dbtable', PARAM_ALPHANUMEXT);

        $mform->addElement('button', 'database_dbtest', get_string('dbtestconnection', 'tool_corplms_sync'));

        // Javascript include
        local_js(array(CORPLMS_JS_DIALOG));

        $PAGE->requires->strings_for_js(array('dbtestconnectsuccess', 'dbtestconnectfail'), 'tool_corplms_sync');

        $jsmodule = array(
                'name' => 'corplms_syncdatabaseconnect',
                'fullpath' => '/admin/tool/corplms_sync/sources/sync_database.js',
                'requires' => array('json', 'corplms_core'));

        $PAGE->requires->js_init_call('M.corplms_syncdatabaseconnect.init', null, false, $jsmodule);
    ...
    function config_save($data) {
        ...
        try {
            ...
            corplms_set_notification(get_string('cannotconnectdbsettings', 'tool_corplms_sync'), qualified_me());
        }
    ...
    function import_data($temptable) {
        ...
        try {
            ...
            $this->addlog(get_string('databaseconnectfail', 'tool_corplms_sync'), 'error', 'importdata');
        }
        ...
        // Finally, perform externaldb to corplms db field mapping
        foreach ($fields as $i => $f) {
        ...
        foreach ($fields as $f) {
            try {
                ...
                $this->addlog(get_string('dbmissingcolumnx', 'tool_corplms_sync', $f), 'error', 'importdata');
                return false;
            }
        }
        ...
        foreach ($data as $row) {
            ...
            if (empty($extdbrow['timemodified'])) {
                ...
                $parsed_date = corplms_date_parse_from_format($csvdateformat, trim($extdbrow['timemodified']));
                if ($parsed_date) {
            ...
            foreach ($datefields as $datefield) {
                if (isset($extdbrow[$datefield])) {
                    if (empty($extdbrow[$datefield])) {
                        ...
                        // Try to parse the contents - if parse fails assume a unix timestamp and leave unchanged.
                        $parsed_date = corplms_date_parse_from_format($csvdateformat, trim($extdbrow[$datefield]));
                        if ($parsed_date) {
            ...
            if ($rowcount >= CORPLMS_SYNC_DBROWS) {
                // bulk insert
                if (!corplms_sync_bulk_insert($temptable, $datarows)) {
                    $this->addlog(get_string('couldnotimportallrecords', 'tool_corplms_sync'), 'error', 'populatesynctabledb');
                    return false;
                }
