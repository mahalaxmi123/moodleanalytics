...
require_once($CFG->dirroot.'/admin/tool/corplms_sync/sources/classes/source.pos.class.php');
require_once($CFG->dirroot.'/admin/tool/corplms_sync/lib.php');

class corplms_sync_source_pos_csv extends corplms_sync_source_pos {
    ...
    function config_form(&$mform) {
        ...
        if (get_config('corplms_sync', 'fileaccess') == FILE_ACCESS_DIRECTORY) {
            $mform->addElement('static', 'nameandloc', get_string('nameandloc', 'tool_corplms_sync'), html_writer::tag('strong', $filepath));
        } else {
        ...
        $mform->addElement('select', 'csvposencoding', get_string('csvencoding', 'tool_corplms_sync'), $encodings);
    ...
    function import_data($temptable) {
        ...
        if ($fileaccess == FILE_ACCESS_DIRECTORY) {
            ...
            $fieldid = get_config('corplms_sync', 'sync_pos_itemid');

            // Check the file exists
            if (!$fs->file_exists($systemcontext->id, 'corplms_sync', 'pos', $fieldid, '/', '')) {
            ...
            // Get the file
            $fsfiles = $fs->get_area_files($systemcontext->id, 'corplms_sync', 'pos', $fieldid, 'id DESC', false);
            ...
            $fs->delete_area_files($systemcontext->id, 'corplms_sync', 'pos', $itemid);
        ...
        foreach ($fieldmappings as $field => $map) {
            if (!in_array($field, $fields)) {
                ...
                if ($field == $map) {
                    throw new corplms_sync_exception($this->get_element_name(), 'mapfields', 'csvnotvalidmissingfieldx', $field);
                } else {
                    throw new corplms_sync_exception($this->get_element_name(), 'mapfields', 'csvnotvalidmissingfieldxmappingx', (object)array('field' => $field, 'mapping' => $map));
                }
            }
        }
        ...
        while ($row = fgetcsv($file, 0, $this->config->delimiter)) {
            ...
            $row = corplms_sync_clean_fields($row, $encoding);
            ...
            if (empty($row['timemodified'])) {
                ...
                $parsed_date = corplms_date_parse_from_format($csvdateformat, trim($row['timemodified']));
                if ($parsed_date) {
