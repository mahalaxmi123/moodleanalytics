...
require_once($CFG->dirroot.'/admin/tool/corplms_sync/sources/classes/source.class.php');
require_once($CFG->dirroot.'/admin/tool/corplms_sync/elements/user.php');

abstract class corplms_sync_source_user extends corplms_sync_source {
     ...
     * @throws corplms_sync_exception if error
     */
    ...
    function __construct() {
        ...
        $this->temptablename = 'corplms_sync_user';
        parent::__construct();
        ...
        $this->element = new corplms_sync_element_user();
    }
    ...
    function config_form(&$mform) {
        // Fields to import
        $mform->addElement('header', 'importheader', get_string('importfields', 'tool_corplms_sync'));
        ...
        foreach ($this->fields as $f) {
            ...
            if (in_array($f, array('idnumber', 'username', 'timemodified'))) {
                ...
                $mform->addElement('checkbox', $name, get_string($f, 'tool_corplms_sync'));
                if (in_array($f, array('country'))) {
                    $mform->addHelpButton($name, $f, 'tool_corplms_sync');
                }
            }
        }
        ...
        // Field mappings
        $mform->addElement('header', 'mappingshdr', get_string('fieldmappings', 'tool_corplms_sync'));
    ...
    function get_sync_table() {
        try {
            ...
            throw new corplms_sync_exception($this->get_element_name(), 'importdata',
