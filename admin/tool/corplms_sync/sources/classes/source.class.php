...
abstract class corplms_sync_source {
    protected $config;
     ...
     *
     * @return sync table name (without prefix), e.g corplms_sync_org
    ...
    function __construct() {
        ...
        $this->filesdir = rtrim(get_config('corplms_sync', 'filesdir'), '/');

        // Ensure child class specified temptablename
        if (!isset($this->temptablename)) {
            throw corplms_sync_exception($this->get_element_name, 'setup', 'error',
    ...
    function addlog($info, $type='info', $action='') {
        ...
        corplms_sync_log($this->get_element_name(), $info, $type, $action);
    }
        ...
        foreach ($datarows as $i => $datarow) {
            foreach ($datarow as $name => $value) {
                if ((($columnsinfo[$name]->type == 'varchar' ||
                    ...
                    $this->addlog(get_string('lengthlimitexceeded', 'tool_corplms_sync', (object)array('idnumber' => $datarow['idnumber'], 'field' => $field,
