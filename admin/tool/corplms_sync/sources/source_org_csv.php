...
require_once($CFG->dirroot.'/admin/tool/corplms_sync/sources/classes/source.org.class.php');
require_once($CFG->dirroot.'/admin/tool/corplms_sync/lib.php');

class corplms_sync_source_org_csv extends corplms_sync_source_org {
    ...
    function config_form(&$mform) {
        ...
        $mform->addElement('select', 'csvorgencoding', get_string('csvencoding', 'tool_corplms_sync'), $encodings);
    ...
    function import_data($temptable) {
        ...
        if ($fileaccess == FILE_ACCESS_DIRECTORY) {
            ...
            $fieldid = get_config('corplms_sync', 'sync_org_itemid');

            // Check the file exists
            if (!$fs->file_exists($systemcontext->id, 'corplms_sync', 'org', $fieldid, '/', '')) {
            ...
            // Get the file
            $fsfiles = $fs->get_area_files($systemcontext->id, 'corplms_sync', 'org', $fieldid, 'id DESC', false);
            ...
            $fs->delete_area_files($systemcontext->id, 'corplms_sync', 'org', $itemid);
