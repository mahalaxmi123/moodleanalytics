...
define('CORPLMS_SYNC_DBROWS', 10000);
define('FILE_ACCESS_DIRECTORY', 0);
define('FILE_ACCESS_UPLOAD', 1);
define('CORPLMS_SYNC_LOGTYPE_MAX_NOTIFICATIONS', 50);
...
function latest_runid() {
    ...
    $runid = $DB->get_field_sql('SELECT MAX(runid) FROM {corplms_sync_log}');
...
/**
 * Run the cron for syncing Corplms elements with external sources
 *
 ...
 * @return void
 */
function tool_corplms_sync_cron($forcerun=false) {
    global $CFG, $OUTPUT;
    ...
    if ($fileaccess == FILE_ACCESS_DIRECTORY && !$filesdir = get_config('corplms_sync', 'filesdir')) {
        $configured = false;
        echo $OUTPUT->notification(get_string('nofilesdir', 'tool_corplms_sync'), 'notifyproblem');
    }
    // Check enabled sync element objects
    $elements = corplms_sync_get_elements(true);
    if (empty($elements)) {
        $configured = false;
        echo $OUTPUT->notification(get_string('noenabledelements', 'tool_corplms_sync'), 'notifyproblem');
    } else {
        foreach ($elements as $element) {
            ...
            if (!$sourceclass = get_config('corplms_sync', 'source_' . $elname)) {
                $configured = false;
                echo $OUTPUT->notification($elname . " " . get_string('sourcenotfound', 'tool_corplms_sync'));
            }
            ...
            if ($sourceclass) {
                ...
                if(empty($props)) {
                    $configured = false;
                    echo $OUTPUT->notification($elname . " " . get_string('nosourceconfig', 'tool_corplms_sync'), 'notifyproblem');
                }
            }
        }
    }
    ...
    if (!$configured) {
        echo $OUTPUT->notification(get_string('syncnotconfigured', 'tool_corplms_sync'), 'notifyproblem');
        return false;
    }
    ...
    foreach ($elements as $element) {
        try {
            ...
            corplms_sync_log($e->tsync_element, $msg, $e->tsync_logtype, $e->tsync_action);
            $element->get_source()->drop_table();
            continue;
        } catch (dml_exception $e) {
            corplms_sync_log($element->get_name(), $e->error, 'error', 'unknown');
            $element->get_source()->drop_table();
            continue;
        } catch (Exception $e) {
            corplms_sync_log($element->get_name(), $e->getMessage(), 'error', 'unknown');
            $element->get_source()->drop_table();
            continue;
        }
 ...
 */
function corplms_sync_log($element, $info, $type='info', $action='', $showmessage=true) {
    global $DB, $OUTPUT;
 ...
 */
function corplms_sync_get_element_files() {
    global $CFG;

    // Get all available sync element files
    $edir = $CFG->dirroot.'/admin/tool/corplms_sync/elements/';
    $pattern = '/(.*?)\.php$/';
 ...
 */
function corplms_sync_get_elements($onlyenabled=false) {
    global $CFG;

    $efiles = corplms_sync_get_element_files();

    $elements = array();
    foreach ($efiles as $filepath) {
        $element = basename($filepath, '.php');
        if ($onlyenabled) {
            if (!get_config('corplms_sync', 'element_'.$element.'_enabled')) {
                continue;
            }
        }
        ...
        $elementclass = 'corplms_sync_element_'.$element;
 ...
 */
function corplms_sync_get_element($element) {
...
function corplms_sync_make_dirs($dirpath) {
    global $CFG;
 ...
 * @return array $fields
 */
function corplms_sync_clean_fields($fields, $encoding) {
    if ($encoding !== 'UTF-8') {
 ...
 * @return boolean
 */
function corplms_sync_bulk_insert($table, $datarows) {
    global $CFG, $DB;
...
function corplms_sync_notify() {
    ...
    $logitems = $DB->get_records_select('corplms_sync_log', "logtype {$sqlin} AND time > ?", $params,
                                        'time DESC', '*', 0, CORPLMS_SYNC_LOGTYPE_MAX_NOTIFICATIONS);
    if (!$logitems) {
        // Nothing to report.
        return true;
    }
    ...
    $notifytypes_str = array_map(create_function('$type', "return get_string(\$type.'plural', 'tool_corplms_sync');"), $notifytypes);
    $subject = get_string('notifysubject', 'tool_corplms_sync', $sitename);

    $a = new stdClass();
    $a->logtypes = implode(', ', $notifytypes_str);
    $a->count = $logcount;
    $a->since = date_format_string($lastnotify, $dateformat);
    $message = get_string('notifymessagestart', 'tool_corplms_sync', $a);
    $message .= "\n\n";
    foreach ($logitems as $logentry) {
        $logentry->time = date_format_string($logentry->time, $dateformat);
        $logentry->logtype = get_string($logentry->logtype, 'tool_corplms_sync');
        $message .= get_string('notifymessage', 'tool_corplms_sync', $logentry) . "\n\n";
    }
    $message .= "\n" . get_string('viewsyncloghere', 'tool_corplms_sync',
            $CFG->wwwroot . '/admin/tool/corplms_sync/admin/synclog.php');

    // Send emails.
    mtrace("\n{$logcount} relevant corplms sync log messages since " .
            date_format_string($lastnotify, $dateformat)) . ". Sending notifications...";
    $supportuser = core_user::get_support_user();
    foreach ($notifyemails as $emailaddress) {
        $userto = \corplms_core\corplms_user::get_external_user(trim($emailaddress));
...
class corplms_sync_exception extends moodle_exception {
     ...
     * @param string $logtype optional corplms sync log type
     */
    public function __construct($element, $action, $errorcode, $a = null, $debuginfo = null, $logtype = 'error') {
        ...
        parent::__construct($errorcode, 'tool_corplms_sync', $link='', $a, $debuginfo);
    }
}
