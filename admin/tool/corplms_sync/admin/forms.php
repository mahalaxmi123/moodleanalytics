 ...
 */
class corplms_sync_element_settings_form extends moodleform {
    function definition() {
        global $CFG;
        $mform =& $this->_form;
        ...
        if ($sources = $element->get_sources()) {
            ...
            foreach ($sources as $s) {
                $sourceoptions[$s->get_name()] = get_string('displayname:'.$s->get_name(), 'tool_corplms_sync');
            }
            $mform->addElement('select', 'source_'.$elementname,
                get_string('source', 'tool_corplms_sync'), $sourceoptions);
            $mform->setDefault('source_'.$elementname, get_config('corplms_sync', 'source_'.$elementname));
        } else {
            $mform->addElement('static', 'nosources', '('.get_string('nosources', 'tool_corplms_sync').')');
        ...
        try {
            ...
            if ($source->has_config()) {
                $mform->addElement('static', 'configuresource', '', html_writer::link(new moodle_url('/admin/tool/corplms_sync/admin/sourcesettings.php', array('element' => $element->get_name(), 'source' => $source->get_name())), get_string('configuresource', 'tool_corplms_sync')));
            }
        } catch (corplms_sync_exception $e) {
 ...
 */
class corplms_sync_source_settings_form extends moodleform {
 ...
 */
class corplms_sync_config_form extends moodleform {
    function definition() {
        global $CFG;
        ...
        // File access.
        if (has_capability('tool/corplms_sync:setfileaccess', context_system::instance())) {
            $mform->addElement('header', 'fileheading', get_string('files', 'tool_corplms_sync'));
            $dir = get_string('fileaccess_directory', 'tool_corplms_sync');
            $upl = get_string('fileaccess_upload', 'tool_corplms_sync');
            $mform->addElement('select', 'fileaccess', get_string('fileaccess', 'tool_corplms_sync'),
                array(FILE_ACCESS_DIRECTORY => $dir, FILE_ACCESS_UPLOAD => $upl));
            $mform->setType('fileaccess', PARAM_INT);
            $mform->setDefault('fileaccess', $dir);
            $mform->addHelpButton('fileaccess', 'fileaccess', 'tool_corplms_sync');
            $mform->addElement('text', 'filesdir', get_string('filesdir', 'tool_corplms_sync'), array('size' => 50));
            $mform->setType('filesdir', PARAM_TEXT);
            $mform->disabledIf('filesdir', 'fileaccess', 'eq', FILE_ACCESS_UPLOAD);
        }

        // Notifications.
        $mform->addElement('header', 'notificationheading', get_string('notifications', 'tool_corplms_sync'));
        $mform->addElement('checkbox', 'notifytypes[error]', get_string('notifytypes', 'tool_corplms_sync'),
                get_string('errorplural', 'tool_corplms_sync'));
        $mform->addElement('checkbox', 'notifytypes[warn]', '', get_string('warnplural', 'tool_corplms_sync'));

        $mform->addElement('text', 'notifymailto', get_string('notifymailto', 'tool_corplms_sync'));
        $mform->setType('notifymailto', PARAM_TEXT);
        $mform->setDefault('notifymailto', $CFG->supportemail);
        $mform->addHelpButton('notifymailto', 'notifymailto', 'tool_corplms_sync');
        $mform->setExpanded('notificationheading');

        // Schedule.
        $mform->addElement('header', 'scheduleheading', get_string('schedule', 'tool_corplms_sync'));
        $mform->addElement('advcheckbox', 'cronenable', get_string('enablescheduledsync', 'tool_corplms_sync'));
        $mform->setDefault('cronenable', 1);
        $mform->addElement('scheduler', 'schedulegroup', get_string('schedule', 'tool_corplms_sync'));
    ...
    function validation($data, $files) {
        ...
        if ($data['fileaccess'] == FILE_ACCESS_DIRECTORY && isset($data['filesdir'])) {
            ...
            if (!preg_match($pattern, $filesdir)) {
                $errors['filesdir'] = get_string('pathformerror', 'tool_corplms_sync');
            } else if (!is_dir($filesdir)) {
                $errors['filesdir'] = get_string('notadirerror', 'tool_corplms_sync', $filesdir);
            } else if (!is_writable($filesdir)) {
                $errors['filesdir'] = get_string('readonlyerror', 'tool_corplms_sync', $filesdir);
            }
        }
        ...
        if (!empty($data['notifymailto'])) {
            ...
            foreach ($emailaddresses as $mailaddress) {
                if (!validate_email($mailaddress)) {
                    $errors['notifymailto'] = get_string('invalidemailaddress', 'tool_corplms_sync', format_string($mailaddress));
                    break;
                }
            }
        }
 ...
 */
class corplms_sync_source_files_form extends moodleform {
    function definition() {
        ...
        if (!count($elements)) {
            $mform->addElement('html', html_writer::tag('p',
                get_string('noenabledelements', 'tool_corplms_sync')));
            return;
        }

        foreach ($elements as $e) {
            $name = $e->get_name();
            if (!has_capability("tool/corplms_sync:upload{$name}", context_system::instance())) {
                continue;
            }
            $mform->addElement('header', "header_{$name}",
                get_string("displayname:{$name}", 'tool_corplms_sync'));
            $mform->setExpanded("header_{$name}");

            try {
                $source = $e->get_source();
            } catch (corplms_sync_exception $e) {
                $link = "{$CFG->wwwroot}/admin/tool/corplms_sync/admin/elementsettings.php?element={$name}";
                $mform->addElement('html', html_writer::tag('p',
                    get_string('nosourceconfigured', 'tool_corplms_sync', $link)));
                continue;
            }

            if (!$source->uses_files()) {
                $mform->addElement('html', html_writer::tag('p',
                    get_string('sourcedoesnotusefiles', 'tool_corplms_sync')));
                continue;
            }


            $mform->addElement('filepicker', $name,
            get_string('displayname:'.$source->get_name(), 'tool_corplms_sync'), 'size="40"');

            if (get_config('corplms_sync', 'fileaccess') == FILE_ACCESS_UPLOAD) {
                $usercontext = context_user::instance($USER->id);
                $systemcontext = context_system::instance();
                $fs = get_file_storage();

                //check for existing draft area to prevent massive duplication
                $existing_files = $fs->get_area_files($systemcontext->id, 'corplms_sync', $name);
                if (sizeof($existing_files) > 0) {
                    ...
                    if (sizeof($existing_draft) < 1) {
                        ...
                        file_prepare_draft_area($draftid, $systemcontext->id, 'corplms_sync', $name, null, $FILEPICKER_OPTIONS);
                        ...
                        foreach ($existing_files as $file) {
                            ...
                            $mform->addElement('static', '', '',
                                get_string('note:syncfilepending', 'tool_corplms_sync'));
                        }
                    }
