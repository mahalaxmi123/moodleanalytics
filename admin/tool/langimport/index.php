...
if ($mode == UPDATE_ALL_LANG) {
    ...
    if (!$availablelangs = $installer->get_remote_list_of_languages()) {
       print_error('cannotdownloadcorplmslanguageupdatelist', 'corplms_core');
    }
...
if ($availablelangs = $installer->get_remote_list_of_languages()) {
    ...
    $a = new stdClass();
    $a->corplmsversion = corplms_major_version();
    $a->langdir = $CFG->dataroot.'/lang/';
    print_string('remotecorplmslangnotavailable', 'corplms_core', $a);
    echo $OUTPUT->box_end();
}
