...
$goodzones = corplms_get_clean_timezone_list();
$errors = array();
$notifications = array();
    ...
    foreach ($data as $key => $value) {
        if(substr($key, 0, 8) == 'badzone_') {
            ...
            if ($DB->execute($sql, array($value, $badzone))) {
                $notifications[] = get_string('updatetimezonesuccess', 'tool_corplms_timezonefix', $a);
            } else {
                $errors[] = get_string('error:updatetimezone', 'tool_corplms_timezonefix', $a);
            }
        }
    }
}
$strheader = get_string('pluginname', 'tool_corplms_timezonefix');
echo $OUTPUT->header();
...
echo $OUTPUT->notification(get_string('infomessage', 'tool_corplms_timezonefix'), 'notifymessage');
...
//first find really strange stuff that we don't understand at all (may have come from e.g. corplms_sync)
$unknownusercount = 0;
$unknownzones = array();
...
if ($unknownusercount > 0) {
    ...
    echo $OUTPUT->notification(get_string('error:unknownzones', 'tool_corplms_timezonefix', $a), 'notifyproblem');
}
...
if ($totalbad > 0) {
    echo $OUTPUT->notification(get_string('numbadusertimezones', 'tool_corplms_timezonefix', $totalbad), 'notifynotice');
    list($insql, $inparams) = $DB->get_in_or_equal(array_keys($badzones));
    $sql = "SELECT DISTINCT timezone from {user} WHERE timezone $insql";
    ...
    $table->head = array(
        get_string('badzone', 'tool_corplms_timezonefix'),
        get_string('numusers', 'tool_corplms_timezonefix'),
        get_string('replacewith', 'tool_corplms_timezonefix'),
    );
    ...
    $output .= $OUTPUT->single_submit(get_string('updatetimezones', 'tool_corplms_timezonefix'));
    $output .= html_writer::end_tag('form');
    echo $output;
} else {
    echo $OUTPUT->notification(get_string('nobadusertimezones', 'tool_corplms_timezonefix'), 'notifysuccess');
}
echo $OUTPUT->footer();
