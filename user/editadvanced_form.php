...
class user_editadvanced_form extends moodleform {
    ...
    public function definition() {
        ...
        if (get_config('corplms_sync', 'element_user_enabled')) {
    ...
    public function validation($usernew, $files) {
        ...
        if(!empty($usernew->idnumber) && corplms_idnumber_exists('user', $usernew->idnumber, $usernew->id)) {
            $err['idnumber'] = get_string('idnumberexists', 'corplms_core');
        }

        // If idnumber is empty, ensure sync is not turned on.
        if (empty($usernew->idnumber) && !empty($usernew->corplmssync)) {
            $err['corplmssync'] = get_string('error:emptyidnumberwithsync', 'corplms_core');
        }
