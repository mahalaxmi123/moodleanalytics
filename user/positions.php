...
if (!$positionsenabled = get_config('corplms_hierarchy', 'positionsenabled')) {
    print_error('error:noposenabled', 'corplms_hierarchy');
}
...
if (!in_array($poscode, $enabled_positions)) {
    print_error('error:postypenotenabled', 'corplms_hierarchy');
}
...
if (!$course = $DB->get_record('course', array('id' => $courseid))) {
    print_error('error:courseidincorrect', 'corplms_core');
}

if (!$user = $DB->get_record('user', array('id' => $user))) {
    print_error('error:useridincorrect', 'corplms_core');
}
...
if ($type == $POSITION_TYPES[POSITION_TYPE_PRIMARY] && !empty($CFG->enabletempmanagers)) {
    if (has_capability('corplms/core:delegateusersmanager', $personalcontext)) {
        $can_edit_tempmanager = true;
    } else if ($USER->id == $user->id && has_capability('corplms/core:delegateownmanager', $personalcontext)) {
        $can_edit_tempmanager = true;
    }
}
...
$positions          = get_string('positions', 'corplms_hierarchy');
$positiontype       = get_string('type'.$type, 'corplms_hierarchy');
...
$PAGE->requires->strings_for_js(array('chooseposition', 'chooseappraiser', 'choosemanager',
    'chooseorganisation', 'currentlyselected'), 'corplms_hierarchy');
$PAGE->requires->strings_for_js(array('choosetempmanager'), 'corplms_core');
$PAGE->requires->strings_for_js(array('error:positionnotselected', 'error:organisationnotselected', 'error:managernotselected',
                                       'error:tempmanagernotselected', 'error:appraisernotselected'), 'corplms_core');
$jsmodule = array(
        'name' => 'corplms_positionuser',
        'fullpath' => '/corplms/core/js/position.user.js',
        'requires' => array('json'));
$selected_position = json_encode(dialog_display_currently_selected(get_string('selected', 'corplms_hierarchy'), 'position'));
$selected_organisation = json_encode(dialog_display_currently_selected(get_string('selected', 'corplms_hierarchy'), 'organisation'));
$selected_manager = json_encode(dialog_display_currently_selected(get_string('selected', 'corplms_hierarchy'), 'manager'));
$selected_tempmanager = json_encode(dialog_display_currently_selected(get_string('selected', 'corplms_hierarchy'), 'tempmanager'));
$js_can_edit = $can_edit ? 'true' : 'false';
$js_can_edit_tempmanager = $can_edit_tempmanager ? 'true' : 'false';
$selected_appraiser = json_encode(dialog_display_currently_selected(get_string('selected', 'corplms_hierarchy'), 'appraiser'));
...
$PAGE->requires->js_init_call('M.corplms_positionuser.init', $args, false, $jsmodule);
...
$position_assignment = file_prepare_standard_editor($position_assignment, 'description', $editoroptions, $editoroptions['context'],
    'corplms_core', 'pos_assignment', $position_assignment->id);
...
if (!$can_edit && !$position_assignment->id && !$can_edit_tempmanager) {
    ...
    echo $OUTPUT->header();
    echo $OUTPUT->heading(get_string('nopositionsassigned', 'corplms_hierarchy'));
}
else {
    ...
    elseif ($data = $form->get_data()) {
        ...
        if (isset($data->managerid) && $data->managerid > 0) {
            if ($data->managerid == $data->userid) {
                print_error('error:userownmanager', 'corplms_hierarchy');
            } else {
                ...
                if ($validmanager && $validmanager->deleted != 0) {
                    ...
                    corplms_set_notification(get_string('error:managerdeleted','corplms_hierarchy', $a), null, array('class' => 'notifynotice'));
                }
            }
        }
        ...
        if (isset($data->appraiserid) && $data->appraiserid > 0) {
            if ($data->appraiserid == $data->userid) {
                print_error('error:userownappraiser', 'corplms_hierarchy');
            } else {
                ...
                if ($validappraiser && $validappraiser->deleted != 0) {
                    ...
                    corplms_set_notification(get_string('error:appraiserdeleted','corplms_hierarchy', $a), null, array('class' => 'notifynotice'));
                }
            }
        }
        ...
        if ($can_edit && $data->type != POSITION_TYPE_ASPIRATIONAL) {
            $data = file_postupdate_standard_editor($data, 'description', $editoroptions, $editoroptions['context'], 'corplms_core', 'pos_assignment', $data->id);
        ...
        if (!empty($data->tempmanagerid)) {
            ...
            if ($validtempmgr && $validtempmgr->deleted != 0) {
                ...
                corplms_set_notification(get_string('error:tempmanagerdeleted','corplms_hierarchy', $a), null, array('class' => 'notifynotice'));
            } else {
                corplms_update_temporary_manager($user->id, $data->tempmanagerid, $data->tempmanagerexpiry);
            }
        } else if (!empty($CFG->enabletempmanagers)) {
            // Unassign the current temporary manager.
            corplms_unassign_temporary_manager($user->id);
        }

        // Display success message
        corplms_set_notification(get_string('positionsaved','corplms_hierarchy'), $currenturl, array('class' => 'notifysuccess'));
    }
