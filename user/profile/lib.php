...
function profile_display_hierarchy_fields($userid) {
    ...
    if (isset($record->pos)) {
        echo html_writer::tag('dt', get_string('position', 'corplms_hierarchy'));
        echo html_writer::tag('dd', $record->pos);
    }

    if (isset($record->org)) {
        echo html_writer::tag('dt', get_string('organisation', 'corplms_hierarchy'));
    ...
    if (isset($record->manid)) {
        ...
        echo html_writer::tag('dt', get_string('manager', 'corplms_hierarchy'));
...
function profile_signup_position($mform, $nojs) {
    ...
    if ($nojs) {
        ...
        if (is_array($allpositions) && !empty($allpositions)) {
            $mform->addElement('select', 'positionid', get_string('chooseposition', 'corplms_hierarchy'),
                array(0 => get_string('chooseposition', 'corplms_hierarchy')) + $allpositions);
        } else {
            $mform->addElement('static', 'positionid', get_string('chooseposition', 'corplms_hierarchy'), get_string('noposition', 'corplms_hierarchy') );
        }
        ...
        $mform->addElement('static', 'positionselector', get_string('position', 'corplms_hierarchy'),
            ...
            html_writer::empty_tag('input', array('type' => 'button', 'value' => get_string('chooseposition', 'corplms_hierarchy'), 'id' => 'show-position-dialog'))
        );
...
function profile_signup_organisation($mform, $nojs) {
    ...
    if ($nojs) {
        ...
        $mform->addElement('static', 'organisationselector', get_string('organisation', 'corplms_hierarchy'),
            ...
            html_writer::empty_tag('input', array('type' => 'button', 'value' => get_string('chooseorganisation', 'corplms_hierarchy'), 'id' => 'show-organisation-dialog'))
        );
