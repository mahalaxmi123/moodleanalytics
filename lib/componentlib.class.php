...
define('DOWNLOAD_BASE', 'http://download.corplmslms.com/lang');
/**
...
class lang_installer {
    ...
    public function __construct($langcode = '') {
        ...
        $corplmsversion = corplms_major_version(); //may return false if something is badly wrong
        if ($corplmsversion) {
            $this->version = 'T' . $corplmsversion;
        } else {
            debugging('Corplms version number could not be determined from version.php!');
        }
    ...
    protected function install_language_pack($langcode) {
        ...
        if ($status == COMPONENT_ERROR) {
            ...
            echo $OUTPUT->notification(get_string('unexpected_installer_result', 'corplms_core', $status), 'notifyproblem');
            return self::RESULT_DOWNLOADERROR;
        }
    }
}
