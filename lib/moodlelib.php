...
function require_login($courseorid = null, $autologinguest = true, $cm = null, $setwantsurltome = true, $preventredirect = false) {
    ...
    if ($course->id == SITEID) {
        ...
        if (is_role_switched($course->id)) {
            ...
            if (!corplms_course_is_viewable($course->id)) {
...
function undelete_user($user) {
    ...
    if ($DB->update_record('user', $updateuser)) {
        $event = \corplms_core\event\user_undeleted::create(
            array(
                'objectid' => $user->id,
    ...
    foreach ($auths as $auth) {
        ...
        if ($user->id) {
            ...
            if (empty($user->firstaccess)) { //prevent firstaccess from remaining 0 for manual account that never required confirmation
                ...
                $event = \corplms_core\event\user_firstlogin::create(
                    array(
                        'objectid' => $user->id,
                        'context' => context_user::instance($user->id),
                        'other' => array(
...
function remove_course_contents($courseid, $showfeedback = true, array $options = null) {
    ...
    $cleanuplugintypes = array('report', 'coursereport', 'format', 'corplms');
...
function get_list_of_timezones($extrazone=null) {
    ...
    $goodzones = corplms_get_clean_timezone_list(true);
