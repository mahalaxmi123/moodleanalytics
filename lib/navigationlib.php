...
class global_navigation extends navigation_node {
    ...
    public function add_course(stdClass $course, $forcegeneric = false, $coursetype = self::COURSE_OTHER) {
        ...
        $dimmed = ($issite) ? '' : corplms_get_style_visibility($course);
...
class settings_navigation extends navigation_node {
    ...
    public function initialise() {
        ...
        switch ($context->contextlevel) {
            ...
            case CONTEXT_PROGRAM:
                corplms_load_program_settings($this, $this->page->context, true);
                break;
            case CONTEXT_COURSE:
    ...
    protected function load_course_settings($forceopen = false) {
        ...
        if (has_capability('moodle/course:update', $coursecontext)) {
            ...
            $coursenode->add(get_string('competencies', 'corplms_hierarchy'), $url, self::TYPE_SETTING, null, null, new pix_icon('i/settings', ''));

            // Add the course reminders link.
            $url = new moodle_url('/course/reminders.php', array('courseid' => $course->id));
            $coursenode->add(get_string('remindersmenuitem', 'corplms_coursecatalog'), $url, self::TYPE_SETTING, null, null, new pix_icon('i/email', ''));
        }
