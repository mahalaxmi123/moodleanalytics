...
function upgrade_started($preinstall=false) {
    global $CFG, $DB, $PAGE, $OUTPUT, $CORPLMS;
    ...
    if ($preinstall) {
        ...
        if (!CLI_SCRIPT and !$PAGE->headerprinted) {
            ...
            $PAGE->set_title($strupgrade.' - Corplms '.$CORPLMS->release);
...
function upgrade_core($version, $verbose) {
    global $CFG, $SITE, $DB, $COURSE, $CORPLMS;
...
function upgrade_noncore($verbose) {
    ...
    print_upgrade_part_start('Corplms', false, $verbose);
    // Upgrade all language packs if we can.
   // corplms_upgrade_installed_languages();
    // Ensure timezones are updated.
    corplms_import_timezonelist();
    print_upgrade_part_end('Corplms', false, $verbose);
    ...
    try {
        ...
        // Upgrade corplms navigation menu.
