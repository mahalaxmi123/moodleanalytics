 ...
 */
class corplms_table extends flexible_table {
    ...
    */
    ...
    /**
     * In Corplms tables, we print the table anyway, just with a message
     ...
     * Setup the table
     *
     * Re-use parent class, but also add 'corplmstable' class
     */
    function setup() {
        parent::setup();
        // Always introduce the "corplmstable" class for the table if not specified
        if (empty($this->attributes)) {
            $this->attributes['class'] = 'corplmstable';
        } else if (!isset($this->attributes['class'])) {
            $this->attributes['class'] = 'corplmstable';
        } else if (!in_array('corplmstable', explode(' ', $this->attributes['class']))) {
            $this->attributes['class'] = trim('corplmstable ' . $this->attributes['class']);
        }
    }
}
