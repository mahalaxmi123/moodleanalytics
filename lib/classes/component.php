...
class core_component {
    ...
    protected static $supportsubplugins = array('mod', 'editor', 'corplms', 'tool', 'local');
    ...
    /** @var string Corplms build */
    protected static $corplmsbuild = null;
    ...
    protected static function init() {
        ...
        if (!CACHE_DISABLE_ALL and !self::is_developer()) {
            ...
            if (is_readable($cachefile)) {
                ...
                } else if (!isset($cache['corplmsbuild']) or $cache['corplmsbuild'] !== self::fetch_corplmsbuild()) {
                    error_log('Resetting core_component cache after corplms upgrade to build ' . self::fetch_corplmsbuild());
    ...
    public static function get_cache_content() {
        ...
        $cache = array(
            ...
            'corplmsbuild' => self::$corplmsbuild,
        );
...
';
    ...
    /**
     * Get the Corplms build.
     *
     * In order for this to work properly, opcache should be reset beforehand.
     *
     * @return string build number
     */
    protected static function fetch_corplmsbuild() {
        global $CFG;
        if (self::$corplmsbuild === null) {

            $CORPLMS = new stdClass(); // Prevent IDE complaints.
            require($CFG->dirroot . '/version.php');
            self::$corplmsbuild = $CORPLMS->build;
        }
        return self::$corplmsbuild;
    }
    ...
    protected static function fetch_plugintypes() {
        ...
        $types = array(
            ...
            'plagiarism'    => $CFG->dirroot.'/plagiarism',
            'corplms'        => $CFG->dirroot.'/corplms', // must be before admin tools so corplms_sync can add to corplms tables
    ...
    public static function get_all_versions_hash() {
        ...
        $versions['corplmsbuild'] = self::fetch_corplmsbuild();
