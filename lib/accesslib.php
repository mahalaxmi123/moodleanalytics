...
function get_component_string($component, $contextlevel) {
    ...
    if ($component === 'moodle' or $component === 'core') {
        switch ($contextlevel) {
            ...
            case CONTEXT_PROGRAM:   return get_string('program', 'corplms_program');
...
function role_get_name(stdClass $role, $context = null, $rolenamedisplay = ROLENAME_ALIAS) {
    ...
    if (trim($role->name) !== '') {
        ...
        switch ($role->shortname) {
            case 'manager':         $original = get_string('sitemanager', 'corplms_core'); break;
...
class context_program extends context {
     ...
     */
    public static function get_level_name() {
        return get_string('program', 'corplms_program');
    }
    ...
    public function get_context_name($withprefix = true, $short = false) {
        ...
        if ($program = $DB->get_record('prog', array('id' => $this->_instanceid))) {
            if ($withprefix){
                $name = get_string('program', 'corplms_program').': ';
            }
            if ($short){
     ...
     */
    public function get_url() {
        return new moodle_url('/corplms/program/view.php', array('id'=>$this->_instanceid));
    }
    ...
    protected static function create_level_instances() {
        ...
        // to corplms and the table hasn't yet been created
