...
function admin_output_new_settings_by_page($node) {
    global $OUTPUT, $CFG;
    $corplmscoreinstall = isset($CFG->corplmscoreinstallation) ? $CFG->corplmscoreinstallation : 0;
    $return = array();
    ...
    if ($node instanceof admin_category) {
        ...
        foreach ($entries as $entry) {
            ...
            $newsettings = array();
            foreach ($node->settings as $setting) {
                // Always show enablecompletion after a Corplms install.
                if($corplmscoreinstall && $setting->name == 'enablecompletion') {
                    $newsettings[] = $setting;
                }
