...
function cohort_add_cohort($cohort, $addcollections=true) {
    ...
    corplms_cohort_increment_automatic_id($cohort->idnumber);
...
function cohort_delete_cohort($cohort) {
    ...
    //delete associations
    $associations = corplms_cohort_get_associations($cohort->id);
    foreach ($associations as $ass) {
        corplms_cohort_delete_association($cohort->id, $ass->id, $ass->type);
    }
...
function cohort_print_tabs($currenttab, $cohortid, $cohorttype, $cohort) {
    ...
    if ($cohort && corplms_cohort_is_active($cohort)) {
        print html_writer::tag('div', '', array('class' => 'plan_box', 'style' => 'display:none;'));
    } else {
        if ($cohort->startdate && $cohort->startdate > time()) {
            $message = get_string('cohortmsgnotyetstarted', 'corplms_cohort', userdate($cohort->startdate, get_string('strfdateshortmonth', 'langconfig')));
        }
        if ($cohort->enddate && $cohort->enddate < time()) {
            $message = get_string('cohortmsgalreadyended', 'corplms_cohort', userdate($cohort->enddate, get_string('strfdateshortmonth', 'langconfig')));
        }
    ...
    $canmanagerules = has_capability('corplms/cohort:managerules', $systemcontext);
    $canmanagevisibility = has_capability('corplms/coursecatalog:manageaudiencevisibility', $systemcontext);
    ...
    if ($canassign && $cohorttype == cohort::TYPE_STATIC) {
        $toprow[] = new tabobject('editmembers', new moodle_url('/cohort/assign.php', array('id' => $cohortid)),
            get_string('editmembers','corplms_cohort'));
    }

    if ($canview && $canmanage) {
        $toprow[] = new tabobject('enrolledlearning', new moodle_url('/corplms/cohort/enrolledlearning.php', array('id' => $cohortid)),
            get_string('enrolledlearning', 'corplms_cohort'));
    }

    if (!empty($CFG->audiencevisibility) && $canmanagevisibility) {
        $toprow[] = new tabobject('visiblelearning', new moodle_url('/corplms/cohort/visiblelearning.php', array('id' => $cohortid)),
            get_string('visiblelearning', 'corplms_cohort'));
    }

    if (corplms_feature_visible('learningplans') && $canmanage) {
        $toprow[] = new tabobject('plans', new moodle_url('/corplms/cohort/learningplan.php', array('id' => $cohortid)),
            get_string('learningplan', 'corplms_cohort'));
    }

    if (corplms_feature_visible('goals') && $canmanage) {
        $toprow[] = new tabobject('goals', new moodle_url('/corplms/cohort/goals.php', array('id' => $cohortid)),
            get_string('goals', 'corplms_hierarchy'));
    }

    if ($canassignroles) {
        $toprow[] = new tabobject('roles', new moodle_url('/corplms/cohort/assignroles.php', array('id' => $cohortid)),
            get_string('assignroles', 'corplms_cohort'));
    }
