...
class cohort_edit_form extends moodleform {
    ...
    public function definition() {
        ...
        $placeholder = get_string('datepickerlongyearplaceholder', 'corplms_core');
        $hint = get_string('dateformatlongyearhint', 'corplms_cohort', $placeholder);
        ...
        $mform->setDefault('idnumber', corplms_cohort_next_automatic_id());

        if (!$cohort->id) {
            $mform->addElement('select', 'cohorttype', get_string('type', 'corplms_cohort'), cohort::getCohortTypes());
            $mform->addHelpButton('cohorttype', 'type', 'corplms_cohort');
        }

        $mform->addElement('editor', 'description_editor', get_string('description', 'cohort'), null, $editoroptions);
        $mform->setType('description_editor', PARAM_RAW);

        $mform->addElement('date_selector', 'startdate', get_string('startdate', 'corplms_cohort'), array('optional' => true));
        $mform->addHelpButton('startdate', 'startdate', 'corplms_cohort');

        $mform->addElement('date_selector', 'enddate', get_string('enddate', 'corplms_cohort'), array('optional' => true));
        $mform->addHelpButton('enddate', 'enddate', 'corplms_cohort');
        ...
        if (!empty($alertoptions)) {
            $mform->addElement(
                'select',
                'alertmembers',
                get_string('alertmembers', 'corplms_cohort'),
                $alertoptions
            );
            $mform->addHelpButton('alertmembers', 'alertmembers', 'corplms_cohort');
        } else {
    ...
    public function validation($data, $files) {
        ...
        if ($data['startdate'] > $data['enddate'] && $data['enddate']) {
            $errstr = get_string('error:startafterfinish','corplms_cohort');
