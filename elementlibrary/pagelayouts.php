...
$layouts = array(
    ...
    'noblocks' => array(
        'name' => 'No Blocks',
        'description' => 'This is a custom Corplms layout used on pages where the full page width is needed. No other block regions are displayed.',
    ),
    'popup' => array(
        'name' => 'Popup',
...
if ($layout) {
    ...
    echo html_writer::alist(array('A file name for the layout template (stored in <code>theme/[themename]/layout/[filename]</code>). If no file exists in the theme, will look for layout files in each parent theme in turn.',
        ...
        '<strong>nocustommenu</strong>: don\'t include the custommenu. In corplms the main Corplms navigation menu replaces the custommenu so this option disables that too.',
    ...
    echo $OUTPUT->container('Each of the files in the theme layout/ folder should contain the logic to correctly handles the options above. See the layouts section of standardcorplms/config.php and standardcorplms/layouts/ for full details.');
