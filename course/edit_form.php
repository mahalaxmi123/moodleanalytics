...
class course_edit_form extends moodleform {
     ...
     */
    function definition() {
        global $USER, $CFG, $DB, $PAGE, $CORPLMS_COURSE_TYPES, $COHORT_VISIBILITY;
        ...
        $mform->addElement('select', 'coursetype', get_string('coursetype', 'corplms_core'), $coursetypeoptions);
        ...
        corplms_add_icon_picker($mform, $action, 'course', $course->icon, $nojs);
        // END Course Icons
...
//--------------------------------------------------------------------------------
        ...
        if (has_capability('moodle/cohort:manage', $systemcontext)) {
            $mform->addElement('header','enrolledcohortshdr', get_string('enrolledcohorts', 'corplms_cohort'));

            if (empty($course->id)) {
                $cohorts = '';
            } else {
                $cohorts = corplms_cohort_get_course_cohorts($course->id, null, 'c.id');
            ...
            $cohortsclass = new corplms_cohort_course_cohorts(COHORT_ASSN_VALUE_ENROLLED);
            $cohortsclass->build_table(!empty($course->id) ? $course->id : 0);
            $mform->addElement('html', $cohortsclass->display(true));

            $mform->addElement('button', 'cohortsaddenrolled', get_string('cohortsaddenrolled', 'corplms_cohort'));
        ...
        if (!empty($CFG->audiencevisibility) && has_capability('corplms/coursecatalog:manageaudiencevisibility', $systemcontext)) {
            ...
            if (empty($course->id)) {
                ...
                $cohorts = corplms_cohort_get_visible_learning($course->id);
                $cohorts = !empty($cohorts) ? implode(',', array_keys($cohorts)) : '';
            }
