...
if ($courseid) { // editing course
    if($courseid == SITEID){
        ...
        print_error('noeditsite', 'corplms_coursecatalog');
    }
    ...
    require_login();
    print_error('error:courseidorcategory', 'corplms_coursecatalog');
}
...
if ($delete) {
    ...
    $PAGE->set_title(get_string('editcoursereminders', 'corplms_coursecatalog'));
    $PAGE->set_heading($course->fullname);
    echo $OUTPUT->header();
    echo $OUTPUT->heading(get_string('deletedreminder', 'corplms_coursecatalog', format_string($reminder->title)));
...
if ($reminderform->is_cancelled()){
    ...
    if (empty($reminder->id)) {
        if (!$reminder->insert()) {
            print_error('error:createreminder', 'corplms_coursecatalog');
        }
        ...
        if (!$reminder->update()) {
            print_error('error:updatereminder', 'corplms_coursecatalog');
        }
    ...
    foreach (array('invitation', 'reminder', 'escalation') as $mtype) {
        ...
        if ($mtype === 'escalation') {
            if (!empty($data->$nosend)) {
                ...
                if ($message->id) {
                    ...
                    if (!$message->update()) {
                        print_error('error:deletereminder', 'corplms_coursecatalog');
                    }
                }
        ...
        if (empty($message->id)) {
            if (!$message->insert()) {
                print_error('errro:createreminder', 'corplms_coursecatalog');
            }
        }
        else {
            if (!$message->update()) {
...
if ($reminder->id > 0) {
    ...
    $buttonhtml = $OUTPUT->single_button(
        ...
        get_string('deletereminder', 'corplms_coursecatalog', format_string($reminder->title)),
        'get'
    );
}


$streditcoursereminders = get_string('editcoursereminders', 'corplms_coursecatalog');
...
$tabs[] = new tabobject('new', $CFG->wwwroot.'/course/reminders.php?courseid='.$course->id.'&id=-1', get_string('new', 'corplms_coursecatalog'));
...
if (!$completion->is_enabled()) {
    echo $OUTPUT->box(get_string('noactivitieswithcompletionenabled', 'corplms_coursecatalog'), 'generalbox adminerror boxwidthwide boxaligncenter');
    echo $OUTPUT->continue_button(new moodle_url('/course/view.php', array('id' => $course->id)));

}
else if (!get_coursemodules_in_course('feedback', $course->id)) {
    echo $OUTPUT->notification(get_string('nofeedbackactivities', 'corplms_coursecatalog'), 'notifynotice');
    echo $OUTPUT->continue_button(new moodle_url('/course/view.php', array('id' => $course->id)));
}
else {
    if (count($tabs)) {
