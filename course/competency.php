...
require_capability('corplms/hierarchy:viewcompetency', $context);
...
$PAGE->requires->string_for_js('assigncoursecompletiontocompetencies', 'corplms_hierarchy');
$jargs = '{"id":'.$course->id;
...
$jsmodule = array(
        'name' => 'corplms_coursecompetency',
        'fullpath' => '/corplms/core/js/course.competency.js',
        'requires' => array('json'));
$PAGE->requires->js_init_call('M.corplms_coursecompetency.init', $args, false, $jsmodule);

$strcompetenciesusedincourse = get_string("competenciesusedincourse", 'corplms_hierarchy');
...
<div class="singlebutton centerbutton">
    <form action="<?php echo $CFG->wwwroot ?>/corplms/hierarchy/prefix/competency/course/add.php?id=<?php echo $id ?>" method="get">
        <div>
            <?php if (!empty($CFG->competencyuseresourcelevelevidence)) { ?>
                <input type="submit" id="show-coursecompetency-dialog" value="<?php echo get_string('addcourseevidencetocompetencies', 'corplms_hierarchy'); ?>" />
            <?php } else { ?>
                <input type="submit" id="show-coursecompetency-dialog" value="<?php echo get_string('assigncoursecompletiontocompetencies', 'corplms_hierarchy'); ?>" />
            <?php } ?>
...
echo $OUTPUT->single_button(
    ...
    $options),
    get_string('returntocourse', 'corplms_core'),
    'get'
);
