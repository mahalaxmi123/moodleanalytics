...
if (empty($course->id)) {
    ...
    $enrolledselected = corplms_cohort_get_course_cohorts($course->id, null, 'c.id');
    $enrolledselected = !empty($enrolledselected) ? implode(',', array_keys($enrolledselected)) : '';
}
$PAGE->requires->strings_for_js(array('coursecohortsenrolled'), 'corplms_cohort');
$jsmodule = array(
        'name' => 'corplms_cohortdialog',
        'fullpath' => '/corplms/cohort/dialog/coursecohort.js',
        'requires' => array('json'));
$args = array('args'=>'{"enrolledselected":"' . $enrolledselected . '",'.
        '"COHORT_ASSN_VALUE_ENROLLED":' . COHORT_ASSN_VALUE_ENROLLED . '}');
$PAGE->requires->js_init_call('M.corplms_coursecohort.init', $args, true, $jsmodule);
...
if (!empty($CFG->audiencevisibility)) {
    if(empty($course->id)) {
        ...
        $visibleselected = corplms_cohort_get_visible_learning($course->id);
        $visibleselected = !empty($visibleselected) ? implode(',', array_keys($visibleselected)) : '';
    }
    $PAGE->requires->strings_for_js(array('coursecohortsvisible'), 'corplms_cohort');
    $jsmodule = array(
...
if ($editform->is_cancelled()) {
    ...
    if (empty($course->id)) {
        ...
        if (false && !is_enrolled($context)) { // T-13383 Disable redirect to enrolments page in Corplms.
    ...
    if (has_capability('moodle/cohort:manage', context_system::instance())) {
        // Enrolled audiences.
        $currentcohorts = corplms_cohort_get_course_cohorts($course->id, null, 'c.id, e.id AS associd');
        ...
        if ($todelete = array_diff(array_keys($currentcohorts), $newcohorts)) {
            ...
            foreach ($todelete as $cohortid) {
                corplms_cohort_delete_association($cohortid, $currentcohorts[$cohortid]->associd, COHORT_ASSN_ITEMTYPE_COURSE);
            }
        }
        ...
        if ($newcohorts = array_diff($newcohorts, array_keys($currentcohorts))) {
            ...
            foreach ($newcohorts as $cohortid) {
                corplms_cohort_add_association($cohortid, $course->id, COHORT_ASSN_ITEMTYPE_COURSE);
            }
        }

        // Visible audiences.
        if (!empty($CFG->audiencevisibility) && has_capability('corplms/coursecatalog:manageaudiencevisibility', context_system::instance())) {
            $visiblecohorts = corplms_cohort_get_visible_learning($course->id);
            ...
            if ($newvisible = array_diff($newvisible, array_keys($visiblecohorts))) {
                ...
                foreach ($newvisible as $cohortid) {
                    corplms_cohort_add_association($cohortid, $course->id, COHORT_ASSN_ITEMTYPE_COURSE, COHORT_ASSN_VALUE_VISIBLE);
                }
            }
         }
