...
require_capability('block/corplms_quicklinks:manageownlinks', $context);
...
$baseurl = new moodle_url('/blocks/corplms_quicklinks/managelinks.php', $urlparams);
...
if ($blockaction == 'add' && confirm_sesskey()) {
    // Save the block link
    if (empty($linkurl)) {
        print_error('error:linkurlrequired', 'block_corplms_quicklinks', $baseurl);
    } else {
        $link = new stdClass;
        $link->userid = $USER->id;
...
$strmanage = get_string('managelinks', 'block_corplms_quicklinks');

$PAGE->set_pagelayout('standard');
$PAGE->set_title($strmanage);
$PAGE->set_heading(format_string($SITE->fullname));

$managefeeds = new moodle_url('/blocks/corplms_quicklinks/managelinks.php', $urlparams);
$PAGE->navbar->add(get_string('blocks'));
$PAGE->navbar->add(get_string('managelinks', 'block_corplms_quicklinks'), $managefeeds);
echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('managelinks', 'block_corplms_quicklinks'), 2, null, 'managelinks');

$table = new flexible_table('corplms-quicklinks-display-links');

$table->define_columns(array('title', 'url', 'actions'));
$table->define_headers(array(get_string('linktitle', 'block_corplms_quicklinks'), get_string('url', 'block_corplms_quicklinks'), get_string('actions', 'moodle')));
...
foreach ($links as $link) {
    ...
    $linkicons = $OUTPUT->action_icon($deleteurl, $deleteicon, new confirm_action(get_string('deletelinkconfirm', 'block_corplms_quicklinks'))) . ' ';
...
$mform = new corplms_quicklinks_add_quicklink_form(null, array('blockinstanceid' => $blockinstanceid));
$mform->display();
