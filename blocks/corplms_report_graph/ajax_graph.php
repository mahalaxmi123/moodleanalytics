...
$block = $DB->get_record('block_instances', array('id' => $blockid, 'blockname' => 'corplms_report_graph'), '*', MUST_EXIST);
...
$svgdata = \block_corplms_report_graph\util::get_svg_data($blockid, $config);

if ($type === 'svg') {
    \block_corplms_report_graph\util::send_svg($svgdata);

} else {
    \block_corplms_report_graph\util::send_pdf($svgdata);
}
