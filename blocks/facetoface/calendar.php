...
if (in_array('timestart', $defaultfields) || in_array('timefinish', $defaultfields)) {
    local_js(array(CORPLMS_JS_DATEPICKER));
}
...
function get_sessions($display, $groups, $users, $courses, $activefilters, &$events, &$sessionids) {
    ...
    if (!empty($events)) {
        ...
        foreach ($events as $eventid => $event) {
            ...
            if (empty($eventcourse) || !corplms_course_is_viewable($eventcourse)) {
...
function get_matches_defaultfields($events) {
    ...
    foreach ($activefilters['defaultfields'] as $fieldname => $value) {
        if (trim($value) && ($value != get_string('datepickerlongyearplaceholder', 'corplms_core'))) {
    ...
    if (count($eventids)) {
        ...
        if (count($fieldvalues)) {
            ...
            if (array_key_exists('timestart', $fieldvalues)) {
                ...
                $cleanedformatdate = str_replace('%', '', get_string('datepickerlongyearphpuserdate', 'corplms_core'));
                $fieldvalues['timestart'] = corplms_date_parse_from_format($cleanedformatdate, $fieldvalues['timestart']);
            ...
            if (array_key_exists('timefinish', $fieldvalues)) {
                ...
                $cleanedformatdate = str_replace('%', '', get_string('datepickerlongyearphpuserdate', 'corplms_core'));
                $fieldvalues['timefinish'] = corplms_date_parse_from_format($cleanedformatdate, $fieldvalues['timefinish']);
...
function defaultfield_chooser($fieldname) {
    ...
    if (in_array($fieldname, array('timestart', 'timefinish')) && !$value) {
        $value = get_string('datepickerlongyearplaceholder', 'corplms_core');
    }
    switch($fieldname) {
        case 'timestart':
            $label = html_writer::empty_tag('input', array('type' => 'text', 'size' => 10, 'name' => $fieldname, 'id' => $fieldname, 'value' => $value));
            build_datepicker_js("#$fieldname", get_string('datepickerlongyeardisplayformat', 'corplms_core'));
...
function get_sessions_by_course($sessionids, $displayinfo, $waitlistedsessions) {
    ...
    list($visibilitysql, $visibilityparams) = corplms_visibility_where(null, 'c.id', 'c.visible', 'c.audiencevisible');
...
function print_waitlisted_content($waitlistedsessions) {
    ...
    foreach ($waitlistedsessions as $session) {
        ...
        $visible = corplms_course_is_viewable($f2f->course);
