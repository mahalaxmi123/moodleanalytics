...
class block_corplms_stats extends block_base {

    function init() {
        $this->title = get_string('pluginname', 'block_corplms_stats');
    }
    ...
    function get_content() {
        ...
        if (corplms_get_staff()) {
            // now get sql required to return stats
            $stats = corplms_stats_manager_stats($USER, $this->config);
            if (!empty($stats)) {
                $renderer = $this->page->get_renderer('block_corplms_stats');
                $this->content->text .= $renderer->display_stats_list(corplms_stats_sql_helper($stats));
            }
        }
    ...
    function cron() {
        ...
        $dist = ($CFG->block_corplms_stats_sche_hour*3600) +      //Hours distance
                ($CFG->block_corplms_stats_sche_minute*60);       //Minutes distance
        ...
        if ($result > 0 && $result < time() && $result+(60*120) > time()) {
            ...
            $lastrun = (int)get_config('block_corplms_stats', 'cronlastrun');
            if (empty($lastrun)) {
            ...
            if (time() > ($lastrun + (24*60*60))) { //if at least 24 hours since last run
                require_once($CFG->dirroot.'/blocks/corplms_stats/locallib.php');
                $nextrun = time();
                $stats = corplms_stats_timespent($lastrun, $nextrun);
                foreach ($stats as $userid => $timespent) {
                    // insert daily stat for each user returned above into new stats table for reading.
                    corplms_stats_add_event($nextrun, $userid, STATS_EVENT_TIME_SPENT, '', $timespent);
                }
                set_config('cronlastrun', $nextrun, 'block_corplms_stats');
            }
        }
    }
}
