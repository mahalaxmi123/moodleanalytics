 ...
 * @package corplms
 * @subpackage block_corplms_certifications
 */
 ...
 */
class block_corplms_certifications extends block_base {

    public function init() {
        $this->title   = get_string('pluginname', 'block_corplms_certifications');
    }
    ...
    public function get_content() {
        ...
        foreach ($renewals as $renewal) {
            ...
            $url = new moodle_url('/corplms/program/required.php', array('id' => $renewal->pid));
            ...
            if ($renewal->certifpath == CERTIFPATH_CERT) {
                ...
                if ($prog_completion) {
                    $duedatestr = (empty($prog_completion->timedue) || $prog_completion->timedue == COMPLETION_TIME_NOT_SET)
                        ? get_string('duedatenotset', 'corplms_program')
                        : userdate($prog_completion->timedue, get_string('strftimedate', 'langconfig'));
                } else {
                    $duedatestr =  get_string('duedatenotset', 'corplms_program');
                }
        ...
        $url = new moodle_url('/corplms/program/required.php', array('userid' => $USER->id, 'filter' => 'certification'));
        if (count($certifications) > 0) {
            $this->content->footer = html_writer::link($url, get_string('allmycertifications', 'block_corplms_certifications'));
        }

        $renderer = $this->page->get_renderer('block_corplms_certifications');
