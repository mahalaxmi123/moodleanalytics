...
$PAGE->set_corplms_menu_selected('mybookings');
$PAGE->set_pagelayout('standard');
$PAGE->set_pagetype('my-bookings');

if (!$user = $DB->get_record('user', array('id' => $userid))) {
    print_error('error:usernotfound', 'corplms_core');
}

// Users can only view their own and their staff's pages.
if ($USER->id != $userid && !corplms_is_manager($userid) && !is_siteadmin()) {
    print_error('error:cannotviewthispage', 'corplms_core');
}

$output = $PAGE->get_renderer('corplms_reportbuilder');

if ($USER->id != $userid) {
    $strheading = get_string('pastbookingsfor', 'corplms_core').fullname($user, true);
    $menuitem = 'myteam';
    $url = new moodle_url('/my/teammembers.php');
} else {
    $strheading = get_string('mypastbookings', 'corplms_core');
    $menuitem = 'mylearning';
