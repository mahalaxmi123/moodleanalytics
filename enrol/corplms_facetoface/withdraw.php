...
$enrol = $DB->get_record('enrol', array('id' => $eid, 'enrol' => 'corplms_facetoface'), '*', MUST_EXIST);
$course = $DB->get_record('course', array('id' => $enrol->courseid), '*', MUST_EXIST);
$corplms_facetoface = new enrol_corplms_facetoface_plugin();
$baseurl = new moodle_url('/enrol/corplms_facetoface/withdraw.php', array('eid' => $eid));
...
if ($confirm) {
    ...
    $DB->delete_records('enrol_corplms_f2f_pending', array('enrolid' => $eid, 'userid' => $USER->id));
...
echo $OUTPUT->heading(get_string('withdrawconfifm', 'enrol_corplms_facetoface'), 3);
