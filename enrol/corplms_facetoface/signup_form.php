...
class enrol_corplms_facetoface_signup_form extends moodleform {
    protected $instance;
    protected $toomany = false;

    /**
     * Overriding this function to get unique form id for multiple corplms_facetoface enrolments.
     *
     ...
     */
    private function add_signup_elements($mform, $instance, $corplms_facetoface) {
        global $DB, $CFG, $OUTPUT;

        $courseid = $instance->courseid;

        $settingautosignup = enrol_corplms_facetoface_plugin::SETTING_AUTOSIGNUP;

        $sessions = $corplms_facetoface->get_enrolable_sessions($courseid);

        if (empty($sessions)) {
            $mform->addElement('static', 'managermissing', get_string('managermissingallsessions', 'enrol_corplms_facetoface'));
        ...
        if (!empty($instance->$settingautosignup)) {
            ...
            if ($corplms_facetoface->sessions_require_manager()) {
                $mform->addElement('static', 'managermissing', get_string('managermissingsomesessions', 'enrol_corplms_facetoface'));
            }

            $mform->addElement('static', 'signuptoenrol', get_string('signuptoenrol', 'enrol_corplms_facetoface'));
            ...
            foreach ($sessrows as $facetofaceid => $sessions) {
                ...
                if ($facetoface->forceselectposition && !get_position_assignments($facetoface->approvalreqd)) {
                    ...
                    $this->enrol_corplms_facetoface_addsessrows($mform, $sessions, $facetoface);
                    $sessionsavailable = true;
                }
     ...
     */
    private function enrol_corplms_facetoface_addsessrows($mform, $sessions, $facetoface) {
        ...
        $mform->addElement('html', html_writer::tag('th', get_string('selectsession', 'enrol_corplms_facetoface'))); // No title for radio button col.
        ...
        $mform->addElement('html', html_writer::tag('th', get_string('additionalinformation', 'enrol_corplms_facetoface')));
