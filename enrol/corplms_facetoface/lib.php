...
class enrol_corplms_facetoface_plugin extends enrol_plugin {
    ...
    public function get_info_icons(array $instances) {
        $icons = array();
        $icons[] = new pix_icon('withoutkey', get_string('pluginname', 'enrol_corplms_facetoface'), 'enrol_corplms_facetoface');
        return $icons;
    }
    ...
    public function add_course_navigation($instancesnode, stdClass $instance) {
        if ($instance->enrol !== 'corplms_facetoface') {
             throw new coding_exception('Invalid enrol instance type!');
        }

        $context = context_course::instance($instance->courseid);
        if (has_capability('enrol/corplms_facetoface:config', $context)) {
            $managelink = new moodle_url('/enrol/corplms_facetoface/edit.php', array('courseid' => $instance->courseid, 'id' => $instance->id));
    ...
    public function get_action_icons(stdClass $instance) {
        ...
        if (has_capability('enrol/corplms_facetoface:config', $context)) {
            $editlink = new moodle_url("/enrol/corplms_facetoface/edit.php", array('courseid' => $instance->courseid, 'id' => $instance->id));
    ...
    public function get_newinstance_link($courseid) {
        ...
        if (!has_capability('moodle/course:enrolconfig', $context) or !has_capability('enrol/corplms_facetoface:config', $context)) {
            return null;
        }
        // Multiple instances supported - different roles with different password.
        return new moodle_url('/enrol/corplms_facetoface/edit.php', array('courseid'=>$courseid));
    }

    /**
     * Enrol user on to course
     *
     * @param enrol_corplms_facetoface_plugin $instance enrolment instance
     ...
     */
    public function enrol_corplms_facetoface($instance, $fromform = null, $course, $returnurl) {
        global $DB, $USER;
        ...
        if ($selectpositiononsignupglobal) {
            $manager = corplms_get_most_primary_manager($USER->id);
        } else {
            $manager = corplms_get_manager($USER->id);
        }
        ...
        if (!empty($autosignup)) {
            $sessionstojoin = enrol_corplms_facetoface_get_sessions_to_autoenrol($this, $course, $facetofaces);
            ...
            if (!is_array($sessionstojoin)) {
                ...
                $message = get_string('autobookingcompleted', 'enrol_corplms_facetoface', $joinedsessions);
            ...
            if ($hasselfapproval && !$selfapprovaltc) {
                corplms_set_notification(get_string('selfapprovalrequired', 'enrol_corplms_facetoface'));
                redirect($returnurl);
            }
            ...
            if ($result['result'] === true) {
                ...
                if ((isset($result['conflict']) && $result['conflict']) || isset($result['result'])) {
                    corplms_set_notification($result['result'], $returnurl);
                } else {
            ...
            if (!empty($facetoface->approvalreqd) && !$hasselfapproval) {
                ...
                $DB->insert_record('enrol_corplms_f2f_pending', $toinsert);
                ...
                add_to_log(
                    ...
                    'course',
                    'enrol',
                    '../enrol/corplms_facetoface/signup.phpusers.php?id='.$instance->courseid,
                    $instance->courseid
                );
            }
        }
        ...
        corplms_set_notification($message, $returnurl, array('class' => $cssclass));
    ...
    public function enrol_page_hook(stdClass $instance) {
        ...
        require_once($CFG->dirroot . '/enrol/corplms_facetoface/signup_form.php');

        $enrolstatus = $this->can_self_enrol($instance);

        // Don't show enrolment instance form, if user can't enrol using it.
        if (true === $enrolstatus) {
            $form = new enrol_corplms_facetoface_signup_form(null, $instance);
            ...
            if ($instance->id == $instanceid) {
                if ($data = $form->get_data()) {
                    ...
                    $this->enrol_corplms_facetoface($instance, $data, $course, $returnurl);
                }
            }
        ...
        if ($enrolstatus == get_string('cannotenrolalreadyrequested', 'enrol_corplms_facetoface')) {
            $url = new moodle_url('/enrol/corplms_facetoface/withdraw.php', array('eid' => $instance->id));

            $output = html_writer::start_tag('p');
            $output .= $enrolstatus;
            $output .= html_writer::end_tag('p');
            $output .= html_writer::start_tag('p');
            $output .= html_writer::link($url, get_string('withdrawpending', 'enrol_corplms_facetoface'), array('class' => 'link-as-button'));
            $output .= html_writer::end_tag('p');
            return $output;
        }
    }
    ...
    public function course_expand_enrol_hook($form, $instance) {
        ...
        if ($data = $form->get_data()) {
            if ($this->enrol_corplms_facetoface($instance, $data, $course, null)) {
                return true;
            }
        }
        return false;
    ...
    public function course_expand_get_form_hook($instance) {
        ...
        // Don't show enrolment instance form, if user can't enrol using it.
        if ($enrolstatus === true) {
            return new enrol_corplms_facetoface_signup_form(null, $instance);
        }
        return $enrolstatus;
    }
    ...
    public function can_self_enrol(stdClass $instance, $checkuserenrolment = true) {
        ...
        if ($checkuserenrolment) {
            if (isguestuser()) {
                // Can not enrol guest.
                return get_string('cannotenrol', 'enrol_corplms_facetoface');
            }
        ...
        if ($instance->customint3 > 0) {
            ...
            if ($count >= $instance->customint3) {
                // Bad luck, no more corplms_facetoface enrolments here.
                return get_string('maxenrolledreached', 'enrol_corplms_facetoface');
            }
        }
        ...
        if ($instance->customint5) {
            ...
            if (!cohort_is_member($instance->customint5, $USER->id)) {
                ...
                return markdown_to_html(get_string('cohortnonmemberinfo', 'enrol_corplms_facetoface', $a));
            }
        }
        ...
        if (empty($sessions)) {
            ...
            return get_string('cannotenrolnosessions', 'enrol_corplms_facetoface');
        }

        // If I already have a pending request, cannot ask again.
        if ($DB->record_exists('enrol_corplms_f2f_pending', array('enrolid' => $instance->id, 'userid' => $USER->id))) {
            return get_string('cannotenrolalreadyrequested', 'enrol_corplms_facetoface');
        }
    ...
    protected function email_welcome_message($instance, $user) {
        ...
        if (trim($instance->customtext1) !== '') {
            ...
            $messagetext = $strmgr->get_string('welcometocoursetext', 'enrol_corplms_facetoface', $a, $user->lang);
            $messagehtml = text_to_html($messagetext, null, false, true);
        }

        $subject = $strmgr->get_string(
            'welcometocourse',
            'enrol_corplms_facetoface',
    ...
    /**
     * Enrol corplms_facetoface cron support.
     * @return void
     */
    ...
    public function sync(progress_trace $trace, $courseid = null) {
        ...
        $trace->output('Verifying corplms_facetoface-enrolments...');
        ...
        // Note: the logic of corplms_facetoface enrolment guarantees that user logged in at least once (=== u.lastaccess set)
        ...
        $sql = "SELECT e.*, ue.userid
                  ...
                  JOIN {enrol} e ON (e.id = ue.enrolid AND e.enrol = 'corplms_facetoface' AND e.customint2 > 0)
        ...
        $trace->output('...user corplms_facetoface-enrolment updates finished.');
        $trace->finished();
    ...
    /**
     * Returns the user who is responsible for corplms_facetoface enrolments in given instance.
     *
     * Usually it is the first editing teacher - the person with "highest authority"
     * as defined by sort_by_roleassignment_authority() having 'enrol/corplms_facetoface:manage'
     * capability.
     *
    ...
    protected function get_enroller($instanceid) {
        ...
        if ($users = get_enrolled_users($context, 'enrol/corplms_facetoface:manage')) {
    ...
    public function get_user_enrolment_actions(course_enrolment_manager $manager, $ue) {
        ...
        $params['ue'] = $ue->id;
        if ($this->allow_unenrol($instance) && has_capability("enrol/corplms_facetoface:unenrol", $context)) {
            $url = new moodle_url('/enrol/unenroluser.php', $params);
            $actions[] = new user_enrolment_action(
                new pix_icon('t/delete', get_string('enroldelete', 'enrol_corplms_facetoface')),
                get_string('unenrol', 'enrol'),
                $url,
                array('class' => 'unenrollink', 'rel' => $ue->id)
            );
        }
        if ($this->allow_manage($instance) && has_capability("enrol/corplms_facetoface:manage", $context)) {
            $url = new moodle_url('/enrol/editenrolment.php', $params);
            $actions[] = new user_enrolment_action(
                new pix_icon('t/edit', get_string('enroledit', 'enrol_corplms_facetoface')),
                get_string('edit'),
                $url,
    ...
    public function restore_instance(restore_enrolments_structure_step $step, stdClass $data, $course, $oldid) {
        ...
        if ($merge and $instances = $DB->get_records('enrol', $merge, 'id')) {
            ...
            if (!empty($data->customint5) && !$step->get_task()->is_samesite()) {
                // Use some id that can not exist in order to prevent corplms_facetoface enrolment,
    ...
    public function restore_role_assignment($instance, $roleid, $userid, $contextid) {
        ...
        // we do not use component in manual or corplms_facetoface enrol.
 ...
 */
function enrol_corplms_facetoface_statushandler($newstatus) {
    $status = enrol_corplms_facetoface_enrol_on_approval($newstatus);
    $status = enrol_corplms_facetoface_unenrol_on_removal($newstatus) && $status;

    return $status;
}

function enrol_corplms_facetoface_deletedhandler($info) {
    global $DB;

    $status = true;
    if ($info->modulename == 'facetoface') { // Facetoface activity deleted.
        // Find all enrolment instances in this course of type corplms_facetoface with 'unenrol when removed' enabled.
        $enrols = $DB->get_records('enrol', array('enrol' => 'corplms_facetoface', 'courseid' => $info->courseid,
            enrol_corplms_facetoface_plugin::SETTING_UNENROLWHENREMOVED => 1));
        foreach ($enrols as $enrolinst) {
            if (!$userids = $DB->get_fieldset_select('user_enrolments', 'userid', 'enrolid = ?', array($enrolinst->id))) {
                continue;
            }
            $status = enrol_corplms_facetoface_unenrol_if_no_signups($enrolinst, $userids) && $status;
        }
    }
    return $status;
}

function enrol_corplms_facetoface_enrol_on_approval($newstatus) {
    global $DB;
    ...
    JOIN {enrol_corplms_f2f_pending} efd ON (efd.enrolid = enr.id)
    WHERE snp.id = :signupid
    AND enr.enrol = :corplms_facetoface
    AND efd.userid = snp.userid
    ";
    $params = array(
        'signupid' => $newstatus->signupid,
        'corplms_facetoface' => 'corplms_facetoface',
    );
    if (!$efdrec = $DB->get_record_sql($sql, $params)) {
        return true;
    }

    $DB->delete_records('enrol_corplms_f2f_pending', array('id' => $efdrec->id));
    ...
    // Enrol.
    if (!$enrol = $DB->get_record('enrol', array('id' => $efdrec->enrolid, 'enrol' => 'corplms_facetoface'))) {
        return false;
    }
    ...
    $corplms_facetoface = new enrol_corplms_facetoface_plugin();
    $corplms_facetoface->enrol_user($enrol, $efdrec->userid, $enrol->roleid, $timestart, $timeend);

    return true;
}

/**
 * If the enrol_corplms_facetoface instance is set to unenrol users on removal:
 ...
 * @return bool
 */
function enrol_corplms_facetoface_unenrol_on_removal($newstatus) {
    global $DB;
    ...
    // Look to see if the user is enroled via 'corplms_facetoface' and 'unenrol when removed' is enabled.
    $sql = "SELECT e.*, su.userid
              ...
              JOIN {enrol} e ON e.courseid = f.course AND e.enrol = :enrol
                   AND e.".enrol_corplms_facetoface_plugin::SETTING_UNENROLWHENREMOVED." = 1
              JOIN {user_enrolments} ue ON ue.enrolid = e.id AND ue.userid = su.userid
             WHERE su.id = :signupid";
    $params = array('signupid' => $newstatus->signupid, 'enrol' => 'corplms_facetoface');
    $enrolinst = $DB->get_record_sql($sql, $params);

    if (!$enrolinst) {
        return true; // User not enroled via 'corplms_facetoface' OR 'unenrol when removed' not enabled for this instance.
    }

    // Check to see if the user is still signed up for any sessions in this course.
    enrol_corplms_facetoface_unenrol_if_no_signups($enrolinst, $enrolinst->userid);
 ...
 * @param object[]|object $userids user(s) who are enroled in the course via corplms_facetoface plugin
 * @return bool
 */
function enrol_corplms_facetoface_unenrol_if_no_signups($enrolinst, $userids) {
    global $DB;
    ...
    $enrol = enrol_get_plugin('corplms_facetoface');
 ...
 * @param enrol_corplms_facetoface_plugin $corplms_facetoface
 * @param int $facetofaceid
 * @return object|null
 */
function enrol_corplms_facetoface_find_best_session($corplms_facetoface, $facetofaceid) {
    $facetofacesessions = $corplms_facetoface->get_enrolable_sessions(null, null, $facetofaceid, true);
 ...
 * @return array
 */
function enrol_corplms_facetoface_get_sessions_to_autoenrol($corplms_facetoface, $course, $facetofaces, $user = null) {
    global $USER;
    $sessions = array();
    ...
    $autosessions = $corplms_facetoface->get_enrolable_sessions($course->id, $user, null, true);
    ...
    foreach ($sessionstochoosefrom as $facetofaceid => $facetofacesessions) {
        ...
        $best = enrol_corplms_facetoface_find_best_session($corplms_facetoface, $facetofaceid);
