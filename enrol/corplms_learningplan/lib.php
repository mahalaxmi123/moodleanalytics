...
class enrol_corplms_learningplan_plugin extends enrol_plugin {
    ...
    public function get_newinstance_link($courseid) {
        ...
        if ($DB->record_exists('enrol', array('courseid' => $courseid, 'enrol' => 'corplms_learningplan'))) {
            return NULL;
        }

        return new moodle_url('/enrol/corplms_learningplan/addinstance.php', array('sesskey' => sesskey(), 'id' => $courseid));
    }
    ...
    /**
     * Add new instance of enrol_corplms_learningplan plugin.
     * @param object $course
    ...
    public function enrol_page_hook(stdClass $instance) {
        ...
        if ($this->is_user_approved($instance->courseid)) {
            ...
            corplms_set_notification($OUTPUT->container(get_string('nowenrolled', 'enrol_corplms_learningplan', $course->fullname), 'plan_box'), null, array('class' => 'notifysuccess'));
        } else {
            // this isn't an approved course in their learning plan or learning plan isn't approved
            $link = $OUTPUT->action_link(new moodle_url('/corplms/plan/index.php', array('userid' => $USER->id)), get_string('learningplan', 'enrol_corplms_learningplan'));
            $form = get_string('notpermitted', 'enrol_corplms_learningplan', $link);

            if (!empty($course->guest)) {
                $destination = new moodle_url('/course/view.php', array('id' => $course->id));
                $form .= get_string('guestaccess', 'enrol_corplms_learningplan', $destination);
            } else {
    ...
    public function get_user_enrolment_actions(course_enrolment_manager $manager, $ue) {
        ...
        $params['ue'] = $ue->id;
        if ($this->allow_unenrol($instance) && has_capability("enrol/corplms_learningplan:unenrol", $context)) {
