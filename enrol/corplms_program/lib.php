...
class enrol_corplms_program_plugin extends enrol_plugin {
    ...
    public function get_newinstance_link($courseid) {
        ...
        if ($DB->record_exists('enrol', array('courseid' => $courseid, 'enrol' => 'corplms_program'))) {
            return NULL;
        }

        return new moodle_url('/enrol/corplms_program/addinstance.php', array('sesskey' => sesskey(), 'id' => $courseid));
    }
    ...
    /**
     * Add new instance of enrol_corplms_program plugin.
     * @param object $course
    ...
    public function get_instance_for_course($courseid) {
        global $DB;
        return $DB->get_record('enrol', array('enrol' => 'corplms_program', 'courseid' => $courseid));
    }
            ...
            if ($result->enroled) {
                ...
                if ($result->notify) {
                    ...
                    if ($courseformat->get_format() == 'singleactivity') {
                        ...
                        $a->url = $viewurl->out();
                        corplms_set_notification($OUTPUT->container(get_string('nowenrolledcontinue', 'enrol_corplms_program', $a), 'plan_box'), null, array('class' => 'notifysuccess'));
                    } else {
                        corplms_set_notification($OUTPUT->container(get_string('nowenrolled', 'enrol_corplms_program', $a), 'plan_box'), null, array('class' => 'notifysuccess'));
                    }
                }
    ...
    public function get_user_enrolment_actions(course_enrolment_manager $manager, $ue) {
        ...
        $params['ue'] = $ue->id;
        if ($this->allow_unenrol($instance) && has_capability("enrol/corplms_program:unenrol", $context)) {
    ...
    public function restore_instance(restore_enrolments_structure_step $step, stdClass $data, $course, $oldid) {
        global $DB;
        // There is only one corplms_program enrol instance allowed per course.
 ...
 */
function enrol_corplms_program_supports($feature) {
    switch($feature) {
