...
if ($frm = data_submitted()) {
    ...
    $attendance = corplms_search_for_value($attendees, 'statuscode', CORPLMS_SEARCH_OP_GREATER_THAN, MDL_F2F_STATUS_REQUESTED);
    $takeattendance = ($attendance && $session->datetimeknown && $sessionstarted) ? 1 : 0;

    // Set approval required for the new users
    $requireapproval = (corplms_search_for_value($attendees, 'statuscode', CORPLMS_SEARCH_OP_EQUAL, MDL_F2F_STATUS_REQUESTED)) ? 1 : 0;
...
if ($searchtext !== '') {   // Search for a subset of remaining users
    ...
    $keywords = corplms_search_parse_keywords($searchtext);
    list($searchwhere, $searchparams) = corplms_search_get_keyword_where_clause($keywords, $fields);
