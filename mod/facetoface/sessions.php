...
$PAGE->requires->string_for_js('error:addpdroom-dialognotselected', 'corplms_core');
...
$jsmodule = array(
    'name' => 'corplms_f2f_room',
    'fullpath' => '/mod/facetoface/sessions.js',
    'requires' => array('json', 'corplms_core'));
$PAGE->requires->js_init_call('M.corplms_f2f_room.init', array($jsconfig), false, $jsmodule);
...
if (isset($session)) {
    ...
    $defaulttimezone = corplms_get_clean_timezone();
}
