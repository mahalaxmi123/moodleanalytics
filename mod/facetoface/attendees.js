...
M.corplms_f2f_attendees = M.corplms_f2f_attendees || {
    ...
    init: function(Y, args){
        ...
        if (typeof $ === 'undefined') {
            throw new Error('M.corplms_f2f_attendees.init()-> jQuery dependency required for this module to function.');
        }

        var notsetoption = M.corplms_f2f_attendees.config.notsetop.toString();

        corplmsDialog_handler_addremoveattendees = function() {};
        corplmsDialog_handler_addremoveattendees.prototype = new corplmsDialog_handler();

        /**
         * Upload background page when closing dialog
         */
        corplmsDialog_handler_addremoveattendees.prototype.submit = function() {

            var handler = this;
            var url = M.cfg.wwwroot + '/mod/facetoface/editattendees.php?s=' + M.corplms_f2f_attendees.config.sessionid + '&action=' + M.corplms_f2f_attendees.config.action + '&onlycontent=1&save=1';
        ...
        corplmsDialog_handler_form.prototype._updatePage = function(response) {
            ...
            if (M.corplms_f2f_attendees.config.approvalreqd == "1") {
                if ($('<div></div>').append(newtable).find('div.addedattendees').length > 0) {
                    ...
                    if (tab.length > 0) {
                        ...
                        tab.parent('a').attr("href", M.cfg.wwwroot + '/mod/facetoface/attendees.php?s=' + M.corplms_f2f_attendees.config.sessionid + '&action=approvalrequired');
            ...
            M.corplms_f2f_attendees.attachCustomClickEvents();

            // Close dialog
            this._dialog.hide();
        };

        corplmsDialog_handler_addremoveattendees.prototype._updatePage = function(response) {
            // Get all root elements in response
            var newtable = $(response);
            ...
            if (waitlisttab.length > 0) {
                if (($('input[name=waitlist]').val() == 1 && $('input[name=attendees]').val()) || $('input[name=waitlisteveryone]').val() == 1) {
                    ...
                    waitlisttab.parent('a').attr("href", M.cfg.wwwroot + '/mod/facetoface/attendees.php?s=' +
                        M.corplms_f2f_attendees.config.sessionid + '&action=waitlist');
                } else {
            ...
            if (cancellationtab.length > 0) {
                if ($('input[name=removedusers]').val()) {
                    ...
                    cancellationtab.parent('a').attr("href", M.cfg.wwwroot + '/mod/facetoface/attendees.php?s=' +
                        M.corplms_f2f_attendees.config.sessionid + '&action=cancellations');
                } else {
            ...
            if (takeattendancetab.length > 0) {
                if ($('input[name=takeattendance]').val() == 1) {
                    ...
                    takeattendancetab.parent('a').attr("href", M.cfg.wwwroot + '/mod/facetoface/attendees.php?s=' +
                        M.corplms_f2f_attendees.config.sessionid + '&action=takeattendance');
                } else {
            ...
            if (approvalrequiredtab.length > 0) {
                if (M.corplms_f2f_attendees.config.approvalreqd == "1" && $('input[name=requireapproval]').val() == 1) {
                    ...
                    approvalrequiredtab.parent('a').attr("href", M.cfg.wwwroot + '/mod/facetoface/attendees.php?s=' +
                        M.corplms_f2f_attendees.config.sessionid + '&action=approvalrequired');
                } else {
        ...
        // Add/remove dialog
        (function() {
            var handler = new corplmsDialog_handler_addremoveattendees();
            var name = 'addremove';
            ...
            corplmsDialogs[name] = new corplmsDialog(
                ...
                M.cfg.wwwroot + '/mod/facetoface/editattendees.php?s=' + M.corplms_f2f_attendees.config.sessionid + '&clear=1',
                handler
                );
        })();
        ...
        (function() {
            ...
            corplmsDialogs[name] = new corplmsDialog(
                    ...
                    M.cfg.wwwroot + '/mod/facetoface/bulkadd_attendees.php?s=' + M.corplms_f2f_attendees.config.sessionid + '&type=file&dialog=1',
                    handler
            );
        })();

        (function() {
            var handler = new corplmsDialog_handler_form();
            ...
            corplmsDialogs[name] = new corplmsDialog(
                ...
                M.cfg.wwwroot + '/mod/facetoface/bulkadd_attendees.php?s=' + M.corplms_f2f_attendees.config.sessionid + '&type=input&dialog=1',
                handler
                );
        })();
        ...
        (function() {
            ...
            corplmsDialogs[name] = new corplmsDialog(
                ...
                M.cfg.wwwroot + '/mod/facetoface/bulkadd_results.php?s=' + M.corplms_f2f_attendees.config.sessionid,
                handler
                );
        })();
        ...
        $('select#menubulk_select').change(function() {
            ...
            switch(selected) {
                case M.corplms_f2f_attendees.config.selectall.toString():
                    $('.selectedcheckboxes').prop('checked', true);
                    break;
                case M.corplms_f2f_attendees.config.selectnone.toString():
                    $('.selectedcheckboxes').prop('checked', false);
                    break;
                case M.corplms_f2f_attendees.config.selectset.toString():
                    mark_set_unset(notsetoption, 'NE');
                    break;
                case M.corplms_f2f_attendees.config.selectnotset.toString():
        ...
        */
        ...
        $(document).on('change', 'select#menuf2f-actions', function() {
            ...
            var data = {
                submitbutton: "1",
                ajax: "1",
                sesskey: M.corplms_f2f_attendees.config.sesskey
            };
            ...
            if (current.substr(0, 6) == "export") {
                var url = M.cfg.wwwroot + '/mod/facetoface/attendees.php?s=' + M.corplms_f2f_attendees.config.sessionid + '&action=' + M.corplms_f2f_attendees.config.action + '&download=';
        ...
        function update_waitlist(action, updateusers, checked) {
            ...
            function do_post() {
                Y.io(M.cfg.wwwroot + '/mod/facetoface/updatewaitlist.php', {
                    data: {
                        courseid: M.corplms_f2f_attendees.config.courseid,
                        sessionid: M.corplms_f2f_attendees.config.sessionid,
                        context: this,
                        action: action,
