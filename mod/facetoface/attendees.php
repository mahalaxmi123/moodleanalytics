...
if ($action == 'cancellations') {
    ...
    // Get list of actions
    $actions = array(
        'exportxls'     => get_string('exportxls', 'corplms_reportbuilder'),
        'exportods'     => get_string('exportods', 'corplms_reportbuilder'),
        'exportcsv'     => get_string('exportcsv', 'corplms_reportbuilder')
    );
        ...
        if ($mform->is_cancelled()) {
            ...
            foreach ($recipients as $recipient) {
                ...
                if (email_to_user($recipient, $facetofaceuser, $data->subject, $bodyplain, $body) === true) {
                    ...
                    if (empty($data->cc_managers) || !$manager = corplms_get_manager($recipient->id, $recipient->positiontype)) {
                        continue;
                    }
            ...
            if ($emailerrors) {
                ...
                corplms_set_notification($message);
            }
...
if (!$onlycontent) {
    ...
    if ($action == 'messageusers') {
        ...
        $jsmodule = array(
            'name' => 'corplms_f2f_attendees_message',
            'fullpath' => '/mod/facetoface/attendees_messaging.js',
            'requires' => array('json', 'corplms_core'));

        $PAGE->requires->js_init_call('M.corplms_f2f_attendees_messaging.init', $args, false, $jsmodule);
    } else {
        $jsmodule = array(
            'name' => 'corplms_f2f_attendees',
            'fullpath' => '/mod/facetoface/attendees.js',
        ...
        $PAGE->requires->js_init_call('M.corplms_f2f_attendees.init', $args, false, $jsmodule);
    }
...
if ($show_table) {
    ...
    if (empty($rows)) {
        ...
        $table = new corplms_table('facetoface-attendees');
