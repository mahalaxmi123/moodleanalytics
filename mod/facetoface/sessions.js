...
M.corplms_f2f_room = M.corplms_f2f_room || {
    ...
    init: function(Y, config){
        ...
        if (typeof $ === 'undefined') {
            throw new Error('M.corplms_f2f_room.init()-> jQuery dependency required for this module to function.');
        }

        var url = M.cfg.wwwroot+'/mod/facetoface/room/ajax/';

        var handler = new corplmsDialog_handler_addpdroom();
        ...
        corplmsDialogs['addpdroom'] = new corplmsDialog(
                'addpdroom-dialog',
                'show-addpdroom-dialog',
                {
                    buttons: buttonsObj,
                    title: '<h2>' + M.util.get_string('chooseroom', 'facetoface') + M.corplms_f2f_room.config.display_selected_item + '</h2>'
                },
                url + 'sessionrooms.php?sessionid=' + M.corplms_f2f_room.config.sessionid + '&facetofaceid=' + M.corplms_f2f_room.config.facetofaceid,
                handler
        );
...
corplmsDialog_handler_addpdroom = function() {};
corplmsDialog_handler_addpdroom.prototype = new corplmsDialog_handler_treeview_singleselect('pdroomid', 'pdroom');

corplmsDialog_handler_addpdroom.prototype.setup_delete = function() {

    this.deletable = true;
    var textel = $('#'+this.text_element_id);
    var idel = $('input[name='+this.value_element_name+']');
    var deletebutton = $('<span class="dialog-singleselect-deletable">'+M.util.get_string('delete', 'corplms_core')+'</span>');
    var handler = this;
...
corplmsDialog_handler_addpdroom.prototype._open = function(alternative_url) {
    var datecount = 0;
    ...
    this._dialog.default_url = M.cfg.wwwroot+'/mod/facetoface/room/ajax/sessionrooms.php?sessionid='+M.corplms_f2f_room.config.sessionid+'&timeslots='+timeslots+'&facetofaceid='+M.corplms_f2f_room.config.facetofaceid;
}

corplmsDialog_handler_addpdroom.prototype.first_load = function() {
    // Call parent function
    corplmsDialog_handler_treeview_singleselect.prototype.first_load.call(this);
}

corplmsDialog_handler_addpdroom.prototype.every_load = function() {
    // Call parent function
    corplmsDialog_handler_treeview_singleselect.prototype.every_load.call(this);
...
corplmsDialog_handler_addpdroom.prototype.external_function = function() {
