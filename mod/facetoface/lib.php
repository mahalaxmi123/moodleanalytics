...
function facetoface_notify_under_capacity() {
    ...
    foreach ($tocheck as $session) {
        ...
        $eventdata = (object)array(
            ...
            'msgtype' => CORPLMS_MSG_TYPE_FACE2FACE,
            'msgstatus' => CORPLMS_MSG_STATUS_NOTOK,
            'urgency' => CORPLMS_MSG_URGENCY_NORMAL,
            'sendmail' => CORPLMS_MSG_EMAIL_YES,
        );
...
function facetoface_is_session_in_progress($session, $timenow) {
    ...
    $startedsessions = corplms_search_for_value($session->sessiondates, 'timestart', CORPLMS_SEARCH_OP_LESS_THAN, $timenow);
    $unfinishedsessions = corplms_search_for_value($session->sessiondates, 'timefinish', CORPLMS_SEARCH_OP_GREATER_THAN, $timenow);
...
class facetoface_event_handler {
     ...
     * when there is limited capacity.
     *
     * @param \corplms_core\event\user_suspended $event
     */
    public static function user_suspended(\corplms_core\event\user_suspended $event) {
        global $DB;
...
function facetoface_can_reserve_or_allocate($facetoface, $sessions, $context, $managerid = null) {
    ...
    if (!corplms_get_staff($managerid)) {
...
function facetoface_get_staff_to_allocate($facetoface, $session, $managerid = null) {
    ...
    if (!$staff = corplms_get_staff($managerid)) {
        return $ret;
    }
